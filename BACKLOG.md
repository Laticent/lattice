...
- [#2305](https://github.com/Laticent/lattice/issues/2305) split(envelope): a split drops the `finish` class, so a cover and every carousel page in a `finish:` deck render with no finish at all
- [#2308](https://github.com/Laticent/lattice/issues/2308) a11y(subtitle): ANY paragraph containing exactly one inline-code span is styled as a subtitle — `:only-child` ignores text nodes
