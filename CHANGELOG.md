...
- **Image-set export: the CLI now re-bakes Mermaid diagrams print-safe for a cross-scheme `print` look.**
  ...
  slides. Charts already recolored for any look. Light/dark *cross-scheme* diagram looks on the CLI
  are still not re-baked (that needs the Studio's full second render); the CLI warns and points there.
