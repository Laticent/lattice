...
| [`agent/`](./agent) | The **LLM agent kit** — a per-component reference, the catalogs, and the authoring primer the Studio chat uses, for a model or coding agent that has to write a Lattice deck. **Start at [`agent/README.md`](./agent/README.md)** — it routes you by task and costs each path in tokens. Every folder inside has its own README. Includes a runnable, dependency-free deck checker (`agent/review/check.mjs`).|
