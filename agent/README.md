...
Paste [`paste/lattice-instructions-solo.md`](./paste/lattice-instructions-solo.md) (9,004 characters)
...
| [`components/`](./components/) | The deep reference — all 70 layouts, one file each |
...
| the authoring path: canon → rules → picker | **~8.2k** once |
| …then one `components/<name>.md` per layout you use | + ~2.0k each |
| drafting a whole deck in one pass: canon → primer | ~23k |
| creating a theme, component, finish or lens | 3.2k–5.6k each |
...
**A Marp-rendered deck needs the three runtime `<script>` tags at the bottom of the file.**
