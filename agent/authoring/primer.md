...
> ~21k. This is for writing a whole deck in one pass.
...
Variants: qr, index, spectrum (append to the class, e.g. `closing qr`).
...
### topic — Topic anchor inside a section. A horizontal cut: the topic above, the section's topics as a scale below.
Variants: fact (append to the class, e.g. `topic fact`).
- `eyebrow` (optional): The section this topic sits in, as inline code. Mono caps, top-left. Usually the divider's own label — `Section 02 · Unit economics`.
- `heading`: The topic's name. Write it as a short noun phrase (`Payback`, `Cost to win`) — two to four words. It is also the label this topic contributes to every sibling slide's derived track, so a full sentence reads as a headline on this slide and o
- `claim` (optional): One sentence of context under the name. In the default it supports the heading; under `fact` it becomes the slide's display line.
- `track` (optional): DERIVED, not authored — `lib/transformers/topic-track.js` builds it from the headings of every `topic` slide in this section and lights the current one. Override it with the `_track` DIRECTIVE, never with a list: `<!-- _track: Cost to win |
- `basis` (optional): What the claim rests on — source, period, method. A trailing inline-code paragraph on a foot rule. Belongs with `fact`, where the slide asserts something; on the default the claim is supporting text and rarely needs provenance.
````
<!-- _class: topic -->
...
Budget: ≤ 6 items. Keep it tight; push detail to speaker notes.
...
- `items`: Each item prefixed with a state marker — [x] done, [-] partial, [!] failed, [?] unknown, [ ] to do, or [/] skipped (struck through). Plain text follows the marker; an optional trailing inline-code pill floats right as a status tag.
````
<!-- _class: checklist -->
...
````
`list numbered` is authored differently — Accent counters on the takeaway rows; add a nested gloss for criteria.:
````
...
Variants: def, metric, spec, register, rule, solid, stacked, outline, fit-name, fit-body, fit-meta, flex-name, flex-meta, fixed (append to the class, e.g. `list-tabular def`).
...
Variants: lead, bench, sides, bio (append to the class, e.g. `team-profile lead`).
Budget: ≤ 12 items, ≤ ~12 words each — one clause on what this person owns, not a paragraph — and the count includes the name and the image reference beside it, so about four of the allowance is fixed cost before a word of prose. Keep it tight; push detail to speaker notes.
- `eyebrow` (optional): Optional kicker above the headline — a short label in backticks, e.g. `Your account team`.
- `title` (optional): Optional headline above the roster. A claim earns its place ('Six people own this program end to end'); a bare label ('The team') does not.
- `people`: One top-level bullet per person, and the bullet IS their name. Everything about them nests under it.
- `portrait` (optional): The person's photo, nested under their name as `- ![](photo.jpg)`. Leave the alt empty — the name beside it is the accessible label. Omit it entirely and the engine draws a monogram from the initials instead.
- `role` (optional): The one backticked nested line is the role, set as a small tracked label under the name — `` `Head of Delivery` ``. Deliberately not a pill: a boxed chip under every one of twelve faces competes with the portraits it is meant to caption.
- `note` (optional): Any remaining plain nested line is the note: one short line on what this person owns.
- `side` (optional): `sides` only. Two `### ` subheadings, each followed by its own list, label the two rosters ('Your team' / 'Our team').
````
<!-- _class: team-profile -->
...
- Third person
  ...
  - One line on what they own.
````
`team-profile sides` is authored differently — Two labeled rosters facing each other across the slide.:
````
...
Budget: ≤ 4 items, ≤ ~10 words each — a short label per quadrant cell. Keep it tight; push detail to speaker notes.
...
Budget: ≤ 4 items. Keep it tight; push detail to speaker notes.
...
- `features`: Feature rows, each led by a state marker, with the same meaning it has in every layout: `[x]` included (green check), `[-]` limited (dash), `[/]` not included (muted, struck through — not on this plan), `[ ]` coming (open ring), `[?]` ask s
````
<!-- _class: pricing -->
...
Budget: ≤ 2 items, ≤ ~14 words each — a terse point per line. Keep it tight; push detail to speaker notes.
...
- `title`: Slide heading framing what the table shows.
- `table`: Markdown table with a header row and 2+ data rows. Column alignment is markdown's own (`:---`, `:---:`, `---:`).
````
<!-- _class: table -->
...
- `options`: One outer li per option, lead with **Option name.**. Then one inner li per criterion, each led by a state marker ([x]/[-]/[!]/[?]/[ ]/[/]) followed by a badge label of AT MOST TWO WORDS. Criteria are shared across every option, in the same 
...
Budget: ≤ 5 items, ≤ ~14 words each — one sentence per step, not a paragraph. The strip gives every card the tallest card's height, so one long step tightens the whole row, and a body that outgrows the card spills past its bottom edge and is clipped at the stage. Keep it tight; push detail to speaker notes.
...
- `scene`: The scene's poster still, authored as an INLINE `<svg>` under the heading. Its `var(--token)` fills recolor with the theme (it must be inline, not a background-image).
...
Variants: row, grouped, diverging (append to the class, e.g. `bar row`).
- `title`: Slide heading — name the takeaway, not the chart ('Growth is concentrated in EMEA', not 'Revenue by region').
- `categories`: One li per category, in the order they should read. Lead text is the category name, the trailing inline-code pill is the value — `EMEA \`4.2\``. Commas, currency symbols, units and magnitude suffixes are all read: `$4.2M` is 4 200 000 and t
- `series` (optional): Nested items whose trailing pill is a NUMBER are the grouped form: each category gets one bar per series, side by side, and a key names them. Two to four series; past four the group gets too dense to compare across categories.
- `detail` (optional): Nested items whose trailing pill is NOT a number (or that carry no pill) are per-category detail, not data. They render nowhere on the chart face: they drive the Present-mode reveal popover and fold into the slide's speaker note, so a chart
````
<!-- _class: bar -->
...
- Fourth `31`
````
`bar grouped` is authored differently — Two to four series side by side in each category, selected by nesting numeric children under each category.:
````
<!-- _class: bar grouped -->
...
Variants: shared-axis, own-axis (append to the class, e.g. `bullet shared-axis`).
- `title`: Slide heading — name the verdict ('Three of five are behind plan'), not the chart type.
- `eyebrow` (optional): Optional eyebrow caption above the heading.
- `subtitle` (optional): Optional plain subtitle after the heading.
- `rows`: One li per KPI. Lead text is the KPI name, then two trailing inline-code pills — the measure first, the target second: - New ARR `4.2M` `5.0M`. Magnitude suffixes scale (`4.2M` is 4 200 000), the unit affix carries onto the axis, and both p
- `range` (optional): Optional nested sublist, each child a NAME plus a value pill. `Target` (or `Plan`/`Goal`) overrides the second pill; `Actual` (or `Measure`) overrides the first; each `Band` (or `Range`) adds one internal cut point, so two Bands make three 
````
<!-- _class: bullet -->
...
- `tasks`: Outer `li` per workstream LANE; nested bullets per TASK. A task is its name followed by trailing inline-code tokens, in any order. **Span or point:** `Q1..Q3` draws a bar (inclusive — `Q1..Q2` covers both quarters); a single point like `Q4`
...
Variants: scale (append to the class, e.g. `heatmap scale`).
- `title`: Slide heading — name what the grid shows ('Retention decays fastest in month two'), not the chart type.
- `eyebrow` (optional): Optional kicker above the title — the dataset and its period.
- `rows`: One table row per ROW of the matrix, leading with its label. The header row names the columns; a blank cell is a crossing nobody measured.
````
<!-- _class: heatmap -->
...
- `key` (optional): OPTIONAL label set renaming the mood scale's two POLES: `[{1, Friction}, {5, Flow}]`, one inline-code span alone in its paragraph, outside the journey list. Only `1` and `5` are keyable — the steps between them show their number, which is t
...
Variants: area, stacked-area, step (append to the class, e.g. `line area`).
- `title`: Slide heading — name the movement, not the chart (‘Renewals recovered in the second half’, not ‘Revenue line chart’).
- `points`: One li per point on the category axis, in order — the authored order IS the axis order and is never sorted. Lead text is the category label, and a trailing inline-code pill is its value: `Q1 2026 \`4.2\``. For several series, nest one li pe
- `detail` (optional): Optional nested bullet under a CATEGORY whose trailing pill is not a number — that is the whole disambiguation rule: a nested item ending in a numeric pill is a data point, anything else is detail. Drives the Present-mode reveal for that ca
````
<!-- _class: line -->
...
- Q4 `26`
````
`line stacked-area` is authored differently — Composition over time: the series stack to a total, in the order they were authored, largest and steadiest at the bottom.:
````
...
- `eyebrow` (optional): OPTIONAL axis labels: ONE bracketed list in its own paragraph ABOVE the table — `` `[Wider reach, Deeper cognition]` ``. The first member names the column (reach) axis and renders centered above the grid; the second names the row (depth) ax
...
- `key` (optional): OPTIONAL label set renaming the cell key: `[{[-], within reach}, {[ ], out of band}]`, one bracketed list in its own paragraph BELOW the table. POSITION is what distinguishes it from the axis — the two are the same shape, so a list above th
- `legend` (optional): Optional single trailing paragraph, doubling as the chart caption. It no longer has to say what the cell shapes mean — the key under the grid names them — so use it for the caveat the key cannot carry (how the placements were derived, what 
````
<!-- _class: matrix-grid -->
...
- `eyebrow` (optional): Optional eyebrow caption above the heading.
- `subtitle` (optional): Optional plain subtitle after the heading.
- `rows`: One li per item: label text then trailing inline-code pills — percent first, optional status second, e.g. - Adoption `68%` `at-risk`. Status vocabulary — the shared CHART_STATUS ten: on-track / done / live / at-risk / warn / blocked / fail 
````
<!-- _class: progress -->
...
- `axes` (optional): Optional axes, as ONE bracketed list in its own paragraph ABOVE the chart list — x first, then y: `[{Effort, 0..10}, {Reach, 0..100}]`. Each member is `{name, min..max, threshold}`; the domain and threshold are optional and derive from the 
...
- `key` (optional): OPTIONAL label set renaming the status key: `[{[x], Enacted}, {[-], In committee}]`, one inline-code span alone in its paragraph. The key is the MARKER you already type in a cell (`[x]` `[-]` `[!]` `[?]` `[ ]` `[/]`), brackets included — a 
````
<!-- _class: roadmap -->
...
The key under the grid names each marker the cells carry.
````

### scatter — An XY plot with real units on both axes — one dot per entity, showing how two measures relate.
Variants: bubble, trend (append to the class, e.g. `scatter bubble`).
- `title`: Slide heading — name the relationship the dots show, not the chart type.
- `axes`: The axis captions, as ONE bracketed list in its own paragraph ABOVE the points — x first, then y; a THIRD member names the bubble size measure. The paragraph is consumed and painted on the axes, so it never prints twice. Quotes are optional
- `points`: One li per entity: the name, then TWO trailing value pills — ``Atlas `$420k` `18%` ``. The first pill is x, the second y; a third sizes the dot under `bubble`. Magnitude suffixes are scale (`1.2M` is 1 200 000) and the affix every value agr
- `detail` (optional): Optional nested sublist under a point. Drives two surfaces from one source (shared with pie/funnel/quadrant via the chart-family mark-detail substrate): the Present-mode reveal popover keyed on the dot's `data-mark`, and the PDF speaker not
````
<!-- _class: scatter -->
...
Variants: dumbbell, signal (append to the class, e.g. `slope dumbbell`).
- `title`: Slide heading — name the change, not the chart ('Two suppliers swapped places on unit cost', not 'Slope chart').
- `entities`: One li per entity, and the entity's own name is the lead text. It carries NO value of its own — the values live in the nested sublist, so a slope is always authored two levels deep.
- `points`: Two nested items per entity: lead text = the point's name, trailing inline-code = its value — `2024 \`31%\``. The point NAMES become the two column headers, so every entity must name its points the same way; name them differently and the ch
- `emphasis` (optional): An optional status pill on the ENTITY's own lead — `- Northwind \`fail\`` — from the chart family's status vocabulary (`on-track` `done` `live` `at-risk` `warn` `blocked` `fail` `pilot` `decision` `deferred`). Mark one or two and every unma
- `detail` (optional): A nested item with NO numeric pill is per-mark detail, not a data point. It drives the Present-mode reveal popover and folds into the slide's speaker note (`Atlas 12 to 19: Won the Rhodes contract`); the printed chart is byte-identical with
````
<!-- _class: slope -->
...
Variants: share, row (append to the class, e.g. `stacked-bar share`).
- `title`: Slide heading — name what the decomposition shows (‘Services is carrying the growth’), not the chart type.
- `categories`: One li per category, in the order they should read left to right (or top to bottom under `row`). Lead text is the category label. Optionally a trailing inline-code value — the authored TOTAL, printed instead of the computed sum when the two
- `parts`: The nested sublist under each category: one li per part, lead text = the series name, trailing inline-code = its value (`Product \`2.4\``). Series order is taken from the FIRST category that names each one and is then held identical in ever
- `detail` (optional): A nested item whose trailing inline-code pill is NOT a number is per-category detail, not a part — the chart-family mark-detail payload. It drives the Present-mode reveal on the whole bar and folds into the slide's speaker note; it paints n
````
<!-- _class: stacked-bar -->
...
Variants: lr, tb, inline, curved (append to the class, e.g. `state-chart lr`).
- `title`: Slide heading framing the state machine.
- `eyebrow` (optional): Optional eyebrow naming the machine or domain.
- `states`: One li per state. Index is the stable ref. Trailing inline code is a closed metadata vocabulary: `start`, `end`, or one of the chart-status keywords (on-track, at-risk, blocked, done, live, decision, deferred, warn, pilot, fail — case is ig
...
- `tint` (optional): Optional `:::token` naming a THEME TOKEN to paint with — `:::state-fail-hue`, never a color literal. On a state it tints the node's gradient and stroke; on a transition it tints the line and its arrowhead. A transition takes an optional sec
````
<!-- _class: state-chart -->
...
````
`state-chart tb` is authored differently — States flow top to bottom, whatever the stage.:
````
...
- `events`: Ordered list (numbered). One li per event: a leading inline-code date pill, then the title, then an optional trailing inline-code status pill, then nested body bullets — e.g. 1. `2025 Q1` Framework approved `decision`. Status vocabulary — t
````
...
Variants: zoom (append to the class, e.g. `waterfall zoom`).
- `title`: Slide heading — name the movement, not the chart ('We ended the year 2.2M below plan').
- `steps`: One li per bar, in walk order. Lead text = label, trailing inline-code = the value. THE SIGN IS THE SYNTAX: a value written with an explicit `+` or `-` is a signed step and floats from the running total; a value written bare is a level and 
- `marker` (optional): An optional SECOND inline-code pill that overrides the sign rule for one bar — `total` (also `subtotal`, `sum`, `level`) anchors it to zero, `step` (also `delta`, `change`) floats it. Needed only for the two shapes the sign cannot express: 
- `detail` (optional): Optional nested sublist under a step. Drives two surfaces from one source, via the shared chart-family mark-detail substrate: the Present/Practice reveal popover on the bar, and the slide's speaker note in the static PDF. It renders NOWHERE
````
<!-- _class: waterfall -->
...
````
`math matrix` is authored differently — A matrix beside a legend of its shape, rank and what rows mean.:
````
<!-- _class: math matrix -->
...
- `key` (optional): OPTIONAL label set renaming the marker key: `[{[x], In force}, {[/], Not subject}]`, one inline-code span alone in its paragraph. The key is the MARKER you already type in a cell (`[x]` `[-]` `[!]` `[?]` `[ ]` `[/]`), brackets included — a 
- `legend` (optional): Optional trailing paragraph — what to take from the matrix, or a caveat about the placements. It no longer has to explain the markers: the key under the grid names them, derived from the ones the cells actually carry.
````
