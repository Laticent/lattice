...
> ~20k. This is for writing a whole deck in one pass.
...
Budget: ≤ 5 items, ≤ ~14 words each — one criterion with a short proof, not a spec. Keep it tight; push detail to speaker notes.
- `title`: Slide heading naming the framework.
...
- `rows`: One top-level list item per ROW. Its nested items are the columns, each `- Column `value``.
````
<!-- _class: heatmap -->
...
- `axes`: The axis captions, as TWO inline-code spans in ONE paragraph — x first, then y; a THIRD names the bubble size measure. The paragraph is consumed and painted on the axes, so it never prints twice. Two codes is the discriminator: a one-code p
