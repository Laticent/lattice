...
- [`list-tabular`](./list-tabular.md) — rows are reference entries, not owners
- [`cards-stack`](./cards-stack.md) — each item needs two sentences of body text
- [`list`](./list.md) — declared statements — the `principles` variant
- [`glossary`](./glossary.md) — the left column is a term, not an actor
