...
- [`progress`](./progress.md) — percent-complete with a status verdict and no target or range — HTML bars, so no marker and no band, but the row carries a status pill and a note the bullet has no room for
- [`bar`](./bar.md) — magnitudes compared against each other rather than against a plan
- [`big-number`](./big-number.md) — one figure against its target is the entire slide
- [`stats`](./stats.md) — a row of independent headline metrics with no shared scale
- [`gantt`](./gantt.md) — the rows are time-bound work, not measures against a threshold
