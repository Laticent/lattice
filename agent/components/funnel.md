...
- [`progress`](./progress.md) — independent metrics as labeled bars, not a narrowing pipeline
- [`stats`](./stats.md) — a row of headline figures with no drop-off relationship
- [`piechart`](./piechart.md) — parts of a single whole rather than sequential stages
- [`list-steps`](./list-steps.md) — the stages are a process to walk through, not values to compare
- [`big-number`](./big-number.md) — a single conversion rate is the whole story
