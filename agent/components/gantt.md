...
- **Status pills add a second channel.** Tint bars with `done` / `live` / `at-risk` / `blocked` to layer health onto schedule. The plan reads as both 'when' and 'how it's going' in one chart. Since the bars carry no status text, a swatch+label status key is emitted automatically below the chart for the statuses present — plus a `no status` chip when some task carries none, because `deferred` and an unstated task share the neutral ramp (that extra chip is dropped, with the bars still distinct, once the row would run past the frame — eight declared statuses in landscape, five in portrait). `deferred` is told apart by DRAWING rather than by hue: a hollow body and a dashed edge. That is a SEMANTIC cue — the shape a schedule uses for "planned, not committed" — and it is the one distinction no ramp-keyed channel can make, since `deferred` and an unstated task resolve to the same mute ramp and differ only in `data-s`. It is NOT this chart's accessibility channel: the engine textures filled marks and dashes stroked ones under `section.print` and the a11y themes (`engineering/textures.md`), and gantt is wired into neither.
...
- [`roadmap`](./roadmap.md) — phased grid of deliverables across workstreams without continuous spans
- [`kanban`](./kanban.md) — current state by stage rather than schedule by lane
- [`list-steps`](./list-steps.md) — sequential process with descriptive steps, no parallel lanes
