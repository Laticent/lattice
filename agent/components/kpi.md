...
- [`stats`](./stats.md) — metric row without targets or status pills
- [`big-number`](./big-number.md) — a single number is the whole argument
- [`split-panel`](./split-panel.md) — one KPI with a paragraph of supporting prose
- [`progress`](./progress.md) — completion percentages across parallel workstreams
- [`timeline-list`](./timeline-list.md) — milestones in time, not metrics at a moment
