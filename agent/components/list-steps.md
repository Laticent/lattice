...
- [`list-criteria`](./list-criteria.md) — gating requirements rather than a sequence of actions
- [`split-panel`](./split-panel.md) — phase label + heading on the left, steps on the right
- [`roadmap`](./roadmap.md) — phased grid across multiple workstreams
- [`list`](./list.md) — tenets or values (the `principles` variant) rather than a sequence
- [`funnel`](./funnel.md) — a value-driven funnel with conversion percentages, rather than the qualitative `converge` variant
