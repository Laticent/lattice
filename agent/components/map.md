...
- [`progress`](./progress.md) — the regions are really a ranking — labeled bars compare magnitudes faster than shades
- [`stats`](./stats.md) — a few headline figures with no geography to place them on
- [`piechart`](./piechart.md) — regional shares of a single whole rather than a value per place
- [`image`](./image.md) — the geography needs detail (counties, cities, routes) the basemap can't draw
