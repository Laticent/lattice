...
- **Assuming a math slide stays whole at portrait, story or mobile. It does not: since #2136 `math` splits on its rendered STRUCTURE, so a hero equation with a five-symbol legend becomes a cover plus one legend page per symbol, and a `theorem` becomes one card per page.** Author for the SEAM the variant has. bare/`feature`/`matrix`/`decompose` paginate their legend with the equation repeating above every page, and `derivation` paginates its rows with the table header repeating — so each legend item and each step must read on its own. `theorem` and `compare` get one card or one column per page, so a card that only makes sense beside its neighbor will not. `stats` and `canvas` are fixed scaffolds and keep the whole slide. 16:9 is untouched; the split has never applied there. A run's pages carry a forward pointer naming the next one, read from the member's title — a member that leads with an equation is named by the prose beside it, so give a legend symbol a real description (`- $\sigma$ — the logistic link`) rather than leaving it bare. A block written BETWEEN two members rides the page of the member it FOLLOWS, so a sentence meant to introduce the next card will sit under the previous one; a block written BEFORE the first member is framing and repeats on every page. And a member whose math does not TYPESET is named by the prose beside it rather than by its broken source, so give one a description.
...
- [`code`](./code.md) — the implementation, not the equation, is the argument
- [`diagram`](./diagram.md) — the structure of the model, not its closed form
- [`stats`](./stats.md) — a row of statistical results without a single equation focus
- [`content`](./content.md) — one inline equation inside a paragraph of prose
