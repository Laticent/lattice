...
- [`quadrant`](./quadrant.md) — items have continuous x/y coordinates rather than discrete quadrant labels
- [`verdict-grid`](./verdict-grid.md) — options scored across more than two dimensions
- [`obligation-matrix`](./obligation-matrix.md) — many rows × many columns of state-marker cells
- [`matrix-grid`](./matrix-grid.md) — both axes are ordered categories and cells mark one position, not four free quadrants
- [`cards-grid`](./cards-grid.md) — the items don't divide along two axes
