...
- [`obligation-matrix`](./obligation-matrix.md) — rows × columns of pass/partial/exempt status, not a single position
- [`roadmap`](./roadmap.md) — phases × workstreams delivery status
- [`matrix-2x2`](./matrix-2x2.md) — two free axes, four cells, qualitative placement
- [`verdict-grid`](./verdict-grid.md) — options scored against shared criteria, one card per option
