...
- [`split-panel`](./split-panel.md) — the featured element deserves its own colored panel, not a shared background
- [`list-tabular`](./list-tabular.md) — the rows need MORE than four fields, or a header row naming the columns
- [`cards-stack`](./cards-stack.md) — each term needs its own multi-sentence body, not a one-clause row
- [`glossary`](./glossary.md) — the terms are unordered and there's no claim to make about their sequence
