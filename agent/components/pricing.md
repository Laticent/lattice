...
- [`compare-table`](./compare-table.md) — a dense feature-by-plan matrix with many rows, not a few highlighted features
- [`verdict-grid`](./verdict-grid.md) — options scored on shared criteria, not priced tiers
- [`cards-grid`](./cards-grid.md) — parallel items with no price and no shared feature checklist
- [`decision`](./decision.md) — the slide recommends one option outright rather than presenting a price ladder
- [`big-number`](./big-number.md) — a single headline price, not a tiered comparison
