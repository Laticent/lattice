...
- [`kpi`](./kpi.md) — value + target + status tiles, not a single percent
- [`stats`](./stats.md) — independent headline metrics, no completion scale
- [`gantt`](./gantt.md) — the rows are time-bound and need a date axis
- [`checklist`](./checklist.md) — binary done / not-done across a flat list
- [`timeline-list`](./timeline-list.md) — the workstreams complete in sequence, not in parallel
