...
- [`quadrant`](./quadrant.md) — two axes are enough — the other six dimensions drop out
- [`verdict-grid`](./verdict-grid.md) — the criteria are categorical (pass/fail), not graded
- [`kpi`](./kpi.md) — the comparison is one option's metrics, not multi-option
- [`compare-table`](./compare-table.md) — a precise tabular comparison reads better than a shape
- [`piechart`](./piechart.md) — the question is part-to-whole, not multi-criterion
