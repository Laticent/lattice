...
- [`timeline-list`](./timeline-list.md) — dated entries with a status read and a sentence each, but no citation per row
- [`authority-chain`](./authority-chain.md) — single rule walked from statute to regulation to guidance to case
- [`list-criteria`](./list-criteria.md) — flat enumeration of requirements without dates or citations
- [`list-steps`](./list-steps.md) — lighter dots-on-a-spine sequence — the `timeline` variant
- [`list-tabular`](./list-tabular.md) — structured metadata per row but no regulatory framing
