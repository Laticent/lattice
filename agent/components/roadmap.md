...
- [`gantt`](./gantt.md) — continuous task bars across a date axis rather than discrete phase cells
- [`kanban`](./kanban.md) — current state by stage rather than phased schedule
- [`list-steps`](./list-steps.md) — single workstream sequence without parallel lanes
- [`verdict-grid`](./verdict-grid.md) — options scored against shared criteria, not phased delivery
- [`checklist`](./checklist.md) — single list with state markers, no workstream dimension
