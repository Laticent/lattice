...
- [`quadrant`](./quadrant.md) — the axes are unitless scores and the read is which of four named zones
- [`matrix-2x2`](./matrix-2x2.md) — items are placed by category, not by coordinate
- [`radar`](./radar.md) — each entity is rated on more than two measures
- [`progress`](./progress.md) — one measure per entity, compared as lengths
- [`list-tabular`](./list-tabular.md) — the exact numbers matter more than the shape of the relationship
