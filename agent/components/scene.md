...
- [`image`](./image.md) — the visual is a still photo or screenshot, not an animated scene
- [`diagram`](./diagram.md) — a static Mermaid graph says it — no order or depth that needs motion
- [`video`](./video.md) — the motion is recorded footage with a provider, not a fabricated vector scene
