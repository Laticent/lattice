...
- [`line`](./line.md) — three or more points in time — a trend rather than a before/after
- [`bar`](./bar.md) — one point in time, comparing magnitudes across categories
- [`stats`](./stats.md) — a row of headline figures with no ranking relationship between them
- [`big-number`](./big-number.md) — one entity's change is the whole story
- [`piechart`](./piechart.md) — the claim is share of a whole at one moment, not movement between two
- [`progress`](./progress.md) — attainment against a target per metric, with no before state
