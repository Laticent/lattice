...
**Capacity** ~2 items (over 2 overflows) — past that, decision / table. Exactly two by contract — the form IS a two-way choice, so there is no band to grow into. A split run paces ONE option per page and lands the verdict on its own closing page, carrying a derived “Option N of 2 · comparing …” signal.
