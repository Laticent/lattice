...
- [`split-compare`](./split-compare.md) — a binary decision with a recommendation card
- [`compare-prose`](./compare-prose.md) — two co-equal options side by side
- [`big-number`](./big-number.md) — the hero number is the whole slide, with no supporting list
- [`list-steps`](./list-steps.md) — an ordered process without a left anchor panel
