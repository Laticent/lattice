...
- [`piechart`](./piechart.md) — one total to decompose, with no comparison across categories
- [`progress`](./progress.md) — independent attainment bars with no parts and no shared value axis
- [`funnel`](./funnel.md) — the stages are a narrowing pipeline, not parts of a total
- [`matrix-grid`](./matrix-grid.md) — the cells are qualitative judgments rather than values that sum
