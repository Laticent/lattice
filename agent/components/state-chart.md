...
- [`diagram`](./diagram.md) — the machine has hierarchical states, parallel regions, or guards that need Mermaid's full state-diagram grammar
- [`journey`](./journey.md) — the sequence is a user's path through tasks with mood / affect, not a system's discrete states
- [`timeline-list`](./timeline-list.md) — events are points in time rather than transitions between named states
- [`list-steps`](./list-steps.md) — a linear procedure with no branching — state-chart is overkill if there are no choices to make
- [`roadmap`](./roadmap.md) — parallel workstreams across phases, not a single machine's transitions
