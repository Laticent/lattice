...
- [`big-number`](./big-number.md) — one number is enough to carry the slide
- [`kpi`](./kpi.md) — metrics need targets, trends, and status pills
- [`split-panel`](./split-panel.md) — one focal KPI with a paragraph of supporting prose
- [`piechart`](./piechart.md) — the numbers are parts of a whole, not independent
- [`progress`](./progress.md) — the metrics are completion percentages across workstreams
