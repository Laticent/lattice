...
- [`compare-prose`](./compare-prose.md) — exactly two options with prose bodies
- [`verdict-grid`](./verdict-grid.md) — options scored against criteria with pass/partial/fail badges
- [`obligation-matrix`](./obligation-matrix.md) — many regimes compared against shared obligations
- [`cards-stack`](./cards-stack.md) — each row needs full-prose breathing room rather than a tabular cell

## Demo deck

Every variant rendered, with a live preview and an in-browser editor:
<https://lattice.style/components/comparison/table>
