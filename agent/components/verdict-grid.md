...
- [`split-compare`](./split-compare.md) — two options with a bottom verdict bar
- [`obligation-matrix`](./obligation-matrix.md) — many regimes scored on shared obligations in a table
- [`compare-table`](./compare-table.md) — cells are textual values, not state markers
- [`checklist`](./checklist.md) — one set of criteria, not many options against them
