...
- [`image`](./image.md) — the visual is a still photo or screenshot, not a video
- [`closing`](./closing.md) — the send-off is a call to action with a QR, not a specific clip
- [`diagram`](./diagram.md) — the motion you want is an animated process — a Mermaid diagram may say it statically
