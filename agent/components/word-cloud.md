...
- [`progress`](./progress.md) — the weights need precise visual comparison
- [`stats`](./stats.md) — the headline metrics are independent numbers, not a corpus
- [`piechart`](./piechart.md) — the items are parts of a whole, not free-form themes
- [`quote`](./quote.md) — the verbatim language matters more than the frequency
- [`list`](./list.md) — single-line takeaways — the `takeaway` variant
