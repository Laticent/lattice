...
**Every component, grouped by bucket.** For the browsable edition — live previews, an in-browser editor, every palette — see the [component pages](https://lattice.style/components/).
...
- [`agenda`](#agenda) — the deck's sections, listed once, as a contents page
...
- [`list`](#list) — tenets or values (the `principles` variant) rather than a sequence; `takeaway numbered` for gating requirements rather than a sequence of actions
...
- [`big-number`](#big-number) — one figure is the whole slide
...
- [`big-number`](#big-number) — one figure against its target is the entire slide
...
- [`table`](#table) — a precise tabular comparison reads better than a shape
...
- [`matrix-2x2`](#matrix-2x2) — items are placed by category, not by coordinate
- [`radar`](#radar) — each entity is rated on more than two measures
- [`progress`](#progress) — one measure per entity, compared as lengths
- [`list-tabular`](#list-tabular) — the exact numbers matter more than the shape of the relationship
...
- [`big-number`](#big-number) — one entity's change is the whole story
...
- [`big-number`](#big-number) — the net movement is the whole point and the drivers are not worth a slide
...
- [`table`](#table) — three or more variants on shared dimensions
...
- [`list`](#list) — `takeaway numbered`: flat enumeration of requirements without tier hierarchy
...
- [`list`](#list) — `takeaway numbered`: a flat enumeration of requirements without a stance or an ask
...
- [`list`](#list) — `takeaway numbered`: flat enumeration of requirements without dates or citations
...
- [`table`](#table) — the comparison is across criteria, not jurisdictions
