...
2. Open `<name>.md` — median ~2.0k tokens.
...
- **`topic`** — Topic anchor inside a section.
  - *not for:* A section with one topic, or with ten
  - *use `divider` when* the boundary BETWEEN sections, rather than a topic inside one
  - *use `agenda` when* the deck's sections, listed once, as a contents page
...
- **`checklist`** — Items with state markers — done, partial, failed, unknown, to do, skipped.
...
- **`list`** — Bulleted list under a heading — plain pills, hairline takeaways (optionally numbered criteria with a gloss line), or display-weight principles.
...
- **`pricing`** — Side-by-side plan tiers with prices, feature checklists, and one recommended column.
  ...
  - *use `table` when* a dense feature-by-plan matrix with many rows, not a few highlighted features
...
- **`table`** — The table component — a GFM pipe table with a row capacity, autosplit, and the portrait card reshape.
...
- **`list-steps`** — Horizontal row of ordered step cards, each with a full description body (the `vertical` variant stacks them instead).
  ...
  - *use `roadmap` when* phased grid across multiple workstreams
...
- **`heatmap`** — A numeric matrix read as intensity — where the value concentrates across two dimensions.
  - *not for:* One row of numbers
  - *use `matrix-grid` when* the cells are qualitative — a verb, an owner, a status — rather than a number
  - *use `map` when* the two dimensions are geographic; same ramp, spatial placement
...
- **`authority-chain`** — Provenance chain — statute to regulation to guidance to case, walked in order.
  ...
  - *use `list` when* `takeaway numbered`: flat enumeration of requirements without tier hierarchy
...
- **`obligation-matrix`** — Regulation × obligation grid — state-marker cells encode applies / partial / exempt at a glance.
  ...
  - *use `table` when* cells are textual values, not state markers
...
| topic | anchor | split/anchor/prose | — |  | walkthrough agenda-setting board-deck | divider agenda big-number | Marks a topic WITHIN a section, where `divider` marks the boundary BETWEEN sections. |
...
| list | inventory | stack/inventory/prose | item:5/6/6* |  | overview summary takeaway walkthrough requirements | cards-stack list-tabular checklist | Use when the items are genuinely a flat list of one-line points. |
...
| q-and-a | inventory | stack/inventory/structure | item:4/5/6 | split across slides (automatic) | pitch board-deck recommendation | list-tabular glossary list cards-stack decision | Use to pre-empt the room: line up the three or four hardest questions the audience will raise and answer each one before it is asked. |
...
| matrix-2x2 | comparison | matrix/comparison/structure | item:4/4/4 |  | two-by-two prioritize strategy risk | quadrant verdict-grid obligation-matrix matrix-grid cards-grid | Use for categorical 2×2 reasoning when the items are fixed and you control which cell each lands in. |
| pricing | comparison | grid/comparison/structure | item:3/4/4 | table, split across slides | pitch tradeoff recommendation | table verdict-grid cards-grid decision big-number | Use for the plans / packages slide — two to four tiers compared on price and features, with one tier elevated as the recommendation. |
...
| split-compare | comparison | split/comparison/structure | item:2/2/2 | decision, table | tradeoff recommendation contrast | compare-prose decision split-panel verdict-grid | Use when a decision frames a binary choice and the recommendation must be unambiguous. |
| table | comparison | ledger/comparison/prose | row:4/6/8 | split across slides | tradeoff ranking assessment reference | compare-prose verdict-grid obligation-matrix cards-stack | Use when the table IS the slide. |
| verdict-grid | comparison | grid/comparison/structure | item:3/4/5 | table, split across slides | scorecard ranking prioritize assessment | compare-prose split-compare obligation-matrix table checklist | Use to evaluate 2–4 options against the same set of criteria, with pass/partial/fail badges. |
...
| list-steps | progression | timeline/progression/structure | item:4/5/5 | timeline-list, split across slides | process walkthrough planning | split-panel roadmap list funnel | Use for richer sequential processes where each step needs a paragraph rather than a label. |
...
| heatmap | chart | canvas/evidence/series | — |  | metric percentage assessment positioning | matrix-grid map bar quadrant | Use when the question is 'where does this concentrate', not 'how do these compare'. |
...
| radar | chart | scatter/evidence/series | — |  | spider assessment positioning | quadrant verdict-grid kpi table piechart | Use to compare 2–4 options across the same 4–8 criteria. |
...
| compare-code | code | split/comparison/structure | — |  | snippet contrast tradeoff | compare-prose redline redline table | Use to contrast a before/after refactor, two API styles, or two configurations. |
| authority-chain | legal | timeline/progression/structure | item:4/5/6 | split across slides (automatic), statute-stack | regulation citation sequence | regulatory-update list list-steps | Use when the audience needs to see how a rule descends: what the statute says, how the agency implemented it, what guidance interpreted it, and what cases have … |
...
| obligation-matrix | legal | matrix/comparison/structure | — |  | compliance regulation stoplight | table verdict-grid matrix-2x2 checklist matrix-grid | Use when many regimes need comparing across the same obligations. |
| policy-recommendation | legal | panel/statement/structure | item:3/3/3 | list takeaway numbered, split across slides | recommendation regulation risk takeaway | split-compare decision list regulatory-update | Use to put ONE policy recommendation before lawmakers. |
| regulatory-update | legal | ledger/progression/structure | item:4/5/6 | split across slides (automatic), list-tabular | changelog compliance regulation | timeline-list authority-chain list list-steps list-tabular | Use when a quarter's regulatory motion needs a single-slide digest. |
| statute-stack | legal | ledger/inventory/structure | item:3/4/5 | split across slides (automatic), list-tabular | citation reference compliance | list-tabular obligation-matrix authority-chain table | Use when three or four parallel jurisdictions need to read at a glance: each row carries the jurisdiction label, the citation, the obligation summary, and an … |
