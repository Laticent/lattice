...
**Contents:** `closing` · `divider` · `title` · `topic` · `big-number` · `content` · `premise` · `quote` · `split-panel`
...
- [`divider`](#divider) — the boundary BETWEEN sections, rather than a topic inside one
- `agenda` — the deck's sections, listed once, as a contents page
- [`big-number`](#big-number) — the metric IS the slide, not an anchor before the evidence
