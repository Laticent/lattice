...
**Contents:** `compare-prose` · `decision` · `matrix-2x2` · `pricing` · `redline` · `split-compare` · `table` · `verdict-grid` · `cycle` · `list-steps`
...
- [`table`](#table) — a dense feature-by-plan matrix with many rows, not a few highlighted features
...
- [`table`](#table) — cells are textual values, not state markers
