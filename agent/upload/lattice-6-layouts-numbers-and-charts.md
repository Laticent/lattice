...
**Contents:** `kpi` · `stats` · `_chart-family` · `bar` · `bullet` · `funnel` · `gantt` · `heatmap` · `journey` · `kanban` · `line` · `map` · `matrix-grid` · `piechart` · `progress` · `quadrant` · `radar` · `roadmap` · `scatter` · `slope` · `stacked-bar` · `state-chart` · `timeline-list` · `waterfall` · `word-cloud`
...
- [`matrix-grid`](#matrix-grid) — the cells are qualitative — a verb, an owner, a status — rather than a number
- [`map`](#map) — the two dimensions are geographic; same ramp, spatial placement
- [`bar`](#bar) — one dimension, and the comparison has to be exact
- [`quadrant`](#quadrant) — two CONTINUOUS axes with items placed on them, rather than two categorical axes with a value at each crossing
...
- **State markers are the second channel.** Every cell can lead with `[x]` shipped, `[-]` in flight, `[!]` missed, `[?]` uncertain, `[ ]` planned, or `[/]` out of scope. The audience sees both 'what' and 'how it's going' without a separate status slide. A status key is emitted automatically below the grid for the markers present (suppressed only on the `status` variant, which already labels every cell).
...
- **No state markers.** A grid of bare deliverables loses half its value. Add state markers (`[x]` `[-]` `[!]` `[?]` `[ ]` `[/]`) so the audience reads progress alongside scope.
