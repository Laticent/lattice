...
> Cross-cutting tokens you add beside a layout name on the same `_class` comment.
...
| `chrome` (5) | `silent` · `no-header` · `no-footer` · `no-paginate` · `no-progress` | Turn the running header, footer, page number or section rail off for one slide. `silent` bundles the first three — it is what bookends use. |
...
| `table` (5) | `table-plain` · `table-fill` · `state-cells` · `row-label` · `no-row-label` | Table treatment switches: drop the zebra, spread rows into leftover height, or decode the six state markers (`[x]` `[-]` `[!]` `[?]` `[ ]` `[/]`) in cells into status discs. |
