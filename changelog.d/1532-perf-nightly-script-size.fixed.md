...
- **Fixed: the nightly no longer profiles two deleted pages, or folds them into a third.**
  `/drawing-board/` and `/workbench/` left both Lighthouse url lists. Both surfaces were
  removed when the Studio succeeded them, and their routes are now 310- and 306-byte
  redirect stubs with zero JavaScript. They never produced rows of their own: Lighthouse
  follows the redirect and reports `/studio/`, which is the field the comparator groups on,
  so their six runs a night were folded into `/studio/` — every comment on #1532 shows five
  URLs from a seven-URL config. **Note the cost:** `/studio/` had been getting an accidental
  median-of-nine and now gets the median-of-three every other route gets, so its LCP/TBT/score
  rows should be expected to get noisier, not quieter.
