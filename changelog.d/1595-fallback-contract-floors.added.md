- **Added: a `var()` fallback's target is now checked against the read's CONTRACT, not just
  ...
  null and is counted, never defaulted to "no floor". Measured over the live tree: 297 token-hop
  fallbacks, 79 distinct hop pairs, 25 that drop a floor and, after six declared exceptions,
  ...
  mark). The 25 repo-wide drops are a pinned set that fails both ways, so the population cannot
  grow and draining shows as a diff — 8 are the `--cat-N-ink → --cat-N-mark` the repo itself
  mandates, and 17 are an unaudited `→ --accent` family that nothing had measured: `--accent`
  carries no floor against anything, so a 4.5:1 text read degrading onto it is the `--cat-N-ink`
  construction in seventeen more places. (`engineering/decisions/2026-08-11-fallback-contract-floors.md`)
