...
- **The kanban status wash is modelled in `tools/composed-contrast.js`** — both inks it
...
- **Fixed a silent mis-scoring in the surface catalog.** `resolveTokenExpr` reduces a
  ...
  incorrectly. The kanban wash is that shape; it is modelled through seeded component tokens
