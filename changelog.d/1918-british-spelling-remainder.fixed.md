- **Fixed: 59 British spellings in house prose, and the 30 that remain are now named
  ...
  deleted with the ratchet. A fresh pass with `britishFormRe()` over the repo's tracked
  text files plus tracked `.py`, the same matcher on both trees, counted 84 in living
  surfaces before and 30 after. (The gross total barely moves, 482 to 465, because a
  British-to-American map is itself a list of British words — which is why the living
  figure is the one quoted.) The `modelled` cluster
...
- **Five `camelCase` identifiers in `tools/check-ownership.js` are renamed** —
  ...
  RULE #21 says to name identifiers US too, and nothing had ever checked. The one remaining
  hit is `_emphasised_`, markdown emphasis inside the pre-registered benchmark calibration
  document, whose bytes set the `bench:check` baseline; the allowlist entry holding it now
