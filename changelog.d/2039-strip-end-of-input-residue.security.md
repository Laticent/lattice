- **Fixed: `--strip-notes` / `--strip-captions` no longer leave a blank line where a comment
  ...
  The cut now takes the blank above instead. Measured against the repo's own hand-written
  note-free twin fixture, which the scrub had been missing by exactly that byte — under both
  cuts, so it was never the `preserve`/`drop` tie-break's to fix, as it had been recorded.
