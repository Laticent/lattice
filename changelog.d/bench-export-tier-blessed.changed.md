- **The rasterize benchmark tier is blessed and checked, not just dumped.**
  `bench:bless -- --export` now writes `exportDatasets` into
  ...
  before/after record (HARD RULE #19(b)).
...
- **A benchmark tier that has never been blessed reports rather than failing the
  run.** Drift exits 1 on any machine because a blessed row has been recording
  nothing; a tier nobody has blessed yet has no rows to rot. A partly blessed tier
  still drifts normally, so a newly added row stays red until it is blessed.
