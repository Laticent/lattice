- **Every chart mark that draws an OUTLINE now draws it at one physical weight.**
  ...
  different physical weights**, from a 0.5px map border to a 2.9px pie wedge, a
  5.8× spread. Now **21 mark classes sit at exactly 1px** through one token,
  `--chart-edge`.
- **It is not one weight overall, and the remainder is deliberate.** Eight
  ...
  **six knockouts** — `funnel-band`, `line-dot`, `quadrant-dot`,
...
- **The export used to lose both properties.** `flattenSvgStyles` — the path
  ...
  viewBox USER unit: measured on the CLI artifact, a map edge came back at 0.83px
  and a scatter dot's at 2.21px. A stroked element now never drops either.
