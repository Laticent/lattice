- **A `list-tabular` status marker survives every list shape an author writes.** An
  ...
  two paths are now checked against each other on fourteen list rows across two
  probes, not one shape, by booting the real runtime bundle and comparing its output
  to the engine's.
