- **Fixed: an unclosed `<!--` no longer freezes the render or the linter.** A pasted
  ...
  on the Studio's main thread. The readers now find each closer once and reuse it:
  those inputs take 0.1–0.4 s, and every example deck renders byte-identical.
