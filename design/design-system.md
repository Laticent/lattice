...
| **Headline alignment** | `headline:` | the framing-text cluster's horizontal alignment (`auto`/`left`) — `auto` keeps each component's own default; `left` pins the cluster (`center`/`right` are a deferred follow-up) |
