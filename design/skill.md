...
| Scaffolding a new slide from a known component | `npm run new:slide <component>` (skeleton via the manifest); `npm run new:slide -- --list` enumerates the 61 shipped components |
...
| Hitting an unexpected behavior, hack, or browser/Marp/Mermaid quirk | [gotchas.md](../engineering/gotchas.md) — the symptom index; skim it, then open the ONE `engineering/gotchas/<topic>.md` it names (searching by API or property name instead? `grep -rn <term> engineering/gotchas/`). Found a new one? Add it as a `##` entry to the topic file — never to the generated index — then `npm run gotchas:index`. Keep the heading to a scannable symptom: it is billed twice per index row and capped at 280 characters |
