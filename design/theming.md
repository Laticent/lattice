...
- `--cat-N-ink` — the category's hue **as text on the slide**, one per slot.
  ...
  what keeps a theme built outside this repo (the Studio's `deriveTheme`, a
  third-party palette) on twelve distinct categorical marks rather than collapsing
  to one accent. The tier still reads *the ink is the mark, unless the mark fails*
