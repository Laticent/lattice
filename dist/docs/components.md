...
**61 components · 13 buckets.** For the browsable edition — live previews, an in-browser editor, every palette — see the [component pages](https://slidewright.github.io/lattice/components/).
...
- [`list-tabular`](#list-tabular) — the rows need MORE than four fields, or a header row naming the columns
- [`cards-stack`](#cards-stack) — each term needs its own multi-sentence body, not a one-clause row
- [`glossary`](#glossary) — the terms are unordered and there's no claim to make about their sequence

#### Demo deck

See [premise.gallery.light.pdf](../../lib/components/statement/premise/premise.gallery.light.pdf) for rendered examples of every variant.
...
- [`matrix-grid`](#matrix-grid) — both axes are ordered categories and cells mark one position, not four free quadrants
...
Use for richer sequential processes where each step needs a paragraph rather than a label. More verbose than timeline; more structured than a plain ordered list. Add `cat` on top of `capsule` when the steps are a categorical set in a sequence's clothing and each badge should carry its own hue instead of one repeated accent.
...
- **`capsule`.** The tone is warmer and more editorial than an ops checklist — a personal or reflective process (a practice, a ritual, a habit) reads better centered under a quiet pill badge than left-aligned under a mono caption. Centers the masthead with the cards and drops the connector arrows between steps: the badges carry the sequence, and a reflective practice is not a pipeline.
- **`cat`.** Composes with `capsule` only. The steps are secretly a categorical set (each one a distinct sample point, not an ordered march) and should read as visually distinct, not as one repeated accent stepping forward. Default `capsule` stays one tint on purpose for a genuine sequence — reach for `cat` only when that reasoning doesn't hold.
...
Centered cards, a pill-shaped step badge, a serif title.
...
Composes with `capsule`: each badge tints with its own categorical color instead of one repeated accent.
...
- [`obligation-matrix`](#obligation-matrix) — rows × columns of pass/partial/exempt status, not a single position
- [`roadmap`](#roadmap) — phases × workstreams delivery status
- [`matrix-2x2`](#matrix-2x2) — two free axes, four cells, qualitative placement
- [`verdict-grid`](#verdict-grid) — options scored against shared criteria, one card per option

#### Demo deck

See [matrix-grid.gallery.light.pdf](../../lib/components/chart/matrix-grid/matrix-grid.gallery.light.pdf) for rendered examples of every variant.
...
- [`matrix-grid`](#matrix-grid) — the grid marks one position on two ordered axes, not a pass/partial/exempt status per cell
