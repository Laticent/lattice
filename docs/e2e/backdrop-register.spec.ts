...
test('deck settings: the Backdrop row writes one `backdrop:` line from its two selects', async ({ page }) => {
...
test('deck settings: no Backdrop row without a finish', async ({ page }) => {
	await setEditorContent(page, '---\ntitle: Plain\n---\n\n# Plain\n');
	await openInspector(page);
	await expect(page.getByRole('combobox', { name: 'Backdrop strength' })).toHaveCount(0);
});

test('slide settings: strength and mask write `backdrop-*` tokens on the slide', async ({ page }, info) => {
