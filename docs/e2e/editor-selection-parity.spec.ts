...
for (const scheme of ['dark', 'light'] as const) {
	test(`both deck editors select natively, identically, and legibly — ${scheme}`, async ({ page }) => {
		...
		for (const s of SURFACES) {
			...
			const over = composite(wash.rgb, hexToRgb(m.bg), wash.alpha);
			expect(ratio(hexToRgb(m.textBody), over), `${where}: body text over the selection`).toBeGreaterThanOrEqual(4.5);
		}
