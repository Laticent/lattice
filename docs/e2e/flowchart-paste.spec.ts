...
// with that change reverted this spec fails at `data-fc-drawn`; with it, six edges draw.
const PLAIN = '---\ntheme: indaco\n---\n\n<!-- _class: title -->\n\n# Before\n';
...
test('a flowchart pasted into a deck with none draws without a reload', async ({ page }) => {
	...
	expect(await livePreview(page).locator('.flowchart-figure svg .fc-edge').count()).toBeGreaterThan(0);
});
