...
import { type SlideFrameLift, slideFrameStyle } from '@/lib/slide-frame';
...
export type DeckPreviewProps = {
	...
	className?: string;
	/**
	 * Frame the slide with the shared SLIDE FRAME (docs/src/lib/slide-frame.ts): an edge and
	 * a lift that trace the slide the engine painted, so the host shows the deck's own corner.
	 * A host that frames its slide passes this and puts NO `rounded-*`, `border`, `shadow-*` or
	 * `bg-*` on `className` — the gate in test/unit/tools/slide-frame-hosts.test.js pins it.
	 */
	frame?: SlideFrameLift;
	'aria-label'?: string;
...
export function DeckPreview({
	...
	return (
		<>
		<figure ref={stageRef} className={cn((loader || failed || chartDetail || splitPage) && 'relative', 'm-0', className)} style={frame ? slideFrameStyle(frame) : undefined} data-slide-frame={frame ? '' : undefined} role={role} {...aria}>
			{loader && !failed && (
