...
//     `var(--fin-canvas)` (accent mixed INTO the bg, never into `transparent`); a fade to
...
//   • 'opaque' (export) — accent mixed INTO var(--fin-canvas); the fade ends on var(--fin-canvas).
...
// any canvas) while the opaque face mixes into var(--fin-canvas).
export type FinishFace = 'rich' | 'opaque';

// An accent mix at N%. The fade-base is var(--fin-canvas) for export, `transparent` for
// screen — the single knob that splits the two faces.
const mix = (pct: number, face: FinishFace) =>
	`color-mix(in srgb, var(--accent) ${Math.round(pct)}%, ${face === 'rich' ? 'transparent' : 'var(--fin-canvas)'})`;
// The end-stop of a full-bleed fade: the canvas for export, nothing for screen.
const fadeEnd = (face: FinishFace) => (face === 'rich' ? 'transparent' : 'var(--fin-canvas)');
...
function washImage(type: WashType, i: number, face: FinishFace, x = 100, y = 0, spread = 100): string {
	...
	switch (type) {
		...
		case 'mesh': {
			...
			// screen so the blooms add cleanly; var(--fin-canvas) on export so no gray cloud — the
...
// color mixes into var(--fin-canvas) for export, into `transparent` for screen.
		...
		case 'vignette': {
			...
			const rim =
				face === 'rich'
					...
					: `color-mix(in srgb, var(--ink, var(--accent)) ${Math.round(i)}%, var(--fin-canvas))`;
			const center = face === 'rich' ? 'transparent 60%' : 'var(--fin-canvas) 62%';
 ...
 *  screen-only; 'opaque' = var(--fin-canvas), export-safe). The non-full-bleed slots (mark
...
export function recipeSlots(r: FinishRecipe, face: FinishFace = 'opaque'): string[] {
	...
	if (r.edge.type === 'vignette') decls.push('--fin-edge-position:center', '--fin-edge-size:cover');
		...
		// keyline strength.
		const c = `color-mix(in srgb, var(--accent) ${Math.round(Math.min(48, 26 + r.edge.intensity))}%, var(--fin-canvas))`;
		decls.push(`--fin-frame:inset 0 0 0 2.6cqi var(--fin-canvas), inset 0 0 0 2.82cqi ${c}`);
	}
...
// pattern that composites over any canvas) but must mix into var(--fin-canvas) on export, so it
...
// The SPOTLIGHT mask — a `var(--fin-canvas)` overlay that REVEALS the finish in one window and
// hides it everywhere else (the inverse of clearance). Palette-blind (`var(--fin-canvas)` only),
...
function spotlightMask(spot: { x: number; y: number; radius: number }, face: 'rich' | 'opaque'): string {
	...
	// transparent = finish shown (the window), var(--fin-canvas) = finish hidden (everywhere else).
	return face === 'opaque'
		? `radial-gradient(ellipse ${rr}% ${rr}% at ${x}% ${y}%, transparent 70%, var(--fin-canvas) 70%)`
		: `radial-gradient(ellipse ${rr}% ${rr}% at ${x}% ${y}%, transparent 42%, var(--fin-canvas) 96%)`;
}
...
export function generateSwatch(recipe: FinishRecipe): { background: string; backgroundSize?: string } {
	...
	return { background: 'var(--fin-canvas)' };
}
