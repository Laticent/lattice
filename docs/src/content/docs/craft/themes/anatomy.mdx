...
themes/
  ...
  evergreen-dark.css              ← the dark wrapper; see Light and dark
  evergreen-dark.manifest.json    ← the wrapper's identity
```

Four files, and `npm run new:theme` writes all four. The dark wrapper does
only one thing, in three declarations — you will see why on
[Light and dark](/craft/themes/light-dark/) — though the generated file wraps
