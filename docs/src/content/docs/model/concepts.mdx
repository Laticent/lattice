...
"Layout" is deliberately absent from this axis table: it is the human word for the **thing an author picks per slide** (the Component join below), not for the Form axis — whose human word is **Arrangement**. Keeping them distinct is what stops the one word doing two jobs.

The one word the model legislates against is **"look"** — it collides between *Arrangement* (Form) and *Finish* (the feel), so "make it look different" is resolved into one or the other before acting.
...
A **Component** — the thing an author picks per slide, called a **layout** in author-facing docs — **fills the content Tile and owns its own internal arrangement** there. It **selects** a Frame for the slide and binds Substance into the Cells that Frame produces; it carries a Function and receives Finish — the join, on one slide. ("Component" is the system word used in code, manifests, and `_class`; "layout" is the human word an author reads.) A **Bucket** is orthogonal bookkeeping: the disk grouping of components (mostly one-to-one with Function; a few — `chart`, `diagram`, `math`, `code`, `legal` — diverge to colocate a shared renderer or authoring vocabulary).
