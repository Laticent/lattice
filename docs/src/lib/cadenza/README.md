...
| `makeCursor(track)` | the lower-level primitive: `at(timeMs)` + `align(cueIndex, onsetMs, durationMs)`. Lives in `@laticent/ltt` since LTT step 2; re-exported here. |
| `ENGINE_HASH` | the timing engine's content hash, written into every LTT as `inputs.engine`. Generated by `npm run engine-hash:build`; a unit test fails when it is stale. |
...
cursor.ts     re-exports makeCursor from @laticent/ltt, where it lives since LTT step 2
engine-hash.ts  ENGINE_HASH, generated by npm run engine-hash:build — do not edit
