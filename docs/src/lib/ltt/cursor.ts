// The cursor: map a clock time to the active word, and re-anchor a cue to measured audio.
// Pure over the timeline; it reads a clock the consumer injects (WebAudio currentTime during
// TTS, a plain timer for a silent read-along, or a scrub bar) and never touches audio itself.
//
// Moved here WHOLE AND UNCHANGED from Cadenza in LTT step 2 (2026-09-24-lattice-timing-track.md
// §5), so the word lookup has one home: `positionAt` builds on it, the exported player inlines
// both, and Cadenza re-exports it for every existing importer.

import type { CaptionTrack, Word } from './types.js';
