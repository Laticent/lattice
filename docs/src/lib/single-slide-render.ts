			...
			.then(async () => {
				...
				try {
					...
					if (out.stats) {
						...
						// as a placeholder.
						s.charts = (out.html.match(/<section\b[^>]*\sclass="[^"]*\b(?:bar|bullet|line|scatter|slope|stacked-bar|waterfall|progress|timeline-list|piechart|gantt|kanban|radar|hub-spoke|quadrant|state-chart|funnel|map|journey|word-cloud|roadmap|matrix-grid|heatmap)\b/g) || []).length;
