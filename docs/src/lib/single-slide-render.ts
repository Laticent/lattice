...
import { hashString, linkGuardAgent, previewRuntimeAttr } from '../playground/deck-preview.js';
...
export function createSingleSlideRenderer(opts: SingleSlideOptions) {
	...
	function srcdoc(html: string, css: string, mode: 'light' | 'dark', mermaid: boolean, geom: Geom, extraCss = ''): string {
		...
		let s =
			...
			'<!doctype html><html' + (specimen ? ' data-lattice-specimen' : '') + previewRuntimeAttr(runtimeUrl) + '><head><meta charset="utf-8">' +
