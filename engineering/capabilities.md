...
| `equiv:check` | Re-run `equiv` and compare vs the committed baseline: decks/slides/preludes must match exactly, and the rate may drift 1.5 points. Cannot produce a true alarm at its own resolution (1.5 points is 18 of 1201 slides), which is why it is on-demand rather than a blocking CI gate. |
