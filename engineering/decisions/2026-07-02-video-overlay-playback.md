...
**On-device follow-up — the lightbox shape.** The first iPhone test confirmed the Instagram reel *plays*, but exposed a second problem: the lightbox shell was hardcoded to `aspect-ratio:16/9`, so a **portrait** reel was letterboxed into a short wide band with the card chrome squished. Two things fixed it: (1) `providerShape(href)` — YouTube/Vimeo are `landscape` (16:9), TikTok + Instagram are `portrait`, so portrait providers now get a tall phone-shaped box (`width:min(92vw,420px);height:min(86vh,760px)`) instead of 16:9. (2) Instagram's `/embed/` is a **card** (header + video + caption) whose height varies per post — its only public embed is the card, NOT a bare player, so some chrome is inherent and unremovable. The card reports its own rendered height to the parent via `postMessage({type:'MEASURE',details:{height}})`; we listen for that (origin-checked to `instagram.com`) and size the shell to it, so the card fits with no letterbox. If the message doesn't arrive, the fixed portrait box stands. Whether the MEASURE message actually fires for the raw `/embed/` iframe (without Instagram's own `embed.js`) is the remaining on-device unknown — it degrades to the fixed box either way, never a broken tap.

**On-device follow-up #2 — the iOS first-tap glitch (two causes).** A later report: on iPhone the *first* poster tap glitched and the mobile single-pane Playground **flipped to the Edit tab** (you had to switch back to Preview and tap again). Diagnosing it took two passes, and the *second* was the real one:

1. *First guess — the ghost click (partial).* The `touchend` that opens the modal is followed ~300 ms later by a synthesized `click` at the same coordinates, which can land on the freshly-mounted backdrop and dismiss it. Hardened it anyway (the backdrop ignores closes within 400 ms of open, via an `openedAt` timestamp; close button + Escape stay live) — correct defensively, but it did NOT fix the report: the user confirmed the overlay *does* appear, then the tab flips.

2. *Real cause — Radix automatic tab activation on FOCUS.* The mobile Edit/Preview tabs are Radix `Tabs`, whose default `activationMode="automatic"` activates a tab the instant its trigger receives **focus** (not just on click). Opening the player moves focus, it lands on the Edit trigger, and the pane silently flips to Edit *with no click* — behind the just-opened lightbox. A tap on the poster can't *click* the Edit tab (it's in the toolbar; the backdrop covers it once open), which is why the ghost-click fix missed it — the switch was focus-driven, not click-driven. Fix: set `activationMode="manual"` on the Playground tabs (`PlaygroundApp.tsx`) — panes now switch only on a deliberate tap/Enter, so no amount of focus movement can change them. Manual activation is also the recommended mode when switching a tab drives an expensive re-render, which ours does.

Neither is exercisable headless (HARD RULE #23) — confirmed by on-device iterations: the overlay-appears-then-flips observation is what ruled out guess 1 and pointed at the focus vector.
