...
- **Providers — YouTube + Vimeo + TikTok.** YouTube/Vimeo resolve synchronously (the id is in the URL). **TikTok is resolved at play time via its oEmbed** (`resolveTikTokSrc`): TikTok's `/oembed?url=` endpoint (a) resolves BOTH the common `/t/{code}` **short links** AND canonical `/@user/video/{id}` links server-side, and (b) sends `access-control-allow-origin: *`, so the browser can fetch it. We parse the NUMERIC `data-video-id` out of the response and build the official `player/v1/{id}` iframe (which loads with no `X-Frame-Options` / no frame-ancestors block) — from the id only, never the response HTML, so no injection. The tap opens the lightbox immediately with a "Loading…" state, then swaps in the player; a failed resolve closes and falls back to opening the link. (An earlier take dropped TikTok as "unembeddable"; that was wrong — the oEmbed round-trip solves the short-link problem client-side. Verified server-side that oEmbed resolves `/t/…` → id and is CORS-open, and that `player/v1/{id}` is frameable.)
  - **Instagram** stays a poster + link — it has NO public iframe player (only its `instgrm.Embeds` widget, which we won't load into the parent: privacy + #24).
- **UNVERIFIED from the sandbox (degrades gracefully regardless):** the actual in-BROWSER oEmbed fetch — headless Chromium here can't reach `tiktok.com` (network isolation; only `curl` via the proxy could), so the browser CORS round-trip is confirmed only by the `access-control-allow-origin: *` header, not exercised. And TikTok autoplay-after-async-resolve on iOS (the async gap may drop the autoplay gesture → tap play once in the player). Both are safe to ship because **any resolve/fetch failure falls back to opening the link** — worst case is the prior poster-link behavior, never a broken tap.
- **Still deferred:** Drawing Board + Present + Practice playback; Instagram (widget); richer positioning.
