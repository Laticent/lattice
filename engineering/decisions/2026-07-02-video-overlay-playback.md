...
**On-device follow-up #2 — the iOS first-tap "shutter" (ghost click).** A later report: on iPhone the *first* poster tap made the lightbox flicker open and immediately vanish, leaving the mobile single-pane Playground looking like it dropped back to the Edit view. Root cause is the classic iOS ghost click — the `touchend` that opens the modal is followed ~300ms later by a synthesized `click` at the same screen coordinates, which now land on the freshly-mounted backdrop and fire its close handler, dismissing the modal a frame after it opened (nothing actually changes the React `pane` state — no tap-to-edit affordance exists; the vanished overlay just *looked* like being kicked out). Fix: the backdrop-close handler ignores clicks fired within 400 ms of open (`openedAt` timestamp); the close button and Escape are separate handlers and stay live immediately. Because the opaque backdrop now stays up, any background scroll-lock reflow is hidden behind it too. Verified on-device (can't be exercised headless — HARD RULE #23).
