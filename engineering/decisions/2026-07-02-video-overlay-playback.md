...
- **Providers — YouTube + Vimeo + TikTok**, split by whether the id is in the URL:
  - **Sync (id in URL, no fetch — rock solid):** YouTube, Vimeo, and **canonical TikTok** (`/@user/video/{id}`, `/embed/{id}`, `/player/v1/{id}`). Built straight into `player/v1/{id}`.
  ...
  - **Instagram** stays a poster + link — NO public iframe player (only its `instgrm.Embeds` widget, which we won't load into the parent: privacy + #24).
...
- **UNVERIFIED from the sandbox (degrades gracefully regardless):** the actual in-BROWSER oEmbed fetch for `/t/…` short links — headless Chromium here can't reach `tiktok.com` (network isolation; only `curl` via the proxy could), so the browser CORS round-trip is confirmed only by the `access-control-allow-origin: *` header, not exercised on a real browser. And TikTok autoplay-after-async-resolve on iOS (the gap may drop the autoplay gesture → tap play once). Canonical TikTok + YouTube + Vimeo don't depend on any of this. Worst case for a short link is the visible "Open on TikTok" tap-through — never a broken/blank tap.
