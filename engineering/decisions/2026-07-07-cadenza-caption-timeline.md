...
lib/cadenza/                 ← pure browser ESM, fs-free, zero Lattice deps, import-boundary gated
  ...
  normalize.js the deterministic display→spoken expansion, reversible
...
re-export). *Validate packaging against a real Vite-served ESM `lib/` module at build time — the earlier
`resolve-spectrum.js` precedent was wrong (it is CJS, imported only by a test).*
...
2. **The Cadenza core** `lib/cadenza/` (§5) — Node-tested: estimate determinism, re-anchor math, display↔
