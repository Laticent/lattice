...
| **chart** — funnel, gantt, journey, kanban, map, piechart, progress, quadrant, radar, roadmap, state-chart, timeline-list, word-cloud | re-hosted as a `<figure>`, caption from title/footer — but by TIER (the projection evolved past "just the SVG"): (a) **self-contained SVG** (funnel, map, piechart, quadrant, radar) → the bare SVG in a `chart-frame` figure so its `--chart-cat-*` colours resolve; (b) **spatial-bounded** (word-cloud) → the whole `.chart-body` in a `.lp-spatial` box (an inline-size container as tall as the cloud's viewBox SVG at full width since 2026-09-26; it was `container-type:size` + a 3:2 `aspect-ratio`, which left two-thirds of the box empty — `2026-09-24-one-style-delivery-spine.md` §8.4); (c) **flow-height** (roadmap, progress, kanban, gantt, timeline-list) → the whole `.chart-body` in a `.lp-chart` WIDTH container (`container-type:inline-size`, content-driven height) — pure HTML+CSS layouts sized only in `cqi`; each component's CSS is broadened `section.<comp>` → `:is(section.<comp>, figure.<comp>)` so it paints in the figure; (d) **placeholder** (journey, state-chart) → the honest "best seen in Present / Read·Slides" card: **`state-chart` is runtime-JS-inflated and baked to the SLIDE's pixel geometry at export**, so a static re-host at another size orphans its edges; journey's affect-contour is height-relative — both need a runtime re-measure in the article box (tracked). |
