...
> **Pivot note (2026-07-13).** Phase A here (the SVG-paint `--viz-*` token hygiene + gate) was
> implemented and verified, then **parked, not merged** — the whole colour-fallback strategy moved
> to a runtime shim (`2026-07-13-old-browser-color-shim.md`), which makes `--viz-*` and its gate
> unnecessary. The parked implementation lives on branch `claude/session-resolution-p9gge0`. The
> **layout** thesis below — one `.viz-frame` for charts and diagrams (Phase B) — is about layout,
> not colour, so it is UNAFFECTED and remains an open proposal.
