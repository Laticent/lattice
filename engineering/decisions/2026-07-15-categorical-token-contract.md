...
**Date:** 2026-07-15 · **Status:** shipped 2026-07-16 — collapsed themes recolored to the
three-layer contrast contract; engine-level seed derivation deferred (see **Shipped** below) · **Owner:** Sharmarke

---

## Shipped (2026-07-16) — what landed, and why it differs from the locked plan

The locked decision below was **Seed & Roles** — declare one `--cat-N-seed` per category and
have the engine *derive* fill/mark via `color-mix`. In implementation that proved
**over-scoped for the actual bug**: it needs a new token, an override seam, and an engine
derivation path — real blast radius — to fix what is, at root, *ten themes shipping
`fill == mark`*. We shipped the **smaller change that satisfies the same contract**:

- **Token structure unchanged.** The concrete `--cat-N-fill` / `--cat-N-mark:
  light-dark(L, D)` slots stay. No `--cat-N-seed`, no engine change, no override seam to learn.
- **Values regenerated by a deterministic recipe** (per theme, from each theme's own 12
  curated hues — the invested brand colors are preserved, only S/L are re-placed):
  - **light fill** = pale but chromatic (L≈82%) — indaco posture, not washed;
  - **light mark** = deep edge/border, *solved* down until **edge-vs-canvas ≥ 3.2:1**;
  - **dark fill** = jewel tone, *solved* down until **label-text-vs-fill ≥ 7:1 (AAA)**;
  - **dark mark** = pale tint. fill and mark are flipped tiers of one hue → `fill == mark`
    is structurally gone.
- **The acceptance contract is three contrast layers** (framing from the owner): ① branch/
  border vs canvas ≥ 3:1, ② leaf-fill vs canvas — intentionally low, delineated by the ①
  border exactly as indaco/cuoio do, ③ text vs leaf ≥ 4.5:1 (we hold 7:1). Measured across
  every recolored theme, both modes, against the resolved token colors and confirmed on the
  rendered PDF.
- **onyx** stays monochrome (a single-hue brand can't separate by hue); its categorical
  distinctness via **luminance + texture** is a tracked follow-up, not part of this change.
- **Gate:** a three-layer contrast check over EVERY theme replaces the "distinctness gate"
  the investigation called for — same intent (no theme silently reships a collapse), keyed to
  the shipped contract.

Seed & Roles is **not rejected** — it remains the right eventual refactor if/when the
categorical cycle needs authoring ergonomics beyond "set the 24 values." It was descoped
*here* because the fix in front of us didn't warrant the engine surface. The design-competition
record below stands as the rationale for that future move.

---

### Historical: Decision — Seed & Roles, locked (2026-07-15)
...
- `matrix-grid.styles.css` row label (`td:first-child`) paints on the **slide surface**.
  ...
  curated ink is taken raw. **This is the only pixel this change moves.**
