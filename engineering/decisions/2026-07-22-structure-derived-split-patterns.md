...
| **viewBox graphic — container-responsive + legibility-floor→ring** | `bar` · `bullet` · `diagram` · `funnel` · `line` · `map` · `piechart` · `quadrant` · `radar` · `scatter` · `scene` · `slope` · `stacked-bar` · `state-chart` *(JS-scaled; no-JS UNVERIFIED)* · `waterfall` · `word-cloud` |
...
| **Read-across → keep whole / carousel** | `compare-code` · `compare-prose` · `compare-table` · `decision` · `journey` *(PORTRAIT ONLY — the vertical `journey-vstack` slices by stage; the landscape grid is one figure over a shared `--col` axis and rings)* · `kanban` *(per-lane — loses the cross-lane read; a keep-whole is arguably better)* · `math` *(splits on STRUCTURE, not on variant — eight variants, four rendered structures; the `stats` and `canvas` scaffolds have no collection and ring)* · `matrix-2x2` *(ENROLLED 2026-09-13, reversing the 2026-07-22 atomic resolution — the quadrant label carries its own position on both axes, and the `Option N of 4` rail carries the rest)* · `obligation-matrix` *(compare-table's shape and compare-table's reader — the grid reshapes to one regime card per page at portrait, and renders natively at landscape)* · `premise` *(one claim beside the points that substantiate it — same shape as split-panel)* · `pricing` *(compare N/M)* · `redline` · `roadmap` *(the TABLE rings; only the transposed `horizons` card form has a seam (#1209))* · `split-compare` *(examined for enrollment 2026-09-02 and declined: N is 2 by contract, the `.verdict` is a sibling of `.options` so a slice repeats it on every page, and one `.option` in a `1fr 1fr` grid leaves half the slide empty at landscape)* · `split-panel` |
