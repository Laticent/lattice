...
- ~~**37 of 74 declared `@font-face` are in `error` state** on a real sidecar, under
  ...
  `CSS.getPlatformFontsForNode`, identical before and after. Fixed by dropping a face the
  document already supplies and touching nothing else: **74 declared → 37, 37 `error` →
  0, 37 failed requests → 0, ~21 ms per navigation.** A face nothing else supplies keeps
  its relative url and stays broken — see the next bullet.
...
- **An uncovered `@font-face` in a caller's `--css` sheet is still unresolvable, and
  making it resolve is not a font question but a trust one.** The fix above deliberately
  leaves it alone. A first cut REBASED it onto the stylesheet's own directory, on the
  theory that this made the "no `assets/` in the tarball" fallback work. Two independent
  reasons killed that, both from the HARD RULE #25 checker. (a) It cannot help the case it
  named: `fontDir()` returns null — the condition that empties the base64 block — only when
  NEITHER `dist/fonts/` nor `assets/fonts/` exists, and `dist/fonts/` is exactly where a
  default-sheet rebase would point. (b) It opened a file-disclosure primitive:
  `lib/export/html-player.js`'s `inlineFileUrls` rewrites `url(file://…)` inside a
  `<style>` to a base64 `data:` URI, so a rebased `url('../SECRET.txt')` in a `--css`
  sheet was read off disk and baked into the shipped `--player` HTML — reproduced end to
  end with a canary file. That function's docblock already warns that a hosted bake path
  must gate `file://` inlining to an allowlisted root; the rebase widened the vector from
  deck markup to the stylesheet, silently. Resolving these faces properly means settling
  that boundary first, and is not this change.
- **`bench:check`'s render tier is red on `main` on this sandbox class**, by +24-26%
  ...
  `REGRESSION` or `not gated` depending on the minute. **Treat the +24-26% as
  run-dependent rather than a settled number**: an independent reviewer on this same
  instance got "within variance band — no regression" and a second pass reading "machine
  load, not code". Both observations are of an unchanged tree, which is the point. This is #1382's failure
...
| 22 | The fix for row 21 also REBASES an uncovered face onto the stylesheet's own directory, which "makes the documented no-`assets/` fallback work for the first time" — asserted in three places (the changelog fragment, and two comments in `lattice-emulator.js`); and the look-scratch page became request-free BECAUSE of that fix | **Three claims, all wrong, none of them measured before being written — and the seventh, eighth and ninth inferred mechanisms in this log.** (a) The rebase could not help the case it named: `fontDir()` returns null only when NEITHER `dist/fonts/` nor `assets/fonts/` exists, and `dist/fonts/` is exactly where a default-sheet rebase points. Self-contradictory by construction, visible from the two functions alone. (b) It opened a FILE-DISCLOSURE primitive: `html-player.js`'s `inlineFileUrls` rewrites `url(file://…)` inside a `<style>` to a base64 `data:` URI, so a rebased `url('../SECRET.txt')` in a `--css` sheet was read off disk and baked into the emailed `--player` HTML — reproduced with a canary. Before the rebase the url stayed relative and that regex did not match. (c) The look-scratch page is built with `setContent`, so it sits at `about:blank`, against which a relative url cannot resolve and is never requested: 0 requests started and 0 failed on a real Chromium, with the PRE-fix sheet. It never fetched anything, and the new comment crediting the font fix OVERWROTE a comment that had said so correctly. The `waitUntil: 'load'` change is right on its own end-to-end measurement (1,986 ms → 154 ms) and is unaffected. Rebase arm withdrawn entirely; an uncovered face is now left byte for byte. **The pattern this log keeps recording held again**: the measurements were sound, the mechanisms narrated around them were not, and this time the narration shipped in a commit before the checker had reported | The HARD RULE #25 checker, which also broke the new scanner four ways on inputs with a `}` or a `font-family` inside a comment or string |
| 23 | The rewrite for row 22 closes the silent-corruption class — "EVERY AMBIGUITY RESOLVES TOWARD KEEPING THE RULE" — and its hand-rolled scan is "~4 ms" against css-tree's 130-219 ms | **The class was narrowed from four inputs to two, not closed, and the perf argument for hand-rolling it was measured warm.** (a) The brace matcher honored strings and comments but not BACKSLASH ESCAPES (`font-feature-settings:x\\}`) or a `}` inside an UNQUOTED `url-token` (`url(a}b)`) — both legal CSS, both still ended the rule early, and the drop arm still spliced out the tail. Confirmed in real Chromium: the following `h1{color:red}` went from `rgb(255,0,0)` to the UA default, exit 0, no warning. (b) `declaredFamily` read the FIRST `font-family` where CSS is LAST-wins, so `font-family:'Outfit';font-family:'MyFont'` deleted a face declaring a family nothing supplied — no exotic syntax, just a duplicated descriptor. (c) The `~4 ms` was the seventh call in a warm loop; the CLI makes the first. Cold and end to end the change added **113 ms per process** against a ~21 ms per-navigation saving — a net LOSS on a single-navigation deck — dominated by a 60-70 ms walk over 845 KB of base64 to recover five family names the manifest already lists. Now 17 ms: `emittedFamilies` filters the manifest by the same `existsSync` test, and the top-level scan HOPS between interesting bytes instead of stepping. (d) The flagship regression test passed for the WRONG REASON — its comment `/* TODO {see #123} */` has BALANCED braces, so it stayed green with comment-awareness deleted from the matcher; a mutation run surfaced it and 5 other surviving mutants. All 9 guards in the scanner are now killed by a test. **Three sentences added by the previous commit were also wrong**, including a face census belonging to `main` labeled as this build's — the third recurrence, in the commit that added row 22 to record exactly this | A second independent checker on the fix for the first checker's findings |
| 24 | Rows 22 and 23 each closed the silent-CSS-deletion class by naming the inputs that broke it | **Naming inputs is not closing a class, and the record says so: two review rounds, two live instances, the second found in the commit that declared the first closed.** The boundary is no longer taken on trust — every span is now second-opinioned by `css-tree` before it is removed, and anything that is not exactly one whole `@font-face` is KEPT. An input nobody anticipated degrades to the pre-existing behavior (one doomed fetch) instead of corrupting the sheet. Three things are worth recording about building it. (a) **A differential fuzz of the scanner against css-tree over 80 adversarial inputs found 0 mis-spans and 10 MISSES, every miss in the safe direction** — so this guard is a net for the unknown, not a patch for a live bug, and its own live risk is the opposite one, a FALSE refusal, which costs the perf win rather than the CSS. Both directions are pinned. (b) **The guard's first cut had a hole its own test caught**: a CSS parser auto-closes an unterminated block at EOF, so css-tree certifies `@font-face{font-family:X` as one complete at-rule — exactly the TRUNCATED shape the guard exists to reject. It needed an explicit closing-brace check beside the parse. (c) **The wiring was initially unkillable**: with no natural input reaching the refusal path, deleting the guard outright left every test green — the same "no test kills it" hazard `style-guard-census.test.js` documents for the #22 guards. `validate` now accepts a predicate purely so the refusal path has a test that fails when the guard is removed. (d) Cost measured IN SITU, not warm: 5.1 ms of parses but 64 ms to `require('css-tree')` once, so ~70 ms total — which is why it runs only for a caller-supplied `--css` sheet. The bundled sheet is fixed bytes whose drop behavior this same oracle already asserts at build time | Answering "do we feel confident to ship?" with the shape of the evidence rather than the state of the gates |
