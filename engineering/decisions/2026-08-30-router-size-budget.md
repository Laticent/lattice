...
| `9504fde` (main, mid-PR) | 58,550 | 15,041 |
...
- **Routing the canonical-doc table** (1,413 tokens, 37 rows) is the clearest case and is
  actively wrong. That table is the thing that tells you which doc to open; moving it behind a
...
the gate judges ONE file, the calibration was unusually tight: 3.9039, 3.9036, 3.9064, 3.9067
bytes per token across four distinct revisions spanning +1,144 tokens — a spread of **0.079%**,
...
gate docblock's own example — fill the whole headroom with a CSS fence and the file is 17,342
real tokens while the gate reports 16,509, 5% understated, with the band green at 29.6%. Across
every tracked `.md` over 4k, non-letter share and bytes/token correlate at **r = −0.30**; of the
625 repo files inside that band, bytes/token spans 3.53 to 4.70. It could not have worked
...
Requiring the `o200k_base` encoding alone costs **~180 ms** and ~120 MB RSS; encoding this file
costs ~40 ms. End to end that is **~190 ms on a ~6.0 s `check:ownership` run**, about 3%
(6,180 / 6,052 / 6,271 ms with, against 6,153 / 5,990 / 5,907 without). *An earlier version of
...
**16,500 tokens**, against 15,117 today — **1,383 tokens of headroom**, the owner's +10%
choice. One rule the size of #22 (1,288 tokens) or #29 (1,164) fits without a trade; a second
does not.
