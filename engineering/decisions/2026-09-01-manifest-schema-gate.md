...
Cost: five packages, 3 MB, `devDependency`. This module is required only from
`tools/`, never from `lib/`. That boundary is deliberate — the hand-copied enums
in `lib/forms/index.js` and `lib/components/index.js` exist to keep schema JSON
out of the browser bundles, and reaching the component gate through the wrong
door once pulled a 41 KB manifest schema into the theme bundle
(`changelog.d/1841-css-scanner-comment-bypass.security.md`).
