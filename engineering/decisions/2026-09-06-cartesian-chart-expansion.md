...
**What `golden-diff`'s "42 slides changed" actually means, checked rather than
assumed.** The chart BUCKET gallery (`lib/components/chart/chart.gallery.md`) is
generated by walking the bucket alphabetically, so the seven new members insert
ahead of most of it — `bar` and `bullet` before `funnel`, all seven before
`word-cloud`. `golden-diff` compares by SLIDE INDEX, so one insertion reports
every later slide as changed: 21 per mood, 42 in total, none of them a
regression.
Verified by matching pages by CONTENT instead of index and diffing the pixels.
`funnel` (base p2 -> head p4), `quadrant` (p10 -> p13), `radar` (p11 -> p14) and
`piechart` (p8 -> p11) are BYTE-identical PNGs. `word-cloud` (p15 -> p22) is
byte-identical at 90 DPI and differs by **6 channel samples out of 4,002,000 at
