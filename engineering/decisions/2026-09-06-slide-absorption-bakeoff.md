...
status: superseded
summary: >
  A measured bake-off of cost models for SLIDE ABSORPTION — how long a slide needs to be looked at —
  built to answer "narration places no weight on content, so nothing breathes". The instrument was
  never merged and the branch (#2079) was closed; this record is kept for what it cost to learn.
  THE PREMISE WAS FALSE, and §0 leads with the correction: `projectDeckToSpeech` is ONE STAGE of the
  narration pipeline, so timing it alone reports a diagram slide at 4.3 s where the shipped caption
  runs 40.6 s — about 9x short, on exactly the slides the work was about. Fed real narration, every
  cost model here collapses to 0-6 of 41 visual slides: the narration already provides the time. Two
  adversarial trios found the measurement broken in six independent ways and the conclusion wrong
  three separate times, each fix pass introducing new defects. What the complaint actually pointed at
  was EMPHASIS — where the pauses fall inside narration that is already long enough — which shipped
  in #2103 along with the defect underneath it: a slide's closing quote was never narrated at all.
  The durable lessons are the method, not the model: time the whole pipeline before modelling one
  stage of it, and never measure a property against an artifact that is structurally blind to it.
companion:
...
> **CLOSED, NOT SHIPPED.** The instrument this record describes (`lib/core/slide-absorption.mjs`,
> `tools/absorption-bakeoff.js`) lives only on the abandoned branch of #2079 and is not in the tree.
> The record is kept because the negative result and the method are worth more than the code was:
> every number below was produced, then disproved, then re-derived, and the sequence is the point.
>
> **What superseded it:** `2026-09-06-narration-emphasis-weight.md` (#2103) — author-marked emphasis
> buys a beat where a passage ends, and the coda is spoken. That change came from re-reading the
> complaint after this one answered a question nobody had asked.
