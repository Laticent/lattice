...
### Charts render solid black / colourless on an old smart-TV browser

**Symptom.** On an old webOS / smart-TV Chromium (predating Chromium 123), pie
wedges and choropleth map regions render **solid black**, gantt bars and journey
chips **lose their colour**. Fine on every modern browser and in the PDF.

**Cause.** Chart colours resolve through `light-dark()` / `color-mix()`. An engine
that doesn't support those drops the *whole* declaration as invalid-at-computed-
value; SVG `fill` then falls to its black initial value, HTML `background`
vanishes. It's a browser-support gap, not a token bug.

**Fix (static palette compilation).** There is no `@supports` fork any more — it was
never executed by headless Chromium, so regressions rode through it unseen. Instead the
chart colour recipe is a build-time INPUT (between `>>> chart-palette-recipe >>>`
sentinels in `chart-family.css`): `tools/build-chart-palette-css.js` resolves it per
theme to two FLAT-literal planes (the theme's declared scheme on the kernel selectors
incl. the bare `.chart-frame`; the opposite scheme as a subject-anchored override on
`.chart-frame.<scheme>` / `[data-lp-scheme=<scheme>] .chart-frame` / the strict OS arm),
`tools/build-css.js` strips the recipe from the shipped bundle and injects the planes
into each `dist/themes/*.min.css`, and `lattice-emulator.js` appends the same planes +
stamps `data-lp-scheme` on the root. Modern and old browsers run byte-identical chart
CSS. **Edit chart colours in the recipe, never in the generated planes**; the leak gate
`test/unit/tools/chart-palette-css.test.js` fails if a plane carries a surviving
`color-mix()` / `light-dark()` / `var()`. Full rationale:
