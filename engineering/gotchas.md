...
### Charts / diagrams render solid black / colourless on an old smart-TV browser
...
chips **lose their colour** — and so do **Mermaid diagram bands / mindmap edges,
legal authority-chain & statute-stack tier accents, and decision option accents**.
Fine on every modern browser and in the PDF.

**Cause.** These colours resolve through `light-dark()` / `color-mix()` (charts via
the `--chart-cat-*` kernel; Mermaid + legal + decision via the engine-wide
`--cat-*` palette). An engine that doesn't support those functions drops the
*whole* declaration as invalid-at-computed-value; SVG `fill` then falls to its
black initial value, HTML `background`/`border` vanishes. It's a browser-support
gap, not a token bug.

**Fix (already in place).** The build emits a per-theme flat-literal fallback
inside `@supports not (color: light-dark(#000,#fff))` (inert on modern engines,
applied on old ones), appended to each `dist/themes/*.min.css`. Generated by
`tools/build-chart-compat-css.js` from the authored CSS — it scans the chart
bucket **plus the DIAGRAM GROUP** (`DIAGRAM_GROUP_FILES`: Mermaid, legal, decision
— non-chart components that ride `--cat-*` but are not `.chart-frame`). **Do not**
hand-edit the fallback or add colours the generator can't flatten (the leak gate
in `test/unit/tools/chart-compat-css.test.js` fails if a colour survives
unflattened). Full rationale:
