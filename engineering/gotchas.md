...
`tools/build-chart-compat-css.js` from the authored CSS — **do not** hand-edit
the fallback or add chart colours the generator can't flatten (the leak gate in
`test/unit/tools/chart-compat-css.test.js` fails if a colour survives
