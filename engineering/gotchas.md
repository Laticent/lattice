...
### [Browser engine (Chromium quirks observed in Marp Preview / Puppeteer)](gotchas/browser-engine.md)
...
### [CI (GitHub Actions / code scanning)](gotchas/ci.md)
...
- [The editor|preview divider snaps to the middle a moment after the page loads](gotchas/ci.md#the-editorpreview-divider-snaps-to-the-middle-a-moment-after-the-page-loads)
- [The Playground's preview pane is empty for seconds after a reload](gotchas/ci.md#the-playgrounds-preview-pane-is-empty-for-seconds-after-a-reload)
- [The Playground's cached slide jumps when the live preview takes over](gotchas/ci.md#the-playgrounds-cached-slide-jumps-when-the-live-preview-takes-over)
- [The Playground's Explore layout arrives a second after the page does](gotchas/ci.md#the-playgrounds-explore-layout-arrives-a-second-after-the-page-does)
- [The Playground's divider is in one place before hydration and another after](gotchas/ci.md#the-playgrounds-divider-is-in-one-place-before-hydration-and-another-after)
- [A header control shows nothing (or the wrong thing) for a second after every page load](gotchas/ci.md#a-header-control-shows-nothing-or-the-wrong-thing-for-a-second-after-every-page-load)
...
- [Blurred `box-shadow` → opaque gray box around the element in some PDF viewers](gotchas/css.md#blurred-box-shadow--opaque-gray-box-around-the-element-in-some-pdf-viewers)
...
### [Docs site (Astro + GitHub Pages)](gotchas/docs-site.md)
...
- [Installed iOS PWA: "Connect OpenRouter" doesn't stick](gotchas/docs-site.md#installed-ios-pwa-connect-openrouter-doesnt-stick)
...
- [Playground/specimen previews 404 on the engine CSS + runtime](gotchas/docs-site.md#playgroundspecimen-previews-404-on-the-engine-css--runtime)
- [Playground preview serves a STALE engine bundle (a 200, not a 404)](gotchas/docs-site.md#playground-preview-serves-a-stale-engine-bundle-a-200-not-a-404)
...
- [The Present rail is completely invisible under `forced-colors: active`](gotchas/docs-site.md#the-present-rail-is-completely-invisible-under-forced-colors-active)
- [A slide surface ignores one input device (a wheel mouse does nothing; arrows are dead)](gotchas/docs-site.md#a-slide-surface-ignores-one-input-device-a-wheel-mouse-does-nothing-arrows-are-dead)
- [A pinch on a slide turns the deck (and `preventDefault` in your React handler does nothing)](gotchas/docs-site.md#a-pinch-on-a-slide-turns-the-deck-and-preventdefault-in-your-react-handler-does-nothing)
- [The Studio "crashed" and reloaded itself, and nothing was logged anywhere](gotchas/docs-site.md#the-studio-crashed-and-reloaded-itself-and-nothing-was-logged-anywhere)
- [Data a user deleted comes back when a parked tab wakes up](gotchas/docs-site.md#data-a-user-deleted-comes-back-when-a-parked-tab-wakes-up)
- [A Web Lock held for the life of a page silently kills its bfcache](gotchas/docs-site.md#a-web-lock-held-for-the-life-of-a-page-silently-kills-its-bfcache)
- [A crash report shows `Script error.` several times and names nothing](gotchas/docs-site.md#a-crash-report-shows-script-error-several-times-and-names-nothing)
- [A multi-line toast renders as a giant lozenge with its last line cut off](gotchas/docs-site.md#a-multi-line-toast-renders-as-a-giant-lozenge-with-its-last-line-cut-off)
- [A control's own icon renders sliced/outside its button, and every overflow guard is green](gotchas/docs-site.md#a-controls-own-icon-renders-slicedoutside-its-button-and-every-overflow-guard-is-green)
- [A CodeMirror `@media (pointer: coarse)` block has no effect on a real touch device](gotchas/docs-site.md#a-codemirror-media-pointer-coarse-block-has-no-effect-on-a-real-touch-device)
...
- [A fixed-size slide frame silently truncates content past 1280×720](gotchas/lattice-internals.md#a-fixed-size-slide-frame-silently-truncates-content-past-1280720)
...
- [Playground preview won't scroll on iOS after opening a settings sheet](gotchas/lattice-internals.md#playground-preview-wont-scroll-on-ios-after-opening-a-settings-sheet)
- [`svh` can resolve LARGER than `dvh` on a real mobile browser](gotchas/lattice-internals.md#svh-can-resolve-larger-than-dvh-on-a-real-mobile-browser)
- [Preview slides collapse (cqi shrinks to near-zero) on iOS if scaled with CSS `zoom`](gotchas/lattice-internals.md#preview-slides-collapse-cqi-shrinks-to-near-zero-on-ios-if-scaled-with-css-zoom)
- [The Studio says a feature "hit an unexpected error" on a tab that has been open a while](gotchas/lattice-internals.md#the-studio-says-a-feature-hit-an-unexpected-error-on-a-tab-that-has-been-open-a-while)
- [A long press on a button selects its label on iOS (Copy / Look Up callout)](gotchas/lattice-internals.md#a-long-press-on-a-button-selects-its-label-on-ios-copy--look-up-callout)
- [Tapping an input zooms the page on iOS (sub-16px text controls)](gotchas/lattice-internals.md#tapping-an-input-zooms-the-page-on-ios-sub-16px-text-controls)
- [Tapping an in-slide link blanks the live preview on iOS](gotchas/lattice-internals.md#tapping-an-in-slide-link-blanks-the-live-preview-on-ios)
- [A blurred `box-shadow` renders as a flat gray block in Apple PDFKit](gotchas/lattice-internals.md#a-blurred-box-shadow-renders-as-a-flat-gray-block-in-apple-pdfkit)
- [A JSON data block inside a `<script>` comes back with `&amp;` in every string](gotchas/lattice-internals.md#a-json-data-block-inside-a-script-comes-back-with-amp-in-every-string)
- [The exported player has no front matter to read](gotchas/lattice-internals.md#the-exported-player-has-no-front-matter-to-read)
- [A slide-level color-scheme pin has to be re-emitted for the exported player](gotchas/lattice-internals.md#a-slide-level-color-scheme-pin-has-to-be-re-emitted-for-the-exported-player)
- [A token flattened for the player took the print band's value](gotchas/lattice-internals.md#a-token-flattened-for-the-player-took-the-print-bands-value)
- [Chart fills took one scheme while the page took the other](gotchas/lattice-internals.md#chart-fills-took-one-scheme-while-the-page-took-the-other)
- [A baked diagram label went dark-on-dark after the player's toggle](gotchas/lattice-internals.md#a-baked-diagram-label-went-dark-on-dark-after-the-players-toggle)
- [`--strip-notes` deleted a comment out of a code fence](gotchas/lattice-internals.md#--strip-notes-deleted-a-comment-out-of-a-code-fence)
- [`--strip-notes` could not remove a note that opens with a directive keyword](gotchas/lattice-internals.md#--strip-notes-could-not-remove-a-note-that-opens-with-a-directive-keyword)
...
- [Color emoji needs an installed font on the owned render paths](gotchas/marp.md#color-emoji-needs-an-installed-font-on-the-owned-render-paths)
- [Marpit "spot replaces global" for the `class:` directive](gotchas/marp.md#marpit-spot-replaces-global-for-the-class-directive)
- [Chromium blocks `file://` URLs as `mask-image` sources](gotchas/marp.md#chromium-blocks-file-urls-as-mask-image-sources)
- [Chromium PDF output of CSS `mask-image` renders inconsistently across viewers](gotchas/marp.md#chromium-pdf-output-of-css-mask-image-renders-inconsistently-across-viewers)
- [SVG images in the exported PDF partially render or vanish in iOS Quartz viewers](gotchas/marp.md#svg-images-in-the-exported-pdf-partially-render-or-vanish-in-ios-quartz-viewers)
- [Flex-centered caps read high in JetBrains Mono (and `text-box-trim` can't fix it here)](gotchas/marp.md#flex-centered-caps-read-high-in-jetbrains-mono-and-text-box-trim-cant-fix-it-here)
- [Custom `logo:` front-matter directive shows nothing in marp-vscode preview](gotchas/marp.md#custom-logo-front-matter-directive-shows-nothing-in-marp-vscode-preview)
...
- [lattice-emulator doesn't auto-load `style:` from front matter](gotchas/marp.md#lattice-emulator-doesnt-auto-load-style-from-front-matter)
- [Rendering in the cloud sandbox needs `CHROME_PATH`](gotchas/marp.md#rendering-in-the-cloud-sandbox-needs-chrome_path)
- [A rendered PDF shows serif/fallback type, not the design fonts](gotchas/marp.md#a-rendered-pdf-shows-seriffallback-type-not-the-design-fonts)
- [A slide loses its EYEBROW and HEADING off the top, and no ring / pill / console line fires](gotchas/marp.md#a-slide-loses-its-eyebrow-and-heading-off-the-top-and-no-ring--pill--console-line-fires)
- [The overflow ring lags an edit, or a slide scrolled past keeps a ring it should have lost](gotchas/marp.md#the-overflow-ring-lags-an-edit-or-a-slide-scrolled-past-keeps-a-ring-it-should-have-lost)
- [A false "Overflows" ring appears on the exported `.html` sidecar for a slide that actually fits](gotchas/marp.md#a-false-overflows-ring-appears-on-the-exported-html-sidecar-for-a-slide-that-actually-fits)
- [One slide renders at ~2x type and overflows, but ONLY in a live preview — the PDF is perfect](gotchas/marp.md#one-slide-renders-at-2x-type-and-overflows-but-only-in-a-live-preview--the-pdf-is-perfect)
- [A slide clips 30-70px in the Playground that the exported PDF renders whole](gotchas/marp.md#a-slide-clips-30-70px-in-the-playground-that-the-exported-pdf-renders-whole)
- [The Studio counts fewer slides than the deck renders — or an edit destroys a slide](gotchas/marp.md#the-studio-counts-fewer-slides-than-the-deck-renders--or-an-edit-destroys-a-slide)
- [A `split-panel proof` run is one hue in the Studio, but only when the deck doesn't paginate](gotchas/marp.md#a-split-panel-proof-run-is-one-hue-in-the-studio-but-only-when-the-deck-doesnt-paginate)
- [The Playground and the Studio disagree about which slides overflow (and a slide's own padding changes when the preview pane is resized)](gotchas/marp.md#the-playground-and-the-studio-disagree-about-which-slides-overflow-and-a-slides-own-padding-changes-when-the-preview-pane-is-resized)
- [A live preview prints "1" as the page number on every slide](gotchas/marp.md#a-live-preview-prints-1-as-the-page-number-on-every-slide)
- [Exported fluid viewer: an overflowing slide shows NO marker tab, or the red author ring leaks to a reader](gotchas/marp.md#exported-fluid-viewer-an-overflowing-slide-shows-no-marker-tab-or-the-red-author-ring-leaks-to-a-reader)
...
- [A CSS reset declaration silently does nothing — the value doesn't exist](gotchas/marp.md#a-css-reset-declaration-silently-does-nothing--the-value-doesnt-exist)
- [The same declaration, but it dies at COMPUTED-VALUE time — and does NOT fall back](gotchas/marp.md#the-same-declaration-but-it-dies-at-computed-value-time--and-does-not-fall-back)
- [A committed render golden doesn't match a fresh render — check staleness FIRST](gotchas/marp.md#a-committed-render-golden-doesnt-match-a-fresh-render--check-staleness-first)
- [A manifest slot's `selector` describes AUTHORING input — a transform may consume it](gotchas/marp.md#a-manifest-slots-selector-describes-authoring-input--a-transform-may-consume-it)
- [Charts export black/unstyled from the Studio image PDF or PPTX](gotchas/marp.md#charts-export-blackunstyled-from-the-studio-image-pdf-or-pptx)
- [Studio PDF/PPTX export shows fallback type on some slides](gotchas/marp.md#studio-pdfpptx-export-shows-fallback-type-on-some-slides)
- [Studio / playground LIVE PREVIEW shows hand-body decks in a system sans](gotchas/marp.md#studio--playground-live-preview-shows-hand-body-decks-in-a-system-sans)
...
- [The preview `<iframe>` — trap catalog (read this first: surfaces × workarounds)](gotchas/mermaid.md#the-preview-iframe--trap-catalog-read-this-first-surfaces--workarounds)
- [Playground renders broken in mobile Safari/WebKit (counters "00", chart text overlaps, marks drop)](gotchas/mermaid.md#playground-renders-broken-in-mobile-safariwebkit-counters-00-chart-text-overlaps-marks-drop)
- [lattice-engine: deck looks fine on desktop but collapses on mobile WebKit (no `:root` token relocation)](gotchas/mermaid.md#lattice-engine-deck-looks-fine-on-desktop-but-collapses-on-mobile-webkit-no-root-token-relocation)
- [Playground math (and any cqi/cqh layout) renders tiny + "jumps/rescales"](gotchas/mermaid.md#playground-math-and-any-cqicqh-layout-renders-tiny--jumpsrescales)
