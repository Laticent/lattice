...
## Charts render black in the Studio Webpage player's Read · Article view

- **Symptom:** A deck exported from the Studio as a **Webpage (.html)** plays fine in Present and Read · Slides, but in **Read · Article** every chart is wrong: radar, pie, bar and heatmap marks solid black; kanban, progress, roadmap and timeline-list unstyled text. Dark mode also leaves the player's chrome and the article page white. The same deck exported with the CLI (`--player`) is correct everywhere.
- **Cause:** The Studio renders through the browser engine, which **packs** the theme for the live preview (`packTheme` in `lib/engine/css.js`): every rule is scoped to the inside of a slide, and `:root` moves onto the slide as `:where(section):not([\20 root])`. The export used to strip only the wrapper (`article.lattice > `). Read · Article lifts each chart's figure **out** of its slide, so it matched neither the chart's re-host rules (`figure.chart-frame …` had packed to `section figure.chart-frame …`, and `.chart-frame`'s palette block to `section .chart-frame`) nor any palette token. SVG's initial `fill` is black. The CLI never packs, so it never had the bug. A packed `section h2` cannot be told apart from a rule written `section h2`, and `lattice.css` writes 637 of those, so the pack's scoping cannot be undone after the fact.
- **Mitigation:** The export renders with `{ styles: 'flat' }` (the engine's named flat mode; it was `{ flatCss: true }` until 2026-09-25), and the engine returns `flatCss`: the same pack, with every arm it re-scoped ALSO emitted as written (`packSelector`'s `flat` mode). Inside a slide the packed arm still wins. Outside one, the written arm matches exactly what the CLI's copy of the rule matches. Two slide-side effects, both measured: (1) arms led by a section-level `:where([data-family=…])` filter can now match inside a slide, but every one is headed by a re-host `figure.<chart>` that no slide emits; (2) with `:root` present, `themeDualMode` now flattens the dark tokens exactly as it does for the CLI (it found no `:root` block before). Across 278 before/after slide captures (a 23-slide chart gallery and the 116-slide gallery, light and dark), no slide content changed. `css` stays the preview shape for the diagram bake's capture frame. Pinned in `test/unit/engine/css-scope.test.js` (the FLAT block), and gated on the rendered surface by `check:render`'s copy-parity pass, which compares every chart's Read · Article copy against its slide and fails on a lost paint (309 distinct losses with the old pack). A new render wrapper must carry `flatCss` through: `lib/playground/index.js` rebuilds the result object field by field, and it silently dropped the field on the first attempt.
- **Triggered by:** Any chart in a Studio Webpage export, viewed in Read · Article. Reported 2026-09-24 as "the radar renders black and white". It followed the earlier map/quadrant/radar black-tile bug in `gotchas/marp.md`, which fixed the SLIDE arm of the same selectors and left the figure arm packed under a slide.
- **Not this:** The Studio's own in-app **Reading view** (`ReadArticle.tsx`) showed the same black charts for a different reason: it shipped no deck stylesheet at all, because it renders in the app's top-level document. Since 2026-09-25 it ships the flat pack pruned and fenced to its figures (`scopeReHostedCss`); see `2026-09-24-one-style-delivery-spine.md` §8.1.

## A `::before` rule is pruned from a Studio sheet though its element is on the page

- **Symptom:** In a sheet the Studio prunes (the Reading view's deck sheet, the Webpage export's used-selector prune), every `::before`/`::after` decoration is missing: roadmap state markers, legend discs. The same deck from the CLI is fine.
- **Cause:** The Studio's engine bundle ships `lattice.css` minified, and the minifier writes `::before` in its CSS 2 form, `:before`. css-tree parses the one-colon form as a pseudo-CLASS, so `baseSelectorString` kept it in the base and asked `querySelector` for `.x:before`, which never matches. The CLI's sheet is unminified but not free of it: it still carries KaTeX's one-colon rules (`.katex .eqn-num:before` and others).
- **Mitigation:** `collectBaseSelectors` / `prunePlayerCss` take `legacyPseudoElements`, which treats `:before`, `:after`, `:first-line` and `:first-letter` as pseudo-elements, and the Reading view's `scopeReHostedCss` opts in. Pinned in `test/unit/export/rehost-scope.test.js` and on the real surface by `docs/e2e/studio-read-article-charts.spec.ts`, which fails on 15 of 15 roadmap markers without it. The PLAYER prune does not opt in yet: that changes export bytes, so it rides with the CLI convergence (`2026-09-24-one-style-delivery-spine.md` §8 step 4).
- **Triggered by:** A prune whose input came through the Studio's engine bundle, or any sheet carrying KaTeX. The Webpage export's prune likely escapes it by failing its computed-style gate and shipping the full sheet.

## A chart's code package renders differently from the in-repo render on a symmetric layout

- **Symptom:** `test/integration/export/code-package-parity.test.js` fails for one chart with "the package differs from the in-repo render" on a slide whose layout is left–right or top–bottom symmetric. The picture looks identical by eye; a diff shows one mark a fraction of a unit off center, or two mirror-image labels emitted in the opposite order.
- **Cause:** The package runs the transform in Chromium's V8, the in-repo render in Node's. `sin`/`cos`/`hypot` may differ between the two in the last bits, so any branch that compares two values which are equal only by symmetry (`cum.findIndex((c) => c >= want)` at exactly half an arc, or a `sort` on a key such as `|y|` that ties for mirror-image marks) can go different ways in the two engines. The engines are each deterministic, so every other gate passes.
- **Mitigation:** Never branch on an exact floating-point tie. Compare with a slack scaled to the quantity (`c >= want - T * 1e-9`), and sort on a quantized key (`Math.round(prio * 1e6)`) so a stable sort falls back to authored order. hub-spoke's `ellipseWing` and `placeLabels` (`lib/components/chart/hub-spoke/hub-spoke.transform.js`) carry both.
- **Triggered by:** Any chart transform whose layout search or label ordering branches on computed geometry. Found on hub-spoke (#2396) right after #2387 added the parity test: gallery slides 2 and 7 differed by 0.64 units and by label order.
- **Commits:** `dc7b174` on #2396.
