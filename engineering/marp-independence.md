...
| VS Code live preview | **clean handoff, KNOWN CEILING** | the Export-to-Marp bundle is self-contained — anyone who wants Marp tooling exports it and runs Marp on the far side of the boundary. What the handoff delivers is now measured, not assumed: `npm run pdf` / `npm run html` in the bundle carry every component layout, the deck-wide registers, and Mermaid (marp-cli drives a real headless browser, so the runtime executes) with **six enumerated exceptions** — `lib/core/marp-fidelity.js` is the ledger and the generated bundle README prints it; the marp-vscode PREVIEW pane is palette + CSS only, because its webview executes no scripts. See §5 Cost 3 and `engineering/gotchas.md`. |
