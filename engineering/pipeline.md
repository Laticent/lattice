...
| "no browser" / Puppeteer launch failure | No Chromium resolved. The renderer tries `PUPPETEER_EXECUTABLE_PATH` (explicit, unconditional), then `CHROME_PATH` (if the file exists), then puppeteer's cache under `$HOME/.cache/puppeteer`, then `which`. The cloud sandbox's SessionStart hook exports `CHROME_PATH` automatically — if you see this, re-export it (see `engineering/development.md` § "Cloud sandbox"). Setting `PUPPETEER_EXECUTABLE_PATH` pins the browser *unconditionally*: a stale value there fails the render rather than falling back, which is deliberate. |
