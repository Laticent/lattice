---
theme: indaco
paginate: true
header: "Lattice · panes"
footer: "Panes — two components, one slide"
---

<!-- _class: title silent -->

# Two components, one slide

`Panes · proof of concept`

---

`Pipeline review · Q3`

## EMEA carried the quarter while APAC held flat.

`Regional pipeline, $M — list beside table, 40/60`

<!-- panes: 40/60 -->
<!-- pane: list -->

- EMEA closed three late deals
- APAC renewals slipped

<!-- pane: table -->

| Region | Q2 | Q3 | Δ |
|---|---|---|---|
| EMEA | 4.1 | 5.3 | +29% |
| APAC | 2.8 | 2.8 | 0% |
| Americas | 6.0 | 6.1 | +2% |

> EMEA's three late deals are the whole quarter's growth.

---

`Revenue mix · FY26`

## Services outgrew licenses for the first time.

<!-- panes: 55/45 -->
<!-- pane: bar -->

- Licenses `42`
- Services `47`
- Support `18`
- Training `6`

<!-- pane: list -->

- Services crossed licenses in March
- Support margin is the steadiest line
- Fold training into services

> The mix shift is structural, not seasonal.

---

`Field report · Lisbon`

## The new office opened on time and under budget.

<!-- panes: 50/50 no-rule -->
<!-- pane: image -->

![Office](assets/sample-photo-tall.svg)

<!-- pane: content -->

Forty desks, two studios and a roof terrace, fitted out in eleven weeks.

- 11 weeks from lease to open
- 6% under the fit-out budget
- 38 of 40 desks taken in week one

— Photo from opening week; fit-out costs are pre-audit.

---

`Unit economics · FY26`

## Payback shortened in every segment.

<!-- panes: 70/30 -->
<!-- pane: table -->

| Segment | CAC | Margin | Payback |
|---|---|---|---|
| Enterprise | $48k | 82% | 14 mo |
| Mid-market | $12k | 76% | 9 mo |
| SMB | $2.1k | 71% | 5 mo |
| Self-serve | $310 | 88% | 2 mo |

<!-- pane: big-number -->

- 9 mo
  - blended payback, down from 13

---

`Hiring · H2`

## Engineering hiring ran ahead of plan every month.

<!-- panes: stack 55/45 -->
<!-- pane: line -->

- Jul `5`
- Aug `10`
- Sep `15`
- Oct `19`
- Nov `24`

<!-- pane: stats -->

1. 24
   - hires vs plan of 20
2. 31d
   - median time to offer
3. 92%
   - offer acceptance

---

`Customer mix · FY26`

## Three segments now carry most of the revenue.

<!-- panes: 45/55 -->
<!-- pane: piechart -->

- Enterprise `46%`
- Mid-market `31%`
- SMB `23%`

<!-- pane: list -->

- Enterprise grew fastest
- Mid-market is steadiest
- SMB is the churn risk
