...
done when — a Studio Share → PDF of `lib/components/inventory/inventory/inventory.gallery.md` and of a `list-criteria` slide shows 01, 02, 03… exactly as the live preview does; an e2e spec on the exported artifact pins it (the PDF text layer or a page raster).
evidence  — seed a deck with `<!-- _class: list-criteria -->` + three `- **Lead.** body` rows into the Studio, Share → PDF, open the file.
