...
//     square, 1:1               1.000             1.051   square   ← 1.051 > 1.05
//     portrait, 4:5             0.800             0.821   tall
//     story, reel, 9:16         0.563             0.560   tall
//     mobile                    0.462             0.453   strip
//
// LOOK AT THE SQUARE ROW. It missed the old 1.05 boundary by 0.001. That is the
// whole defect: not a wild mismatch, a thousandth — which is exactly why it
// survived so long. Only `square` crossed at all; its band is 0.15 wide against a
// +0.051 drift, while `tall`'s 0.4-wide band absorbs its +0.021. That is why
...
//
//     boundary        gap between                              deck   css   midpoint
//     strip | tall    mobile 0.453 .. story 0.560               0.5   0.5      0.507
//     tall  | square  portrait 0.821 .. square 1.051            0.9   0.93     0.936
//     square| wide    square 1.051 .. standard(4:3) 1.472      1.05   1.25     1.262
//
// `standard` is why the upper one matters: at 4:3 its content box reads 1.472, so
// a square|wide boundary of 1.4 (a first attempt) would have sat just 0.07 from a
// real deck size. 1.25 leaves ~0.2 on both sides.
//
// Content-box aspects include the section's BORDER as well as its padding — the
// `border-top` spectrum strip is small but decisive at a 0.001 margin.
