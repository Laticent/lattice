...
// The generator was taught the universal budget while the GRADER became genre-relative,
// so an AI-drafted teaching deck could never reach the density its own profile grants.
const { PROFILES } = require('./deck-profiles.js');
...
function buildDeckCanon() {
  ...
  return [
    ...
    `• RESTRAINT: aim ~${SLIDE_PROSE_BUDGET.words} words of body and <= ${SLIDE_PROSE_BUDGET.bullets} bullets per slide ` +
      `(a deck declaring \`profile: teaching\` gets ${PROFILES.teaching.slideWords}). ` +
...
function buildDeckCanonShort() {
  return [
    ...
    `• RESTRAINT: aim ~${SLIDE_PROSE_BUDGET.words} words of body and <= ${SLIDE_PROSE_BUDGET.bullets} bullets per slide ` +
      `(${PROFILES.teaching.slideWords} under \`profile: teaching\`; title <= ${b.title.soft} words). When content overflows, SPLIT the slide — never shrink the font.`,
