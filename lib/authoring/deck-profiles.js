...
function withProfile(source, name) {
  ...
  if (!prof) return src;
  const line = `profile: ${prof.key}`;

  // Work on the BOM-STRIPPED body throughout, and match the front matter against THAT.
  // The first cut matched against `src` (BOM included) and then sliced `body` (BOM
  // stripped) by `m[0].length`, so every BOM'd deck lost the first character after its
  // front matter: `# Title` became ` Title`, and on BOM+CRLF the `\r` went instead,
  // leaving a bare `\n` inside a CRLF document. The offset bug was hidden behind a dead
  // ternary whose branches were identical. Reconstructing explicitly, rather than splicing
  // by computed offsets, is what makes that class of error visible.
  const bom = src.startsWith('\ufeff') ? '\ufeff' : '';
  const body = bom ? src.slice(1) : src;

  const m = FRONT_MATTER.exec(body);
  // Line endings come from the FRONT MATTER when there is one, not the whole document —
  // a single stray CRLF in the prose used to flip the front matter to CRLF and hand back a
  // mixed-ending file (engineering/decisions/2026-08-04-line-endings-lf-boundaries.md).
  const eol = /\r\n/.test(m ? m[0] : body) ? '\r\n' : '\n';

  if (!m) return `${bom}---${eol}${line}${eol}---${eol}${eol}${body}`;

  // `m[1]` is the front matter's inner text and `m[0]` the whole block including its
  // closing fence and trailing newline, so the remainder is simply what follows. The first
  // cut located the inner text with `m[0].indexOf(m[1])`, which finds the OPENING FENCE
  // when the inner text is '', '-', '--' or '---' — writing `profile: teaching---` (which
  // `BARE_NAME` accepts, so the Coach then reported its own write as invalid) and, for
  // '---', duplicating a rule line into the body as a spurious extra slide.
  const rest = body.slice(m[0].length);
  // Top-level `profile:` only — an indented one belongs to a nested mapping and is not the
  // deck's declaration, the same rule `topLevelFrontMatterValue` applies.
