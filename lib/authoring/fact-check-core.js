 ...
 *
 * THE DIVISION OF LABOR (the ADR's founding rule, mirroring ArchitectModel's
...
// lib/core/class-directive-scan.mjs. A running global `<!-- class: … -->` was
// invisible to the `_class:`-only regex this replaces.
...
function triageClaim(text, opts = {}) {
  ...
  // unverifiable, but telling public from private is a model judgment — the
...
function extractClaims(source) {
  ...
  slides.forEach((slide, idx) => {
    ...
    const component = (directives[idx]?.payload || '').trim().split(/\s+/)[0] || null;
