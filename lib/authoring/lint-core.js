...
const SCALE_CAPACITY = Object.freeze({
  ...
  'list': Object.freeze({ 6: [9, 9, 9, 8], 14: [6, 5, 4, 3] }),
        ...
        if (scaled && n > scaled.ceiling && !(cap.hard != null && n > cap.hard) && !isStressSlide) {
          ...
          findings.push({
            ...
            message: n > scaled.designed
              ...
              : `at scale-${scaleKey} (${FONT_SCALE_VALUE[scaleKey]}x) '${t}' holds about ${scaled.ceiling} ${noun} of up to ${scaled.words} words (${scaled.designed} at the designed size); this slide has ${n} — if it does not fit at ${FONT_SCALE_VALUE[scaleKey]}x, the engine renders it at the largest smaller font scale that fits rather than clip it, and the export's SCALE line names the page`,
    ...
    if (codeScale) {
      ...
      if (over.length) {
        findings.push({
          ...
          severity: clipsAtOne ? 'warning' : 'info',
          classToken: 'code',
          line: m[0],
          message: clipsAtOne
            ...
            : `at scale-${codeScale} (${FONT_SCALE_VALUE[codeScale]}x) this code block has ${over.join(' and ')}, so the slide renders at a smaller font scale than the rest of the deck to fit — nothing is clipped`,
