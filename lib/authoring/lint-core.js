...
/**
 * Whether a comment at `at` is its OWN block — the only position the engine reads.
 *
 * `lib/engine/slides.js` walks the FLAT block token stream and takes any
 * `html_block`, so what decides this is whether markdown-it started a block here.
 * A first cut tested "≤3 spaces of indent" and called that "markdown-it's
 * html_block rule". It is not: a comment inside a BLOCKQUOTE or a LIST ITEM is
 * still its own `html_block`, so the engine applies it, and the rule went silent
 * on five live directives — the same false negative it had just been fixed for.
 *
 * So the prefix may carry block markers. It may NOT carry prose: the `html_inline`
 * branch in that walk never fires for a comment inside a paragraph, because inline
 * tokens live in an `inline` token's children and the walk does not descend.
 */
const BLOCK_PREFIX = /^(?:[ \t]{0,3}(?:>|(?:[-*+]|\d{1,9}[.)])(?=[ \t]))[ \t]*)*[ \t]{0,3}$/;
 ...
 * `lib/engine/slides.js` itself calls, so case (`_TRACK:` is NOT a directive — the
 * vocabulary is case-sensitive) and the spot/global distinction come from the
 * engine rather than from a regex in this file. Two false warnings came from
 * getting exactly those two wrong by hand, each asserting something the renderer
 * had not done. It is already in this module's dependency graph through
 * `class-directive-scan.mjs`, so sharing it costs nothing on the docs route.
 */
function liveTrackDirective(slide) {
  ...
  while ((m = COMMENT_SPAN.exec(text))) {
    if (!BLOCK_PREFIX.test(text.slice(text.lastIndexOf('\n', m.index - 1) + 1, m.index))) continue;
    const dir = readDirectiveComment(m[0], TRACK_VOCAB);
    if (dir) last = { line: m[0], spot: dir.spot, value: dir.value };
  }
  return last;
}
