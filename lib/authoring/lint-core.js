 ...
 *
 * `span` is the exact inline-code span, so the editor underlines the pill itself rather
 * than the whole line it sits in. A deck that sets `inline-code: literal` draws no pills,
 * so it gets no finding.
 */
const CROWDABLE_PILL_SHAPES = new Set(['circle', 'diamond']);
function pillFitsShape(label) {
  return [...label].length <= 1 || /^\d{1,2}$/.test(label);
}
function findCrowdedShapePills(source) {
  const fmBlock = source.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?/);
  const inlineCode = fmBlock ? frontMatterValue(fmBlock[1], 'inline-code') : null;
  if (inlineCode && inlineCode.trim().toLowerCase() === 'literal') return [];
  const findings = [];
  const slides = splitTopLevel(source);
  const directives = slideClassDirectives(source);
  const fm = fmChunks(source);
  slides.forEach((slide, idx) => {
    const classToken = (directives[idx]?.payload || '').split(/\s+/).filter(Boolean)[0] || null;
    stripFencedCode(slide).split('\n').forEach((line) => {
      // SINGLE-backtick spans only: the decoder never runs on ``double`` ones.
      for (const m of line.matchAll(/(?<!`)`([^`\n]+)`(?!`)/g)) {
        const pill = resolveInlinePill(m[1]);
