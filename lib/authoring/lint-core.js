...
function applyFix(source, finding) {
  ...
  if (finding.rewriteSlide) {
    const match = new RegExp(finding.rewriteSlide.line);
    let chunkNo = 0;
    let changed = false;
    // Fence-aware, like the rule that raised the finding: a `[ ]` inside a fenced
    // example on the same slide is quoted material and is never rewritten.
    let fence = null;
    let comment = false;
    const out = lines.map((line, i) => {
      if (seps.has(i)) { chunkNo++; fence = null; comment = false; return line; }
      // Inside a multi-line HTML comment: the rule strips comments before it looks, so
      // the fix skips them. A one-line comment beside a criterion leaves that line live.
      if (!fence) {
        if (comment) {
          if (line.includes('-->')) comment = false;
          return line;
        }
        if (line.lastIndexOf('<!--') > line.lastIndexOf('-->')) { comment = true; return line; }
      }
      ...
      if (fence || chunkNo !== targetChunk || !match.test(line)) return line;
      const next = replaceToken(line, finding.rewriteSlide.from, finding.rewriteSlide.to);
...
// `split: headings` in the front matter: slides start at headings, not only at `---`.
// This module splits on `---` alone (a heading split needs a markdown parse, and lint-core
// is require-free), so one chunk can hold several slides. A slide-wide rewrite there would
// reach a neighboring slide the rule never judged — a checklist's `[ ]` rewritten as `[!]`
// — so the finding stays and the rewrite is withheld.
const SPLIT_HEADINGS = /^---\r?\n(?:(?!---\r?\n)[^\n]*\n)*?[ \t]*split:[ \t]*headings\b/;
...
function findMovedEmptyBoxes(source) {
  ...
  const directives = slideClassDirectives(source);
  const fm = fmChunks(source);
  const headingsSplit = SPLIT_HEADINGS.test(source);
  slides.forEach((slide, idx) => {
    const tokens = (directives[idx]?.payload || '').split(/\s+/).filter(Boolean);
    ...
    // `withoutHtmlComments`, the module's one comment blanker: it also blanks an
    // UNTERMINATED `<!--` to the end, as a browser hides it, so no comment text is
    // ever read as a criterion (and CodeQL's incomplete-sanitization shape is gone).
    const body = withoutHtmlComments(stripFencedCode(slide));
    if (SIX_MARKER_SLIDE.test(body) || NAMES_EMPTY_BOX.test(body)) return;
    const lines = body.split('\n').filter((l) => (rule.table ? l.includes('|') : true) && rule.line.test(l));
    if (!lines.length) return;
    const also = lines.length > 1 ? ` (${lines.length} on this slide)` : '';
    findings.push({
      slide: Math.max(1, idx - fm + 1),
      ...
      fix: rule.fix,
      short: rule.short,
      ...(rule.autofix && !headingsSplit
        ? { autofixable: true, rewriteSlide: { line: rule.line.source, from: '[ ]', to: '[!]' } }
        : {}),
    });
  });
  return findings;
