...
/**
 * Front-matter `cards:` register validation — where a sparse card row puts the height it
 * does not need (stretch / center / top / spread). Warns when the value isn't recognized,
 * so a typo (`cards: centre`) silently shipping the stretch default surfaces. Canonical
 * list injected (lib/core/resolve-cards.js `CARDS_NAMES`), keeping this core require-free.
 */
function findUnknownCards(source, cardsNames) {
  ...
  return [withTokenSuggestion({
    ...
    classToken: value,
    line: fmCards[0].trim(),
    message: `'${value}' is not a known cards value — the deck would silently keep stretching its card rows`,
