...
const { findShapeGlyphs, stripFencedCode, shapeGlyphAdvice, isQuadrantAxisEyebrow } = require('../core/shape-glyphs.js');
...
const { parseInlineSet, labelSetFor, labelSetNames } = require('../core/label-set');
...
/* ── WHICH `_track` THE ENGINE WILL ACTUALLY APPLY ───────────────────────────
 *
 * Not a regex for `<!-- _track: … -->` over a mangled copy of the slide. That
 * shape got four of the six cases below wrong, in both directions, and a warning
 * that is wrong about what the renderer did is worse than no warning — it trains
 * an author to ignore the rule. Measured through `require('./lib/engine').render`,
 * only ONE of these stamps `data-track`:
 *
 *     <!-- _track: A | [B] -->                    at a line start   → APPLIED
 *     Some text <!-- _track: A | [B] -->          inside a paragraph → inert
 *         <!-- _track: A | [B] -->                indented 4, code   → inert
 *     `<!-- _track: A | [B] -->`                  inline code        → inert
 *     <!-- outer <!-- _track: A | [B] --> tail -->  nested           → inert
 *     <!-- oops … <!-- _track: A | [B] -->        after an unclosed  → inert
 *
 * So this SCANS COMMENTS THE WAY THE ENGINE DOES — left to right, each span
 * running to the first `-->` (`parseCommentDirectives`) — and keeps a span only
 * where the whole body is the directive AND the span opens a line, which is
 * markdown-it's `html_block` rule and the reason the inline and nested forms are
 * inert. The last surviving one wins, because the engine's map overwrites.
 *
 * `withoutCodeBlocks` first, for the code forms: it is this file's own helper for
 * exactly this ("so a sample in the docs — including THIS rule's own
 * documentation — is never read as a directive"), and it handles tilde fences and
 * indented blocks besides. An earlier cut reached for `stripFencedCode`, which
 * blanks from an UNCLOSED fence to the end of the slide — so a stray ``` inside a
 * speaker note silenced the rule on a real directive after it, and the file
 * linted CLEAN on a slide whose track the engine then declined to draw.
 */
const COMMENT_SPAN = /<!--[\s\S]*?-->/g;

/**
 * How far into a line the leading BLOCK MARKERS run — the index of the first
 * character that is content, or -1 when the leading indent opens a code block.
 *
 * Two callers, one rule. `liveTrackDirective` asks whether the text BEFORE a
 * comment is nothing but markers (the only position the engine reads a directive
 * from); `htmlBlockRuns` asks where a line's markup starts so `> <div>` classifies
 * like `<div>`.
 *
 * A SCANNER, NOT A REGEX, and the reason is a denial of service I shipped here.
 * The pattern this replaces —
 *
 *     /^(?:[ \t]{0,3}(?:>|(?:[-*+]|\d{1,9}[.)])(?=[ \t]))[ \t]*)*[ \t]{0,3}$/
 *
 * — let the trailing `[ \t]*` of one iteration and the leading `[ \t]{0,3}` of
 * the next match the same run of spaces, so every gap of three-plus spaces had
 * four ways to split and a FAILING match tried all of them: 4ⁿ. Measured, with a
 * comment whose prefix is `>` plus three spaces repeated n times — it does not
 * even have to be a `_track`, because this test runs on every comment before the
 * key is read:
 *
 *     n=8   109 bytes     11 ms
 *     n=12  125 bytes    244 ms
 *     n=14  133 bytes  3,842 ms
 *     n=16  141 bytes 61,243 ms
 *
 * `lintTextWith` is the shared kernel: the Studio's CodeMirror lint source calls
 * it on the main thread, on decks that HARD RULE #22 says are untrusted. This is
 * the same hazard `lib/core/comment-directive.mjs` documents having already fixed
 * once, reopened next door. A scanner cannot backtrack, so it cannot come back.
 *
 * COLUMNS, MEASURED AGAINST THE ENGINE, not from memory of CommonMark: four
 * columns of indent opens a code block, so a bare `\t` is inert while `-\t` is
 * not, and a marker's content may start up to four columns past it.
 *
 * THE SEPARATOR IS REQUIRED, and defaulting it cost 174 false positives. An
 * earlier cut read the character after a bullet as `prefix[i + 1] || ' '`, so at
 * the END of the prefix — where the next character is not absent but the `<` of
 * the comment — the default made `-<!-- … -->` look like a list item. CommonMark
 * needs a space or tab after `-` or `1.`, the engine applies nothing there, and
 * the rule warned about a directive that was never read. The default is `''`.
 */
function blockMarkerEnd(s) {
  const TAB = 4;
  let col = 0;
  let i = 0;
  const runTo = (limit) => {
    ...
    while (i < s.length) {
      const c = s[i];
      if (c === ' ') col += 1;
      else if (c === '\t') col += TAB - (col % TAB);
      else break;
      i += 1;
      if (col - from > limit) return -1;
    }
    return i;
  };
  // Leading indent: four columns is an indented code block.
  if (runTo(3) === -1) return -1;
  while (i < s.length) {
    const c = s[i];
    const sep = (at) => s[at] === ' ' || s[at] === '\t';
    if (c === '>') { i += 1; col += 1; }
    else if ((c === '-' || c === '*' || c === '+') && sep(i + 1)) { i += 1; col += 1; }
    else if (c >= '0' && c <= '9') {
      let j = i;
      while (j < s.length && s[j] >= '0' && s[j] <= '9' && j - i < 9) j += 1;
      if (s[j] !== '.' && s[j] !== ')') break;
      if (!sep(j + 1)) break;
      col += (j - i) + 1;
      i = j + 1;
    } else break;
    // The marker's own separator plus up to three more columns of indent.
    if (runTo(TAB) === -1) return -1;
  }
  return i;
}

const blockPrefixOnly = (prefix) => blockMarkerEnd(prefix) === prefix.length;

/**
 * Line starts that are INSIDE an already-open HTML block — where a `<!-- … -->`
 * is that block's content rather than a token of its own, so the engine reads no
 * directive there.
 *
 * ONE PASS OVER THE SLIDE, and the shape is the point. The version this replaces
 * scanned BACKWARDS to the previous blank line for every comment it met, which
 * made `lintTextWith` quadratic on a comment-dense slide with no blank line in it:
 * 14 KB took 71 ms, 112 KB took 2.3 s, 330 KB took 32 s — on the same main-thread
 * CodeMirror lint source, on the same untrusted deck text, whose 4ⁿ regex this
 * same commit had just removed. Superlinearity came back through the helper added
 * beside it. A forward pass sees each line once.
 *
 * THREE BLOCK KINDS, because a blank line is not what ends all of them:
 *   · `<!-- … -->` (type 2) ends at its own `-->`, so consecutive comment lines
 *     each open their own token and a MULTI-LINE speaker note does not swallow
 *     the directive after it;
 *   · `<pre>` `<script>` `<style>` `<textarea>` (type 1) end at their CLOSING TAG
 *     and run straight through blank lines — which a blank-line model cannot see;
 *   · everything else tag-like (types 6 and 7) ends at a blank line, and a
 *     whitespace-only line counts as blank.
 *
 * KNOWN RESIDUAL, in the silent direction: a line of INLINE html with trailing
 * prose — `<b>bold</b> text` — is a paragraph to markdown-it, because type 7 wants
 * the tag alone on the line and `b` is not a type-6 tag. Telling those apart needs
 * the 62-name type-6 tag list, which is ~250 bytes gz on a route with 358 bytes of
 * headroom; this reads it as a block instead, so a directive on the very next line
 * goes unwarned. A false NEGATIVE, pinned by its own parity row.
 */
const RAWTEXT = /^<(pre|script|style|textarea)\b/i;

function htmlBlockRuns(text) {
  const inside = new Set();
  let state = null;   // null | 'comment' | 'rawtext' | 'block'
  let tag = '';
  let at = 0;
  for (const line of text.split('\n')) {
    ...
    if (state === 'comment') {
      inside.add(here);
      if (line.includes('-->')) state = null;
      continue;
    }
    if (state === 'rawtext') {
      inside.add(here);
      if (line.toLowerCase().includes(`</${tag}`)) state = null;
      continue;
    }
    if (state === 'block') {
      if (!blank) { inside.add(here); continue; }
      state = null;
    }
    if (blank) continue;
    const from = blockMarkerEnd(line);
    if (from === -1) continue;              // indented code: opens nothing
    const c = line.slice(from);
    if (c.startsWith('<!--')) {
      if (!c.slice(4).includes('-->')) state = 'comment';
    } else if (RAWTEXT.test(c)) {
      tag = RAWTEXT.exec(c)[1].toLowerCase();
      if (!c.toLowerCase().includes(`</${tag}`)) state = 'rawtext';
    } else if (/^<\/?[a-zA-Z]/.test(c)) state = 'block';
  }
  return inside;
}
...
function liveTrackDirective(slide) {
  const text = withoutCodeBlocks(String(slide));
  const inside = htmlBlockRuns(text);
  COMMENT_SPAN.lastIndex = 0;
  let last = null;
  let m;
  while ((m = COMMENT_SPAN.exec(text))) {
    const lineAt = text.lastIndexOf('\n', m.index - 1) + 1;
    if (!blockPrefixOnly(text.slice(lineAt, m.index))) continue;
    if (inside.has(lineAt)) continue;
    // THE ENGINE READS A TOKEN, NOT A COMMENT SPAN. An `html_block` runs to the
    // END OF THE LINE its `-->` sits on, so `<!-- _track: A --> tail` reaches
    // `readDirectiveComment` as the whole line, which rejects it — the comment
    // alone would have been accepted, and the rule warned about a directive the
    // renderer had ignored. Hand the reader the bytes the engine hands it.
    const lineEnd = text.indexOf('\n', m.index + m[0].length);
    const dir = readDirectiveComment(
      text.slice(m.index, lineEnd === -1 ? text.length : lineEnd), TRACK_VOCAB);
    // An EMPTY value is not applied either (`slides.js` skips it), so the slide
    // keeps its DERIVED track — warning "no track is drawn" there was false.
    if (dir?.value) last = { line: m[0], spot: dir.spot, value: dir.value };
  }
  return last;
}
...
//   Four markers, status semantics: [x] met · [-] partial · [ ] not met /
//   exempt · [/] out of scope. lib/integrations/markdown-it/plugins.js.
//
...
function findTypedShapeGlyphs(source) {
  ...
  slides.forEach((slide, idx) => {
    ...
    stripFencedCode(slide).split('\n').forEach((line) => {
      ...
      // A quadrant axis eyebrow is the component's DSL, not chrome — see above.
      if (isQuadrantAxisEyebrow(line, tokens)) return;
    ...
    for (const { context, glyphs, line, lines } of groups.values()) {
      ...
      if (context === 'cell') {
        ...
        fix = decodesPositional
          ...
          : decodesCells
            ? 'Write the state marker instead: `[x]` done · `[-]` partial · `[ ]` not met · `[/]` ' +
              'out of scope. This slide already decodes them in cells.'
            : 'Write the state marker instead — `[x]` done · `[-]` partial · `[ ]` not met · `[/]` ' +
              'out of scope — and add `state-cells` to the slide\'s class so the cells decode them ' +
...
function findLabelSetIssues(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    for (const line of stripFencedCode(slide).split('\n')) {
...
function lintTextWith(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    for (const t of tokens) {
      ...
      const renamedTo = RENAMED_CLASSES.get(t);
      if (renamedTo) {
        ...
        findings.push({
          ...
          classToken: t,
          line: m[0],
          message: `'${t}' was renamed to '${renamedTo}'`,
          fix: `Replace '${t}' with '${renamedTo}' in this slide's class. There is no alias — the old name renders as a plain \`content\` slide. The table keeps the house treatment, but loses the first-column row-label emphasis (the one change you SEE), plus its row capacity, autosplit, portrait card reshape and focus axes.`,
        });
        continue;
      }
...
function findUnknownGuards(source, guardsNames) {
  ...
  return [{
    ...
    fix: `Set front-matter \`guards:\` to one of: ${[...guardsNames].join(', ')}.`,
  }];
}
 ...
 */
const AUTHOR_SCRIPT_RE = /<script\b(?![^>]*\bsrc=)([^>]*)>([\s\S]*?)<\/script>/gi;
...
function findAuthorScriptDefers(source) {
  ...
  splitTopLevel(text).forEach((chunk, idx) => {
    ...
    for (const m of body.matchAll(AUTHOR_SCRIPT_RE)) {
      ...
      const api = m[2].match(DEFERRAL_API_RE);
      if (!api) continue;
