...
function findRetiredAutosplitDirective(source) {
  ...
  return [{
    ...
    fix: off
      ...
      : 'Remove the line — an overflowing slide is divided without asking at every presentation @size (square · portrait · story · mobile). A landscape @size never paginates: 16:9 is the box the deck is authored in.',
  }];
}
    ...
    if (vocab.capacity) {
      ...
      for (const t of tokens) {
        ...
        if (cap.hard != null && n > cap.hard) {
          ...
          //
          // Neither one claims more than it knows. `capacity` is an editorial budget measured
          // against a synthetic probe deck, not a fit predictor (2026-07-28-capacity-basis.md),
          // and since 2026-07-29 the SPLITTER does not read it at all — it splits on measured
          // overflow only. So past `hard` the honest sentence is "expect this not to fit", and
          // what follows from that is the box's business, not the count's.
          if (family === 'wide') {
            findings.push({
              slide: idx - fm + 1,
