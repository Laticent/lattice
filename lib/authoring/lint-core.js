...
/**
 * Is the text before a comment nothing but block markers — the only position the
 * engine reads a directive from?
 *
 ...
 */
function blockPrefixOnly(prefix) {
  const TAB = 4;
  let col = 0;
  let i = 0;
  const runTo = (limit) => {
    ...
    while (i < prefix.length) {
      ...
      i += 1;
      if (col - from > limit) return false;
    }
    return true;
  };
  // Leading indent: four columns is an indented code block.
  if (!runTo(3)) return false;
  while (i < prefix.length) {
    const c = prefix[i];
    if (c === '>') { i += 1; col += 1; }
    else if ((c === '-' || c === '*' || c === '+') && /[ \t]/.test(prefix[i + 1] || ' ')) {
      i += 1; col += 1;
    } else if (/\d/.test(c)) {
      let j = i;
      while (j < prefix.length && /\d/.test(prefix[j]) && j - i < 9) j += 1;
      if (prefix[j] !== '.' && prefix[j] !== ')') return false;
      if (!/[ \t]/.test(prefix[j + 1] || ' ')) return false;
      col += (j - i) + 1;
      i = j + 1;
    } else return false;
    // The marker's own separator plus up to three more columns of indent.
    if (!runTo(TAB)) return false;
  }
  return true;
}

/**
 * Is this line inside an already-open HTML block?
 *
 * A `<div>`-style (type 6) block runs to the next BLANK line, so a comment inside
 * one is part of that block's content and never its own token — the engine
 * ignores it, and the rule used to warn about it. A comment block (type 2) ends
 * at its own `-->`, so consecutive `<!-- … -->` lines each start a fresh token and
 * must NOT be swallowed by this test; that is why a preceding line only counts
 * when it is not itself a complete comment.
 */
function insideHtmlBlock(text, lineAt) {
  const runAt = text.lastIndexOf('\n\n', lineAt - 1);
  const run = text.slice(runAt === -1 ? 0 : runAt + 2, lineAt);
  return run.split('\n').some((line) => {
    const t = line.trim();
...
function liveTrackDirective(slide) {
  ...
  while ((m = COMMENT_SPAN.exec(text))) {
    ...
    if (insideHtmlBlock(text, lineAt)) continue;
