...
/**
 * Front-matter `overflow-marker:` validation.
 *
 * The register decides who the overflow signal in a rendered deck is addressed to
 * (lib/core/resolve-overflow-marker.js). An unrecognized value FALLS BACK rather
 * than throwing — a typo must never decide what a delivered artifact shows — and
 * the resolver's own comment says "deck-lint is where a typo gets named." This is
 * that. Without it the fallback was silent on every channel: `lint:deck` passed,
 * and the export then OVERWROTE the typo with the default, erasing it from the
 * artifact so nothing downstream could report it either.
 *
 * `^overflow-marker:` at column 0, matching how the resolver reads it — a key
 * nested under a mapping (`meta:`) is different config and is not this register.
 * The value is read to end-of-line and unquoted the same way, so a trailing YAML
 * comment (`off # for the board`) is flagged rather than silently ignored, which
 * is exactly what the runtime will do with it.
 */
function findUnknownOverflowMarker(source) {
  ...
  if (!m.length) return [];
  // LAST wins, as YAML does and as the resolver does.
  const raw = m[m.length - 1][1].trim().replace(/^['"]/, '').replace(/['"]$/, '');
  if (raw === '' || OVERFLOW_MARKER_LEVELS.includes(raw.toLowerCase())) return [];
  return [{
    slide: 0, rule: 'unknown-overflow-marker', severity: 'warning', classToken: 'overflow-marker',
