...
const { parseInlineSet, labelSetFor } = require('../core/label-set');
    ...
    for (const line of stripFencedCode(slide).split('\n')) {
      ...
      if (!declared) {
        findings.push({
          ...
          fix: 'Remove the span, or move it to a slide whose component has a key — ' +
               'today that is heatmap, roadmap, obligation-matrix, matrix-grid or journey.',
        });
        continue;
      }
