 ...
 * line, and the multi-line-comment flag clears on the first `-->`.
 *
 ...
 *
 * Two things about that sentence are worth keeping, because both were got wrong.
 ...
 * the fence test, the markup test, and this voiding itself. What is enforced is
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (open !== null) {
      ...
      if (!markup) {
        ...
        if (multi?.value) last = { line: t, spot: multi.spot, value: multi.value };
      } else if (readDirectiveComment(open.join('\n'), TRACK_VOCAB)?.value) blind = true;
      open = null;
      continue;
    }
    if (t === '') continue;
    ...
    if (COMMENT_LINE.test(line)) {
      ...
      if (markup) { if (readDirectiveComment(line, TRACK_VOCAB)?.value) blind = true; continue; }
