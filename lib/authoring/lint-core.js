...
/**
 * Layouts that JOINED the slot-label lift and left the inline `- **Title.** body`
 * shape behind. The lift takes an li's whole lead up to its first nested list, so
 * an inline body has nothing delimiting it and gets swallowed into the label — a
 * bare `redline` row really did lift `Why this matters. What the amendment
 * changes…` as one label, measured before these moved.
 *
 * WARN, NOT ERROR, and the difference is the point. The lift is idempotent: a deck
 * still written `- **Lead.** body` keeps its own `<strong>` and renders exactly as
 * it did, so nothing is broken and refusing the deck would be a lie about the
 * consequence. What the author gets instead is the nested shape they now want, one
 * keystroke away — the layout supplies the label, so the asterisks buy nothing.
 */
const LIFTED_INLINE_LAYOUTS = Object.freeze(['inventory', 'redline']);
        ...
        fix = decodesPositional
          ...
          : decodesCells
            ...
            : 'Write the state marker instead — `[x]` done · `[-]` partial · `[ ]` not met · `[/]` ' +
              ...
              `(e.g. \`<!-- _class: ${classToken || 'compare-table'} state-cells -->\`). The engine then ` +
...
function lintTextWith(source, vocab) {
  ...
  const liftedInline = new Set(LIFTED_INLINE_LAYOUTS);
  ...
  slides.forEach((slide, idx) => {
    ...
    // Rule 2c — a layout that joined the slot-label lift, still authored inline.
    // Not an error: the lift skips an li whose lead is already a <strong>, so the
    // old shape still renders. It just declares a label the layout now supplies.
    if (tokens.some((t) => liftedInline.has(t))) {
      const offending = findInlineTitleBodyLine(slide);
      if (offending) {
        findings.push({
          slide: idx - fm + 1,
          rule: 'lifted-inline-title',
          severity: 'warning',
          classToken: tokens.find((t) => liftedInline.has(t)),
          line: offending.trim(),
          autofixable: !!autofixNestedTitle(offending),
          message: 'inline "- **Title.** body" on a layout that now supplies the label itself — the bold buys nothing',
          fix: 'Drop the asterisks and nest the body:\n    - Title\n      - body text',
        });
      }

      // Rule 2d — the same layouts, authored FLAT: a row with no nested body.
      // This is the shape rule 2c's autofix produces half of, and it is the one
      // that bites hardest here. The lift takes an li's whole lead up to its
      // first nested list, so a row with no nested list at all has its ENTIRE
      // sentence lifted into the label — on `inventory` that renders a whole
      // clause at 700/display/--fs-h3, which is a mis-render an author only sees
      // by looking. Rule 2c cannot catch it: there are no asterisks to notice.
      // Same detector the split-slot and number-slot rules use (rules 4 and 8),
      // which exist for the mirror-image failure on layouts whose lift DOES
      // require a nested body.
      const bodyless = findSplitBodylessItem(slide);
      if (bodyless && !findInlineTitleBodyLine(slide)) {
        findings.push({
          slide: idx - fm + 1,
          rule: 'lifted-bodyless-item',
          severity: 'warning',
          classToken: tokens.find((t) => liftedInline.has(t)),
          line: bodyless.trim(),
          message: 'a row with no nested body on a layout that lifts its label — the whole line becomes the header',
          fix: 'Nest the detail under the label:\n    - Label\n      - detail sentence',
        });
      }
    }
