 ...
 *
 * DELIBERATELY NARROWER THAN THE ENGINE, and that is the whole design. Eight review
 ...
 *
 * WHAT MAKES THE WHOLE SET QUIET-ONLY IS THE SIXTH: any directive-looking line
 * this rule does not READ voids the slide's answer. Without that, poison was not
 * quiet-only at all — it was LOUDER. The rule returns the LAST directive it
 * accepts, so declining a later one promoted an earlier one, and where the earlier
 * was degenerate and the later was not, silence became a false warning. Two
 * separate rounds asserted "over-broad only declines more" of conditions that did
 * not have that property; this is the line that finally gives it to them. Being
 * over-broad is free only once a decline cannot change WHICH directive answers.
...
function isTableDelimiter(line) {
  ...
  const space = (c) => c === ' ' || c === '\t';
  let i = 0;
  while (i < s.length && (space(s[i]) || s[i] === '>')) i += 1;
  if (s[i] === '|') i += 1;
  let cells = 0;
  while (i < s.length) {
