 ...
 * directive comments, since the engine treats them the same way.
 */
const PAGINATE_UNSUPPORTED_RE = /(?:^|<!--)[ \t]*(_?paginate)[ \t]*:[ \t]*(skip|hold)\b/im;
...
function findUnsupportedPaginateValues(source) {
  ...
  splitTopLevel(text).forEach((chunk, idx) => {
    // Strip fenced code before scanning — the same guard countPrimaryCollection and
    // findBigNumberHeroInHeading use. A `_paginate: skip` inside a code sample is
    // DOCUMENTATION, not a directive, and this rule's own docs entry is exactly that.
