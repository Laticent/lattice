...
// Blanks HTML comments and RAWTEXT while preserving offsets — the same reader the
// render path uses to tell markup from text that merely looks like it.
const { maskInert } = require('../core/top-level-h2');
...
// `[\s\S]*?`, not `[^>]*?`: a label may carry a `>` (`a > b`), and a value-class
// matcher that cannot cross one simply does not match — so the two arms this rule
// exists to catch, a scale of one and a track with nothing lit, went unreported on
// exactly the labels most likely to be malformed. The engine's own reader
// (lib/engine/directives.js) runs to the first `-->` and so does this.
//
// GLOBAL, and the LAST match is the live one. A slide carrying two `_track` lines
// resolves last-wins in the engine (`parseCommentDirectives` overwrites), so
// reading the first reported a finding about a value the renderer ignored — and
// its message asserted "NO track is drawn" about a slide that drew one. A warning
// that is wrong about what the reader saw is worse than no warning.
const TRACK_DIRECTIVE = /<!--\s*_track:\s*([\s\S]*?)\s*-->/g;

/** The `_track` the ENGINE will apply — the last one on the slide, or null. */
function liveTrackDirective(slide) {
  TRACK_DIRECTIVE.lastIndex = 0;
  let last = null;
  let m;
  while ((m = TRACK_DIRECTIVE.exec(slide))) last = m;
  return last;
}
...
const renderedBody = (slide) => maskInert(stripFencedCode(slide));

// What the ENGINE will read as DIRECTIVES: comments kept, fenced code dropped, so
// a slide that documents the syntax in a fence is not warned about its own
// example.
const directiveText = (slide) => stripFencedCode(slide);
...
function lintTextWith(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    const td = liveTrackDirective(directiveText(slide));
    if (td) {
