...
const TRACK_BODY = /^_?track\s*:\s*([\s\S]*)$/i;
...
function liveTrackDirective(slide) {
  ...
  while ((m = COMMENT_SPAN.exec(text))) {
    // `html_block` position: the comment opens a line, with markdown-it's ≤3
    // spaces of indent. Four or more is a code block, and `withoutCodeBlocks`
    // has already taken those out.
    const indent = text.slice(text.lastIndexOf('\n', m.index - 1) + 1, m.index);
    if (!/^[ \t]{0,3}$/.test(indent)) continue;
    const body = TRACK_BODY.exec(m[1].trim());
    if (body) last = [m[0], body[1].trim()];
  }
  return last;
}
...
function lintTextWith(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    if (td) {
      const { labels, current } = parseTrackSpec(td[1]);
