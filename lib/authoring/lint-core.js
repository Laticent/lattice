 ...
 *
 * DELIBERATELY NARROWER THAN THE ENGINE, and that is the whole design. Six review
 * rounds each replaced a hand-written model of markdown-it's `html_block` rule
 ...
 * three block types never modelled at all. Writing a fifth of CommonMark §4.6
 * from memory is the defect, not any one of its bugs.
 *
 * So this rule no longer tries to AGREE with the engine. It tries to be a SUBSET
 * of it: everything it warns about, the engine really applies; plenty that the
 * engine applies, it says nothing about. For an advisory rule that is the right
 * asymmetry — HARD RULE #29's posture is warn and coach, and a warning about a
 * directive the renderer never read is worse than silence, because it sends the
 * author to fix something that is not broken.
 *
 * The subset is four conditions, each of which can only make it quieter:
 *   · the comment opens its own line, at most three spaces in — no blockquote, no
 *     list item, no tabs, so nothing about container indentation matters;
 *   · it CLOSES on that line, with `-->` last — an `html_block` runs to the end of
 *     its line, so trailing prose makes it a token the reader rejects;
 *   · a multi-line comment above it is tracked open until a line carries `-->`
 *     (that much is one flag, and speaker notes are common enough to earn it);
 *   · and ANY other markup line above it in the slide — a tag, `<!`, `<?`, a
 *     closing tag — declines the rest of the slide, because it may have opened a
 *     block this rule does not model. It never tries to work out whether that
 *     block closed.
 *
...
function liveTrackDirective(slide) {
  let last = null;
  let open = false;     // inside a multi-line comment
  let fence = '';       // inside a fenced code block, holding its opening run
  ...
  // turns an UNCLOSED ``` into a single backtick, so a fence test over its output
  // cannot see the one fence that matters. Fences are tracked here instead.
  for (const line of String(slide).split('\n')) {
    const t = line.trim();
    if (open) { if (t.includes('-->')) open = false; continue; }
    ...
    if (COMMENT_LINE.test(line)) {
      ...
      if (!t.includes('-->')) { open = true; continue; }
      if (markup) continue;
      ...
      if (dir?.value) last = { line: t, spot: dir.spot, value: dir.value };
      continue;
    }
    if (MARKUP_LINE.test(t)) markup = true;
  }
  return last;
}
