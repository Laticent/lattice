...
function findBadRenderTargetValues(source) {
  // `\uFEFF?` and `\r?\n` on both sides, matching findUnknownPace: a BOM'd or CRLF deck
  // has front matter like any other, and a reader that silently sees none reports nothing
  // for a deck that carries a key.
  ...
  for (const key of RENDER_TARGET_KEY_NAMES) {
    ...
    // The raw line, for the report — read the same loose way the kernel reads the value,
    // so the line quoted back is the line that was actually consulted.
    ...
    findings.push({
      ...
      severity: 'warning',
      classToken: value,
      line: line ? line[0].trim() : `${key}: ${value}`,
