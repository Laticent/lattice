 ...
 * (and the only one with `system` / `inherited`). Info-severity, and suppressed when
 * the deck already carries a `color-mode:` key (the key wins; the alias is inert then).
 */
function findDeprecatedClassColorMode(source) {
  const fmBlock = source.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?/);
  if (!fmBlock) return [];
  const body = fmBlock[1];
  if (/^\s*color-mode:\s*\S/m.test(body)) return []; // key present → alias inert, no nudge
