...
/**
 * `overflow-marker:` in a deck's front matter does NOTHING — flag it.
 *
 * It shipped as a deck register for one commit and was moved to an export setting:
 * the level is a property of the render target, not an authoring fact (one source
 * is previewed, exported, and printed, and those want three different answers —
 * engineering/decisions/2026-07-30-overflow-marker-register.md). Nothing reads the
 * key any more.
 *
 * This rule exists because the alternative is silence, and silence is exactly what
 * the move was meant to end. An exported bundle used to carry the key in its front
 * matter, so a recipient — or an author following an older doc — can reasonably
 * have one sitting in a deck, believing it does something. Say so, and name where
 * the setting actually lives.
 *
 * `^overflow-marker:` at column 0: a key nested under a mapping (`meta:`) is
 * different config and is not this.
 */
