...
/**
 * Blank every `<!-- … -->` span, keeping newlines so line indices still match
 * the source. An index scan rather than a lazy regex: an unclosed `<!--` repeated
 * down untrusted markdown (the browser linter, HARD RULE #22) makes a lazy
 * `[\s\S]*?` quadratic, and this cannot be.
 */
function blankCommentSpans(text) {
  ...
  for (;;) {
    ...
    // Only what markdown-it treats as a comment: an HTML block opens at a line
    // start (up to three spaces of indent) and may run unclosed to the end; an
    // inline one needs its `-->`. Anywhere else — `<!--` quoted in a code span
    // or in prose — it is text, and blanking from it hid the rest of the slide.
    const lineStart = src.lastIndexOf('\n', open - 1) + 1;
    const atLineStart = /^ {0,3}$/.test(src.slice(lineStart, open));
    const inCode = (src.slice(lineStart, open).match(/`/g) || []).length % 2 === 1;
    if (inCode || (!atLineStart && close < 0)) { from = open + 4; continue; }
    const end = close < 0 ? src.length : close + 3;
    out += src.slice(at, open) + src.slice(open, end).replace(/[^\n]/g, ' ');
    at = end;
    from = end;
  }
