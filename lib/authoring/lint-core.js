...
const TRACK_DIRECTIVE = /<!--\s*_track:\s*([^>]*?)\s*-->/;
...
function lintTextWith(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    // Rule 12 — the `topic` anchor's `_track` override.
    const td = slide.match(TRACK_DIRECTIVE);
    if (td) {
    ...
    if (tokens.includes('topic') && TOP_LEVEL_BULLET.test(stripFencedCode(slide))) {
      findings.push({
        slide: idx - fm + 1,
        rule: 'track-list',
        severity: 'warning',
        line: (stripFencedCode(slide).match(TOP_LEVEL_BULLET) || [''])[0].trim(),
