...
const { readAxisMember } = require('../core/axis-member');
...
function retiredQuadrantAxis(text) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const m = side.match(rangeTail);
    const name = (m ? side.slice(0, m.index) : side).trim();
    const parts = [];
    if (m && Number(m[2]) > Number(m[1])) parts.push(`${m[1]}..${m[2]}`);
    ...
    slideLines.forEach((line, lineIdx) => {
      ...
      if (axis && bodyAt >= 0 && lineIdx < bodyAt) {
        ...
        const vocabKeys = declared
          ...
          : null;
        if (!vocabKeys || !set.some((e) => vocabKeys.has(String(e.key).trim()))) return;
        findings.push({
          slide: Math.max(1, idx - fm + 1),
...
function findQuadrantAxisIssues(source) {
  ...
  slides.forEach((slide, idx) => {
    ...
    const lines = stripFencedCode(slide).split('\n');
    let seenAxis = false;
