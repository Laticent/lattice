 ...
 * every layout. Two layouts used to read `[ ]` their own way, and a deck written
 * before the change still says what it meant then:
 *
 *   verdict-grid       `[ ]` was "not met" (red ✕). It is now the open ring, "not
 *                      assessed". "Not met" is `[!]`.
 *   obligation-matrix  `[ ]` was "exempt". It is now "undetermined". Exempt is `[/]`.
 *
 * The render cannot tell which reading the author wanted, so the linter says what the
 * slide now shows and names the marker for the other reading. ONE finding per slide:
 * a matrix of twelve empty cells is one decision, not twelve.
 * engineering/decisions/2026-09-24-six-state-marks.md §7.
 */
const MOVED_EMPTY_BOX = Object.freeze({
  'verdict-grid': {
    now: 'the open ring, "not assessed"',
    was: 'the red cross, "not met"',
    fix: 'If a criterion was not met, write `[!]`, which draws the red cross. Keep `[ ]` ' +
      'for a criterion nobody has assessed yet.',
    // A criterion is a NESTED list item.
    line: /^\s+(?:[-*+]|\d+[.)])\s+\[ \]/,
  },
  'obligation-matrix': {
    now: '"undetermined"',
    ...
    fix: 'If the regime is exempt, write `[/]`, which the key names "Exempt". Keep `[ ]` for ' +
      'an obligation nobody has determined yet.',
    line: /^\s*\|.*\|\s*\[ \]\s*(?=\|)/,
  },
});
...
function findMovedEmptyBoxes(source) {
  ...
  slides.forEach((slide, idx) => {
    ...
    // A label set that names `[ ]` on this slide (`{[ ], Controlled}`) is the author saying
    // what the open box means HERE — the question this rule asks is already answered.
    if (/\{\s*\[ \]\s*,/.test(body)) return;
    const lines = body.split('\n').filter((l) => rule.line.test(l));
