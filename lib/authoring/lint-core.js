...
/**
 * Authoring warning: a single-LETTER-or-DIGIT `lexicon:` key (`e:`, `s:`, `2:`). The
 * read-aloud built-in (the Speech Symbol Commons) matches a lexicon entry per code
 * point, so a one-character alphanumeric key rewrites EVERY embedded occurrence — a
 * key `e: EEK` garbles "revenue" into "r EEK v EEK n u EEK", mis-narrating the whole
 * deck. A single GLYPH key (`→`, `×`, `©`) is the intended use and never fires (it
 * isn't a letter/digit). Warn, don't block — a lone letter may be deliberate (rare).
 * Self-contained: it parses the nested `lexicon:` block itself (mirroring parseTokenMap
 * in lib/core/resolve-captions.mjs), so it needs no injected vocab. One finding per key.
 */
function findSingleLetterLexiconKeys(source) {
  ...
  for (i += 1; i < lines.length; i++) {
    ...
    if (entryIndent < 0) entryIndent = indent; // first child sets the entry indent
    if (indent !== entryIndent) continue; // a deeper stray line is not an entry key
