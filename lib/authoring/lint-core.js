...
// Pure + fs-free, so the browser bundle contract above holds. It is the ONE reader of
// the deck-level `autosplit:` directive, shared with lattice-emulator.js.
const { autosplitEnabled, autosplitDirective, ON_RE } = require('../core/autosplit-flag');
 ...
 * opts in, so the flag is never a silent no-op. Universal rule — the boundary is
 * the `wide` family, derived from the resolved `@size` geometry.
 */
function findAutosplitOrientationMismatch(source, vocab) {
  const fmMatch = String(source || '').match(/^---\n[\s\S]*?\n---/);
  if (!fmMatch) return [];
  const fm = fmMatch[0];
  // The EXPLICIT opt-in only. Since #1234 the directive defaults ON, so a landscape
  // deck that says nothing is not making a mistake — it is taking a default that does
  // not apply to it, silently and correctly. Warning there would fire on every
  // landscape deck in the catalog. What is still worth saying is "you WROTE
  // `autosplit: on` and it does nothing", which is a claim about the directive, not
  // about the resolved behavior — hence `autosplitDirective`, not `autosplitEnabled`.
...
function lintTextWith(source, vocab) {
  ...
  // Shared with the engine (HARD RULE #1) — this was a second copy of the emulator's
  // regex, and the two would have silently disagreed the moment the default flipped:
  // every portrait deck that says nothing now SPLITS, so telling its author their
  // over-capacity slide merely "overflows" would be the lie-to-the-author defect
  // again. Default ON for non-landscape; `autosplit: off` opts out.
  ...
  slides.forEach((slide, idx) => {
    ...
    if (vocab.capacity) {
      ...
      // specimenVoice) suppresses capacity-crowd alone — capacity-overflow
      // still fires, so a stress slide past `hard` remains a real warning.
      ...
      for (const t of tokens) {
        ...
        if (cap.hard != null && n > cap.hard) {
          ...
          // The slide will NOT overflow, so this is not a warning — but the budget must
          // still speak, or an author gets no signal at all that a slide is past what the
          // component holds and is about to become several (the hole that let a 14-item
          // checklist ship as a badly-paced run). Report what will actually happen, at the
          // advisory `info` tier so a deliberate split doesn't red `lint:deck --strict`.
