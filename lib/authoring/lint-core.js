 ...
 * and it covers the one case the runtime probe structurally CANNOT — a
 * `<script type="module">`, where `document.currentScript` is null, so the probe has
 * nobody to attribute the timer to and stays silent.
 *
 * SCOPE, and both edges are deliberate:
 ...
 * Code fences are blanked first, so this rule's own documentation does not trip it.
 */
const AUTHOR_SCRIPT_RE = /<script\b(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/gi;
const DEFERRAL_API_RE =
  /\b(setTimeout|setInterval|requestAnimationFrame|requestIdleCallback|XMLHttpRequest)\b|\bfetch\s*\(|\.then\s*\(|\bawait\s/;
...
function findAuthorScriptDefers(source) {
  ...
  splitTopLevel(text).forEach((chunk, idx) => {
    ...
    for (const m of body.matchAll(AUTHOR_SCRIPT_RE)) {
      ...
      if (/\btype\s*=\s*["'][^"']*json/i.test(m[0])) continue;
      const api = m[1].match(DEFERRAL_API_RE);
      if (!api) continue;
      const call = (api[1] || api[0]).replace(/[\s(]+$/, '');
      findings.push({
        slide: Math.max(1, idx - fm + 1),
        ...
        line: `<script> … ${call} …`,
        message:
          `this inline \`<script>\` defers work with \`${call}\` — the export captures at the load event and does not wait for it, so whatever the callback writes will be MISSING from the PDF/PPTX/PNG`,
        fix:
