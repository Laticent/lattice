 ...
 */
const FONT_SCALE_KEYS = Object.freeze({ 'scale-l': 'l', 'scale-xl': 'xl', 'scale-2xl': '2xl' });
...
function lintTextWith(source, vocab) {
  ...
  const deckClassTokens = fmClaimBlock ? deckClassTokensFromFrontMatter(fmClaimBlock[1]) : [];
  ...
  slides.forEach((slide, idx) => {
    ...
    if (vocab.capacity) {
      ...
      for (const t of tokens) {
        ...
        const scaleKey = family === 'wide' ? fontScaleKey([...tokens, ...deckClassTokens]) : null;
        ...
        if (scaled && n > scaled.ceiling && !(cap.hard != null && n > cap.hard) && !isStressSlide) {
          ...
          findings.push({
            ...
            severity: slideFitLevel(tokens) === 'report' ? 'warning' : 'info',
            classToken: t,
            line: m[0],
            ...
            message: n > scaled.designed
              ...
              : slideFitLevel(tokens) === 'report'
                ? `at scale-${scaleKey} (${FONT_SCALE_VALUE[scaleKey]}x) '${t}' holds about ${scaled.ceiling} ${noun} of up to ${scaled.words} words; this slide has ${n}, and \`fit: report\` stops the engine from stepping it down — if it does not fit at ${FONT_SCALE_VALUE[scaleKey]}x, it is clipped`
                : `at scale-${scaleKey} (${FONT_SCALE_VALUE[scaleKey]}x) '${t}' holds about ${scaled.ceiling} ${noun} of up to ${scaled.words} words (${scaled.designed} at the designed size); this slide has ${n} — if it does not fit at ${FONT_SCALE_VALUE[scaleKey]}x, the engine renders it at the designed size (1x) rather than clip it, and the export's SCALE line names the page`,
            fix: `To be sure of the full scale, keep ${scaled.ceiling} ${noun} or fewer, or split the slide. ${capacityFix(cap)}`,
          });
    ...
    const codeScale = family === 'wide' && tokens.includes('code') && !stageResized
      && !/<!--\s*stress-slide\s*-->/.test(slide)
      ? fontScaleKey([...tokens, ...deckClassTokens]) : null;
    if (codeScale) {
      ...
      if (over.length) {
        findings.push({
          ...
          severity: clipsAtOne || codeReport ? 'warning' : 'info',
          classToken: 'code',
          line: m[0],
          message: clipsAtOne
            ...
            : codeReport
              ? `at scale-${codeScale} (${FONT_SCALE_VALUE[codeScale]}x) this code block has ${over.join(' and ')}, and \`fit: report\` stops the engine from stepping the slide down — it is clipped`
              : `at scale-${codeScale} (${FONT_SCALE_VALUE[codeScale]}x) this code block has ${over.join(' and ')}, so the slide renders at the designed size (1x), smaller than the rest of the deck — nothing is clipped`,
...
function findFitRegister(source, fitNames) {
  ...
  if (guards) {
    ...
    out.push({
      ...
      fix: to ? `Write \`fit: ${to}\` instead${to === 'heal' ? ' — or drop the line; heal is the default' : ''}.` : `Write \`fit:\` with one of: ${[...fitNames].join(', ')}.`,
    });
  }
  return out;
}
