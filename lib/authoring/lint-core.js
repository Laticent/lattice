...
function findLabelSetIssues(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    const bodyOpens = (l) => (axis?.body === 'table' ? l.trimStart().startsWith('|') : /^\s*[-*+]\s/.test(l));
    const slideLines = stripFencedCode(slide).split('\n');
    const bodyAt = axis ? slideLines.findIndex(bodyOpens) : -1;
    ...
    slideLines.forEach((line, lineIdx) => {
      ...
      if (axis && bodyAt >= 0 && lineIdx < bodyAt) {
        findings.push({
          ...
          line: line.trim(),
          message: `this label set sits ABOVE the ${axis.body}, where \`${component}\` reads the AXIS — it will be used to name axes, not to rename the key`,
          fix: `Move the span below the ${axis.body}. Position is what separates the two: a bracketed list above names the axes (${axis.members.join(', ')}), one below renames the key.`,
        });
      }
      if (!declared) {
        findings.push({
