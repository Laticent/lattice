...
const CODE_LINE_BUDGET = Object.freeze({
  ...
  // RE-MEASURED after `code` took its own type step: it reads `--fs-body-compact`
  // (1.40cqi) where the rest of the deck's chrome reads `--fs-meta` (1.17cqi), so
  // every column got ~20% wider and the budgets fell by the same ratio. Measured
  // the same way as the originals — a 100-character fence rendered through the
  // real pipeline, pane width over character advance, fonts loaded: wide 1104px /
  // 10.744px = 102. `compare-code` is untouched because it kept `--fs-meta`.
  code: Object.freeze({ wide: 102, square: 58, tall: 37, strip: 36 }),
});
