 ...
 * the opening tag and then locating the end the way the parser does (`endOfScriptBody`)
 * removes the span rather than re-spelling it, and it is the idiom already used for the
 * other script scan in the tree (lib/export/player-core.mjs).
 */
const AUTHOR_SCRIPT_OPEN_RE = /<script\b(?![^>]*\bsrc=)([^>]*)>/gi;

// Where a `<script>`'s RAWTEXT body ends. The HTML tokenizer leaves RAWTEXT on `</script`
// followed by whitespace, `/` or `>` — so all four spellings terminate, and a run like
// `</scriptish` does not. An unclosed element runs to the end of the chunk, which is what
// the browser does with it too.
const SCRIPT_END_RE = /<\/script(?=[\s/>]|$)/gi;
function endOfScriptBody(text, from) {
  SCRIPT_END_RE.lastIndex = from;
  const end = SCRIPT_END_RE.exec(text);
  return end ? end.index : text.length;
}
...
function findAuthorScriptDefers(source) {
  ...
  splitTopLevel(text).forEach((chunk, idx) => {
    ...
    for (let m = AUTHOR_SCRIPT_OPEN_RE.exec(body); m; m = AUTHOR_SCRIPT_OPEN_RE.exec(body)) {
      ...
      const bodyEnd = endOfScriptBody(body, bodyStart);
      // Resume AFTER this element, so a `<script>` quoted inside another one's body cannot
      // be read as a second element. (The span form got this for free; the search form has
      // to say it.)
      AUTHOR_SCRIPT_OPEN_RE.lastIndex = bodyEnd;
