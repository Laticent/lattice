 ...
 *
 * Six conditions. TWO ARE STICKY POISON — `FENCE_LINE` and `MARKUP_LINE` set a
 * flag that declines every later line — and a checker caught an earlier version of
 * this paragraph calling four of them that. `COMMENT_LINE` declines only its own
 * line, and a multi-line comment is collected and PARSED on its closing `-->`,
 * not merely tracked.
 *
 * WHAT KEEPS THE SET QUIET IS THE SIXTH: a directive this rule declines to read
 * voids the slide's answer. Without it, poison was not quiet-only at all — it was
 * LOUDER. The rule returns the LAST directive it accepts, so declining a later one
 * PROMOTED an earlier one, and where the earlier was degenerate and the later was
 * not, silence became a false warning. Being over-broad is free only once a
 * decline cannot change WHICH directive answers.
 *
 * Three things about that sentence are worth keeping, because all three were got
 * wrong in consecutive rounds.
 * It voids only for a line that would ITSELF have been applied — blinding on every
 * skipped comment cost 720 extra silences on one-directive slides, where a `<div>`
 * plus a speaker note hid a real defect. And a MULTI-LINE comment is read rather
 * than declined, because `readDirectiveComment` takes the whole block exactly as
 * the engine does; declining those would go quiet on every deck with a note in it.
 *
 * And "would itself have been applied" means IS A DIRECTIVE, not has a non-empty
 * value. `<!-- _track: -->` is applied, supersedes everything above it, and leaves
 * the slide with no authored track — so it CLEARS the answer. Testing the value's
 * truthiness promoted the earlier directive instead, which is the same failure one
 * door along.
 *
 ...
 * the fence test, the markup test, and the voiding itself. What is enforced is
 * the generated fuzz below; nothing here proves anything, and the next
 * counterexample is one axis the generator cannot yet express. That has been true
 * ten times.
 ...
 *     whole, because that is what the engine does with the block;
...
// A DIRECTIVE WITH AN EMPTY VALUE CLEARS THE ANSWER, it does not fail to be one.
...
function liveTrackDirective(slide) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (open !== null) {
      ...
      if (multi) { if (markup) blind = true; else last = keep(t, multi); }
      open = null;
      continue;
    }
    if (t === '') continue;
    ...
    if (COMMENT_LINE.test(line)) {
      ...
      if (!t.includes('-->')) { open = [line]; continue; }
      // ONLY A REAL DIRECTIVE VOIDS. Blinding on every skipped comment cost 720
      // extra silences on ONE-directive slides (+38%) — a `<div>` plus a speaker
      // note below a degenerate `_track` went quiet, which is the exact failure
      // this rule exists to report. The line is readable even when declined, so
      // ask: only a line that would itself have been applied can supersede.
      ...
      if (dir) last = keep(t, dir);
      continue;
    }
    if (MARKUP_LINE.test(line)) markup = true;
  }
  return blind ? null : last;
}
