 ...
 *
 * Five conditions. FOUR ARE POISON — they set a sticky flag and decline the rest
 * of the slide, so they can only ever make the rule quieter, whatever they get
 * wrong. That shape is the lesson: being over-broad is free, being too narrow is
 * the bug.
...
// the pass looks DOWN, and it is the minimum lookahead that closes the family.
...
function liveTrackDirective(slide) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (COMMENT_LINE.test(line)) {
      ...
      if (t.includes('|') && TABLE_DELIMITER.test(lines[i + 1] ?? '')) continue;
