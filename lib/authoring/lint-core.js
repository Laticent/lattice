 ...
 * closing, image, premise, scene, split-panel, split-compare, compare-code), which
...
function findRetiredFormTokens(source) {
  ...
  lines.forEach((line, i) => {
    const m = line.match(/<!--\s*_?class:\s*([^>]*?)\s*-->/);
    if (!m) return;
    for (const tok of ['no-form', 'form']) {
      ...
      findings.push({
        ...
        fix: tok === 'no-form'
          ? 'Delete `no-form`. If this slide should carry no chrome, give it a sovereign component instead — `title`, `divider`, `closing`, `image`, `premise`, `scene`, `split-panel`, `split-compare` or `compare-code`.'
