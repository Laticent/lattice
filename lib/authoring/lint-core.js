...
// The registered `@size` names that are not landscape (social/mobile presets +
// aspect aliases), and the adaptive FAMILY each one lands in. This module is
// pure and browser-safe (HARD RULE #7), so it cannot read the theme's `@size`
// table itself — `vocab.sizes` carries the parsed geometry when the caller has
// it (lib/authoring/lint.js injects it via the engine's own parseSizes), and
// this list is the fallback for the browser/no-vocab path.
//
// The list is a LAST RESORT, not the source of truth: a theme that registers a
// new `@size` is invisible to it, which is the same class of drift that made
// every square reflow rule inert (#1218). Derive from `vocab.sizes` wherever it
// is available.
const SIZE_FAMILY_FALLBACK = Object.freeze({
  square: 'square', '1:1': 'square',
  portrait: 'tall', '4:5': 'tall',
  story: 'tall', reel: 'tall', '9:16': 'tall',
  mobile: 'strip',
});
 ...
 * Derived from the resolved `@size` geometry when `vocab.sizes` is injected —
 * the same classifier the engine and runtime stamp with (lib/adaptive/families.js)
 * — falling back to the name table above.
 */
...
function findRetiredAutosplitDirective(source) {
  ...
  return [{
    ...
    line: hit[0].trim(),
    message: off
      ? 'autosplit: off is retired — splitting is intrinsic, so this deck WILL paginate an overflowing slide despite this line.'
    ...
    fix: off
      ...
      : 'Remove the line — an overflowing slide is divided at every @size without asking.',
  }];
}
...
function lintTextWith(source, vocab) {
  const findings = [];
  // Over-capacity slides are DIVIDED at export, at every @size, so the capacity-OVERFLOW
  // warning is moot — it is replaced by the `capacity-autosplit` advisory, which reports
  // what will actually happen. The milder capacity-crowd (soft < n <= hard) still applies:
  // it is an editorial judgment about a crowded slide, not a prediction of overflow. See
  // lib/core/auto-split.js + engineering/decisions/2026-07-29-autosplit-is-not-a-toggle.md.
  ...
  slides.forEach((slide, idx) => {
    ...
    if (vocab.capacity) {
      ...
      // specimenVoice) suppresses capacity-crowd. Past `hard` the slide is SPLIT
      // rather than warned about — except that a stress slide is a specimen and
      // marks itself so the splitter leaves it whole (see the marker's other half
      // in lib/core/auto-split.js).
      ...
      for (const t of tokens) {
        ...
        if (cap.hard != null && n > cap.hard) {
          // Splitting is INTRINSIC (2026-07-29), so an over-`hard` slide is DIVIDED at
          // export, at every @size — it will not overflow, and `capacity-overflow` is
          // therefore unreachable and gone (it used to fire in the landscape case, back
          // when the split move was skipped there). The budget must still speak, or an
          // author gets no signal that a slide is past what the component holds and is
          // about to become several (the hole that let a 14-item checklist ship as a
          // badly-paced run). Report what will actually happen, at the advisory `info`
          // tier so a deliberate split doesn't red `lint:deck --strict`.
          //
          // …unless the slide is a SPECIMEN. `<!-- stress-slide -->` makes the splitter
          // leave it whole (lib/core/auto-split.js), so promising "auto-split will divide
          // this into N pages" would describe a split that never happens — the same
          // lie-to-the-author defect this advisory exists to fix, pointed the other way.
          // A specimen author has already declared they know; say nothing.
          if (isStressSlide) continue;
          {
            ...
            findings.push({
              ...
              fix: hasHeadline
                ? `Intended? Nothing to do — the split leads with a cover and ${evenness}. ${trim}`
                : `Intended? ${evenness.charAt(0).toUpperCase()}${evenness.slice(1)}, but this slide has no \`## \` headline, so the run gets no cover page to open on — add one. ${trim}`,
            });
            continue;
          }
    ...
    // Rule — a deck-wide finish silently covers a title/closing bookend. The
    // bookends are inverse "moments" (display-white text on the inverse
    // surface); a deck `finish:` paints its backdrop over that surface, and on a
    // light canvas the display text washes out (the house pattern is
    // `finish-none` on bookends — see examples/finish-backdrops.md). Warn UNLESS
    // the slide makes an explicit finish choice (finish-none to opt out, or an
    // explicit finish-<name> if the finish there is intended).
 ...
 * (and the only one with `system` / `inherited`). Info-severity. The message adapts:
 * with NO `color-mode:` key it nudges migration; with a `color-mode:` key ALSO present
 * (a half-migrated deck) it flags the now-superseded, redundant `class:` for removal —
 * the key wins the render (the propagation kernels strip the alias), so the leftover
 * `class:` only muddies the source.
 */
function findDeprecatedClassColorMode(source) {
  ...
  return [{
    ...
    line: cm[0].trim(),
    message: hasKey
      ? `\`class: ${t}\` is superseded by the \`color-mode:\` key present in this deck — the key wins, so this legacy color token is redundant`
      : `\`class: ${t}\` is the legacy color axis — prefer the first-class \`color-mode: ${t}\` (which also offers system / inherited)`,
    fix: hasKey
      ? `Remove the redundant \`${t}\` from the deck-wide \`class:\` (the \`color-mode:\` key already governs color).`
      : `Replace the deck-wide \`class: ${t}\` with \`color-mode: ${t}\`.`,
  }];
}
