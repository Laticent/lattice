...
// Slides whose `<td>` cells already decode `[x]`/`[-]`/`[ ]`/`[/]` into a state
// disc. `state-cells` is the universal opt-in that widens the same decoder to
// any table — it is the ANSWER this rule points a typed ✓ at, so the list has to
// stay in step with lib/integrations/markdown-it/plugins.js.
const STATE_CELL_TOKENS = Object.freeze(new Set(['obligation-matrix', 'matrix-grid', 'state-cells']));

// A `quadrant` slide's axis eyebrow is OUT OF SCOPE, and the reason is measured
// rather than assumed. The eyebrow is the component's axis DSL — quadrant.transform.js
// splits it to get the two axis names and their ranges — and the arrow there is not
// chrome, it is the delimiter.
//
// The obvious advice, "write ASCII `->`, the parser accepts it", is WRONG. The regex
// does list `->`, but the eyebrow reaches the transform as already-escaped HTML, so
// the source `->` arrives as `-&gt;` and the split never fires. Rendered, that is not
// a cosmetic difference: the whole string becomes ONE axis name (printed literally as
// `Effort 0–10 -&gt; Reach`), the second axis and its range vanish, the scale falls
// back, and THE DATA POINTS MOVE. Measured on examples/stage-inset.md slide 3.
//
// And the arrow is not consumed either way — the eyebrow is also set verbatim on the
// masthead — so even with the escaping fixed, `->` would be an ASCII arrow on a
// boardroom slide. There is no better spelling to coach toward, and a warning with no
// good answer is worse than silence. Recorded in
// engineering/decisions/2026-08-25-typed-glyphs.md § "The quadrant eyebrow".
const ARROW_EYEBROW_TOKENS = Object.freeze(new Set(['quadrant']));

const EYEBROW_LINE = /^\s*`[^`]*`\s*$/;
const TABLE_ROW = /^\s*\|/;
...
function findTypedShapeGlyphs(source) {
  ...
  slides.forEach((slide, idx) => {
    ...
    const parsesArrow = [...tokens].some((t) => ARROW_EYEBROW_TOKENS.has(t));
    ...
    stripFencedCode(slide).split('\n').forEach((line) => {
      ...
      if (EYEBROW_LINE.test(line) && roles.has('arrow') && parsesArrow) return;
