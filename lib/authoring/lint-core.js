...
const { splitPaneMarkdown, specOf, paneFitWarning, paneBudgetLimit, WIDE_MIN } = require('../core/pane-spec');
 ...
 *   pane-fit      — the component opts out of panes (renders as `content`), needs a wider
 *                   share side by side than the author gave it (a table under 65%), or does not
 *                   fit a stacked band and was stacked.
...
function findPaneIssues(source, vocab) {
  ...
  slides.forEach((slide, idx) => {
    ...
    split.panes.forEach((p, i) => {
      const paneSpec = specOf(vocab.paneSpec, p.cls);
      const fit = paneSpec.fit;
      const line = `<!-- pane: ${p.cls} -->`;
      const fitWarning = paneFitWarning(paneSpec, p.cls, split.layout, i);
      if (fitWarning) {
        findings.push({
          slide: idx - fm + 1,
          rule: 'pane-fit',
          ...
          classToken: p.cls,
          line,
          message: fitWarning.replace(/^panes: /, ''),
          fix: fit === 'none'
            ? `Give '${p.cls}' its own slide, or pick a component that goes in a pane.`
            : split.layout.direction === 'stack'
              ? 'Put the panes side by side (drop `stack` from the `<!-- panes: -->` comment).'
              : `Widen this pane to ${WIDE_MIN}% or more (\`<!-- panes: ${i === 0 ? `${WIDE_MIN}/${100 - WIDE_MIN}` : `${100 - WIDE_MIN}/${WIDE_MIN}`} -->\`), or stack the panes (\`<!-- panes: stack -->\`).`,
        });
      }
      if (fit === 'none') return;
      const budget = vocab.paneBudget && Object.hasOwn(vocab.paneBudget, p.cls) ? vocab.paneBudget[p.cls] : null;
      const limit = paneBudgetLimit(budget, paneSpec, split.layout, i);
      if (!limit) return;
      const n = countPrimaryCollection(p.markdown, budget.axis);
      if (!n) return;
      const share = i === 0 ? split.layout.a : split.layout.b;
      const where = split.layout.direction === 'stack' ? `a ${share}% stacked pane` : `a ${share}% pane`;
      if (n > limit.hard) {
        findings.push({
          slide: idx - fm + 1,
          rule: 'pane-overflow',
