...
const { FAMILY_NAMES } = require('../adaptive/families.js');
...
const { LIFT_NAMES } = require('../core/resolve-lift');
const { BACKDROP_NAMES } = require('../core/resolve-backdrop');
const { VENUE_NAMES } = require('../core/resolve-venue');
const { PRESET_NAMES } = require('../core/resolve-preset');
const { CARDS_NAMES } = require('../core/resolve-cards');
const { CORNERS_NAMES } = require('../core/resolve-corners');
const { GUARDS_NAMES, FIT_NAMES } = require('../core/resolve-guards');
const {
  SPECTRUM_EDGE_NAMES,
  SPECTRUM_CARD_NAMES,
...
const { RULE_NAMES } = require('../core/resolve-rule');
const { EYEBROW_NAMES } = require('../core/resolve-eyebrow');
const { INLINE_CODE_NAMES } = require('../core/resolve-inline-code');
const { HEADLINE_NAMES } = require('../core/resolve-headline');

// BASE_MODIFIERS / FRAME_MODIFIERS / ANIMATION_MODIFIERS moved to lib/components/index.js,
// beside MODIFIER_GROUPS, which places every universal token in exactly one group.
...
// The `@size` vocab this file used to build — reading the shipped bundle and
// regex-scanning 1.3 MB of CSS to recover a table we had written INTO it — is
// gone. lint-core imports the registry directly (lib/engine/sizes.js is pure and
// fs-free, so a browser-safe module can read it), which removes the injection,
// the memo it needed to be affordable, and the "a theme that OVERRODE a size
// would be invisible here" caveat that came with parsing only the base sheet.
...
function buildVocab(manifests) {
  ...
  // One registry now: MODIFIER_GROUPS (lib/components/index.js) places every one of
  // these tokens in a group, and the flat union is what the linter accepts.
  const universalModifiers = new Set(UNIVERSAL_MODIFIER_TOKENS);
  ...
  for (const m of ms) {
    ...
    const hasFamilies = families && FAMILY_NAMES.some((f) => families[f]);
    // A component may declare EITHER form. Gating on the flat block alone skipped
    // kpi, list, matrix-2x2 and split-compare — which declare only the per-family
    // one — so their numbers stayed unread by the very change meant to read them.
    if (!m.capacity && !hasFamilies) continue;
    const flat = m.capacity || { axis: families?.axis };
    // Carry ONLY the four family keys. `adapt.capacity` also holds prose fields
    // (`axis`, and `axisRetired` — ~750 chars of internal rationale on
    // matrix-2x2 and split-compare), and a blanket copy shipped all of it into
    // every browser lint vocab AND made those two components look like they
    // declared a budget, which changed the capacity loop's control flow.
    capacity[m.name] = hasFamilies
      ? { ...flat, families: Object.fromEntries(
          FAMILY_NAMES.filter((f) => families[f]).map((f) => [f, families[f]])) }
      : flat;
  }
  ...
  const claimExcludes = {};
  for (const m of ms) {
    // Semi-universal claim entries only: the widened `excludes` (groups, any universal
    // token) drives completion, and a matching lint rule is a separate change.
    const cl = (Array.isArray(m.excludes) ? m.excludes : []).filter((e) => e.startsWith('claim-') && SEMI_UNIVERSAL_VARIANTS.includes(e));
    if (cl.length) claimExcludes[m.name] = cl;
  }
  // The PANE contract per component (lib/core/pane-spec.js): its fit and stack flag, and its
  // pane budget. Plain data, so it serializes to the browser handoff like `capacity`. A
  // component with no `pane` field (an installed package) has no row: a `half` that stacks,
  // with no budget to count against.
  const paneSpec = {};
  const paneBudget = {};
  for (const m of ms) {
    if (!m.pane) continue;
    ...
    if (m.pane.budget) {
      const { axis, side, stack } = m.pane.budget;
      paneBudget[m.name] = { axis, side: { ...side }, ...(stack ? { stack: { ...stack } } : {}) };
    }
  }
  const stampStyles = { boardroom: [...STAMP_STYLES.boardroom], range: [...STAMP_STYLES.range] };
  // No `sizes` entry: the registered canvases are no longer injected through the
  // vocab. lint-core imports lib/engine/sizes.js directly — the renderer's own
  // table, in both Node and the browser (HARD RULE #1 without the handoff).
  return { names, modifiers, universalModifiers, capacity, paneSpec, paneBudget, claimExcludes, mapRegions: buildMapVocab(), finishNames: [...FINISH_NAMES], modeNames: [...MODE_NAMES], colorModeNames: [...COLOR_MODE_NAMES], splitNames: [...SPLIT_NAMES], claimNames: [...CLAIM_NAMES], universalGroups, modifierGroups: MODIFIER_GROUPS.map((g) => ({ ...g })), semiUniversalVariants: [...SEMI_UNIVERSAL_VARIANTS], exclusiveAxes, stampStyles, toneStyles: [...TONE_STYLE_TOKENS], stampStyleNames: [...STAMP_STYLE_NAMES], toneStyleNames: [...TONE_STYLE_NAMES], spectrumNames: [...SPECTRUM_NAMES], spectrumEdgeNames: [...SPECTRUM_EDGE_NAMES], spectrumCardNames: [...SPECTRUM_CARD_NAMES], spectrumCardEdgeNames: [...SPECTRUM_CARD_EDGE_NAMES], spectrumTrimNames: [...SPECTRUM_TRIM_NAMES], ruleNames: [...RULE_NAMES], eyebrowNames: [...EYEBROW_NAMES], inlineCodeNames: [...INLINE_CODE_NAMES], headlineNames: [...HEADLINE_NAMES], liftNames: [...LIFT_NAMES], venueNames: [...VENUE_NAMES], presetNames: [...PRESET_NAMES], backdropNames: [...BACKDROP_NAMES], cardsNames: [...CARDS_NAMES], cornersNames: [...CORNERS_NAMES], guardsNames: [...GUARDS_NAMES], fitNames: [...FIT_NAMES], paceNames: [...core.PACE_NAMES], deliveryNames: [...core.DELIVERY_NAMES] };
}
