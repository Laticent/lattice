 ...
 *
 * Parsed from the shipped BASE stylesheet with the ENGINE's own `parseSizes`, so
 * the linter reads the `@size` table off the same bytes the renderer does
 ...
 *
 * SCOPE, precisely: the renderer resolves theme-FIRST — `lib/engine/css.js`
 * calls `resolveSize(name, [themeCss, baseLatticeCss])` and takes the first
 * source that declares the name. This reads only the base sheet, because the
 * linter has no deck to tell it which theme applies. Every in-repo theme carries
 * an identical `@size` table, so there is no live disagreement; a theme that
 * OVERRODE a size would win in the renderer and be invisible here. That is a
 * known bound on this vocab, not a guarantee it does not have.
 *
 * MEMOIZED: `lintText(source)` with no `opts.vocab` rebuilds the whole vocab per
 * call, and re-reading + regex-scanning the 1.3 MB bundle each time cost +71% on
 * a 20-call loop. The bundle cannot change inside a process run.
 */
let _sizeVocab;
function buildSizeVocab() {
  if (_sizeVocab !== undefined) return _sizeVocab || undefined;
  try {
    const css = fs.readFileSync(path.join(__dirname, '..', '..', 'dist', 'lattice.css'), 'utf8');
    _sizeVocab = Object.fromEntries(parseSizes(css));
  } catch {
    _sizeVocab = null;   // no dist yet — lint-core uses its fallback table
  }
  return _sizeVocab || undefined;
}
