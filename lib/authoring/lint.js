...
// Animation opt-in modifiers — behavior hooks the live host reads (no CSS of their
// own), accepted on any section so the deck-level `class:` / slide `_class:` opt-in
// isn't flagged as an unknown class. `motion-*` selects the per-slide chart-animation
// style (build / together / rise), and `motion-off` pins one slide static under a deck
// `motion:` default (§0.75, docs/src/playground/anima-host-sel.ts); `chart-anima` is the
// legacy alias for `motion-build`. All are no-ops on a non-chart section — safe everywhere.
const ANIMATION_MODIFIERS = ['chart-anima', 'motion-build', 'motion-together', 'motion-rise', 'motion-off'];
