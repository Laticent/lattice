...
// precedence in the narration chain (caption → front-matter caption → note →
// projection), and — like `describe:` — it is a CONSUMED structured comment, NOT
...
/**
 * Carry a slide's HTML COMMENT NODES from one rendering of it onto another.
 *
 * Exists because the note channel is comments, and a slide that makes a round trip
 * through a DOM loses them: `sanitizeSlideHtml` deletes comment nodes (correctly —
 * they are neither content nor a sink), so ANY pipeline that re-reads a slide out of
 * a sanitized preview frame hands back markup with every note, `describe:` and
 * `caption:` silently gone. The Studio's webpage export does exactly that to bake its
 * diagrams, and the loss is not merely a missing notes sheet: `--strip-notes` derives
 * its envelope scrub set from these same comments, so an empty set means
 * `stripNotesFromSource` finds nothing to remove and the note text ships VERBATIM in
 * the shared file — the privacy strip inverted into a leak.
 *
 * Comments are appended just inside the closing tag rather than re-threaded to their
 * original positions: every reader on this channel (`noteBodiesFromHtml`,
 * `descriptionFromHtml`, `captionFromHtml`) scans the whole section string, so
 * POSITION carries no meaning — only presence and relative ORDER, which the append
 * preserves. They never reach the shipped DOM either way: `materializeNotes` runs
 * `stripCommentNodes` before injecting the `<aside>`.
 *
 * @param {string} baked the re-read markup, comments stripped
 * @param {string} source the original rendering of the SAME slide, comments intact
 * @returns {string} `baked` with `source`'s comments appended inside its last element
 */
function carryCommentsForward(baked, source) {
	...
	// Only ever re-attach what the target LOST — a caller that already carries a comment
	// (a host whose round trip preserves them) must not end up with it twice, which would
	// double the note text in the notes sheet and in the scrub set.
 ...
 *
 * Directive-SAFE by construction: it removes a comment ONLY when its trimmed body
 * is in `noteBodies` — the set of note texts already lifted from the RENDERED slides
 * (where notes-core's boundary is unambiguous, directives having been consumed). A
 * `<!-- _class: … -->` is never in that set, so it can never be stripped. Pass the
 * INDIVIDUAL note bodies (split a joined multi-note slide on the blank line).
 *
