...
function auditStrippedSource(strippedSource) {
  ...
  for (const m of String(strippedSource == null ? '' : strippedSource).matchAll(new RegExp(COMMENT_SOURCE, 'g'))) {
    ...
    // A deck-scope directive (`class:`, `color:`) is ambiguous with prose by design — see
    // DIRECTIVE_LINE. It is NOT excluded here: the whole point of the audit is that an
    // ambiguous survivor gets reported rather than assumed innocent.
    survivors.push(body);
  }
  return survivors;
}
