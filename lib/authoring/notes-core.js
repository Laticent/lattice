...
function removeCommentSpans(src, removes, boundary) {
  ...
  for (const m of src.matchAll(re)) {
    ...
    if (prevBlank && nextBlank) {
      cursor = nl2 === -1 ? src.length : nl2 + 1; // one adjacent blank goes too — no run left
 ...
 * that says one was removed, in a source the recipient can read. It takes two shapes, and the
 * regex's own split is why. `FRONT_MATTER_BLOCK`'s CLOSE fence carries the last body line's
 * terminator, so `body` never ends with one — rejoin a body whose LAST key was `captions:` and
 * the surviving line keeps a terminator the close then doubles. And when `captions:` was the
 * ONLY key, the block empties out: an empty fence cannot be emitted, because `---\n---\n` is not
 * front matter to `parseFrontMatter` (measured — it parses as BODY, so the deck would gain a
 * thematic break where its front matter used to be). A front matter with no keys left is a deck
 * with no front matter, so the whole block goes, BOM kept.
 *
 * @param {string} source raw LFM source
...
function stripCaptionsFrontMatter(source) {
  ...
  // The close fence owns the terminator of whatever line now precedes it.
  const kept = out.join('').replace(/(\r\n|\r|\n)$/, '');
...
function stripCaptionsFromSource(source, { boundary = 'preserve' } = {}) {
  const src = String(source == null ? '' : source);
  return stripCaptionsFrontMatter(removeCommentSpans(src, isCaptionComment, boundary));
}
