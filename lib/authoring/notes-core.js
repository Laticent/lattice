 ...
 * by slide number). The caption text there is NOT a comment, so `stripCaptionsFromSource`'s
 * comment pass can't reach it — this drops the `captions:` key line plus every deeper-indented
 * map line, stopping at the next same/lesser-indent key or the closing fence. Every other
 * front-matter key (theme, acronyms, …) and the body are left byte-identical. Pure text
 * surgery; only touches the leading fenced block, so a `captions:` word in the body is safe.
 * @param {string} source raw LFM source
...
function stripCaptionsFrontMatter(source) {
  ...
  for (const line of body.split(/\r?\n/)) {
    if (!skipping) {
      ...
      if (key) { skipping = true; keyIndent = key[1].length; continue; } // drop the key line
      out.push(line);
      continue;
    }
    if (line.trim() === '') continue; // a blank line inside the block — drop with it
    if (line.match(/^(\s*)/)[1].length > keyIndent) continue; // a deeper map line — drop
    skipping = false; // dedented back to a sibling key → block over
    out.push(line);
  }
