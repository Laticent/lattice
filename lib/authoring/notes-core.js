...
// repo invented rather than inherited. Kept SEPARATE from that set on purpose: the set above is
// copied verbatim from Marpit's comment plugin and the parity test locks the two together, so a
// Lattice entry added there fails it by construction.
//
...
//
// Every matcher is VALUE-CONSTRAINED wherever a prose reading is possible, because the expensive
// failure here is the opposite one: over-stripping eats a real note silently, and an author who
// finds one missing has no way to tell what ate it. So `tier:` takes exactly the three tier names
// — `<!-- tier: short -->` is the exemplar marker, while `<!-- tier: we should discuss the
// pricing tier -->` stays the speaker note it plainly is. A matcher that cannot be constrained
// earns its place only when no note would ever open with that word.
const LATTICE_PRAGMA_MATCHERS = [
  // Exemplar length-variant marker, read by lib/exemplars/tier-filter.js (554 in the tree).
  // Its own docblock asserts the marker is "inert to the renderer and simply dropped from the
  // visible output" — true of the SLIDE, false of the note channel, which is how this survived.
  /^tier\s*:\s*(short|standard|full)$/i,
  // Gallery build marker: tools/build-bucket-galleries.js reads it to leave a hand-authored
  // gallery alone. Free-text value by nature, so this one matches on the key — safe because the
  // key is a camelCase tool word no speaker note opens with.
  /^galleryAuthored\s*:/,
  // The comment form of the `color-mode:` FRONT-MATTER register (lib/core/resolve-color-mode.js),
  // and the only HYPHENATED key any resolver reads. It belongs in front matter, so the comment
  // form is an authoring slip the engine cannot consume either way: directives.js matches
  // `[A-Za-z_][\w]*` and `\w` excludes `-`, so a hyphenated key is never recognized as a
  // directive in EITHER form. It therefore silently does nothing AND lands in the note field —
  // the double failure #1350 names. Constrained to the register's own single-token values, so
  // `<!-- color-mode: we should discuss the palette -->` stays a note.
  /^color-mode\s*:\s*[a-z-]+$/i,
];
