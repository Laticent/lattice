...
function auditStrippedSource(strippedSource) {
  ...
  // actually see, so it has to hold the same standard. Masked to spaces rather than
  // deleted, so a multi-character marker cannot reconstitute across the cut and so the
  // offsets stay put. Measured: `examples/deck-class-register.md` and
  // `examples/slide-class-forms.md` — two decks this repo ships — raised 4 and 2 alarms.
  const masked = src
