 ...
 *
 * PURE — `scrub` and `render` are injected, so the decision is testable against synthetic
 * documents without a deck, an engine or a browser. That matters because step 3 needs a document
 * whose pre-render moves a comment's own neighbours, and no realistic deck shape does; before this
 * was extracted the step was reachable only by fixture and its ANSWER was never asserted at all.
 *
 * @param {object} opts
...
function measureScrubBoundary({
  attached, rendered, inherited, inheritedMeasured = true,
