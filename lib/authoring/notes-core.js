 ...
 *
 * AT END OF INPUT ALL THREE COLLAPSE INTO A FOURTH RULE, and that case is the one that leaked.
 * End-of-file counts as blank on the right, but it is the file ENDING rather than a line that
 * can be taken — so "one of the two blanks goes" took neither, and the blank ABOVE a trailing
 * comment survived: a one-byte tell in the source the export ships, measured against this repo's
 * own note-free twin fixture.
 *
 * The rule here is NOT "take one blank", because there is no block below for any of them to be
 * separating: the WHOLE trailing run comes off, from either side, and every end-of-input variant
 * lands on `text\n`. That is deliberately more than the counterfactual strictly requires — an
 * author's trailing blank lines go with it — and it is the anti-fingerprint answer: 1318 of the
 * 1325 markdown files in this repo end with a single newline, so a trailing blank is itself the
 * anomaly. Two earlier cuts of this fix got it wrong in opposite directions, one taking a single
 * blank and one taking every blank below but only one above, and the second left this docblock
 * describing a rule the code did not implement.
 *
 * "End of input" is NOT "the comment's newline is the last byte" — a file ending with a BLANK
 * LINE after the comment still has a newline to find, which is how the blank-below branch kept
 * firing and leaving the one above, on a deck this repo ships. The test is whether the REMAINDER
 * is all whitespace, and it is `[ \t\r\n]` rather than `\s`: a non-breaking space is a paragraph
 * to markdown, not a blank line, and `\s` deleted it.
 *
 ...
 * exactly. Same neighbors — text above, text below — opposite correct answers.
 *
...
/**
 * A rendered slide reduced to its MARKUP — the only thing a cut measurement may compare.
 *
 * WHITESPACE-BLIND, necessarily: the whole point of re-rendering a scrubbed source is that it no
 * longer carries the comment's leftover whitespace, so a byte comparison reports every noted
 * slide as a divergence. Collapsing to one space is not enough either — the residue is one space
 * on one side and NOTHING on the other (`</header> <p>` vs `</header><p>`), which collapsing
 * cannot equalize; it flagged 10 of the 23 shipped noted decks. What must match is the paragraph
 * that split in two and the slide that appeared. Both are TAG differences, and tags survive
 * dropping whitespace entirely.
 *
 * It lives here because THREE call sites had written it out by hand — `strippedSlidesOrAuthored`
 * and `attachmentCut` in `lattice-emulator.js`, and `stripNotesCut` in the Studio — and a
 * hand-written copy of this measurement in the browser runtime is the exact mechanism that made
 * the CLI and the Studio disagree in the first place (#2003). Same argument as
 * `SCRUB_BOUNDARIES`: the piece the paths must agree on is read, not re-typed.
 *
 * @param {string} section one rendered `<section>`
 * @returns {string} its markup with every whitespace run removed
 */
function slideShape(section) {
  return stripCommentNodes(String(section == null ? '' : section)).replace(/\s+/g, '');
}

/**
 * Do these two renders describe the same deck? Same slide count, same markup on every slide.
 *
 * @param {string[]} a rendered sections
 * @param {string[]} b rendered sections
 * @returns {boolean}
 */
function sameSlideShape(a, b) {
  return Array.isArray(a) && Array.isArray(b) && a.length === b.length
    && a.every((sec, i) => slideShape(sec) === slideShape(b[i]));
}

/**
 * WHICH CUT REPRODUCES THE DOCUMENT THAT ACTUALLY SHIPS — and was that answer measured, or
 * inherited from a measurement taken on something else?
 *
 * The export writes the same deck into two artifacts and a pre-processing step sits between them:
 * the player envelope carries the source that was RENDERED (post-Mermaid, post-glossary), while
 * `--embed-source` attaches the deck AS THE AUTHOR WROTE IT, so the PDF round-trips to an editable
 * file rather than to machine-expanded output. That difference is deliberate. Applying the
 * rendered document's boundary to the attached one is not: it is a measurement about the other
 * file, and the fidelity guard never saw this one.
 *
 * Three steps, cheapest first, because the expensive one is reached by almost nothing:
 *   1. The two documents are the same string — the measurement IS of this document. Inherits the
 *      answer AND its confidence: an inherited boundary can be a real measurement or just the
 *      initial value left behind by a fidelity fallback, and those must not read alike.
 *   2. Every cut produces the same bytes here. Then no choice of boundary can change what ships,
 *      so there is nothing left to measure and `measured` is true on its own terms, whatever the
 *      other document concluded. Costs one scrub per candidate and no render.
 *   3. The choice genuinely matters on a document nothing rendered, so render it and each cut of
 *      it and keep the one that reproduces it.
 * Fail-closed: if no cut reproduces the document, the caller still scrubs (the text must go from
 * every copy) and `measured: false` says the artifact may re-import restructured.
 *
 * NO MODULE STATE, and `scrub` and `render` are injected, so the decision is testable against
 * synthetic documents without a deck, an engine or a browser. (Not "pure" in the strict sense —
 * `onRenderError` is a side effect, and in the CLI it writes to the console.) That matters because
 * step 3 needs a document whose pre-render moves a comment's own neighbors, and no realistic deck
 * shape does. Be precise about what that cost, because an earlier draft of this comment was not:
 * the step-3 ANSWER *was* asserted, by the integration arm that pins the attachment's tight list.
 * What no test could see was the guard's PRESENCE — degenerate this function to "always inherit"
 * and both strip-notes integration files still pass 28/28.
 *
 * @param {object} opts
 * @param {string} opts.attached the document that ships, and that the boundary will be applied to
 * @param {string} opts.rendered the document the inherited boundary was measured against
 * @param {string} opts.inherited that boundary
 * @param {boolean} [opts.inheritedMeasured] whether it was measured rather than defaulted
 * @param {(src: string, boundary: string) => string} opts.scrub apply one cut
 * @param {(src: string) => string[]} opts.render source → rendered sections
 * @param {readonly string[]} [opts.boundaries] candidates, in order — defaults to SCRUB_BOUNDARIES
 * @param {(e: Error) => void} [opts.onRenderError] told when step 3's render throws
 * @returns {{boundary: string, measured: boolean, step: 'inherited'|'cuts-agree'|'measured'|'unmeasured'}}
 */
function measureScrubBoundary({
  attached, rendered, inherited,
  // FAIL-CLOSED DEFAULT. This parameter exists precisely because an inherited boundary can be a
  // real measurement or the initial value left behind by a fidelity fallback; defaulting it to
  // `true` resolved that ambiguity in the one direction the parameter was invented to prevent,
  // so a caller that forgot it would claim a measurement nobody took.
  inheritedMeasured = false,
  scrub, render, boundaries = SCRUB_BOUNDARIES, onRenderError = null,
}) {
  // No candidates is not "every candidate agrees" — `[].every()` is vacuously true, which would
  // report `measured: true` off zero measurements and hand back `cuts[0]`, i.e. undefined.
  if (!boundaries.length) return { boundary: inherited, measured: false, step: 'unmeasured' };
  if (attached === rendered) return { boundary: inherited, measured: inheritedMeasured, step: 'inherited' };
  const cuts = boundaries.map((b) => scrub(attached, b));
  if (cuts.every((c) => c === cuts[0])) return { boundary: inherited, measured: true, step: 'cuts-agree' };
  // GUARDED. The caller is an export step whose own `catch` would drop the artifact and blame the
  // PDF library; a render that throws here must cost the MEASUREMENT, not the feature. Falling
  // through still scrubs and still reports `measured: false`.
  try {
    const authored = render(attached);
    for (const [i, boundary] of boundaries.entries()) {
      if (sameSlideShape(render(cuts[i]), authored)) return { boundary, measured: true, step: 'measured' };
    }
  } catch (e) {
    if (onRenderError) onRenderError(e);
  }
  return { boundary: inherited, measured: false, step: 'unmeasured' };
}

/**
 * Cut every trailing blank line off `text`, keeping the last content line's own terminator.
 *
 * A BACKWARD INDEX SCAN, not a regex, and the reason is measured rather than inherited. The
 * obvious `/(?:[ \t]*\r?\n)+$/` is NOT the exponential shape `engineering/gotchas/export.md`
 * records — that one is `(?:\r\n|\r|\n)`, whose alternation lets `\r\n` parse two ways; `\r?\n`
 * has one parse per segment. It is, however, QUADRATIC: 78 ms at 5k CRLF pairs and 36 seconds at
 * 100k, and the same 4×-per-doubling on a long run of spaces that the gotcha already documents.
 * This scan is 3.6 ms at 100k and 15 ms at a million — the same answer the gotcha reaches for
 * `stripCaptionsFrontMatter`, which trims the line array it already has.
 *
 * @param {string} text emitted output
 * @returns {string} the same text with any run of trailing blank lines removed
 */
function trimTrailingBlankLines(text) {
  let p = text.length;
  while (p > 0 && (text[p - 1] === ' ' || text[p - 1] === '\t' || text[p - 1] === '\r' || text[p - 1] === '\n')) p--;
  if (p === 0) return ''; // nothing but whitespace
  // `p` stopped at the last content character, so everything from here on is whitespace and the
  // first `\n` at or after it opens the trailing run. Trailing spaces on that content line are
  // the author's hard break and stay — they sit BEFORE `nl`. The `-1` arm cannot fire from
  // `removeCommentSpans` (its `out` always ends in a newline or is empty, measured over 59,080
  // calls) and is kept only so this reads correctly as a general helper.
...
function removeCommentSpans(src, removes, boundary) {
  ...
  for (const m of src.matchAll(re)) {
    ...
    // with a single newline. So take the whole whitespace tail, and the whole run of blank lines
    // off what has already been emitted — NOT one of them. Taking one is what the previous cut
    // did, and it left `A\n\n\n<!-- n -->\n` as `A\n\n` while every comment around it claimed
    // `A\n`. Every end-of-input shape lands on `text\n` now, whichever side the author's blanks
    // were on.
    // `[ \t\r\n]`, NOT `\s`. Every other whitespace test in this function is the explicit class,
    // and `\s` is wider than markdown's idea of a blank line: it also matches U+00A0, U+3000,
    // U+FEFF, U+2028 and the vertical tab. A deck ending with a non-breaking-space paragraph
    // would have had that paragraph deleted — `A\n\n<!-- n -->\n \n` rendered one `<p>`
    // instead of two.
 ...
 * `preserve`, may leave an empty one, so the second scrub reads different neighbors than the
 ...
 * the SOURCE'S OWN neighbors, so the composition cannot depend on an order it no longer has.
