 ...
 *
 * The first of those has an END-OF-INPUT case, and it is the one that leaked. End-of-file
 * counts as blank on the right, but it is the file ENDING rather than a line that can be
 * taken — so "one of the two blanks goes" took neither, and the blank ABOVE a trailing
 * comment survived it. That is a one-byte tell in the source the export ships, and this
 * repo's own note-free twin fixture is the thing it was measured against. At end of input
 * the blank on the LEFT goes instead: there is no block below for it to be separating.
 *
...
function removeCommentSpans(src, removes, boundary) {
  ...
  for (const m of src.matchAll(re)) {
    ...
    if (prevBlank && nextBlank && nl2 !== -1) {
      cursor = nl2 + 1; // the blank line BELOW goes too — no run left
    } else if (prevBlank && nextBlank) {
      // END OF INPUT, where "one of the two blanks goes" used to take NEITHER. The blank on
      // the right is the file ending — not a line that can be removed — and the blank on the
      // left is already in `out`, behind the cursor, so the branch above ran and consumed
      // nothing. The comment's line went and its blank neighbour stayed: `Closing.\n\n` where
      // the deck written without the comment has `Closing.\n`. Measured against this repo's
      // own hand-written note-free twin (`test/fixtures/strip-notes-deck-no-notes.md`), which
      // the scrub missed by exactly that byte — under BOTH cuts, so it was never the
      // `preserve`/`drop` tie-break's to fix. So take the blank on the LEFT instead: trim it
      // back off the output. Nothing is below for it to separate.
      cursor = src.length;
      out = out.replace(/[ \t]*\r?\n$/, '');
