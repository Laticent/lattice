...
function stripCaptionsFrontMatter(source) {
  ...
  // residue it left: the surviving line's terminator (the close fence supplies one) and any
  // blank line the author had put above the block.
  if (blockAt === out.length) kept = kept.replace(/(?:[ \t]*(?:\r\n|\r|\n))*$/, '');
