...
const anyCase = (word) => [...word].map((c) => `[${c}${c.toUpperCase()}]`).join('');
...
const LATTICE_PRAGMA_MATCHERS = [
  ...
  new RegExp(`^logo-style:[ \\t]*["']?(?:${anyCase('auto')}|${anyCase('brand')})["']?[ \\t]*$`),
  new RegExp(`^logo-on:[ \\t]*["']?(?:${anyCase('all')}|${anyCase('title')})["']?[ \\t]*$`),
  ...
  // matter's nested blocks and reads its layers off the indented lines below it. The only
  // single-line form the producer can read is therefore the bare header with no value, and
  // that is all this matches — case-sensitive, because the block lookup is `k ===
  // 'finish-override'`.
  //
 ...
 *
 * LINE-AWARE. A note written on its own line takes the LINE with it, terminator included;
 * an inline note takes only its span. Both are the counterfactual — the source as if the
 * author had never typed the comment — and the difference matters twice. Leaving the line
 * behind as an empty one names WHICH slides carried a note, in the very source the player
 * envelope ships for re-import; and since `--strip-notes` re-renders THIS source, that
 * residue reached the exported bytes as one byte per noted slide (#1985). Deleting an
 * inline note's neighboring whitespace has the opposite failure: it joins two words.
 *
 * WHAT IT DOES NOT CLOSE, stated because the guarantee should be readable: the author's own
 * blank lines on either side of a removed line stay, so a note that sat between two blank
 * lines leaves a run of them. That is whitespace an author writes all the time and it is not
 * computable against anything — unlike the rendered-byte delta, which a recipient could
 * derive from the shipped file alone by re-rendering the source beside it.
 *
...
function stripNotesFromSource(source, noteBodies) {
  ...
  // A NOTE ON ITS OWN LINE TAKES THE LINE WITH IT, terminator included — not just the
  // `<!-- … -->` span. Replacing the span alone leaves the line behind as an empty one, and
  // an empty line where a note used to sit is a fingerprint in its own right: the shipped
  // envelope source names WHICH slides carried a note, which is the disclosure #1985 filed.
  // It also propagates — the emulator re-renders this scrubbed source, and a stray blank
  // line inside a slide is a block boundary markdown-it can act on, so the residue reaches
  // the rendered bytes too.
  //
  // Only a whole-line comment is treated this way. An INLINE note (`text <!-- … --> more`)
  // keeps the whitespace its author typed on either side, because deleting a side would
  // join two words: `a <!-- n --> b` must scrub to `a  b`, never `a b` — and the counterfactual
  // source (the author never typing the comment) is `a  b` too, so the span-only cut is
  // already exact there.
  //
  // `Math.max(lineStart, cursor)` is what makes two notes on ONE line collapse the line
  // rather than leave it blank: by the time the second is considered, everything before it
  // on that line has already been cut, so the line legitimately begins at the cursor.
  const re = new RegExp(COMMENT_SOURCE, 'g');
  let out = '';
  let cursor = 0;
  for (const m of src.matchAll(re)) {
    ...
    if (masked[start] !== '<' || !set.has(norm(m[1]))) continue;
    let cut = start;
    let cutEnd = end;
    const lineStart = Math.max(src.lastIndexOf('\n', start - 1) + 1, cursor);
    if (/^[ \t]*$/.test(src.slice(lineStart, start))) {
      const nl = src.indexOf('\n', end);
      // `\r?` so a CRLF deck's line ending is consumed with its line rather than left
      // dangling as a lone carriage return.
      if (/^[ \t]*\r?$/.test(nl === -1 ? src.slice(end) : src.slice(end, nl))) {
        cut = lineStart;
        cutEnd = nl === -1 ? src.length : nl + 1;
      }
    }
    out += src.slice(cursor, cut);
    cursor = cutEnd;
  }
