...
function stripCaptionsFrontMatter(source) {
  ...
  // `/(?:[ \t]*(?:\r\n|\r|\n))*$/` is a nested quantifier anchored at `$`, which is
  // POLYNOMIAL on front matter ending in a long run of spaces with no newline: the engine
  // retries every split of the run at every start position. Measured before it was replaced —
  // 10k trailing spaces 163 ms, 20k 627 ms, 40k 2.5 s, 80k 10 s, i.e. 4x for every 2x. That is
  // a render-DoS on a malformed deck reached through an author-controlled path, and it is the
  // same hazard class the comment matcher at the top of this file already carries a note about.
  // CodeQL flagged it on the PR that introduced it. The loop below is linear and says the same
  // thing more plainly.
