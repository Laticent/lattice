...
function reviewText(source, opts = {}) {
  ...
  slides.forEach((slide, idx) => {
    ...
    // A `divider numbered` heading long enough to run under its own masthead.
    //
    // The section mark is a numeral over a hairline pinned to the TOP of the slide
    // (base.modifiers.css), and the headline block is vertically centered — so a long
    // enough heading climbs into it. Measured on the packed browser path at 1280x720:
    // the mark's bottom edge sits at 200px and the heading's top reaches 206px at four
    // lines and 168px at five, where the rule cuts the copy and the numeral strikes
    // through the eyebrow.
    //
    // ADVISORY, and deliberately so: the divider slot is already capped at
    // `--measure-bookend-heading` (~33 characters) and the docs tell authors a section
    // name sits on one line, so this only fires well off-contract. It exists because
    // NOTHING ELSE CATCHES IT — the overflow probe compares scrollHeight to
    // clientHeight, and an absolutely-positioned pseudo lying on top of the copy is an
    // overlap, not an overflow, so the collision renders silently. We warn and coach
    // (HARD RULE #29's posture), we do not refuse the author's deck.
    //
    // CHARACTERS, not words: the collision is driven by how many LINES the heading
    // wraps to, and the measure cap wraps on width. ~33 characters a line x 4 safe
    // lines = 132; past that the fifth line is the one that collides.
    if (comp === 'divider' && tokens.includes('numbered') && h && h.length > 132) {
      findings.push({
        slide: human, rule: 'divider-numbered-heading', severity: 'suggestion', classToken: comp, line: h,
        message: `a ${h.length}-character heading on a numbered divider — past ~132, it wraps under the section mark and the hairline crosses the copy`,
...
const RUBRIC = Object.freeze([
  ...
  { id: 'divider-numbered-heading', trap: 'a numbered divider whose heading wraps up under its own section mark', fix: 'trim the section name, or drop `numbered` on that slide' },
