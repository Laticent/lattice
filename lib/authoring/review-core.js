...
/* Module scope: this used to be a literal inside the per-slide loop, so it was
  ...
  slides.forEach((slide, idx) => {
    ...
    for (const m of slide.matchAll(PILL_MARKER_RE)) {
      findings.push({
        ...
        line: excerpt(m[0]),
        message: `\`{${m[1]}}\` is not a checkbox — a state marker is bare at the start of a bullet, not inside inline code`,
        fix: `Write \`- [${m[1]}] your text\` on its own bullet. Inline code is for a pill (\`{LABEL}\`) or for literal code.`,
      });
    }
...
const RUBRIC = Object.freeze([
  ...
  { id: 'pill-not-a-checkbox', trap: 'a state marker written inside inline code, where it renders as literal text', fix: 'put the marker bare at the start of its own bullet — `- [x] text`' },
