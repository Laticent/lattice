...
const { splitPaneMarkdown } = require('../core/pane-spec');
...
function reviewText(source, opts = {}) {
  ...
  slides.forEach((slide, idx) => {
    ...
    // A PANES slide holds two collections side by side, and each already has its own budget
    // (`pane-overflow` in lint-core). Summing them read a 2-and-5 split as one 7-bullet wall, so
    // the bullet ceiling applies per pane; the word ceiling stays on the whole slide.
    const panes = slide.includes('pane:') ? splitPaneMarkdown(slide) : null;
    const lead = panes ? slide.split('\n').slice(0, panes.panes[0].line).join('\n') : '';
    const bullets = panes
      ? Math.max(topBulletCount(lead), ...panes.panes.map((p) => topBulletCount(p.markdown)))
      : topBulletCount(slide);
    if (proseWordCount(slide) > profile.slideWords || bullets > profile.slideBullets) {
      findings.push({
        ...
        message: proseWordCount(slide) > profile.slideWords
          ...
          : `a dense slide — ${bullets} top-level bullets${panes ? ' in one pane' : ''}, past the ${profile.slideBullets} the ${profile.label.toLowerCase()} budget allows`,
    ...
    // (A `{LABEL}:circle` / `:diamond` holding more than the shape can is NOT checked here
    // any more. Past its budget the shape visibly stops being the shape, so it renders
    // WRONG rather than communicating poorly, and it moved to lint-core as the
    // `pill-shape-crowded` WARNING, with an editor underline on the span itself.)
