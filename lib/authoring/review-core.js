...
// The universal CHROME budgets (eyebrow / subtitle / key-insight / pill) stay
// universal — they budget a structure that must read as its KIND regardless of
// genre. The two WHOLE-SLIDE numbers that used to live here beside them —
// `SLIDE_PROSE_BUDGET` and the title ceiling — moved to the profile, because they
// are the two the measurement showed fit one genre rather than one corpus.
const { elementWordEntries, universalProseOverages } = require('./prose-budgets');
const { splitTopLevel } = require('./slide-split');
// The genre a deck is judged against — prose budgets, and whether the two
// genre-relative structural rules grade. Data, not code; see deck-profiles.js for
// the measured basis of every number.
const { resolveProfile } = require('./deck-profiles');
// BOTH directive forms, resolved the way the engine resolves them — see
// lib/core/class-directive-scan.mjs. The `_class:`-only regex this replaces was
// blind to a running global `<!-- class: … -->`, and on a deck using one it took
// whatever `_class:` the PROSE quoted instead.
const { slideClassDirectives } = require('../core/class-directive-scan.mjs');
const { RESERVED_MARKERS } = require('../core/inline-pills');

// The strip used by `proseWordCount` below: BOTH forms, every occurrence. A
// global-form directive left in the text counted as prose.
const CLASS_COMMENT_G = /<!--\s*_?class:\s*[^>]*?\s*-->/g;
...
const ANCHORS_NO_TITLE_CHECK = new Set(['title', 'closing', 'divider', 'topic', 'agenda', 'quote']);
// Layouts where a heading with no body is legitimate (anchors / section breaks),
// so the stub-slide check doesn't fire on them.
const HEADING_ONLY_OK = new Set(['title', 'closing', 'divider', 'topic', 'agenda', 'quote', 'section']);
...
// A leading PERIOD qualifier — "Q2", "FY24", "H1", "2026", "January". It scopes
// a label without turning it into a claim, and the digit guard below used to
// treat its digit as evidence of a metric: "Q2 Results" and "Q3 Financials"
// both came back clean, and both are the deck canon's own worked examples of a
// heading that says nothing. Stripped before the checks, never after — a digit
// anywhere ELSE really does read as a takeaway ("Revenue grew 18%").
// The trailing `(?:\s+|$)` is load-bearing: with `\s+` alone the pattern could
// never consume the LAST token, so a bare "Q2" stripped to "q2" and was caught
// incidentally by the digit guard below while the empty-string guard sat
// unreachable. Mutation-testing found it — removing that guard left the suite
// green. Now a heading that is nothing but a period strips to "" and is
// declined deliberately, as a divider we have no claim to judge.
const PERIOD_PREFIX_RE =
  /^(?:(?:q[1-4]|h[12]|fy\s?'?\d{2,4}|cy\s?\d{2,4}|\d{4}|jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+|$))+/;

/** Conservative: a heading is a "label" (not a takeaway) only on clear cases. */
function isLabelHeading(h) {
  if (!h) return false;
  const raw = h.replace(/[*`_.:]/g, '').trim().toLowerCase();
  if (!raw) return false;
  const text = raw.replace(PERIOD_PREFIX_RE, '').trim();
  if (!text) return false; // a bare "Q2" is a divider, not a claim we can judge
...
/* Module scope: these used to be literals inside the per-slide loop, so they were
 * re-evaluated once per slide. `matchAll` clones the regex internally, so a shared `g`
 * instance carries no `lastIndex` state between slides. */
const PILL_SHAPE_RE = /`\{([^}\n]{3,})\}:(?:[a-z0-9:-]*:)?(circle|diamond)\b/g;
// A brace-wrapped marker the pill grammar RESERVES (lib/core/inline-pills.js) — the four
// that read as a typed checkbox. Built from that set, not from the full marker class:
// `{!}` and `{?}` are legitimate pills and get no suggestion.
const PILL_MARKER_RE = new RegExp(
  `\`\\{([${[...RESERVED_MARKERS].map((c) => (c === '-' ? '\\-' : c)).join('')}])\\}(?::[a-z0-9:-]+)?\``, 'g',
);

// A finding's `line` is shown verbatim to a human, so a multi-line element has
// to collapse to something that fits one: first line, markers stripped, capped.
 ...
 * (optional); `opts.profile` a resolved profile record and `opts.profileOverride` a
 * profile NAME (both optional — absent, the deck's own `profile:` front matter decides,
 * and a deck that declares nothing gets `general`; see lib/authoring/deck-profiles.js).
 *
 * Returns findings: { slide, rule, severity:'suggestion', message, fix }. The two
 * GENRE-RELATIVE structural rules (`no-ask`, `agenda-missing`) always fire and always
 * count; what the profile changes is how much they COST, via `framingScale` applied in
 * `scoreFraming`. They used to carry a `scored:false` stamp instead, which switched the
 * deduction off entirely — and since those two rules are Framing's only deduction paths,
 * `teaching` pinned that category to a constant 100 on all 198 committed decks, making it
 * an exemption rather than a different bar. Budget rules (`wall-of-text`, `long-heading`)
 * instead move their THRESHOLD, because a 90-word teaching slide is not a wall of text and
 * should not be advised as one.
 */
function reviewText(source, opts = {}) {
  ...
  // The genre this deck is judged against. Resolved HERE rather than demanded from every
  // caller, so `tools/lint-deck.js` and the Studio Coach became profile-aware without
  // either changing: an explicit record wins, else an override, else the deck's own
  // `profile:` front matter, else `general`. Declared-only — there is no component
  // vocabulary to pass, because a genre is never guessed.
  const profile = opts.profile || resolveProfile({ source, override: opts.profileOverride }).profile;
  ...
  slides.forEach((slide, idx) => {
    ...
    // Wall of text / more than one idea per slide. The threshold is the deck's
    // PROFILE budget, not one universal number — measured, 70 words fits every
    // professional genre in the corpus except teaching, whose median slide is 72
    // because the learner re-reads it with no narrator. See deck-profiles.js.
    if (proseWordCount(slide) > profile.slideWords || topBulletCount(slide) > profile.slideBullets) {
      findings.push({
        slide: human, rule: 'wall-of-text', severity: 'suggestion', classToken: comp,
        // Name the budget AND the genre it came from. The old message said only "a dense
        // slide", which on a teaching deck read as a verdict the author had no way to
        // question — they could not see that they were being measured as a boardroom deck.
        // Name whichever budget ACTUALLY tripped. This said "past the N-word budget"
        // unconditionally, so a 20-word slide with 9 bullets was told it had overrun a
        // 95-word budget — false, and unactionable.
        message: proseWordCount(slide) > profile.slideWords
          ? `a dense slide — past the ${profile.slideWords}-word ${profile.label.toLowerCase()} budget; one idea per slide reads better`
          : `a dense slide — ${topBulletCount(slide)} top-level bullets, past the ${profile.slideBullets} the ${profile.label.toLowerCase()} budget allows`,
        fix: 'Split it or cut to the essential point; push the detail to speaker notes.',
      });
    }

    // Over-long heading — a takeaway should land in one tight line. The ceiling is
    // the PROFILE's, because this is the axis mission decks run long on: 17% of
    // nonprofit headings clear 14 words, against 2% of corporate ones, since a program
    // name plus its outcome does not compress. See deck-profiles.js.
    if (h && !ANCHORS_NO_TITLE_CHECK.has(comp) && wordCount(h) > profile.titleHard) {
      findings.push({
        slide: human, rule: 'long-heading', severity: 'suggestion', classToken: comp, line: h,
        message: `a ${wordCount(h)}-word heading — past the ${profile.titleHard}-word ${profile.label.toLowerCase()} budget; a takeaway should fit one tight line`,
        fix: 'Trim to a single assertion; push qualifiers and caveats to the body.',
      });
    }

    // A `divider numbered` heading long enough to run its copy off the bottom of the
    // frame.
    //
    // The masthead reserves its own band now (base.modifiers.css: symmetric padding plus
    // `justify-content: safe center`), so the heading can no longer climb into the numeral
    // — that failure is unreachable rather than warned about. What is left is the other
    // end: past the band the block pins to the top and grows DOWNWARD, and far enough down
    // it leaves the frame. Measured on the built engine at 1280x720/indaco with the
    // eyebrow present (the eyebrow is the top of the block, so it is what arrives first —
    // an earlier cut of this rule measured the h2 alone and set its threshold a line late):
    //
    //     3 lines   66 chars   block pins at 232.1, bottom 500.1
    //     4 lines   96 chars                        bottom 576.9
    //     5 lines  129 chars                        bottom 653.7   ← last length that fits
    //     6 lines  164 chars                        bottom 730.5   CLIPPED (frame is 720)
    //
    // Fires at the 5-line length, which buys the author exactly one line of notice. Past
    // it the engine speaks for itself: the section genuinely overflows now, so the export
    // prints `⚠ OVERFLOW`, tags the slide "Content clipped" and the runtime rings it —
    // none of which happened before the band, because a pseudo lying on top of the copy is
    // an overlap and the overflow probe measures spill past the frame.
    //
    // ADVISORY, and deliberately so: `--measure-bookend-heading` caps the slot at 16em
    // (~33 characters) and divider.docs.md tells authors a section name sits on one line,
    // so this only fires well off-contract. We warn and coach (HARD RULE #29's posture),
    // we do not refuse the author's deck.
    //
    // CHARACTERS, not words: what drives the overflow is how many LINES the heading wraps
    // to, and the measure cap wraps on width.
    //
    // A character COUNT is a proxy for that, and it is a loose one at the extremes: the
    // table above was measured on ordinary prose, and wide-set text wraps sooner per
    // character. Measured, a 97-character heading of capital Ms clips 87px past the frame
    // and this rule never fires. So "one line of notice before the clip" holds for prose,
    // not as a guarantee. It is the coaching floor, not the gate — the gate is the
    // overflow the band now produces, which fires on geometry and cannot be fooled by
    // set width.
    // NOT on `divider light`: it renders a type tier down with `max-width: none`, so a
    // 133-character heading measures 281.5 -> 438.5 of 720 and a 359-character one still
    // clears both the band and the frame. The message below would be false there. Light's
    // own exposure is through its LEDE, not its heading (it collides at -15.2 pre-band),
    // which a heading-length rule is the wrong instrument for; the band handles it.
    if (comp === 'divider' && tokens.includes('numbered') && !tokens.includes('light') && h && h.length > 128) {
      findings.push({
        slide: human, rule: 'divider-numbered-heading', severity: 'suggestion', classToken: comp, line: h,
        message: `a ${h.length}-character heading on a numbered divider — past ~128 it fills the band under the section mark, and one more line runs off the bottom of the frame`,
        fix: 'Trim the section name — a divider heading is a label, not a sentence. Or drop `numbered` on this slide.',
      });
    }
    ...
    if (dens && Number.isInteger(dens.soft) && Number.isInteger(dens.hard)) {
      const entries = elementWordEntries(slide, dens.axis || 'item');
      // The WORST element, and which one it is. Reporting only the count left a
      // reader of a seven-element split-compare slide to find it by hand.
      let worstEntry = null;
      for (const e of entries) if (!worstEntry || e.words > worstEntry.words) worstEntry = e;
      const worst = worstEntry ? worstEntry.words : 0;
      // `dens.note` is a DESCRIPTION of the target ("a stacked card is a short
      // paragraph at most"), so "Tighten to " + note built a non-sentence.
      const tighten = dens.note
        ? `Tighten it toward the target — ${dens.note}.`
        : `Trim to ~${dens.soft} words per element; push detail to speaker notes.`;
      // One line of the offending element, so the finding points at something.
      const at = worstEntry ? excerpt(worstEntry.text) : undefined;
      if (worst > dens.hard) {
        findings.push({
          slide: human, rule: 'density-overflow', severity: 'suggestion', classToken: comp, line: at,
        ...
        findings.push({
          slide: human, rule: 'density-crowd', severity: 'suggestion', classToken: comp, line: at,
    ...
    // A `{LABEL}:circle` / `:diamond` holding a WORD. Both shapes are only as tall as a
    // capsule, and neither has a long straight run (a rhombus offers its full width only
    // along its midline, and a circle has none), so a label longer than about two
    // characters forces the box wide enough to stop reading as the shape: the circle
    // becomes a capsule and the diamond a long flat sliver. Coach, never block — the deck still renders, and HARD RULE #29's "we warn,
    // we coach" line applies to an author's own choice of shape too.
    // `{x}` reaching for the bare `- [x]` checkbox. It renders as literal code, so the
    // author sees SOMETHING is wrong but not what — this says what.
    for (const m of slide.matchAll(PILL_MARKER_RE)) {
      findings.push({
        slide: human, rule: 'pill-not-a-checkbox', severity: 'suggestion', classToken: comp,
        line: excerpt(m[0]),
        message: `\`{${m[1]}}\` is not a checkbox — braces make a pill, brackets make a mark`,
        fix: `Write \`[${m[1]}]\` in inline code for a mark anywhere, or \`- [${m[1]}] your text\` bare at the start of a bullet for a checklist row.`,
      });
    }

    for (const m of slide.matchAll(PILL_SHAPE_RE)) {
      findings.push({
        slide: human, rule: 'pill-shape-crowded', severity: 'suggestion', classToken: comp,
        line: excerpt(m[0]),
        message: `\`${m[1]}\` is ${m[1].length} characters inside a \`${m[2]}\` — that shape holds one or two`,
        fix: 'Use `:tag` or `:chip` for a word; keep `:circle` and `:diamond` for a digit or a mark.',
      });
    }
...
const RUBRIC = Object.freeze([
  ...
  { id: 'pill-not-a-checkbox', trap: 'a state marker written with braces, which makes a pill rather than a mark', fix: 'use brackets — `` `[x]` `` inline, or `- [x] text` bare at the start of a bullet' },
  { id: 'pill-shape-crowded', trap: 'a word inside a shape that can only hold a character or two', fix: 'use `:tag` or `:chip` for a word; keep `:circle` and `:diamond` for a digit or a mark' },
  ...
  { id: 'divider-numbered-heading', trap: 'a numbered divider whose heading fills the band under the section mark and runs off the frame', fix: 'trim the section name, or drop `numbered` on that slide' },
