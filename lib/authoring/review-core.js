...
function reviewText(source, opts = {}) {
  ...
  const profile =
    ...
    resolveProfile({
      source,
      override: opts.profileOverride,
      componentNames: directives.map((d) => (d?.payload || '').trim().split(/\s+/)[0]).filter(Boolean),
    }).profile;
  ...
  slides.forEach((slide, idx) => {
    ...
    if (proseWordCount(slide) > profile.slideWords || topBulletCount(slide) > profile.slideBullets) {
      findings.push({
        ...
        message: `a dense slide — past the ${profile.slideWords}-word ${profile.label.toLowerCase()} budget; one idea per slide reads better`,
