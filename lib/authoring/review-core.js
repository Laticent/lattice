...
const { elementWordCounts, universalProseOverages } = require('./prose-budgets');
 ...
 * profile NAME (both optional — absent, the deck's own `profile:` front matter and
 * then component inference decide; see lib/authoring/deck-profiles.js).
 *
 * Returns findings: { slide, rule, severity:'suggestion', message, fix }. The two
 * GENRE-RELATIVE structural rules (`no-ask`, `agenda-missing`) carry `scored:false`
 * when the resolved profile does not grade them — they are still surfaced as advice,
 * and the scorecard reads the flag rather than re-deriving the profile. Budget rules
 * (`wall-of-text`, `long-heading`) instead move their THRESHOLD, because a 90-word
 * teaching slide is not a wall of text and should not be advised as one.
 */
function reviewText(source, opts = {}) {
  ...
  // The genre this deck is judged against. Resolved HERE rather than demanded from
  // every caller, so `tools/lint-deck.js` and the Studio Coach became profile-aware
  // without either changing: an explicit record wins, else the deck's own front
  // matter, else component inference, else the lenient `general` fallback.
  // Resolved HERE rather than demanded from every caller, so `tools/lint-deck.js` and
  // the Studio Coach are profile-aware without either changing. Declared-only: there is
  // no component vocabulary to pass, because a genre is never guessed.
  ...
  if (contentSlides >= 4 && !hasAsk) {
    findings.push({
      slide: 0, rule: 'no-ask', severity: 'suggestion', scored: profile.scoresAsk,
  ...
  if (contentSlides >= 10 && !hasAgenda) {
    findings.push({
      slide: 0, rule: 'agenda-missing', severity: 'suggestion', scored: profile.scoresAgenda,
