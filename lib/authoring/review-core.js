...
function reviewText(source, opts = {}) {
  ...
  slides.forEach((slide, idx) => {
    ...
    // A `{LABEL}:circle` / `:diamond` holding a WORD. Both shapes are square boxes by
    // construction (a rhombus offers its full width only along its midline, and a
    // circle has no straight run at all), so a label longer than about two characters
    // either overflows the fill or forces the box wide enough to stop reading as the
    // shape. Coach, never block — the deck still renders, and HARD RULE #29's "we warn,
