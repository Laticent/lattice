 ...
 * same word, "Glossary" — and the rarity is substantially REACH: `isLabelHeading` accepts
 * 8 of the 1,575 headings it is actually offered (the corpus has 1,579 `^## ` lines, but
 * `headingOf` hands it only the first h2 per slide; three of the 8 are then suppressed as
 * anchors, leaving those 5), every one through its single-bare-word branch, with the
 * 34-entry LABEL_WORDS phrase list contributing zero.
 *
 * Re-derive the shares and the ledger with `npm run score:variance`, and this paragraph's
 * heading census with `npm run score:variance -- --reach`. The tool exists because this
 * decomposition had by then been rebuilt three times from scratch and the three runs
 * disagreed — and `--reach` exists because a checker found this very sentence promising a
 * re-derivation the tool could not perform. See
...
// Layouts whose substance is data (chart + evidence buckets, plus the solo hero
// metric). Used only to decide whether Data is a scorable category — if a deck has
// none of these, Data is N/A rather than a free 100.
//
// Projected from each manifest's `projection.data`, not hand-listed. The literal it
// replaces had drifted three names — `journey`, `matrix-grid` and `roadmap` are chart
// layouts its own comment says belong, and a deck built on any of them scored Data as
// N/A instead of scoring. Nothing went red, which is the whole reason this moved.
// Frozen data, so the module stays browser-safe and fs-free (deck-profiles.js's
// family contract); `require()` of an .mjs is the bridge already proven by the
// class-directive-scan.mjs import two lines up.
const { DATA_LAYOUTS: DATA_LAYOUT_NAMES } = require('../core/projection-catalog.generated.mjs');
const DATA_LAYOUTS = new Set(DATA_LAYOUT_NAMES);
