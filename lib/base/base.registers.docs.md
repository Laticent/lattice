...
|---|---|---|
| [`preset:`](#the-preset-front-matter-register-a-named-look) | A named look that sets the accent + surface registers below at once | `classic` |
...
`preset:` is **one word that sets ten registers**: the brand bar and its placement, the card
rail and its placement, structural trim, the heading rule, the eyebrow, headline alignment,
card lift and corners. Each of those still works on its own — **an explicit key always
wins over the preset** — so a preset is a starting point and the keys are its overrides.

| `preset:` value | Sets | Look |
|---|---|---|
| `classic` | *(nothing — every register at its default)* | The house default, named. **The default** (omit the key). |
| `editorial` | `rule: short` · `eyebrow: bar` · `spectrum-trim: restrained` · `lift: on` | A magazine hand: short rule, a bar on the kicker, single-hue trim, lifted cards. |
| `brand` | `spectrum: solid` · `spectrum-card: auto` · `spectrum-trim: on` · `rule: accent` · `eyebrow: dot` · `lift: on` | The accent everywhere — set the theme accent to a client's color to white-label. |
| `minimal` | `spectrum: off` · `rule: none` · `corners: rounded` | No bar, no rule, rounded corners. |
...
A preset has **no class token and no CSS of its own**. It resolves in the one front-matter
reader every path shares (`frontMatterValue` in `lib/core/front-matter-key.js`): when a key in
the family is absent, the reader answers with the preset's value. So `preset: editorial`
renders exactly as if the deck had written its four keys, per-slide `_class:` overrides work
unchanged, and the CLI, the export and the Studio agree by construction. A preset
deliberately leaves alone `theme:` (the palette), `mode:`, `finish:`, `claim:`, `cards:`,
`stamp:` / `tone:` and every content key. An unknown name is flagged by the linter
(`unknown-preset`) and resolves to no preset. The Studio shows it as the **Preset** row at
the top of the deck panel, with a count of the keys that differ from it and a Reset. See
