...
const { wrapSvgLabel, measureLabel, ADVANCE, LINE_HEIGHT } = require('./svg-label');
 ...
 */
const NUMERIC_PILL =
  /^[(+\-\u2212]?\s*[^\w\s]{0,3}\s*[-\u2212]?\d[\d,]*(?:\.\d+)?\s*(?:%|‰|[A-Za-z]{1,6})?\s*\)?$/;
...
function axisUnit(max, step) {
  ...
  if (abs >= 1e9 && step >= 1e8) return { unit: 'B', divisor: 1e9 };
  if (abs >= 1e6 && step >= 1e5) return { unit: 'M', divisor: 1e6 };
  if (abs >= 1e4 && step >= 1e3) return { unit: 'k', divisor: 1e3 };
...
function axisFormatter({ ticks = [], step = 1, affix = {} } = {}) {
  ...
  return (v) => `${prefix}${trimZeros((v / divisor).toFixed(decimals))}${unit}${suffix}`;
}
...
function markFormatter({ ticks = [], step = 1, affix = {}, sig = 2 } = {}) {
  ...
  return (v) => {
    ...
    const decimals = abs === 0 || Number.isInteger(scaled) ? 0
      : Math.min(4, Math.max(1, sig - Math.max(0, Math.floor(Math.log10(abs)) + 1)));
    return `${prefix}${trimZeros(scaled.toFixed(decimals))}${unit}${suffix}`;
  };
}
...
function buildCategoryLabels({ plot, labels, center, width, maxLines = 2, axis = 'x', gap = 5 }) {
  ...
  labels.forEach((label, i) => {
    ...
    const painted = Math.min(width, widestLine(m.lines, FS.cat));
    if (axis === 'x') {
      const half = painted / 2;
      ...
      // The vertical axis needs the same cull, and used to have NONE — which is
      // the branch a horizontal bar chart lives on. Thirty rows on the default
      // landscape plot gave a 4.9-unit band pitch against a 9.4-unit line
      // height: every row label overprinted its neighbors and nothing was
      // legible. Here the extent is the label BLOCK's height, not its width.
...
module.exports = {
  ...
  niceStep, niceTicks,
  parseValue, parseAffix, affixOf, isValuePill,
