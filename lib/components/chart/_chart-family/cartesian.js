...
const { plainText, escAttr } = require('./transform-utils');
const { noteLabelDrop } = require('./label-drops');
...
// A PANE's canvas (lib/core/panes.js). The engine stamps each chart pane with its box in the
// SAME user units a full-slide chart is drawn in (`data-pane-view`, lib/engine/index.js
// `paneView`), so a label sized in user units prints at the slide's px size, and a narrow or
// short pane gets a narrow or short plot instead of the slide's drawing scaled down. Floored so a
// sliver of a pane still gets a plot the gutters fit around; below it, the drawing scales.
const PANE_VIEW_MIN = Object.freeze({ w: 140, h: 72 });

/** The viewBox for a deck orientation stamp. `square` → landscape (see above). A pane's own
 *  canvas, when the engine stamped one, wins: see PANE_VIEW_MIN. */
function viewFor(orientation, paneView) {
  if (paneView && paneView.w > 0 && paneView.h > 0) {
    return { w: Math.max(PANE_VIEW_MIN.w, Math.round(paneView.w)), h: Math.max(PANE_VIEW_MIN.h, Math.round(paneView.h)) };
  }
 ...
 * fill the box.
 *
 * THIS SNAPS THE DOMAIN OUT to whole steps, which is right for a bar — the axis
 * top is a round number and the baseline is zero. It is wrong for a chart that
 * encodes POSITION, where it spends the plot on nothing: a $60k-$420k series
 * gets a $0-$600k axis, i.e. the right third of the box empty. Those charts
 * want `niceDomain` below, which pads the data instead of snapping past it, and
 * which used to exist here as a `tight` flag AND again, more completely, inside
 * `scatter`. One copy now (HARD RULE #1); the flag is gone rather than left
 * unused, because a dead second answer is what the next person reaches for.
 *
...
function niceTicks(minIn, maxIn, { target = 4, includeZero = true } = {}) {
  if (target === 'auto') {
    ...
    for (const t of [3, 4, 5, 6]) {
      const cand = niceTicks(minIn, maxIn, { target: t, includeZero });
  ...
  return { min: lo, max: Number.isFinite(hi) ? hi : last, step, ticks };
}
...
/**
 * A PADDED DATA DOMAIN with a readable tick set — the axis a chart wants when
 * its marks encode POSITION rather than length.
 *
 * WHY THIS IS NOT `niceTicks({ tight: true })`, which it replaces. `tight` gets
 * the shape right — keep the ladder's step, let the domain be the data plus a
 * pad, draw the ticks inside it — and stops there. Four things a positional
 * chart needs are missing from it, and `scatter` had grown all four privately
 * before this was folded back (HARD RULE #1):
 *
 *   1. A TARGET SWEEP. The nice-number ladder is coarse, so one `target` is a
 *      lottery: a $60k-$420k series takes a 200k step at target 4 and keeps two
 *      ticks, a 100k step at 5 and keeps four. Sweeping and scoring picks the
 *      readable one instead of the first one.
 *   2. A CEILING. A scatter grids BOTH axes, so five ticks a side is ten lines
 *      behind the data — which is where a plot starts reading as a table.
 *   3. THE ZERO WALL. A negative tick on a quantity that cannot be negative is
 *      wrong, and it is a TICK rule, not a domain rule: clamping the domain at
 *      zero instead pinned a bubble's center to the plot corner and deleted the
 *      `$0` tick the docs promise by name.
 *   4. SNAP-OUT WHEN IT IS NEARLY FREE. A domain that stops just short of a
 *      tick leaves the outermost point floating past the last gridline with
 ...
 * magnitude by LENGTH from a baseline; these charts encode position, and
 * neither end is measured from an origin. Two measures with narrow ranges
 * (margin 38-44%, NPS 51-58) forced to include zero collapse into one corner
 * and the relationship they were drawn to show disappears.
 *
 * @param {number} minIn   the data minimum, unpadded
 * @param {number} maxIn   the data maximum, unpadded
 * @param {object} [o]
 * @param {number} [o.pad]      air at each end, as a fraction of the span
 * @param {number} [o.target]   the ideal tick count the sweep aims at
 * @param {number} [o.maxTicks] the hard ceiling — past this a grid competes
 * @returns {{min:number,max:number,step:number,ticks:number[]}}
 */
function niceDomain(minIn, maxIn, { pad = 0.08, target = 4, maxTicks = 5 } = {}) {
  let min = Number.isFinite(minIn) ? minIn : 0;
  let max = Number.isFinite(maxIn) ? maxIn : 0;
  // ORDERED FIRST, because every line below assumes it. Handed a reversed pair
  // this returned a domain that excluded the data outright — `niceDomain(10, 2)`
  // gave [7.5, 12.5], so the 2 plotted off the chart with no error anywhere.
  // Neither shipped caller can do it (`scatter` derives the pair with
  // Math.min/max, `slope` from a model that sorts), but this is exported
  // substrate now and the next caller has no reason to know the order matters.
  ...
  // steps — which is right for a bar (the axis should end on a round number you
  // can read a length against) and wrong here: it handed a $60k-$420k series a
  // $0-$600k axis and spent the right third of the plot on nothing. So the
  // substrate still chooses the STEP (one nice-number ladder for the whole
  // family), and this picks which of its ticks the domain actually contains.
  let best = null;
  for (const t of [3, 4, 5, 6, 7]) {
    const nice = niceTicks(min, max, { target: t, includeZero: false });
    const ticks = nice.ticks.filter((v) => v >= min - 1e-9 && v <= max + 1e-9
      && (!nonNegative || v >= 0));
    // Distance from the ideal, with a wall at the ceiling. There is no separate
    // "too few to read against" term: a draft carried one and it was measured
    // dead — over ~55k domains across six option sets, including degenerate and
    // denormal ones, deleting it changed no result. `Math.abs(n - target)`
    // already prices a one-tick candidate above every richer one, and when
    // EVERY candidate is starved the term is a constant offset that cancels.
    const n = ticks.length;
    const score = -(Math.abs(n - target) + (n > maxTicks ? 20 : 0));
  ...
  // …and never past the ceiling: a snap that ADDS a tick past `maxTicks` has
  // traded one small unlabeled gap for a grid that competes with the data.
  ...
  if (hi != null && hi - max <= snap && ticks.length < maxTicks) { max = hi; ticks.push(hi); }
  ...
  if (lo != null && min - lo <= snap && ticks.length < maxTicks && !(nonNegative && lo < 0)) {
...
// The authored-value parser moved to `lib/core/chart-values.js` so narration can
// read a pill the same way the picture does — see that file's docblock. It is
// re-exported below, so every caller of this module is unaffected.
const {
  MAGNITUDE_ONLY, normalizeSeparators, parseValue, signedValue, isValuePill,
} = require('../../../core/chart-values');
 ...
 */
function buildGrid({ plot, ticks, scale, axis = 'y', from = null, to = null }) {
  const parts = [];
  // `from`/`to` shorten a gridline to a BAND rather than the whole plot. A
  // member whose rows carry text INSIDE the plot box needs it: the bullet's
  // name and readout sit on a baseline above each track, and a full-height
  // vertical gridline ran straight through them — the last one landing a pixel
  // off the final glyph of every `4.2M vs 5M` readout on the chart.
  const lo = from == null ? (axis === 'y' ? plot.x0 : plot.y0) : from;
  const hi = to == null ? (axis === 'y' ? plot.x1 : plot.y1) : to;
  for (const t of ticks) {
    const p = round2(scale(t));
    const zero = t === 0;
    const cls = zero ? 'cart-zero' : 'cart-grid';
    parts.push(axis === 'y'
      ? `<line class="${cls}" x1="${round2(lo)}" y1="${p}" x2="${round2(hi)}" y2="${p}"/>`
      : `<line class="${cls}" x1="${p}" y1="${round2(lo)}" x2="${p}" y2="${round2(hi)}"/>`);
  }
  return parts.join('');
}
 ...
 * @param {boolean|function(number): boolean} [o.markFor] stamp `data-mark-for="i"` on
 *                             label i, linking it to mark i: it recedes with that mark under
 *                             the chart hover and the Present Guide, and the reveal layer
 *                             treats it as a tap proxy. Only for a chart whose mark index i IS
 *                             category i (waterfall, bar, stacked-bar); a chart whose index runs
 *                             per series must not opt in. Pass a predicate when a category can
 *                             draw NO mark (a non-numeric bar, an all-zero stack): a label linked
 *                             to a mark that does not exist opens an empty card.
 */
function buildCategoryLabels({
  plot, labels, center, width, maxLines = 2, axis = 'x', gap = 5, edgeAnchor = false, pitch = 0, names = null,
  markFor = false,
}) {
  const parts = [];
  ...
  labels.forEach((label, i) => {
    if (!label) return;
    const c = center(i);
    // THE CATEGORY'S IDENTITY, for the Present Guide. The data-series narration reads a
    // cartesian chart one CATEGORY at a time — "Q1 2025: Enterprise, four point one; …",
    // "FY23: Licenses, …", "Jan 2026: M0, one hundred; …" — and a sentence about a whole
    // column or row belongs to no single mark, so no mark's label led it and the pointer fell
    // back to the whole chart. The category's own label is the one element that sentence is
    // about. It declares no value: a mark that says the same name AND corroborates its value
    // (a single bar) still outranks it (present-guide.ts `findMarkTarget`).
    //
    // `names` is for a caller that FITS its labels before handing them in (waterfall shortens a
    // long word to "Receip…"): the painted text is the fitted one, the identity is the name the
    // voice says. Stamping the fitted text made "Receipts, three point one million" lead nothing.
    const ident = ` data-anima-role="label" data-label="${escAttr(plainText(names?.[i] ?? label))}"` +
      ((typeof markFor === 'function' ? markFor(i) : markFor) ? ` data-mark-for="${i}"` : '');
    ...
    if (axis === 'x') {
      ...
      // on reserving space it no longer occupies — and cull only on an actual
      // OVERLAP, with no extra courtesy gap. `painted` is an ESTIMATE from a
      // flat 0.6 advance, measured over-wide by roughly a third on a real sans
      // face, so it already carries its own slack; charging MIN_LABEL_GAP on
      // top of it deleted the second and last labels of an evenly spaced
      // six-quarter axis whose boxes were 3 units apart.
      if (from < lastEnd) { noteLabelDrop(label, 'pitch'); return; }
      lastEnd = to;
      parts.push(wrapSvgLabel(label, {
        ...
        className: 'cart-cat', attrs: ident, emitFontSize: false,
      }).svg);
    } else {
      ...
      // Reported, not just skipped (#2171). Single pass, so this notes in place —
      // unlike `placeLabels`, which quadrant runs speculatively nine times.
      if (!fits && c - half < lastEnd) { noteLabelDrop(label, 'pitch'); return; }
      ...
      parts.push(wrapSvgLabel(label, {
        ...
        attrs: ident, emitFontSize: false,
      }).svg);
    }
  });
  return parts.join('');
...
/**
 * Wrap a chart's marks so a screen reader gets the curated sentence and NOT a
 * second, stripped copy of the same numbers.
 *
 * THE ONE PLACE THE FAMILY DOES THIS (HARD RULE #1). Nine members hand-roll
 * their own `<svg>` root rather than going through `buildSvgRoot`, and every
 * one of them needs the same wrapper — nine copies is nine chances to drop it,
 * and the belief that `role="img"` already handled it is exactly how the family
 * got here. Members that build their own root call this; members on
 * `buildSvgRoot` get it for free.
 *
 * A `<g>` RATHER THAN AN ATTRIBUTE ON EACH CHILD, deliberately: a body arrives
 * here as an already-concatenated string of sibling elements, so there is
 * nothing to attribute one by one without re-parsing it. The wrapper is safe to
 * add — no chart stylesheet selects a direct child of an svg root (checked), and
 * the motion system keys on `data-anima-role` attributes rather than on depth.
 *
 * @param {string} body  the marks and chrome, already emitted
 * @returns {string}     the same markup inside an aria-hidden group
 */
function ariaHiddenMarks(body) {
  return body ? `<g aria-hidden="true">${body}</g>` : '';
}
 ...
 * hand-writing it is seven chances to drop one.
 *
 * `role="img"` DOES NOT PRUNE THE SUBTREE, and this docblock said it did.
 * ARIA calls the role children-presentational, so the whole family was written
 * on the belief that `<title>` and `<desc>` were the ONLY route to the data for
 * a screen reader. Chromium does not implement it that way for SVG. Measured
 * over the rendered chart gallery with `Accessibility.getFullAXTree`: all
 * fifteen chart roots exposed a correct name and description AND every text
 * node beneath them — 24 unignored descendants under `bar`, 223 under `map`.
 * A reader heard the curated sentence and could then walk into `North America`,
 * `EMEA`, `$4.2M` as loose text: the numbers twice, the second time stripped of
 * the relationship that made them mean anything.
 *
 * So the pruning is done explicitly, by an `aria-hidden` group around the
 * marks. The `<title>`/`<desc>` stay outside it and keep naming the root, which
 * is why `desc` is still a required argument rather than an option — and why a
 * member's desc should carry the RELATIONSHIP the chart is for, not just its
 * values (the funnel's lists conversion rates, because drop-off is what a
 * funnel is).
 *
 * IT COSTS NOTHING IN FIND-IN-PAGE, which is the objection this change was
 * expected to have to answer. `aria-hidden` text still paints, stays in
 * `textContent`, and is still matched by Blink's text finder — measured with
 * `window.find()`, where only `display:none` content goes unfound.
 *
 * @param {object} o
...
function buildSvgRoot({ view, className, title, desc, defs = '', body }) {
  ...
  return `<svg class="${className}" viewBox="0 0 ${view.w} ${view.h}"` +
    ...
    `${defs ? `<defs>${defs}</defs>` : ''}${ariaHiddenMarks(body)}</svg>`;
}
...
/**
 * A screen-reader-only DATA TABLE for a matrix-shaped chart — every reading, navigable.
 *
 * WHY A TABLE AND NOT A LONGER `<desc>`. A heatmap's and a line's `<desc>` state the FINDING —
 * where a matrix peaks, where a series started, ended and turned — on purpose: a screen reader
 * says a `<desc>` in one breath, and 120 numbers in one breath is the raw data rather than the
 * finding (heatmap.transform.js `describe`). But the finding was then the ONLY route to the
 * data. The readings between the peaks were on the picture and nowhere a listener could reach.
 *
 * A table is the one structure assistive tech lets a person walk: row by row, column by column,
 * each cell announced with its row and column headers. So the chart keeps its summary as the
 * image's description, and the numbers ride here, after it, for whoever wants them.
 *
 * Hidden inside the family's `.chart-sr-only` clip, so it paints nothing and takes no layout. The
 * caption walker never reaches it on these charts: both are read by a chart narrator
 * (`narrateDataSeries`), which replaces the walker for the slide.
 *
 * @param {{caption: string, corner: string, cols: string[], rows: {head: string, cells: (string|null)[]}[]}} t
 * @returns {string}
 */
function buildSrDataTable({ caption, corner = '', cols, rows }) {
  if (!rows.length || !cols.length) return '';
  const e = (v) => escAttr(plainText(String(v)));
  const head = `<tr><th scope="col">${e(corner)}</th>${cols.map((c) => `<th scope="col">${e(c)}</th>`).join('')}</tr>`;
  const body = rows.map((r) => `<tr><th scope="row">${e(r.head)}</th>${cols.map((_, i) => `<td>${r.cells[i] == null || r.cells[i] === '' ? 'no value' : e(r.cells[i])}</td>`).join('')}</tr>`).join('');
  // WRAPPED, not classed: a `<table>` sizes to its content and ignores the 1px `width`/`height`
  // the clip sets, so a table carrying `.chart-sr-only` itself laid out at up to 1321x456 and
  // widened the slide's scroll extent while painting nothing. A block wrapper takes the clip.
  return `<div class="chart-sr-only"><table class="chart-sr-table"><caption>${e(caption)}</caption><thead>${head}</thead><tbody>${body}</tbody></table></div>`;
}
...
module.exports = {
  ...
  buildValueLabel, buildAxisTitle, buildFillDefs, buildSvgRoot, ariaHiddenMarks, buildSrDataTable,
  round2,
};
