...
const CHART_LAYOUTS = ['progress', 'timeline-list', 'piechart', 'gantt', 'kanban', 'radar', 'quadrant', 'state-chart', 'funnel', 'map', 'journey', 'word-cloud', 'roadmap', 'matrix-grid'];
...
// Shared wrapping <text> emitter — the gantt is SVG-native now, so its lane
// names, tick labels and bar captions wrap in-viewBox like every other chart.
// See svg-label.js.
const { wrapSvgLabel, measureLabel, ADVANCE_MONO_TRACKED } = require('./svg-label');
...
// The shared tag-strip + entity-decode. One implementation (HARD RULE #15), and
// it strips to a fixed point rather than in one pass.
const { plainText } = require('./transform-utils');

// Canonical depth-aware list walkers + section walker (moved to lib/core so
// core primitives like split-panels never import a component kernel — see
// lib/core/html-lists.js / lib/core/section-walk.js).
...
// text) and decodes entities; a local single-pass copy here was both a second
...
let GANTT_GRAD_SEQ = 0;
function ganttGradientDefs(keys) {
  const ns = ++GANTT_GRAD_SEQ;
  const defs = keys.map((k) =>
    `<linearGradient id="gantt-fill-${k}-${ns}" x1="0" y1="0" x2="0" y2="1">` +
    ...
    `</linearGradient>`).join('');
  return { defs, id: (k) => `gantt-fill-${k}-${ns}` };
}

function buildGanttChart(ulInner, eyebrowHtml, orientation) {
  ...
  allTicks.forEach((t, i) => {
    const half = stripTags(t.label).length * G.fsTick * ADVANCE_MONO_TRACKED / 2;
    const x = xOf(t.mid);
  ...
  for (const t of ticks) {
    axisSvg += wrapSvgLabel(stripTags(t.label), {
      x: xOf(t.mid), y: G.axisH - 6, width: 56, fontSize: G.fsTick,
      // .gantt-tick is --font-mono with letter-spacing: 0.12em — wider per
      // character than the proportional default assumes.
...
// ── Per-layout section builders ─────────────────────────────────────────────
// One small builder per chart layout: extract the list, run the kernel,
// splice the figure back. Each takes (html, ctx) where ctx carries
// { cls, classTokens, orientation } and returns { html, cls } — cls comes
// back because roadmap can add the auto-selected `horizons` token.
// transformChartSection dispatches through SECTION_BUILDERS below.
...
function buildGanttSection(html, ctx) {
  ...
  return spliceFirstList(html, (ext) => buildGanttChart(ext.inner, eyeMatch ? eyeMatch[1] : '', ctx.orientation));
}

function buildKanbanSection(html) {
  return spliceFirstList(html, (ext) => buildKanbanBoard(ext.inner));
}

function buildRadarSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
    const variant = radar.pickVariant(ctx.classTokens);
    const isMinimal = ctx.classTokens.includes('minimal');
    const model = radar.parseRadar(ext.inner, variant === 'quadrant');
    if (!model) return null;
    const scale = radar.resolveScale(model, radar.matchEyebrowText(html));
    return radar.buildRadar(model, variant, scale, isMinimal, ctx.orientation);
  });
}

function buildQuadrantSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
    const variant = quadrant.pickVariant(ctx.classTokens);
    const model = quadrant.parseQuadrant(ext.inner);
    if (!model) return null;
    const scale = quadrant.resolveScale(model, quadrant.matchEyebrowText(html));
    return quadrant.buildQuadrant(model, variant, scale, ctx.orientation);
  });
}

function buildStateChartSection(html, ctx) {
  ...
  // lost to indentation. See state-chart.transform.js §extractStateList.
  const ext = stateChart.extractStateList(html);
  if (!ext) return html;
  const model = stateChart.parseStateChart(ext.inner);
  ...
  // box (state-chart.transform.js §buildStateChart).
  const figure = stateChart.buildStateChart(model, ctx.classTokens, ctx.orientation);
  return html.slice(0, ext.start) + figure + html.slice(ext.end);
}

function buildFunnelSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
...
function buildMapSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
    const basemap = map.pickBasemap(ctx.classTokens);
    const model = map.parseMap(ext.inner, basemap);
    if (!model) return null;
    const variant = map.pickVariant(ctx.classTokens);
    return map.buildMap(model, variant, ctx.classTokens, ctx.orientation);
  });
}

function buildJourneySection(html, ctx) {
  // The kernel rewrites the nested <ul> into a `.journey-board` in place,
  // leaving the h2 for the chart-frame wrap to lift into the header.
  return journey.transformJourneySection(html, ctx.cls, ctx.orientation);
}

function buildWordCloudSection(html, ctx) {
  // The kernel rewrites the first <ul> into a `.word-cloud-canvas` in place,
  // leaving the h2 for the chart-frame wrap.
  return wordCloud.transformWordCloudSection(html, ctx.cls, ctx.orientation);
}

function buildRoadmapSection(html, ctx) {
  let cls = ctx.cls;
...
const AXIS_ARROW_DIRECTION = new Map([
  ['→', 'right'], ['▶', 'right'], ['➔', 'right'], ['➤', 'right'],
  ['←', 'left'], ['◀', 'left'],
  ['↑', 'up'], ['▲', 'up'],
  ['↓', 'down'], ['▼', 'down'],
]);
const AXIS_ARROW_GLYPH = {
  col: { right: '▶', left: '◀' },
  // Pre-flipped: the row axis's 180° rotation turns this glyph into its
  // opposite by the time it's on screen.
  row: { up: '▼', down: '▲' },
};
function normalizeAxisArrow(text, axis) {
  const m = text.match(/\s*([→←↑↓▶◀▲▼➔➤])\s*$/);
  if (!m) return text;
  const dir = AXIS_ARROW_DIRECTION.get(m[1]);
  const glyph = dir && AXIS_ARROW_GLYPH[axis][dir];
  if (!glyph) return text;
  return `${text.slice(0, m.index).trim()} ${glyph}`;
}
...
function buildMatrixGridSection(html, ctx) {
  ...
  const beforeH2 = h2Idx === -1 ? html : html.slice(0, h2Idx);
  const afterH2 = h2Idx === -1 ? '' : html.slice(h2Idx);
  const eyebrowMatch = beforeH2.match(/(<p[^>]*>\s*<code[^>]*>)([\s\S]*?)(<\/code>\s*<\/p>)\s*$/);
  let newBeforeH2 = beforeH2;
  if (eyebrowMatch) {
    const parts = eyebrowMatch[2].split('·'); // ·
    if (parts.length >= 2) {
      colAxis = normalizeAxisArrow(plainText(parts[0]).trim(), 'col');
      rowAxis = normalizeAxisArrow(plainText(parts.slice(1).join('·')).trim(), 'row');
      // BOTH halves move out of the masthead entirely — the source places
      // the column-axis label centered above the grid, beside the rotated
      // row-axis label, not as a masthead eyebrow next to the title. A
      // single-part eyebrow (no `·`) is unaffected and keeps the generic
      // chart-eyebrow treatment via liftChartEyebrow downstream.
      newBeforeH2 = beforeH2.slice(0, eyebrowMatch.index) +
        beforeH2.slice(eyebrowMatch.index + eyebrowMatch[0].length);
    }
  }
  const eyebrowHtml = newBeforeH2 + afterH2;
...
const SECTION_BUILDERS = {
  ...
  'timeline-list': buildTimelineSection,
  piechart: buildPiechartSection,
  gantt: buildGanttSection,
  kanban: buildKanbanSection,
  radar: buildRadarSection,
  quadrant: buildQuadrantSection,
  'state-chart': buildStateChartSection,
  funnel: buildFunnelSection,
  map: buildMapSection,
  journey: buildJourneySection,
  'word-cloud': buildWordCloudSection,
  roadmap: buildRoadmapSection,
  'matrix-grid': buildMatrixGridSection,
};

// ── The chart-frame wrap (eyebrow / h2 / subtitle, body, caption) ──────────

// Locate the h2 and the figure div the builder emitted, then find the
...
function extractChartBody(html) {
  ...
  const bodyRE = /<div\s+class="(?:progress-bars|timeline-spine|piechart-figure|gantt-chart|kanban-board|radar-figure|quadrant-figure|state-chart-figure|funnel-figure|map-figure|journey-board|word-cloud-canvas|roadmap-figure|matrix-grid-figure)"[^>]*>/;
...
function liftChartCaption(afterBody) {
  ...
  const captionScope = footerM ? afterBody.slice(0, footerM.index) : afterBody;
...
function transformChartSection(innerHtml, cls, orientation) {
  ...
  // to <ul>/<ol>; the builders' regexes all tolerate optional attributes on
  // the opening tag. Lattice's own emulator emits attribute-free tags so
  // the same patterns work for both render paths.
  const built = SECTION_BUILDERS[chartLayout](innerHtml, { cls, classTokens, orientation });
  let html = built;
  if (built && typeof built === 'object') { html = built.html; cls = built.cls; } // roadmap threads cls
