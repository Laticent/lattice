...
const { nextRenderSeq, renderIdPrefix, setRenderSection } = require('../../../core/render-ids');
...
// See svg-label.js.
const { wrapSvgLabel, measureLabel, ADVANCE_MONO_TRACKED, ADVANCE_HAND_TRACKED } = require('./svg-label');
...
function buildGanttSection(html, ctx) {
  ...
  // the same way journey and word-cloud below read ctx.cls. Keyed on the token
...
function buildRadarSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
    ...
    // Same `sketch` selection as the gantt axis above, for the same reason:
    // `.radar-sector-label` is `--font-body`, which the finish re-points at the
    // hand sans, and its wrap + de-collision boxes are estimated at build time.
    return radar.buildRadar(model, variant, scale, isMinimal, ctx.orientation,
      ctx.classTokens.includes('sketch'));
  });
}
