...
//
// The caption stays a `<p>` HERE. It becomes a `<figcaption>` only where a matching
// `<figure>` is actually built — inside `buildStageCell` (masthead.transform.js), which
// owns both halves of the pair. Emitting the `<figcaption>` from this end looked
// tidier and shipped three defects: with `form: off` no stage exists, so the caption
// went out with no `<figure>` anywhere (a conformance error); the two render paths
// re-derived "is there a caption?" from different evidence and disagreed; and a
// `state-chart` stage put the caption mid-figure, which the content model forbids.
