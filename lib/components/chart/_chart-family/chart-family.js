...
// Plain TEXT out of a markdown-it fragment. The SVG charts take text, not
// markup: the wrapping emitter escapes whatever it is handed, so tags must come
// off AND entities must be decoded first. Skipping the decode double-escapes —
// markdown-it writes `Ops &amp; IT`, the emitter escapes the `&` again, and the
// chart paints the literal `Ops &amp; IT`. (The old gantt interpolated the
// fragment straight into an HTML <div>, where the entity simply rendered.)
function stripTags(s) {
  return String(s == null ? '' : s)
    .replace(/<[^>]*>/g, '')
    .replace(/&nbsp;/g, ' ')
    // Decode last, and `&amp;` last of all, so `&amp;lt;` becomes `&lt;` (the
    // author's literal text) rather than being re-decoded into `<`.
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'")
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
