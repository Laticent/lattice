...
//
// The caption is a `<figcaption>`, not a `<p>` — it IS the caption of the
// graphic beside it, and `<figcaption>` is the one element that says so to an
// accessibility tree. The stage cell that later wraps {chart-body, caption}
// becomes the matching `<figure>` (masthead.transform.js `stageTag`); the two
// must stay in lockstep, which is why the tag decision keys on THIS string.
// Emitting `<figcaption>` outside a `<figure>` for the one render that skips
// the stage wrap is still valid HTML — it simply carries no association, which
// is what a `<p>` carried anyway. Styling is class-keyed, so nothing moves.
function liftChartCaption(afterBody) {
  ...
  if (emM) cap = emM[1];
  return {
    captionEl: `<figcaption class="chart-caption">${cap}</figcaption>`,
