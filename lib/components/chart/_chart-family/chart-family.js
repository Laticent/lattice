...
// Plain text out of a markdown-it fragment. The SVG charts take label TEXT, not
// markup — the wrapping emitter re-escapes what it is handed, so any inline tags
// must come off first (the gantt used to inline `<em>`/`<code>` straight into an
// HTML <div>, where they simply rendered).
...
// colour and a legend chip can never drift from the bar it stands for.
...
const GANTT_GEOM_TALL = {
  ...{},
  vbW: 300,
  laneW: 0,          // no label column — the name rides above the bars
  gutter: 0,
  padRight: 8,
  axisH: 20,
...
function buildGanttChart(ulInner, eyebrowHtml, orientation) {
  ...
  const ticks = buildGanttTicks(axisMin, axisMax, mode, hasMonthVocab)
  ...
  lanes.forEach((lane, li) => {
    ...
    lanesSvg += G.portrait
      ...
      : wrapSvgLabel(stripTags(lane.label), {
        ...
        anchor: 'end', vAlign: 'middle', maxLines: 2,
    ...
    lane.tasks.forEach((t, ti) => {
      ...
      if (isMilestone) {
        ...
        const wide = w.lines[0].length * G.fsBar * 0.6;
      ...
      barsSvg += `<rect class="gantt-bar" data-anima-role="bar"${sAttr}${mAttr} ` +
        ...
        `height="${G.barH}" rx="${G.barRx}" fill="url(#__GRAD_${fillKey}__)"/>`;
  ...
  if (keyStatuses.length) {
    ...
    legendSvg = '<g class="gantt-legend" role="list" aria-label="Status key">';
    for (const c of chips) {
      usedFills.add(ganttFillKey(c.st));
      legendSvg += `<rect class="gantt-legend-swatch" data-s="${escAttr(c.st)}" ` +
        ...
        `fill="url(#__GRAD_${ganttFillKey(c.st)}__)"/>`;
  ...
  // Only the gradients actually used are emitted, and their ids are namespaced
  // per chart so two gantts on one deck cannot collide (the SVG duplicate-id
  // trap the pie/quadrant/radar defs also guard against).
  const grad = ganttGradientDefs([...usedFills]);
  let body = axisSvg + lanesSvg + barsSvg + todaySvg + legendSvg;
  body = body.replace(/__GRAD_(\w+)__/g, (_m, k) => grad.id(k));

  const svg = `<svg class="gantt-svg" viewBox="0 0 ${G.vbW} ${vbH}" ` +
    `preserveAspectRatio="xMidYMid meet" role="img"><title>Gantt chart</title>` +
...
function ganttBarLabel(label, bx0, bx1, midY, limitX, G) {
  if (!label) return '';
  const inner = bx1 - bx0 - 6;
  const est = label.length * G.fsBar * 0.6;
  ...
  if (est <= inner) {
    return wrapSvgLabel(label, {
      ...
      anchor: 'start', vAlign: 'middle', maxLines: 1,
  ...
  // printed across a neighbouring task. The popover and the speaker note both
...
function buildWordCloudSection(html, ctx) {
  ...
  return wordCloud.transformWordCloudSection(html, ctx.cls);
}
