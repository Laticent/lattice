...
function graphLayoutKernel() {
  ...
  function layoutOnce(model, sizes, opts, dagre) {
    ...
    return { dir: lr ? 'lr' : 'tb', width, height, nodes, groups: gboxes, titles, routes, quality: measureQuality(nodes, gboxes, routes, titles), crossings: countCrossings(routes), grew };
    ...
    // count may get worse.
    // A run within 5 units of a stranger's box (outside it) for more than 8 reads as
    // touching it.
    function grazes(rs) {
      let n = 0;
      for (const r of rs) {
        for (const [id, b] of Object.entries(nodes)) {
          ...
          if (pathHits(r.points, { x: b.x - 5, y: b.y - 5, w: b.w + 10, h: b.h + 10 }) && !pathHits(r.points, b)) n++;
        }
      }
      return n;
    }
     ...
     * back, left by the far side, or shared a port. Kept only if nothing gets worse.
     */
    function routeFans(rs) {
      ...
      for (const { N, side, list } of sides.values()) {
        const n = list.length;
        if (n < 2 || new Set(list.map((it) => it.oid)).size < n) continue;
        ...
        const ends = list.map((it) => it.end);
        const span = Math.min(cHi(N) - cLo(N) - 16, (n - 1) * 14);
        if (span < (n - 1) * 8) continue;
        const port = list.map((_, i) => cMid(N) - span / 2 + (i * span) / (n - 1));
        const base = side > 0 ? aHi(N) : aLo(N);
        const near = side > 0 ? Math.min(...list.map((it) => aLo(it.O))) : Math.max(...list.map((it) => aHi(it.O)));
        const gap = Math.abs(near - base);
        const straight = list.map((it, i) => port[i] >= cLo(it.O) + 8 && port[i] <= cHi(it.O) - 8);
        const up = list.map((_, i) => i).filter((i) => !straight[i] && ends[i] < port[i]);
        const down = list.map((_, i) => i).filter((i) => !straight[i] && ends[i] > port[i]).reverse();
        const lanes = Math.max(up.length, down.length);
        const step = lanes > 1 ? Math.min(10, (gap - 32) / (lanes - 1)) : 0;
        if (lanes > 1 && step < 8) continue;
        const laneAt = (j) => base + side * (16 + j * step);
        const was = list.map((it) => ({ points: it.r.points, labelAt: it.r.labelAt }));
        // Scored with the labels seated as they will be, since a fan line's new run can
        // leave its label nowhere clean; then the labels go back for the real seating.
        const seated = () => {
        ...
        const before = seated();
        const draw = (i, lane) => {
          const it = list[i], c = port[i], far = side > 0 ? aLo(it.O) : aHi(it.O);
          let pts = lane == null
            ? [P(base, c), P(far, c)]
            : [P(base, c), P(lane, c), P(lane, ends[i]), P(far, ends[i])];
          if (it.at === 'end') pts = pts.reverse();
          it.r.points = pts;
          if (it.r.labelAt) it.r.labelAt = onLongestRun(pts);
        };
        list.forEach((_, i) => { if (straight[i]) draw(i, null); });
        up.forEach((i, j) => { draw(i, laneAt(j)); });
        down.forEach((i, j) => { draw(i, laneAt(j)); });
        // A labeled line needs a run long enough to hold its label.
        const room = list.every((it) => {
        ...
        if (!room || !noWorse(before, seated())) list.forEach((it, i) => { Object.assign(it.r, was[i]); });
      }
    }
    ...
    function rescueRoutes(rs, uncross) {
      ...
      const hitsOf = (pts, r) => {
        ...
        // A stranger's box also counts when the line merely grazes it: a run 3 units under
        // a box reads as touching it (the demo's release train, `fails` under Test).
        for (const [id, b] of Object.entries(nodes)) {
      ...
      // same x on a top or bottom side, the same y on a left or right one). A move may not
    ...
    const sharedOf = (pts, r) => {
      ...
      for (const r of order) {
        ...
        for (const c of cands) {
          ...
          const x = uncross ? crossOf(pts, r) : 0;
          ...
          const score = 300 * x + len(pts) + 24 * (pts.length - 2) + 400 * shared(pts) + 60 * turned;
  ...
  function layout(model, sizes, opts, dagre) {
    ...
    const misses = (geo) => Object.values(geo.quality).reduce((a, b) => a + b, 0);
    for (const d of dirs) {
      // Growing a fanning shape moves dagre's placement too, which can cost a crossing
      // elsewhere; so each direction is laid out both ways, and the grown one is kept only
      // when it misses no more, crosses no more, and sets the type no more than 3% smaller.
      ...
      const geo = misses(grown) <= misses(plain) && grown.crossings <= plain.crossings && grown.scale >= plain.scale * 0.97 ? grown : plain;
