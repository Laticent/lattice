...
function graphLayoutKernel() {
  ...
  function layoutOnce(model, sizes, opts, dagre) {
    ...
    return { dir: lr ? 'lr' : 'tb', width, height, nodes, groups: gboxes, titles, routes, quality: measureQuality(nodes, gboxes, routes, titles), crossings: countCrossings(routes) };
    ...
    function levelFans(rs) {
      ...
      const score = () => ({ q: measureQuality(nodes, gboxes, rs, {}), x: countCrossings(rs), g: foreign() });
      const noWorse = (a, b) => b.x <= a.x && b.g <= a.g && Object.keys(a.q).every((k) => b.q[k] <= a.q[k]);
    ...
    function rescueRoutes(rs, uncross) {
      const PAD = 8, AROUND = 18;
      const boxOf = (id) => nodes[id] || gboxes[id];
      const hitsOf = (pts, r) => {
        let n = 0;
        for (const [id, b] of Object.entries(nodes)) if (id !== r.from && id !== r.to && pathHits(pts, b)) n++;
        return n;
      };
      ...
      // Two ends on one side of a box closer than spreadPorts' own spacing (14) read as a
      // doubled line leaving together.
      const clashes = (pts, r) => {
        let n = 0;
        for (const o of rs) {
          if (o === r) continue;
          for (const e of [o.points[0], o.points[o.points.length - 1]]) {
            for (const q of [pts[0], pts[pts.length - 1]]) {
              const sameX = Math.abs(e.x - q.x) < 0.5, sameY = Math.abs(e.y - q.y) < 0.5;
              if ((sameX && Math.abs(e.y - q.y) < 14) || (sameY && Math.abs(e.x - q.x) < 14)) n++;
            }
          }
        }
        return n;
    ...
    const sharedOf = (pts, r) => {
      ...
      for (const r of order) {
        ...
        const curClash = uncross === 'keep' && curCross ? clashes(r.points, r) : Infinity;
        ...
        for (const c of cands) {
          ...
          if (uncross && (!roomFor(pts, r) || jogs(pts) || intrusions(pts, r) > curIn || clashes(pts, r) > curClash)) continue;
      ...
      if (uncross === 'keep') {
        for (const r of rs) {
          ...
          if (hitsOf(was, r) || crossOf(was, r) > crossOf(r.points, r) || sharedOf(was, r) > sharedOf(r.points, r) || intrusions(was, r) > intrusions(r.points, r) || clashes(was, r) > clashes(r.points, r)) continue;
          r.points = was;
