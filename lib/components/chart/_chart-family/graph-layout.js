 ...
 * `rescueRoutes(routes, true)` is the crossing solver, which moves a line that crosses
...
function graphLayoutKernel() {
  ...
  function layoutOnce(model, sizes, opts, dagre) {
    ...
    // carry it into a neighbour. A route still crossing a shape here is re-drawn.
    rescueRoutes(routes);
    // The solver: lines that cross others try every clean elbow and keep one that crosses fewer.
    rescueRoutes(routes, true);
    ...
    function rescueRoutes(rs, uncross) {
      ...
      // A labelled line needs one run long enough to carry its label, or seatLabels has
      ...
      const roomFor = (pts, r) => {
        ...
        return pts.slice(1).some((q, j) => Math.abs(q.x - pts[j].x) + Math.abs(q.y - pts[j].y) >= need);
      };
      ...
      for (const r of rs) {
        ...
        for (const [x0, x1, mid] of [
          ...
          for (const y1 of [ty, T.cy]) cands.push([{ x: x0, y: sy }, { x: mid, y: sy }, { x: mid, y: y1 }, { x: x1, y: y1 }]);
        }
        ...
        for (const [y0, y1, mid] of [
          ...
          for (const x1 of [tx, T.cx]) cands.push([{ x: sx, y: y0 }, { x: sx, y: mid }, { x: x1, y: mid }, { x: x1, y: y1 }]);
        }
        ...
        for (const c of cands) {
          ...
          if (uncross && (!roomFor(pts, r) || jogs(pts))) continue;
          const x = uncross ? crossOf(pts, r) : 0;
          const score = 300 * x + len(pts) + 24 * (pts.length - 2) + 400 * shared(pts);
