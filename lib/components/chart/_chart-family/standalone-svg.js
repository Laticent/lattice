 ...
 *   1. COLOR + TYPE come from the deck's stylesheet. Swatches/spine use
 ...
 *     Board imports it through Vite. It is closure-free so it serializes cleanly.
...
// literal, so the detached file needs no external CSS.
//
// It does NOT follow that the file needs no token definitions, and this comment
// claimed that it did. A paint that matches a scheme-varying token is deliberately
// left as `var(--token)` so the exported player can re-theme it (see SCHEME_TOKENS
// below) — which is precisely a reference the detached file has to define for
// itself. `collectTokens` + `finalizeStandaloneSvg` are what close that gap.
const STYLE_PROPS = [
  'fill', 'fill-opacity', 'fill-rule',
  'stroke', 'stroke-width', 'stroke-opacity', 'stroke-linecap', 'stroke-linejoin',
  'stroke-dasharray', 'stroke-dashoffset', 'stroke-miterlimit', 'vector-effect',
  'opacity', 'color',
  // Gradient stops carry their color here, NOT in `fill` — and the chart's
...
const INITIAL = {
  ...
  'paint-order': 'normal', 'mix-blend-mode': 'normal', 'vector-effect': 'none',
};

const SVG_NS = 'http://www.w3.org/2000/svg';

// What `finalizeStandaloneSvg` will admit as a custom-property value: a hex, a bare
// keyword (`red`, `none`, `currentColor`), or ONE call to a NAMED color function with
// a flat argument list. Every value it sees is `getComputedStyle` output, and a
// computed color is always one of those three — `color-mix()` and `var()` have already
// been resolved away by the time the bake reads it.
//
// The function name is an allowlist, not a shape test, because neither weaker form
// holds. A charset that merely listed the legal characters admitted `url(//host/x)`,
// and so did "any identifier followed by a flat argument list" — the `/` has to stay
// in the inner set for `oklch(62% 0.13 264 / 0.4)`'s alpha, which is exactly the
// character a protocol-relative fetch needs. Naming the functions closes it.
const TOKEN_VALUE_RE =
  /^(#[0-9a-fA-F]{3,8}|[a-zA-Z]+|(?:rgba?|hsla?|hwb|lab|lch|oklab|oklch|color|device-cmyk)\(\s*[a-zA-Z0-9\s.,%/-]*\))$/;

/**
 * Parse the `data-lattice-tokens` declaration list the bake writes — ONE grammar,
 * two consumers.
 *
 * `flattenSvgStyles` is the producer; `finalizeStandaloneSvg` (file exporters) and
 * `applyCollectedTokens` (the Studio's rasterizer path) are the consumers, and they
 * put the definitions in different places — a `<style>` rule in one, inline custom
 * properties in the other. They must not each keep their own copy of the format:
 * a second reader that re-derives the producer's grammar is a check re-asking the
 * question the producer already answered, and it drifts silently the first time the
 * emit changes. So both call this, and only this.
 *
 * Rejects anything that is not `--name: <color literal>`, per TOKEN_VALUE_RE above.
 * Pure string work — no DOM, Node-testable.
 *
 * @param {string} css - `--a:rgb(1,2,3);--b:#fff;`
 * @returns {Array<[string, string]>} admitted [name, value] pairs, in source order
 */
function parseTokenDecls(css) {
  const out = [];
  const decls = String(css || '').split(';');
  for (let i = 0; i < decls.length; i++) {
    const at = decls[i].indexOf(':');
    if (at < 1) continue;
    const name = decls[i].slice(0, at).trim();
    const value = decls[i].slice(at + 1).trim();
...
/**
 * Move the bake's collected token definitions onto an element's OWN inline style,
 * and strip the scratch attribute. DOM, no serialization — the other half of
 * `collectTokens`, for the consumer that has an element rather than a file.
 *
 * WHY A SECOND CONSUMER AT ALL. `finalizeStandaloneSvg` turns the same attribute
 * into a `<style>` rule, which is right when the output is a `.svg` FILE. The
 * Studio's PDF/PPTX rasterizer has no file to write into: it hands the flattened
 * clone to html-to-image, which serializes the subtree into a detached document.
 * And an `<svg>` is the one shape html-to-image does NOT walk — `cloneSingleNode`
 * deep-clones it (`node.cloneNode(isSVGElement(node))`) and `cloneChildren` returns
 * early on it, so not one descendant ever gets a computed-style copy. Every
 * descendant arrives carrying exactly the inline style the bake wrote, `var(--token)`
 * included, into a document where nothing defines the token. `var()` resolves to
 * nothing, `fill` falls to its SVG initial, and the initial is BLACK: measured at 47
 * of 62 paints on a flattened heatmap (#2210).
 *
 * The root, though, IS cloned with its attributes — so a definition written here
 * reaches the serialized document and every descendant inherits it. That is the
 * whole fix, and it is why `collectTokens` alone was never enough: nothing finalizes
 * on this path.
 *
 * The values go in through CSSOM (`setProperty`), never by building CSS text, so
 * there is no stylesheet for a bad value to escape into — the parse above is a
 * grammar check, not an injection guard. The scratch attribute is removed whether or
 * not anything in it was admitted.
 *
 * @param {SVGElement|HTMLElement} el - a clone returned by `flattenSvgStyles`
 * @returns {number} how many definitions were applied
 */
function applyCollectedTokens(el) {
  if (!el?.getAttribute || !el.style) return 0;
  const css = el.getAttribute('data-lattice-tokens');
  // Strip FIRST, and unconditionally: it is scratch space between the two halves of
  // one export, and a clone that keeps it publishes the deck's resolved palette into
  // whatever the caller does with the element next. The docblock said "always" while
  // the code returned early on an empty attribute, leaving `data-lattice-tokens=""`
  // in place. Unreachable from today's producer (the bake only sets the attribute
  // when it has something to say), but the docblock is what the next reader trusts.
  if (el.removeAttribute) el.removeAttribute('data-lattice-tokens');
  if (!css) return 0;
  const decls = parseTokenDecls(css);
  for (let i = 0; i < decls.length; i++) el.style.setProperty(decls[i][0], decls[i][1]);
  return decls.length;
}
 ...
 * parallel clone. Returns the clone (caller serializes it). Closure-free so it
 ...
 *
 * `opts.collectTokens: true` additionally records every custom property the clone
 * still references and hands the resolved values out on a `data-lattice-tokens`
 * attribute, for `finalizeStandaloneSvg` to turn into the file's own scoped rule.
 * Pass it when the output is headed for that call.
 *
 * WITHHOLDING IT IS NOT THE SAME AS BEING SAFE, and an earlier draft of this line
 * conflated the two by calling the three non-collecting sites "an inline clone that
 * stays in a themed host". Two of them are that. The third was
 * `deck-export.js`'s `flattenChartSvgs`, which hands its clone to `html-to-image`
 * — and html-to-image serializes into a detached document that copies no custom
 * property at all (measured: `getComputedStyle(documentElement)` enumerates none,
 * and `cssText` on the clone is empty), so a followed token there resolved to
 * nothing exactly as it would in a file. Measured on a real flattened heatmap, 47
 * of 62 paints computed opaque black in that serialized clone — on the PDF and PPTX
 * a user actually downloads.
 *
 * THAT SITE NOW COLLECTS TOO (#2210), which makes it six of eight rather than five.
 * It does not finalize — there is no file — so it takes the definitions off the
 * attribute and onto the clone root's own inline style via `applyCollectedTokens`,
 * which is the second consumer of the same collection and the reason the flag alone
 * was never the fix. The two remaining non-collecting sites are genuinely inline
 * clones in a themed host, where the token is already defined around them.
 *
 * @param {SVGElement} srcSvg - the live, rendered chart <svg>
 * @param {Window} [win] - defaults to the global window
 * @param {{foreignObjectLabels?: 'keep'|'text', collectTokens?: boolean}} [opts]
...
function flattenSvgStyles(srcSvg, win, opts) {
  ...
  // resolve a stop's color we evaluate its authored expression through a probe
  ...
  // EVERY TOKEN THIS BAKE EMITS, with the value it resolved to here. The
  // `var(--token)` above exists so the PLAYER can re-theme a baked chart when its
  // toggle moves — the host defines the token, the chart follows. A DETACHED .svg
  // file has no host: the var() resolves to nothing and the paint falls to the SVG
  // initial, which for `fill` is BLACK. Measured before this map existed: a heatmap
  // exported at 3 of 5 stops below AA (worst 1.98:1), and a `map` export painted 172
  // of its regions black. `finalizeStandaloneSvg` — which only the FILE exporters
  // call — turns this into a `svg{…}` rule so the file carries its own definitions.
  //
  // OPT-IN, because eight call sites share this bake and only six of them CONSUME the
  // collection. The attribute is scratch space between the two halves of one export:
  // set it unconditionally and the other two ship a dead attribute nobody strips, each
  // copy publishing the deck's fully resolved palette to a file that had no reason to
  // carry it. Measured on the CLI player export (`--player`, which flattens a diagram
  // inline and never finalizes): 2 copies / 504 bytes on the baseline gallery, 2 / 168
  // on `examples/a11y.md`. Re-derive by forcing COLLECT true and diffing the export —
  // the plain `.html` sidecar is NOT the arm to use, it never reaches this bake at all.
  //
  // CONSUME, not "finalize", and the distinction is the whole of #2210. Five sites hand
  // the attribute to `finalizeStandaloneSvg`, which turns it into a `<style>` rule in a
  // FILE: the CLI's chart and diagram bakes (`lattice-emulator.js`, one of them the
  // cross-scheme look re-render, which is easy to miss because its markup reaches the
  // finalize by SUBSTITUTION rather than by return), `tools/export-chart-svg.js`, and
  // the Studio's single-chart and whole-deck SVG exports. The sixth is the Studio's
  // rasterizer (`deck-export.js`'s `flattenChartSvgs`), which writes no file and calls
  // `applyCollectedTokens` instead — same collection, inline custom properties on the
  // clone root. Counting FINALIZERS rather than consumers is what made this look like a
  // safe site for as long as it did.
  const COLLECT = opts ? !!opts.collectTokens : false;
  const usedTokens = {};
  // A name the attribute sweep finds may not hold a COLOR — it can be a length, a
  // number, or a token nothing defines. `color: var(--x)` is then invalid at
  // computed-value time, and because `color` INHERITS, the probe answers with the
  // inherited ink: a real color, indistinguishable from a real resolution, and baked
  // into the file as a definition the token never had. `background-color` does not
  // inherit, so the same failure lands on the initial `rgba(0, 0, 0, 0)` and is
  // recognizable. A genuinely transparent token reads the same and is dropped, which
  // is the right call either way: a definition we cannot trust is worse than none.
  function resolveTokenPaint(name) {
    probe.style.backgroundColor = '';
    probe.style.backgroundColor = 'var(' + name + ')';
    const got = w.getComputedStyle(probe).backgroundColor || '';
    probe.style.backgroundColor = '';
    return (got === 'rgba(0, 0, 0, 0)' || got === 'transparent') ? '' : got;
  }
  ...
  function walk(src) {
    ...
    if (src.nodeType === 1) {
      ...
      if (tag === 'stop') {
        ...
        // A STROKED element never drops its width or its vector-effect, even at
        // the initial value — because `stroke-width: 1px` does not mean the same
        // thing here and there. On the slide the chart family pins every mark
        // edge with `vector-effect: non-scaling-stroke`, so `1px` is one CSS
        // pixel. In the detached clone there is no deck CSS, so an omitted width
        // falls back to the SVG default of one USER UNIT, which the viewBox then
        // scales: measured by driving both versions of this function over the
        // same live SVGs, a pinned 1px edge came back at 0.84px on a map region
        // and 2.46px on a scatter bubble. (Not the scatter DOT — that is a
        // knockout with an explicit 0.9, which is not the initial and was never
        // dropped.) Both properties are still omittable on an element
        // that paints no stroke, where they change nothing and would otherwise be
        // emitted onto every <g> in the tree.
        const strokes = cs.getPropertyValue('stroke') && cs.getPropertyValue('stroke') !== 'none';
        for (let i = 0; i < PROPS.length; i++) {
          const p = PROPS[i];
          const v = cs.getPropertyValue(p);
          if (!v) continue;
          const pinned = strokes && (p === 'stroke-width' || p === 'vector-effect');
          if (!pinned && Object.hasOwn(INIT, p) && v === INIT[p]) continue;
          ...
          if (Object.hasOwn(PAINT_PROPS, p)) {
            ...
            if (token) { out = 'var(' + token + ')'; usedTokens[token] = v; }
          }
          decl += p + ':' + out + ';';
        }
      }
      ...
      for (let n = src.firstChild; n; n = n.nextSibling) {
        if (n === probe) continue; // never serialize the resolver probe (parentNode fallback)
  ...
  // EVERY OTHER ROUTE OUT, and there are three. The loop above records what the
  // COMPUTED-STYLE bake emitted. A token can also reach the clone through the
  // element's own inline style, which rides along verbatim in `keepPrev`, and
  // through a PRESENTATION ATTRIBUTE the bake never rewrites — `map` carries 16
  // `fill="color-mix(in oklab, var(--chart-cat-1-hue) 88%, var(--map-base))"` of
  // exactly the third shape, and a sweep that read only `[style]` left that token
  // undefined in the file. So scan EVERY attribute of every element and resolve
  // whatever names it finds, while the probe is still alive and still parented
  // inside the chart frame (which is what lets a frame-scoped token resolve at all).
  if (COLLECT && out.querySelectorAll) {
    const all = out.querySelectorAll('*');
    const seenName = {};
    for (let si = -1; si < all.length; si++) {
      const el = si < 0 ? out : all[si];
      const attrs2 = el.attributes;
      if (!attrs2) continue;
      for (let ai = 0; ai < attrs2.length; ai++) {
        const txt = attrs2[ai].value || '';
        if (txt.indexOf('var(') === -1) continue;
        let m;
        // Match the NAME and stop — not `var\((--x)\)`. That form missed the two
        // shapes CSS allows around it: `var( --x )` (whitespace is legal after the
        // paren) and `var(--x, var(--y))` (a fallback, where the outer group never
        // closes before the inner one). Ending the match at the name catches both,
        // and the global scan then finds the nested name on its next pass.
        const re = /var\(\s*(--[a-zA-Z0-9-]+)/g;
        while ((m = re.exec(txt))) {
          const nm = m[1];
          if (seenName[nm] || usedTokens[nm]) continue;
          seenName[nm] = 1;
          const resolved = resolveTokenPaint(nm);
          if (resolved) usedTokens[nm] = resolved;
        }
      }
    }
  }
  ...
  probe.remove();
  // Carry the namespace explicitly — a cloned-then-serialized SVG needs xmlns to
  // open as a file (finalizeStandaloneSvg also guards this). Literal, not the
  // module SVG_NS const: this fn is serialized via toString() into a browser
  ...
  // Ride out on the root rather than in the return value: this function hands back
  // an ELEMENT, and all eight callers destructure nothing — changing the shape to
  // carry a second value would break all of them for one consumer's benefit.
  // `finalizeStandaloneSvg` reads this and strips it.
  if (out.setAttribute) {
    // Clear it unconditionally first. The clone copies the source's attributes, so a
    // re-flattened SVG would otherwise carry a STALE definition list forward into a
    // path that never asked for one.
    if (out.removeAttribute) out.removeAttribute('data-lattice-tokens');
    if (COLLECT) {
      const names = Object.keys(usedTokens);
      let css = '';
      for (let i = 0; i < names.length; i++) css += names[i] + ':' + usedTokens[names[i]] + ';';
      if (css) out.setAttribute('data-lattice-tokens', css);
    }
  }
  return out;
}
...
function finalizeStandaloneSvg(svgMarkup, opts) {
  ...
  // `flattenSvgStyles` emits `fill:var(--token)` for every paint that matched a
  // scheme-varying token, so the PLAYER can re-theme a baked chart. A detached file
  // has no host to define them: the var() resolves to nothing and the paint falls to
  // the SVG initial — black for `fill`. So the bake hands the resolved values out on
  // `data-lattice-tokens` and they become a `svg{…}` rule here, where the file can
  // carry itself. The bake only collects when asked (`collectTokens`), which is the
  // FIVE file exporters enumerated at that flag; the three inline paths never do, so
  // their re-theming — and their byte count — are unaffected. (Five, not four. This
  // line said four, which made it the THIRD time this count shipped wrong in one
  // branch, in the docblock of the commit that corrected the other two. Count the
  // call sites that reach a finalize, not the ones that look like exporters.)
  //
  // SANITIZED like the background below, and for the same reason: the values arrive
  // as `getComputedStyle` output (`rgb(30, 58, 95)`), but a future caller could hand
  // this something else, and CSS text inside CDATA is a place to be careful. A name
  // must be a custom property; a value must be a COLOR LITERAL — a hex, a bare
  // keyword, or one un-nested functional form. The earlier charset test was a
  // character filter rather than a grammar, so it passed `url(//evil/x)` and
  // `a(b(c))`; this one cannot, because `:` is not in the inner set and an inner
  // `(` is not either.
  //
  // Read the root's attributes by TOKENIZING them rather than searching the text for
  // the name. `attrs` is one element's attribute list, so every value is `"`-quoted
  // and cannot itself contain a `"` — which makes this walk exact, and keeps a value
  // that merely SPELLS `data-lattice-tokens="…"` from being read as the attribute.
  let tokenCss = '';
  const ATTR_RE = /([:A-Za-z_][-:.\w]*)\s*=\s*"([^"]*)"/g;
  let am;
  while ((am = ATTR_RE.exec(attrs))) {
    if (am[1] !== 'data-lattice-tokens') continue;
    attrs = attrs.slice(0, am.index) + attrs.slice(am.index + am[0].length);
    const decls = parseTokenDecls(am[2]);
    for (let i = 0; i < decls.length; i++) tokenCss += `${decls[i][0]}:${decls[i][1]};`;
    break;
  }

  // SCOPE THE RULE TO THIS FILE'S OWN ROOT. A `<style>` inside an SVG is
  // DOCUMENT-scoped wherever that SVG ends up, and a bare `svg{…}` selector matches
  // every `<svg>` in the document — so the moment someone INLINES an exported chart
  // into an HTML page (the most natural thing to do with one), its palette lands on
  // every other chart on that page. Measured: a light-mode and a dark-mode heatmap
  // export inlined into one page resolved the SAME `--heatmap-base` and
  // `--heatmap-step5-ink`, because whichever `<style>` parsed last won both.
  //
  // THE SCOPE IS OUR OWN ATTRIBUTE, NOT THE ROOT'S `id`, and the first cut of this
  // fix got that wrong in a way that reinstated the whole bug for diagrams. It
  // reused an existing id when the root had one — and a Mermaid export always has
  // one, numbered per deck from 1, so two decks on two themes both emitted
  // `#lattice-mmd-1{…}` and the second re-palettized the first. Measured in
  // Chromium: `--c-container` on deck 1 flipped to deck 2's value the moment both
  // were on one page.
  //
  // Taking the id over instead is worse than it looks: a root id can be the target
  // of `url(#…)` or `aria-labelledby` inside the same file, so rewriting it breaks
  // references we did not author. A private attribute collides with nothing, needs
  // no rewrite, and cannot produce the duplicate-`id` document the id path produced
  // whenever an existing id was unusable.
  //
  // The value is a content hash, so it is DETERMINISTIC — the same export re-runs
  // byte for byte, and two different palettes can never share a selector. Two
  // exports with IDENTICAL declarations do share one, which is harmless by
  // construction: the rules they would win from each other are the same rules.
  // `:root` was the obvious alternative and is worse: standalone it means the
  // `<svg>`, but inlined it means `<html>`, which pushes the palette onto the whole
  // host page instead of containing it.
  let tokenSel = '';
  if (tokenCss) {
    let h = 2166136261; // FNV-1a over the declarations
    for (let i = 0; i < tokenCss.length; i++) { h ^= tokenCss.charCodeAt(i); h = Math.imul(h, 16777619); }
    const scope = 'c' + (h >>> 0).toString(36);
    attrs = attrs.replace(/(?:^|\s)data-lattice-scope\s*=\s*"[^"]*"/g, '');
    attrs += ` data-lattice-scope="${scope}"`;
    tokenSel = `[data-lattice-scope="${scope}"]`;
  }
  ...
  const sheet = [tokenCss ? `${tokenSel}{${tokenCss}}` : '', fontCss].filter(Boolean).join('\n');
  const styleBlock = sheet
...
/**
 * The engine's BAKED delivery mode, for a consumer that keeps an element — BROWSER ONLY.
 *
 * `baked` is one of the three named style-delivery modes
 * (`engineering/decisions/2026-09-24-one-style-delivery-spine.md` §4): the host has no
 * stylesheet, so each chart SVG carries its own computed paint and its own token
 * definitions. `scoped` and `flat` are strings `render()` returns; this one cannot be,
 * because it reads computed styles, so it is this function instead. Every element
 * consumer calls it, and none of them pairs the two halves by hand any more: the
 * Studio's diagram bake and PDF/PPTX rasterizer (`deck-export.js`) and the
 * `check:render` gate's baked pass (`tools/check-viz-render.js`). A FILE consumer
 * (a `.svg` download) still calls `flattenSvgStyles` + `finalizeStandaloneSvg`,
 * because it writes the definitions into the file's own `<style>`, not onto an element.
 *
 * `opts.freezeTokens` (default true) collects every token the clone still reads and
 * defines it on the clone root (`applyCollectedTokens`). Pass `false` ONLY where the
 * clone stays inside a host that defines the tokens and must keep following them: the
 * exported player's diagram bake, whose dark/light toggle moves them.
 * `opts.foreignObjectLabels` passes through to `flattenSvgStyles`.
 *
 * @param {SVGElement} srcSvg - the live, rendered <svg>
 * @param {Window} [win]
 * @param {{freezeTokens?: boolean, foreignObjectLabels?: 'keep'|'text'}} [opts]
 * @returns {SVGElement} a detached, self-styled clone
 */
function bakeSvg(srcSvg, win, opts) {
  const plan = bakePlan(opts);
  const flat = flattenSvgStyles(srcSvg, win, plan.flatten);
  if (plan.freeze) applyCollectedTokens(flat);
  return flat;
}

/**
 * `bakeSvg`'s option policy, pure so a test can pin it without a browser (jsdom
 * resolves no custom property, so it cannot watch a token freeze). Freezing is the
 * DEFAULT, and only a literal `false` turns it off: a host with no stylesheet is the
 * reason this mode exists, and a bake that forgets to freeze paints black there (#2210).
 * @param {{freezeTokens?: boolean, foreignObjectLabels?: 'keep'|'text'}} [opts]
 * @returns {{freeze: boolean, flatten: {foreignObjectLabels?: 'keep'|'text', collectTokens: boolean}}}
 */
function bakePlan(opts) {
  const o = opts || {};
  const freeze = o.freezeTokens !== false;
  return { freeze, flatten: { foreignObjectLabels: o.foreignObjectLabels, collectTokens: freeze } };
}
