...
function flattenSvgStyles(srcSvg, win, opts) {
  ...
  // the player's inline path never finalises, so its re-theming is untouched.
  const usedTokens = {};
    ...
    for (let si = -1; si < all.length; si++) {
      ...
      for (let ai = 0; ai < attrs2.length; ai++) {
        ...
        while ((m = re.exec(txt))) {
          ...
          seenName[nm] = 1;
          const resolved = resolveColor('var(' + nm + ')');
  ...
  // an ELEMENT, and three callers destructure nothing — changing the shape to carry
  // a second value would break all of them for one consumer's benefit.
...
function finalizeStandaloneSvg(svgMarkup, opts) {
  ...
  // carry itself. Only the three FILE exporters call this function; the player's
  // inline path never does, so its re-theming is unaffected.
  //
  // SANITISED like the background below, and for the same reason: the values arrive
  ...
  // must be a custom property; a value admits only what a resolved paint contains.
  let tokenCss = '';
  const tokenAttr = attrs.match(/\sdata-lattice-tokens\s*=\s*"([^"]*)"/);
  if (tokenAttr) {
    attrs = attrs.replace(tokenAttr[0], '');
    const decls = tokenAttr[1].split(';');
    for (let i = 0; i < decls.length; i++) {
      const at = decls[i].indexOf(':');
      if (at < 1) continue;
      const name = decls[i].slice(0, at).trim();
      const value = decls[i].slice(at + 1).trim().replace(/&quot;/g, '').replace(/&amp;/g, '&');
