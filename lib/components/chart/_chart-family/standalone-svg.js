...
function flattenSvgStyles(srcSvg, win, opts) {
  ...
  // SCHEME-VARYING paint tokens. Freezing a computed color is right for a diagram whose
  // scheme is pinned, and WRONG for one on an unpinned slide in the exported player, where
  // the viewer owns a light/dark toggle: the paint stays at its export-time value while the
  // surface under it flips. Measured on an ordinary unpinned `_class: diagram` slide, the
  // flowchart's connector strokes stayed `#1A1A1A` on a `#001D33` canvas — 1.09:1, invisible
  // — while their arrowheads re-themed through a CSS `!important` rule, so the diagram read
  // as floating arrowheads with no lines between them.
  //
  // So when a frozen paint value EQUALS the current resolution of one of these tokens, emit
  // the token instead of the literal and let it follow the toggle — exactly what the label
  // ink below already does with `--text-heading`. Only tokens that actually vary by scheme
  // are listed: a fixed token gains nothing from the indirection and only adds a way to
  // mis-attribute an author's own color. A coincidental match re-themes a hand-picked
  // color, which is the same risk the label path already accepts and a far better failure
  // than paint that disappears against its own background.
  const SCHEME_TOKENS = [
    '--diagram-line', '--diagram-active', '--diagram-done',
  ...
  function foreignObjectToText(fo) {
    ...
    for (let n = walker.nextNode(); n; n = walker.nextNode()) {
      ...
      for (const run of lineRuns(n)) {
        ...
        // alone judged all of those to be author choices: on the diagram gallery deck, 18
        ...
        // light card at 1.12:1. Check the whole set of label inks and emit whichever token
        // matches; only a color that matches NONE of them is the author's own.
