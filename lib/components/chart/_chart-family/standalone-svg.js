 ...
 * attribute, for `finalizeStandaloneSvg` to turn into the file's own `svg{…}` rule.
 * Pass it ONLY when the output is headed for that call: a detached file needs the
 * definitions, and an inline clone that stays in a themed host does not.
 *
...
function finalizeStandaloneSvg(svgMarkup, opts) {
  ...
  // four FILE exporters; the player's inline path never does, so its re-theming — and
  // its byte count — are unaffected.
  //
  ...
  const sheet = [tokenCss ? `svg{${tokenCss}}` : '', fontCss].filter(Boolean).join('\n');
