 ...
 * stays in a themed host". Two of them are that. The third is
 ...
 * and `cssText` on the clone is empty), so a followed token there resolves to
 * nothing exactly as it would in a file. Measured on a real flattened heatmap, 47
 * of 62 paints compute opaque black in that serialized clone.
 *
 * That is PRE-EXISTING, not something the flag changed: `followToken` emits
 * `var(--token)` on `origin/main` and that call site passed no options there
 * either, so the behavior is byte-identical. It is logged as #2210 rather than
 * fixed here (HARD RULE #18, found-not-caused), and `collectTokens` alone would
 * not fix it —
 * nothing finalizes on that path, so the definitions would have to be written onto
 * the clone's own root. What this docblock owes is an accurate account of which
 * sites are safe and why, which is this.
 *
...
function flattenSvgStyles(srcSvg, win, opts) {
  ...
  //
  // OPT-IN, because eight call sites share this bake and only five of them finalize.
  // The attribute is scratch space between the two halves of one export: set it
  // unconditionally and the other three ship a dead attribute nobody strips, each copy
  // publishing the deck's fully resolved palette to a file that had no reason to carry
  // it. Measured on the CLI player export (`--player`, which flattens a diagram inline
  // and never finalizes): 2 copies / 504 bytes on the baseline gallery, 2 / 168 on
  // `examples/a11y.md`. Re-derive by forcing COLLECT true and diffing the export — the
  // plain `.html` sidecar is NOT the arm to use, it never reaches this bake at all.
  // The five are the CLI's chart and diagram bakes (`lattice-emulator.js`, one of them
  // the cross-scheme look re-render, which is easy to miss because its markup reaches
  // the finalize by SUBSTITUTION rather than by return), `tools/export-chart-svg.js`,
  // and the Studio's single-chart and whole-deck SVG exports.
