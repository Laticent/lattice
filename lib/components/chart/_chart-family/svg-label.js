 ...
 *
 *   flat 0.68        0.61 … 2.04   under-counts `WORKFLOW` by 39%, over-counts
 *                                  `IL ILI` by 104%
 *   this table       1.02 … 1.08   never under-counts; at most 8% generous
 *
...
// An unmapped character (any non-Latin script, a dingbat, a currency mark) bills
// at the WIDEST glyph in its own face — `W` clean, `M` hand. Wrong in the
// generous direction by construction, which is the only direction that cannot
// clip. A label written entirely in an unmapped script therefore wraps early
// rather than overrunning; measuring those scripts is a separate change, and
// this fallback is what makes it a safe one to defer.
const GLYPH_UPPER_MAX = { clean: 1.00, hand: 1.05 };
 ...
 */
function upperAdvance(text, { hand = false, tracking = 0 } = {}) {
  const face = hand ? 'hand' : 'clean';
  const tbl = GLYPH_UPPER[face];
  const max = GLYPH_UPPER_MAX[face];
  const s = String(text == null ? '' : text).toUpperCase();
  if (!s.length) return max + tracking;
  let sum = 0;
  for (const ch of s) sum += tbl[ch] === undefined ? max : tbl[ch];
  return sum / s.length + tracking;
}
...
function measureLabel(text, { width, fontSize, maxLines = Infinity, advance = ADVANCE, lineHeight = LINE_HEIGHT }) {
  ...
  // over the WHOLE label, so a narrow word can subsidise a wide one: `IL ILI
...
module.exports = {
  ...
  LABEL_ANCHORS, upperAdvance, GLYPH_UPPER, GLYPH_UPPER_MAX,
