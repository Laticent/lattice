 ...
 *
 * The order IS the preference order. Above first (a name reads as a caption to
 * the thing under it), then below, then the sides, then the diagonals — which
 * are what let a tight cluster fan out instead of stacking.
 */
const LABEL_ANCHORS = [
  { key: 'above', dx: 0, dy: -1, anchor: 'middle', baseline: 'auto', vAlign: 'bottom' },
  { key: 'below', dx: 0, dy: 1, anchor: 'middle', baseline: 'hanging', vAlign: 'baseline' },
  { key: 'right', dx: 1, dy: 0, anchor: 'start', baseline: 'middle', vAlign: 'middle' },
  { key: 'left', dx: -1, dy: 0, anchor: 'end', baseline: 'middle', vAlign: 'middle' },
  { key: 'above-right', dx: 1, dy: -1, anchor: 'start', baseline: 'auto', vAlign: 'bottom' },
  { key: 'above-left', dx: -1, dy: -1, anchor: 'end', baseline: 'auto', vAlign: 'bottom' },
  { key: 'below-right', dx: 1, dy: 1, anchor: 'start', baseline: 'hanging', vAlign: 'baseline' },
  { key: 'below-left', dx: -1, dy: 1, anchor: 'end', baseline: 'hanging', vAlign: 'baseline' },
];
...
function placeLabels(items, { obstacles = [], bounds = null, gap = 4, minGap = 1.5, nudge = 12 } = {}) {
  ...
  for (const it of items) {
    ...
    // adjacency, which is why it is scored down, but the label is still
    // unmistakably that dot's.
    ...
    for (let ring = 0; ring < 3 && !best; ring++) {
      for (let i = 0; i < LABEL_ANCHORS.length; i++) {
        ...
        if (score === 0) { best = { cand, a }; break; }
        // Ring and preference order only break ties between equally-bad spots;
        // they must never outweigh an actual collision.
        const ranked = score + ring * 0.5 + i * 0.1;
        if (!leastOverlap || ranked < leastOverlap.score) leastOverlap = { cand, a, score: ranked };
      }
    }
      ...
      chosen = dy
        ? {
          ...
          cand: wrapSvgLabel(it.text, {
            ...it.spec,
            x: it.cx + a.dx * (it.r + gap),
            y: it.cy + a.dy * (it.r + gap) + dy,
            anchor: a.anchor, baseline: a.baseline, vAlign: a.vAlign,
          }),
        }
        : { cand, a };
