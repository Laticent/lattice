...
// (weight 600, 0.12em tracking), over the COMPLETE tick vocabulary — all 1616
// strings `buildGanttTicks` can emit (`Q1`…`Q4` / `Jan`…`Dec`, each with every
// two-digit year tag). That set is CLOSED: no author text reaches this label, so
// the measured maximum is a real bound, not a sample.
//
...
//   ≥ 0.889 — below the measured worst case the wrapper under-counts and the
//             label overruns its box (the defect this module exists to remove).
//   ≤ 0.941 — 56 units / (7 chars × 8.5 fsTick). Above that the one-line budget
//             falls to 6 characters and `maxLines: 1` ELLIPSIZES the ordinary
//             7-character tick ('Jan '26' → 'Jan …'). Here "break early" means
//             lose text, so the usual generous safety margin is itself the bug —
//             which is why this is calibrated to the vocabulary instead of
...
module.exports = {
  ...
  LABEL_ANCHORS,
  ADVANCE, ADVANCE_UPPER, ADVANCE_MONO_TRACKED, ADVANCE_HAND_TRACKED, LINE_HEIGHT, BASELINE_EXTENT,
};
