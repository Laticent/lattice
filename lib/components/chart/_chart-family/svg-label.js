...
function wrapSvgLabel(text, {
  ...
  // `dominant-baseline: auto`, and the two engines resolve that `auto`
  // differently: Chromium (SVG2) resolves it against the parent's COMPUTED
  // value, so the glyphs center on the tspan's y; WebKit (SVG 1.1) resolves it
  // to `alphabetic`, so y becomes the glyph baseline and the whole label paints
  // about one font-size too high. Measured across the chart gallery, 107 of 256
  // labels sat 4-13px high on a 1280x720 slide in WebKit and 0 in Chromium —
  // and every gate in this repo renders through headless Chromium, so nothing
  // here could see it (#2297,
