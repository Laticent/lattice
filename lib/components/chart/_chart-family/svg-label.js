 ...
 */
function leaderLine(mark, placed, { className = 'chart-leader', minGap = LEADER_MIN_GAP } = {}) {
  if (!mark || !placed?.svg || !placed.box) return '';
  const { cx, cy, r } = mark;
  const b = placed.box;
  // The point on the label's box nearest the mark — where a reader's eye would
  ...
  if (!(dist > r + minGap)) return '';
  const ux = dx / dist;
  const uy = dy / dist;
  return `<line class="${className}" x1="${round(cx + ux * (r + 0.9))}" y1="${round(cy + uy * (r + 0.9))}" `
    + `x2="${round(nx - ux * 1.2)}" y2="${round(ny - uy * 1.2)}"/>`;
}
