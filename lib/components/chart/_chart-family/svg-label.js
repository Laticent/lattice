...
const GLYPH_UPPER = {
  clean: {
    ...
    // which bills 1.10.
    //
    // FIVE OF THESE ARE NOT IN EITHER WOFF2: `―` `⸺` `⸻` `→` `　`. The HOST
    // paints them, so their numbers are readings of the host's fonts and no
    // digest in GLYPH_UPPER_FONTS pins them — `npm run fonts:measure` labels
    // them `unpinned` for exactly this reason, and holds them at their committed
    // values rather than overwriting them with one machine's reading.
    //
    // And only ONE of the five comes from a fallback FACE. Measured: `→` is
    // system-ui (0.838em here), while `―` `⸺` `⸻` `　` measure identically
    // under system-ui, `sans-serif` and a family that cannot exist — they are
    // LAST-RESORT boxes, so 1.00 / 2.00 / 3.00em is the notdef advance, not a
    // measurement of any face that has the glyph. That still makes 2.05 / 3.05 a
    // real bound for the em-quad dashes (a font that HAS them draws them at a
    // definitional 2em and 3em, and a font that lacks them draws the same box we
    // measured), which is why they stay mapped: no single fallback value could
    // bound them. For the three that sit BELOW 1.10 the mapping is a NARROWER
    // bound than the fallback would have given, retained rather than dropped
    // only because every host measured so far paints them under it — see the
    // 2026-08-12 note, "found, not fixed".
    '—': 0.90, '–': 0.60, '―': 1.05, '⸺': 2.05, '⸻': 3.05,
    '…': 0.85, '•': 0.40, '→': 0.90, '™': 0.75, '°': 0.40, '×': 0.60,
    '€': 0.75, '£': 0.65, '　': 1.05,
  },
  hand: {
    ...
    // See the clean table.
    '—': 1.10, '–': 0.75, '―': 1.05, '⸺': 2.05, '⸻': 3.05,
    '…': 0.90, '•': 0.55, '→': 0.90, '™': 1.20, '°': 0.65, '×': 0.75,
    '€': 0.75, '£': 0.75, '　': 1.05,
  },
};
