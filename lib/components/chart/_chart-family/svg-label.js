...
function wrapSvgLabel(text, {
  ...
  //
  // It is emitted on the <text> AND on every <tspan>, and the duplication is
  // load-bearing rather than belt-and-braces. A `<tspan>` carries its own
  // `dominant-baseline: auto`, and both SVG 1.1 (10.9.2) and SVG 2 say that on a
  // tspan `auto` keeps the PARENT's dominant baseline. Chromium does that.
  // WebKit resolves it to `alphabetic` instead — conforming to neither spec — so
  // y becomes the glyph baseline and the label paints high: 0.35em for `central`,
  // 0.72em for `hanging`, measured at 20px in both engines. Across the chart
  // gallery that put 107 of 256 labels 4-13px high on a 1280x720 slide in WebKit
  // and 0 in Chromium — and every gate in this repo renders through headless
  // Chromium, so nothing here could see it (#2297,
  ...
  const tspans = m.lines
    .map((ln, i) => `<tspan x="${round(x)}" y="${round(firstBaseline + i * LH)}"${db}>${xmlEsc(ln)}</tspan>`)
    .join('');
...
/**
 * ── ORDER, in the same currency as the anchors ───────────────────────────
 *
 * WHAT A COLUMN OF NAMES CLAIMS. Two labels stacked in one column are read
 * top-to-bottom as an order, whether or not the pass meant one. So the rule is
 * not "rank the labels" — it is that the column must not CONTRADICT the marks
 * it names. Measured over 20,000 randomized layouts through this pass, 18.6% of
 * the label pairs sharing a column read against their own marks, and 6.3% did
 * so with marks a reader can plainly separate. That second number is not a
 * matter of taste: the reader can see one dot above the other and the names say
 * the opposite.
 *
 * WHY IT IS A COST AND NOT A COLLISION. An out-of-order position is still a
 * CLEAN position — nothing overlaps — so the penalty rides the anchor `cost`,
 * never the collision `score`. Two things follow, and both are the point. The
 * pass answers "that reads backwards" with a different ANCHOR, which is its own
 * native move (see the docblock on `placeLabels`); and when every clear
 * position reads backwards it takes one anyway, rather than dropping the label
 * or shoving it through a neighbor. Order is worth an anchor, never a name.
 *
 * WHY 10. The anchor scale runs 0 (`above`) to 15 (`left`), so 10 buys the
 * whole vertical family and both diagonals before it gives up on the order —
 * an inverted `above` (0 + 10) loses to a correct `below-left` (9) and to a
 * correct `above` one ring out (0 + 2). It stops short of buying a pure side
 * placement (14), which is the read this module deliberately avoids. Measured
 * on the same corpus, 10 takes the inversion rate from 18.61% to 6.04% and the
 * separable half from 6.33% to 2.50%. The neighbors bracket it: 1 is too weak
 * to change an anchor (16.78%), while 100 buys 0.31 points more and DROPS THREE
 * MORE NAMES for them (84 hidden against 81), which is the trade this module
 * refuses. What remains at 10 is the honest residue —
 * clusters where EVERY clear position reads backwards — so this pass makes a
 * column that does not contradict its marks, and never advertises one as a rank
 * readout.
 */
const ORDER_COST = 10;

// How much of the narrower box two labels must share horizontally before they
// count as ONE COLUMN. At half, a name set directly over another is a column
// and a name a full box-width to the side is not — which is the distinction a
// reader makes. Boxes that barely graze each other carry no order at all.
const COLUMN_SHARE = 0.5;

// Below this the label is already touching its mark, and a connector between
// two things that are visibly together is noise — a 2-unit stub reads as a
// speck of dirt, not as a leader. Measured against what it is FOR: on the two
// densest slides in the tree, every label whose nearest mark is not its own
// (3 of 12 on the scatter stress slide, 6 of 14 on the quadrant one) is still
// covered at this threshold, and raising it further covers no more.
const LEADER_MIN_GAP = 5;

/** Horizontal share of the narrower box — see COLUMN_SHARE. */
function columnShare(a, b) {
  const w = Math.min(a.right, b.right) - Math.max(a.left, b.left);
  // `w > 0` and not `!(w <= 0)`: a NaN box must fail CLOSED. Written the other
  // way, NaN slips past the guard and returns NaN, and the caller's
  // `NaN < COLUMN_SHARE` is false — so a pair with no computable overlap gets
  // billed as sharing a column. Not reachable from a finite mark today; free to
  // rule out, and the wrong side to be wrong on.
  if (!(w > 0)) return 0;
  return w / Math.min(a.right - a.left, b.right - b.left);
}

/**
 * A hairline from a mark to the label that names it.
 *
 * THE DEFECT THIS FIXES, seen on real renders of both callers. `placeLabels`
 * searches three rings out from a mark, and on a tight cluster the labels that
 * lose the first ring end up a line-height or two away — far enough that a
 * reader cannot tell which of several adjacent marks a name belongs to. It is
 * not a placement problem the pass can solve: there is genuinely nowhere
 * adjacent left to put them. So the label keeps the position the pass found and
 * a leader states the ownership.
 *
 * PROXIMITY IS THE THING BEING REPAIRED, and it is measurable. On the quadrant's
 * fourteen-initiative stress slide, SIX of the fourteen names sit nearer another
 * initiative's dot than their own — so proximity, the only channel that chart
 * had, gives the wrong answer six times. This draws ten lines there and covers
 * all six. On an ordinary six-item quadrant it draws none and the render is
 * unchanged, because no label had to travel.
 *
 * Drawn only when it is NEEDED — a label already touching its mark gets none.
 * The line stops a hair outside the mark and a hair short of the box, so it
 * touches neither and reads as a pointer rather than a tether.
 *
 * AND ONLY WHEN THE CORRIDOR IS CLEAR. `placeLabels` guarantees the label's BOX
 * clears everything; it says nothing about the space between the box and the
 * mark, and this pass paints exactly that space. On the quadrant's `bubble`
 * gallery slide the first cut drew "Weekly signal brief" straight down through
 * the word *calibration* in the name above it — 10.1 of the segment's 12.3
 * units inside another label's box, entering the ink mid-word. A reader tracing
 * that line lands on the wrong name, which is the misattribution this whole
 * mechanism exists to remove.
 *
 * So a caller hands over what the corridor must not cross, and the line is
 * aimed at the NEAREST POINT OF THE LABEL THAT CAN BE REACHED CLEANLY: the
 * point a reader's own eye would pick first, then the box's corners and edge
 * midpoints, in order of distance from the mark. A label reachable at none of
 * them gets no leader at all — the same call the module already makes about a
 * label it cannot place, because an honestly missing pointer beats a
 * confidently wrong one.
 *
 * The re-route is not decoration, and the first cut of this pass left it out on
 * the guess that a corner would read as pointing past the name. Measured on the
 * quadrant's fourteen-initiative stress slide, eight names want a leader: seven
 * are reachable head-on and ONE is only reachable at a corner, so dropping the
 * re-route is the difference between every name being attributable and one
 * orphan. On a genuinely packed corner it stops helping — six names inside four
 * points leave three whose every corridor is blocked, and drawing nothing there
 * is the honest answer.
 *
 * @param {{cx:number, cy:number, r:number}} mark the mark being named
 * @param {{svg:string, box:object}} placed one `placeLabels` result; a hidden
 *        label (empty `svg`) gets no leader, because there is nothing to lead to
 * @param {object} [o]
 * @param {string} [o.className] the painted class — one shared rule, see
 *        chart-family.css § Label leaders
 * @param {number} [o.minGap] the clearance below which no line is drawn
 * @param {Array} [o.avoid] boxes the segment must not enter — the other labels
 *        and the other marks. The label's OWN box is not one of them: the line
 *        ends there by construction.
 * @returns {string} one `<line>`, or '' when no leader is warranted
 */
function leaderLine(mark, placed, { className = 'chart-leader', minGap = LEADER_MIN_GAP, avoid = [] } = {}) {
  if (!mark || !placed?.svg || !placed.box) return '';
  const { cx, cy, r } = mark;
  const b = placed.box;
  const targets = leaderTargets(cx, cy, b);
  // WHETHER a leader is warranted is decided on the NEAREST point and nowhere
  // else: that is what "the label is already touching its mark" means. Asking
  // each candidate in turn would draw a line to a far corner of a box the mark
  // is sitting inside — a leader for a label that needs none.
  const [n0x, n0y] = targets[0];
  if (!(Math.hypot(n0x - cx, n0y - cy) > r + minGap)) return '';
  for (const [px, py] of targets) {
    ...
    const x2 = px - ux * 1.2;
    const y2 = py - uy * 1.2;
    if (avoid.some((box) => box && segmentEntersBox(x1, y1, x2, y2, box))) continue;
    return `<line class="${className}" x1="${round(x1)}" y1="${round(y1)}" `
...
/**
 * Where a leader may land on its label's box, nearest first.
 *
 * The first entry is the point a reader's own eye would pick — the closest
 * point on the box to the mark — and it is what gets drawn whenever its
 * corridor is clear, which is the overwhelming majority. The rest are the four
 * corners and the four edge midpoints, sorted by distance so a blocked head-on
 * line falls back to the closest alternative rather than the far side of the
 * name.
 */
function leaderTargets(cx, cy, b) {
  const midX = (b.left + b.right) / 2;
  const midY = (b.top + b.bottom) / 2;
  const nearest = [Math.min(Math.max(cx, b.left), b.right), Math.min(Math.max(cy, b.top), b.bottom)];
  const rest = [
    [b.left, b.top], [b.right, b.top], [b.left, b.bottom], [b.right, b.bottom],
    [midX, b.top], [midX, b.bottom], [b.left, midY], [b.right, midY],
  ].sort((p, q) => Math.hypot(p[0] - cx, p[1] - cy) - Math.hypot(q[0] - cx, q[1] - cy));
  return [nearest, ...rest];
}

/**
 * Does the segment pass through the INTERIOR of this box?
 *
 * Liang-Barsky, which is the cheap standard answer and the one that gets the
 * degenerate cases right: a segment that merely grazes an edge, or ends exactly
 * on one, is not "through" it, so a leader is not refused for touching the
 * neighbor it is trying to avoid. A zero-length segment is inside nothing.
 */
function segmentEntersBox(x1, y1, x2, y2, box) {
  ...
  if (!clip(-dx, x1 - box.left) || !clip(dx, box.right - x1)
    || !clip(-dy, y1 - box.top) || !clip(dy, box.bottom - y1)) return false;
  // A positive-length overlap is a crossing; a single touching point is not.
  return t1 > t0;
}
 ...
 * from its dot — up to `maxShift`, a fifth of the plot — and read as labeling
 ...
 */
/**
 * What this candidate position would cost in ORDER — see ORDER_COST.
 *
 * One `ORDER_COST` per already-placed label that shares this candidate's column
 * and would end up on the wrong side of it. Counted rather than flagged, so a
 * position that reads backwards against three neighbors loses to one that reads
 * backwards against one.
 *
 * TIES ARE FREE, on BOTH sides, and each side is a separate escape. Two labels
 * whose box centers land on the same line make no claim about which mark is
 * higher — neither reading is contradicted. And two MARKS at the same height
 * make no claim either: there is no order to contradict, so every position for
 * the second label would otherwise be billed the same penalty and the pass
 * would leave the column for no reason at all.
 */
function orderPenalty(cand, item, placed, placedMarks) {
  let penalty = 0;
  const mid = (cand.top + cand.bottom) / 2;
  for (let i = 0; i < placed.length; i++) {
    if (columnShare(cand, placed[i]) < COLUMN_SHARE) continue;
    const markSign = Math.sign(item.cy - placedMarks[i].cy);
    if (markSign === 0) continue;
    const qMid = (placed[i].top + placed[i].bottom) / 2;
    const labelSign = Math.sign(mid - qMid);
    if (labelSign !== 0 && labelSign !== markSign) penalty += ORDER_COST;
  }
  return penalty;
}
...
function placeLabels(items, { obstacles = [], bounds = null, gap = 4, minGap = 1.5, nudge = 24 } = {}) {
  ...
  const placed = [];
  // The mark each placed label names, in step with `placed`. The order term
  // needs the MARK's position, not the label's, and `placed` carries boxes.
  const placedMarks = [];
  const out = [];
  ...
  for (const it of items) {
    ...
    for (let ring = 0; ring < 3; ring++) {
      ...
      for (const a of LABEL_ANCHORS) {
        ...
        // ORDER rides the anchor cost, never the collision score — see
        // ORDER_COST. A position that reads backwards against a label already
        // in this column is still CLEAN; it has just become expensive.
        const ordered = cost + orderPenalty(cand, it, placed, placedMarks);
        // The un-penalized `cost` is a lower bound on `ordered`, so the two
        // prunes above stay valid: nothing further out can beat what we hold.
        if (best && ordered >= best.cost) continue;
        const score = onMark * 1000 + onLabel + escapesBounds(cand, bounds) * 5000;
        if (score === 0) { best = { cand, a, cost: ordered, ring, reach }; continue; }
        // Ring and preference only break ties between equally-bad spots; they
        // must never outweigh an actual collision.
        const ranked = score + ordered * 0.01;
    ...
    if (!chosen) {
      ...
      if (residual > 0) {
        // `label` rides along so the caller can REPORT what was dropped (#2171).
        // Carried rather than noted here: `quadrant` runs this whole pass once per
        // rung of its nine-rung ladder and keeps one, so a note taken inside the
        // search would report every rejected rung's casualties as if they shipped.
        // The caller notes the array it actually emits — see label-drops.js.
        out.push({ svg: '', box: chosen.cand, anchorKey: chosen.a.key, ring: chosen.ring, reach: chosen.reach, fallback: fellBack, hidden: true, label: it.text });
        continue;
      }
    }
    placed.push(chosen.cand);
    placedMarks.push(it);
    out.push({ svg: chosen.cand.svg, box: chosen.cand, anchorKey: chosen.a.key, ring: chosen.ring, reach: chosen.reach, fallback: fellBack, label: it.text });
  }
  return out;
}

module.exports = {
  wrapSvgLabel, measureLabel, charBudget, deCollideLabels, placeLabels, leaderLine,
  segmentEntersBox,
  ...
  ORDER_COST, COLUMN_SHARE, LEADER_MIN_GAP, columnShare,
};
