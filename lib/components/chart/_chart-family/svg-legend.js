...
function emitRows(measured, rowHeights, { startY, swatchX, labelX, valueX, FS, LH, SW, ROW_GAP }) {
  ...
  measured.forEach((m, i) => {
    ...
    // KEYED swatches (categorical `cat`, choropleth `ramp`, or `muted`) paint via a
    // CSS rule keyed on the data attribute — NEVER an inline `fill=`/`stroke=`
    // presentation attribute, because a var()/color-mix() in a presentation attr is
    // the flaky old-WebKit feature that renders the swatch black (D2). stroke-WIDTH is
    // a plain number, safe inline. An UNKEYED row (a transparent "unmatched" chip)
    // keeps its literal inline paint.
    ...
    else paint = ` fill="${m.swatchFill}"${m.swatchStroke ? ` stroke="${m.swatchStroke}"${strokeW}` : ''}`;
    out.push(
...
function buildSvgLegend({ rows, diagramRight, diagramHeight, hasValues = true, orientation, diagramPadX = 0 }) {
  ...
  const measured = rows.map((row) => {
    ...
    return {
      ...
      // Optional swatch paint key — emitted as data-cat (0-based categorical slot;
      // also the a11y texture hook), data-ramp (choropleth bucket), or data-muted, so
      // the swatch paints via a CSS rule, never an inline color-mix presentation attr
      // (D2). Absent for an un-keyed legend (a transparent "unmatched" chip).
      cat: row.cat,
      ramp: row.ramp,
      muted: row.muted,
