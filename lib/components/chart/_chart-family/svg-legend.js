...
/**
 * Build a keyed chart's legend for a PANE: the key beside or below the diagram — whichever
 * leaves the diagram larger — with its type scaled so that, once the whole unit is fitted into
 * the pane, labels print at slide size and only the diagram shrinks. `build(orientation,
 * fontScale)` is the kernel's own `buildSvgLegend` call. Without a `paneView` (every ordinary
 * slide) it is exactly that call at `fontScale` 1, in the deck's own orientation. A kernel
 * whose DIAGRAM depends on the orientation (radar pads its axis labels by it) passes
 * `orientations: [orientation]` and gets the type scaled only.
 *
...
function fitKeyToPane(build, { orientation, paneView, diagramHeight, orientations = [undefined, 'portrait'] } = {}) {
  ...
  for (const o of orientations) {
    ...
    // passes converge (each moves m by the ratio still missing).
    for (let pass = 0; pass < 4; pass++) {
      const s = Math.min(paneView.w / key.viewW, paneView.h / key.viewH);
      const next = Math.min(3, Math.max(1, PANE_KEY_TEXT / (FS_OF_HEIGHT * diagramHeight * s)));
      if (Math.abs(next - m) < 0.02) break;
      m = next;
      key = build(o, m);
    }
    const s = Math.min(paneView.w / key.viewW, paneView.h / key.viewH);
    if (!best || diagramHeight * s > best.px) best = { key, orientation: o, fontScale: m, px: diagramHeight * s };
  }
