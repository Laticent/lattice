...
function emitRows(measured, rowHeights, { startY, swatchX, labelX, valueX, FS, LH, SW, ROW_GAP }) {
  ...
  measured.forEach((m, i) => {
    ...
    // EVERY swatch paints via a CSS rule keyed on a data attribute — categorical `data-cat`,
    // choropleth `data-ramp`, `data-muted`, or the transparent-"?"-chip `data-unmatched` —
    // NEVER an inline `fill=`/`stroke=` presentation attribute, because a var()/color-mix()
    // in a presentation attr is the flaky old-WebKit feature that renders the swatch black
    // (D2). stroke-WIDTH is a plain number, safe inline.
    const strokeW = ` stroke-width="${(SW * STROKE_W_R).toFixed(2)}"`;
    let paint;
    if (m.cat != null) paint = ` data-cat="${m.cat}"${strokeW}`;
    else if (m.ramp != null) paint = ` data-ramp="${m.ramp}"${strokeW}`;
    else if (m.muted) paint = ` data-muted=""${strokeW}`;
    else paint = ` data-unmatched=""${strokeW}`;
    out.push(
      `<rect class="chart-key-swatch"${paint} x="${swatchX.toFixed(2)}" y="${swatchY.toFixed(2)}" ` +
...
function buildSvgLegend({ rows, diagramRight, diagramHeight, hasValues = true, orientation, diagramPadX = 0 }) {
  ...
  const measured = rows.map((row) => {
    ...
    return {
      // Swatch paint key — emitted as data-cat (0-based categorical slot; also the a11y
      // texture hook), data-ramp (choropleth bucket), data-muted, or data-unmatched (the
      // transparent "?" chip), so the swatch paints via a CSS rule, NEVER an inline
      // fill=/stroke= presentation attr — a var()/color-mix() there is the flaky old-WebKit
      // feature that renders the swatch black (D2). Every legend row is one of these keys.
      cat: row.cat,
      ramp: row.ramp,
      muted: row.muted,
  ...
  const defs =
    `<linearGradient id="${spineId}" x1="0" y1="0" x2="0" y2="1">` +
      ...
      `<stop offset="14%" style="stop-color:var(--chart-spine-soft)"/>` +
      `<stop offset="50%" style="stop-color:var(--chart-spine-mid)"/>` +
      `<stop offset="86%" style="stop-color:var(--chart-spine-soft)"/>` +
