...
const { stripTags, plainText } = require('../../../core/plain-text');
...
// `stripTags` / `plainText` moved to `lib/core/plain-text.js` and are re-exported
// here so every existing call site keeps its import. The move was forced by
// `lib/core/label-set.js`, which lifts a label set out of a markdown-it paragraph
// and so needs the same extractor — and a core primitive must never import a
// component kernel (the same rule the `html-lists.js` note above states).
// Behavior is unchanged: this is still the &nbsp;-folding quadrant/radar flavor.
...
// The family's STATUS vocabulary and its one case rule. They live in lib/core
// (chart-status.js) so narration reads the SAME ten words the paint stamps;
// re-exported here unchanged, so every kernel's import still resolves.
const { CHART_STATUS, chartStatus } = require('../../../core/chart-status');
...
module.exports = {
  ...
  stripTrailingPills, spliceFirstList, readsHandBody, CHART_STATUS, chartStatus,
};
