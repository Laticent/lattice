 ...
 * used chart in a board deck and why the family's nearest neighbor is not a
...
// its neighbor's. Both ends are relative to the plot, not absolute, because
...
// The substrate's default. It was briefly 11 here, to clear an ascender that
// `buildCategoryLabels` did not actually leave room for — it passed
// `vAlign: 'top'`, which `wrapSvgLabel` does not know, so the value fell
// through to `baseline` and the glyphs straddled the line the bars stand on.
// That was fixed in the substrate (`vAlign: 'hanging'`) in the same commit that
// added this component, and the compensation then DOUBLE-counted: the second
// line of a two-line category label landed at y=180.2 in a 0 0 320 180 viewBox
// and the SVG clipped it. Keep this at the substrate default; `GUT_COL.bottom`
// is derived from it.
const CAT_GAP_X = 5;
...
function wouldShortenALabel(model, shape, ctx) {
  const view = cart.viewFor(ctx.orientation, ctx.paneView);
  ...
  const width = catLabelWidth(band.step);
...
function buildBar(model, ctx = {}) {
  ...
  const view = cart.viewFor(orientation, ctx.paneView);
  return shape.row
...
/**
 * The width one category label may occupy, from the band step.
 *
 * The substrate states the rule: passing the band width EXACTLY means two
 * labels that each paint their full budget overlap by `MIN_LABEL_GAP` and the
 * widest ones cull. Measured at twenty categories, `step * 0.98` painted
 * fifteen of twenty names; this formula painted all twenty.
 */
function catLabelWidth(step) {
  return step * Math.min(0.99, 1 - 2 / step);
}
...
function marksOf(model) {
  ...
  model.groups.forEach((g, gi) => {
    // A group with NO nested points draws its own lead value — whether or not
    // the model as a whole is flat. Branching on the model-wide `flat` flag
    // meant that in a MIXED list (an author converts one group to multi-series
    // and leaves another alone, which `parseSeries` calls the commonest
    // authoring edit there is) the flat group emitted no bar at all, while its
    // value still set the domain. The chart squashed into its bottom eighth for
    // a number that appeared nowhere on it.
    if (!g.points.length) {
      if (Number.isFinite(g.num)) out.push({ gi, si: 0, label: g.label, raw: g.raw, num: g.num });
      return;
    }
...
/** Whether category i drew any bar: the `markFor` predicate for its linked labels. */
function drewBar(model) {
  const drawn = new Set(marksOf(model).map((m) => m.gi));
  return (i) => drawn.has(i);
}
...
function buildColumns(model, shape, { view, axis, orientation }) {
  ...
  chrome.push(cart.buildCategoryLabels({
    ...
    width: catLabelWidth(band.step), maxLines: 2, axis: 'x', gap: CAT_GAP_X,
    // Mark i IS category i here (every series' rect in band i carries `data-mark="i"`), so the
    // name recedes with its bar under the chart hover and the Present Guide — when band i drew a
    // bar at all (a non-numeric value draws none).
    markFor: drewBar(model),
  }));
  ...
  for (const m of marksOf(model)) {
    ...
    if (!axis && m.raw) {
      ...
      values.push(cart.buildValueLabel(m.raw, {
        ...
        // The BAND, not the bar. `wantsValueAxis` decides these labels fit by
      // measuring against the band step; painting them at the bar width meant a
      // figure that passed the fit test was then ellipsized — `$1,234,567,890`
      // printed as `$1,234,…`, a well-formed and wrong number beside a correct
      // bar. Measure and paint against the same box.
      width: Math.max(band.step, 30),
      // The value recedes with its bar (see `markFor` above).
      extra: ` data-mark-for="${m.gi}"`,
      }));
    }
  }
...
function buildRows(model, shape, { view, axis, orientation }) {
  ...
  // No upper clamp. It was 46, which ellipsized `$1,234,567,890` into
  // `$1,234,…` — a different, readable, wrong number. A value label is never
  // truncated; the gutter grows to hold the widest figure and the plot gives up
  // the width, which is visible and self-correcting.
  const right = Math.max(24, widestValue + 6);
  ...
  // `pitch` is REQUIRED on the vertical axis. Without it the substrate culls a
  // label that collides with its neighbor, and a dropped category name is
  // invisible data loss — measured, ten business-unit names rendered six. That
  // is the exact failure the auto-`row` flip exists to prevent, so losing names
  // in the destination made the flip self-defeating. With the pitch the budget
  // comes from the row height and a long name ellipsizes instead.
  chrome.push(cart.buildCategoryLabels({
    plot, labels: model.groups.map((g) => g.label), center: (i) => band.center(i),
    width: plot.gutter.left - 5, maxLines: 2, axis: 'y', pitch: band.step, markFor: drewBar(model),
  }));
...
function assemble({ model, shape, view, plot, chrome, bars, values, orientation }) {
  ...
  for (let si = 0; si < nS; si++) {
    const rects = bars.filter((b) => b.m.si === si).map((b) => {
      ...
      // data-hue is the 1-BASED categorical slot, the family-wide hook the a11y
      // and print stylesheets texture on (the shared legend already keys its
      // swatches `[data-hue="1"]`, so a bar and its key entry texture alike).
      ...
      return `<rect class="${cls}" data-mark="${b.m.gi}" data-anima-role="bar"` +
        ` data-hue="${slot}" data-encodes="hue" data-paint="fill"${sign}` +
  ...
  // sign-colored diverging rules and the per-series lane rotation never meet
  ...
  if (shape.grouped) {
    ...
    const legend = buildSvgLegend({
      rows: model.series.map((name, si) => ({
        // `slotOf` answers the SIGN first when diverging, so asking it for a
        // series slot with no value returned slot 1 for every row — two
        // identical swatches under two different series names, and the
        // bar-to-key texture pairing broken with them. A diverging chart's key
        // is its two directions, not its series; the legend is suppressed there
        // (see `wantsLegend`), and this asks for the series slot directly.
        swatchFill: fill.url((si % 6) + 1),
        // `cat` makes `svg-legend` stamp `data-hue` on the swatch, which is what
        // the a11y and print texture rules key on. Without it the bars carried
        // six distinct textures and the key carried six flat grays, so on the
        // one palette whose whole job is the non-color channel the key could
        // not be matched to the chart.
        cat: si % 6,
        label: name,
      })),
  ...
  const svg = `<svg class="cart-svg bar-svg" viewBox="0 0 ${viewW} ${viewH}"` +
    ...
    `<title>${title}</title>${desc}<defs>${defs}</defs>${cart.ariaHiddenMarks(body)}</svg>`;
 ...
 *
 * The marks ride in an `aria-hidden` group (cartesian.js § ariaHiddenMarks), so
...
function buildDesc(model) {
  const parts = [];
  // Only the points the chart actually DREW. `parseSeries` caps the categorical
  // palette at six and tags the rest `over`; reading `g.points` raw announced a
  // seventh series that is painted nowhere, and then computed a "highest" from
  // the drawn six that the listed seven contradicted. The overflow is named at
  // the end instead, because dropping it silently is the thing the cap is not
  // allowed to do.
  const drawn = (g) => g.points.filter((p) => !p.over);
  if (model.flat) {
    ...
    parts.push(model.groups.map((g) => {
      const pts = drawn(g).map((p) => `${p.series} ${p.raw}`).join(', ');
      return pts ? `${g.label} — ${pts}` : g.label;
    }).join('; '));
  }
  if (model.seriesOverflow?.length) {
    parts.push(`Not shown, past the six-series limit: ${model.seriesOverflow.join(', ')}`);
  }
...
module.exports = {
  transformSection, parseBar, buildBar, buildDesc,
  resolveShape, wantsValueAxis, padFor, domainFor, bestTicks, marksOf, rowGutterFor, slotOf, valueRoom,
  maxBarFor, maxRowBarFor, estWidth, catLabelWidth,
  GUT_COL, GUT_COL_NOAXIS, GUT_ROW, CAT_GAP_X,
};
