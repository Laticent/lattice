...
// The row FACTS — zone cuts, attainment from the floor, the at-or-above verdict.
// They used to live in this file and be computed for the `<desc>` alone; the
// caption voice needs the same relationship and could not reach it from Markdown.
// One derivation, two surfaces (HARD RULE #1) — `lib/core/bullet-facts.js`.
const bulletFacts = require('../../../core/bullet-facts');
const { BAND_FRACTIONS, MAX_ZONES } = bulletFacts;
...
// BAND_FRACTIONS (the derived 60/85 cut points) and MAX_ZONES (four zones, past
// which a qualitative range is a gradient) are imported above from
// `lib/core/bullet-facts.js` and re-exported below, so the two surfaces that
// describe this chart cannot disagree about where its zones are.
...
// `cutsFor` and `round6` moved to `lib/core/bullet-facts.js` with their comments
// intact — the two notes in `zoneCuts` each record a real defect (a row losing
// its range when every authored band sat under the floor; an SLO scale silently
// discarding the two thresholds the slide was about), and a paraphrased defect
// note is worth nothing. The caption voice needs the same cuts to say which zone
// a measure landed in, so they are shared rather than copied.
const cutsFor = bulletFacts.zoneCuts;
...
function buildBullet(model, ctx = {}) {
  ...
  const view = viewFor(ctx.orientation, ctx.paneView);
  ...
  rows.forEach((r, i) => {
    ...
    // THE GRID SPANS THE TRACK, NOT THE PLOT. A bullet's rows carry their name
    // and their `4.2M vs 5M` readout INSIDE the plot box, on a baseline above
    // each track — so a full-height vertical gridline ran through the text, and
    // the last one landed a pixel off the final glyph of every readout on the
    // chart. Drawn per row, behind the bands it belongs to.
    if (axis) {
      parts.push(buildGrid({
        plot, ticks: axis.ticks, scale: axisScale, axis: 'x',
        from: y, to: y + trackH,
      }));
    }
    ...
    if (Number.isFinite(r.measure)) {
      ...
      // `data-hue="1" data-encodes="hue" data-paint="fill"` is the family's categorical-slot attribute (the shared
      ...
      parts.push(`<rect class="bullet-measure" data-mark="${i}" data-hue="1" data-encodes="hue" data-paint="fill" data-anima-role="bar"` +
        ' style="--i:0"' +
    ...
    // The name and readout recede with row i's measure (`data-mark="i"`) under the chart hover
    // and the Present Guide — when the row drew a measure. A name-only row draws none, and a
    // label linked to a missing mark opens an empty card.
    const link = Number.isFinite(r.measure) ? ` data-mark-for="${i}"` : '';
    ...
    if (r.measureRaw) {
      const valueBox = wrapSvgLabel(fmt(r.measure), {
        ...
        className: 'cart-value bullet-value', attrs: ` data-anima-role="label"${link}`,
        emitFontSize: false,
      });
    ...
    if (r.label) {
      parts.push(wrapSvgLabel(r.label, {
        ...
        className: 'cart-cat bullet-name',
        attrs: ` data-anima-role="label"${link}`,
        emitFontSize: false,
      }).svg);
    }
  });
  ...
  // The marks ride in an `aria-hidden` group (cartesian.js § ariaHiddenMarks), so this <desc> is the ONLY route a
  ...
  const descText = rows.map((r) => {
    ...
    const at = bulletFacts.attainment(r);
    if (!at) return head;
