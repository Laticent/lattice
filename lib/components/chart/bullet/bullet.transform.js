  ...
  rows.forEach((r, i) => {
    ...
    if (r.measureRaw) {
      const valueBox = wrapSvgLabel(fmt(r.measure), {
        ...
        className: 'cart-value bullet-value', attrs: ` data-anima-role="label" data-mark-for="${i}"`,
        emitFontSize: false,
      });
    ...
    if (r.label) {
      parts.push(wrapSvgLabel(r.label, {
        ...
        // The name and readout recede with row i's measure (`data-mark="i"`) under the chart
        // hover and the Present Guide.
        attrs: ` data-anima-role="label" data-mark-for="${i}"`,
        emitFontSize: false,
      }).svg);
    }
  });
