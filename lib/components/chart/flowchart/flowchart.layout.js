...
function installFlowchartLayout(rootDoc, kernelFactory, opts) {
  ...
  // One kernel per document, kept across installs: the Studio calls this on every edit,
  ...
  if (!doc.__fcKernel || doc.__fcKernelFactory !== kernelFactory) {
    doc.__fcKernel = kernelFactory();
    doc.__fcKernelFactory = kernelFactory;
  }
  const K = doc.__fcKernel;
  const HD = 1280;
  ...
  // out in a worker, so a keystroke never waits on dagre and the router: measured on the
  // typing deck's 17-shape chart, a synchronous draw held every key 70-250 ms. The figure
  // keeps its last drawing meanwhile, and only the newest request for a chart is ever
  // painted (a worker cannot be interrupted, so at most one more waits behind the one in
  // flight). The first draw of a chart stays synchronous, and no export sets the flag, so
  // nothing that captures a page can capture a drawing still in flight. Without a Worker,
  // or without dagre's script URL to load into it, every draw stays synchronous.
  const live = Boolean(opts?.live) || Boolean(doc.documentElement?.hasAttribute?.('data-lattice-live-layout'));
  function liveWorker() {
    if (doc.__fcWorker !== undefined) return doc.__fcWorker;
    doc.__fcWorker = null;
    try {
      const w = doc.defaultView;
      if (!w || typeof w.Worker !== 'function' || typeof w.Blob !== 'function' || !w.URL?.createObjectURL) return null;
      let dagreSrc = '';
      for (const el of doc.querySelectorAll('script[src]')) if (/lattice-dagre(\.min)?\.js(\?|#|$)/.test(el.src)) { dagreSrc = el.src; break; }
      ...
      const W = { worker, id: 0, jobs: new Map(), busy: new Set(), next: new Map() };
      ...
      const fail = () => {
        if (doc.__fcWorker !== W) return;
        try { worker.terminate(); } catch (_e) { /* gone */ }
        doc.__fcWorker = null;
        for (const f of doc.querySelectorAll('.flowchart-figure')) { f.__fcPendingSig = null; f.removeAttribute('data-fc-pending'); f.__fcSig = null; }
        drawAll();
      };
      const DEADLINE = 3000;
      const send = (job) => {
      ...
      W.post = (key, args, cb) => {
        const job = { key, args, cb };
        if (W.busy.has(key)) W.next.set(key, job);
        else send(job);
      };
      worker.onmessage = (e) => {
      ...
      worker.onerror = fail;
      doc.__fcWorker = W;
    } catch (_e) { doc.__fcWorker = null; }
    return doc.__fcWorker;
  }
  ...
  // remembers across the Studio replacing the element on each edit.
  function chartKey(fig, sec) {
    const secs = doc.querySelectorAll('section');
    let si = -1;
    for (let i = 0; i < secs.length; i++) if (secs[i] === sec) { si = i; break; }
    const figs = sec ? sec.querySelectorAll('.flowchart-figure') : [];
    let fi = 0;
  ...
  function draw(fig) {
    ...
    const sigNow = () => [fig.getAttribute('data-fc-model'), fig.getAttribute('data-fc-dir'), fig.getAttribute('data-fc-spacing'), sec ? sec.offsetWidth : 0, port0.clientWidth, port0.clientHeight, doc.fonts ? doc.fonts.status : '',
    ...
    if (fig.__fcPendingSig === sig) return;
    // ...or it answered, with no layout for exactly these inputs.
    if (fig.__fcNoLayoutSig === sig) return;
    readVis(sec);
    ...
    // since the Studio replaces the element on every edit), measure and lay out, and go
    // again only while the floor that scale implies moves by more than 1% (3 rounds at most).
    const floorFor = (k) => {
      if (k < 1) box.style.setProperty('--chart-text-min', `${(readTextMin(fig, 11) / k).toFixed(3)}px`);
      else box.style.removeProperty('--chart-text-min');
    };
    const lift = (k) => (k < 1 ? 1 / k : 1);
    const fitKey = chartKey(fig, sec);
    if (!doc.__fcFit) doc.__fcFit = new Map();
    let kGuess = doc.__fcFit.get(fitKey) ?? 1;
    const measure = () => {
      const sizes = {};
      const lines = {};
      const fontPx = {};
      ...
      return { sizes, lines, fontPx, labelSizes, labelFont, groupTitleSizes, titleFont, shapes, edges, noteLines, noteFont, port, args, geo: null };
    };
    // Another round only while the floor the new fit implies moves by more than 1%.
    const settled = (geo) => {
      const kNow = fitOf(fig.querySelector('.fc-canvas') || fig, geo.width * S, geo.height * S);
      if (kNow == null || Math.abs(lift(kNow) - lift(kGuess)) / lift(kGuess) < 0.01) return true;
      kGuess = kNow;
      return false;
    };
    if (!doc.__fcPrev) doc.__fcPrev = new Map();
    const prev = doc.__fcPrev.get(fitKey);
    // Live: an EDIT of the chart drawn here before. The Studio patches one section in place
    // as the author moves between slides, so position alone would hand a new slide's chart
    // the last slide's drawing to show meanwhile: all but two of the shapes must be the same
    // ones (a keystroke renames one). A figure outside a section has no position to key on.
    ...
    if (W) {
      if (!doc.__fcLatest) doc.__fcLatest = new Map();
      const token = (doc.__fcTokens = (doc.__fcTokens || 0) + 1);
      doc.__fcLatest.set(fitKey, token);
      const round = (r) => {
        readVis(sec);
        unlay();
        floorFor(kGuess);
        ...
        fig.__fcPendingSig = sigNow();
        W.post(fitKey, m.args, (geo) => {
          if (doc.__fcLatest.get(fitKey) !== token || !fig.isConnected) return;
          ...
          if (!geo) { unlay(); fig.removeAttribute('data-fc-pending'); fig.__fcPendingSig = null; fig.__fcNoLayoutSig = sigNow(); return; }
          m.geo = geo;
    ...
    function finish(m) {
      const { sizes, lines, fontPx, labelFont, titleFont, edges, noteLines, noteFont, port, geo } = m;
      // ── Paint ────────────────────────────────────────────────────────────────
      const parts = [];
      ...
      const kFit = applyFit(port, box, geo.width * S, geo.height * S);
      if (kFit != null) doc.__fcFit.set(fitKey, kFit);
      ...
      fig.__fcSig = sigNow();
      fig.__fcPendingSig = null;
      fig.removeAttribute('data-fc-pending');
      doc.__fcPrev.set(fitKey, { ids: model.shapes.map((x) => x.id), drawn, vb, w: box.style.width, h: box.style.height, transform: box.style.transform, k: box.getAttribute('data-fit-k'), floor: box.style.getPropertyValue('--chart-text-min'), dir: geo.dir });
    }
    ...
    // fitted as it was, instead of its measuring tiles. `data-fc-pending` says so.
    function showPrev(p) {
      ...
      fig.setAttribute('data-fc-drawn', '1');
      fig.setAttribute('data-fc-pending', '1');
      if (fig.getAttribute('data-fc-laid') !== p.dir) fig.setAttribute('data-fc-laid', p.dir);
    }
  }
