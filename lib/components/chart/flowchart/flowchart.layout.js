 ...
 *   - the text lines are read with Range rects, which is not a DOM write, so
 *     measuring never re-arms that observer;
...
function installFlowchartLayout(rootDoc, kernelFactory, opts) {
  ...
  function draw(fig) {
    ...
    let model;
    try { model = JSON.parse(fig.getAttribute('data-fc-model') || ''); } catch (_e) { return; }
    if (!model || !Array.isArray(model.shapes) || !model.shapes.length) return;
    ...
    for (const g of groups) {
      ...
      if (!b) continue;
      parts.push(`<rect class="fc-group" data-group="${esc(g.id)}"${g.slot ? ` data-slot="${g.slot}"` : ''} data-depth="${depth(g.id)}" x="${r1(b.x)}" y="${r1(b.y)}" width="${r1(b.w)}" height="${r1(b.h)}" rx="10"/>`);
    }
    ...
    for (const r of geo.routes) {
      ...
      const attrs = `${e.heavy ? ' data-heavy="1"' : ''}${st.pattern ? ` data-pattern="${st.pattern}"` : ''}${st.slot ? ` data-slot="${st.slot}"` : ''}${e.back ? ' data-back="1"' : ''}${e.tether ? ' data-tether="1"' : ''}`;
    ...
    for (const g of groups) {
      ...
      if (!t) continue;
      parts.push(`<text class="fc-group-title" data-group="${esc(g.id)}"${g.slot ? ` data-slot="${g.slot}"` : ''} x="${r1(t.x)}" y="${r1(t.y + t.h / 2)}" font-size="${r1(titleFont)}" dominant-baseline="central">${esc(g.name)}</text>`);
    }
    model.shapes.forEach((s, i) => {
      const b = geo.nodes[s.id];
      if (!b) return;
      const kind = s.shape || 'box';
      const a = `${s.status ? ` data-s="${esc(s.status)}"` : ''}${s.slot ? ` data-slot="${s.slot}"` : ''}${s.fill ? ` data-fill="${s.fill}"` : ''}${s.border ? ` data-border="${s.border}"` : ''}${s.text ? ` data-text="${s.text}"` : ''}`;
      ...
      parts.push(`<g class="fc-node-group" data-id="${esc(s.id)}" data-shape="${kind}"${a}>` +
        outline(kind, b.x, b.y, b.w, b.h, ` class="fc-shape" data-mark="${i}" data-label="${esc(s.name)}" data-shape="${kind}"${a}`) +
