...
function installFlowchartLayout(rootDoc, kernelFactory, opts) {
  ...
  function liveWorker() {
    ...
    try {
      ...
      // load) falls back to drawing in place, for good.
      worker.onerror = () => {
  ...
  function draw(fig) {
    ...
    // Live: a chart drawn here before, of about the same size (a slide inserted above would
    // hand this position another chart's drawing to show meanwhile).
    const W = live && prev && Math.abs(prev.n - model.shapes.length) <= 3 ? liveWorker() : null;
    if (W) {
      ...
      const round = (r) => {
        ...
        W.post(fitKey, m.args, (geo) => {
          ...
          if (!geo) { fig.__fcPendingSig = null; return; }
          m.geo = geo;
    ...
    function finish(m) {
      ...
      doc.__fcPrev.set(fitKey, { n: model.shapes.length, drawn, vb, w: box.style.width, h: box.style.height, transform: box.style.transform, k: box.getAttribute('data-fit-k'), floor: box.style.getPropertyValue('--chart-text-min'), dir: geo.dir });
    }
