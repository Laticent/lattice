...
function installFlowchartLayout(rootDoc, kernelFactory, opts) {
  ...
  function draw(fig) {
    ...
    if (fig.__fcSig === sigNow() && fig.getAttribute('data-fc-drawn')) return;
    readVis(sec);
    ...
    const measureAndLayout = () => {
      ...
      return { sizes, lines, fontPx, labelSizes, labelFont, groupTitleSizes, titleFont, shapes, edges, noteLines, noteFont, port, geo };
    };
    let m = null;
    for (let round = 0; round < 3; round++) {
      m = measureAndLayout();
      if (!m.geo) return;
      const kNow = fitOf(m.port, m.geo.width * S, m.geo.height * S);
      if (kNow == null || Math.abs(lift(kNow) - lift(kGuess)) / lift(kGuess) < 0.01) break;
      kGuess = kNow;
      floorFor(kGuess);
    }
