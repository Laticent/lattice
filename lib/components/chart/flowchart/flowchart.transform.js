...
function keySwatch(entry, model) {
  const k = entry.key;
  if (k === '=>' || k === '->') return `<span class="fc-key-swatch" data-kind="line"${k === '=>' ? ' data-heavy="1"' : ''} aria-hidden="true"></span>`;
  if (k === ':dashed' || k === ':dotted') return `<span class="fc-key-swatch" data-kind="line" data-pattern="${k.slice(1)}" aria-hidden="true"></span>`;
  if (k === ':open' || k === ':dot' || k === ':cross') return `<span class="fc-key-swatch" data-kind="head" data-head="${k.slice(1)}" aria-hidden="true"></span>`;
