  ...
  for (let i = 0; i < n; i++) {
    ...
    if (s.label) {
      parts.push(wrapSvgLabel(s.label, {
        ...
        baseline: 'central',
        // `data-mark-for`: a tap on the stage's name or value opens the stage's card. Read only by
        // the reveal layer — see slope.transform.js `proxyAttrs` for why it is not `data-mark`.
