...
function buildFunnel(model, orientation) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // `id` + `data-anima-role`: native ANIMATION metadata (2026-07-19 §0.75). Roles let Anima
    // choreograph the chart authoritatively (no class-guessing); ids make each mark addressable.
    // Ids are unique WITHIN this chart's svg (the only scope Anima's per-section hydration reads);
    // like `data-mark`/the gradient defs, they don't paint — the rendered PDF stays byte-identical.
    parts.push(`<polygon class="funnel-band" data-mark="${i}" id="funnel-band-${i}" data-anima-role="bar"` +
    ...
    if (s.label) parts.push(`<text class="funnel-label" id="funnel-label-${i}" data-anima-role="label" x="${labelX}" y="${yMid.toFixed(1)}">${esc(s.label)}</text>`);
    if (s.valueRaw) parts.push(`<text class="funnel-value" id="funnel-value-${i}" data-anima-role="label" x="${valueX}" y="${yMid.toFixed(1)}">${esc(s.valueRaw)}</text>`);
    ...
    if (i < n - 1) {
      ...
      if (prev > 0) {
        ...
        parts.push(`<text class="funnel-conv" id="funnel-conv-${i}" data-anima-role="label" x="${cx}" y="${(y1 + gap / 2).toFixed(1)}">${pct}%</text>`);
      }
    }
  }
