 ...
 * member. A vertical stack of centered trapezoids whose width is proportional
...
const { plainText, spliceFirstList } = require('../_chart-family/transform-utils');
...
const GEOM = {
  ...
  top: 16,
  bottom: 172,
  cx: 160, // funnel center = viewBox center, so the bands sit on the slide's
           // optical center; labels (left) and values (right) flank symmetrically
           // (was 188, which pushed the whole composition right-of-center).
...
// left-label / right-value columns and the band center line are unchanged — but
...
function buildFunnel(model, orientation) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // sit on the canvas (not on the colored band) so contrast is never at the
    ...
    if (s.label) {
      parts.push(wrapSvgLabel(s.label, {
        ...
        baseline: 'central',
        // `data-mark-for`: a tap on the stage's name or value opens the stage's card, and both recede
        // with the stage. Read by the reveal layer and the Present Guide — see slope.transform.js
        // `proxyAttrs` for why it is not `data-mark`.
        className: 'funnel-label', attrs: ` data-anima-role="label" data-mark-for="${i}"`,
    ...
    if (s.valueRaw) {
      parts.push(wrapSvgLabel(s.valueRaw, {
        ...
        baseline: 'central',
        className: 'funnel-value', attrs: ` data-anima-role="label" data-mark-for="${i}"`,
        emitFontSize: false,
      }).svg);
    }
  ...
  // `<text>` above is unreachable and this string is the ONLY route to the data.
  // It therefore has to carry the CONVERSION RATES too: a funnel exists to show
  // drop-off, and an earlier version of this desc listed stage values only, which
  // left the one number the chart is for unreachable by any route. Stages are joined
  // with `;` because a label may itself contain a comma ("Logged, with the decision"),
  // which made a comma-joined list ambiguous read aloud.
  const descText = stages
    .map((s, i) => {
      const head = s.valueRaw ? `${s.label} ${s.valueRaw}` : s.label;
      if (i === 0 || !stages[i - 1].num || !s.num) return head;
      const pct = Math.round((s.num / stages[i - 1].num) * 100);
      return `${head} (${pct}% of ${stages[i - 1].label})`;
    })
    .filter(Boolean)
    .join('; ');
  ...
  const svg = `<svg class="funnel-svg" viewBox="${g.viewBox}" preserveAspectRatio="xMidYMid meet" role="img">` +
    `<title>Funnel chart</title>${desc}${ariaHiddenMarks(parts.join(''))}</svg>`;
...
/**
 * The chart-family entrypoint (see the `kernel` block in funnel.manifest.json).
 */
...
module.exports = {
  transformSection, parseFunnel, buildFunnel, GEOM };
