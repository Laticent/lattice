...
- A dense chart tightens its own rows — past a certain row count the bar height, sub-row gap and lane padding compress together so the unit keeps its width instead of scaling down into a narrow column. Bars never go below a floor; if you are hitting it, the slide is carrying more plan than one slide should.
