...
- A dense chart tightens its own rows — past a certain row count the bar height, sub-row gap and lane padding compress together so the unit keeps its width instead of scaling down into a narrow column. A chart of up to four landscape lanes (three in portrait) at one task each keeps the full-size band; past that it compresses, toward a floor it never goes below. If you are hitting the floor, the slide is carrying more plan than one slide should.
