...
const GANTT_GEOM_TALL = {
  ...
  //
  // THE MARGIN IS ONE MORE BAR ROW, the same unit landscape's comment budgets in.
  ...
  // bar row costs barH + rowGap = 21. So the chart can take one more row before the
  // stage cuts it, and that is what the surplus buys. Landscape on the same fixture
  // measures 76.5px over a 371.7px stage = 31.9 units against a 16-unit row pitch,
  // so both bands hold at least one row in reserve.
  rowGap: 5,
  lanePadY: 7,
  // NOT 12, and the exception is deliberate. Everything above took the 0.8; this did
  // not, because it is a TYPE box rather than a bar band — and the type did not take
  // the 0.8 either (fsLane 9 -> 8.5 is 0.94). Measured on the same fixture: the lane
  // name's rendered box is 10.8 units tall in this 15-unit band, so it sits with 4.2
  // units of clearance, about 2.1 above and 2.1 below. Scaled to 12 the same type
  // would leave 1.2 units in total — roughly 2px on each side at 3.24px/unit, with
  // the lane rule immediately above it.
  //
  // The consequence is real and it is the right trade: non-bar chrome's share of a
  // one-row portrait lane rose from 29% (15 of 52.5) to 33% (15 of 45) across the
  // trim. Holding the share instead would have bought back 3 units — a seventh of a
  // bar row — by crowding the one piece of the lane a reader has to READ.
