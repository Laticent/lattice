...
const GANTT_GEOM = {
  ...
  // gave, which is what keeps every one-task-per-lane chart — and the tests that
  // pin them — byte-identical through the packing change.
  barH: 15,          // bar height CEILING, centered in its sub-row
  barHMin: 11,       // …and the floor. Below this the caption stops clearing the
                     // bar's own rounded corner at fsBar 8.5.
...
function buildGanttChart(ulInner, eyebrowHtml, orientation, hand = false) {
  ...
  const clipDefs = [];
  const clipSeq = nextRenderSeq('gantt-clip');
  ...
  const barClip = (x, y, w) => {
    ...
    if (!id) {
      ...
      clipIds.set(key, id);
      clipDefs.push(`<clipPath id="${id}"><rect x="${x.toFixed(2)}" y="${y.toFixed(2)}" ` +
  ...
  const laid = lanes.map((lane) => lane.tasks.map((t) => {
    ...
    // A caption that fits inside its bar costs the row nothing; one that does not
    // will sit after the bar, so the row has to hold it.
    const insideRoom = x1 - x0 - G.fsBar * 0.7;
    const right = est <= insideRoom ? x1 : x1 + G.fsBar * 0.47 + est;
    return { t, label, kind: 'bar', x0, x1, est, left: x0, right };
  }));
  ...
  // row, so the common "three sequential phases" lane stays one row tall and a
  // single-task lane is byte-identical to what it rendered before.
  ...
  if (todayText) {
    ...
    if (tp) {
      ...
      if (p >= 0 && p <= 100) {
        const x = xOfPct(p);
        const cap = 3;
        // The cap sits ABOVE the plot band and the line starts below it, so the
        // marker reads as a header tick pointing INTO the schedule rather than a
        // triangle dropped on the first lane's first bar — which is what it did
        // when both started at lanesTop.
...
const GANTT_ASPECT_FLOOR = { landscape: 2.6, portrait: 1.2 };
 ...
 * ceiling — so every chart that fits comfortably keeps identical geometry, and a
 * one-task-per-lane chart is byte-identical to what it rendered before sub-rows
 * existed. A DENSE one spends it on rows instead, compressing toward barHMin.
 */
function ganttBandFor(G, rows, lanes) {
  ...
  return { barH, rowGap: barH * G.rowGapR, lanePadY: barH * G.lanePadR };
}
