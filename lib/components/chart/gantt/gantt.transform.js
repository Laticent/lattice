...
const GANTT_GEOM = {
  ...
  // A lane is no longer ONE row. Its height is derived from how many sub-rows
  // the packer needed: laneHeightOf() = rows*barH + (rows-1)*rowGap + 2*lanePadY.
  //
  // `barH` is a CEILING, not a constant — see ganttBandFor(). rowGapR and
  // lanePadR are ratios OF barH, so one number drives the whole band and a
  // compressed chart stays in proportion. At the ceiling they give exactly the
  // 5 and 5.5 that make a SINGLE-row lane measure the 26 units the flat `laneH`
  // gave.
  //
  // THAT IS A THRESHOLD, NOT A BLANKET "NOTHING CHANGES". A one-task-per-lane
  // chart keeps the ceiling only while the whole chart still clears the aspect
  // floor — measured, up to three landscape lanes and three portrait ones. Past
  // that the band compresses and the geometry moves. That is intended (the
  // compressed chart uses more of its width and its bars come out physically
  // thicker), but it is not "byte-identical", which is what this comment used to
  // claim. The threshold also moved once in a single session when the key block
  // was retuned, which is why it is stated as measured rather than asserted.
  barH: 15,          // bar height CEILING, centered in its sub-row
  // …and the floor: the thinnest bar that still seats an fsBar 8.5 caption. Cap
  // height is ~0.7 x fsBar = 5.95u, so 8 leaves ~1u of clearance above and below
  // it — snug, and measured legible at 19.3px on the dense stress page. It was 11
  // while the aspect floor was 2.6; at the measured 3.0 floor an 11u minimum is
  // the thing that stops a dense chart reaching full width, so the two move
  // together and neither is meaningful alone.
  barHMin: 8,
  rowGapR: 1 / 3,    // sub-row gap WITHIN a lane / barH (tighter than 2*lanePadR,
                     // so a lane's rows group before they separate)
  lanePadR: 11 / 30, // lane band → first/last bar, / barH
  barRx: 3,
  // THE KEY'S BLOCK IS LANDSCAPE-ONLY TIGHT, and the asymmetry with
  // GANTT_GEOM_TALL is the point. vbW is fixed and vbH grows with the row count,
  // so on a landscape stage every unit of non-row chrome is a unit the whole
  // drawing scales down by when the stage is shorter than the chart's aspect
  // wants — and a landscape slide's stage is short precisely because a lede or a
  // two-line heading took the room. Measured on the committed gallery at
  // 1152x335: 14 + 16 + 6 of gap/key/pad cost the drawing 8% of its width
  // (1059px of 1152) and 2.6px of bar height. At 9 + 12 + 4 it draws 1142px —
  // 99% — with 35.7px bars.
  //
  // CLEARANCE ABOVE THE KEY is lanePadY + legendGap - legendSwatch/2, so it moves
  // with the band rather than being fixed: 30u at the barH ceiling, 8.5u once
  // barH sits on its floor. It does NOT always exceed the swatch's own height —
  // an earlier version of this comment claimed it did, and the dense stress page
  // is exactly where that is false. It still reads as a separated key there,
  // because the lane rule above it is the real divider. Portrait keeps the looser
  // block: its stage is tall, so the same units buy nothing back and only open a
  // gap.
...
const GANTT_GEOM_TALL = {
  ...
  padRight: 8,
  axisH: 20,
  // As landscape: a single-row lane still measures the 52 units the flat
  // `laneH` gave (15 name + 20 bar + 2*8.5 pad), so the portrait geometry test
  // keeps reading the same number.
  barHMin: 13,
  rowGapR: 6 / 20,
  lanePadR: 8.5 / 20,
...
function buildGanttChart(ulInner, eyebrowHtml, orientation, hand = false) {
  ...
  let G = orientation === 'portrait' ? GANTT_GEOM_TALL : GANTT_GEOM;
  ...
  // The band is resolved ONCE, from the finished packing, and then read as if it
  // were part of the geometry — so a chart's bars are all one thickness even
  // though one lane needed three rows and another needed one.
  const band = ganttBandFor(G, packed.reduce((a, q) => a + q.rows, 0), lanes.length);
  G = { ...G, ...band };
  ...
  lanes.forEach((lane, li) => {
    ...
    // The clear space around a caption is bounded by this task's NEIGHBOURS ON
...
// the bar's own leading edge, the SVG port of the HTML bar's `border-left` —
// so it scales with the bar's height and corner radius, exactly as they do. The
...
// The viewBox aspect a gantt holds itself to. It is not a style choice — it is
// what decides whether the chart FITS the stage it is handed.
//
// vbW is fixed and vbH grows with the row count, and the svg is WIDTH-DRIVEN
// (gantt.styles.css: `width:100%; height:auto; flex-shrink:0`), so the drawing
// always spans the full width and the aspect decides how TALL it comes out.
// Past the floor a chart is taller than its stage, overflows, and the engine
// reports it by name — see that rule's comment for why being told beats the
// silent shrink a capped, letterboxing svg used to do instead.
//
// 3.5 (landscape) IS THE MEASURED STAGE. The chart body's aspect runs 3.04-3.48
// across the committed gallery and the demo deck, so a chart emitted at 3.5 is
// at least as wide-for-its-height as every stage in the corpus and fits all of
// them. An earlier cut used 2.6, picked as a "reasonable" number rather than
// measured, and the dense stress page drew at 79% of its body's width.
//
// A HIGHER floor is strictly safer for fit and strictly worse for bar thickness.
// Measured on the gallery: at 3.5 every page draws 100% wide with 34.5px bars
// and 95-100% of the body's height; at 3.0 every page also draws 100% wide with
// 36.0px bars, but runs 101-102% of the height and leans on that overrun being
// absorbed. 1.5px of bar buys a fit that does not depend on slack.
//
// IT IS PER-GEOMETRY, and a single constant was wrong in the expensive
// direction. Portrait's whole purpose is to be "narrow-and-tall so it fills a
// portrait box rather than letterboxing into a thin band" (GANTT_GEOM_TALL): its
// natural aspect at the ceiling is 1.83, so a landscape floor above that
// compressed EVERY portrait chart to barHMin — including a one-task-per-lane one
// with room to spare. The portrait floor sits below its natural aspect, so a
// sparse portrait chart keeps the ceiling and only a genuinely dense one
// compresses.
const GANTT_ASPECT_FLOOR = { landscape: 3.5, portrait: 1.2 };

/**
 * Resolve the row band for a chart of `rows` sub-rows across `lanes` lanes:
 * `{ barH, rowGap, lanePadY }`, in viewBox user units.
 *
 * A SPARSE chart spends its height budget on thickness and pins barH at the
 * ceiling; a DENSE one spends it on rows instead, compressing toward barHMin.
 *
 * "Sparse" is a MEASURED threshold, not "has one task per lane": the clamp binds
 * on the whole chart's row count against the aspect floor, so a one-task-per-lane
 * chart keeps the ceiling up to three landscape lanes and three portrait ones, and
 * compresses past that. Do not restate it as "a single-task chart never changes"
 * — that was the earlier claim, and it is false at four lanes.
 */
function ganttBandFor(G, rows, lanes) {
  // The height everything that is NOT a row costs: the tick row, the gap under
  // the axis rule, and the key. Approximate by design — it only steers a clamp,
  // and a chart with no status emits no key, which can only leave more room.
  const chrome = G.axisH + 4 + G.legendGap + G.legendH + G.padBottom +
    (G.portrait ? lanes * G.laneNameH : 0);
  const budget = G.vbW / GANTT_ASPECT_FLOOR[G.portrait ? 'portrait' : 'landscape'] - chrome;
  // barH factored out of rows*barH + (rows-lanes)*rowGap + 2*lanes*lanePadY.
  const denom = rows + (rows - lanes) * G.rowGapR + 2 * lanes * G.lanePadR;
  const barH = Math.max(G.barHMin, Math.min(G.barH, budget / denom));
  // ROUNDED, because barH is written straight into `height=` on every bar, every
  // milestone clip and every accent, and an unrounded one emits
  // `height="13.039940828402369"` fifteen times per chart. Deterministic, so
  // never a correctness risk — but it lands in every export and every future
  // PDF diff. Rounding here fixes all four sites at once.
  const h = +barH.toFixed(2);
  return { barH: h, rowGap: +(h * G.rowGapR).toFixed(2), lanePadY: +(h * G.lanePadR).toFixed(2) };
}
...
// `ganttGutter` and `ganttBandFor` are exported for the TESTS, which express
// their assertions as percentages of the axis and so have to undo the inset the
// gutter adds. They used to restate it as a literal `BAR_INSET = 1.5`, which is
// the one thing the helpers' own comment says not to do — and it went stale the
// first time the gutter moved.
module.exports = {
  transformSection, buildGanttChart, GANTT_GEOM, GANTT_GEOM_TALL,
  ganttGutter, ganttBandFor,
};
