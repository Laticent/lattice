...
const GANTT_GEOM = {
  ...
  // floor — measured, up to four landscape lanes and three portrait ones. Past
...
// The viewBox aspect a gantt tries not to fall below. It is not a style choice —
// it is the point where the chart stops using its stage.
//
// vbW is fixed and vbH grows with the row count, so a tall viewBox scales the
// WHOLE unit down: `preserveAspectRatio="xMidYMid meet"` fits the taller
// dimension and the drawing narrows, leaving the side margins empty. Measured in
// Chromium on the committed stress page (4 lanes, 12 tasks, a 1152x379 chart
// body): at vbH 264 the drawing used 689px of 1152 — 60% of the width — with
// 12.2px captions. The same page at vbH 210 uses 866px (75%) and 15.3px
// captions. The bars lose 1.6px of thickness to buy that.
//
// 2.6 (landscape) is where those two curves cross for the body aspects this
// engine actually hands a chart (measured 3.04-3.48 across the gallery). Chasing
// full width would need barH ~9, at which point a caption no longer clears the
// bar's own corner radius — which is what barHMin defends.
//
...
// natural aspect at the ceiling is 1.83, so a 2.6 floor compressed EVERY portrait
// chart to barHMin — including a one-task-per-lane one with room to spare. The
// portrait floor sits below that natural aspect, so a sparse portrait chart keeps
// the ceiling and only a genuinely dense one compresses.
const GANTT_ASPECT_FLOOR = { landscape: 3.0, portrait: 1.2 };
 ...
 * chart keeps the ceiling up to four landscape lanes and three portrait ones, and
 * compresses past that. Do not restate it as "a single-task chart never changes"
 * — that was the earlier claim, and it is false at five lanes.
 */
