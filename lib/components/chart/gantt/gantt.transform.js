...
const GANTT_GEOM = {
  ...
  // a single-row lane still measures the 26 units the flat `laneH` gave.
  //
  ...
  //
  // The component owns a BUDGET instead — `capacity` in gantt.manifest.json,
  // which `lint:deck` reports against before anything renders, and which every
  // other component of this size already declares (kanban: soft 5, hard 6). The
  // geometry stays put: a gantt of any size draws its bars and its captions at
  // exactly one physical size, and a plan too big for its stage overflows and is
  // named, the same as any other component past its budget.
  barH: 15,          // bar height, centered in its sub-row
  rowGap: 5,         // sub-row gap WITHIN a lane (tighter than 2*lanePadY, so a
                     // lane's rows group before they separate)
  lanePadY: 5.5,     // lane band → first/last bar
  barRx: 3,
...
const GANTT_GEOM_TALL = {
  ...
  padRight: 8,
  axisH: 20,
  // As landscape, and constant for the same reason: a single-row lane measures
  // the 52 units the flat `laneH` gave (15 name + 20 bar + 2*8.5 pad).
  rowGap: 6,
  lanePadY: 8.5,
