...
const GANTT_GEOM_TALL = {
  ...
  //
  // THE TRIM'S CAUSE, AND ITS MARGIN, ARE TWO DIFFERENT NUMBERS. The cause was a
  // capped SVG: both bands had been sized against a `max-height` letterbox that no
  // longer happens, and portrait was the half that got missed — the canonical
  // two-lane fixture painted 9px past `.cell-stage`, which is `overflow: clip`, so
  // it was cut in silence. In viewBox units that overflow was 2.8 (9px at 3.24px/u)
  // and the trim spends 24. The surplus is not slack that was left lying around, and
  // "it carries the same 0.8 landscape took" — which is what this comment used to
  // say — is the MECHANISM, not the justification.
  //
  // THE MARGIN IS ONE MORE BAR ROW — measured in PITCH (barH + rowGap), where
  // landscape's comment above budgets in barH alone (12u). The pitch is the right
  // unit; read that as this comment correcting its neighbor, not agreeing with it.
  // Measured on `test/fixtures/chart-fit.md` p9 at portrait (1080x1350, real
  // Chromium): stage 972x690.8, svg 972x622.1, viewBox 300x192 at 3.24px/unit —
  // 68.7px of headroom, 9.9% of the stage, which is 21.2 viewBox units. A portrait
  // bar row costs barH + rowGap = 21, confirmed rather than assumed: adding one
  // overlapping task moves vbH 192 -> 213. Landscape on the same fixture measures
  // 76.5px over a 371.7px stage = 31.9 units against a 16-unit pitch, so ~2 rows.
  //
  // THE RESERVE IS ONE ROW BY 0.67px, which is INSIDE `check-chart-fit`'s own 1.5px
  // slack. "One more row" is the ceiling, not a comfortable margin — and it is a
  // property of THIS STAGE, not of these constants. p9's own heading ALREADY wraps to
  // two lines at portrait, at 91.4px a line, and every further line comes straight off
  // the stage while the chart does not move: three lines leaves a 599.4px stage and the
  // chart overflows by 22.7px, five leaves 416.7px and it overflows by 205px. The band
  // reserves a row on the fixture's stage; nothing here can promise one on every stage,
  // and an earlier draft of this comment read as if it did — it also attached the 416.7px
  // figure to a three-line heading, which is the five-line one.
  rowGap: 5,
  lanePadY: 7,
