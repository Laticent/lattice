...
const {
  escAttr, plainText, stripTrailingPills, spliceFirstList, CHART_STATUS, chartStatus,
...
const { spokenStatus } = require('../../../core/chart-status');
// The shared axis grammar (lib/core/bracket-list.js), the position rule that
// makes a bracketed list above the chart body its axis, and the one reading of
// a time-axis member.
const { liftBracketSpans, axisAcceptor } = require('../../../core/lift-bracket-span');
const { parseBracketList } = require('../../../core/bracket-list');
const { readTimeAxisMember } = require('../../../core/axis-member');
...
// The point and span parsers live in lib/core/gantt-time.js, shared with narration.
const { parseTimePoint, parseSpanToken } = require('../../../core/gantt-time');
...
// thing a gantt's color is for. Running work is `info`; finished work is `pass`.
//
...
/**
 * Five ramps, ten status words — so some words must share, and WHICH share is
 ...
 * declared status and the absence of one paint the same bar. Until #2255 an unstated
 * bar emitted no legend chip either, so nothing named it; it emits a `no status` chip
 * now, when the chart already draws a key.
 */
...
const GANTT_GEOM_TALL = {
  ...
  padRight: 8,
  axisH: 20,
  // As landscape, and constant for the same reason. A single-row lane measures 45
  // units (15 name + 16 bar + 2*7 pad).
  //
  // THE TRIM'S CAUSE AND ITS MARGIN ARE TWO DIFFERENT NUMBERS, which is why "it
  // carries the same 0.8 landscape took" is the mechanism and not the justification.
  // The cause was a capped SVG sized against a `max-height` letterbox that no longer
  // happens.
  //
  // WHAT #2250 SPENT IS NOT A SINGLE NUMBER, and an earlier version of this comment
  // gave one ("the trim spends 24") that does not reproduce under any reading. #2250
  // did not only retune constants, it RESTRUCTURED the band: portrait was a flat
  // `laneH: 52` before, and is composed after — laneNameH 15 + barH 16 + 2*lanePadY 7
  // = 45 for a single-row lane. That is 7 units a lane cheaper, and a lane that packs
  // a second sub-row now costs 45 + 21 = 66, which is DEARER than the flat 52 it
  // replaced. Subtracting one from the other was comparing two different shapes.
  //
  // THE MARGIN IS ONE MORE BAR ROW, measured in PITCH (barH + rowGap) rather than in
  // barH alone as landscape's comment above does — read that as this comment
  // correcting its neighbor. On `test/fixtures/chart-fit.md` p9 at portrait the band
  // leaves 68.7px of headroom, 21.2 viewBox units against a 21-unit pitch.
  //
  // THAT RESERVE IS ONE ROW BY 0.67px, inside `check-chart-fit`'s own 1.5px slack, and
  // it is a property of THAT STAGE rather than of these constants: p9's heading already
  // wraps to two lines at portrait and every further line comes straight off the stage
  // while the chart does not move. Nothing here can promise a spare row on every stage.
  rowGap: 5,
  lanePadY: 7,
  // NOT 12, and the exception is deliberate. The BAR BAND took the 0.8 — barH 20->16,
  // rowGap 6->5, lanePadY 8.5->7, the figures #2250's own commit message states — and
  // this did not, because it is a TYPE box. The type did not move AT ALL: portrait's
  // `fsLane` was 8.5 before the trim and 8.5 after, a ratio of 1.00. (Landscape's is
  // 9, so 8.5/9 = 0.94 is a CROSS-ORIENTATION ratio. An earlier draft spliced that
  // into the before/after list, where it read as if the type had shrunk with the
  // bars. It did not, and 1.00 makes the point more strongly than 0.94 did.)
  //
  // Measured on the same fixture: the lane name's rendered box is 10.8 units tall in
  // this 15-unit band, with 2.36 above it and 1.84 below — ASYMMETRIC, and necessarily
  // so: the label is placed `vAlign: 'baseline'` at `rowY + laneNameH - 4`, which
  // fixes the bottom and leaves the top with whatever remains.
  //
  // At 12 the same type does NOT sit tight with ~2px a side, which is what an earlier
  // draft claimed. The baseline moves down with the band while the ascent does not, so
  // the box top lands 0.64 units ABOVE rowY and the name CROSSES THE LANE RULE. The
  // real floor at this baseline offset is laneNameH 12.65.
  //
  // WHAT THAT DOES NOT ESTABLISH IS 15. 14 clears the floor and saves a unit per lane,
  // and the binding term is really the `- 4` offset at the wrapSvgLabel call below, not
  // this constant. 15 is the value that shipped and measures clean; it is not derived,
  // and nothing in the tree holds it — the only guard (`gantt.test.js`, laneNameH >=
  // fsLane * LINE_HEIGHT) permits 9.86, well under the 12.65 floor. Logged, not fixed
  // here (HARD RULE #18: found, not caused).
  //
  // The share the trim moved is the NAME BAND's: 15 of 52 (29%) before, 15 of 45 (33%)
  // after. NOT "non-bar chrome", which is laneNameH + 2*lanePadY = 29 of 45 (64%) — an
  // earlier draft used that label for these numbers and measured a different quantity.
  // Holding the name band's share would have bought back 3 units, a seventh of a bar
  // row, by crowding the one piece of the lane a reader has to READ.
...
function buildGanttChart(ulInner, eyebrowHtml, orientation, hand = false) {
    ...
    if (sub) {
      for (const item of parseTopLevelLis(sub.inner)) {
        ...
        for (const p of pills) {
          const pl = p.trim();
          // A dependency is never DRAWN (lint validates it), but it is said: the
          // <desc> and the narration both name it, so a reader learns the order.
          if (/^after\s*:/i.test(pl)) { after.push(pl.replace(/^after\s*:\s*/i, '').trim()); continue; }
          if (/^milestone$/i.test(pl)) { milestone = true; continue; }
          const folded = chartStatus(pl);
          if (folded) { status = folded; continue; }
        ...
        tasks.push({ label: leadStripped.trim(), status, milestone, span: sp, pts, detail, spanText, after: after.filter(Boolean) });
      }
    }
    return { label, tasks };
  });
  ...
  lanes.forEach((lane, li) => {
    ...
    lanesSvg += G.portrait
      ? wrapSvgLabel(stripTags(lane.label), {
        ...
        className: 'gantt-lane-label', attrs: ` data-pos="above" data-label="${escAttr(stripTags(lane.label))}"`,
      }).svg
      : wrapSvgLabel(stripTags(lane.label), {
        x: G.laneW - 6, y: midY, width: G.laneW - 10, fontSize: G.fsLane,
        anchor: 'end', vAlign: 'middle', baseline: 'central', maxLines: 2,
        // `data-label`: `narrateGantt` opens each lane with its name ("Framework,
        // three tasks."), and the Present Guide points at the label that says it.
        className: 'gantt-lane-label', attrs: ` data-label="${escAttr(stripTags(lane.label))}"`,
      }).svg;
    ...
    packed[li].tasks.forEach((it) => {
      ...
      // The statuses present drive the key — and so does the ABSENCE of one, which the
      // key could not name. An unstated bar takes the family's neutral, and so does
      // `deferred`; the two differ by paint alone (a hollow body, and now a dashed
      // edge), so a reader meeting a pale bar cannot tell "we pushed this" from
      // "nobody said" unless the key says which is which. `unscaled` is excluded: it is
      // the no-parseable-span placeholder, it carries its own class and `lint:deck`
      // names the cause, so keying it as "no status" would put two meanings on one chip.
      if (t.status) presentStatuses.add(t.status);
      else if (it.kind !== 'unscaled') hasUnstated = true;
      const mAttr = markAttrs(t);
      // The task's caption names this mark, so it recedes with it (chart hover, Present Guide).
      const link = ` data-mark-for="${markIdx - 1}"`;
      ...
      if (it.kind === 'unscaled') {
        barsSvg += `<rect class="gantt-bar gantt-bar--unscaled" data-anima-role="bar"${sAttr}${mAttr} ` +
          ...
          `fill="${fillOf('mute')}"/>`;
        barsSvg += ganttBarLabel(label, plotX0, plotX0 + plotW, midYr, nextX, G, link);
        return;
      }
      ...
      if (it.kind === 'milestone') {
        ...
        barsSvg += wrapSvgLabel(label, {
          ...
          className: 'gantt-mlabel', attrs: link,
        }).svg;
        return;
      }
      ...
      barsSvg += `<rect class="gantt-bar" data-anima-role="bar"${sAttr}${mAttr} ` +
        ...
        `height="${G.barH}"/>`;
      barsSvg += ganttBarLabel(label, bx0, bx1, midYr, nextX, G, link);
    });
  });
  lanesSvg += '</g>';
  ...
  if (keyStatuses.length) {
    // THE KEY NAMES THE NEUTRAL TOO, when there is one to name. An unstated bar and a
    // `deferred` bar both take the family's mute ramp, so the key used to explain every
    // bar on the slide EXCEPT the pale ones — and those are exactly the two a reader
    // cannot tell apart. The chip carries no `data-s`, so it falls to the swatch's own
    // neutral fallback and cannot impersonate a status.
    //
    // Only when a key is already being drawn: a chart with no declared status at all has
    // no ambiguity to resolve, and a one-chip key reading "no status" on it would cost a
    // lane of height to say nothing.
    //
    // AND ONLY WHEN IT FITS. The key is one centered row with no wrap, so a chart
    // carrying many statuses can already run its chips past the viewBox; adding a chip
    // unconditionally would make this change the cause of that on charts it has nothing
    // to do with. When the extra chip will not fit, the bars keep the paint distinction
    // (hollow + dashed for `deferred`, solid for unstated) and the key stays legible.
    ...
    const chipOf = (st, text) => ({ st, text, w: G.legendSwatch + swGap + text.length * G.fsLegend * 0.6 });
    const chips = keyStatuses.map((st) => chipOf(st, st));
    const widthOf = (cs) => cs.reduce((a, c) => a + c.w, 0) + (cs.length - 1) * 10;
    if (hasUnstated) {
      const neutral = chipOf(null, 'no status');
      if (widthOf([...chips, neutral]) <= G.vbW - 8) chips.push(neutral);
    }
    const totalW = widthOf(chips);
    ...
    for (const c of chips) {
      ...
      legendSvg += `<rect class="gantt-legend-swatch"${c.st ? ` data-s="${escAttr(c.st)}"` : ''} ` +
  ...
  //
  // It says what the picture says and a listener cannot see: a DIAMOND is a
  // milestone, the dashed `after:` order, the window the axis spans and where
  // today falls. Statuses in their spoken form (`at risk`, not `at-risk`) — the
  // same words `narrateGantt` uses, from the one table in lib/core/chart-status.js.
  const descLanes = lanes.map((lane) => {
    const tasks = lane.tasks.map((t) => {
      const point = t.span.pointRaw != null;
      // Diamond iff `isMilestone` below draws one: a `milestone` flag or a single point.
      // "Q1 to Q2", not the drawn "Q1–Q2": a screen reader says the dash as "dash".
      const said = t.spanText.replace(/–/g, ' to ');
      const when = !t.spanText ? '' : (t.milestone || point) ? `, milestone ${point ? 'at' : 'over'} ${said}` : `, ${said}`;
      const st = t.status ? `, ${spokenStatus(t.status)}` : '';
      const dep = t.after.map((a) => `, after ${a}`).join('');
      return `${stripTags(t.label)}${when}${st}${dep}`;
    }).join('; ');
    return tasks ? `${stripTags(lane.label)}: ${tasks}` : stripTags(lane.label);
  }).filter(Boolean).join('. ');
  const frameNote = [
    windowText ? `Plan from ${windowText.split('..').map((x) => x.trim()).join(' to ')}` : '',
    todayText ? `today is ${todayText}` : '',
  ].filter(Boolean).join(', ');
  const descText = [frameNote ? frameNote.charAt(0).toUpperCase() + frameNote.slice(1) : '', descLanes].filter(Boolean).join('. ');
  const desc = descText ? `<desc>${escAttr(descText)}</desc>` : '';
...
// gutter: two adjacent bars showed a 6px colored band between them that was
...
function ganttBarLabel(label, bx0, bx1, midY, limitX, G, link = '') {
  if (!label) return '';
  ...
  if (est <= inner) {
    return wrapSvgLabel(label, {
      ...
      className: 'gantt-bar-label', attrs: ` data-pos="inside"${link}`,
    }).svg;
  }
  ...
  if (roomRight >= est) {
    return wrapSvgLabel(label, {
      ...
      anchor: 'start', vAlign: 'middle', baseline: 'central', maxLines: 1,
      className: 'gantt-bar-label', attrs: ` data-pos="after"${link}`,
    }).svg;
  }
...
/**
 * The bracketed axis — `[{Timeline, 2026 Q1..2026 Q4, Q3}]` — as the SAME
 * `{window, today}` pair the keyword pills produce.
 *
 * TWO FORMS, ONE BUILDER, and that is the point. The pills are keyword-tagged
 * and order-independent (`today Q3` means the same anywhere), which is a
 * genuinely different reading from a positional list, so no author loses them.
 * The list is the shared axis grammar (engineering/decisions/2026-09-22-chart-axis-grammar.md).
 * Both feed `buildGanttChart` the same two strings, so both draw the same chart
 * by construction rather than by a second implementation that has to agree.
 *
 * The list is LIFTED — the pills never were, because they read as an eyebrow;
 * the list reads as syntax. Its name has no place on the chart yet (a gantt
 * draws no axis caption), so it is the one part not drawn; the decision note
 * records that. THE DECLARATION IS REQUIRED: a missing catalog entry would turn
 * `keyBelow` on and eat the trailing coda this manifest claims.
 */
function readGanttAxis(html) {
  const decl = AXIS_CATALOG.gantt;
  if (!decl) throw new Error('gantt: no axisSet entry — run build-stage-catalog.');
  ...
  if (!lifted.above) return null;
  const m = readTimeAxisMember(parseBracketList(lifted.above, { maxParts: 3 })?.[0]);
  return { html: lifted.html, window: m.window, today: m.today };
}

// The same pair, as the pill markup `parseGanttEyebrow` reads, so the list and
// the pills enter the builder through one door.
function axisAsPills(axis) {
  const esc = (t) => String(t).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  return (axis.window ? `<code>${esc(axis.window)}</code>` : '') +
    (axis.today ? `<code>today ${esc(axis.today)}</code>` : '');
}
...
function transformSection(html, ctx) {
  ...
  const axis = readGanttAxis(html);
  if (axis) {
    const out = spliceFirstList(axis.html, (ext) => buildGanttChart(ext.inner, axisAsPills(axis), ctx.orientation, hand));
    // No chart drawn means nothing to be the axis OF — put the list back.
    return out === axis.html ? html : out;
  }
...
// gutter moved.
module.exports = {
  transformSection, buildGanttChart, readGanttAxis, GANTT_GEOM, GANTT_GEOM_TALL, ganttGutter,
};
