...
function parseHeatmapTable(tableHtml) {
  ...
  // column after it to its neighbor's data and dropped the tail — silently, with
    ...
    row.cells.forEach((cell, ci) => {
      ...
      if (cellFitsValue(shown, cellW, advance)) {
        ...
        parts.push(cart.buildValueLabel(shown, {
          ...
          fontSize: FS.value,
          width: cellW,
          // Linked to its own cell, so the number recedes with the cell under the chart hover and
          // the Present Guide instead of floating at full strength over a dimmed cell.
          extra: ` data-step="${step}" data-mark-for="${ri * cols.length + ci}"`,
        }));
      }
    });
  });

  // Every crossing, as a table a screen reader can walk — the `<desc>` above keeps the finding.
  const table = cart.buildSrDataTable({
    caption: 'Heatmap values', corner: '', cols,
    rows: rows.map((r) => ({ head: r.label, cells: r.cells.map((c) => (c ? fmt(c.num) : null)) })),
  });
  if (!key) {
...
/**
 * The accessible description — the chart's FINDING, now that the marks are
 * `aria-hidden` (cartesian.js § ariaHiddenMarks). Every crossing rides the
 * screen-reader-only table `buildHeatmap` appends (cartesian.js
 * `buildSrDataTable`), so this can stay the summary it is meant to be.
 *
