...
const { spliceFirstList, readsHandBody } = require('../_chart-family/transform-utils');
const {
...
function stepFor(num, min, max) {
  ...
  // CLAMPED AT BOTH ENDS. The top clamp is because t === 1 at the matrix maximum;
  // the bottom one is because a value BELOW the domain gives step 0, which matches
  // no rule for the same reason a NaN does. In-tree the domain is taken from the
  // rows that survive the cap and every painted cell sits inside it, so this is a
  // contract guard rather than a live bug — but the contract is what four other
  // places rely on, so it holds unconditionally rather than by argument.
  const step = Math.max(1, Math.min(RAMP_STEPS, Math.floor(t * RAMP_STEPS) + 1));
  // AND A FINITENESS FLOOR, because `max > min` is not enough to make the ratio
  // finite. A span that overflows a double — `min` at -1e308 and `max` at 1e308,
  // both authorable as plain literals — makes both terms Infinity and the ratio
  // NaN, so `data-step="NaN"` reaches the DOM on the rect AND the text. No
  // `[data-step]` rule matches, `--mix` is never set, `color-mix()` is invalid at
  // computed-value time, and the cell paints the inherited BLACK — measured at
  // 1.83:1 for the value on it, on the matrix maximum, which is the one cell a
  // reader most needs. Four places (the stylesheet, the generator, the print
  // remap, the tests) assume `data-step` is 1..RAMP_STEPS; this keeps that true.
  return Number.isFinite(step) ? step : Math.ceil(RAMP_STEPS / 2);
}
