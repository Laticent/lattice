...
const { parseInlineSet, resolveLabelSet, unboundKeys } = require('../../../core/label-set');
 ...
 *
 * Every other member arrives here already wrapped — it hands `detailPayload` the
 * inner HTML of an authored sublist, which is a run of `<li>`s by construction.
 * A heatmap cell has no sublist to come from (a table cell cannot nest one), so
 * this is where the shape is put back.
 *
