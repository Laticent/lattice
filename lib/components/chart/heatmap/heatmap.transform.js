...
const { parseInlineSet, resolveLabelSet } = require('../../../core/label-set');
...
function buildHeatmap(model, ctx = {}) {
  ...
  const bands = model.bands
    ? resolveLabelSet(deriveBands(model, fmtEarly), model.bands)
    : null;
 ...
 */
function describe(model, fmt, bands) {
  const { rows, cols } = model;
  ...
  return `${rows.length} rows by ${cols.length} columns, `
    + `${fmt(model.min)} to ${fmt(model.max)}. ${lines.join('; ')}.${gap}${over}${scale}`;
}
...
function deriveBands(model, fmt) {
  const { breaks } = model;
  if (!breaks?.length) {
    return [{ key: String(Math.ceil(RAMP_STEPS / 2)), label: fmt(model.min) }];
  }
  // ONLY THE BANDS THE MATRIX ACTUALLY USES, which is the rule `roadmap`'s status
  // key already follows ("one chip per state ACTUALLY present"). A quantile cut
  // over few distinct values collapses boundaries onto each other, so a small
  // matrix can leave two or three of the five steps empty — and an empty band is
  // a key row pointing at a color nowhere on the slide, printed with a range
  // that reads as a duplicate of its neighbour.
  const used = new Set(model.rows.flatMap((r) => r.cells.filter(Boolean).map((c) => stepFor(c.num, breaks))));

  // CLOSED on the data at both ends: the first band starts at the matrix's own
  // minimum and the last ends at its maximum, so every label names two real
  // numbers from the slide rather than an open `<` or `and up` a reader has to
  // resolve against values that are not there. The bands are contiguous and
  // lower-inclusive, which is the classed-choropleth convention.
  return Array.from({ length: RAMP_STEPS }, (_, i) => i + 1).filter((n) => used.has(n)).map((n) => {
    const i = n - 1;
    const lo = i === 0 ? model.min : breaks[i - 1];
    const hi = i === RAMP_STEPS - 1 ? model.max : breaks[i];
    // A band whose ends meet carries one value, not a range — the case a
    // skewed matrix reaches when a quantile boundary lands on the extreme.
    const label = fmt(lo) === fmt(hi) ? fmt(lo) : `${fmt(lo)}–${fmt(hi)}`;
    return { key: String(n), label };
  });
}

/**
 * The author's label set for this slide, or null.
 *
 * Read from a ONE-CODE paragraph whose whole text is the bracketed set — the
 * same position `matrix-grid` reads its axis eyebrow from, and for the same
 * reason: a known position competes with nothing, where a global inline-code
 * rule would have to out-guess the pill grammar at every span in every deck.
 *
 * The paragraph is REMOVED from the body, so the set names the bands instead of
 * also printing as an eyebrow above the grid.
 */
const SET_PARA_RE = /<p[^>]*>\s*<code[^>]*>([\s\S]*?)<\/code>\s*<\/p>/g;

function liftLabelSet(html) {
  // EVERY one-code paragraph is a candidate, not just the first. A heatmap slide
  // routinely carries an EYEBROW in exactly this shape — `Retention · 2026
  // cohorts` — and it sets above the set. Testing only the first match found the
  // eyebrow, failed to parse it and gave up, so an authored set rendered as a
  // subtitle and the key never appeared. `matrix-grid`'s axis-eyebrow reader
  // documents the same trap from the other side: a whole-section scan that does
  // not check WHAT it matched shreds a code-only subtitle or legend.
  SET_PARA_RE.lastIndex = 0;
  let m;
  while ((m = SET_PARA_RE.exec(html)) !== null) {
...
function transformSection(html, ctx) {
  ...
  const src = lifted.set ? lifted.html : html;
  return spliceFirstTable(src, (ext) => {
    ...
    const payload = markDetail.detailPayload(marks);
...
module.exports = {
  ...
  deriveBands, liftLabelSet, cellMarks,
