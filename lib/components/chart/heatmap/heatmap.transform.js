 ...
 * function exists to make impossible. `gantt` already reads the face off the
 * slide for its tick advance, for exactly this reason; this does the same.
 *
...
function buildHeatmap(model, ctx = {}) {
  ...
  const advance = (ctx.classTokens || []).includes('sketch') ? ADVANCE_HAND_TRACKED : ADVANCE;
