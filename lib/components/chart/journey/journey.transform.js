...
function moodFaceSvg(mood) {
  ...
  return (
    ...
    `stroke-linejoin="round" aria-hidden="true">` +
      `<circle class="journey-face-disc" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="1.2"/>` +
...
function emitJourneyBoard(model) {
  ...
  const lanesHtml = actors.map(([name, color], ai) => {
    // --actor-color cycles the fixed cat palette; the lane's connector track + dot ring
    // paint compiled per-cat alphas (no color-mix in the paint). Derive the cat slot from
    // the palette entry and point --actor-track/--actor-ring at the matching flat tokens.
    const catN = /--cat-(\d)-mark/.exec(color)?.[1] || '1';
    const actorVars = `--actor-track:var(--journey-cat-${catN}-track); --actor-ring:var(--journey-cat-${catN}-ring)`;
    let lcol = 0;
    const lDots = [];
    for (const s of model.sections) {
      for (const t of s.tasks) {
    ...
    return (
      ...
      `style="--actor-color:${color}; ${actorVars}; --row:${ai + 1}">` +
