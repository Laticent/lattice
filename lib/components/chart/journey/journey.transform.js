...
function emitJourneyBoard(model, moodSet) {
  ...
  const sectionsHtml = model.sections.map((s, i) =>
    ...
    `style="--span:${s.tasks.length}; --section-index:${i}; --section-volume:${sectionVolumes[i]}" data-label="${escHtml(s.name)}">` +
  ...
  for (let si = 0; si < model.sections.length; si++) {
    ...
    for (const t of sec.tasks) {
      ...
      taskParts.push(
        ...
        `style="--col:${col}; --mood:${t.mood}; --volume:${vol}; --volume-pct:${volPct}" data-label="${escHtml(t.label)}">` +
