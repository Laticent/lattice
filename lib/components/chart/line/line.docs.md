...
- **Two to six series, named at the end of their own lines.** Past six the categorical palette repeats and the lines stop being distinguishable (Wong 2011). Each series is labelled at its right-hand end in its own color, so the chart carries no key to look away to — consolidate a long tail into ‘Other’ rather than adding a seventh line.
...
- [`bar`](../../chart/bar/bar.docs.md) — the categories are unordered and the claim is magnitude, not movement
- [`slope`](../../chart/slope/slope.docs.md) — there are exactly two points and the story is which entities changed rank
- [`stacked-bar`](../../chart/stacked-bar/stacked-bar.docs.md) — each period decomposes into parts and the periods are compared, not connected
- [`bullet`](../../chart/bullet/bullet.docs.md) — the number is read against a target and a qualitative band, not against its own past
- [`gantt`](../../chart/gantt/gantt.docs.md) — the horizontal axis is real dates carrying durations rather than an ordered set of measurements

## Demo deck

See [line.gallery.light.pdf](./line.gallery.light.pdf) for rendered examples of every variant.
