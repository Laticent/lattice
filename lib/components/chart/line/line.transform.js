...
function buildLine(model, ctx = {}, opts = {}) {
  ...
  const chrome =
    ...
    // NOT on a signed chart: `buildGrid` already draws the zero rule at the real
    // reference, and the plot-edge rule under it reads as the floor — the
    // substrate docblock says so and three siblings gate it the same way.
    (model.min >= 0 ? cart.buildAxisRule({ plot, axis: 'x' }) : '') +
