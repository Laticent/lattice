...
function buildLine(model, ctx = {}, opts = {}) {
  ...
  if (variant === 'stacked-area') {
    ...
    model.rows.forEach((row, s) => {
      ...
      const named = model.series[s] ? ` data-label="${escHtml(model.series[s])}"` : '';
      ...
      segs.forEach((idxs) => {
        ...
        if (idxs.length < 2) {
          ...
          dots.push(`<circle class="line-dot line-dot-lone" data-hue="${s + 1}" data-encodes="hue" data-paint="fill" data-anima-role="point"`
            + ` data-label="${escHtml(model.cats[idxs[0]])}" data-value="${escHtml(lp.raw != null && lp.raw !== '' ? lp.raw : String(lp.num))}"`
        ...
        const named = model.series[s] && !labelled.has(s) ? ` data-label="${escHtml(model.series[s])}"` : '';
      ...
      if (showDots) {
        row.forEach((p, i) => {
          ...
          dots.push(`<circle class="line-dot" data-hue="${s + 1}" data-encodes="hue" data-paint="fill" data-anima-role="point"`
            + ` data-label="${escHtml(model.cats[i])}" data-value="${escHtml(p.raw != null && p.raw !== '' ? p.raw : String(p.num))}"`
