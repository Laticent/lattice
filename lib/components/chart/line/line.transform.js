 ...
 * `data-hue`; not one color literal is emitted. Area fills reference the
...
const { spliceFirstList, escHtml, escAttr } = require('../_chart-family/transform-utils');
...
function parseLine(ulInner) {
  ...
  const rows = series.map((name) => model.groups.map((g) => {
    ...
    // The FIRST point wins a duplicate name, and the loser is recorded so the
    // domain can be recomputed without it. `parseSeries` takes min/max over
    // every point it parsed, so a copy-pasted `- EMEA \`9.9\`` under one
    // quarter used to stretch the axis to 9.9 with nothing drawn above 6 —
    // 40% of the plot height spent on a value no reader can see, and absent
    // from the description too.
    const hits = g.points.filter((q) => q.series === name);
    if (hits.length > 1) shadowed.push(...hits.slice(1).map((q) => q.num));
    const p = hits[0];
  ...
  // Counted in POINTS, not categories. `cats.length` let a half-filled draft
  // through — `- Q1 \`4.2\`` then three bullets reading `tbc` — and rendered
  // exactly the "lone dot adrift in an empty box" this refusal names, with a
  // full grid and an axis floated around one value.
  const present = keptRows.reduce((n, r) => n + r.filter(Boolean).length, 0);
  if (cats.length < 2 || present < 2) return null;
  ...
  return {
    ...
    marks,
    ...domainOf(keptRows, model, shadowed),
  };
}

/**
 * The min/max the AXIS is scaled to — over the points that are actually drawn.
 *
 * `parseSeries` reports the extremes of everything it parsed, which includes a
 * point shadowed by a duplicate series name and a category left flat among
 * nested ones. Neither is rendered, and either one stretches the axis around a
 * value no reader can find.
 */
function domainOf(rows, model, shadowed) {
  if (!shadowed.length && !model.mixedDepth) return { min: model.min, max: model.max };
  const drawn = rows.flat().filter(Boolean).map((c) => c.num);
  if (!drawn.length) return { min: model.min, max: model.max };
  return { min: Math.min(...drawn), max: Math.max(...drawn) };
}
 ...
 *      starts. A floating axis that is LABELED is not a deception; an unlabeled
...
function buildDesc(model, variant, totals) {
  ...
  const parts = [];
  // The span names the FIRST and LAST category — and says so, because a
  // category axis may repeat a label across a year boundary. The shipped `step`
  // sample runs Q1 Q2 Q3 Q4 Q1 Q2, and `Q1 to Q2` read as two adjacent quarters
  // when it means six readings across two years.
  const span = cats.length > 1
    ? `first ${cats[0]}, last ${cats[cats.length - 1]}`
    : cats[0];
  ...
  rows.forEach((row, s) => {
    ...
    // PEAK AND TROUGH. A summary carrying only the high point describes a rise,
    // not a shape: a dip-and-recover series read as a monotone climb, and the
    // gallery's own "never dipped below one quarter of runway" slide could not
    // state the dip it is about.
    if (peak.i !== last.i && peak.i !== first.i) bits.push(`peak ${shown(peak.p)} at ${cats[peak.i]}`);
    const low = present.reduce((b, q) => (q.p.num < b.p.num ? q : b), present[0]);
    if (low.i !== last.i && low.i !== first.i && low.i !== peak.i) {
      bits.push(`low ${shown(low.p)} at ${cats[low.i]}`);
    }
  ...
  if (variant === 'stacked-area' && totals) {
    // Through the MARK formatter, like every other number on the chart. Bare
    // `round2` printed `10400000` beside an axis reading `$10M` — eight digits
    // with no currency and no unit, read out as a digit string, in the one
    // channel a screen-reader user has.
    const fmt = cart.markFormatter({
      ticks: totals, step: Math.max(...totals.map(Math.abs)) / 4 || 1, affix: model.affix,
    });
    const f = totals[0];
    const l = totals[totals.length - 1];
    parts.push(`Total — ${fmt(f)} at ${cats[0]}, ${fmt(l)} at ${cats[cats.length - 1]}`);
  }
  // A SERIES THE CHART DID NOT DRAW IS STILL THE AUTHOR'S DATA. `parseSeries`
  // caps at six and reports the rest; `bar` puts the overflow in its desc and
  // this member received the report and dropped it, so the seventh series was
  // absent from the plot, the labels, the key AND the only accessible route.
  if (model.seriesOverflow?.length) {
    parts.push(`Not shown, past the six-series limit: ${model.seriesOverflow.join(', ')}`);
  }
  // A category left flat among nested ones carries no point for ANY series, so
  // every line breaks across it at once. Named, rather than left as an
  // unexplained gap in six lines.
  if (model.mixedDepth) {
    parts.push('One category was written as a single value rather than broken out by series, so every line breaks across it');
  }
  return `${parts.join('. ')}.`;
}
...
function buildLine(model, ctx = {}, opts = {}) {
  ...
  // WHERE THE SERIES ACTUALLY ENDS. Every direct label is painted in the right
  // gutter, past `plot.x1`, because that is where a label at the end of a line
  // belongs — but a series that stops reporting early does NOT end there. A
  // pilot with points at Q1 and Q2 had `Pilot 1.4` printed in the Q4 column
  // with 160 units of blank canvas between the line and its name, which reads
  // as "Pilot = 1.4 at Q4". Naming the category makes the label true.
  const endCats = model.series.map((_, si) => {
    if (variant === 'stacked-area') return null;
    const row = model.rows[si];
    let last = nCats - 1;
    while (last >= 0 && !row[last]) last -= 1;
    return last >= 0 && last !== nCats - 1 ? model.cats[last] : null;
  });
  const joined = model.series.map((name, si) => {
    if (!endValues[si]) return name;
    return endCats[si]
      ? `${name} ${endValues[si]} at ${endCats[si]}`
      : `${name} ${endValues[si]}`;
  });
  const oneLine = model.flat
    ? []
    : joined.map((t) => paintedWidth(t, labelBudget, FS.series, 1));
  // ALL OR NONE, counted over the series that HAVE a value. A stacked series
  // whose last category is a hole returns no end value, so `joined` held a bare
  // name beside five `Name 5.2` siblings and the tier decision — which this
  // file's own comment says is taken for every series at once — never fired.
  const withValues = !model.flat
    && oneLine.every((m) => !m.clipped)
    && endValues.every((v) => v);
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    // The gutters do NOT grow to hold a category label any more — `edgeAnchor`
    // turns the two edge labels inward instead, which is what the substrate
    // added the option for. Growing them cost two defects: `left` was clamped
    // at `LEFT_HARD_MAX`, so past ~20 characters the first label simply painted
    // outside the viewBox (`check-chart-fit` catches it at 30), and `right` was
    // not clamped at all, so an 80-character label squeezed the plot to 29% of
    // the canvas and a 140-character one inverted it to a negative width.
  ...
  const chrome =
    ...
    // ONLY WHEN THE AXIS ACTUALLY REACHES ZERO. A `line` axis floats when the
    // data sits well above zero, so the plot's bottom edge is not the baseline —
    // and the rule drawn there sat a couple of units under the lowest gridline,
    // reading as a duplicated rule whose heavier half a reader takes for zero.
    // On a signed chart the substrate's own docblock forbids it outright, and
    // three siblings gate it the same way; this is the same rule, stated as the
    // condition that makes it true.
    (ax.lo === 0 ? cart.buildAxisRule({ plot, axis: 'x' }) : '') +
    cart.buildValueTicks({ plot, ticks: ax.ticks, scale: y, step: ax.step, affix: model.affix, axis: 'y', gap: TICK_GAP }) +
    // `edgeAnchor` on the POINT scale only: a step chart's labels sit inside
    // their own band and are already clear of the edges.
    cart.buildCategoryLabels({
      plot, labels: catLabels, center: axis.center, width: catSlotW,
      maxLines: CAT_MAX_LINES, axis: 'x', gap: CAT_GAP,
      edgeAnchor: variant !== 'step',
    });
  ...
  if (variant === 'stacked-area') {
    ...
    model.rows.forEach((row, s) => {
      ...
      marks.push(`<path class="line-band" data-hue="${s + 1}" data-anima-role="area"`
        + ` fill="${fills.url(slots[s])}" d="${bandPath(idxs, axis, yUp, yLo, variant)}"/>`);
      // A band's top edge is its series' line: named, as the plain line is, for the Guide.
      const named = model.series[s] ? ` data-label="${escAttr(model.series[s])}"` : '';
      marks.push(`<path class="line-path line-edge" data-hue="${s + 1}" data-series="${s}" data-anima-role="line"${named}`
    ...
    model.rows.forEach((row, s) => {
      ...
      if (filled) {
        segs.forEach((idxs) => {
          ...
          marks.push(`<path class="line-area" data-hue="${s + 1}" data-anima-role="area"`
      ...
      segs.forEach((idxs) => {
        ...
        if (idxs.length < 2) {
          lone.add(idxs[0]);
          const lp = row[idxs[0]];
          dots.push(`<circle class="line-dot line-dot-lone" data-hue="${s + 1}" data-series="${s}" data-encodes="hue" data-paint="fill" data-anima-role="point"`
            + ` data-label="${escAttr(model.cats[idxs[0]])}" data-value="${escAttr(lp.raw != null && lp.raw !== '' ? lp.raw : String(lp.num))}"`
            + ` cx="${num(axis.center(idxs[0]))}" cy="${num(yv(idxs[0]))}" r="${DOT_R}"/>`);
          return;
        }
        // The SERIES NAME on its line, so the Present Guide can point at the line a sentence opens
        // with ("Enterprise fell overall, …"). One segment carries it — the first — because two
        // marks with one label are an ambiguity the Guide refuses, and a gap-split line is rare.
        const named = model.series[s] && !labelled.has(s) ? ` data-label="${escAttr(model.series[s])}"` : '';
        if (named) labelled.add(s);
        marks.push(`<path class="line-path" data-hue="${s + 1}" data-series="${s}" data-anima-role="line"${named}`
          + ` d="${variant === 'step' ? stepPath(idxs, axis, yv) : linePath(idxs, axis.center, yv)}"/>`);
      });
      if (showDots) {
        row.forEach((p, i) => {
          // Not the isolated readings — those already have their own dot, and a
          // second circle at the same coordinates doubles the
          // `[data-anima-role="point"]` nodes the motion layer animates.
          if (!p || lone.has(i)) return;
          // Each point names its CATEGORY and says its VALUE: the narration reads a series point by
          // point ("Q2 2025, four point four."), and the Guide finds the one dot whose category
          // leads that sentence and whose value it says — the value is what tells the three
          // `Q2 2025` dots apart. `data-series` ties the dot to its line, so `_focus: series N`
          // recedes a faded line's dots with it instead of leaving them at full color.
          dots.push(`<circle class="line-dot" data-hue="${s + 1}" data-series="${s}" data-encodes="hue" data-paint="fill" data-anima-role="point"`
            + ` data-label="${escAttr(model.cats[i])}" data-value="${escAttr(p.raw != null && p.raw !== '' ? p.raw : String(p.num))}"`
  ...
  if (nCats && useDirect) {
    ...
    const emit = (a, dy) => (a.name
      ? wrapSvgLabel(a.text, {
        ...
        // `data-series-for` links the name to its line, so it recedes with the line when the Present
        // Guide walks another one. Not `data-series`: that makes an element one of the line's SHAPES.
        attrs: ` data-hue="${a.s + 1}" data-series-for="${a.s}" data-anima-role="label"`,
        emitFontSize: false,
      })
      : wrapSvgLabel(a.text, {
        ...
        attrs: ` data-series-for="${a.s}" data-anima-role="label"`, emitFontSize: false,
      }));
  ...
  // `--line-count` used to ride here and nothing in `lib/`, `docs/src`, `tools`
  // or `test` ever read it. A custom property no rule consumes is a promise the
  // next editor has to disprove before they can delete it.
  // Every reading, as a table a screen reader can walk: the `<desc>` states the movement (start,
  // end, change, peak) and deliberately not each point, so the points between ride here.
  const shown = (p) => (p.raw != null && p.raw !== '' ? p.raw : String(p.num));
  const table = cart.buildSrDataTable({
    caption: `${TITLES[variant]} values`, corner: '', cols: model.flat ? ['Value'] : model.series,
    rows: model.cats.map((c, i) => ({ head: c, cells: model.rows.map((row) => (row[i] ? shown(row[i]) : null)) })),
  });
  return `<div class="line-figure" data-variant="${variant}">${svg}${table}${detailWrap}</div>${note}`;
}
