...
// Choropleth ramp BUCKET (0…K-1) for a value within [min,max]. The ramp is quantized
// (K=16) so each bucket compiles to a flat `--map-ramp-j` literal (no runtime
// color-mix on the old-browser path). The bucket's actual mix % lives in the token
// (map.styles.css, MIX_FLOOR 24% → MIX_CEIL 88%). A flat series pins to the top.
const RAMP_K = 16;
function rampBucket(num, minNum, maxNum) {
  if (maxNum <= minNum) return RAMP_K - 1;
  const t = (num - minNum) / (maxNum - minNum);
  return Math.round(t * (RAMP_K - 1));
}
...
function buildMap(model, variant, classTokens = [], orientation) {
  ...
  const entries = matched.map((r, i) => {
    // `cls` + a region ATTRIBUTE colour the region <path>s (CSS); `cat`/`ramp` key the
    // SVG legend swatch <rect> to the SAME compiled token, so a swatch mirrors its
    // region exactly. No inline color-mix rides a presentation attribute (D2).
    let cls, regionAttr, cat, ramp;
    if (variant === 'highlight') {
      cat = i % CAT_SLOTS; // 0-based (matches svg-legend data-cat + a11y textures); token is 1-based
      cls = 'map-region map-region--hl';
      // The region reads its 82% slot solid via an inline custom prop (style=, a CSS
      // context — var() resolves); the swatch reads the same slot via `data-cat`.
      regionAttr = ` style="--region-solid:var(--chart-cat-${cat + 1}-solid)"`;
    } else {
      ramp = rampBucket(r.num, minNum, maxNum);
      cls = 'map-region map-region--on';
      // The region + swatch carry the SAME `data-ramp` bucket → one CSS rule paints
      // both from `--map-ramp-j` (structural region↔swatch parity).
      regionAttr = ` data-ramp="${ramp}"`;
    }
    ...
    for (const id of r.ids) fillById.set(id, { cls, regionAttr, mark: i, label: r.name, value: r.valueRaw });
    return { ...r, cat, ramp };
  });
  ...
  const paths = Object.entries(basemap.regions).map(([id, r]) => {
    ...
    const regionAttr = fill ? fill.regionAttr : '';
    ...
    const mark = fill
      ...
      : '';
    return `<path class="${cls}"${regionAttr}${mark} d="${r.d}"><title>${esc(r.name)}</title></path>`;
  }).join('');
  ...
  const toRow = (e) => ({
    ...(e.cat != null ? { cat: e.cat } : {}),
    ...(e.ramp != null ? { ramp: e.ramp } : {}),
  ...
  for (const r of unmatched) {
    // The "?" chip paints via `data-unmatched` + a CSS rule (transparent fill, --text-muted
    // stroke) — never an inline `stroke="var()"` presentation attr, which is flaky on old
    // WebKit (D2). See svg-legend.js + the .chart-key-swatch[data-unmatched] rule.
    rows.push({ unmatched: true, label: r.name, value: '?' });
  }
  ...
  const [vbX, vbY, vbW, vbH] = basemap.viewBox.split(/\s+/).map(Number);
  ...
  // The region paths carry their NATIVE projected coordinates, whose origin is
  // (vbX, vbY) — not (0,0). buildSvgLegend lays the diagram box out from 0 (it only
  // gets the width/height), so the group must subtract that native origin or every
  // region lands (vbX, vbY) off inside its slot. The world basemap's origin is
  // (67, 37), so the un-offset group shoved the whole map ~7% right + down — dead
  // space at the top-left, the southern edge clipped, Oceania jammed into the key.
  // The US origin (−64, 8) is small + lands in empty ocean, which is why only the
  // world map read as broken. The keyless path above keeps the full native viewBox,
  // so it never had this gap. See 2026-07-05 world-map alignment fix.
  const gx = +(key.diagramDx - vbX).toFixed(2);
  const gy = +(key.diagramDy - vbY).toFixed(2);
  const svg = `<svg class="map-svg" viewBox="0 0 ${key.viewW} ${key.viewH}" ` +
    ...
    `<defs>${key.defs}</defs>` +
    `<g transform="translate(${gx} ${gy})">${paths}</g>` +
    `${key.body}</svg>`;
  return `<div class="map-figure" data-basemap="${esc(basemap.id)}"${unmatchedAttr}>${svg}${detailWrap}</div>${note}`;
}
