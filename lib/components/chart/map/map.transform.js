 ...
 * marker classes styled in map.styles.css — no hard-coded color.
 *
 ...
 * the color rules), preserving the zero-fetch contract.
 */
...
function parseMap(ulInner, basemap = BASEMAP) {
  ...
  // wins) — the long-standing behavior; groups are always distinct entries.
  const ordered = [];
  for (const r of rows) {
...
function buildMap(model, variant, classTokens = [], orientation, paneView) {
  ...
  const entries = matched.map((r, i) => {
    // `cls`/`style` color the region <path>s (CSS); `keyFill`/`keyStroke` color
    ...
    if (variant === 'highlight') {
      ...
      keyFill = `var(--chart-cat-${slot}-body)`;
    ...
    return { ...r, style, keyFill, keyStroke, mark: i };
  });
  ...
  const paths = Object.entries(basemap.regions).map(([id, r]) => {
    ...
    const mark = fill
      ...
      : '';
    // `data-anima-role`: the region declares its own motion role, like every
    // other chart mark, so the motion system reads it instead of guessing from
    // the class. The attribute doesn't paint — the PDF stays byte-identical.
    return `<path class="${cls}" data-anima-role="region"${style}${mark} d="${r.d}">` +
  ...
  const toRow = (e) => ({
    ...
    // The legend is sorted by value, so row order is NOT mark order; carry the entry's own
    // mark index, which is what its regions' `data-mark` holds.
    markFor: e.mark,
  });
  ...
  for (const r of unmatched) {
    rows.push({ swatchFill: 'transparent', swatchStroke: 'var(--muted-mark)', label: r.name, value: '?' });
  }
  ...
  // In a PANE the key keeps slide-size type and the basemap gives (svg-legend.js fitKeyToPane).
  const { key } = fitKeyToPane(
    (o, fontScale) => buildSvgLegend({ rows, diagramRight: vbW, diagramHeight: vbH, hasValues, orientation: o, fontScale }),
    { orientation, paneView, diagramHeight: vbH });
  ...
  const svg = `<svg class="map-svg" viewBox="0 0 ${key.viewW} ${key.viewH}" ` +
    ...
    `<defs>${key.defs}</defs>` +
    ariaHiddenMarks(
      `<g transform="translate(${gx} ${gy})">${paths}</g>${key.body}`,
    ) + '</svg>';
  return `<div class="map-figure" data-basemap="${esc(basemap.id)}"${unmatchedAttr}>${svg}${detailWrap}</div>${note}`;
}
...
/**
 * The chart-family entrypoint (see the `kernel` block in map.manifest.json).
 */
function transformSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
    const basemap = pickBasemap(ctx.classTokens);
    const model = parseMap(ext.inner, basemap);
    if (!model) return null;
    const variant = pickVariant(ctx.classTokens);
    return buildMap(model, variant, ctx.classTokens, ctx.orientation, ctx.paneView);
  });
}
