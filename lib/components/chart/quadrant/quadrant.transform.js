...
function quadrantTintsSvg(splitX, splitY) {
  ...
  cells.forEach((c, i) => {
    ...
    // The radial stops are the slot's compiled --chart-cat-N-g0/g1/g2 tokens (the same
    // 42/58/82%-into-chart-cat-base recipe the pie wedges use) — flat literals, so no
    // color-mix ships in the inline stop-color (which an old engine drops → black stop).
    const slot = i + 1;
...
function renderCohort(model, scale, orientation) {
  ...
  // SVG-native key — one chip per cohort with its item count. The swatch paints via
  // the `.chart-key-swatch[data-cat]` CSS rule (0-based slot = cellIdx; fill = the
  // slot's 82% solid + its ink edge) — never an inline presentation attribute (D2).
  // The count is the value column. Cohorts without a name are dropped from the key.
