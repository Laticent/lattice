 ...
 *   <!-- _class: quadrant -->
 *
 *   `[{Effort, 0..10}, {Reach, 0..100}]` <- axes: name, (optional) domain, (optional) threshold
 ...
 *
 * One bracketed list above the chart names the axes, x then y, and each member
 * may carry its domain and threshold (see "Axis" below):
 *
 *   [{<x-name>, <min>..<max>?, <threshold>?}, {<y-name>, …}]
 *
 * If a domain is omitted on either axis it auto-fits (nice-ceil) the data.
 *
...
// The kernel just emits the cell index; the cascade picks the color.
...
const {
  wrapSvgLabel, placeLabels, upperAdvance, leaderLine, LEADER_MIN_GAP,
} = require('../_chart-family/svg-label');
const { noteHiddenLabels, noteLabelDrop } = require('../_chart-family/label-drops');
...
// The shared axis grammar (lib/core/bracket-list.js) and the position rule that
// says a bracketed list above the chart body is the axis.
...
//
// RAISED FOR LEGIBILITY (#680, 2026-08-10). The ticket's measured complaint was
// that a point label sets at ~11.8px on a 1920-wide slide (8.5 units against a
// 348-unit box) — about 45% of body size, fine on a laptop and thin on a
// projector. The sizes below are the largest that buy that back WITHOUT changing
// the chart's shape or dropping a name; the geometry is deliberately untouched
// (see the note on `dotLabel`, and `LW.item` below).
//
// `bubbleValue` is the one that did NOT rise: it is painted INSIDE its bubble,
// whose radius is the data, so a bigger numeral would overflow the small end of
// the scale rather than read better.
const FS = {
  label: 12,        // .quadrant-label (corner) — 13 wraps "STRATEGIC BETS" in a 120-unit budget
  labelMagic: 13,   // .quadrant-label--magic
  labelZone: 11.5,  // .quadrant-label--zone
  // 9.5 is a CEILING, and the corpus — not the unit fixtures — is what sets it.
  // An earlier pass stopped at 10.5 because both dense fixtures in
  // quadrant.test.js still placed every name there. They are 5- and 6-point
  // clusters; the gallery's OWN "Stress test — fourteen initiatives … the
  // ceiling" slide is denser than either, and at 10.5 it silently drops
  // "Per-decision profiles" (14 labels in, 13 out) — a name deleted from a
  // committed artifact by a change whose whole point is legibility. Swept over
  // every quadrant slide in the repo (88 items), labels emitted / wrapped:
  //
  //     8.5 / 104  (was)   88 of 88    28 wrapped
  //     9.5 / 120          88 of 88    17 wrapped   <- ships
  //     10   / 120         83 of 88    23 wrapped
  //     10.5 / 120         83 of 88    39 wrapped
  //
  // 9.5/120 dominates BOTH the old constants and the 10.5 attempt: bigger type,
  // no name lost, and fewer wrapped lines than the code it replaces. Validate a
  // change here against the corpus, not against the two fixtures.
  dotLabel: 9.5,    // .quadrant-dot-label / .quadrant-bubble-label
  bubbleValue: 8.5, // .quadrant-bubble-value — inside the bubble; see above
  cohort: 12,       // .quadrant-cohort-label
  axis: 14,         // --quadrant-axis-size default
  tick: 12,         // --quadrant-tick-size default
  badge: 10,        // .quadrant-target-badge
};
...
const LW = {
  // 140, not 120, because `label` rose to 12: at a 120-unit budget a 12-unit
  // uppercase name gets ~14.7 characters a line, and a realistic two-word group
  // ("Quick Operational Wins") lost its last word to an ellipsis at maxLines 2.
  // 140 restores ~17 characters and still fits inside the 168-unit column, so
  // the two top names keep a 28-unit gap between their painted boxes.
  corner: 140,   // a quadrant name, centered on its column outside the plot
  // charBudget() is floor(width / (fontSize * 0.6)), so this is 21 chars a line
  // at 9.5 against the old 20 at 104/8.5 — names wrap slightly LESS than they
  // used to, which the corpus sweep above confirms (17 wrapped vs 28).
  item: 120,     // a dot / bubble name
...
/**
 * The rungs a dot label may be sized at, largest first (#1605).
 *
 * THE TOP IS A TARGET, NOT A CEILING WE FOUND. 13.5 is the house's smallest
 * type tier (`--fs-meta`) expressed in this viewBox's units, after #1598 gave
 * the stage the outer inset and the chart its box back. Measured per slide,
 * plenty of sparse quadrants place every name at 18 or 20 — and they are not
 * allowed to, because a dot name that outgrows `--quadrant-axis-size` (14)
 * inverts the chart's hierarchy: the thing being measured would print larger
 * than the axis it is measured against. Reaching the legibility bar is the
 * goal; passing it is a different defect.
 *
 * The bottom rung is the constant this used to be, so a slide the ladder
 * cannot lift renders byte-identically to before.
 *
 * Coarse on purpose. Each rung costs a placement pass, the gain from 12 to
 * 12.25 is invisible on a projector, and a finer ladder buys resolution the
 * reader cannot see with time the renderer can feel.
 */
const DOT_LABEL_LADDER = [13.5, 13, 12.5, 12, 11.5, 11, 10.5, 10, 9.5];
...
// at the bottom-center and left-center even at that compact size.
const GEOM = {
  viewBox: '0 0 420 348',
  vbW: 420, vbH: 348,
...
// honor the group structure (cohort still uses groups; bubble/threshold/
...
// ── Axis: ONE bracketed list above the plot ────────────────────────────────
//
//   `[Effort, Reach]`                          names only
//   `[{Effort, 0..10}, {Reach, 0..100}]`       name + domain
//   `[{Effort, 0..10, 5}, {Reach, 0..100, 50}]` name + domain + threshold
//
// x first, then y. POSITION IS THE AUTHORITY (lib/core/lift-bracket-span.js):
// the list is read only when it sits above the chart's list, and a paragraph
// there that is not a bracketed list is the ordinary eyebrow, left alone.
//
// WHAT THIS RETIRES. The eyebrow used to be `Effort 0–10 → Reach 0–100 ·
// targets 5, 50` — an arrow glyph as the axis delimiter, an en-dash as the range
// delimiter, and a detached `targets` blob a reader matched to its axis by
// counting. The arrow needed a HARD RULE #29 carve-out to pass the typed-glyph
// gate, and its documented ASCII `->` spelling never fired, because markdown-it
// escapes it to `-&gt;` before this code sees it. The eyebrow ALSO printed on
// the slide while the same names were painted on the axes, so every quadrant
// said its axes twice. The list is consumed, like scatter's and matrix-grid's.
// Why: engineering/decisions/2026-09-22-chart-axis-grammar.md.
//
// Numbers are derived and words are not: an omitted domain nice-ceils from the
// data, an omitted threshold takes the midpoint, and an omitted name leaves the
// axis unnamed (`magic` alone has canonical names to fall back on).
...
/**
 * The axis list's text -> `{ xName, yName, xRange, yRange, xThreshold, yThreshold }`.
 * Not a bracketed list (or empty) reads as "nothing authored": every field
 * blank, so the whole axis derives from the data.
 *
 * `maxParts: 3` because a member is name, domain, threshold; past that a comma
 * is text, and the stray part is ignored (lib/core/axis-member.js says which
 * part is which, for this and for `lint:deck`).
 */
function parseAxisList(text) {
  const members = text ? parseBracketList(text, { maxParts: 3 }) : null;
  const x = readAxisMember(members?.[0]);
  const y = readAxisMember(members?.[1]);
  return {
    xName: x.name,
    yName: y.name,
    xRange: x.range,
    yRange: y.range,
    xThreshold: x.threshold,
    yThreshold: y.threshold,
  };
}

/**
 * Lift the axis list out of the section, by position.
 *
 * THE DECLARATION IS REQUIRED, NOT PREFERRED — same guard, same reason, as
 * scatter's `readAxisTitles`: read through `?.`, a missing catalog entry turns
 * `keyBelow` into `undefined !== false`, re-enables the below slot, and eats
 * the trailing coda this component's manifest claims.
 */
function readAxis(html) {
  const decl = AXIS_CATALOG.quadrant;
  if (!decl) throw new Error('quadrant: no axisSet entry — run build-stage-catalog.');
  const lifted = liftBracketSpans(html, {
    below: decl.keyBelow !== false, bodyTags: decl.bodyTags, acceptAbove: axisAcceptor(decl, { maxParts: 3 }),
  });
  return { text: lifted.above || '', html: lifted.html };
}

// Build per-axis scale objects. Honors authored domains, falls back to
// nice-ceil of the data. Trail items contribute both endpoints. A threshold on
// EITHER axis turns targets on, and the other axis's line sits at its midpoint.
function resolveScale(model, axisText) {
  const eb = parseAxisList(axisText);
  ...
  const hasTarget = eb.xThreshold !== null || eb.yThreshold !== null;
  return {
    x: { ...xScale, label: eb.xName },
    y: { ...yScale, label: eb.yName },
    targets: hasTarget
      ? {
          x: eb.xThreshold ?? (xScale.min + xScale.max) / 2,
          y: eb.yThreshold ?? (yScale.min + yScale.max) / 2,
        }
      : null,
  };
}
...
// Per-region radial depth gradient, all four sharing one focal center at the
...
function quadrantTintsSvg(splitX, splitY, names = []) {
  const { plot } = GEOM;
  const cells = [
  ...
  cells.forEach((c, i) => {
    ...
    const gradId = `${renderIdPrefix()}q-tint-${nextRenderSeq('q-tint')}`;
    ...
    defs.push(`<radialGradient id="${gradId}" gradientUnits="userSpaceOnUse" ` +
      ...
      `</radialGradient>`);
    // THE REGION'S NAME, for the Present Guide. `narrateQuadrant` reads a quadrant at a
    // time — "Quick Wins: Weekly signal digest at two, eighty-two; …" — so the region
    // that sentence names is what the pointer should land on, not the whole plot.
    const name = names[i] ? plainText(names[i]) : '';
    const ident = name ? ` data-label="${escAttr(name)}"` : '';
    out += `<rect class="quadrant-tint" data-cell="${i}"${ident} style="fill:url(#${gradId})" ` +
...
function cornerSpec(i, extraClass, splitX, hand) {
  ...
  const fontSize = extraClass === 'quadrant-label--magic' ? FS.labelMagic
    : extraClass === 'quadrant-label--zone' ? FS.labelZone
      : FS.label;
  // Each variant TRACKS differently, and tracking is additive per character —
  // read it off the rule this label will actually match, not off the base one.
  const tracking = extraClass === 'quadrant-label--magic' ? 0.08
    : extraClass === 'quadrant-label--zone' ? 0.06
      : 0.04;
  return {
    x: p.x, y: p.y, width: p.width, fontSize,
    baseline: p.baseline,
    // .quadrant-label is `text-transform: uppercase` + `letter-spacing`, so it
    // paints wider than the mixed-case source string implies — and by how much
    // depends on the STRING (`WORKFLOW` paints over twice `IL ILI` per
    // character) and on the FACE (`sketch` re-points --font-body at the hand
    // sans). No constant can say that for author text, so the estimate is
    // summed from the glyph table. See svg-label.js §"The UPPERCASE label
    // estimate", and #1672.
    advance: (s) => upperAdvance(s, { hand, tracking }),
...
function quadrantLabelsSvg(names, extraClass, splitX, hand) {
  let out = '<g class="quadrant-labels">';
  names.forEach((name, i) => {
    if (!name) return;
    out += wrapSvgLabel(name, cornerSpec(i, extraClass, splitX, hand)).svg;
  });
  out += '</g>';
  return out;
}
...
function cornerObstacles(names, extraClass, splitX, hand) {
  if (!Array.isArray(names)) return [];
  const out = [];
  names.forEach((name, i) => {
    if (!name) return;
    const m = wrapSvgLabel(name, cornerSpec(i, extraClass, splitX, hand));
 ...
 * slide has one direction, so a crowded label traveled a long way from its dot
...
function itemLabelSet(obstacles) {
  ...
  return {
    ...
    emit() {
      ...
      // PER-SLIDE SIZING (#1605). The size below used to be one constant shared
      // by every quadrant in every deck, and the number had to be the largest
      // the MOST CROWDED slide in the corpus could carry without losing a name
      // — so a four-dot slide with a half-empty plot printed at the size a
      // fourteen-dot slide needed, in a different file, written by someone
      // else. Measured across the 27 quadrant slides on main: 25 of them were
      // running below what their own geometry allows, and none reached the
      // house's smallest type tier.
      //
      // So ask the geometry instead of a constant: take the largest rung at
      // which EVERY name still places, and fall back down the ladder from
      // there. The floor is the old constant, so a slide that can carry
      // nothing more renders exactly as it did.
      //
      // The search re-runs placement, which is why the ladder is coarse and
      // ordered large-first: a sparse slide settles on the first attempt, and
      // only a crowded one pays for more. `placeLabels` is pure and
      // deterministic, so this is a measurement, not a heuristic.
      // THE BOX SCALES WITH THE TYPE, and leaving it fixed was a real defect
      // rather than a detail. `LW.item` is a width in viewBox units, tuned so a
      // name wraps to a couple of lines instead of reading as a caption — but
      // it buys a number of CHARACTERS only in combination with a font size.
      // Held at 120 while the type grew from 9.5 to 13.5, a line went from
      // about 21 characters to about 15, so names started wrapping to three
      // lines and breaking mid-word: the gallery's "Maturity self-assessment"
      // rendered as "self-assessmen / t". Scaling the budget by the same factor
      // keeps characters-per-line constant, so a label is simply a larger copy
      // of the label it was — which is the whole intent — and the extra width
      // it claims is paid for honestly, by the search settling a rung lower.
      const floor = DOT_LABEL_LADDER[DOT_LABEL_LADDER.length - 1];
      const specFor = (it, rung) => ({
        ...it.spec, fontSize: rung, width: (it.spec?.width ?? LW.item) * (rung / floor),
      });
      // "EVERY NAME PLACES" IS NOT ENOUGH, and the gallery's own headline slide
      // is what proved it. The first cut of this search accepted the largest
      // rung at which nothing was dropped, and on that slide it took 13.5 —
      // where SEVEN of the eight names had been pushed off their dots, the
      // worst by 49 viewBox units (about 15% of the plot width), stacking three
      // of them into a column that reads as a list rather than as annotations.
      // At the old constant, two names travelled and the worst gap was 7 units.
      // Every name was present at both sizes, so the presence test saw nothing
      // wrong; the render did. A quadrant says WHICH ITEM IS WHERE, so a name
      // that has left its dot has given up the thing the chart is for — and
      // "Custom calibration", parked on the STRATEGIC BETS fill while its dot
      // sat in TIME SINKS, reads as the wrong answer even with a leader drawn.
      //
      // So the rung has to buy its type without spending attribution. The test
      // is RELATIVE TO THE FLOOR, not an absolute distance: a crowded slide may
      // genuinely need its names to travel — that is what the leader lines are
      // for, and the fourteen-initiative stress slide has always been that
      // slide — and an absolute cap would punish it for a fact about its data.
      // What must not happen is a rung making a slide WORSE than the size it
      // already rendered at. Same shape as HARD RULE #18: leave it no worse.
      // "TRAVELLED" IS MEASURED AT THE RUNG'S OWN SCALE, for exactly the reason
      // `LEADER_MIN_GAP` is scaled below: whether a name still reads as sitting
      // ON its dot is a typographic distance, not an absolute one. A 13.5 label
      // is a bigger box anchored the same way, so its near edge sits further
      // out at rest — charged against the floor's fixed 5 units, ordinary
      // growth counts as flight and the guard rejects every rung on every
      // slide. Measured: against a fixed 5, the gallery's default slide scored
      // 7 travelled at 13.5 and 2 at 9.5, and the ladder collapsed the whole
      // corpus back toward the floor; scaled, the same slide scores what it
      // actually looks like. This is the same defect the width budget had.
      const travelOf = (attempt, rung) => {
        let shown = 0;
        let count = 0;
        let worst = 0;
        attempt.forEach((r, i) => {
          if (!r || r.hidden || !r.box) return;
          shown++;
          const b = r.box;
          const it = items[i];
          const dx = Math.max(b.left - it.cx, it.cx - b.right, 0);
          const dy = Math.max(b.top - it.cy, it.cy - b.bottom, 0);
          // NORMALIZED TO FLOOR UNITS — divide the measured gap by the rung's
          // scale — so a distance at 13.5 is directly comparable to one at 9.5
          // and both can be read against the same `LEADER_MIN_GAP`.
          const gap = (Math.hypot(dx, dy) - (it.r || 0)) * (floor / rung);
          if (gap > LEADER_MIN_GAP) { count++; if (gap > worst) worst = gap; }
        });
        // `shown` first: it is the primary key of the search below, and the one
        // thing `count`/`worst` cannot speak to, since both are measured only
        // over the labels that placed.
        return { shown, count, worst };
      };
      const base = items.map((it) => it.spec);
      const at = (rung) => {
        items.forEach((it, i) => { it.spec = specFor({ spec: base[i] }, rung); });
        return placeLabels(items, opts);
      };
      // The floor's own profile is the budget every taller rung is held to. It
      // costs one extra placement pass on a slide that would have settled at
      // the top rung immediately — the price of knowing what the slide used to
      // look like, which is the only way to tell a gain from a regression.
      // PRECEDENCE, AND IT IS THE WHOLE POINT: how many names get PAINTED beats
      // how well they are attributed, every time. The first cut of this guard
      // had it the other way round and the inversion was invisible in review.
      //
      // `travelOf` can only measure labels that placed, so when the floor hides
      // a name its budget is taken over the survivors. A taller rung that
      // succeeds in placing that name then has one MORE travelling label than
      // the budget, trips the count test, and is refused — so the slide keeps
      // the floor, and the floor is the size that drops the name. The guard
      // written to stop a chart misattributing a name was choosing to DELETE
      // one instead, which is worse on the same axis and contradicts the census
      // this branch exists around. Measured over 600 generated quadrants: 18 of
      // them painted fewer names than before the guard existed.
      //
      // So `shown` is the primary key and attribution is only ever a TIE-BREAK
      // between rungs that put the same number of names on the slide. A rung
      // that paints more than the floor is taken on that basis alone; a rung
      // that paints the same must also be no worse attributed; a rung that
      // paints fewer is never taken. Ties among equal `shown` go to the larger
      // type, which is what the ladder is for.
      const floorAttempt = at(floor);
      const budget = travelOf(floorAttempt, floor);
      let placed = floorAttempt;
      let size = floor;
      let shown = budget.shown;
      for (const rung of DOT_LABEL_LADDER) {
        if (rung === floor) break; // already measured, and it is the incumbent
        const attempt = at(rung);
        const t = travelOf(attempt, rung);
        // A HAIR OF SLACK ON THE DISTANCE, none on the count. A taller label is
        // a larger box, so its near edge sits a little further out at the same
        // anchor; charging that as travel would reject every rung on every
        // slide. The count is what says a name LEFT its dot, so it is exact.
        // Applied ONLY against a rung showing exactly what the floor showed —
        // comparing travel across two different sets of names is meaningless.
        if (t.shown === budget.shown
          && (t.count > budget.count || t.worst > budget.worst + LEADER_MIN_GAP)) continue;
        if (t.shown < shown) continue;
        if (t.shown === shown && rung < size) continue;
        placed = attempt;
        size = rung;
        shown = t.shown;
        // Nothing can beat every name placed, and the ladder runs large-first,
        // so this is already the largest rung that manages it.
        if (shown === items.length) break;
      }
      items.forEach((it, i) => { it.spec = base[i]; });
      // REPORT the names the winning rung could not paint (#2171). Taken here and
      // not inside `at()`: the ladder runs up to nine speculative passes and keeps
      // one, so noting inside the search would report every rejected rung's
      // casualties as if they had shipped. `placed` is the array this slide emits.
      noteHiddenLabels(placed);
      // A LEADER FOR EVERY NAME THAT HAD TO TRAVEL — the channel this chart has
      // never had. `placeLabels` keeps a name beside its own dot, but "beside"
      // is not "nearest": on the fourteen-initiative stress slide SIX of the
      // fourteen names sit closer to another initiative's dot than to their own,
      // and a quadrant carries no other way to say which is which — no leaders,
      // no value pills, no axis to read a position off. The hairline is the
      // family's own (svg-label.js § leaderLine, shared with `scatter`), so the
      // two charts answer the same problem with the same mark.
      //
      // Drawn only where a label actually travelled, so an ordinary six-item
      // quadrant renders exactly as it did before — verified pixel-identical.
      // Leaders come first in document order: the dots are already painted, and
      // a line that stops a hair outside the dot must never sit over the name.
      // What the corridor must not cross: every OTHER name's box, every other
      // dot or bubble, the marks that carry no label of their own, and the
      // corner names. A hairline through a neighbor's letterforms points the
      // reader at the wrong initiative — the defect this whole pass removes.
      const markBox = (m) => ({ left: m.cx - m.r, right: m.cx + m.r, top: m.cy - m.r, bottom: m.cy + m.r });
      const blockers = obstacles.concat(extraMarks).concat(items.map(markBox));
      // THE "IS IT ATTACHED?" THRESHOLD SCALES WITH THE TYPE, for the same
      // reason the width budget does. `LEADER_MIN_GAP` is 5 viewBox units, and
      // it was tuned when every dot label was 9.5 — it asks whether a name
      // still reads as sitting ON its dot, which is a typographic distance, not
      // an absolute one. Left fixed, a 13.5 label sits naturally further out
      // and trips the threshold on plots that need no leader at all: the
      // four-point spread fixture drew a 3-unit hairline between a dot and the
      // name already touching it, which is precisely the "line nobody needs"
      // its own arm is named for. Scaling by the same factor keeps the ratio
      // the 5 was chosen at. `scatter` passes nothing and is unaffected.
      const leaders = placed.map((res, i) => leaderLine(items[i], res, {
        minGap: LEADER_MIN_GAP * (size / floor),
        avoid: placed.filter((q, j) => j !== i && q.svg).map((q) => q.box)
          .concat(blockers.filter((_, j) => j !== obstacles.length + extraMarks.length + i)),
      })).join('');
      // The chosen rung reaches the paint through the same token the axis and
      // tick sizes already use, set on a group that wraps only these labels.
      // CSS owns what is painted (see the FS table's note); this tells it WHICH
      // size to paint, per slide, without the kernel emitting a font-size onto
      // every single <text> and without a theme losing its ability to retune
      // the token. The floor stays declared in the stylesheet, so a render that
      // never reaches this code still paints at a size the geometry holds.
      const body = leaders + placed.map((r) => r.svg).join('');
      return `<g class="quadrant-dot-labels" style="--quadrant-dot-size:${size}px">${body}</g>`;
    },
  };
}
...
function dotWithLabelSvg(p, label, cellIdx, opts) {
  ...
  const spec = label ? {
    ...
    // A tap proxy: the dot's name opens the dot's card (`data-mark-for`, read by the reveal layer
    // and the Present Guide — see slope.transform.js `proxyAttrs` for why it is not `data-mark`).
    attrs: ` data-cell="${cellIdx}"${opts && opts.mark != null ? ` data-mark-for="${opts.mark}"` : ''}`,
    emitFontSize: false,
  } : null;
...
function openSvg(extraClass, hasNames = true, title = 'Quadrant chart', desc = '') {
  return `<svg class="quadrant-svg${extraClass ? ' ' + extraClass : ''}"` +
    `${hasNames ? '' : ' data-band="none"'} ` +
    `viewBox="0 0 ${GEOM.vbW} ${vbHeight(hasNames)}" role="img"><title>${escHtml(title)}</title>${desc}`
    // Opens the marks group; every caller closes it with `</g></svg>`. See
    // cartesian.js § ariaHiddenMarks for why `role="img"` does not do this.
    + '<g aria-hidden="true">';
}

/**
 * The `<desc>` — the ONLY route to this chart's data.
 *
 * The marks ride in an `aria-hidden` group (cartesian.js § ariaHiddenMarks), so the ~18
 * `<text>` nodes a sighted reader gets (axis names, the four quadrant names, every
 * item and its position) are unreachable. A title alone would announce "Quadrant
 * chart, image" over an empty region — which is the same defect the ADR (§17.5)
 * rejects `<figure>`-over-a-hidden-graphic for. The chart names what it is AND says
 * what is in it, or it is still a locked box.
 *
 * Positions are stated in the axes' own units, which is what the dots encode.
 */
function quadrantDesc(model, scale, names, groupWord = 'Quadrants') {
  const parts = [];
  const xl = scale?.x?.label || 'x';
  const yl = scale?.y?.label || 'y';
  parts.push(`Axes — ${xl} horizontal, ${yl} vertical`);
  const named = (names || []).filter(Boolean);
  if (named.length) parts.push(`${groupWord} — ${named.map((n) => stripTags(String(n))).join(', ')}`);
  const rows = [];
  for (const g of (model.groups || [])) {
    for (const it of (g.items || [])) {
      if (!it || it.label == null) continue;
      const pos = (it.x != null && it.y != null) ? ` at ${xl} ${fmtNum(it.x)}, ${yl} ${fmtNum(it.y)}` : '';
      rows.push(`${stripTags(String(it.label))}${pos}${g.name ? ` (${stripTags(String(g.name))})` : ''}`);
    }
  }
  if (rows.length) parts.push(`Items — ${rows.join('; ')}`);
...
function renderStandard(model, scale, variant, hand) {
  ...
  // THE ALL-OR-NOTHING DROP, and the one the diagnostic channel most needs to
  // cover (#2171). Past the cliff no name is even offered to `placeLabels`, so
  // there is no `hidden` entry for `noteHiddenLabels` to find — and the author who
  // loses EVERY name was the only one getting no warning, while the author who
  // lost three got a precise one. Measured on one deck shape at the boundary: 16
  // items paints 13 and names the 3 it dropped; 17 painted 0 and said nothing.
  //
  // Reported here rather than at the `dotWithLabelSvg('')` call below, because a
  // blanked label is indistinguishable from `line`'s deliberate tick thinning by
  // the time it reaches the emitter — this frame is the last one that still knows
  // the difference between "no name authored" and "every name suppressed".
  if (!labelEveryDot) {
    for (const it of items) noteLabelDrop(it.label, 'density');
  }
  ...
  const labels = itemLabelSet(cornerObstacles(names, labelClass, splitX, hand));
  ...
  const svg = openSvg(isMagic ? 'quadrant-svg--magic' : '', hasNames,
    isMagic ? 'Magic-quadrant chart' : 'Quadrant chart', quadrantDesc(model, labelScale, names)) +
    quadrantTintsSvg(splitX, splitY, names) +
    plotFrameSvg(splitX, splitY, 'centerline') +
    axisLabelsSvg(labelScale, hasNames) +
    quadrantLabelsSvg(names, labelClass, splitX, hand) +
    plot +
  '</g></svg>';
...
function renderBubble(model, scale, hand) {
  const splitX = (GEOM.plot.x0 + GEOM.plot.x1) / 2;
  const splitY = (GEOM.plot.y0 + GEOM.plot.y1) / 2;
  ...
  const labels = itemLabelSet(cornerObstacles(names, '', splitX, hand));
  let plot = '<g class="quadrant-plot">';
  model.groups.forEach((g, gi) => {
    const cellIdx = gi % 4;
    for (const it of g.items) {
      ...
      if (it.sizePill) {
        ...
        // a magnitude reads as one number labeling the other.
        const v = wrapSvgLabel(it.sizePill, {
          ...
          attrs: ` data-pos="${inside ? 'inside' : 'above'}" data-cell="${cellIdx}"${it.mark != null ? ` data-mark-for="${it.mark}"` : ''}`,
          emitFontSize: false,
        });
        plot += v.svg;
      ...
      if (it.label) {
        labels.add(it.label, { cx: p.x, cy: p.y, r }, {
          ...
          attrs: ` data-cell="${cellIdx}"${it.mark != null ? ` data-mark-for="${it.mark}"` : ''}`,
          emitFontSize: false,
        });
      } else {
  ...
  // are on top — the same empty-gap defect the cohort variant had, one row over.
  const hasNames = Boolean(names[2] || names[3]);
  const svg = openSvg('quadrant-svg--bubble', hasNames, 'Quadrant bubble chart', quadrantDesc(model, scale, names)) +
    quadrantTintsSvg(splitX, splitY, names) +
    plotFrameSvg(splitX, splitY, 'centerline') +
    axisLabelsSvg(scale, hasNames) +
    quadrantLabelsSvg(names, '', splitX, hand) +
    plot +
  '</g></svg>';
...
function renderTrail(model, scale, hand) {
  const splitX = (GEOM.plot.x0 + GEOM.plot.x1) / 2;
  const splitY = (GEOM.plot.y0 + GEOM.plot.y1) / 2;
  const names = [0, 1, 2, 3].map(i => (model.groups[i] ? model.groups[i].name : ''));
  ...
  // are on top — the same empty-gap defect the cohort variant had, one row over.
  const hasNames = Boolean(names[2] || names[3]);
  const svg = openSvg('quadrant-svg--trail', hasNames, 'Quadrant trail chart', quadrantDesc(model, scale, names)) +
    quadrantTintsSvg(splitX, splitY, names) +
    plotFrameSvg(splitX, splitY, 'centerline') +
    axisLabelsSvg(scale, hasNames) +
    quadrantLabelsSvg(names, '', splitX, hand) +
...
function renderCohort(model, scale, orientation, hand, paneView) {
  const splitX = (GEOM.plot.x0 + GEOM.plot.x1) / 2;
  const splitY = (GEOM.plot.y0 + GEOM.plot.y1) / 2;
  ...
  for (const h of hulls) {
    ...
    labels += wrapSvgLabel(h.name, {
      ...
      // uppercase + 0.04em tracked in CSS, per-string and per-face (#1672)
      advance: (s) => upperAdvance(s, { hand, tracking: 0.04 }),
      anchor: 'middle', vAlign: 'middle', maxLines: 2,
      className: 'quadrant-cohort-label',
      baseline: 'middle',
      // The cohort's name, at its cluster's centroid: `narrateQuadrant` reads a cohort at
      // a time ("Quick Wins: …"), and this label is where that cluster is.
      attrs: ` data-cell="${h.cellIdx}" data-label="${escAttr(plainText(h.name))}"`,
      emitFontSize: false,
    }).svg;
  }
  labels += '</g>';
  ...
  const rows = hulls.filter(h => h.name).map(h => ({
    swatchFill: `var(--chart-cat-${h.cellIdx + 1}-body)`,
  ...
  const bodyInner =
    ...
    plot +
    labels;
  // In a PANE the key keeps slide-size type and the plane gives (svg-legend.js fitKeyToPane).
  const { key } = fitKeyToPane(
    (o, fontScale) => buildSvgLegend({ rows, diagramRight: GEOM.vbW, diagramHeight: vbHeight(false), hasValues: true, orientation: o, fontScale }),
    { orientation, paneView, diagramHeight: vbHeight(false) });
  const svg = `<svg class="quadrant-svg quadrant-svg--cohort" ` +
    // The same description every other variant carries — axes, groups, and every item at
    // its position — with the groups named as the cohorts they are. It used to be the key's
    // own `<desc>` ("Key — Strategic Bets 2, …"): the cohort sizes, and not one item.
    `viewBox="0 0 ${key.viewW} ${key.viewH}" role="img"><title>Quadrant cohort chart</title>` +
    `${quadrantDesc(model, scale, model.groups.map((g) => g.name), 'Cohorts') || key.desc}` +
    `<defs>${key.defs}</defs>` +
    ariaHiddenMarks(
      `<g transform="translate(${key.diagramDx} ${key.diagramDy})">${bodyInner}</g>${key.body}`,
    ) + '</svg>';
...
function renderThreshold(model, scale, hand) {
  ...
  const labels = itemLabelSet(cornerObstacles(names, 'quadrant-label--zone', splitX, hand));
  let plot = '<g class="quadrant-plot">';
  model.groups.forEach((g, gi) => {
    const cellIdx = gi % 4;
    for (const it of g.items) {
  ...
  // are on top — the same empty-gap defect the cohort variant had, one row over.
  const hasNames = Boolean(names[2] || names[3]);
  const svg = openSvg('quadrant-svg--threshold', hasNames, 'Quadrant threshold chart', quadrantDesc(model, scale, names)) +
    quadrantTintsSvg(splitX, splitY, names) +
    plotFrameSvg(splitX, splitY, 'target') +
    axisLabelsSvg(scale, hasNames) +
    quadrantLabelsSvg(names, 'quadrant-label--zone', splitX, hand) +
    badges +
    plot +
  '</g></svg>';
...
function buildQuadrant(model, variant, scale, orientation, hand = false, paneView) {
  switch (variant) {
    case 'bubble':     return renderBubble(model, scale, hand);
    case 'trail':      return renderTrail(model, scale, hand);
    case 'cohort':     return renderCohort(model, scale, orientation, hand, paneView);
    case 'threshold':  return renderThreshold(model, scale, hand);
    case 'magic':      return renderStandard(model, scale, 'magic', hand);
    default:           return renderStandard(model, scale, 'default', hand);
  }
}

/**
 * The chart-family entrypoint (see the `kernel` block in quadrant.manifest.json).
 */
function transformSection(html, ctx) {
  // Lift first, so the list is off the slide before the figure is spliced in;
  // an unmatched section returns the ORIGINAL html, lift and all untouched.
  const axis = readAxis(html);
  const out = spliceFirstList(axis.html, (ext) => {
    const variant = pickVariant(ctx.classTokens);
    const model = parseQuadrant(ext.inner);
    if (!model) return null;
    const scale = resolveScale(model, axis.text);
    // `.quadrant-label` / `.quadrant-cohort-label` are `--font-body` too — see
    // the note on readsHandBody.
    return buildQuadrant(model, variant, scale, ctx.orientation,
      readsHandBody(ctx.classTokens), ctx.paneView);
  });
  return out === axis.html && axis.html !== html ? html : out;
}
...
module.exports = {
  ...
  cornerBudget,
  vbHeight,
  // Exported alongside GEOM so tests assert against the real budgets rather
  // than literals that drift the moment a font size moves (#680).
  LW,
  MAGIC_DEFAULT_NAMES,
  MAGIC_DEFAULT_AXES,
