...
const LW = {
  ...
  axis: 300,     // the axis title, centered under the full plot
};
...
const {
  escHtml, escAttr, stripTags, fmtNum, findOuterUL, splitTopLevelLI,
...
function cornerSpec(i, name, extraClass) {
  ...
  return {
    ...
    attrs: ` data-cell="${i}" dominant-baseline="${p.baseline}"`,
    emitFontSize: false,
  };
}
...
function quadrantLabelsSvg(names, extraClass) {
  ...
  names.forEach((name, i) => {
    if (!name) return;
    out += wrapSvgLabel(name, cornerSpec(i, name, extraClass)).svg;
  });
  out += '</g>';
  return out;
}
...
function cornerObstacles(names, extraClass) {
  ...
  names.forEach((name, i) => {
    if (!name) return;
    const r = wrapSvgLabel(name, cornerSpec(i, name, extraClass));
...
function itemLabelSet(obstacles) {
  ...
  return {
    ...
    emit() {
      ...
      const boxes = obstacles.concat(first.map((r, i) => ({
        left: r.left, right: r.right, top: r.top, bottom: r.bottom, dir: specs[i].spec.pushDir,
      })));
      const shifts = deCollideLabels(boxes).slice(obstacles.length);
...
function axisLabelsSvg(scale) {
  ...
  if (scale.x.label) {
    out += wrapSvgLabel(scale.x.label, {
      ...
      attrs: ' dominant-baseline="auto"', emitFontSize: false,
    }).svg;
  }
  if (scale.y.label) {
    out += wrapSvgLabel(scale.y.label, {
      ...
      attrs: ` transform="rotate(-90 18 ${yMid.toFixed(2)})" dominant-baseline="auto"`,
      emitFontSize: false,
    }).svg;
  }
  ...
  const tick = (x, y, anchor, baseline, text) => wrapSvgLabel(text, {
    ...
    className: 'quadrant-tick', attrs: ` dominant-baseline="${baseline}"`, emitFontSize: false,
  }).svg;
...
function dotWithLabelSvg(p, label, cellIdx, opts) {
  ...
  const spec = label ? {
    ...
    attrs: ` data-cell="${cellIdx}" dominant-baseline="${baseline}"`,
    emitFontSize: false,
  } : null;
    ...
    for (const it of g.items) {
      ...
      if (it.sizePill) {
        ...
        plot += wrapSvgLabel(it.sizePill, {
          ...
          attrs: ` data-pos="${inside ? 'inside' : 'above'}" data-cell="${cellIdx}" dominant-baseline="${valBaseline}"`,
          emitFontSize: false,
        }).svg;
      }
...
function renderCohort(model, scale, orientation) {
  ...
  for (const h of hulls) {
    ...
    labels += wrapSvgLabel(h.name, {
      ...
      attrs: ` data-cell="${h.cellIdx}" dominant-baseline="middle"`,
      emitFontSize: false,
    }).svg;
  }
  labels += '</g>';
...
function renderThreshold(model, scale) {
  ...
  badges += wrapSvgLabel(fmtNum(tx), {
    ...
    attrs: ' dominant-baseline="hanging"', emitFontSize: false,
  }).svg;
  badges += wrapSvgLabel(fmtNum(ty), {
    ...
    attrs: ' dominant-baseline="middle"', emitFontSize: false,
  }).svg;
  badges += '</g>';
