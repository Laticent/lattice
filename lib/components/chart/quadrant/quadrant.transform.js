...
function dotWithLabelSvg(p, label, cellIdx, opts) {
  ...
  const spec = label ? {
    ...
    // A tap proxy: the dot's name opens the dot's card (`data-mark-for`, read only by the reveal
    // layer — see slope.transform.js `proxyAttrs` for why it is not `data-mark`).
