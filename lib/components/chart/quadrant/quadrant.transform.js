...
function cornerSpec(i, extraClass, splitX, name, hand) {
  ...
  return {
    ...
    // estimate", and #1672.
    advance: upperAdvance(name, { hand, tracking }),
...
function quadrantLabelsSvg(names, extraClass, splitX, hand) {
  ...
  names.forEach((name, i) => {
    if (!name) return;
    out += wrapSvgLabel(name, cornerSpec(i, extraClass, splitX, name, hand)).svg;
  });
  out += '</g>';
  return out;
}
...
function cornerObstacles(names, extraClass, splitX, hand) {
  ...
  names.forEach((name, i) => {
    if (!name) return;
    const m = wrapSvgLabel(name, cornerSpec(i, extraClass, splitX, name, hand));
...
function renderCohort(model, scale, orientation, hand) {
  ...
  for (const h of hulls) {
    ...
    labels += wrapSvgLabel(h.name, {
      ...
      advance: upperAdvance(h.name, { hand, tracking: 0.04 }),
