  ...
  return {
    ...
    emit() {
      ...
      for (const rung of DOT_LABEL_LADDER) {
        ...
        if (attempt.some((r) => r.hidden)) continue;
        ...
        if (t.count > budget.count || t.worst > budget.worst + LEADER_MIN_GAP) continue;
        placed = attempt;
        size = rung;
        break;
      }
      if (!placed && !floorAttempt.some((r) => r.hidden)) {
        // Every taller rung spent attribution, so the slide keeps the size it
        // had. Reuse the floor pass already taken rather than re-running it.
        placed = floorAttempt;
        size = floor;
      }
      items.forEach((it, i) => { it.spec = base[i]; });
      if (!placed) {
        // No rung fits — including the floor, which is where the corpus census
        // (test/unit/components/chart-label-drop-census.test.js) would already
        // be red. Emit at the floor so the slide degrades exactly the way it
        // did before this change rather than at whatever rung failed last.
        for (const it of items) it.spec = { ...it.spec, fontSize: size };
        placed = placeLabels(items, opts);
      }
