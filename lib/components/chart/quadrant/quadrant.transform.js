...
function openSvg(extraClass, hasNames = true) {
  return `<svg class="quadrant-svg${extraClass ? ' ' + extraClass : ''}"` +
    `${hasNames ? '' : ' data-band="none"'} ` +
    `viewBox="0 0 ${GEOM.vbW} ${vbHeight(hasNames)}" role="img" aria-hidden="true">`;
}
...
function renderBubble(model, scale) {
  ...
  model.groups.forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      if (it.label) {
        ...
        // Flip when the label BLOCK would cross the plot floor — below it is
        // the bottom quadrant names' band. Measured against the wrapped height,
        // not a font-size fudge.
        const flipUp = belowY + bh > GEOM.plot.y1 && aboveY - bh > GEOM.plot.y0 + 2;
  ...
  const hasNames = names.some(Boolean);
  const svg = openSvg('quadrant-svg--bubble', hasNames) +
    quadrantTintsSvg(splitX, splitY) +
    plotFrameSvg(splitX, splitY, 'centerline') +
...
function renderTrail(model, scale) {
  ...
  const hasNames = names.some(Boolean);
  const svg = openSvg('quadrant-svg--trail', hasNames) +
    quadrantTintsSvg(splitX, splitY) +
    plotFrameSvg(splitX, splitY, 'centerline') +
    axisLabelsSvg(scale, hasNames) +
    quadrantLabelsSvg(names, '', splitX) +
...
function renderThreshold(model, scale) {
  ...
  const hasNames = names.some(Boolean);
  const svg = openSvg('quadrant-svg--threshold', hasNames) +
    quadrantTintsSvg(splitX, splitY) +
