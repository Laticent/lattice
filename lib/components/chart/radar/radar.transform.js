...
const { wrapSvgLabel, measureLabel, ADVANCE_UPPER, LINE_HEIGHT } = require('../_chart-family/svg-label');
...
function miniCaptionBand(lines) {
  return Math.ceil(MINI_CAPTION_GAP + lines * MINI_CAPTION_FS * LINE_HEIGHT);
}
...
function renderSmallMultiples(model, scale) {
  ...
  // miniCaptionLines). The viewBox height rides out on `--radar-mini-vb` so the
  // CSS box divides by the height actually emitted — a hard-coded divisor would
  // letterbox every one-line chart, or crop the caption the moment a name wrapped.
  const capLines = miniCaptionLines(series);
  const vbH = DIAGRAM_H + miniCaptionBand(capLines);
  const minis = series.map((s, idx) => {
    const color = RADAR_PALETTE[idx % RADAR_PALETTE.length];
    const svg = openSvg('radar-svg--mini', miniViewBox(vbH)) +
      ...
      '</svg>';
    return `<figure class="radar-mini" style="--series-color:${color};--radar-mini-vb:${vbH}">${svg}</figure>`;
  }).join('');
