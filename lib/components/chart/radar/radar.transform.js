...
const { wrapSvgLabel, ADVANCE_UPPER, LINE_HEIGHT } = require('../_chart-family/svg-label');
...
const MINI_CAPTION_LINES = 2;
const MINI_CAPTION_GAP = MINI_CAPTION_FS * 0.5;
// A CONSTANT band, not content-sized: the minis lay out in a flex row, so a
// taller viewBox on the one mini with a two-line name would misalign the row.
// Two lines always fit; a third is a naming problem, not a layout one.
const MINI_CAPTION_BAND =
  Math.ceil(MINI_CAPTION_GAP + MINI_CAPTION_LINES * MINI_CAPTION_FS * LINE_HEIGHT);
const MINI_VIEWBOX =
  `${-MINI_LABEL_PAD} 0 ${DIAGRAM_W + 2 * MINI_LABEL_PAD} ${DIAGRAM_H + MINI_CAPTION_BAND}`;
...
// (buildSvgLegend / wrapSvgLabel / the family constants are required at the top
// of the file — the mini viewBox is computed from FS_OF_HEIGHT before this point.)
...
const {
  stripTags, plainText, fmtNum, findOuterUL, splitTopLevelLI,
...
function openSvg(extraClass, viewBox) {
...
function renderSmallMultiples(model, scale) {
  ...
  const minis = series.map((s, idx) => {
    const color = RADAR_PALETTE[idx % RADAR_PALETTE.length];
    const svg = openSvg('radar-svg--mini', MINI_VIEWBOX) +
      ...
      wrapSvgLabel(s.name, {
        ...
        width: DIAGRAM_W + 2 * MINI_LABEL_PAD,
        fontSize: MINI_CAPTION_FS,
        anchor: 'middle',
        vAlign: 'hanging',
        maxLines: MINI_CAPTION_LINES,
