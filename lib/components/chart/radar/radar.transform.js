...
// Series color rotation. Same categorical tokens the other native charts
...
// caught because the unit tests pin SVG structure, not fill color.
...
// the categorical chart family (see _chart-family.css). Each color drives a
...
// Geometry — a 300×300 viewBox, plot centered with room for rim labels.
...
// The PANE canvas (ctx.paneView) for the transform in flight. Radar's five render paths all
// end in composeFigure, so the pane box is held here for the length of one synchronous
// transformSection rather than threaded through each; it is cleared in a finally.
let paneViewNow;
...
const {
  wrapSvgLabel, measureLabel, upperAdvance, LINE_HEIGHT, BASELINE_EXTENT,
} = require('../_chart-family/svg-label');

const DIAGRAM_W = 300, DIAGRAM_H = 300;   // the radar diagram box; the key rail appends to its right
// Portrait legend-below: the axis labels (anchored at R+labelGap from center)
...
function tickLabelsSvg(scale) {
  // THE TICKS PAINT LAST, OVER THE SERIES. A scale rung is the reference the
  // polygons are read AGAINST, so a polygon covering it costs the reader the
  // number that makes the shape mean something. SVG has no z-index: paint order
  // is document order, and every caller therefore appends this group at the end
  // of its body rather than next to the grid it belongs with.
  //
  // Measured before the move, with each glyph sampled against the pixels behind
  // it: the 2.5 / 5 / 7.5 rungs failed AA on all twelve themes, 2.60-4.36:1
  // against a 4.5 floor. The canvas-coloured halo on `.radar-tick` could not fix
  // that on its own and this is why — a translucent polygon painted afterwards
  // dims the halo and the glyph together.
...
function openSvg(extraClass, viewBox, label, titled = false) {
  ...
  // labeled mini is `role="img"` with an accessible name instead of hidden.
  // NOTE the aria-hidden default is what the NON-mini radars get — they are
  // built by composeFigure, not through here, so this function only ever runs
  // for a mini today. The branch is kept because the signature is the natural
  // place for it if another caller appears; it is not currently reachable.
  //
  // `titled`: the caller writes its own `<title>` + `<desc>` as the first
  // children, so the root is a named, described image with no `aria-label`
  // (an `aria-label` would win over the `<title>` and drop the description's
  // pairing). That is how a mini is built now.
  ...
  return `<svg class="radar-svg${extraClass ? ` ${extraClass}` : ''}" ` +
    `viewBox="${viewBox || GEOM.viewBox}" ${a11y}>`;
}

// THE RADAR'S ACCESSIBLE DESCRIPTION, with the scores in it.
//
// The shared legend's `<desc>` is "Key — Meridian, Vantage, Helios": the series
// NAMES, which is all a key row carries. On a pie or a map the key rows hold the
// values too, so that desc is complete; on a radar the values live on the spokes,
// and the one thing a screen reader user most needed — how each series scored on
// each axis — was the one thing the description left out. Narration already reads
// every score (`narrateRadar`); this puts the same facts in the tree.
function seriesScores(model, s) {
  return model.axes.map((ax, i) => `${plainText(ax.label)} ${fmtNum(s.values[i])}`).join(', ');
}
function radarDesc(model) {
  const lines = model.series.map((s) => `${plainText(s.name)}: ${seriesScores(model, s)}`);
  // A sectored radar (`quadrant`) groups its axes; the groups are what it shades, so they are
  // named with their axes rather than flattened into one list.
  const axes = model.groups?.length
    ? `Sectors — ${model.groups.map((g) => `${plainText(g)}: ${model.axes.filter((ax) => ax.group === g).map((ax) => plainText(ax.label)).join(', ')}`).join('; ')}`
    : `Axes — ${model.axes.map((ax) => plainText(ax.label)).join(', ')}`;
  return lines.length ? `<desc>${escAttr(`${axes}. ${lines.join('; ')}`)}</desc>` : '';
}
// The series NAME a polygon draws, as its mark identity for the Present Guide:
// `narrateRadar` reads "Meridian: Performance, nine; …", so the polygon that
// label leads is the one the pointer lands on.
const seriesIdent = (name) => ` data-label="${escAttr(plainText(name))}"`;

// Compose the diagram (defs + body, in the 300×300 box) with the shared
// SVG-native key into ONE <svg>, then the figure — the radar analog of
...
// series. The diagram is vertically centered by the builder's diagramDy.
function composeFigure(variant, model, extraClass, defsInner, bodyInner, entries, orientation) {
  const rows = entries.map((e) => {
    ...
    return {
      swatchFill: muted
        ? 'color-mix(in oklab, var(--muted-mark) 32%, var(--bg))'
        : `color-mix(in oklab, ${e.color} 82%, var(--chart-cat-base))`,
  ...
  // In a PANE the key keeps slide-size type (svg-legend.js fitKeyToPane). The diagram's label
  // padding follows the orientation, so only the type is scaled, never the key's side.
  const { key } = fitKeyToPane(
    (o, fontScale) => buildSvgLegend({ rows, diagramRight: DIAGRAM_W + pad, diagramHeight: DIAGRAM_H, hasValues: false, orientation: o, diagramPadX: PORTRAIT_LABEL_PAD, fontScale }),
    { orientation, paneView: paneViewNow, diagramHeight: DIAGRAM_H, orientations: [orientation] });
  ...
  const svg = `<svg class="radar-svg${extraClass ? ' ' + extraClass : ''}" ` +
    `viewBox="0 0 ${key.viewW} ${key.viewH + vpad * 2}" role="img"><title>Radar chart</title>${radarDesc(model) || key.desc}` +
...
// series gets its own <radialGradient> centered on the plot. PROTOTYPE
...
// still flips with the canvas. Unique ids per render avoid the SVG duplicate-id trap where the
// first def wins for every later chart. Siblings: chart-family.js pie wedges,
// quadrant.transform.js region rects.
//
// The "one Node process per deck" assumption the old comment rested on was false for the docs
// site, which renders many times per page — so a module-level counter made render() output
// depend on how many renders preceded it. The sequence is RENDER-scoped now
// (lib/core/render-ids.js); see that module for why determinism is load-bearing.
function areaGradient(color) {
  const id = `${renderIdPrefix()}radar-area-${nextRenderSeq('radar-area')}`;
...
function renderStandard(model, scale, isMinimal, orientation) {
  ...
  series.forEach((s, idx) => {
    ...
    plot += `<polygon class="radar-poly" data-anima-role="region" data-series="${idx}"${seriesIdent(s.name)} style="--series-color:${color}${fill}" ` +
  ...
  const bodyInner = gridSvg(n) + axisLabelsSvg(axes, null, labelPad(orientation)) + plot + tickLabelsSvg(scale);
...
function renderTarget(model, scale, orientation) {
  ...
  const bodyInner = gridSvg(n) + axisLabelsSvg(axes, null, labelPad(orientation)) +
    `<polygon class="radar-poly radar-poly--target" data-anima-role="region"${seriesIdent(target.name)} points="${seriesPoints(target.values, n, scale)}"/>` +
    gaps +
    `<g class="radar-plot">` +
      `<polygon class="radar-poly" data-anima-role="region" data-series="0"${seriesIdent(actual.name)} style="--series-color:${actualColor}" ` +
        `points="${seriesPoints(actual.values, n, scale)}"/>` +
      dotsSvg(actual.values, n, scale, 0, actualColor) +
    `</g>` + tickLabelsSvg(scale);
  return composeFigure('target', model, '', '', bodyInner, [
    { name: actual.name, color: actualColor },
    { name: target.name, color: 'var(--muted-mark)', kind: 'target' },
  ], orientation);
}
...
function renderDelta(model, scale, orientation) {
  ...
  const bodyInner = gridSvg(n) + axisLabelsSvg(axes, null, labelPad(orientation)) +
    `<polygon class="radar-poly radar-poly--before" data-anima-role="region"${seriesIdent(before.name)} points="${seriesPoints(before.values, n, scale)}"/>` +
    segs +
    `<g class="radar-plot">` +
      `<polygon class="radar-poly" data-anima-role="region" data-series="0"${seriesIdent(after.name)} style="--series-color:${afterColor}" ` +
  ...
  return composeFigure('delta', model, '', '', bodyInner, [
    { name: before.name, color: 'var(--muted-mark)', kind: 'before' },
...
function renderBenchmark(model, scale, orientation) {
  ...
  const bodyInner = gridSvg(n) + axisLabelsSvg(axes, null, labelPad(orientation)) +
    band +
    `<g class="radar-plot">` +
      `<polygon class="radar-poly radar-poly--hero" data-anima-role="region" data-series="0"${seriesIdent(hero.name)} style="--series-color:${heroColor}" ` +
  ...
  return composeFigure('benchmark', model, '', '', bodyInner, [
    ...
    { name: pack.length ? 'Comparison range' : hero.name, color: 'var(--muted-mark)', kind: 'band' },
  ], orientation);
}
...
function renderQuadrant(model, scale, isMinimal, orientation, hand) {
  ...
  groups.forEach((g, gi) => {
    ...
    // The sector's NAME, for the Present Guide: `narrateRadarSectors` reads a sector a
    // sentence ("People, averaging three: Hiring, four; …"), and this wedge is where it is.
    sectors += `<path class="radar-sector" data-group="${gi}" data-label="${escAttr(plainText(g))}" style="--series-color:${color}" ` +
  ...
  series.forEach((s, idx) => {
    const color = RADAR_PALETTE[idx % RADAR_PALETTE.length];
    plot += `<polygon class="radar-poly" data-anima-role="region" data-series="${idx}"${seriesIdent(s.name)} style="--series-color:${color}" ` +
      `points="${seriesPoints(s.values, n, scale)}"/>`;
  });
  series.forEach((s, idx) => {
  ...
  const bodyInner = sectors + gridSvg(n) + arcs +
    axisLabelsSvg(axes, GEOM.labelGap - 6, labelPad(orientation)) + rim + plot + tickLabelsSvg(scale);
  const entries = series.map((s, i) => ({ name: s.name, color: RADAR_PALETTE[i % RADAR_PALETTE.length] }));
...
function renderSmallMultiples(model, scale) {
  ...
  const minis = series.map((s, idx) => {
    const color = RADAR_PALETTE[idx % RADAR_PALETTE.length];
    // WRAPPED, like every other chart root in the family. The mini keeps its
    // `role="img"` + `aria-label` (that is its accessible NAME), but everything
    // painted inside is marks — the axis names, the polygon, the dots, and a
    // caption that only repeats the label already on the root. Left exposed, a
    // reader heard the series name and then walked "Adoption", "Margin",
    // "NPS", "Velocity" and the name a second time, which is the verbosity the
    // family-wide pass removed everywhere else. Measured on the shipped radar
    // gallery: 5 of 15 roots still leaked, and these minis were four of them.
    // NAMED AND DESCRIBED, like every full-size chart: a `<title>` (the series)
    // and a `<desc>` with its scores, which the document pass wires to
    // `aria-labelledby` / `aria-describedby`. The bare `aria-label` it replaced
    // named the mini and said nothing about it — "Atlas, image", no scores.
    const svg = openSvg('radar-svg--mini', miniViewBox(vbH), null, true) +
      `<title>${escAttr(plainText(s.name))}</title><desc>${escAttr(seriesScores(model, s))}</desc>` +
      ariaHiddenMarks(
      gridSvg(n) + axisLabelsSvg(axes, GEOM.labelGap - 8, MINI_LABEL_PAD) +
      `<g class="radar-plot">` +
        `<polygon class="radar-poly" data-anima-role="region" data-series="0"${seriesIdent(s.name)} style="--series-color:${color}" ` +
          `points="${seriesPoints(s.values, n, scale)}"/>` +
...
function buildRadar(model, variant, scale, isMinimal, orientation, hand = false) {
  switch (variant) {
    ...
    case 'quadrant':         return renderQuadrant(model, scale, isMinimal, orientation, hand);
...
/**
 * The chart-family entrypoint (see the `kernel` block in radar.manifest.json).
 * Signature is the family's ratified one: (html, ctx) -> html | {html, cls} | null.
 */
function transformSection(html, ctx) {
  return spliceFirstList(html, (ext) => {
    const variant = pickVariant(ctx.classTokens);
    const isMinimal = ctx.classTokens.includes('minimal');
    const model = parseRadar(ext.inner, variant === 'quadrant');
    if (!model) return null;
    const scale = resolveScale(model, matchEyebrowText(html));
    // `.radar-sector-label` is `--font-body`, re-pointed at the hand sans under
    // the finish, and its wrap + de-collision boxes are estimated at build time.
    paneViewNow = ctx.paneView;
    try {
      return buildRadar(model, variant, scale, isMinimal, ctx.orientation,
        readsHandBody(ctx.classTokens));
    } finally {
      paneViewNow = undefined;
    }
  });
}
