...
function axisLabelsSvg(axes, gap, padX) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out += wrapSvgLabel(axes[i].label, {
      ...
      attrs: ` data-mark="${i}" dominant-baseline="${baseline}"`,
      emitFontSize: false,
    }).svg;
  }
  out += '</g>';
...
function tickLabelsSvg(scale) {
  ...
  for (let r = 1; r <= GEOM.rings; r++) {
    ...
    out += wrapSvgLabel(fmtNum(val), {
      ...
      className: 'radar-tick', attrs: ' dominant-baseline="middle"', emitFontSize: false,
    }).svg;
  }
  out += '</g>';
  return out;
...
function renderSmallMultiples(model, scale) {
  ...
  const minis = series.map((s, idx) => {
    ...
    const svg = openSvg('radar-svg--mini') +
      gridSvg(n) + axisLabelsSvg(axes, GEOM.labelGap - 8, 0) +
      `<g class="radar-plot">` +
