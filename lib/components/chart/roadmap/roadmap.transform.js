...
const { derivedFrom, labelSetFor, liftLabelSet, resolveLabelSet } = require('../../../core/label-set');
...
// catalog), not from a constant here. Same four words, same lifecycle order —
...
const STATE_LABEL = Object.fromEntries(STATE_ORDER.map((sc) => {
  ...
  if (!member?.label) {
    ...
    throw new Error(
      ...
      + 'All four markers need a default word — edit roadmap.manifest.json and rerun '
...
function transformSection(html, ctx) {
  ...
  html = transformRoadmapSection(lifted.html, cls);
