...
function buildScatter(model, ctx, axes) {
  ...
  for (const p of model.points) {
    ...
    // `data-hue="1"` — every dot takes the SAME categorical slot, because a
    ...
    marks += `<circle class="${cls}" data-hue="1" data-anima-role="point" data-mark="${p.mark}"` +
...
function buildSizeKey({ view, plot, maxSize, model, caption }) {
  ...
  for (const row of rows) {
    ...
    // `data-hue="1"`, like the marks: the a11y and print grayscale rules reach
    ...
    out += `<circle class="scatter-size-ring" data-hue="1" cx="${cart.round2(cx)}" ` +
