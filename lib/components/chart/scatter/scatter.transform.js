...
function readAxisTitles(html) {
  ...
  const lifted = liftBracketSpans(html, {
    below: decl.keyBelow !== false, bodyTags: decl.bodyTags, acceptAbove: axisAcceptor(decl),
  });
    ...
    if (p.label) {
      ...
      labelItems.push({
        ...
        spec: {
          ...
          // The name recedes with its dot (and a tap on it opens the dot's card), as quadrant's does.
          attrs: ` data-mark-for="${p.mark}"`,
        },
      });
    }
  }
