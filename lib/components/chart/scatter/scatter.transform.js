...
function readAxisTitles(html) {
  ...
  //
  // `below: false` because a scatter has no label set and DOES declare a coda —
  // without it, a trailing paragraph that happened to be bracketed would be
  // consumed as a key this chart never draws.
  const lifted = liftBracketSpans(html, { below: false, bodyTags: ['<ul', '<ol'] });
