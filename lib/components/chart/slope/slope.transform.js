...
const { spliceFirstList, escAttr, chartStatus } = require('../_chart-family/transform-utils');
...
function parseSlope(ulInner) {
  ...
  const raw = s.groups
    ...
    .map((g) => ({
      ...
      // Case folds, the family's one rule: `AT-RISK` marks the line `at-risk`.
      status: chartStatus(g.totalRaw),
...
function slopeTicks(model) {
  // A PADDED domain, not a snapped one, and that part has not changed: snapping
  // out to whole steps widens 9…20 to 5…20, which costs a quarter of the
  // vertical range on the slopegraph — a quarter of every slope angle — and a
  // quarter of every bar length on the dumbbell. A zero baseline would flatten
  // a 60→75 slope to nothing, so zero is never forced in.
  //
  // IT WAS `niceTicks({ tight: true })` UNTIL THE DUPLICATE WAS FOLDED BACK.
  // `scatter` had grown a fuller answer to the same question privately — the
  // target sweep, the tick ceiling, the zero wall, the snap-out — and both
  // docblocks argued for themselves with the same anecdote. `niceDomain` is the
  // one copy now (HARD RULE #1), so this inherits all four.
  //
  // WHAT THAT ACTUALLY MOVES IS NARROWER THAN IT LOOKS, and knowing which half
  // is which is the point of this note. The two variants read this return value
  // differently: the DUMBBELL scales its bars across `[ticks.min, ticks.max]`
  // and draws a grid, so everything here is visible on it; the SLOPEGRAPH
  // scales to `model.min/max` with its own pad and draws no gridlines at all,
  // so the ticks reach it only through `markFormatter` — they fix the unit the
  // endpoint values print in, nothing more. Measured over every slope and
  // dumbbell slide in the shipped decks: one tick set changes, on a slopegraph,
  // and that slide renders PIXEL-IDENTICAL for exactly that reason. The one
  // dumbbell keeps its 40/50/60 and only widens its domain, 6% of air at each
  // end to the family's 8%.
  //
  // `maxTicks: 4` is a per-component choice, not a tuning to reproduce the old
  // picture. A dumbbell PRINTS its value at both endpoints — the gridlines are
  // a secondary calibration, not how the chart is read — so the ceiling that
  // suits a scatter (which grids both axes and prints nothing) is one line too
  // many here. Left at 5, the sweep's smaller-step tie-break took the gallery's
  // dumbbell from three gridlines to five, behind data that already carries its
  // own numbers.
  return cart.niceDomain(model.min, model.max, { target: 4, maxTicks: 4 });
}
...
// The TAP TARGET for entity `i`, stamped on its line, its name and its values (and on a lone
// dot, which has no line). A slope's line is ~1.5 units wide, so on a phone preview it is under a
// screen pixel; the text beside it is what a reader actually points at. `data-label` rides along
// so the reveal titles the card from it, never from the node it hit (a value's text is "31%").
// The dots of a two-point entity stay UNtagged on purpose: `chartToScene` (docs/src/lib/
// chart-anima.ts) animates every `[data-mark]` node, so tagging them would add motion the chart
// does not have today, and the reveal's near-miss search already reaches a dot at a line's end.
// The labels are safe to tag: they declare `data-anima-role="label"`, which that code already
// choreographs as labels.
function hitAttrs(e, i) {
  return ` data-mark="${i}" data-label="${esc(e.label)}"`;
}
...
function buildSlopegraph(model, view) {
    ...
    for (const j of e.present) {
      ...
      interior.push({
        ...
        spec: {
          ...
          attrs: `${markAttrs(e, slot)}${hitAttrs(e, i)} data-anima-role="label"`,
          emitFontSize: false,
        },
      });
    }
  ...
  const interiorSvg = interior.length
    // NOT reported through the label-drop channel (#2171), deliberately. These are
    // VALUE pills, not names: for a non-lone entity `text` is the formatted number
    // alone, so a drop here surfaces to an author as `"52%" is not painted` with the
    // advice "shorter names", which is neither true nor actionable. And the number
    // really is not lost — it rides the `<desc>` and the speaker note either way,
    // which is the test the channel's other four mechanisms are chosen by.
  ...
  entities.forEach((e, i) => {
    ...
    for (const [p, px, outward] of ends) {
      ...
      values.push(wrapSvgLabel(fmt(p.num), {
        ...
        attrs: `${markAttrs(e, slot)}${hitAttrs(e, i)} data-anima-role="label"`, emitFontSize: false,
      }).svg);
    }
  });
...
/**
 * The marks ride in an `aria-hidden` group (cartesian.js § ariaHiddenMarks), so every
...
function descText(model) {
  ...
  const rows = entities.map((e, i) => {
    ...
    // SINGULAR AT ONE. A one-point move read "down 1 points" on the real
    // accessibility tree — the only surface this string is ever heard on, and
    // the only place the slip is audible. `pointsOverflow` below already
    // pluralizes; this line did not.
    const mag = d
      ? ` ${pct ? `${cart.round2(Math.abs(d))} point${cart.round2(Math.abs(d)) === 1 ? '' : 's'}` : fmt(Math.abs(d))}`
      : '';
