...
// A TAP PROXY for entity `i`: its name and value labels say which entity they belong to, so a
// tap on "Northwind" or its "31%" opens Northwind's card, the same as its line. A slope's line is
// ~1.5 units wide, so on a phone preview it is under a screen pixel, and the text beside it is what
// a reader actually points at. It is `data-mark-for`, not `data-mark`, on purpose: `chartToScene`
// (docs/src/lib/chart-anima.ts) strokes every `[data-mark]` it is asked to highlight, and the
// Present Guide ranks `[data-label]` nodes, so a label posing as a mark would change both. Only the
// reveal layer (docs/src/playground/chart-interact.js) reads this attribute.
function proxyAttrs(i) {
  return ` data-mark-for="${i}"`;
}
    ...
    for (const j of e.present) {
      ...
      interior.push({
        ...
        spec: {
          ...
          attrs: `${markAttrs(e, slot)}${proxyAttrs(i)} data-anima-role="label"`,
          emitFontSize: false,
        },
      });
    }
...
function buildDumbbell(model, view) {
  ...
  const chrome = cart.buildGrid({ plot, ticks: ticks.ticks, scale: x, axis: 'x' })
    ...
    + cart.buildCategoryLabels({
      ...
      // Row i IS entity i, and every entity draws its bar (`data-mark="i"` below): the name
      // recedes with its row.
      markFor: true,
    });
  ...
  entities.forEach((e, i) => {
    ...
    for (const [p, px, outward] of ends) {
      ...
      values.push(wrapSvgLabel(fmt(p.num), {
        ...
        attrs: `${markAttrs(e, slot)}${proxyAttrs(i)} data-anima-role="label"`, emitFontSize: false,
      }).svg);
    }
  });
