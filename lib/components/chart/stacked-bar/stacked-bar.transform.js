...
function buildStackedBar(model, ctx = {}) {
  ...
  parts.push(cart.buildCategoryLabels({
    ...
    axis: row ? 'y' : 'x',
    maxLines: 2,
    // THE GAP IS NOT COSMETIC. `buildCategoryLabels` asks `wrapSvgLabel` for
    // `vAlign: 'top'`, which is not one of the four vAligns that emitter knows
    // ('baseline' | 'middle' | 'hanging' | 'bottom'), so it falls through to
    // `baseline` and `y` becomes the FIRST LINE'S BASELINE rather than the
    // block's top edge. At the substrate's default gap of 5 the glyphs
    // therefore climb about 0.78em back over the axis rule and the category
    // names render struck through. Until that is fixed in the substrate the
    // members have to pay for the ascent themselves: 5 units of air plus the
    // ~5.9 the 7.5-unit face rises above its baseline.
    gap: row ? 5 : 11,
  }));
