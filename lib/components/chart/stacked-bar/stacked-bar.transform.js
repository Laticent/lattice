 ...
 *
 * NO COLOR (HARD RULE #3): segments carry a class and `data-hue`, and take their
 * paint from the family's slot table, which resolves that slot to --mark-body.
 *
 * THIS SEGMENT IS FLAT, and that is not yet a decision anybody made. An earlier
 * version of this comment said the paint came from `buildFillDefs` — the shared
 * Cartesian gradient builder — and it never has: `git log -S buildFillDefs` over
 * this file returns only the commit that added the sentence. bar, waterfall and
 * line call it and wear the canonical vertical gradient; stacked-bar, scatter and
 * slope landed in the SAME commit (#2080) without it and without a reason, while
 * bullet opted out in the same commit and said why ("six units is a gradient
 * nobody can see"). So the family paints its bodies two ways and only one member
 * can say why. Whether a stacked segment should wear the gradient is an open
 * design question, tracked with the chart design-language work — but the comment
 * describing an intent nobody wired is not, so it is gone.
 */
...
function buildStackedBar(model, ctx = {}) {
  ...
  // The bands that draw at least one segment — the same `!v` test the stack below skips on.
  const drawn = new Set(groups.flatMap((g, gi) => (g.segs.some((seg, si) => (share ? fracs[gi][si] : seg.num)) ? [gi] : [])));
  parts.push(cart.buildCategoryLabels({
    ...
    // Every segment in band i carries `data-mark="i"`, so category i names mark i: the name
    // recedes with its bar under the chart hover and the Present Guide — when band i drew a
    // segment at all (an all-zero stack draws none).
    markFor: (i) => drawn.has(i),
  }));
    ...
    if (g.segs.length && showTotals) {
      ...
      marks.push(cart.buildValueLabel(text, row
        ? {
          ...
          extra: drawn.has(gi) ? ` data-mark-for="${gi}"` : '',
        }
        : {
          x: band.center(gi),
          y: end - LABEL_GAP,
  ...
  const svg = cart.buildSvgRoot({
    view: { w: svgW, h: svgH },
    className: 'cart-svg stacked-bar-svg',
...
function nameColumn(groups, anchors, plot, share, pcts, valueFmt) {
  ...
  const out = rows.map((r, i) => {
    ...
    return wrapSvgLabel(r.sg.series, {
      ...
      // The number is a part OF bar `gi` (the last populated one, which the column annotates), so
      // it recedes with that bar. The series name beside it is a key to every bar and stays put.
      className: 'cart-tick sbar-part', attrs: ` data-anima-role="label" data-mark-for="${gi}"`,
      emitFontSize: false,
    }).svg;
  });
 ...
 *
 * The marks ride in an `aria-hidden` group (cartesian.js § ariaHiddenMarks), so every `<text>` above is
