...
function installStateChartLayout(rootDoc) {
  ...
  function labelLines(labelEl) {
    ...
    // (measured in the Playground: 60 redraws in 10s on an idle slide). The browser's
    // line breaks are a function of the text and the box width alone (the font is
    // fixed per label), so an unchanged pair cannot break differently.
    const key = text + '\u0000' + rectL(labelEl).width.toFixed(2);
  ...
  function nodeShape(n, el, figRect) {
    ...
    // An explicit `:::token` outranks the status: the author named a paint.
    ...
    //
    // Inset by the hairline and clipped to the node's own corner radius, so the
    // stripe sits INSIDE the rounded tile rather than as a slab across its corner.
  ...
  function placeLabel(pts, dagreDir, nlines, w, ctx) {
    ...
    // are skipped, since a label beside its own line is the point.
    const segHit = (box) => {
      let hit = 0;
      for (const sg of ctx.segs || []) {
        if (sg.owner === ctx.cur) continue;
  ...
  function roundedPath(pts) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
      ...
      const r = Math.min(G.laneStep * 0.6, l1 / 2, l2 / 2);
  ...
  function gridLayout(nodes, transitions, dir, lines, metrics) {
    ...
    // A self-loop's hook reaches `selfPeak` past the node's leading corner.
    for (const pl of plans) {
      if (pl.kind !== 'self') continue;
      const p = out.pos[pl.a.index];
      if (p) grow(p.x - G.selfPeak, p.y - G.selfPeak);
    }
  ...
  function draw(fig, figIndex) {
    ...
    const labelMetrics = (d) => {
      ...
      return { wrapped, m: { maxW: Math.max(maxW, G.extentTb), maxLines } };
    };
    ...
    if (view.width > 0 && view.height > 0 && !droppedNode) {
      if (dagreLayout) {
        ...
        // fraction of body size. The grid draws a branch as a skip rather than as
        // parallel ranks, which is a weaker picture of a fan-out, so dagre goes
        // first in the preference order and the wrap has to earn its place by the
        // same 12% margin as any other wrap.
        const cands = [];
        const real = nodes.filter((nd) => !nd.isFinal).length;
        for (const d of dirs) {
          const dm = d === dir ? lm : labelMetrics(d);
          const L = d === dir ? dagreLayout : dagrePositions(nodes, transitions, d, dm.m, 1, droppedNode);
          if (L) {
            const cv = dagreCanvas(L, d);
            cands.push({ dir: d, lines: 1, k: fitK(cv.w, cv.h, view), L, dm });
          }
          for (let Ln = 2; Ln <= Math.min(6, Math.floor(real / 2)); Ln++) {
            const g = gridLayout(nodes, transitions, d, Ln, dm.m);
            if (g) cands.push({ dir: d, lines: g.lines, k: fitK(g.width, g.height, view), g, dm });
          }
        }
        const pick = pickCandidate(cands, prefDir, capK);
        if (pick?.g) {
          gridSel = pick.g; dagreLayout = null; dir = pick.dir; lm = pick.dm; wrapped = lm.wrapped; dagreMetrics = lm.m;
        } else if (pick && pick.dir !== dir) {
          dir = pick.dir; dagreLayout = pick.L; lm = pick.dm; wrapped = lm.wrapped; dagreMetrics = lm.m;
        }
      } else if (!dagreLayout) {
        const real = nodes.filter((n) => !n.isFinal).length;
        const maxLines = Math.min(6, Math.floor(real / 2));
        const cands = [];
        for (const d of dirs) {
          const dm = d === dir ? lm : labelMetrics(d);
          for (let L = 1; L <= Math.max(1, maxLines); L++) {
            const g = gridLayout(nodes, transitions, d, L, dm.m);
            if (g) cands.push({ dir: d, lines: g.lines, k: fitK(g.width, g.height, view), g, dm });
          }
        }
        const pick = pickCandidate(cands, prefDir, capK);
        ...
        // BESIDE it. Three label styles in one component, and the switch between
        // two of them happened at an arbitrary state count. One producer, one
        // look. The column router still draws self-loop hooks, and still draws
        // any figure the grid declines (fewer than two states).
        if (pick) {
          gridSel = pick.g; dir = pick.dir; lm = pick.dm; wrapped = lm.wrapped; dagreMetrics = lm.m;
        }
      }
    }
  ...
  function drawAll() {
    ...
    for (let i = 0; i < figs.length; i++) draw(figs[i], i);
