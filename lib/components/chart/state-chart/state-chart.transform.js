...
function installStateChartLayout(rootDoc) {
  ...
  // Mirrors `edgeLabel`'s own geometry: `(x, y)` is the block CENTRE with
  // `dominant-baseline: middle`, so the block spans `nlines * labelLine`
  // vertically about `y`, and horizontally by the anchor.
  // THE MODEL BOX IS THE LINE BOX, AND THE PAINTED LABEL IS BIGGER THAN IT.
  // Measured over the 22 labels of `examples/state-chart-branching.md`: a
  // one-line label's real `getBBox()` is 14px tall against the 13px line box and
  // sits 1.48px ABOVE it (the ascent `dominant-baseline: middle` does not
  // centre). On top of that `.state-edge-label` paints a `paint-order: stroke`
  // halo — visible ink `getBBox` does not report at all. Testing the bare line
  // box therefore certified placements that still overlapped by ~0.7px, which is
  // how the first cut of this walk left two of three collisions standing.
  //
  // So the tested box is inflated to a true superset: the 1.5px measured
  // overshoot, plus a little air so a cleared label does not merely touch.
  const LABEL_PAD_Y = 2.5;
  const LABEL_PAD_X = 2;
  ...
  function labelBoxAt(mid, dagreDir, nlines, w, side) {
    ...
    if (dagreDir === 'lr') {
      ...
      return { x: mid.x - w / 2 - LABEL_PAD_X, y: ly - h / 2 - LABEL_PAD_Y,
        w: w + LABEL_PAD_X * 2, h: h + LABEL_PAD_Y * 2, lx: mid.x, ly, anchor: 'middle' };
    }
    const lx = mid.x + sgn * G.labelOff;
    return { x: (sgn > 0 ? lx : lx - w) - LABEL_PAD_X, y: mid.y - h / 2 - LABEL_PAD_Y,
      w: w + LABEL_PAD_X * 2, h: h + LABEL_PAD_Y * 2, lx,
  ...
  function placeLabel(pts, dagreDir, nlines, w, ctx) {
    const home = labelBoxAt(midpointOf(pts), dagreDir, nlines, w, 1);
    ...
    if (clash(home)) {
      ...
      outer: for (const side of [1, -1]) {
        ...
        for (let i = side > 0 ? 1 : 0; i < LABEL_SPOTS.length; i++) {
          const box = labelBoxAt(pointAlong(pts, LABEL_SPOTS[i]), dagreDir, nlines, w, side);
          if (!clash(box)) { chosen = box; bestScore = 0; break outer; }
          const sc = score(box);
  ...
  function edgeDagre(t, from, to, ev, dagreDir, style, ctx) {
    ...
    // centre: `mid.y + labelOff` would put the first line's CENTRE 7px below the
