...
function installStateChartLayout(rootDoc) {
  ...
  function applyFit(fig, geo, natRect, maxK) {
    ...
    // Keep the absolute-centring translate; append the fit scale (scales about the
    // box centre, so the diagram stays centred in the viewport).
  ...
  function fitOnly(fig) {
    ...
    // Reset to bare centring BEFORE measuring, or the fit compounds on itself.
    ...
    // size and contradict the variant's whole reason to exist. Nothing here caps the
    // shrink direction: an over-tall machine scales down until it fits, and
    // probeFigureLegibility's type floor is what catches a scale that went too far.
