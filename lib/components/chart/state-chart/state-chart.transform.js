...
const LABEL_BREAK_RE = /\s*(?:\\n|<br\s*\/?>)\s*/gi;
function normalizeEventBreaks(event) {
  return String(event).replace(LABEL_BREAK_RE, '\n').trim();
}
...
function installStateChartLayout(rootDoc) {
  ...
  function edgeDagre(t, from, to, ev, dagreDir, style) {
    ...
    if (ev) {
      ...
      if (dagreDir === 'lr') {
        ly = mid.y + G.labelOff + ((nlines - 1) * G.labelLine) / 2;
      } else {
        lx = mid.x + G.labelOff;
        anchor = 'start';
      }
  ...
  function dagrePositions(nodes, transitions, dir, metrics, stretch) {
    ...
    const lr = dir === 'lr';
    const rankClear = (lr ? metrics.maxW : metrics.maxLines * G.labelLine)
      + G.arrow + G.gap * 2;
    const crossClear = (lr ? metrics.maxLines * G.labelLine : metrics.maxW)
      + G.labelOff + G.gap;
  ...
  function draw(fig) {
    ...
    const wrapBudget = (dir === 'lr' ? G.gapFloorLr : G.gapFloorTb) * 2.4;
    let maxW = 0, maxLines = 1;
    for (const t of transitions) {
      if (!t.event) continue;
      t.event = wrapEventLabel(t.event, wrapBudget, measure);
