...
function installStateChartLayout(rootDoc) {
  ...
  function edgeDagre(t, from, to, ev, dagreDir, style) {
    ...
    const d = style === 'curved' && pts.length > 2 ? smoothPath(pts) :
    ...
    if (ev) {
      const nlines = String(ev).split('\n').length;
      if (dagreDir === 'lr') {
        // `ly` is the BLOCK's centre, so the offset has to clear half a line box
        // as well — otherwise `mid.y + labelOff` positions the first line's
        // CENTRE 7px below the edge, which puts its box TOP 0.5px below it, and
        // `.state-edge-label`'s `paint-order: stroke` halo (0.46875cqi, ~3px
        // half-width at the HD baseline) reaches back across the line. Measured
        // on page 7 of `examples/state-chart-branching.md`: the one-line `auto
        // approve` notched the green edge and the two-line `escalate to legal /
        // counsel` broke its connector into a dashed run — in the deck whose own
        // coda says a label "never punches a hole through the edge it belongs
        // to". Offsetting the block TOP puts the halo clear of the line.
        ly = mid.y + G.labelOff + G.labelLine / 2 + ((nlines - 1) * G.labelLine) / 2;
      } else {
        lx = mid.x + G.labelOff;
        anchor = 'start';
      }
  ...
  function edge(t, byIndex, dir, style) {
    ...
    const body = t._pts && t._pts.length > 1
      ? edgeDagre(t, from, to, ev, dir, style)
  ...
  function dagrePositions(nodes, transitions, dir, metrics, stretch) {
    ...
    try {
      ...
      g.setGraph({
        ...
        //
        // THE FLOOR IS AN AXIS-ALIGNED GUARANTEE, and this comment used to claim
        // more than that. It reserves room along the rank and cross axes; a
        // DIAGONAL run gets its label at the polyline's arc-length midpoint plus
        // a fixed perpendicular offset, which the floor does not bound. Two labels
        // in the shipped decks still graze a node box by 1.6px and 3.3px on
        // measured bboxes — no glyph collision, but not the "ever" this sentence
        // asserts. Placing the label on the longest AXIS-ALIGNED segment of the
        // route (dagre's orthogonal routes always have one) would make the claim
        // true; that is a placement change with its own visual review, so it is
  ...
  function draw(fig) {
    ...
    for (let k = 0; k < nodeEls.length; k++) {
      ...
      if (!Number.isFinite(r.left) || !Number.isFinite(r.top)
        || !Number.isFinite(r.width) || !Number.isFinite(r.height)) continue;
      const n = {
    ...
    const wrapBudget = dir === 'lr' ? G.gapFloorLr * 2.4 : G.gapFloorLr * 5;
    //
    // WRAPPING IS A PROPERTY OF THE RE-RANKED PATH, so the wrapped text is held
    // back until we know dagre took the layout. Only `edgeDagre` draws the label
    // BESIDE the line; a column still draws it ON the line under a `paint-order:
    // stroke` halo, and there a second line makes that halo cut a taller gap out
    // of the edge — the exact failure moving the label off the line exists to
    // avoid. Mutating `t.event` before the adoption test therefore re-rendered
    // every CHAIN too, which is every machine in the six shipped galleries: an
    // `lr` chain with a 27-character event went from one line on `main` to two
    // straddling its own connector. The shipped decks missed it by 15px — the
    // longest event any of them carries is `return for changes` at ~119px against
    // a 134.4px budget — so no gallery moved and the byte-identity check that is
    // this branch's safety net saw nothing. An author writing a slightly longer
    // label would have got it.
    //
    // The metrics still have to be computed from the WRAPPED text, because they
    // are what dagre is given; `dagrePositions` reads the transitions for their
    // endpoints only, never for their event text, so deferring the write is safe.
    const wrapped = [];
    ...
    const dagreMetrics = { maxW: Math.max(maxW, G.extentTb), maxLines };
    let dagreLayout = dagrePositions(nodes, transitions, dir, dagreMetrics, 1);
    // An AUTHORED break (`\n`, `<br/>`) is untouched either way — it is an explicit
    // request, normalized at parse time, and it renders on both paths.
    if (dagreLayout) for (const [t, w] of wrapped) t.event = w;
    if (dagreLayout && figRect.width > 0 && figRect.height > 0) {
      ...
      if (needsMore) {
        const probe = dagrePositions(nodes, transitions, dir, dagreMetrics, 2);
        if (probe) {
          ...
          if (stretch > 1.02) {
            const retry = dagrePositions(nodes, transitions, dir, dagreMetrics, stretch);
