 ...
 * by it, `byFrom.get(s.index)`) plus, when present, a status encoded ONLY as colour.
 *
...
function buildDefault(model, dir, style) {
  ...
  // floored so an overstuffed machine still spills the stage and the probe catches
  // it. See 2026-07-16-state-chart-self-scale.md.
...
function installStateChartLayout(rootDoc) {
  ...
  function nodeFillDefs() {
    gradId = 'sc-node-fill-' + (++scGradSeq);
    const stop = (pos, l, d) =>
      '<stop offset="' + pos + '" style="stop-color:light-dark(' +
        'color-mix(in oklab, var(--text-muted) var(' + l + '), var(--bg)),' +
        'color-mix(in oklab, var(--text-muted) var(' + d + '), black))"/>';
    return '<defs><linearGradient id="' + gradId + '" x1="0" y1="0" x2="0" y2="1">' +
      stop('0%', '--chart-fill-top-l', '--chart-fill-top-d') +
      stop('100%', '--chart-fill-bottom-l', '--chart-fill-bottom-d') +
      '</linearGradient></defs>';
  }
  ...
  function nodeShape(n, el, figRect) {
    ...
    let out = '<rect class="state-node-shape" data-index="' + n.index + '"' +
      ...
      '" rx="' + rx.toFixed(1) + '" fill="url(#' + gradId + ')"/>' +
    ...
    if (idxEl) {
      ...
      const cx = ir.left - figRect.left + ir.width / 2;
      const cy = ir.top - figRect.top + ir.height / 2;
    ...
    if (labEl) {
      ...
      const cx = lr.left - figRect.left + lr.width / 2;
      const first = lr.top - figRect.top + lh / 2;
  ...
  function draw(fig) {
    ...
    // The scale box is absolutely centred (translate -50%); reset to just the
    // centring (no scale) BEFORE measuring so the boxes read at natural size.
    ...
    VIS = 1;
    if (_sec && typeof _sec.getBoundingClientRect === 'function' && typeof _sec.offsetWidth === 'number' && _sec.offsetWidth > 0) {
      const _kr = _sec.getBoundingClientRect().width / _sec.offsetWidth;
      if (_kr > 0 && _kr < 100 && Math.abs(_kr - 1) > 0.005) VIS = _kr;
    }
    ...
    if (geo !== fig && typeof fig.getBoundingClientRect === 'function') {
      const view = fig.getBoundingClientRect();
      if (view.width > 0 && view.height > 0 && figRect.width > 0 && figRect.height > 0) {
        const k = Math.min(view.height / figRect.height, view.width / figRect.width);
        geo.style.transformOrigin = 'center center';
        // Keep the absolute-centring translate; append the fit scale (scales about
        // the box centre, so the diagram stays centred in the viewport).
        geo.style.transform = 'translate(-50%, -50%)' + (Math.abs(k - 1) < 0.005 ? '' : ' scale(' + k.toFixed(4) + ')');
      }
    }
  }
