...
// The status word -> tone table, for the legend. The browser pass keeps its own
// copy (`STATUS_TONE` in installStateChartLayout, which is serialized and closes
// over nothing) and the stylesheet's status table is the third statement; a unit
...
//
// ONE CHIP PER TONE, not per word. A legend decodes COLOR, and two words that
// paint the same tile are one color: listed apart, `on-track` and `done` showed
// two identical green chips that read as a duplicated key (followups.d 2355-p3).
// Words sharing a tone share a chip, joined with a middle dot, in the order they
// first appear.
function buildStatusLegend(model) {
  const groups = [];
  for (const s of model.states) {
    if (!s.status) continue;
    const tone = LEGEND_TONE[s.status] || s.status;
    let g = groups.find((x) => x.tone === tone);
    if (!g) groups.push((g = { tone, words: [] }));
    if (!g.words.includes(s.status)) g.words.push(s.status);
  }
  if (!groups.length) return '';
  const items = groups.map(({ words }) =>
    `<li class="state-legend-item" data-s="${escAttr(words[0])}">` +
    `<span class="state-dot" data-s="${escAttr(words[0])}" aria-hidden="true"></span>` +
    `<span class="state-legend-label">${words.map(escHtml).join(' · ')}</span></li>`
  ).join('');
...
function installStateChartLayout(rootDoc, opts) {
  ...
  // The chart's text floor (`--chart-text-min`) in px, read off `el`. The token is
  // `calc(11px * var(--canvas-scale, 1))`, registered as a `<length>` with `@property`
  // so the computed value comes back resolved ("24.09px"). An engine that does not
  // apply the registration hands back the calc() TEXT instead, with var() already
  // substituted ("calc(11px * 2.19)"); `parseFloat` read that as NaN and the pass fell
  // back to 11px, so a portrait chart's labels were budgeted at half the size they
  // paint. That shape is evaluated here rather than by a probe element: inserting one
  // would re-arm the runtime's childList observer, the idle redraw loop #2355 closed.
  ...
  function applyFit(fig, geo, natRect, maxK) {
    ...
    if (k < 1 && geo.style && typeof geo.style.setProperty === 'function') {
      // Read from the FIGURE, never from `geo`: the override below lands on `geo`,
      // so reading there would compound 1/k on every redraw. A synthetic DOM with
      // no getComputedStyle keeps the 11px default.
      const base = typeof getComputedStyle === 'function' ? readTextMin(fig, 11) : 11;
  ...
  // share five ramps, and the table is gantt's: `live` is running work (info),
  // apart from finished `done` (pass). The pill vocabulary (chart-family.css
  // `.chart-status`) still paints `live` pass; whether it follows is the owner's
  // call (followups.d/2355-p3-status-green-shared-by-three-words.md), so do not
  // "fix" this back to match it. `LEGEND_TONE` and the stylesheet's status table
  // restate it, and a unit test holds the three equal. Held here rather than imported for the reason `gradId` gives below:
  ...
  // and the screen size the ordinal is set at: the family's chart-text floor
  // (`--chart-text-min`, 11px on a 1280 stage), the size an edge label paints at.
  // The engine's hard floor is 1% of slide height (7.2px at 720); an ordinal set
  // just above THAT read as a speck in the corner. Set per figure in draw().
  let paintK = 1;
  let ordinalPx = 11;
  ...
  function nodeShape(n, el, figRect) {
    ...
    let out = '<rect class="state-node-shape" data-index="' + n.index + '"' +
      ...
      // Escaped though `st` is already one of STATUS_TONE's keys: it was READ BACK
      // OUT OF THE DOM, and this string ends at `svg.innerHTML` (HARD RULE #22).
      // The table lookup is the real guard; the escape is the one a scanner can see
      // (CodeQL flagged the raw form as DOM text reinterpreted as HTML on #2355).
      (st ? ' data-s="' + escText(st) + '"' : '') +
    ...
    // k = 0.32). So: the chart-text floor on screen (an edge label's size), at
    // most 30% of the tile's height, anchored to the tile's own top-right corner.
    // Where the two bounds cross the chart is past its budget anyway, and the tile
    // wins. The inset is ONE number on both axes, measured to the digit's ink (a
    // lining numeral's cap height is ~0.7em, so its ink top sits 0.35em above the
    // central baseline): a corner numeral with equal air above and beside it reads
    // as placed, and the old 2px-top / 3px-side inset read as crammed.
    if (idxEl) {
      const natural = readFontPx(idxEl);
      const floorPx = ordinalPx / Math.max(paintK, 0.05);
      const fs = Math.min(Math.max(natural, floorPx), n.h * 0.3);
      const inset = Math.max(rx * 0.75, fs * 0.6);
      const x = n.x + n.w - inset;
      const y = n.y + inset + fs * 0.35;
      out += '<text class="state-index-t"' + (st && tone !== 'mute' ? ' data-s="' + escText(st) + '"' : '') +
  ...
  function accentPath(n, rx, aw, st) {
    ...
    return '<path class="state-node-accent" aria-hidden="true" data-s="' + escText(st) + '" d="' + d + '"/>';
  }
  ...
  // A self-loop's label, kept OFF its own arc. The label's anchor is a fixed
  // fraction of the loop's reach off the node's corner, which sits inside the
  // loop's own vertical span; a label taller than about a third of that reach
  // lands on the arc (the polish deck's `revise` touches it at 16:9, and the
  // story-sized portrait deck's `revise` / `escalate` sat on it once the chart's
  // text floor followed `--canvas-scale`). So the arc's extreme is measured from
  // the cubic, and the label is lifted (or dropped) just clear of it, `x` kept.
  // A label that already clears keeps its old place. Only the stretch of arc
  // under the label's measured width (`t._lw`) counts; with no width, all of it.
  // How far past its node's edge a self-loop and its LIFTED label can reach: the
  // tip gap and arrowhead, the arc's bulge (under 0.45 of its reach on every
  // variant), then one label line and its pad on both sides. The canvas reserves
  // this, so `selfLabel` can never lift a label out of the drawing.
  const selfReach = () => G.gap + G.arrow + G.selfPeak * 0.45 + G.labelLine + G.labelPad * 2;

  function selfLabel(t, x, y, event, c, side) {
    const lines = String(event).split('\n').length;
    const half = (lines * G.labelLine) / 2 + G.labelPad;
    const reach = t && t._lw > 0 ? t._lw / 2 + G.labelPad : Infinity;
    let ext = side === 'below' ? -Infinity : Infinity;
    for (let i = 0; i <= 32; i++) {
      const u = i / 32, v = 1 - u;
      const px = v * v * v * c[0] + 3 * v * v * u * c[2] + 3 * v * u * u * c[4] + u * u * u * c[6];
      if (Math.abs(px - x) > reach) continue;
      const py = v * v * v * c[1] + 3 * v * v * u * c[3] + 3 * v * u * u * c[5] + u * u * u * c[7];
      ext = side === 'below' ? Math.max(ext, py) : Math.min(ext, py);
    }
    if (!Number.isFinite(ext)) return edgeLabel(x, y, event, 'self', 'middle');
    const yy = side === 'below' ? Math.max(y, ext + half) : Math.min(y, ext - half);
    return edgeLabel(x, yy, event, 'self', 'middle');
  }
    ...
    if (t.isSelf) {
      ...
      if (t._selfSide === 'left') {
        ...
        return '<path class="state-edge" data-anima-role="bar" data-dir="self" data-self="true" d="' + d + '"/>' +
          ...
          (ev ? selfLabel(t, from.x - P * 0.7, ty - P * 0.2, ev, [tx, ty, tx, ty - P, hx - P, hy, hx, hy], 'above') : '');
      }
      ...
      return '<path class="state-edge" data-anima-role="bar" data-dir="self" data-self="true" d="' + d + '"/>' +
        ...
        (ev ? selfLabel(t, from.x + from.w + P * 0.7, ty - P * 0.2, ev, [tx, ty, tx, ty - P, hx + P, hy, hx, hy], 'above') : '');
    }
    if (t.to === t.from + 1) {
    ...
    if (t.isSelf) {
      ...
      if (t._selfSide === 'top') {
        ...
        return '<path class="state-edge" data-anima-role="bar" data-dir="self" data-self="true" d="' + d + '"/>' +
          ...
          (ev ? selfLabel(t, tx - P * 0.2, from.y - P * 0.7, ev, [tx, ty, tx - P, ty, hx, hy - P, hx, hy], 'above') : '');
      }
      ...
      return '<path class="state-edge" data-anima-role="bar" data-dir="self" data-self="true" d="' + d + '"/>' +
        ...
        (ev ? selfLabel(t, tx - P * 0.2, from.y + from.h + P * 0.7, ev, [tx, ty, tx - P, ty, hx, hy + P, hx, hy], 'below') : '');
    }
    if (t.to === t.from + 1) {
  ...
  function placeLabel(pts, dagreDir, nlines, w, ctx) {
    ...
    // Other edges' RUNS (`ctx.segs`), on a grid route and, since the 2355 follow-up,
    // a dagre one: there `unblock` sat on `block`'s line. On a wrapped chain a full-width
    ...
    // `publish` run. A grid segment is tested as a hairline box — the edge's OWN runs
    // included (a dagre diagonal is not; see below): a label placed beside its run never touches it (it sits
    ...
    const segHit = (box) => {
      ...
      for (const sg of ctx.segs || []) {
        // On a dagre route only OTHER edges' lines count. A label beside a shallow
        // diagonal always crosses its own line (the box is 13 tall and the line
        // runs further than `labelOff` across it), so testing it sent every such
        // label wandering to a least-bad spot; the grid's runs are axis-aligned and
        // keep the own-run test the note above describes.
        if (!ctx.ownRuns && sg.owner === ctx.cur) continue;
        if (sg.x0 !== sg.x1 && sg.y0 !== sg.y1) {
          // A DIAGONAL run (dagre's, not the grid's): its bounding box is not the
          // line, and testing that box read a label anywhere near a diagonal as
          // sitting on it. Clip the segment to the box (Liang-Barsky) instead and
          // count the length inside.
          const dx = sg.x1 - sg.x0, dy = sg.y1 - sg.y0;
          let u0 = 0, u1 = 1;
          const clip = (pp, q) => {
            if (pp === 0) return q >= 0;
            const r = q / pp;
            if (pp < 0) { if (r > u1) return false; if (r > u0) u0 = r; } else { if (r < u0) return false; if (r < u1) u1 = r; }
            return true;
          };
          if (clip(-dx, sg.x0 - box.x) && clip(dx, box.x + box.w - sg.x0) &&
              clip(-dy, sg.y0 - box.y) && clip(dy, box.y + box.h - sg.y0) && u1 >= u0) {
            hit += Math.max((u1 - u0) * Math.hypot(dx, dy), 1);
          }
          continue;
        }
  ...
  function dagrePositions(nodes, transitions, dir, metrics, stretch, dropped) {
    ...
    try {
      ...
      // Node pairs joined by more than one edge, either way round. Only these put
      // two labels into ONE rank gap; a fan-out's labels already sit on separate
      // lines, and reserving room for them too widened every `tb` fan-out enough
      // that a wrapped grid out-fit it (measured on the branching deck's slide 5).
      const pairKey = (t) => (t.from < t.to ? t.from + '-' + t.to : t.to + '-' + t.from);
      const pairCount = new Map();
      for (const t of transitions) {
        if (t.isSelf || t.from === t.to) continue;
        pairCount.set(pairKey(t), (pairCount.get(pairKey(t)) || 0) + 1);
      }
      const paired = new Set([...pairCount].filter(([, c]) => c > 1).map(([k]) => k));
      for (const t of transitions) {
        ...
        const name = 'e' + (e++);
        edgeName.set(t, name);
        // ON `tb`, DAGRE IS TOLD ABOUT THE LABEL. The rank gap above only clears one
        // label's height, and a label is drawn beside its line after layout, so two
        // edges sharing a rank gap — a `block => 7` / `unblock => 3` pair, or a
        // forward edge beside a back-edge — put two labels in one short gap, on
        // each other and on the neighbor's line, and a label's box reached the node
        // above it. Given the box, dagre reserves it on the edge's label rank and the
        // label is anchored there (`lab` below). Only a PAIRED edge gets the box
        // (`paired` above). It is centered on the line (`labelpos: 'c'`); the walk
        // in `placeLabel`, which now sees every dagre segment, settles the rest.
        // `lr` keeps its measured clearance, which is clean on the same machine.
        const sz = !lr && t.event && paired.has(pairKey(t)) ? metrics.sizes?.get(t) : null;
        const lab = sz
          ? { width: sz.w * (metrics.ls || 1) + G.labelOff + G.labelPad * 2, height: sz.lines * G.labelLine + G.labelPad * 2, labelpos: 'c' }
          : { width: 0, height: 0 };
        g.setEdge(String(t.from), String(t.to), lab, name);
      }
      D.layout(g);
      ...
      const out = { pos: Object.create(null), pts: Object.create(null), lab: Object.create(null), width: 0, height: 0 };
      for (const n of real) {
        const v = g.node(String(n.index));
      ...
      for (const ed of g.edges()) {
        ...
        // The point on the line where dagre reserved this edge's label box.
        if (rec?.width > 0 && Number.isFinite(rec.x) && Number.isFinite(rec.y)) {
          out.lab[ed.name] = { x: rec.x, y: rec.y };
          // The reserved box is part of the drawing, so the canvas holds all of it.
          out.width = Math.max(out.width, rec.x + rec.width / 2);
          out.height = Math.max(out.height, rec.y + rec.height / 2);
        }
  ...
  function gridLayout(nodes, transitions, dir, lines, metrics) {
    ...
    for (const pl of plans) {
      ...
      if (lr) grow(p.x - G.gap - G.selfPeak * 0.2 - half, p.y - Math.max(G.selfPeak * 0.7 + G.labelLine, selfReach()));
      else grow(p.x - G.selfPeak * 0.7 - half, p.y - Math.max(G.gap + G.selfPeak * 0.2 + G.labelLine, selfReach()));
    }
  ...
  function draw(fig, figIndex) {
    ...
    const labelMetrics = (d) => {
      ...
      // Each edge's own label extent, for dagre's `tb` reservation (dagrePositions).
      const sizes = new Map();
      for (const t of transitions) {
        if (!t.event) continue;
        ...
        let own = 0;
        for (const ln of lines) own = Math.max(own, measure(ln));
        maxW = Math.max(maxW, own);
        sizes.set(t, { w: own, lines: lines.length });
      }
      return { wrapped, m: { maxW: Math.max(maxW, G.extentTb), maxLines, selfW, sizes } };
    };
    ...
    // The scale applyFit will actually APPLY is capped at this ceiling (see the
    // upscale note at the applyFit call below). A candidate's raw k can exceed it on
    // a small machine in a tall stage, and every size budgeted "at the chosen k" —
    // the ordinal's floor via `paintK` — has to use the k the figure really gets. With the raw k, the story-sized portrait deck's ordinals
    // were floored for k = 2.13 and painted at 1.6: 18.1px against a 19.2px floor.
    const fitCeil = (capK / CAP_OVER_BODY) * MAX_OVER_BODY;
    ...
    const dagreCanvas = (L, d) => {
      ...
      const padCross = (hasSelfLoop ? Math.max(G.selfPeak + G.gap, selfReach()) : G.gap) * 2;
      return d === 'lr'
    ...
    const textMinBase = typeof getComputedStyle === 'function' ? readTextMin(fig, 11) : 11;
    ordinalPx = textMinBase;
    // `min(k, 1)`: applyFit raises the floor by 1/k only when it SHRINKS the figure.
    // Scaled up, a label paints at the full floor in drawing units, so its geometry
    // is budgeted at that size. On landscape (floor 11px, S = 1) this is 1 for every
    // k >= 1, as before; on a story-sized portrait deck the floor is 24px and the
    // budget has to follow it, or a lifted self-loop label ran off the canvas.
    const labelScaleFor = (k) => (k > 0 ? Math.min(3, Math.max(1, textMinBase / (Math.min(k, 1) * 11 * S))) : 1);
    // `selfPeak` rides the label scale too: a self-loop's label is placed at a
    // fraction of the loop's reach off the node's corner, so a label that grows
    // while its loop does not lands on its own arc (measured on the story-sized
    // portrait deck once its floor followed `--canvas-scale`). Scaling the reach with
    // the label keeps the landscape proportions, and at label scale 1 changes nothing.
    const LABEL_BASE = { labelLine: G.labelLine, labelPad: G.labelPad, labelOff: G.labelOff, labelMargin: G.labelMargin, selfPeak: G.selfPeak };
    const setLabelScale = (ls) => { for (const key in LABEL_BASE) G[key] = LABEL_BASE[key] * ls; };
    const scaled = (m, ls) => ({ maxW: m.maxW * ls, maxLines: m.maxLines, selfW: (m.selfW || 0) * ls, sizes: m.sizes, ls });
    ...
    if (view.width > 0 && view.height > 0 && !droppedNode) {
      ...
      if (pick) {
        paintK = pick.k > 0 ? Math.min(pick.k, fitCeil) : 1;
    ...
    if (dagreLayout) {
      ...
      const padCross = hasSelf ? Math.max(G.selfPeak + G.gap, selfReach()) : G.gap;
      const padEnd = G.gap;
      const lrFlow = dir === 'lr';
      ...
      for (const t of transitions) {
        ...
        // A `tb` label's home is the room dagre reserved for it (dagrePositions).
        const lp = dagreLayout.lab?.[dagreLayout.name.get(t)];
        if (lp) { t._anchor = { x: lp.x + offX, y: lp.y + offY }; t._anchorDir = 'tb'; }
      }
    ...
    const labelCtx = {
      ...
      ownRuns: Boolean(gridSel),
      segs: (gridSel || dagreLayout) ? transitions.flatMap((t) => (t._pts || []).slice(1).map((q, i) => ({
    ...
    // is tall and grows UP when there's room, up to 1.6x body size (`MAX_OVER_BODY`;
    // the owner's call, 2026-09-24-state-chart-fit-and-paint.md), sized by the container (crisp
