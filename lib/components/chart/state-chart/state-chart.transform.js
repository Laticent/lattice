...
function installStateChartLayout(rootDoc) {
  ...
  function draw(fig, figIndex) {
    ...
    // this the reader hears the curated <desc> and then walks every state name
    // and edge label again as loose text. `keep` stays OUTSIDE the group so it
    // still names the root.
    svg.innerHTML = keep + ariaHiddenMarks(parts.join(''));
