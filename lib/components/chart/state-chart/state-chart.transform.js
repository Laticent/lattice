...
function installStateChartLayout(rootDoc) {
  ...
  // also guard against).
  //
  // RENDER-SCOPED, from the shared kernel (lib/core/render-ids.js), not a local
  // counter. The local one lived in this closure and advanced once per draw() —
  // and draw() runs a VARIABLE number of times per render, because the layout
  // re-measures. So two identical renders of the same deck disagreed on the id
  // (measured: `sc-node-fill-5` on one run, `-4` on the next), which is exactly
  // the "render() is not a pure function of its input" defect render-ids.js was
  // written to abolish — and what makes a byte-comparison render-cache test
  // writable. Four members already use it; this was the one that never
  // migrated. HARD RULE #1.
  let gradId = '';
  ...
  function nodeFillDefs(tints) {
    gradId = `${renderIdPrefix()}sc-node-fill-${nextRenderSeq('sc-node-fill')}`;
  ...
  function draw(fig) {
    ...
    const labelCtx = {
      ...
      canvas: dagreLayout ? { w: canvasW, h: canvasH } : null,
    };
