 ...
 * TORN BOTTOM EDGE.
 *
 * The tear is the whole license for the `zoom` variant. A bar cut off flat at a
...
function buildWaterfall(model, ctx = {}) {
  ...
  rows.forEach((r, i) => {
    ...
    labels.push(cart.buildValueLabel(text, {
      ...
      extra: ` data-s="${r.dir}" data-mark-for="${i}"`,
    }));
  });
  ...
  parts.push(cart.buildCategoryLabels({
    plot, labels: fitCategoryLabels(rows.map((r) => r.label), catWidth), names: rows.map((r) => r.label),
    center: (i) => band.center(i), width: catWidth, maxLines: 2, axis: 'x', markFor: true,
  }));
...
/**
 * The marks ride in an `aria-hidden` group (cartesian.js § ariaHiddenMarks), so this
...
function buildDesc(model, fmtIn) {
  ...
  // channel a screen reader has (the marks are `aria-hidden` — cartesian.js
  // § ariaHiddenMarks).
