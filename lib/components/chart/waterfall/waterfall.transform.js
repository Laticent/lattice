...
const BAND = { padInner: 0.3, padOuter: 0.1 };
...
function parseWaterfall(ulInner) {
  ...
  for (const item of parseTopLevelLis(ulInner)) {
    ...
    // U+2212 MINUS SIGN is what a paste out of Excel, Word or a PDF carries, and
    // `parseValue` does not know it — an author who pastes `−0.8M` would get a
    // silent +0.8M and a walk that climbs when it should fall. Normalized here,
    // once, before any number is read.
    const norm = valueRaw.replace(/\u2212/g, '-').trim();
    // THE SIGN IS READ HERE, NOT BY `parseValue`. `parseValue` scans for the
    // first numeric run, and a currency symbol between the sign and the digits
    // pushes that run past the sign: `-$0.9M` parses as +900 000, so a churn
    // line rendered as a GREEN RISE. On a chart whose entire claim is signed
    // contributions that is not a rounding error, it is the opposite number.
    // So an explicit leading `+`/`-` is stripped, the magnitude is parsed from
    // what is left, and the sign is reapplied.
    //
    // A value with NO explicit lead sign still goes through `parseValue`
    // untouched — `$-0.9M`, which the substrate does read correctly, keeps
    // working, and a bare negative closing level stays negative.
    const signLead = norm[0];
    const explicit = signLead === '+' || signLead === '-';
    const bare = explicit ? norm.slice(1).trim() : norm;
    const magnitude = cart.parseValue(bare);
    if (!Number.isFinite(magnitude)) continue;
    const num = explicit ? (signLead === '-' ? -1 : 1) * Math.abs(magnitude) : cart.parseValue(norm);
    if (!Number.isFinite(num)) continue;
    raw.push({
      label: plainText(leadStripped), valueRaw, num,
  ...
  return {
    ...
    opening, closing, arrivedAt,
    net: closing - opening,
    // The geometric invariant this chart lives on: the steps must land on the
    // closing level. Compared with a relative tolerance because 12.0 − 1.4 − 0.8
    // is not exactly 9.8 in binary floating point.
    reconciles: rows[n - 1].kind !== 'total'
...
function buildWaterfall(model, ctx = {}) {
  ...
  parts.push(cart.buildAxisRule({ plot, axis: 'x' }));
  ...
  rows.forEach((r, i) => {
    ...
    // — see buildClippedBar.
    const clipped = zoom && r.kind === 'total' && Math.min(r.start, r.end) < min;
    if (clipped) { h = plot.y1 - top; }
    ...
    bars.push(r.dir === 'flat'
      ...
      : clipped
        ? `<path ${common} data-clipped="1" d="${clippedBarPath(x, top, w, plot.y1)}"/>`
        : `<rect ${common}` +
...
function buildDesc(model, fmtIn) {
  ...
  const head = `Bridge from ${rows[0].label} ${minus(rows[0].valueRaw)} to ` +
    `${rows[rows.length - 1].label} ${minus(rows[rows.length - 1].valueRaw)}, ` +
