...
// those rendered 13.48px for the label and 36 / 25.2 / 19.24px for the ramp, at a
// 0.9 px-per-unit canvas scale — hence 14.98 and 40 / 28 / 21.38 user units. The
// conversion is deliberately size-NEUTRAL: nothing about this change is meant to
// look different.
//
...
// every size.
const KEY_FS = 14.98;
// The A-ramp glyph sizes, as multiples of KEY_FS.
const KEY_RAMP = [2.67, 1.869, 1.427];

function buildSizeKey() {
  ...
  // One entry per painted row: the height of its line BOX, and the gap after it.
  // Laid out top-down from a cursor, then centered as a block on the canvas.
  //
  // `boxR` is the row's line-height ratio. The ramp glyphs get 0.95, mirroring
  // the `line-height: 0.95` the CSS ramp used — a glyph's box was TIGHTER than
  // its font size, which is why the three A's nested closely. Modelling every row
  // at 1.0 spread the ramp out and shortened the block; these ratios and gaps are
  // solved from the measured before/after centres so the converted key sits on
  // the same rhythm rather than a new one.
  const rows = [
    { text: 'size = frequency', size: KEY_FS, boxR: 1, cls: 'wc-key-label', gapAfter: KEY_FS * 1.25, wrap: true },
    { text: 'more', size: KEY_FS, boxR: 1, cls: 'wc-key-edge', gapAfter: KEY_FS * 0.855 },
    ...KEY_RAMP.map((r, i) => ({
      text: i === KEY_RAMP.length - 1 ? 'a' : 'A',
      size: +(KEY_FS * r).toFixed(2),
      boxR: 0.95,
      cls: 'wc-key-a',
      gapAfter: i === KEY_RAMP.length - 1 ? KEY_FS * 0.851 : KEY_FS * 0.08,
    })),
    { text: 'less', size: KEY_FS, boxR: 1, cls: 'wc-key-edge', gapAfter: 0 },
  ];

  const boxH = (r) => r.size * r.boxR;
  const stackH = rows.reduce((s, r) => s + boxH(r) + r.gapAfter, 0);
  let cursor = (CANVAS_H - stackH) / 2;
  const body = rows.map((r) => {
    const yMid = +(cursor + boxH(r) / 2).toFixed(2);
    cursor += boxH(r) + r.gapAfter;
    // The heading is the only row that can outgrow the rail, so it is the only
    // one that goes through the wrapping emitter; a single glyph cannot wrap.
    return r.wrap
      ? wrapSvgLabel(r.text, {
        x: railMid, y: yMid, width: railW, fontSize: r.size,
        anchor: 'middle', baseline: 'middle', vAlign: 'middle', className: r.cls,
      }).svg
      : `<text class="${r.cls}" x="${railMid.toFixed(2)}" y="${yMid}" font-size="${r.size}">${r.text}</text>`;
  }).join('');
  ...
  // `aria-hidden` as before the conversion: the key explains an encoding
  // visually, and a screen reader gets the words themselves from the SVG
  // `<text>` nodes — reading "A A a more less" would be noise, not access.
  return {
    defs: spine.defs,
...
function buildCanvas(rawItems, variant) {
  ...
  const { defs, body: keySvg } = buildSizeKey();
