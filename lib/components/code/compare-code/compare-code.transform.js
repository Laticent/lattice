...
function transformCompareCodeSection(innerHtml, cls) {
  ...
  // The universal coda cell is the FRAME's, not a column's. Peel it off beside
  // the footer above and re-append it after the columns: the split below cuts on
  // `p>code` boundaries and would otherwise sweep a trailing `.cell-coda` into
  // whichever column happened to be last, where the author's Key Insight
  // rendered as unstyled body text under the right-hand code block (lib/core/coda.js).
  ...
  return `${header}${eyeEl}${h2El}<div class="code-cols">${cols}</div>${peeled.coda}${footer}`;
}
