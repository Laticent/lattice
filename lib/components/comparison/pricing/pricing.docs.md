...
| `features` | `ul > li > ul > li` | yes | Feature rows, each led by a state marker: `[x]` included (green check), `[/]` not included (muted, struck through), `[-]` limited (half). `[ ]` draws the same neutral open ring it draws in a checklist or an inline mark — never verdict-grid's red ✕. The LAST nested li carries NO marker — a short ‘who it's for’ line that anchors the bottom of the card. Keep the feature set and its order identical across every tier so the columns scan. |
...
- **An open ring for a feature the tier lacks.** Use `[/]` (muted, struck through) for an absent feature. `[ ]` is an open, unchecked ring — it reads 'not yet', which suggests the feature is coming, not that the tier leaves it out.
