...
const CLOSE = '</section>';
// The Anima spec placeholder emitted by the ```anima fence (animaSceneFences): a hidden
// div carrying the base64 scene spec. Stage 6 lifts it onto the <section> as
// `data-scene-spec` (the docs-site host reads it to mount the live animation) and drops
// the div. base64 has no HTML-special chars, so it re-stamps verbatim (no re-escaping).
const SPEC_DIV_RE = /<div class="anima-spec[^"]*"[^>]*\sdata-scene-spec="([A-Za-z0-9+/=]*)"[^>]*><\/div>/i;
// Every anima-spec placeholder (the lifted one AND a malformed `anima-spec-error`), so a
// leftover never folds into `.scene-text`. Global — strip all.
const ANIMA_SPEC_ANY_RE = /<div class="anima-spec[^"]*"[^>]*><\/div>/gi;
...
function renderSection(inner) {
  ...
  // …except the universal coda cell, which is the frame's own band and must not
  // become part of the placard. Peeled beside the chrome above and re-appended as
  // a full-width sibling below (lib/core/coda.js).
  ...
  return `${header}${figure}${text}${peeled.coda}${footer}`;
}

// Lift the ```anima placeholder's base64 spec onto the open tag as `data-scene-spec`
// and strip the div from the body. Idempotent (skips an already-lifted tag).
function liftSpec(openTag, inner) {
  let out = openTag;
  if (!/\bdata-scene-spec=/i.test(openTag)) {
    const m = inner.match(SPEC_DIV_RE);
    if (m) out = openTag.replace(/^<section\b/i, `<section data-scene-spec="${m[1]}"`);
  }
  // Strip every anima-spec placeholder (the lifted one + a malformed anima-spec-error).
  return { openTag: out, inner: inner.replace(ANIMA_SPEC_ANY_RE, '') };
}

// Stamp the composition on the tag AND wrap the inner of one `section.scene`, given
// as its open tag and inner; null when the section is not a scene.
function transformParts(openTag0, rawInner) {
  // Quote-agnostic class read (mirrors stampComposition / image's stampImageComposition).
  if (!/\bscene\b/.test(attrOf(openTag0, 'class') || '')) return null;
  const { openTag, inner } = liftSpec(openTag0, rawInner);
  return { openTag: stampComposition(openTag, inner), inner: renderSection(inner) };
}

// The same, over one whole section string. A section whose close is not the literal
// `</section>` passes through: slicing a fixed length off a `</section >` would cut
// into the body.
function transformSection(sectionHtml) {
  const om = sectionHtml.match(OPEN_TAG_RE);
  if (!om || !sectionHtml.endsWith(CLOSE)) return sectionHtml;
  const r = transformParts(om[0], sectionHtml.slice(om[0].length, sectionHtml.length - CLOSE.length));
  return r ? `${r.openTag}${r.inner}${CLOSE}` : sectionHtml;
}
...
// in a comment ended the slide there, and the wrap split the placard in two. It
// hands over the open tag and inner, so no close tag is sliced by length.
function applyToRenderedHtml(html) {
  return mapSections(html, (openTag, _cls, inner) => transformParts(openTag, inner));
}
...
function applyToDom(root) {
  ...
  for (const sec of root.querySelectorAll('section.scene')) {
    // Lift the ```anima spec placeholder onto the section BEFORE the wrap (else it folds
    // into .scene-text). Mirrors liftSpec on the string path; the host reads the attr.
    if (!sec.getAttribute('data-scene-spec')) {
      const specDiv = sec.querySelector('.anima-spec[data-scene-spec]');
      if (specDiv) sec.setAttribute('data-scene-spec', specDiv.getAttribute('data-scene-spec'));
    }
    // Remove every anima-spec placeholder (lifted + malformed) so none folds into .scene-text.
    for (const stray of Array.from(sec.querySelectorAll('.anima-spec'))) stray.remove();
