...
function renderSection(inner, cls = '') {
  ...
  const peeled = peelCoda(stripConsumed(inner, p.consumed));
  ...
  if (hasClass(cls, 'companion')) {
    ...
    return `<div class="video-lead">${h2}${leadP}</div>${fig}${peeled.coda}`;
  }

  return injectAfterIntro(body, fig) + peeled.coda;
}
