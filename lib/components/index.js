...
function checkPane(m, prefix, errors) {
  ...
  // A direction the component never takes has no counts, and every direction it takes does.
  for (const where of ['side', 'stack']) {
    if (p[where] === false) {
      if (b[where] !== undefined) errors.push(`${prefix}pane.${where} is false: pane.budget declares no ${where} counts`);
    } else checkPaneBudgetCounts(b[where], where, prefix, errors);
  }
}
