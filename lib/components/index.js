...
// Deep-freeze: the require cache hands every consumer the SAME object, so an
// accidental mutation (enum.push, a new property key) would silently rewrite
// the contract for the whole process. The checker demonstrated it live.
...
const SUPPORTED_AXES = Object.freeze(SCHEMA_PROPS.capacity.properties.axis.enum);
...
function checkAxisMeasurable(m, axis, field, prefix, errors) {
  if (!SUPPORTED_AXES.includes(axis)) return;
...
// per-variant caption/sample docs (each key must be a declared variant).
    ...
    for (const v of m.variants) {
      if (!documented.has(v)) {
        errors.push(
          ...
          `Add variantDocs."${v}" = { caption: <one sentence: what the variant changes and when to reach for it>, sample: <full slide markdown demonstrating it> } ` +
...
function checkSampleFields(m, prefix, errors) {
  ...
  // manifest carries stressDoc { caption, sample } or a reasoned exemption.
  if (m.stressSample !== undefined) {
    errors.push(`${prefix}stressSample is retired — use stressDoc { caption, sample } (2026-07-05 Specimen Book decision §2.2)`);
  }
  // stressDoc { caption, sample } — the target spelling; stressSample is the
  ...
  if (m.stressDoc !== undefined) {
    if (typeof m.stressDoc !== 'object' || m.stressDoc === null || Array.isArray(m.stressDoc)) {
      errors.push(`${prefix}stressDoc must be an object { caption, sample } if present`);
    } else {
      if (typeof m.stressDoc.caption !== 'string' || !m.stressDoc.caption) {
        errors.push(`${prefix}stressDoc.caption must be a non-empty string (one sentence naming the limit shown)`);
      }
...
function checkSplit(m, prefix, errors) {
  ...
  if (sp.axis !== undefined && !SUPPORTED_AXES.includes(sp.axis)) {
    errors.push(`${prefix}split.axis must be one of: ${SUPPORTED_AXES.join(', ')} (got ${JSON.stringify(sp.axis)})`);
  }
