 ...
 */
const SEMI_UNIVERSAL_VARIANTS = Object.freeze(['compact', 'loose', 'accent', 'claim-bleed']);

/**
 * Stamp STYLE axis — the SHAPE a state marker (confidential/wip/…) renders in
 * (base.variants.css). Orthogonal to the semantic markers: the marker sets a label +
 * color, the style sets the shape, so `confidential stamp-seal` composes. One uniform
 * default (`stamp-tab`) keeps a mixed-marker deck coherent; the author overrides
 * per-slide or deck-wide via the `stamp:` register. `boardroom` is the curated subset
 * the Studio drawer surfaces first; `range` is the rest. Single-source for the lint
 * vocabulary AND the drawer's style picker, so neither drifts.
 */
const STAMP_STYLES = Object.freeze({
  boardroom: Object.freeze(['stamp-tab', 'stamp-notch', 'stamp-bracket', 'stamp-seal', 'stamp-pill']),
  range: Object.freeze(['stamp-ribbon', 'stamp-flag', 'stamp-underline', 'stamp-dot', 'stamp-mark', 'stamp-veil', 'stamp-bar', 'stamp-pin']),
});
const STAMP_STYLE_TOKENS = Object.freeze([...STAMP_STYLES.boardroom, ...STAMP_STYLES.range]);

/** Tone STYLE axis — the SHAPE a tone marker renders in (rail default / edge / glow),
 *  orthogonal to the semantic tone (`tone-fail tone-edge`). All box-shadow-shaped so
 *  they never collide with the state `::before` or the pagination `::after`. */
const TONE_STYLE_TOKENS = Object.freeze(['tone-rail', 'tone-edge', 'tone-glow']);
...
const EXCLUSIVE_AXES = Object.freeze({
  ...
  density: Object.freeze(['compact', 'loose']),
