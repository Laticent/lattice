...
const { BACKDROP_STRENGTH_TOKENS, BACKDROP_MASK_TOKENS } = require('../core/resolve-backdrop');
const { VENUE_TOKENS } = require('../core/resolve-venue');
...
const MODIFIER_GROUPS = Object.freeze([
  ...
  { name: 'venue', label: 'Venue', tokens: [...VENUE_TOKENS], exclusive: true },
  ...
  { name: 'backdrop', label: 'Backdrop', tokens: [...BACKDROP_STRENGTH_TOKENS, ...BACKDROP_MASK_TOKENS], axes: [[...BACKDROP_STRENGTH_TOKENS], [...BACKDROP_MASK_TOKENS]] },
...
// The PANE contract — whether and how this component goes in a pane (lib/core/panes.js).
// Optional here, so an installed package validates without it (a pane treats it as `fit:
// half`); test/unit/core/panes.test.js requires it of every built-in component.
function checkPaneBudgetCounts(side, where, prefix, errors) {
  if (typeof side !== 'object' || side === null) {
    errors.push(`${prefix}pane.budget.${where} must be an object { sweet, hard }`);
    return;
  }
  for (const k of Object.keys(side)) {
    if (k !== 'sweet' && k !== 'hard') errors.push(`${prefix}pane.budget.${where} has unknown key '${k}'`);
  }
  for (const k of ['sweet', 'hard']) {
    if (!Number.isInteger(side[k]) || side[k] < 1) errors.push(`${prefix}pane.budget.${where}.${k} must be an integer >= 1`);
  }
  if (Number.isInteger(side.sweet) && Number.isInteger(side.hard) && side.sweet > side.hard) {
    errors.push(`${prefix}pane.budget.${where}.sweet (${side.sweet}) must not exceed hard (${side.hard})`);
  }
}

function checkPane(m, prefix, errors) {
  if (m.pane === undefined) return;
  const p = m.pane;
  if (typeof p !== 'object' || p === null || Array.isArray(p)) {
    errors.push(`${prefix}pane must be an object if present`);
    return;
  }
  const spec = SCHEMA_PROPS.pane.properties;
  for (const k of Object.keys(p)) {
    if (!Object.hasOwn(spec, k)) errors.push(`${prefix}pane has unknown key '${k}'`);
  }
  // Each direction is the least share a pane of this component reads at, or false for none.
  const isShare = (v) => Number.isInteger(v) && v >= 25 && v <= 75 && v % 5 === 0;
  for (const dir of ['side', 'stack']) {
    if (!(p[dir] === false || isShare(p[dir]))) errors.push(`${prefix}pane.${dir} must be false or a share from 25 to 75 in steps of 5`);
  }
  if (p.form !== undefined && (typeof p.form !== 'string' || !new RegExp(spec.form.pattern).test(p.form))) {
    errors.push(`${prefix}pane.form must be a component name`);
  }
  if (p.review !== undefined && !(typeof p.review === 'string' && p.review.length >= 8)) {
    errors.push(`${prefix}pane.review must say why, in words`);
  }
  if (p.side === false && p.stack === false) {
    if (p.budget !== undefined || p.noBudget !== undefined || p.form !== undefined) {
      errors.push(`${prefix}pane: a component that fits no pane (side and stack false) declares no form, budget or noBudget — a panes slide naming it splits`);
    }
    return;
  }
  if ((p.budget === undefined) === (p.noBudget === undefined)) {
    errors.push(`${prefix}pane declares EITHER budget OR noBudget (a component that can go in a pane decides which)`);
  }
  if (p.noBudget !== undefined && (typeof p.noBudget !== 'string' || p.noBudget.length < 8)) {
    errors.push(`${prefix}pane.noBudget must say why, in words`);
  }
  if (p.budget === undefined) return;
  const b = p.budget;
  if (typeof b !== 'object' || b === null) {
    errors.push(`${prefix}pane.budget must be an object`);
    return;
  }
  const bspec = spec.budget.properties;
  for (const k of Object.keys(b)) {
    if (!Object.hasOwn(bspec, k)) errors.push(`${prefix}pane.budget has unknown key '${k}'`);
  }
  if (!bspec.axis.enum.includes(b.axis)) errors.push(`${prefix}pane.budget.axis must be one of ${bspec.axis.enum.join(', ')}`);
  if (!bspec.basis.enum.includes(b.basis)) errors.push(`${prefix}pane.budget.basis must be one of ${bspec.basis.enum.join(', ')}`);
  if (b.basis === 'editorial' && !(typeof b.note === 'string' && b.note.length)) {
    errors.push(`${prefix}pane.budget.basis is editorial: say why in pane.budget.note`);
  }
  if (b.min !== undefined && !(Number.isInteger(b.min) && b.min >= 1)) errors.push(`${prefix}pane.budget.min must be an integer >= 1`);
  for (const where of ['side', 'stack']) {
    const c = b[where];
    if (Number.isInteger(b.min) && c && Number.isInteger(c.sweet) && c.sweet < b.min) {
      errors.push(`${prefix}pane.budget.${where}.sweet (${c.sweet}) is below the component's own minimum (${b.min}): a correct pane would be warned`);
    }
  }
  checkPaneBudgetCounts(b.side, 'side', prefix, errors);
  if (b.at !== undefined && !isShare(b.at)) errors.push(`${prefix}pane.budget.at must be a share from 25 to 75 in steps of 5`);
  // A component that does not go in a stacked band has no stacked budget, and every other one does.
  if (p.stack === false) {
    if (b.stack !== undefined) errors.push(`${prefix}pane.stack is false: pane.budget declares no stack counts`);
  } else checkPaneBudgetCounts(b.stack, 'stack', prefix, errors);
}
