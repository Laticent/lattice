...
const MANIFEST_SCHEMA = (() => {
  try {
    ...
    if (e && e.code !== 'ENOENT') {
      process.emitWarning(
        `could not read ${SCHEMA_PATH} (${e.message}) — validating manifests against the ` +
          'schema bundled at build time, which may not match this tree. Fix the file, or ' +
          'expect manifest errors naming fields the schema already declares.',
