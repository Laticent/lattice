...
function checkPane(m, prefix, errors) {
  ...
  if (!spec.fit.enum.includes(p.fit)) errors.push(`${prefix}pane.fit must be one of ${spec.fit.enum.join(', ')}`);
  ...
  if (p.stack !== undefined && p.stack !== false) errors.push(`${prefix}pane.stack is either absent or false`);
  if (p.fit === 'none') {
    if (p.budget !== undefined || p.noBudget !== undefined || p.form !== undefined || p.stack !== undefined) {
      errors.push(`${prefix}pane.fit is none: a component that opts out of panes declares no form, stack, budget or noBudget`);
    }
    return;
  }
