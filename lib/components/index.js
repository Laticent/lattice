...
const UNIVERSAL_GROUPS = Object.freeze({
  ...
  // cells into the universal state-marker decoding (`[x]` `[-]` `[ ]` `[/]` →
  ...
  table: Object.freeze(['table-plain', 'table-fill', 'state-cells']),
  state: Object.freeze([
    'wip',
    'draft',
    'tbd',
...
function checkKernelMarks(k, prefix, errors) {
  ...
  k.marks.forEach((mark, i) => {
    ...
    // wrong about three real members. `state-chart`'s node carries the family's
    // full gradient and encodes NOTHING in it, deliberately — "the node fill is
    // ALWAYS neutral — status color lives only in the index badge", so a green
    // on-track node cannot blend into a green wash. `map`'s bare basemap region
...
function checkProjection(m, prefix, errors) {
  ...
  for (const key of Object.keys(p)) {
    if (key !== 'figure' && key !== 'data') {
      errors.push(
        `${prefix}projection.${key} is not part of the block — it carries figure and data only.`);
    }
  }
}
