 ...
 * both halves are the shape CodeQL flags in a hand-rolled cleaner, and both were
 * flagged here (alerts 241/242 on #2102).
 *
 * TAGS: one pass, then every residual angle bracket goes. An earlier version ran
 * the strip to a fixpoint and its comment claimed `<<b>b>` "has its inner `<b>`
 * removed and the outer halves close up into a live `<b>`". Both halves of that
 * were wrong, and an independent checker proved it: `String.replace` with a `/g`
 * regex is LEFTMOST-first, so the match is `<<b>` and what is left is `b>` — no
 * tag reforms, and after one pass no `<` can still be followed by a `>`.
 * Exhaustively searched over every string up to length 9 on `{<,>,a,b}`, the loop
 * never reached a second useful iteration, and deleting it left all four of its
 * own tests passing. So the loop was dead and the comment described a mechanism
 * that does not occur. What actually carries the guarantee is the line below it —
 * an unconditional `[<>]` strip, which also covers the UNCLOSED `<script` that
 * has no `>` for the first pass to match. That is the whole property: nothing out
 * of the markup still opens a tag.
 *
 ...
 * regex with a lookup can only ever consume each entity once. (That half is real:
 * the same checker confirmed it is non-vacuous.)
 */
