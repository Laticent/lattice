...
function figureHtml(person, baseUrl) {
  if (person.photo?.src) {
    ...
    return `<span class="person-figure"><img class="person-photo" src="${escapeAttr(src)}" alt=""></span>`;
  }
 ...
 */
function rebuildLists(body, baseUrl) {
  let out = '';
  let rest = body;
  while (rest) {
    const list = extractFirstList(rest);
    if (!list) break;
    const items = parseTopLevelLis(list.inner).map((li) => personHtml(li, baseUrl)).join('');
...
function transformSection(inner, baseUrl) {
  ...
  return rebuildLists(rest, baseUrl) + coda;
}
