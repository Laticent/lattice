...
// The token boundaries are `(^|\s)` / `(\s|")`, NOT `\b`: the marker is hyphenated,
// and `\b` does not fire between `-` and a letter, so `\bteam-roster\b` also matched
// `my-team-roster` and `team-roster-legacy`. Nothing in the tree carries such a class,
// so this was a false comment rather than a live bug — but a future reader would
// have relied on it.
const MARKER_RE = new RegExp(`\\sclass="([^"]*\\s)?${MARKER}(\\s|")`);
...
/**
 * Escape only what can break OUT of a double-quoted attribute value.
 *
 * Deliberately NOT `&`: the input is already-escaped rendered HTML, where an
 * ampersand is already `&amp;`, so escaping it again is the double-escape that
 * 404'd query-string portraits. `<`/`>` cannot break the attribute on their own
 * but are cheap to neutralize and keep a malformed tag from being reinterpreted.
 */
function escapeAttrDelimiters(value) {
...
function figureHtml(person, baseUrl) {
  if (person.photo?.src) {
    ...
    // Escaped for the DELIMITER, not re-escaped whole — the two are different and
    // getting it wrong breaks a different thing each way.
    //
    // Not whole: the src was read out of ALREADY-escaped rendered HTML, so a full
    // `escapeAttr` turned `&amp;` into `&amp;amp;` and a portrait URL carrying a
    // query string (`?w=200&h=200`) resolved to a literal `&amp;` and 404'd. That
    // is why `&` is deliberately absent from the set below.
    //
    // But the delimiter DOES have to go, and the argument that it could not appear
    // was wrong. It rested on the source having passed markdown-it's escaper —
    // except `html: true` means it need not have. `readAttr` accepts a
    // SINGLE-quoted attribute precisely because an author can hand-write
    // `<img src='ada.svg'>`, and a single-quoted value may legally contain a `"`.
    // So `<img src='ada.svg" onerror="alert(1)'>` used to emit
    // `<img class="person-photo" src="ada.svg" onerror="alert(1)" alt="">` — a
    // value channel turned into a markup channel, live in the CLI HTML/`--player`
    // export, which no sanitizer downstream of the engine sees.
    // (`logo-marks.js`, the kernel this mirrors, is safe by a route this one does
    // not take: it reads double-quoted values only, so the value provably cannot
    // carry its own delimiter — and it still escapes the quote it does use.)
    const src = escapeAttrDelimiters(resolveAssetUrl(person.photo.src, baseUrl));
...
function applyToDom(root) {
  ...
  for (const sec of root.querySelectorAll(`section.${SECTION_CLASS}`)) {
    ...
    // Mirrors `rebuildRosters`, which only touches lists that are DIRECT children
    // of the stage (or of the section when there is no stage). A bare `:scope li`
    // is any-depth, so a section whose only `<li>`s sit inside a blockquote passed
    // this gate, had every image pinned by the loop below, and was then left
    // unchanged — narrowing the unconditional write rather than closing it.
    const scope = sec.querySelector(':scope > .cell-stage') || sec;
    if (!scope.querySelector(':scope > ul > li, :scope > ol > li')) continue;
