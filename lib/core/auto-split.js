...
const { splitEnvelope, balancedPerPage, chromeOf } = require('./split-envelope');

const SPLITTABLE = new Set(['item', 'row']);

// A page this splitter emitted that must never grow a SECOND cover — a native BODY page
// (`lat-split-native`) or the run's CLOSING page. It already has its cover; if it still
// overflows it must paginate FURTHER between its own members.
