...
function splitDoc(docHtml, capacityMap) {
  ...
  const body = splitSections(docHtml.slice(firstSlide))
    .map((p) => {
      ...
      //
      // A RECIPE THAT DECLINES FALLS THROUGH TO THE DERIVED AXIS — it does not end the slide's
      // chances of splitting. `return whole` here is what left a VARIANT unsplit whenever its
      // rendered DOM was not the shape its component's one strategy reads, and the census found
      // that on every re-authoring strategy: `compare-prose axis` renders numeral-led facet
      // cards instead of two panes, so `readSubjects` found no sides; `split-panel pullquote`
      // leads with the quote and carries no `<h2>`, so `readFeature` found no feature. Both hold
      // a plain top-level list of independent members, both clip at portrait, and neither split.
      // A strategy declining means "this is not my shape", which is not the same claim as "this
      // slide has no seam".
      ...
      // redline's reasoning away from the passage it explains. A recipe that declined the slide
      // outright never produced those members, so there is nothing to protect: the rendered DOM
      // is the only account of the seam left, and asking it is the same question a plain layout
      // is asked. The `count <= 1` guard below is what keeps a genuinely single-member slide
      // (redline's default and `annotated` shapes, one passage each) ringing.
