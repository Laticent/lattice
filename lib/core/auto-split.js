 ...
 * longer split. That is the rule's intent — `hard` is an authoring-comfort bound, and a slide
 * that fits does not need more pages — and it is why `lint:deck`'s `capacity-autosplit` (the
 * advisory that reports what the split will do) is the right home for the count signal.
 *
...
function resplitDoc(docHtml, overflow, capacityMap) {
  ...
  const body = splitSections(docHtml.slice(firstSlide))
    .map((p) => {
      ...
      const axis = cap.split ? (cap.split.axis || null) : deriveAxis(p.inner);
