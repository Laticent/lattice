...
//
// NO OTHER STRATEGY'S `null` EARNS THE FALLTHROUGH, and the nine are excluded for two distinct
// reasons rather than the one an earlier version of this comment gave.
//
// FIVE ARE VETOES — they return null to ENFORCE a scope their own comments state.
// `journey-stages` and `roadmap-horizons` keep the landscape board whole, `redline-blocks`
// keeps a single passage whole, `kanban-lanes` keeps a single-lane board whole, and
// `math-structures` refuses `stats` and `canvas` by name, ending "there is no seam". Falling
// through re-derives an axis the component deliberately does not have.
//
// FOUR SERVE READ-ACROSS COMPONENTS, and their null is not always "no collection" — that claim
// was here and was wrong. `cover-cards` returns null when `readCover` finds no masthead and
// `compare-options` when there is no `<h2>`; neither means the slide has nothing to divide. It
// means there is no TITLE to open a run with, and falling through would cut a read-across
// component — a table's rows, an option set — into unframed pages, which is the #1193 shape
// those treatments exist to forbid. Ringing there is also exactly what the merge base did, so
// nothing regresses by keeping it.
//
// A new strategy is NOT admitted here by default: it joins by showing that its null is a parse
// failure rather than either of those.
...
function splitDoc(docHtml, capacityMap) {
  ...
  const body = splitSections(docHtml.slice(firstSlide))
    .map((p) => {
      ...
      //
      // FOR FOUR STRATEGIES `null` IS NOT A PARSE FAILURE, IT IS THE VETO, and each says so in
      // its own comment: `journey-stages` returns null on the landscape grid because "the
      // landscape grid form stays whole and rings"; `roadmap-horizons` on the TABLE form for the
      // same reason; `redline-blocks` when a slide holds one passage; `kanban-lanes` on a
      // single-lane board. Falling through on those re-derives an axis the component
