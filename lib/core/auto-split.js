...
function applyRelationshipSignals(html, capacityMap) {
  ...
  for (const run of runs) {
    ...
    const membersOf = (idx) => {
      ...
      // So an empty stamp yields an empty member list, and `relationshipSignals` degrades to the
      // un-labeled `continues →` every kind now has. `undefined` — no attribute at all — still
      // means "this strategy stamps nothing", and still falls through unchanged.
      if (stamped !== undefined) return stamped ? [stamped] : [];
