...
const { splitEnvelope, balancedPerPage, partitionKeepingNote, chromeOf, withRole, injectTrailing } = require('./split-envelope');
...
function applyRails(html) {
  ...
  // inside a footer Cell goes `position: static; max-width: none` and so has no bounded berth
  // to offset from. Seen on a real portrait render: the k-of-N segments struck through the
  // section label. Frames with no footer Cell (the re-authored split cover/body pages) keep
  // the positioned rail, which clears the section rail's reserved 30cqi berth in CSS.
  const dock = (inner, rail) => {
    if (!inner.includes('class="cell-footer"')) return inner + rail;
    return inner.includes('class="lat-pagination"')
  ...
  return prefix + parts
    .map((p, idx) => {
      ...
      return `${tag}${rail ? dock(p.inner, rail) : p.inner}</section>`;
    })
    .join('');
}
...
function autoSplitDeck(html, capacityMap) {
  ...
  const out = splitSections(html).map((p) => {
    ...
    // (resplitDoc → carouselize), which re-authors its body as well as covering it. The
    // static count pass must not pre-split it between members here.
    if (!cap || cap.split || cap.hard == null || !SPLITTABLE.has(cap.axis)) return whole;
    const count = countAxis(p.inner, cap.axis);
    if (count <= cap.hard) return whole;
    // Balanced against the authored target, so no page of the run is a runt.
    const perSlide = balancedPerPage(count, splitTargetOf(cap));
    // The universal envelope (§0a): cover → native body pages → optional closing.
    const envelope = splitEnvelope(p.openTag, p.inner, chromeOf(p.inner), {
      axis: cap.axis,
      per: perSlide,
      layoutName: layoutTokenFor(p.cls, capacityMap),
    });
    if (envelope) {
      splits += 1;
      return stampRun(envelope, runIdOf(p.openTag));
    }
    // No masthead to cover with (a title-less slide) — an accent cover would be an
    // empty field, so the honest fallback is the bare partition: the heading (if any)
    // repeats with "(cont.)", exactly as before the envelope landed.
  ...
  // Splitting inserted pages, so the engine-baked page numbers are stale.
  return { html: splits ? repaginate(joined) : joined, splits };
}
...
function applyRelationshipSignals(html, capacityMap) {
  ...
  for (const run of runs) {
    ...
    // The axis the run was actually cut on: a carousel recipe's own axis when it has one
    // (a layout may author as a list and render as a table), else the capacity axis.
    const axis = cap.split?.axis || cap.axis;
...
module.exports = { autoSplitDeck, resplitDoc, capacityForClass, layoutTokenFor, repaginate, applyRails, applyRelationshipSignals };
