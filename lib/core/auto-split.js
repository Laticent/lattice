...
// The Form's own footer-Cell builder — a split page's band is the same shape as every
// other slide's, built by the same function (HARD RULE #1). See `ensureRunFooterCell`.
const { buildFooterCell } = require('../forms/cell/masthead/masthead.transform');
...
function applyRelationshipSignals(html, capacityMap) {
  ...
  for (const run of runs) {
    ...
    // The forward pointer docks in the run's footer Cell, which `ensureRunFooterCell`
    // guarantees exists on every page of a run (see there). #2219 suppressed the pointer on
    // coverless `split-panel` pages because without that Cell `dockInFooterCell` degrades to
    // appending at SECTION level, and on this layout the section IS the panel flex row: the
    // pointer became a third COLUMN and squeezed `.panel-right` from 450.9px to 252.7px at
    // `square`. With the Cell present it lands in the band like every other layout's, so the
    // suppression is retired rather than worked around.
  ...
  return prefix + parts
    .map((p, idx) => {
      ...
      //
      // WITH NO CONTENT CELL the seam does not exist, and `injectTrailing`'s fallback keys on a
      // trailing run of `<header>`/`<footer>`/`<nav>` ELEMENTS — which a `<div class="cell-footer">`
      // is not — so the mark lands as a direct child of the section. On a layout whose section IS
      // its content row (`split-panel` is a flex row) that makes the pointer a COLUMN: measured at
      // `square`, `.panel-right` squeezed 450.9px -> 252.7px and the pill 53px off-slide. So on
      // these pages it docks in the run's footer Cell instead, which is where the rail already
      // goes (`applyRails`, same helper) and which `ensureRunFooterCell` guarantees is present.
      const place = (inner, mark) => (/<div class="cell-stage"/.test(inner)
        ? injectTrailing(inner, mark)
        : dockInFooterCell(inner, mark));
      return `${p.openTag}${sig ? place(p.inner, sig) : p.inner}</section>`;
    })
    .join('');
}
...
/**
 * EVERY PAGE OF A RUN CARRIES A FOOTER CELL, even when its Frame suppresses one.
 *
 * A sovereign Frame (`split-panel`, `split-compare`, `compare-code`, `premise` — the four
 * of the nine that split at all; the rest are `NEVER_SPLIT`) declares
 * `suppresses: ["footer", "footer-left", "progress-center", "pagination-right"]` and claims
 * the whole canvas. That is right for a STANDALONE slide, which is one statement and needs
 * no furniture. It is wrong for a split RUN, which is multi-page and cannot be read without
 * wayfinding — which page is this, is there more.
 *
 * Every piece of that wayfinding docks INTO the footer Cell, and every one of them degrades
 * when it is missing: `footer-dock.js` appends the forward pointer at SECTION level
 * (`if (at < 0) return html + mark`), `progress.transform.js` places the rail only
 * `.cell-footer when present`, and the page number is built with the Cell or not at all.
 * Measured on a coverless `split-panel` body page before this: `cells: []`, with both marks
 * direct children of a section that IS the panel flex row — so the pointer became a THIRD
 * COLUMN and squeezed `.panel-right` from 450.9px to 252.7px at `square`.
 *
 * Two earlier attempts hand-rolled substitutes for this Cell — a reserved band with an
 * absolutely-positioned pill, then a declaration teaching the overflow probe to read that
 * band — and both were reverted. They were reinventing a construct the engine already has:
 * `engineering/decisions/2026-09-14-a-reserved-band-is-not-an-overflow-allowance.md`.
 *
 * The Cell is built with the KERNEL's own `buildFooterCell` (HARD RULE #1), never a local
 * string, so a split page's band is the same shape as every other slide's. It is emitted
 * EMPTY of text: the page number is stamped by `repaginate`, the rail by `applyRails` and
 * the pointer by `applyRelationshipSignals`, all of which run after this and all of which
 * look for `.cell-footer`. The run's own caption is NOT added here — `stripRunFooter` says
 * it once, on the page that opens the run, and that stays true.
 *
 * Runs AFTER `stripDeckChrome`, which drops a Cell the strip left empty; adding one before
 * that would be removed again.
 */
function ensureRunFooterCell(html) {
  const firstSlide = html.search(/<section\b[^>]*\bdata-lattice-slide=/);
  if (firstSlide < 0) return html;
  return html.slice(0, firstSlide) + splitSections(html.slice(firstSlide))
    .map((p) => {
      if (p.type !== 'section') return p.text;
      // Only pages that belong to a run, and only those without a Cell already.
      if (!/\sdata-split-run="/.test(p.openTag)) return `${p.openTag}${p.inner}</section>`;
      if (/<div class="cell-footer"/.test(p.inner)) return `${p.openTag}${p.inner}</section>`;
      const pagination = (p.openTag.match(/\sdata-lattice-pagination="([^"]*)"/) || [])[1] || '';
      // MOVE the page number the pagination Tile already minted; never mint a second.
      // Since #2206 the Tile puts a section-level `<span class="lat-pagination">` on any
      // paginated frame without one, so on these pages it is already a sibling. Building
      // the Cell around a fresh span would draw the number TWICE — once in the corner and
      // once in the band — and the Tile's own idempotence guard cannot repair it, because
      // that guard is a descendant query and a span inside the Cell still counts as
      // present. The DOM half of this Cell (`masthead-lift.js`) reuses the node for the
      // same reason; this is the string half doing the same thing.
      let inner = p.inner;
      const minted = inner.match(/<span class="lat-pagination">[\s\S]*?<\/span>/);
      if (minted) inner = inner.replace(minted[0], '');
      // THE RUN'S CAPTION MOVES INTO THE CELL TOO, and leaving it behind is a collision, not a
      // tidiness question. A `<footer>` on these pages is `position: absolute` in the same bottom
      // strip the Cell occupies, so an emptied Cell does not sit beside the caption — it sits ON
      // it. Measured on `split-panel.gallery.md` p9 at `square` before this line existed: the two
      // boxes overlapped 987.2x38.5px and the forward-pointer pill printed over the caption for
      // 305.0x38.5px of it. The DOM half of this Cell moves the trailing footer for the same
      // reason (`masthead-lift.js`, `if (trailingFooter) footerCell.appendChild(trailingFooter)`);
      // this is the string half doing the same thing, and the two halves disagreeing is what
      // produced the overprint.
      const caption = inner.match(/<footer\b[^>]*>[\s\S]*?<\/footer>/);
      if (caption) inner = inner.replace(caption[0], '');
      const captionHtml = caption ? caption[0] : '';
      // Re-minting from the attribute is equivalent to carrying the stripped node across:
      // the DOM half overwrites the reused node's text with this same attribute value
      // (`pag.textContent = pagination`), so both paths put the identical number in the
      // band. What matters is that the section-level one is GONE, not which span survives.
      //
      // `buildFooterCell` returns '' when it has neither text nor a number, so a run with
      // `paginate: false` would get nothing — yet its rail still needs somewhere to dock.
      // Ask for the Cell explicitly in that case.
      const cell = buildFooterCell(captionHtml, pagination) || '<div class="cell-footer"></div>';
      return `${p.openTag}${inner}${cell}</section>`;
    })
    .join('');
}
...
module.exports = {
  authoredIndexPerPage, splitDoc, capacityForClass, layoutTokenFor, repaginate, applyRails, applyRelationshipSignals,
  ensureRunFooterCell, stripDeckChrome, deckChromeFrom };
