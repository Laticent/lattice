...
//
// EVERY OTHER STRATEGY'S `null` IS A DECISION, and the distinction is the whole of it.
// `journey-stages`, `roadmap-horizons`, `redline-blocks` and `kanban-lanes` return null to
// ENFORCE a scope their own comments state (portrait-only, card-form-only, two passages, two
// lanes); `cover-paginate`, `cover-cards`, `compare-options` and `code-cards` route through the
// envelope already, so their null means there is no collection and re-deriving asks the same
// question twice. A new strategy is NOT admitted here by default: it joins by showing that its
// null is a parse failure rather than a veto.
