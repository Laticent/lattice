 ...
 *     by that ratio so it fits. This catches DENSITY overflow — a slide with
 ...
 *
 * There used to be a second entry point, `autoSplitDeck`: a pre-render pass that counted a
 * slide's collection against `capacity.hard` and handed every over-budget slide to the
 * measured loop as a candidate. It is gone (2026-07-29). A count is an AUTHORING-shape claim
 * and fit is a fact about glyphs in a box; the two disagree in both directions, and where they
 * disagreed the count won — so a slide authored past its budget that fit comfortably was cut
 * anyway. `capacity` speaks to the author through `lint:deck`; the engine's only question is
 * "does it fit". See engineering/decisions/2026-07-29-autosplit-is-not-a-toggle.md.
 *
...
const closingSignal = (label) => `<div class="lat-split-rel">&rarr; next: ${label}</div>`;
...
// The longest run the k-of-N rail still draws as DOTS. Past this it prints "k/N" instead.
//
// FOUR, which makes the count form the common case and dots the exception — the opposite of the
// balance before 2026-09-01, and the right way round now: one structural element per page means
// most runs are longer than four, so a dot per page is neither compact nor countable. A first
// attempt at eight was set from the readability side alone and did not fix the defect that
// prompted it — a six-dot rail still pushed a deck's `footer:` into an ellipsis, because the
// footer band is a shared width budget and the dots are its inflexible member. Four dots is the
// most the band carries with a full-length footer string beside it, measured at `portrait` on
// the two committed demo decks.
const RAIL_DOT_MAX = 4;
...
function applyRails(html) {
  ...
  for (const run of runs) {
    ...
    run.members.forEach((idx, k) => {
      ...
      //
      // Up to RAIL_DOT_MAX pages it is dots — a shape the eye reads at a glance without
      // counting. Past that it is the count itself. A dot rail does not degrade gracefully:
      // it grows linearly, and the footer band is a shared width budget (footer text · rail ·
      // page number), so a fourteen-dot rail pushed the deck's own `footer:` string into a
      // `text-overflow: ellipsis` — 21 pages of one demo deck reported "Content clipped", and
      // the thing being clipped was the footer, not the slide. Dots also stop MEANING anything
      // at that length: nobody counts fourteen of them to find they are on the ninth.
      //
      // The threshold is where the two failures meet — eight dots still read as a shape and
      // still leave the band room. Measured on the two committed demo decks at `portrait`.
