...
// `content` is NOT here, deliberately, and it used to be. The stated reason was
// that a prose layout claims its trailing `<p>` as main content — real on its
// face, but a case the STRUCTURAL guard below already prevents: BOTH promotion
// paths require the preceding sibling to be a DIV/UL/OL/TABLE/PRE/BLOCKQUOTE, so
// a paragraph after a paragraph is never promoted, on any layout. The exclusion
// only ever cost `content` the treatment when the note DID follow a structural
// block — no hairline, no muted ink, no --fs-body sizing, and after #1292 that
// would be every un-classed slide in the corpus.
// See engineering/decisions/2026-08-02-default-slide-layout.md §2.
const EXCLUDED = [
  'title', 'closing', 'quote', 'big-number', 'divider', 'image',
  'split-panel', 'split-compare', 'diagram', 'stats', 'code',
  'roadmap', 'progress', 'timeline-list', 'piechart', 'gantt', 'kanban',
  'image-razor', 'image-brief', 'image-chamber', 'math',
];

// The author's opt-out, for the case the exclusion list cannot express: a layout
// that SHOULD promote in general, on a slide where the trailing paragraph is meant
// to stay body copy.
//
// It exists because #1292 made `content` the default layout and #1322 took `content`
// off EXCLUDED — both correct — with a consequence nobody had a lever for: "a list,
// then a concluding sentence" is an ordinary prose shape, and promotion turns that
// conclusion into a footnote (muted ink, hairline rule, pushed to the stage floor).
// 5 slides across 3 decks land that way today. This does NOT change what any of them
// render; it gives the author the choice they did not have, per slide or deck-wide
// (`class: no-note`).
//
// A RE-ADDED `EXCLUDED` ENTRY WAS THE OTHER OPTION AND IS THE WRONG SHAPE. It would
// withhold the treatment from every `content` slide — including the un-classed ones
// #1292 created, which is most of the corpus — to serve the minority that wants the
// paragraph plain, and it would make `content` special again exactly where #1322 made
// it consistent with every other non-excluded layout. The default stays promotion; the
// escape hatch is per-slide.
//
// TOKEN-EXACT, not `cls.includes()` like the layout list above: that substring test is
// safe for component names (a real class list never contains `quote` as part of another
// token) but a suppression flag has to be, so a future `no-notebook` cannot silently
// disable notes.
...
// token and matches nothing — while the layout list beside it works on arrays for free,
// because `Array.prototype.includes` does. Two shapes, one answer.
function hasOptOut(cls = '') {
  const tokens = Array.isArray(cls) ? cls : String(cls).split(/\s+/);
  return tokens.includes(OPT_OUT_TOKEN);
}

// The `no-*` SUPPRESSION vocabulary is withheld from the layout test below, because that
// test is a SUBSTRING test and `no-progress` — the chrome modifier that hides the progress
// rail — contains `progress`, the CHART COMPONENT. A deck declaring `class: no-progress`
// therefore lost below-note promotion on every slide, from a token that says nothing about
// layout. (The comment beside `hasOptOut` above claims the substring test is safe because
// "a real class list does not contain `quote` as a fragment of another token". It does.)
//
// IT BECAME REACHABLE WHEN #1358 FIXED THE CLASS READ. Before that, below-note read
// `data-class` — the raw per-slide `_class:` payload — which never carried a deck-wide
// `no-progress`. Fixing the read handed the substring test the whole deck-wide register,
// which is a pre-existing fragility tipped into failure by this change and therefore this
// change's to fix, not to file (HARD RULE #18). The Studio's own "Hide rail" switch writes
// `class: no-progress` into front matter, so the reachable path is one click.
//
// NARROW ON PURPOSE. This does NOT make the layout list token-exact — the real fix, which
// flips 28 committed sections across decks HARD RULE #8 isolates and needs a design ruling
// on `compare-code` ⊃ `code` and `pull-quote` ⊃ `quote` (#1363). It withholds ONE thing: no
// `EXCLUDED` entry begins with `no-`, so a `no-*` token can never legitimately name a
// layout, and dropping them from the substring arm changes nothing else in the corpus.
const SUPPRESSION_PREFIX = 'no-';

function isExcluded(cls = '') {
  if (hasOptOut(cls)) return true;
  // An ARRAY is already token-exact (`Array.prototype.includes`), and has been since this
  // kernel was written — leave that path exactly as it was rather than loosening it to the
  // substring test the string path uses.
...
// the section content.
//
// THE BODY CAPTURE MUST NOT SPAN A `</p>`. With a plain lazy `[\s\S]*?` the `$`
// anchor makes the engine backtrack past the FIRST paragraph's close and swallow
// `structural + <p>a</p><p>b</p>` into ONE wrapper — promoting a paragraph that
// follows a paragraph, which `applyToDom` (guarding on `prev.tagName`) correctly
// refuses. That divergence is exactly what marp-fidelity.js asserts does not
// exist, and it is what made the claim "a paragraph after a paragraph is never
// promoted on any layout" false on the HTML path only. Excluding `</p>` from the
// capture makes the two paths agree. Found by the maker-checker pass on #1292,
// which newly exposed it by taking `content` off EXCLUDED. An optional trailing marp `<footer>` is matched but
...
const TRAILING_NOTE =
  /((?:<\/div>|<\/ul>|<\/ol>|<\/table>|<\/pre>|<\/blockquote>)\s*)<p>((?:(?!<\/p>)[\s\S])*?)<\/p>\s*(<footer\b[^>]*>[\s\S]*?<\/footer>)?\s*$/;
...
const STAGE_TRAILING_NOTE =
  /((?:<\/div>|<\/ul>|<\/ol>|<\/table>|<\/pre>|<\/blockquote>)\s*)<p>((?:(?!<\/p>)[\s\S])*?)<\/p>\s*$/;
...
function applyToHtml(html) {
  ...
  while (i < html.length) {
    ...
    // `readClassAttr`, never a bare `/class="…"/`: the section tag carries
    // `data-class="<raw _class: payload>"` BEFORE the resolved `class`, and the
    // leftmost match wins. Reading it raw is what made `class: no-note` fail on a
    // slide naming its own `_class:` — the deck-wide token was in the resolved
    // list and nowhere in the directive payload (#1358, lib/core/section-walk.js).
