...
//
// KEY ON THE CLASS, NEVER THE TAG. The stage cell's ELEMENT is not fixed: it is a
// `<div>` normally and a `<figure>` when it holds a captioned graphic
// (masthead.transform.js `stageTag`). A matcher pinned to `<div class="cell-stage">`
// finds nothing on a captioned chart slide and falls silently back to the flat
// section-level anchor — a wrong answer that renders, which is the worst kind. This
// repo has hit that same footgun in five separate places now; the fix is always the
// same, so it is stated as a rule here: match the class, capture the tag, balance the
// tag you captured.
const STAGE_OPEN_RE = /^<([a-zA-Z][a-zA-Z0-9-]*) class="cell-stage">$/;
function findStageOpen(inner) {
  ...
  while ((m = tagRe.exec(inner))) {
    ...
    if (depth === 0) {
      ...
      if (stage) return { index: m.index, tag: stage[1], openLen: full.length };
    }
    depth++;
  }
  return null;
...
function extractStage(inner) {
  ...
  const tagRe = new RegExp(`<${open.tag}\\b[^>]*>|</${open.tag}>`, 'g');
  tagRe.lastIndex = bodyStart;
  let depth = 1;
  let m;
  while ((m = tagRe.exec(inner))) {
    if (m[0][1] === '/') { if (--depth === 0) return { bodyStart, bodyEnd: m.index }; }
    else depth++;
  }
