 ...
 *
 * TWO DIFFERENCES from the string version, both forced by WHEN each runs:
 *   1. `.image-scrim` is in the keep-out list here. On the engine path the scrim
 *      is injected AFTER this fold (`applyImageStructure`), so it cannot be
 *      present; in the runtime the registry's image-scrim adapter runs BEFORE
 *      this, so it can.
 *   2. `.backdrop` likewise — the runtime's `injectBackdrops()` fires at bootstrap,
 *      before any content transform. Folding it into the panel would bury it and
 *      break `section.finish > .backdrop` (the hazard the engine avoids purely by
 *      ordering; see the comment on applyBackdropToHtml in lib/engine/index.js).
 *
 ...
 * absolutely positioned and header/footer are chrome, so sibling order carries no
 * layout meaning, and moving in place keeps the mirror from reordering an author's
 * content. Idempotent: a section that already has an `.image-text` is skipped.
 */
const IMAGE_TEXT_KEEP_OUT = ['lattice-bg', 'image-scrim', 'backdrop'];
...
function wrapImageTextToDom(root) {
  ...
  for (const section of scope.querySelectorAll('section.image')) {
    if (section.querySelector(':scope > .image-text')) continue; // idempotent
    const fold = [];
