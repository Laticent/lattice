 ...
 *
 * The trailing newline is OPTIONAL (`\r?\n?`), matching `parseFrontMatter`
 * (lib/engine/directives.js). Requiring it made a front-matter-only file — no
 * body at all — read as one slide of raw YAML rather than as an empty deck, so
 * the reconstruction reported a slide the engine does not render.
 */
