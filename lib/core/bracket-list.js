...
function tidy(s) {
  ...
  // One pass to decide whether any INTERNAL run needs collapsing. When none
  // does — the normal case — we slice at most once and never build a buffer.
  let runStart = -1;
  for (let i = a; i < b; i++) {
    if (isWs(src.charCodeAt(i))) {
      if (runStart >= 0) {
        // A real run. Fall back to building the collapsed string, once, from
        // the START of the run — not from here, which would keep its first
        // space and collapse `Wide   reach` to `Wide  reach`.
        let out = src.slice(a, runStart);
        let ws = true;
        for (let j = runStart; j < b; j++) {
          const c = src.charCodeAt(j);
          if (isWs(c)) { ws = true; continue; }
          if (ws) { out += ' '; ws = false; }
          out += src[j];
        }
        return out;
 ...
 *
 * An empty part is DROPPED rather than kept as a blank: `[A, , B]` is a typo,
 * and a blank axis name renders as a missing label that reads as an engine
 * fault. A list whose parts all drop returns `null`, for the same reason
 * `label-set.js` refuses `[{}]` — far likelier a mistake than a request.
 *
 ...
 * commas. Without the cap those two readings would need two parsers, which is
 * the duplication this module exists to avoid (HARD RULE #1).
 *
...
function parseBracketList(text, opts) {
  ...
  const flushMember = () => {
    flushPart();
    if (parts.length) members.push(parts);
    parts = [];
  };
  ...
  for (let i = lo + 1; i < hi - 1; i++) {
    ...
    if (c === 125 /* } */) { if (depth > 0) { depth--; flushPart(); } continue; }
    if (c === 44 /* , */) {
      if (depth > 0) {
