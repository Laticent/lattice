...
function parseBracketList(text, opts) {
  ...
  const members = [];
  let parts = [];
  // A quote's partner must END A PART — be followed by optional whitespace and
  // then `,` `}` or the closing `]`. Any later quote of the same kind was not
  // enough: in `['90s cohort, Customer's spend]` the apostrophe in "Customer's"
  // was taken as the partner and the comma was swallowed again. The ending
  // positions of each kind are collected once, right to left, so the check is a
  // lookup rather than a scan per quote.
  const ends = { 34: [], 39: [] };
  ...
  // `ends[q]` is descending; the partner is the nearest ending quote after i.
  const endPtr = { 34: ends[34].length - 1, 39: ends[39].length - 1 };
  const closesPart = (q, at) => {
    const list = ends[q];
    let p = endPtr[q];
    while (p >= 0 && list[p] <= at) p--;
    endPtr[q] = p;
    return p >= 0;
  };
  let depth = 0;
  ...
  for (let i = lo + 1; i < hi - 1; i++) {
    ...
    // …and only when its partner exists further on. An unclosed quote ran to the
    // end and swallowed every later member: `['90s cohort, Revenue]` was ONE.
    // The last index of each quote kind is found once, so this stays O(1).
