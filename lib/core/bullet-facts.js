...
//
// THE THIRD NAME IS A TRAP FOR CALLERS AND IS LABELLED AS ONE. The top derived band
// runs from 85% of target UPWARD with no ceiling, so it holds a row that just missed
// the tick and a row that doubled it alike. "At or past plan" is true of the band's
// upper reach and false of its floor, and a caller that spoke it on a short row
// produced "ninety-six percent of plan, at or past plan" — a sentence arguing with
// itself. Check `index` before speaking a name: only 0 and 1 sit wholly below the
// tick and are safe on any row.
//
...
const DERIVED_ZONE_NAMES = Object.freeze(['well short of plan', 'closing on plan', 'at or past plan']);
