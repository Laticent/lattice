...
const {
  splitEnvelope, balancedPerPage, readCover, coverSection, roleOpenTag, chromeOf, footerCell, stripChrome, introOf, deriveAxis,
  splitRegions, insightPage, insightPageFrom, trailingMaterialOf, trailingSlotMaterialOf, markNote, injectTrailing, injectTrailingInSection, withRole, removeSpans,
...
const { tagHasClass } = require('./coda');
...
/**
 * The inner HTML of a `.below-note` wrapper's paragraph, or null.
 *
 * ROUTED THROUGH THE KERNEL'S PREDICATE (HARD RULE #1) rather than matching the wrapper
 * with a regex of its own. Two things forced this. The wrapper gained a second class
 * (`.is-annotation`, lib/core/coda.js), which an exact-string match stops seeing; and the
 * regex that replaced the exact-string one still ended `"\s*>`, so it silently required
 * `class` to be the LAST attribute while `tagHasClass` accepts it anywhere — two
 * implementations of one question, already disagreeing on
 * `<div id="x" class="below-note">`. Unreachable today, and exactly the divergence the
 * rule exists to stop.
 */
function belowNoteParagraph(html) {
  const tagRe = /<div\b[^>]*>/g;
  let m;
  while ((m = tagRe.exec(html))) {
    if (!tagHasClass(m[0], 'below-note')) continue;
    const p = html.slice(m.index + m[0].length).match(/^\s*<p[^>]*>([\s\S]*?)<\/p>/);
...
function redlineBlockSplit(openTag, inner) {
  ...
  // trio, one per pass.)
  //
  // So cut exactly what the hoist will rescue, and no more. A RESCUABLE extra (trailing, so
  // `insightPageFrom` gives it the closing slide per rule 9) leaves both body pages. A STRANDED
  // one is dropped from page 1 only and rides page 2 — the last body page, which is where the
  // envelope puts trailing material it cannot give a page of its own. Either way it appears
  // exactly once.
  const rescuable = new Set(trailingMaterialOf(inner).insight.map((s) => `${s.start}:${s.end}`));
  ...
  return [
    `${slideTag}${mark(drop([bqs[1], note, ...trailing, ...extra, ...stranded]), 'rl-old')}</section>`,
...
function kanbanLaneSplit(openTag, inner) {
  ...
  const cols = directChildren(boardInner, 'div')
    .filter((s) => /^<div\b[^>]*\sclass="[^"]*\bkanban-column\b/.test(boardInner.slice(s.start, s.end)));
...
//
// THE BUDGET. Owner's call (#1209): 2–4 parts, 4 being the max. One card per page reads
// best, so a 2–4 phase roadmap paginates 1-per-page; beyond that the cards group evenly
// rather than spilling past the budget, so a 6-phase roadmap is 2+2+1+1 and never 6 pages.
// `evenGroups` does the balancing (every page within 1 of every other).
const MAX_HORIZON_PAGES = 4;

function roadmapHorizonSplit(openTag, inner) {
  const gridOpen = inner.indexOf('<div class="horizons"');
  if (gridOpen < 0) return null;                       // table form — atomic, rings
  const afterOpen = inner.indexOf('>', gridOpen) + 1;
  const chrome = chromeOf(inner);
  // Prefix starts at index 0 and so carries the section's own <header>; strip it or
  // `${header}${pfx}` below emits it twice (the redline/kanban defect).
  const prefix = stripChrome(inner.slice(0, afterOpen), chrome);
  if (!grab(prefix, /<h2[^>]*>([\s\S]*?)<\/h2>/)) return null;
  const gridInner = inner.slice(afterOpen);
  // `.horizon-card` children ONLY — Form nests the footer CELL inside the figure, and
  // taking every div child would count it as a phase and emit a spurious final page
  // holding nothing but a doubled footer (the bug kanban-lanes hit on a real render).
  const cards = directChildren(gridInner, 'div')
    .filter((s) => /^<div\b[^>]*\sclass="[^"]*\bhorizon-card\b/.test(gridInner.slice(s.start, s.end)));
  if (!cards || cards.length < 2) return null;
  // Everything after the last card: the status key, the footer cell, and the grid /
  // figure / chart-body / stage closes. Kept whole (not chrome-stripped) so each page
  // gets a real `.cell-footer` for `repaginate` to re-stamp — which is also why this
  // strategy must NOT append chrome.footer/chrome.rail; that would be a second copy.
  const suffix = gridInner.slice(cards[cards.length - 1].end);
  const { header } = chrome;
  const baseCls = (openTag.match(/\sclass="([^"]*)"/) || ['', ''])[1];
  const idless = openTag.replace(/\s+id="[^"]*"/, '');
  const pageTag = (first) => withRole(
    (first ? openTag : idless).replace(/(\sclass=")[^"]*(")/, `$1${baseCls} lat-split-native$2`), 'body');
  // perSlide from the budget: ceil(N / 4) cards per page caps the run at 4 pages, and
  // evenGroups then balances them (never 3+3+3+1 when 3+3+2+2 is available).
  const perSlide = Math.ceil(cards.length / MAX_HORIZON_PAGES);
  const out = [];
  let i = 0;
  for (const size of evenGroups(cards.length, perSlide)) {
    const group = cards.slice(i, i + size);
    const html = gridInner.slice(group[0].start, group[group.length - 1].end);
    const pfx = out.length === 0
      ? prefix
      : prefix.replace(/<\/(h[12])>/, ' <span class="lat-cont">(cont.)</span></$1>');
    out.push(`${pageTag(out.length === 0)}${header}${pfx}${html}${suffix}</section>`);
    i += size;
  }
  return out;
}
...
function splitCoverRows(inner, ctx) {
  ...
  const cover = { heading, lede: belowNoteParagraph(inner), intro: introOf(ctx.recipe, rows.length) };
...
function splitCoverSides(inner, ctx) {
  ...
  const heading = grab(inner, /<h2[^>]*>([\s\S]*?)<\/h2>/) || '';
  const synthesis = belowNoteParagraph(inner);
  ...
  if (synthesis) {
    ...
    parts.push(`${roleOpenTag(ctx.openTag, cls('verdict'), false, 'insight')}${header}<div class="split-runhead">The verdict</div><div class="split-pullq">${synthesis}</div>${footerCell(ctx.openTag, ctx.chrome)}</section>`);
  }
...
function carouselize(openTag, inner, recipe, ratio, layoutName) {
  ...
  // TEXT against TEXT. Comparing `textOf(span)` to the RAW emitted HTML looked equivalent and was
  // not: any inline markup in the material (`*italic*`, a `<code>` span, a link) — or a soft line
  // wrap in the markdown source — makes the stripped text absent from the raw bytes even though the
  // material IS on a page. The hoist then emitted a SECOND copy: on the committed
  // `examples/split-envelope.md` compare-table slide, italicising one word in the key insight
  // produced two pixel-identical insight slides, and an `<em>` in a below-note printed it twice on
  // the same page. Found by the HARD RULE #25 red team on a real export; the rule-6 conservation
  // gate is structurally blind to it, since a multiset-containment check only reports shortfalls.
  const emittedText = () => textOf(parts.join(''));
  const carried = (spans) => {
