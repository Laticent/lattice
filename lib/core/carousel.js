...
function carouselize(openTag, inner, recipe, ratio, layoutName) {
  ...
  // manifest typo like strategy:"toString" would return a truthy non-splitter
  // (or crash on "hasOwnProperty") instead of the unknown-strategy null the
  // old if-chain gave. Manifests do not validate split.strategy, so degrade
  // gracefully here. (Found by the PR's independent checker.)
