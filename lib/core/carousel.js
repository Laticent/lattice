...
function stageMembers(inner, tag, keep = () => true) {
  ...
  return directChildren(cell, tag)
    .map((sp) => ({ start: b.start + sp.start, end: b.start + sp.end }))
...
function splitMathStructures(inner, ctx) {
  ...
  // 1 + 2 — equation + legend (`<ul>` → `item`) and step table (`<table>` → `row`). The shared
  // paginator resolves which off the rendered DOM and repeats the equation, the heading and a
  // table's `<thead>` on every body page.
  const paginated = coverPaginateSections(openTag, inner, chrome, recipe, ratio, layoutName);
  if (paginated && paginated.length >= 2) return paginated;
  // 3 — card stack (`theorem`): sibling `<blockquote>`s, one per page. `stats` has exactly ONE
  // blockquote (its CI/p-value panel), so the `< 2` refusal below is what keeps that scaffold
  // whole without naming it.
  // The card's own title is a LEADING `<strong>` ("**Definition.**"), styled positionally rather
  // than classed, so the label is asked for by TAG — the same `tag:` form `split-compare` uses
  // for its option titles.
  const cards = mathMemberSplit(openTag, inner, chrome, recipe, layoutName, stageMembers(inner, 'blockquote'), 'tag:strong');
  if (cards) return cards;
  // 4 — columns (`compare`): one `### label` group per page.
  const cols = mathMemberSplit(openTag, inner, chrome, recipe, layoutName, mathColumnGroups(inner), 'tag:h3');
  if (cols) return cols;
  // `canvas` lands here — an equation, a reading, and a plot, with no repeating collection to
  // slice — and so does any math slide whose author wrote a shape none of the four arms
  // recognizes. Null is the ring, which is the honest answer: there is no seam.
  return null;
}
