...
function readRows(inner) {
  ...
  const rows = directChildren(body, 'li')
    .map((s) => {
      ...
      // A LOOSE list (blank lines between items — ordinary authoring) has markdown-it wrap the
      // item's content in `<p>`, and `.split-pt-t` is a SPAN: `<span><p>…</p></span>` is invalid
      // (a `<p>` is not phrasing content). The wrapper is unwrapped, the inline markup inside it
      // kept — which is the whole point of not flattening. Found by the HARD RULE #25 checker.
      const rawTitle = (sub >= 0 ? liInner.slice(0, sub) : liInner).trim();
      const title = /^<p\b[^>]*>[\s\S]*<\/p>$/.test(rawTitle) && rawTitle.indexOf('<p', 1) < 0
        ? rawTitle.replace(/^<p\b[^>]*>/, '').replace(/<\/p>$/, '').trim()
        : rawTitle;
...
function codeLines(codeInner) {
  ...
  // silent drop is wrong wherever it sits. The third alternative takes the stray `<` as text.
  const re = /<\/?[a-zA-Z][^>]*>|[^<]+|</g;
...
function compareOptionSections(openTag, parsed, chrome, intro) {
  const { header } = chrome;
  ...
  const wrap = (klass, first, body, role) => `${roleOpenTag(openTag, klass, first, role)}${header}${body}${footerCell(openTag, chrome)}</section>`;
...
function splitCompareOptions(inner, ctx) {
  ...
  const parts = compareOptionSections(ctx.openTag, parsed, ctx.chrome, introOf(ctx.recipe, parsed.options.length));
