...
const { textOf } = require('./relationship');
...
function withMemberLabelAt(tag, memberHtml, open, elName) {
  ...
  // The tag strip is the KERNEL's (`textOf`), not a fourth hand-rolled copy of it. Three copies of
  // that one-liner in the test tree drew three high-severity CodeQL alerts for incomplete
...
function mathColumnGroups(inner) {
  ...
  return heads.map((h, k) => ({ start: h.start, end: k + 1 < heads.length ? heads[k + 1].start : b.end }));
}
