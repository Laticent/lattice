...
function withMemberLabel(tag, memberHtml, labelClass) {
  ...
  if (labelClass.startsWith('tag:')) {
    ...
    const openAt = new RegExp(`<${name}\\b[^>]*>`, 'i').exec(memberHtml);
    if (!openAt) return tag;
    return withMemberLabelAt(tag, memberHtml, openAt, name);
  }
...
function mathMemberPages(openTag, inner, members, label) {
  ...
  // The trailing beats come off EVERY page — they close the run on the shared closing page
  // (`carouselize`'s hoist). Left in, a slide's key insight would print once per member.
  //
  // SCOPED BY THE LAYOUT CLASS, unlike the native-slice kernel's unscoped strip, because `math`
  // CLAIMS `blockquote` and `trailing-paragraph` and here the claim is true: the theorem cards
  // and a column's prose ARE the members. Unscoped, the trailing scan read all three theorem
  // cards as one contiguous trailing run and this dropped every one of them from every page —
  // measured, three body pages with an empty stage. See MEMBER_CLAIM_STRATEGIES.
  const beats = trailingBeatsOf(inner, classOf(openTag));
  return members.map((m, k) => {
...
function mathMemberSplit(openTag, inner, chrome, recipe, layoutName, members, label) {
  ...
  return [cover, ...mathMemberPages(openTag, inner, members, label)];
}
...
function splitMathStructures(inner, ctx) {
  ...
  const paginated = coverPaginateSections(openTag, inner, chrome,
    withIntro(recipe, deriveAxis(inner) === 'row' ? MATH_INTRO.row : null), ratio, layoutName);
  if (paginated && paginated.length >= 2) return paginated;
 ...
 *
 * `math-structures` PASSES THE TEST ON EVERY ARM, and it was measured failing without the entry.
 * `math` claims `blockquote` and `trailing-paragraph`, and on three of its four structures the
 * claimed element IS a member: `theorem`'s Definition/Theorem/Proof cards are the blockquotes,
 * and `compare`'s per-column prose is the trailing paragraph of its group. Rendered without this
 * entry, the hoist read all three theorem cards as one contiguous trailing run, cut them from
 * every body page, and re-emitted them on a closing page — three empty pages and a card dump
 * (`examples/math-split-structure.md` at portrait). On the fourth structure, equation+legend, the
 * entry is what keeps the hoist from DOUBLING material `splitEnvelope` already resolved: that
 * path passes the layout class to `splitRegions` itself, so a claimed beat stays in the trunk by
 * design and a second, class-blind scan here would give it a closing page as well.
 *
