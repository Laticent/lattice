...
function readRows(inner) {
  ...
  const rows = directChildren(body, 'li')
    .map((s) => {
      ...
      const title = (sub >= 0 ? liInner.slice(0, sub) : liInner).trim();
...
function splitCodeCards(inner, ctx) {
  ...
  const baseCls = (openTag.match(/\sclass="([^"]*)"/) || ['', ''])[1];
  const idless = openTag.replace(/\s+id="[^"]*"/, '');
  const pageTag = () => withRole(idless.replace(/(\sclass=")[^"]*(")/, `$1${baseCls} lat-split-native$2`), 'body');
  ...
  if (blocks.length > 1) {
    const pages = blocks.map((keep, k) => {
      ...
      return `${pageTag()}${k === 0 ? page : cont(page)}</section>`;
    });
    const cover = mast
