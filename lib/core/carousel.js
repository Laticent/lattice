...
function redlineBlockSplit(openTag, inner) {
  ...
  // A THIRD (or later) top-level blockquote is neither passage — it is the run's key insight,
  // and rule 9 gives that its own closing page. It belongs to NEITHER body page, so cut it from
  // both. Leaving it in was FM-2 by another route: it is in no drop-set, so it survived on both
  // pages, and the rule-6 conservation hoist then saw its text on an emitted page and stood
  // down — a containment check can only report a shortfall, never a duplicate, so the gate is
  // blind to this by construction. Removed here, the hoist's `carried()` correctly reports it
  // missing and `insightPageFrom` gives it the closing slide. (HARD RULE #25 checker, on a
  // three-blockquote redline: 2 pages, roles body/body, the insight printed twice.)
  const extra = bqs.slice(2);
  ...
  return [
    `${slideTag}${mark(drop([bqs[1], note, ...trailing, ...extra]), 'rl-old')}</section>`,
