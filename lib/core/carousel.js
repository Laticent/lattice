...
function splitCoverRows(inner, ctx) {
  ...
  const cover = { heading, lede: grab(inner, /\sclass="(?:[^"]*\s)?below-note(?:\s[^"]*)?">\s*<p[^>]*>([\s\S]*?)<\/p>/), intro: introOf(ctx.recipe, rows.length) };
...
function splitCoverSides(inner, ctx) {
  ...
  const heading = grab(inner, /<h2[^>]*>([\s\S]*?)<\/h2>/) || '';
  const synthesis = grab(inner, /\sclass="(?:[^"]*\s)?below-note(?:\s[^"]*)?">\s*<p[^>]*>([\s\S]*?)<\/p>/);
