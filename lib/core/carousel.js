...
const {
  ...
  splitRegions, trailingMaterialOf, trailingSlotMaterialOf, withRole, removeSpans, readMasthead,
...
function redlineBlockSplit(openTag, inner) {
  ...
  const drop = (spans) => removeSpans(inner, [...spans, ...trailingBeatsOf(inner)].filter(Boolean));
  return [
...
function kanbanLaneSplit(openTag, rawInner) {
  ...
  const inner = withoutTrailingBeats(rawInner);
...
function carouselize(openTag, inner, recipe, ratio, layoutName) {
  ...
  const material = trailingSlotMaterialOf(inner);
  ...
  // Rule 9 allows ONE closing slide. A strategy that still emits an `insight`-role page
  // (`redline-blocks` leaves one for a rescuable extra blockquote) re-stamps to `body`, so the
  // run ends on the closing page and the invariant holds. Only the ROLE changes — the page
  // keeps its class, so its finish is byte-identical.
