...
const { topLevelElements } = require('./coda');
const { findMatchingClose } = require('./find-matching-close');
const { safeName } = require('./relationship');
...
// The stage cell's own bounds — a math slide's structure lives INSIDE `.cell-stage`, and the
// two bespoke arms below must not read a `<blockquote>` from the coda cell beside it.
...
// A LOOSE list's `<p>` wrapper, off. Blank lines between items is ordinary authoring, and
// markdown-it then wraps each item's content in `<p>`. Both of this file's member readers put
// that content inside a `<span>` — `.split-pt-t` for the title, `.split-pt-b` / `.split-pt-line`
// for the body — where a block-level `<p>` computes `display: block` and breaks the line the
// span exists to hold together.
//
// WHAT THIS DOES NOT DO, because an earlier comment here, its test and its changelog entry all
// claimed it did: the browser does NOT close the span and reparent the paragraph. Measured in
// real Chromium over `setContent` — `<span class="split-pt-t"><p>…</p></span>` parses with the
// `<p>` still inside the span, and the label stays in its row. `<p>` is not phrasing content, so
// the markup is invalid per spec, and Chromium's parser does not act on that here. The reason to
// unwrap is the layout one above, and only that one. Found by the HARD RULE #25 checker.
//
// ONE paragraph spanning the whole string, or nothing: a value holding two paragraphs is not a
// wrapper to peel, and flattening the pair would silently join two blocks into one line.
//
// THE "ONLY ONE" TEST IS ON `<p` WITH A TAG-NAME BOUNDARY, not on the substring `<p`. The first
// cut used `v.indexOf('<p', 1) < 0`, which also matches `<pre>`, `<picture>` and `<param>` — so
// a title reading ``Label `z` `` with a fenced sample in it was never unwrapped, silently and
// for a reason the docblock did not mention. `<pre>` inside a list item is authorable.
//
// AND AN EMPTY WRAPPER IS LEFT ALONE. `<p></p>` unwraps to `''`, and `readPoints` / `readSubjects`
// filter members on `title && body` — so peeling an empty paragraph turns a member that survived
// into one that is DROPPED. Conservation beats tidiness: an empty wrapper is returned as it came,
// and the member is still a member. Both found by the HARD RULE #25 checker.
const ONLY_INNER_P = /<p(?=[\s/>])/g;
const unwrapLoneParagraph = (html) => {
  const v = String(html || '').trim();
  if (!/^<p\b[^>]*>[\s\S]*<\/p>$/.test(v)) return v;
  if ((v.match(ONLY_INNER_P) || []).length !== 1) return v;
  const inner = v.replace(/^<p\b[^>]*>/, '').replace(/<\/p>$/, '').trim();
...
function subjectBody(li) {
  ...
  // `unwrapLoneParagraph` per bullet, for the reason it states: the body lands inside
  // `.split-pt-b` / `.split-pt-line`, the same span shape the title lands in, so a loose list's
  // wrapper has to come off on BOTH halves of a member. The title-only fix shipped first and
  // left this one — a member's two halves held to different standards, which is the same defect
  // this reader was already carrying for a different reason.
  const bullets = directChildren(body, 'li')
    .map((s) => unwrapLoneParagraph(body.slice(s.start, s.end).replace(/^<li[^>]*>/, '').replace(/<\/li>$/, '')));
...
function coverWindow(openTag, cover, items, chrome, perPage, cls, layoutName) {
  const { header } = chrome;
  // `layoutName` is PASSED to `roleOpenTag`, not left to be inferred from `klass`: two of this
  // file's role strings are literals that do not contain the layout token (`compare-prose`
  // renders `compare-split-*` classes), so the inference fell to dropping token zero and lost
  // the author's modifier. See `roleOpenTag`'s docblock.
  const wrap = (klass, first, body, role = first ? 'cover' : 'body') => `${roleOpenTag(openTag, klass, first, role, layoutName)}${header}${body}${footerCell(openTag, chrome)}</section>`;
  const out = [];
  ...
  for (const size of evenGroups(items.length, per)) {
    const rows = items
      .slice(i, i + size)
      // The body span is emitted only when the member HAS a body. Interpolating a null body
      // printed the literal string "null" onto the page, which is what a bodyless member would
      // have rendered the moment `readRows` stopped dropping them.
      .map((p) => {
        const t = `<span class="split-pt-t">${p.title ?? p.label}</span>`;
        return `<li>${t}${p.body ? `<span class="split-pt-b">${p.body}</span>` : ''}</li>`;
      })
      .join('');
...
function featureCoverSections(openTag, feat, chrome, recipe, layoutName) {
  return coverWindow(openTag, { ...feat, intro: introOf(recipe, feat.points.length) }, feat.points, chrome, recipe.perPage, (role) => `content split-panel-split split-panel-${role} form`, layoutName);
}
...
// Does a fragment carry any visible text? The membership test for a re-authored member, which
// may legitimately be nothing but an inline `<code>` chip.
const visibleOf = (html) => String(html || '').replace(/<[^>]*>/g, ' ').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
...
function readRows(inner) {
  ...
  const rows = directChildren(body, 'li')
    .map((s) => {
      ...
      // THE TITLE KEEPS ITS INLINE MARKUP. It used to be flattened with
      // `.replace(/<[^>]+>/g, '')`, which threw away the `<code>` chip that IS the register's
      // second column — a `list-tabular def` row authored `1. Label \`Term\`` reached its split
      // page as the bare word "Label". The body span never flattened (it comes from
      // `subjectBody`), so one member's two halves were held to different standards. Nothing
      // downstream needs text: `labelOf` (lib/core/relationship.js) reads `.split-pt-t` through
      // `textOf`, which strips tags itself.
      // A loose list's `<p>` wrapper comes off here and in `subjectBody` — see
      // `unwrapLoneParagraph` for what it does and, more usefully, for what it does NOT do.
      const title = unwrapLoneParagraph(sub >= 0 ? liInner.slice(0, sub) : liInner);
      return { title, body: sub >= 0 ? subjectBody(li) : null };
    })
    // A ROW WITH NO CLAUSE IS STILL A ROW. The filter used to require `r.body`, so a FLAT
    // register — `1. Coverage \`98.4% of policies\``, no nested clause — yielded zero rows and
    // the strategy declined: 5 of `list-tabular`'s 14 variants (metric, register, solid,
    // outline, fit-meta) never split at all, measured on the component's own gallery at
    // portrait. Worse, a MIXED list lost its flat rows silently — they were dropped from the
    // member set while the strategy still succeeded on the rest, and the rule-6 conservation
    // gate is a containment check, so a shortfall of this shape is exactly what it reports
    // only when the whole strategy runs. `title` alone is the membership test now; a bodyless
    // member renders without its body span (`coverWindow`).
    .filter((r) => visibleOf(r.title));
...
function coverCodeSections(openTag, code, chrome, intro, layoutName) {
  const { header } = chrome;
  const wrap = (klass, first, body, role = first ? 'cover' : 'body') => `${roleOpenTag(openTag, klass, first, role, layoutName)}${header}${body}${footerCell(openTag, chrome)}</section>`;
  const out = [];
...
// ── cover-cards strategy (table portrait — the RESHAPE move) ───────────
 ...
 * content.
 *
 * THREE STATES, not two. No attribute means "this strategy names nothing" — a member with no
 * title element at all — and `applyRelationshipSignals` falls back to its own heuristic, which is
 * what every page did before this existed. An attribute with a value is the name. An EMPTY one
 * means the title was found and the label rules declined it, and the reader must NOT fall back
 * there: the heuristic is known to be wrong on these strategies (a `kanban` card names the first
 * row inside it), which is why the stamp exists.
 *
 * This docblock said the opposite for one commit — "a title that is empty once tags are stripped
 * is left unstamped rather than stamped blank" — describing the behavior the empty state
 * replaced. A comment that is wrong is worse than none (HARD RULE #30).
 */
function withMemberLabel(tag, memberHtml, labelClass) {
  if (!labelClass) return tag;
  // A member whose title element carries NO class — `split-compare`'s `.option` titles itself
  // with a bare `<strong>`, styled positionally as `.option > strong`. Passing a TAG NAME finds
  // it, and everything after this point (depth-aware close, strip, decode, escape, cap) is
  // shared rather than copied into a fourth caller. Keyed on the `tag:` prefix so a class named
  // `strong` could never be read as a tag by accident.
  if (labelClass.startsWith('tag:')) {
    const name = labelClass.slice(4);
    // LEADING, not "anywhere in the member". Both callers document a leading title — a theorem
    // card is `> **Definition.** …`, a split-compare option `- **Build in region.** …` — and an
    // unanchored match took the first `<strong>` in the body instead: a card written
    // `> A theorem about **compactness**.` labeled its page, and the previous page's forward
    // pointer, "compactness". Same defect `labelOf` records for its own `<strong>` read, and the
    // same fix: up to three opening tags may precede it, because markdown wraps a loose list
    // item's first run in a `<p>` and a card in a `<blockquote><p>`.
    const openAt = new RegExp(`^(?:\\s*<[a-z0-9]+\\b[^>]*>\\s*){0,3}<${name}\\b[^>]*>`, 'i').exec(memberHtml);
    if (!openAt) return tag;
    // `exec` on an anchored pattern reports index 0; the title's own open tag is the LAST one.
    const at = openAt[0].lastIndexOf('<');
    return withMemberLabelAt(tag, memberHtml, { index: at, 0: openAt[0].slice(at) }, name);
  }
  ...
  // `<div class="kanban-column-header"><strong>Backlog</strong> and triage</div>` and labeled
  ...
  const open = new RegExp(`<([a-z0-9]+)\\b[^>]*\\sclass="[^"]*(?<![\\w-])${labelClass}(?![\\w-])[^"]*"[^>]*>`, 'i')
    .exec(memberHtml);
  if (!open) return tag;
  return withMemberLabelAt(tag, memberHtml, open, open[1]);
}

/** The shared tail of `withMemberLabel`: slice the title's text, sanitize it, stamp it. */
function withMemberLabelAt(tag, memberHtml, open, elName) {
  // `findMatchingClose` wants the index OF the `<` and returns the index AFTER the close.
  const from = open.index + open[0].length;
  const end = findMatchingClose(memberHtml, elName, open.index);
  if (end < 0) return tag;
  const inner = memberHtml.slice(from, end - `</${elName}>`.length);
  ...
  // The tag strip is the KERNEL's (inside `safeName`), not a fourth hand-rolled copy of it. Three
  // copies of that one-liner in the test tree drew three high-severity CodeQL alerts for incomplete
  // multi-character sanitization; this was the fourth, and it went the same way rather than
  // waiting for its own alert. Strip THEN decode, never the reverse: decoding first would turn an
  // author's `&lt;script&gt;` into a real tag for the strip to miss.
  // `safeName`, not `textOf` — the SPLITTER's stamp is a label, so it owes the label rules.
  // `applyRelationshipSignals` hands this attribute to `labelOf` as an already-FLATTENED plain
  // string: `dropLeadEquations` finds no `.katex` span in it and `mathSafe`'s span-scoped test
  // returns −1, so both guards no-op. A theorem card titled `**$A \to B$ Theorem.**` shipped the
  // chip `A→B Theorem`, and one titled `**$f(x) = y$ Theorem.**` shipped `f(x)=y Theorem` — a
  // typed shape glyph and a raw equation, the two defects this branch's own docblocks say it
  // removed, through the arm this branch created. Declining leaves the page unstamped, which is
  // the fallback that already existed. (HARD RULE #25 checker, final pass; HARD RULE #18.)
  const text = safeName(inner)
    .replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
    .replace(/&#39;|&apos;/g, "'").replace(/&amp;/g, '&')
    .replace(/\s+/g, ' ').trim();
  //
  // STAMPED BLANK, not left unstamped. The two are different answers and the reader tells them
  // apart: no attribute means "this strategy names nothing, use your heuristic"; an EMPTY one
  // means "I found the title and the label rules declined it". Falling through to the heuristic
  // in the second case would re-enable exactly what the stamp exists to override — on `kanban` it
  // names the first row inside the card rather than the card. The docblock's "left unstamped
  // rather than stamped blank" still holds for its own case, a member with NO title element,
  // which returns above before reaching here.
...
function nativeSliceSplit(openTag, rawInner, spec) {
  const { container, member, tag = 'div', containerTag = tag, min = 2, heading = true, label,
    recipe = null, layoutName = null } = spec;
  ...
  const idless = openTag.replace(/\s+id="[^"]*"/, '');
  // A NATIVE-SLICE RUN OPENS ON A COVER, like every other run (owner's call, 2026-09-13).
  //
  // The four native strategies emitted `body` pages only, so a split `roadmap` opened straight
  // into "Phase 01 · Q1" while a split `content` slide beside it opened on the shared accent
  // field — two shapes for one operation, in the same deck. The 2026-09-01 record logged that
  // as an open question precisely because adding the cover is a real change: a page per run,
  // and a decision about what the cover says when the component's own heading already repeats
  // on every page. It says what every other cover says — the masthead, plus a derived lead-in
  // naming the first member, from the same `introOf`/`labelOf` the body pages use for "next:".
  //
  // It DECLINES rather than inventing one: no masthead, no cover, and the run is exactly what
  // it was. `heading: false` specs (none today) therefore keep their old shape by construction.
  const cover = (() => {
    const mast = readMasthead(prefix);
    if (!mast) return null;
    const firstMember = holdInner.slice(members[0].start, members[0].end);
    return coverSection(openTag, chrome, { ...mast, intro: introOf(recipe, members.length, firstMember) }, layoutName);
  })();
  // With a cover holding the engine id, EVERY body page drops it — never two elements with one
  // id. Without a cover the first body page keeps it, which is the rule `splitEnvelope` already
  // applies for the same reason: a title-less run must still emit an anchor into the deck.
  const pageTag = (first) => withRole(
    (first && !cover ? openTag : idless).replace(/(\sclass=")[^"]*(")/, `$1${baseCls} lat-split-native$2`), 'body');
  const bodies = members.map((m, k) => {
  ...
  return cover ? [cover, ...bodies] : bodies;
}

function redlineBlockSplit(openTag, inner, ctx = {}) {
  const { recipe = null, layoutName = null } = ctx;
  ...
  // The run opens on a cover, as every other run does (owner's call, 2026-09-13). `redline`'s
  // two pages are the SAME clause before and after, so the cover is what tells the reader that
  // — a masthead they would otherwise meet already inside the "before" page.
  const mast = readMasthead(inner);
  const cover = mast
    ? coverSection(openTag, chromeOf(inner), { ...mast, intro: introOf(recipe, 2) }, layoutName)
    : null;
  // The cover takes the engine id when there is one, so page 1 must not also carry it.
  const firstTag = cover ? contTag : slideTag;
  const pages = [
    `${firstTag}${mark(drop([bqs[1], note, ...extra, ...stranded]), 'rl-old')}</section>`,
...
function roadmapHorizonSplit(openTag, rawInner, ctx = {}) {
...
function journeyStageSplit(openTag, rawInner, ctx = {}) {
  ...
  return nativeSliceSplit(openTag, rawInner, {
    ...
    label: 'journey-vstage-name', recipe: ctx.recipe, layoutName: ctx.layoutName });
}

// ── math-structures strategy (math — split on STRUCTURE, not on variant) ──────
//
// THE RULING THIS IMPLEMENTS: "we split on structure, unique structure gets a split
// preprocessor" (owner, #2136). Variants are the wrong unit. `math` has EIGHT of them and
// only FOUR rendered structures, so this is one strategy that dispatches on the shape it
// finds in the stage — three variants share the equation+legend shape and one arm between
// them, and the two scaffold variants fall out with no collection to slice.
//
//   | variant                     | rendered stage children          | structure         |
//   |-----------------------------|----------------------------------|-------------------|
//   | bare / `feature` / `matrix` |                                  |                   |
//   | / `decompose`               | `p > ul`                         | equation + legend |
...
//
// (measured off the committed manifest samples rendered at `hd`, not read off the CSS.)
//
// WHY MATH NEEDED A SEAM AT ALL. It was `atomic` — whole slide, ring on overflow — so the Fit
// Spine's ladder (collapse → shed → split → NEVER scale) had its third rung missing, and
// #2129's portrait work reached for the fourth at every turn: `1.15em` on the derivation
// table, `1.55em` on decompose's equation, tighter card padding on theorem. Each bought fit
// by spending legibility. One step per page sets every equation at full size.
//
// TWO OF THE FOUR ARMS ARE THE SHARED KERNEL, NOT NEW CODE. `equation + legend` is a `<ul>`
// under a `<p>`, and `step table` is a `<table>`: both are exactly what `cover-paginate`
// paginates — an accent cover, then the layout's OWN native body with the collection sliced
// and everything else repeated per page. The equation repeating on every legend page is not a
// concession there, it is the requirement (a legend page without its equation is unreadable,
// the same argument the `journey` oracle entry makes about repeating its mood key), and
// `partitionAxis` gives it for free because the equation is outside the collection. So this
// strategy DELEGATES rather than re-authoring, and `deriveAxis` picks `item` or `row` off the
// rendered DOM — HARD RULE #15, and #1: one paginator, not a third copy of it.
//
// THE OTHER TWO HAVE NO COLLECTION, which is precisely why they need a preprocessor.
// Markdown renders theorem's cards as SIBLING `<blockquote>` elements and compare's columns as
// a flat `h3, p, p, h3, p, p` run — neither is a list, so the generic path finds nothing to
// cut. Same refusal `split-compare` is recorded with in split-facts.js: "slot insufficiency",
// a generic walk cannot drive it, so a sovereign reader does.
//
// BOTH BESPOKE ARMS SLICE BY REMOVING SPANS, never by re-authoring the body from parsed
// parts — the plain envelope's own discipline, and `redline-blocks`'s. Every wrapper, cell and
// chrome node stays exactly where the engine put it, so nothing can go missing and the stage
// cell cannot be left open. Re-authoring is what made `redline` emit two `<header>`s and drop
// its footer cell.

/** The stage cell's `{start, end}` in `inner` coordinates, or null on a stage-less section. */
function stageBounds(inner) {
  const stage = extractStage(inner);
  return stage ? { start: stage.bodyStart, end: stage.bodyEnd } : null;
}

/**
 * Top-level `<tag>` children of the STAGE that match `keep`, as `inner`-coordinate spans.
 *
 * Scoped to the stage on purpose: the frame peels a slide's coda (a key insight, a below-note)
 * out into a Cell BESIDE the stage, so a section-wide walk would count a trailing
 * `<blockquote>` as a theorem card and emit a page holding the author's closing quote as if it
 * were a proof step.
 */
function stageMembers(inner, tag, keep = () => true) {
  const b = stageBounds(inner);
  if (!b) return [];
  const cell = inner.slice(b.start, b.end);
  // `topLevelElements`, NOT `directChildren`. The two names promise the same thing and deliver
  // different things: `directChildren(html, 'ul')` tracks nesting of `ul` ALONE, so a `<ul>` sat
  // inside a `<div>` comes back as a child of the string. That is right for its own callers (a
  // list inside a list is not a sibling row) and wrong here, where the question is literally
  // "is this element a child of the stage" — a blockquote inside another card, or a legend inside
  // a wrapper, is not a member. Measured: the `stageHasDirectCollection` guard below read as
  // satisfied by a `<ul>` nested one `<div>` down, which is exactly the case it exists to refuse.
  return topLevelElements(cell)
    .filter((el) => el.tag === tag)
    .map((el) => ({ start: b.start + el.start, end: b.start + el.end }))
    .filter((sp) => keep(inner.slice(sp.start, sp.end)));
}

/**
 * Does the STAGE hold a collection as a DIRECT CHILD — the shape the paginated arms cut?
 *
 * This is the guard that keeps `cover-paginate` from claiming a slide whose list is nested
 * INSIDE one of the other structures. `firstList` (via `splitRegions`) is deliberately
 * nesting-tolerant — it finds the collection with the most `<li>` children anywhere in the cell,
 * which is right for a component whose transform buries its list one level down. It is wrong
 * here, because a theorem card or a compare column may CONTAIN a list, and that list is not the
 * slide's seam — the cards and the columns are.
 *
 * Measured before this guard existed: a `theorem` whose Proof card carried a three-step list
 * came out as a cover plus three pages that had lost the Definition and Theorem cards entirely
 * and repeated one step across two pages; a `compare` with bullets under each `###` came out
 * with BOTH columns on every page and only the first column's list sliced, out of order.
 */
function stageHasDirectCollection(inner) {
  return ['ul', 'ol', 'table'].some((tag) => stageMembers(inner, tag).length > 0);
}

/**
 * COLUMNS (`math compare`) — one `### label` group per page.
 *
 * A group runs from its own `<h3>` to the next one, so it carries whatever the author put
 * under the label: the equation, the prose beneath it, a second paragraph. Returns the group
 * spans, or [] when the stage is not h3-led.
 */
function mathColumnGroups(inner) {
  const b = stageBounds(inner);
  if (!b) return [];
  const heads = stageMembers(inner, 'h3');
  if (heads.length < 2) return [];
  // THE LAST COLUMN STOPS AT THE FIRST TRAILING BEAT, not at the end of the stage. Running it to
  // `b.end` swallowed the coda into the last member's span, `mathBeats` then filtered that beat out
  // as member-overlapping, and `withMathClosing` never fired — so a key insight after two columns
  // rendered INSIDE the Ridge page and the run built no closing page. The `theorem` arm handles the
  // identical shape correctly, and this file's own comment claimed all four arms did.
  // (HARD RULE #25 red team.)
  //
  // A BARE `<p>` IS NEVER THE CODA HERE, and that qualification is not a nicety — the first cut
  // of this rule broke the shipped demo deck. `compare` renders a FLAT `h3, p, p, h3, p, p` run,
  // so nothing in the markup separates "the last column's last paragraph" from "the section's
  // closing paragraph"; `trailingSlotMaterialOf`'s `bareParagraphIsNote` reads a `<p>` after a
  // structural element as a note, and on `examples/math-split-structure.md` it claimed BOTH of the
  // Ridge column's own `<p>`s — its display equation and its explanation. The last column then ran
  // to the first of them, and the deck went from 28 pages to 29: page 7.3 read "Ridge" and nothing
  // else, with the Ridge equation dumped on a closing page. Verified on the real render, both
  // before and after.
  //
  // A `blockquote` (a key insight) or a `.below-note` div ANNOUNCES itself as a coda, which is the
  // shape the red team's finding was actually about; a bare paragraph does not, and in this
  // structure it belongs to the column above it. The other three arms are unaffected — `theorem`'s
  // members are blockquotes, so a trailing `<p>` after them is genuinely trailing.
  // SORTED BY POSITION. `trailingBeatsOf` returns `[...insight, ...note]`, so `find` took the
  // first INSIGHT rather than the earliest beat — and with a `.below-note` div ahead of a
  // blockquote the note was stranded on the last column's page while the blockquote started the
  // coda, contradicting the comment below that names both. Latent today (`math` claims
  // `trailing-paragraph`, so the below-note wrapper is never produced on a math slide) and fixed
  // rather than filed, because a comment and its code disagreeing is the thing that rots.
  const beats = trailingBeatsOf(inner)
    .filter((x) => x.name !== 'p')
    .sort((a, b) => a.start - b.start);
  const lastStart = heads[heads.length - 1].start;
  const coda = beats.find((x) => x.start > lastStart);
  const tail = coda ? coda.start : b.end;
  return heads.map((h, k) => ({ start: h.start, end: k + 1 < heads.length ? heads[k + 1].start : tail }));
}

/**
 * The shared body-page builder for the two bespoke arms: one member per page, every other
 * member's span removed, the slide otherwise identical to the source.
 */
function mathMemberPages(openTag, inner, members, label) {
  const baseCls = classOf(openTag);
  const idless = openTag.replace(/\s+id="[^"]*"/, '');
  // `lat-split-native` marks these as finished bodies, so a page that STILL overflows
  // paginates again rather than growing a second cover (the re-split guard in auto-split.js).
  const tag = withRole(idless.replace(/(\sclass=")[^"]*(")/, `$1${baseCls} lat-split-native$2`), 'body');
  // The trailing beats come off EVERY page — they close the run on its own page, built by
  // `mathMemberSplit` below. Left in, a slide's closing sentence prints once per member.
  const beats = mathBeats(inner, members);
  // AN INTERSTITIAL BLOCK RIDES ONE PAGE — the member it follows. `trailingBeatsOf` is
  // class-blind, so a paragraph BETWEEN two cards is neither a member nor a trailing beat: it was
  // in neither list, so it stayed in the trunk and `removeSpans` kept the trunk on every page.
  // Measured on a real render — an authored "that definition is the only one we need for the
  // argument that follows below" printed above the Theorem card and again above the Proof, with
  // no definition on either, which reads as a rendering fault. Token conservation reports 0 lost,
  // which is why no gate saw it: the material is REPEATED, not dropped.
  //
  // A branch-created window, because `math` did not split at all on `origin/main`. An earlier
  // arm PINNED the repetition at three copies and said deciding this was "a rule this branch does
  // not have" — but #18 does not let a branch ship a window it opened, and the rule was already
  // written two hundred lines up for `mathColumnGroups`: a bare paragraph "belongs to the column
  // above it". Same principle, same file. (HARD RULE #25 checker, on the fold that pinned it.)
  //
  // Scanned inside the STAGE, not over the section: a member is a top-level child of the stage
  // cell, so scanning the section returns the stage wrapper itself and matches nothing.
  // A BLOCK BEFORE THE FIRST MEMBER IS FRAMING, AND IT REPEATS — the `el.start > members[0].start`
  // floor below. That is the opposite call from the one this rule makes for an interstitial, and it
  // is deliberate: "We fix a closed interval and a continuous real function on it for the whole of
  // this argument" is a premise every card is read under, the same way the equation repeats over
  // every legend page of the paginated arm. A sentence BETWEEN two cards belongs to the one above
  // it; a sentence before them all belongs to all of them.
  //
  // The rule is "the block ABOVE it", not "the block it introduces", and that is a real limit: an
  // `<h3>PART II</h3>` between two cards trails the card above rather than heading the one below.
  // `###` inside `math theorem` is off-contract authoring — it is `compare`'s member selector — so
  // this is stated rather than solved. (Both from the HARD RULE #25 checker on this fold.)
  const claimed = [...members, ...beats];
  const sb = stageBounds(inner);
  const strays = (sb ? topLevelElements(inner.slice(sb.start, sb.end))
    .map((el) => ({ start: sb.start + el.start, end: sb.start + el.end })) : [])
    .filter((el) => !claimed.some((c) => el.start < c.end && c.start < el.end))
    .filter((el) => el.start > members[0].start);
  const trailsMember = (el) => members.reduce(
    (best, m, j) => (m.start < el.start ? j : best), 0,
  );
  return members.map((m, k) => {
    const elsewhere = strays.filter((el) => trailsMember(el) !== k);
    const drop = [...members.filter((_, j) => j !== k), ...beats, ...elsewhere];
    const page = removeSpans(inner, drop);
    // NAME THE MEMBER, for the same reason `nativeSliceSplit` does (its rule 5): the forward
    // pointer is built by `applyRelationshipSignals`, which resolves a page's members with
    // `membersIn` — the first list on the page and its `<li>` children. Neither of these two
    // structures HAS a list, so that heuristic finds nothing and `relationshipSignals` degrades
    // every page to silence. The splitter is the only thing that knows what it cut, so it says
    // so: "-> Theorem", "-> Ridge".
    const named = withMemberLabel(tag, inner.slice(m.start, m.end), label);
    // Page 1 states the slide's heading; the rest mark themselves a continuation, the same
    // `(cont.)` every other native slice carries.
    return `${named}${k === 0 ? page : page.replace(/<\/(h[12])>/, ' <span class="lat-cont">(cont.)</span></$1>')}</section>`;
  });
}

/**
 * The trailing BEATS of a math section — class-blind, minus anything that IS a member.
 *
 * `math` claims `blockquote` AND `trailing-paragraph` (`coda.claims`), and asking the trailing
 * scan with that class hands back NOTHING — which is why an author's closing sentence printed on
 * every page of a run: four copies on a legend split, three on a card stack, measured. The claim
 * describes how math renders a WHOLE slide; it says nothing useful about a run.
 *
 * So the scan is class-blind, and the correction the claim WAS standing in for is made
 * precisely instead: a span that overlaps a member is not a beat. That is the whole of what the
 * claim got right — a theorem's three cards are a contiguous trailing run of blockquotes, and a
 * class-blind scan reads them as a coda unless it is told they are the members.
 */
function mathBeats(inner, members = []) {
  // AFTER the last member, not merely outside every member. `trailingBeatsOf` is class-blind here,
  // so once the cards are filtered out as members a paragraph sitting BETWEEN two of them was left
  // standing as a "beat" — and the closing page then printed it after the Proof. An authored
  // "That definition is the only one we need for the argument that follows below" followed
  // nothing. Token conservation reports 0 lost, which is why no gate saw it: the material is
  // REORDERED, not dropped. (HARD RULE #25 red team, rendered at hd and portrait.)
  const floor = members.length ? Math.max(...members.map((m) => m.end)) : 0;
  return trailingBeatsOf(inner)
    .filter((b) => b.start >= floor)
    .filter((b) => !members.some((m) => b.start < m.end && m.start < b.end));
}

/** Cover + one page per member, for a structure with no paginable collection. */
function mathMemberSplit(openTag, inner, chrome, recipe, layoutName, members, label) {
  if (members.length < 2) return null;
  const b = stageBounds(inner);
  const mast = readMasthead(inner.slice(0, b.start));
  // No heading means no cover to build — and unlike the paginated arms there is no envelope
  // to fall back to, so the slide keeps its own treatment and rings.
  if (!mast) return null;
  const cover = coverSection(openTag, chrome, { ...mast, intro: introOf(recipe, members.length) }, layoutName);
  return withMathClosing(openTag, chrome, layoutName, mast.heading,
    [cover, ...mathMemberPages(openTag, inner, members, label)], mathBeats(inner, members));
}

/**
 * Append the run's CLOSING page — the strategy builds it rather than leaving it to
 * `carouselize`'s hoist, because that hoist is class-blind and would read a theorem's three
 * cards as the coda. Emitting the `closing` role here is also what makes the hoist stand down
 * (it tests for the role structurally), so the two cannot both fire.
 */
function withMathClosing(openTag, chrome, layoutName, heading, parts, beats) {
  if (!parts || !beats.length) return parts;
  const page = closingPageFromMaterial(openTag, chrome, beats, layoutName, heading || null);
  return page ? [...parts, page] : parts;
}

/** The two FIXED SCAFFOLDS, by name. See the refusal at the top of `splitMathStructures`. */
const MATH_SCAFFOLDS = new Set(['stats', 'canvas']);

/**
 * The lead-in each arm's cover carries. One `split.intro` cannot serve four structures: printing
 * "The equation, then its symbols" over a cover that opens a stack of proof cards describes
 * symbols that are not there, which is what the first cut of this shipped in the demo deck's own
 * PDF. The manifest's string stays the equation+legend one; the other three are named here,
 * beside the arms that use them.
 */
const MATH_INTRO = Object.freeze({
  row: 'The derivation, one step per page',
  cards: 'The statement, then its proof',
  columns: 'One formulation at a time',
});
const withIntro = (recipe, intro) => (intro ? { ...recipe, intro } : recipe);

function splitMathStructures(inner, ctx) {
  const { openTag, chrome, recipe, ratio, layoutName } = ctx;
  // 0 — THE TWO FIXED SCAFFOLDS REFUSE BY NAME, and this is the correction that matters most.
  //
  // The first cut let them "fall out of the rules" — `stats` has one blockquote, below the
  // two-member floor, and `canvas` has a plot rather than a collection — and was pleased with
  // itself for not special-casing them. That reasoning is a claim about the two COMMITTED
  // SAMPLES, not about the variants, and one authoring keystroke breaks it: write the stats
  // reading as a list instead of a paragraph, which is what this repo's own house style trains
  // an author to do, and `cover-paginate` claims the slide. Measured, at portrait: four pages,
  // the cover carrying the CI and p-value stripped of their panel while the estimate they
  // qualify sits on the next page.
  //
  // Being indivisible is a fact about the VARIANT. The manifest already asserts it in prose; it
  // is asserted here in code, where the engine can act on it. This is the same argument
  // `split-facts.js` makes for placing math `read-across` — a bare axis must never be able to
  // paginate these two — and the dispatch was quietly undoing it on its own first line.
  const tokens = new Set(classOf(openTag).trim().split(/\s+/));
  for (const scaffold of MATH_SCAFFOLDS) if (tokens.has(scaffold)) return null;

  // 1 — card stack (`theorem`): sibling `<blockquote>`s, one per page. The card's own title is a
  // LEADING `<strong>` ("**Definition.**"), styled positionally rather than classed, so the label
  // is asked for by TAG — the same `tag:` form `split-compare` uses for its option titles.
  const cards = mathMemberSplit(openTag, inner, chrome, withIntro(recipe, MATH_INTRO.cards), layoutName, stageMembers(inner, 'blockquote'), 'tag:strong');
  if (cards) return cards;
  // 2 — columns (`compare`): one `### label` group per page.
  const cols = mathMemberSplit(openTag, inner, chrome, withIntro(recipe, MATH_INTRO.columns), layoutName, mathColumnGroups(inner), 'tag:h3');
  if (cols) return cols;

  // 3 + 4 — equation + legend (`<ul>` → `item`) and step table (`<table>` → `row`). The shared
  // paginator resolves which off the rendered DOM and repeats the equation, the heading and a
  // table's `<thead>` on every body page.
  //
  // LAST, AND ONLY ON A DIRECT-CHILD COLLECTION. Both constraints are load-bearing and neither
  // was there at first. `cover-paginate` asks one question — is there a list or a table with two
  // or more members anywhere in the stage — so run first it outranked the two structural arms
  // whenever a card or a column happened to contain a list of its own, and the run came out
  // sliced along the wrong seam with material dropped (see `stageHasDirectCollection`).
  if (!stageHasDirectCollection(inner)) return null;
  // THE TRAILING MATERIAL COMES OUT OF THE TRUNK FIRST. `splitEnvelope` resolves its own regions
  // with the layout class, and math's `coda.claims` makes that scan return nothing — so a closing
  // sentence stayed in the trunk and `partitionAxis` copied it onto every body page (measured:
  // four copies on a four-symbol legend). Stripped here and given the run's own closing page,
  // which is the 2026-09-01 placement every other path takes.
  const beats = mathBeats(inner);
  const trunk = beats.length ? removeSpans(inner, beats) : inner;
  const paginated = coverPaginateSections(openTag, trunk, chrome,
    withIntro(recipe, deriveAxis(trunk) === 'row' ? MATH_INTRO.row : null), ratio, layoutName);
  if (paginated && paginated.length >= 2) {
    return withMathClosing(openTag, chrome, layoutName,
      readMasthead(inner.slice(0, stageBounds(inner)?.start ?? inner.length))?.heading, paginated, beats);
  }

  // A math slide whose author wrote a shape none of the four arms recognizes lands here. Null is
  // the ring, which is the honest answer: there is no seam.
  return null;
}

// ── code-cards strategy (code — by BLOCK, then by LINE-RUN) ────────────────────
//
// `code` was the one component in the catalog with a treatment and no processor: §0c placed it
// as `code` — "code-cards (by line / block — PROPOSED)" — and nothing was ever built, so a
// listing past the component's own stated wall rang at every presentation @size. Its docs tell
// authors "ten lines reads from the back row; fourteen is the wall … split it", which is a
// pagination instruction the engine could not carry out.
//
// TWO SEAMS, IN THIS ORDER, and the order is the whole design:
//
//   · BY BLOCK first. A slide holding more than one fenced block is the structural case — each
//     `<pre>` is an independent element and one per page is the single-element rule applied
//     literally. Nothing is re-authored: the page is the section with the other blocks removed,
//     so `code.styles.css` is reused whole and every variant keeps its look.
//   · BY LINE-RUN when there is exactly one block over the budget. This is the case that
//     actually clips, because the canonical code slide IS one block — a block-only seam would
//     never fire on it. A run carries `capacity.hard` lines, and the pages after the first are
//     marked `(cont.)` like every other continuation.
//
// THE LINE CUT CLOSES AND RE-OPENS ITS HIGHLIGHT SPANS. Highlighted code is HTML, and a span
// can legitimately cross a newline — a block comment, a template literal, a multi-line string.
// Cutting the string at a `\n` would leave one page with an unclosed `<span>` and the next
// starting inside a tag that never opened. So the walk tracks the open stack: at a line break it
// closes what is open, and the next line re-opens it. That is why the cut is line-exact rather
// than "cut at a newline where nothing is open", which would silently refuse to paginate exactly
// the listings most likely to need it.
function codeLines(codeInner) {
  const out = [];
  const stack = [];
  let cur = '';
  const closes = () => stack.map((f) => `</${f.tag}>`).reverse().join('');
  const opens = () => stack.map((f) => f.open).join('');
  // `<` that opens no tag is TEXT. The pattern was `/<\/?[a-zA-Z][^>]*>|[^<]+/g`, and neither
  // alternative matches a bare `<`, so `exec` skipped it and the character was DELETED —
  // `codeLines('a < b')` returned `'a  b'`. Not reachable from authored markdown (the highlighter
  // escapes `<` to `&lt;` inside a fence), but this function's contract is conservation, so a
  // silent drop is wrong wherever it sits. The third alternative takes the stray `<`.
  //
  // AND THE DISPATCH BELOW HAS TO ADMIT IT AS TEXT, WHICH IS THE HALF THE FIRST FIX MISSED. The
  // branch tested `t[0] === '<'`, so the stray `<` fell into the TAG arm, pushed a frame with
  // `tag: undefined`, and every line break after it emitted a literal `</undefined>` and
  // re-opened a stray `<`. On a 30-line block with one `if (a < b)`, rendered in Chromium, the
  // previous dispatch loses MOST OF THE BLOCK.
  //
  // THE EXACT COUNT IS NOT STATED, and the reason is worth more than the number. It has been
  // measured three times — 24 closers / 7 lines, 27 / 26, 24 / 22 — by three reconstructions of
  // the same defect, because the figure depends on where the page boundary falls relative to the
  // stray `<` and on whether lines are counted per page or per run. Every telling that pinned a
  // specific pair was wrong by the next measurement. What reproduces every time, and is the
  // whole point: a stray `<` costs the REMAINDER OF THE BLOCK, to save the one character the
  // change set out to save. The arm below asserts the properties (`undefined` absent, tags
  // balanced) rather than a count, for the same reason. The tag alternative can only produce `<x…>`,
  // three characters at minimum, so length is an exact discriminator. Found by the HARD RULE #25
  // checker, on the render — the first arm asserted the lines were present in the MARKUP, which
  // they were, and said nothing about what the page showed.
  const re = /<\/?[a-zA-Z][^>]*>|[^<]+|</g;
  let m = re.exec(codeInner);
  while (m) {
    const t = m[0];
    if (t.length > 1 && t[0] === '<') {
      if (t[1] === '/') stack.pop();
      else if (!/\/>$/.test(t) && !VOID_IN_CODE.test(t)) stack.push({ tag: (t.match(/^<([a-zA-Z][\w-]*)/) || [])[1], open: t });
      cur += t;
    } else {
      const segs = t.split('\n');
      for (let i = 0; i < segs.length; i += 1) {
        if (i > 0) { out.push(cur + closes()); cur = opens(); }
        cur += segs[i];
      }
    }
    m = re.exec(codeInner);
...
const VOID_IN_CODE = /^<(br|hr|img|wbr|input|source)\b/i;

// Every DEPTH-0 `<pre>` inside the section's stage.
function codeBlocks(inner) {
  const stage = inner.indexOf('<div class="cell-stage"');
  const from = stage < 0 ? 0 : inner.indexOf('>', stage) + 1;
  const spans = [];
  const re = /<pre\b/g;
  re.lastIndex = from;
  let m = re.exec(inner);
  while (m) {
    const end = findMatchingClose(inner, 'pre', m.index);
    if (end < 0) break;
...
function splitCodeCards(inner, ctx) {
  const { openTag, chrome, recipe, layoutName } = ctx;
  const body = withoutTrailingBeats(inner);
  const blocks = codeBlocks(body);
  if (!blocks.length) return null;
  const mast = readMasthead(body);
  const baseCls = (openTag.match(/\sclass="([^"]*)"/) || ['', ''])[1];
  const idless = openTag.replace(/\s+id="[^"]*"/, '');
  // With a cover holding the engine id every body page drops it; WITHOUT one the first body
  // page keeps it. Same rule `nativeSliceSplit` and `redlineBlockSplit` apply, and for the same
  // reason they state: two elements with one id is a defect, and a title-less run must still
  // emit an anchor into the deck. This took no `first` parameter at all, so a heading-less
  // `code` run emitted THREE pages and zero ids. Found by the HARD RULE #25 checker.
  // `mast` decides whether this run opens on a cover, and BOTH seams below ask before stamping
  // their first page. With a cover the cover holds the engine id and every body page drops it;
  // without one the FIRST body page keeps it. Same rule `nativeSliceSplit` and
  // `redlineBlockSplit` apply, for the reason they state: two elements with one id is a defect,
  // and a title-less run must still leave an anchor in the deck. This took no `first` argument
  // at all, so a heading-less `code` run emitted three pages and zero ids. Found by the
  // HARD RULE #25 checker.
  const pageTag = (first) => withRole(
    (first && !mast ? openTag : idless).replace(/(\sclass=")[^"]*(")/, `$1${baseCls} lat-split-native$2`), 'body');
  const cont = (frag) => frag.replace(/<\/(h[12])>/, ' <span class="lat-cont">(cont.)</span></$1>');

  // ── seam 1: more than one fenced block → one block per page, source-sliced.
  if (blocks.length > 1) {
    const pages = blocks.map((keep, k) => {
      const page = removeSpans(body, blocks.filter((b) => b !== keep));
      return `${pageTag(k === 0)}${k === 0 ? page : cont(page)}</section>`;
    });
    const cover = mast
      ? coverSection(openTag, chrome, { ...mast, intro: introOf(recipe, blocks.length) }, layoutName)
      : null;
    return cover ? [cover, ...pages] : pages;
  }

  // ── seam 2: one block over the budget → line-runs.
  const per = Number.isInteger(recipe?.perPage) && recipe.perPage > 0 ? recipe.perPage : 14;
  const [only] = blocks;
  const pre = body.slice(only.start, only.end);
  const codeOpen = pre.match(/<code\b[^>]*>/);
  if (!codeOpen) return null;
  const from = pre.indexOf(codeOpen[0]) + codeOpen[0].length;
  const to = pre.lastIndexOf('</code>');
  if (to <= from) return null;
  const codeInner = pre.slice(from, to);
  // A leading newline is markdown-it's own (`<code…>\n`), not a line the author wrote.
  const lines = codeLines(codeInner);
  while (lines.length && !lines[0].trim()) lines.shift();
  while (lines.length && !lines[lines.length - 1].trim()) lines.pop();
  if (lines.length <= per) return null;   // inside the budget — nothing to cut
  const groups = [];
  for (let i = 0; i < lines.length; i += per) groups.push(lines.slice(i, i + per));
  const pages = groups.map((g, k) => {
    const nextPre = pre.slice(0, from) + `\n${g.join('\n')}\n` + pre.slice(to);
    const page = body.slice(0, only.start) + nextPre + body.slice(only.end);
    return `${pageTag(k === 0)}${k === 0 ? page : cont(page)}</section>`;
  });
  const cover = mast
    ? coverSection(openTag, chrome, { ...mast, intro: introOf(recipe, groups.length) }, layoutName)
    : null;
  return cover ? [cover, ...pages] : pages;
}
...
//   · 'math-structures'— math: dispatch on the RENDERED structure — equation+legend and step
//                        table delegate to cover-paginate; theorem's card stack and compare's
//                        columns slice one member per page; the two fixed scaffolds ring.
...
function splitFeatureCover(inner, ctx) {
  ...
  if (!feat) return null;
  const parts = featureCoverSections(ctx.openTag, feat, ctx.chrome, ctx.recipe, ctx.layoutName);
...
function splitCoverRows(inner, ctx) {
  ...
  const parts = coverWindow(ctx.openTag, cover, rows, ctx.chrome, ctx.recipe.perPage, (role) => `content list-tabular-split list-tabular-${role} form`, ctx.layoutName);
  return parts.length >= 2 ? parts : null;
}
...
function splitCoverSides(inner, ctx) {
  ...
  const parts = coverWindow(ctx.openTag, { heading, intro: introOf(ctx.recipe, subjects.length) }, subjects, ctx.chrome, 1, cls, ctx.layoutName);
...
function splitCoverDecision(inner, ctx) {
  ...
  const parts = coverWindow(ctx.openTag, cover, points, ctx.chrome, ctx.recipe.perPage, (role) => `content decision-split decision-${role} form`, ctx.layoutName);
  return parts.length >= 2 ? parts : null;
}

// ── compare-options strategy (split-compare — the two-way decision) ───────────
// WHY THIS NEEDS A STRATEGY AND NOT AN AXIS, measured rather than argued. By the time
// the splitter sees a `split-compare` section its transform has already built
// `.compare-right > .options > .option` DIVS — there is NO top-level list of options
// left. Enrolling it on a bare `item` axis therefore paginates the FIRST `<ul>` it
// finds, which is option A's inner rows, and leaves option B and the verdict as
// untouched siblings that repeat on every page (rendered and read, 2026-09-06). That
// is the `split-facts.js` refusal's real mechanism, one level deeper than the refusal
// recorded it: the note says the verdict repeats because it is a sibling of `.options`,
// which is true, but the options themselves stopped being a sliceable collection first.
// §5 of 2026-07-22 calls this class "slot insufficiency" — a generic walk cannot drive
// it, so a sovereign builder does.
//
// THE RUN, per the owner's ruling of 2026-09-06 (title, first card, second card,
// recommendation): cover (frame label · heading · context) → one `.option` per page →
// the verdict on its own CLOSING page. That last move is what answers the other half of
// the refusal: the verdict lands ONCE, at the end, where a recommendation belongs.
//
// Each body page keeps the component's OWN class and its own `.option` markup — ruling 1
// (reuse the component's styling, refine the shared envelope). `coverWindow` is
// deliberately NOT used here: it flattens members into generic `split-pt` rows, which
// would throw away the option card and its `.preferred` treatment. The left panel
// REPEATS on every body page on purpose — it is the question, and a comparison read one
// answer at a time needs the question beside each. The narrow-box `.options` grid is
// already single-column at tall/square/strip (split-compare.styles.css `[data-family]`),
// so a lone option fills the measure with no new rule.
function readCompareOptions(inner) {
  const leftAt = inner.indexOf('<div class="compare-left"');
  const rightAt = inner.indexOf('<div class="compare-right"');
  if (leftAt < 0 || rightAt < 0) return null;
  const left = inner.slice(leftAt, rightAt);
  const right = inner.slice(rightAt);
  const heading = grab(left, /<h2[^>]*>([\s\S]*?)<\/h2>/);
  const optsAt = right.indexOf('<div class="options"');
  if (!heading || optsAt < 0) return null;
  const optsOpen = right.indexOf('>', optsAt) + 1;
  const [optsSpan] = directChildren(right.slice(optsAt), 'div');
  if (!optsSpan) return null;
  const optsBody = right.slice(optsOpen, optsAt + optsSpan.end - '</div>'.length);
  const options = directChildren(optsBody, 'div').map((s) => optsBody.slice(s.start, s.end));
  if (options.length < 2) return null;
  // TRAILING MATERIAL BEYOND THE VERDICT. `.verdict` is the component's own slot, but an author
  // may also close the slide with a key insight or a below-note, and those land as siblings AFTER
  // `.compare-right`. Reading only `.verdict` dropped them — the rule-6 conservation gate caught
  // it on the `+key-insight` and `+note+insight` variants, which is exactly the job §5 gives it.
  // Berth divs (`data-lattice-berth`) are engine chrome, never content, so they are skipped.
  const [rightSpan] = directChildren(inner.slice(rightAt), 'div');
  const tail = rightSpan ? inner.slice(rightAt + rightSpan.end) : '';
  // `el.tag`, NOT `el.name`. TWO walkers with the same name return DIFFERENT shapes:
  // `coda.topLevelElements` (imported here) yields `{tag, start, end}` while
  // `split-envelope`'s yields `{name, outer, …}`. Reading `.name` off coda's made both
  // guards below dead — `includes(undefined)` is always false, so the chrome filter never
  // filtered, and every beat carried `name: undefined`, which made
  // `closingPageFromMaterial`'s `s.name === 'p'` branch unreachable. That branch is the one
  // whose absence shipped a closing page with no hairline rule under the insight panel
  // (split-envelope.js). Found by the HARD RULE #25 checker; latent today only because
  // `applyCompare` leaves nothing after `.compare-right` but berth divs.
  const extras = topLevelElements(tail)
    .filter((el) => !['header', 'footer', 'nav'].includes(el.tag)
      && !/\sdata-lattice-berth\b/.test(tail.slice(el.start, tail.indexOf('>', el.start) + 1)))
    .map((el) => ({ outer: tail.slice(el.start, el.end), name: el.tag }))
    .filter((b) => b.outer.trim());
  return {
    heading,
    eyebrow: grab(left, /<span class="frame-label">([\s\S]*?)<\/span>/),
    lede: grab(left, /<p[^>]*>([\s\S]*?)<\/p>/),
    options,
    verdict: (right.match(/<blockquote[\s\S]*?<\/blockquote>/) || [''])[0],
    extras,
  };
}

function compareOptionSections(openTag, parsed, chrome, intro, layoutName) {
  const { header } = chrome;
  // `form` is appended by the caller below; strip any copy the source already carries so a
  // section authored with an explicit `form` does not end up with it twice.
  const bodyCls = classOf(openTag).split(/\s+/).filter((c) => c && c !== 'form').join(' ') || 'split-compare';
  const wrap = (klass, first, body, role) => `${roleOpenTag(openTag, klass, first, role, layoutName)}${header}${body}${footerCell(openTag, chrome)}</section>`;
  // The SHARED accent cover (`coverSection`, split-envelope.js) — the same field the
  // plain-partition path and cover-paginate open with, so a split-compare run leads in
  // looking like every other run (ruling 1: refine the shared envelope). Hand-rolling a
  // `split-compare-cover` class here shipped a cover with NO styling behind it, because
  // the `-cover` classes the other strategies pass all have CSS and an invented one does
  // not — caught by rasterizing the run, not by any gate.
  const out = [coverSection(openTag, chrome, {
    heading: parsed.heading, eyebrow: parsed.eyebrow, lede: parsed.lede, intro,
  }, 'split-compare')];
  // The question rides every body page beside its one answer.
  const ask = `<div class="compare-left">${parsed.eyebrow ? `<span class="frame-label">${parsed.eyebrow}</span>` : ''}<h2>${parsed.heading}</h2></div>`;
  for (const opt of parsed.options) {
    // NAME THE MEMBER. `applyRelationshipSignals` otherwise falls back to `membersIn`, which
    // reads a page's members as the first list's `<li>`s — and this page carries ONE option
    // card that has a list of its OWN, so the signal counted the option's ROWS: "Options 1-2
    // of 4" on a two-option comparison (rendered and read, 2026-09-06). auto-split.js names
    // this exact shape as the case a native-slice page must stamp itself out of.
    // THE SOURCE SECTION'S OWN CLASS, not a literal. `roleOpenTag` REPLACES the class
    // attribute and keeps only the canvas tokens, so a hardcoded `split-compare form` threw
    // away every modifier the author wrote — `accent` (which swaps the spectrum edge for a
    // solid bar, split-compare.styles.css:46), `compact` (which shrinks the whole --sp-*
    // scale), and any tint/mark treatment. Measured: `<!-- _class: split-compare accent
    // compact -->` split to body pages classed plain `split-compare form` while the unsplit
    // 16:9 render kept both. Found by the HARD RULE #25 checker, not by any gate. Every
    // native-slice strategy already reads `baseCls` off the source tag; this one now does too.
    const page = wrap(`${bodyCls} form`, false,
      `${ask}<div class="compare-right"><div class="options">${opt}</div></div>`, 'body');
    out.push(page.replace(/^<section\b[^>]*>/, (t) => withMemberLabel(t, opt, 'tag:strong')));
  }
  return out;
}

function splitCompareOptions(inner, ctx) {
  const parsed = readCompareOptions(inner);
  if (!parsed) return null;
  const parts = compareOptionSections(ctx.openTag, parsed, ctx.chrome, introOf(ctx.recipe, parsed.options.length), ctx.layoutName);
  if (parts.length < 3) return null;
  // The recommendation closes the run on its own page — the whole point of the ruling — beside
  // any trailing beats the author wrote past it, in document order.
  const beats = [
    ...(parsed.verdict ? [{ outer: parsed.verdict, name: 'blockquote' }] : []),
    ...parsed.extras,
  ];
  const closing = beats.length
    ? closingPageFromMaterial(ctx.openTag, ctx.chrome, beats, 'split-compare', parsed.heading)
    : null;
...
function splitCoverCode(inner, ctx) {
  ...
  if (!code) return null;
  const parts = coverCodeSections(ctx.openTag, code, ctx.chrome, introOf(ctx.recipe, code.cols.length), ctx.layoutName);
  return parts.length >= 2 ? parts : null;
}

function splitRedlineBlocks(inner, ctx) {
  const parts = redlineBlockSplit(ctx.openTag, inner, ctx);
  return parts && parts.length >= 2 ? parts : null;
}

function splitRoadmapHorizons(inner, ctx) {
  const parts = roadmapHorizonSplit(ctx.openTag, inner, ctx);
  return parts && parts.length >= 2 ? parts : null;
}

function splitKanbanLanes(inner, ctx) {
  const parts = kanbanLaneSplit(ctx.openTag, inner, ctx);
  return parts && parts.length >= 2 ? parts : null;
}

function splitJourneyStages(inner, ctx) {
  const parts = journeyStageSplit(ctx.openTag, inner, ctx);
  return parts && parts.length >= 2 ? parts : null;
}
 ...
 *
 * `math-structures` IS HERE FOR A DIFFERENT REASON FROM `redline-blocks`, and the first version of
 * this note got that wrong — it claimed the entry "passes the test on every arm", which is false
 * on two of the four and cost an author's closing sentence four copies on a legend run and three
 * on a card stack (measured at portrait; found by the HARD RULE #25 checker).
 *
 * `math` claims `blockquote` AND `trailing-paragraph`, so asking the trailing scan with math's
 * class returns NOTHING — and on the paginated arms `splitEnvelope` asks it that way itself, so
 * the beat stayed in the trunk and `partitionAxis` copied it onto every page. Honoring the claim
 * did not de-duplicate anything there; it only switched off the rescue.
 *
 * So `math-structures` OWNS its trailing material on all four arms — it strips the beats
 * class-blind (minus anything that is a member) and builds the run's closing page itself, see
 * `mathBeats` / `withMathClosing`. The entry here is what stops the dispatcher from ALSO acting:
 * a class-blind hoist reads a theorem's three cards as a contiguous trailing run and would dump
 * them on a closing page. That is a narrower claim than "the beat rides a member", and it is the
 * true one.
 *
 ...
 */
const MEMBER_CLAIM_STRATEGIES = new Set(['redline-blocks', 'math-structures']);
...
const CAROUSEL_STRATEGIES = {
  ...
  'compare-options': splitCompareOptions,
  'math-structures': splitMathStructures,
  'code-cards': splitCodeCards,
};
