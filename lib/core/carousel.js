...
const {
  ...
  splitRegions, insightPage, markNote, injectTrailing, withRole,
...
function coverWindow(openTag, cover, items, chrome, perPage, cls) {
  ...
  const wrap = (klass, first, body, role = first ? 'cover' : 'body') => `${roleOpenTag(openTag, klass, first, role)}${header}${body}${footer}</section>`;
  const out = [];
...
function splitCoverSides(inner, ctx) {
  ...
  if (synthesis) {
    ...
    parts.push(`${roleOpenTag(ctx.openTag, cls('verdict'), false, 'insight')}${header}<div class="split-runhead">The verdict</div><div class="split-pullq">${synthesis}</div>${footer}</section>`);
  }
