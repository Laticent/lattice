...
function codeLines(codeInner) {
  ...
  // previous dispatch loses 26 of the 30 lines outright. (An earlier revision of this comment
  // said "24 closers and SEVEN lines"; the closer count is placement-dependent and the seven was
  // per-page rather than per-run, so neither figure reproduces as written — the independent
  // checker measured 27 and 26. The point is unchanged and larger than the first telling: a
  // whole-block loss, to save the one dropped character it set out to fix.) The tag alternative can only produce `<x…>`,
