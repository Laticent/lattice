...
const unwrapLoneParagraph = (html) => {
  ...
  return /^<p\b[^>]*>[\s\S]*<\/p>$/.test(v) && v.indexOf('<p', 1) < 0
    ? v.replace(/^<p\b[^>]*>/, '').replace(/<\/p>$/, '').trim()
    : v;
};
...
function codeLines(codeInner) {
  ...
  // re-opened a stray `<` — measured on a 30-line block with one `if (a < b)`, 24 `</undefined>`
  // closers and SEVEN code lines erased from the rendered page. That is a far worse trade than
  // the one dropped character it set out to fix. The tag alternative can only produce `<x…>`,
