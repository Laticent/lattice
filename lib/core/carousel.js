...
function kanbanLaneSplit(openTag, rawInner) {
  // The trailing beats first — `suffix` below is everything after the last lane, and it is
  // repeated on every lane page, so a key insight left in it printed once per lane AND on the
  // closing page (measured: 2 lanes, 3 copies). They belong to the closing page alone.
  const inner = withoutTrailingBeats(rawInner, classOf(openTag));
...
function roadmapHorizonSplit(openTag, rawInner) {
  // Same sweep as `kanbanLaneSplit`: `suffix` is everything after the last horizon card (the
  // status key, the footer cell, the closes) and rides every page, so trailing beats left in it
  // would be repeated per card. They ride the run's closing page instead.
  const inner = withoutTrailingBeats(rawInner, classOf(openTag));
...
/**
 * The strategies whose body IS THE SOURCE MARKUP, re-emitted as slices.
 *
 * This set decides whether a layout's `coda.claims` is honored, and the reason is not the claim
 * itself — it is whether a claimed element SURVIVES if the splitter leaves it alone.
 *
 *   · A SOURCE-SLICE strategy re-emits the section's own markup, so a claimed element stays on a
 *     body page, styled by the component that claimed it. Hoisting it would dismantle the slide:
 *     `redline` claims `blockquote` and its two passages ARE blockquotes.
 *   · A RE-AUTHORING strategy (the five cover recipes) rebuilds its body from parsed parts, so an
 *     element its parser does not read reaches NO page at all. `split-panel` claims both beats and
 *     renders NEITHER — it sweeps them into `.panel-right` unstyled, which is one of the swallowed
 *     cases lib/core/coda.js was written to end. Honoring that claim would restore the outright
 *     loss the closing page exists to fix, so the beats are hoisted whatever the manifest says.
 *
 * So the claim is a statement about the component's own DOM, and it is only load-bearing where the
 * component's own DOM is what ships. Where the strategy replaces it, the honest question is
 * "would this text survive?" and the answer decides.
 */
const SOURCE_SLICE_STRATEGIES = new Set(['redline-blocks', 'kanban-lanes', 'roadmap-horizons']);
...
function carouselize(openTag, inner, recipe, ratio, layoutName) {
  ...
  // SOURCE_SLICE_STRATEGIES). A re-authoring strategy passes no class, so every trailing shape
  // counts as a beat — because on that path the alternative to hoisting is losing it.
  const claimScope = SOURCE_SLICE_STRATEGIES.has(recipe.strategy) ? classOf(openTag) : undefined;
...
module.exports = { carouselize, readSubjects, readFeature, readRows, readCode, CAROUSEL_STRATEGIES, SOURCE_SLICE_STRATEGIES };
