 ...
 *     per member AND on the closing page (measured: 2 lanes, 3 copies). A beat docked OUTSIDE
 *     the sliced subtree is worse — `roadmap` puts its coda beside `.cell-stage` while the
 *     slice is taken inside `.chart-body`, so it lands in neither prefix nor suffix and
 *     reaches no page at all (measured: a three-card roadmap printed its below-note zero
 *     times). Both are why the class is NOT passed to the strip: a claim means "this layout
 *     renders the beat itself", which is only true while the layout's own page ships whole.
 *     See MEMBER_CLAIM_STRATEGIES.
...
function nativeSliceSplit(openTag, rawInner, spec) {
  const { container, member, tag = 'div', containerTag = tag, min = 2, heading = true } = spec;
  ...
  return members.map((m, k) => {
    ...
    return `${pageTag(k === 0)}${header}${pfx}${html}${suffix}</section>`;
  });
}
...
function kanbanLaneSplit(openTag, rawInner) {
  return nativeSliceSplit(openTag, rawInner, { container: 'kanban-board', member: 'kanban-column' });
}
...
function roadmapHorizonSplit(openTag, rawInner) {
  ...
  return nativeSliceSplit(openTag, rawInner, { container: 'horizons', member: 'horizon-card' });
}
...
// of its DOM.
//
// The two legends (`.journey-legend` actors, `.journey-mood-legend` scale) sit BEFORE the
// vstack, so they land in the prefix and repeat on every page — which is what a reader
// needs: a page showing mood faces without the mood key is unreadable.
