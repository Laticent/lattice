...
const {
  splitEnvelope, closingPage, balancedPerPage, readCover, coverSection, roleOpenTag, chromeOf, footerCell, stripChrome, introOf, deriveAxis,
  splitRegions, insightPageFrom, trailingMaterialOf, trailingSlotMaterialOf, markNote, injectTrailingInSection, withRole, removeSpans,
...
function roadmapHorizonSplit(openTag, inner) {
  ...
  // ONE card per page. There is no grouping left to balance — `evenGroups` existed to spread a
  // budget-derived chunk size evenly, and a chunk size of one is already even.
  const out = [];
  for (const card of cards) {
    const html = gridInner.slice(card.start, card.end);
