...
// repeats the verb, reads edges in source order, and explodes a fan-out into identical
// stubs. Instead we describe the graph the way a person walks a diagram: follow the flow
// from the entry points, COALESCE a linear chain ("A leads to B, then C"), GROUP a
// fan-out ("D fans out to X, Y, and Z"), and close at the terminal nodes. The naturalness
// comes from STRUCTURE (entry/chain/fan-out/terminal) + the authored edge LABELS — never
// from inventing edge semantics the diagram doesn't state (no "writes to"/"reads from" on
// an unlabeled arrow; that would be the confidently-wrong this narrator exists to avoid).
// A graph that isn't narratable as a clean flow — a CYCLE, or no entry node — falls back
// to a grouped-adjacency reading (still coalesced, but neutral and order-faithful).

/** Adjacency + degrees + first-appearance order, from the parsed {nodes,edges}. */
...
/** Kahn topological order, stable by first-appearance; null when the graph has a CYCLE
 *  (the signal to fall back to the grouped reading rather than impute a flow order). */
function topoSort(g) {
...
/**
 * Flow-path narrative from a DAG: walk entry→terminal, coalescing linear chains and
 * grouping fan-outs, folding edge labels in as faithful adverbial clauses. Returns null
 * (→ grouped fallback) when the graph has a cycle or no entry node.
 */
function renderFlowNarrative(parsed) {
  const { nodes } = parsed;
  const g = analyzeGraph(parsed);
  const topo = topoSort(g);
  if (!topo) return null; // cyclic — no honest flow order
  const entries = g.order.filter((id) => (g.inDeg.get(id) || 0) === 0);
  if (!entries.length) return null; // no clear entry
  ...
  for (const start of topo) {
    ...
    if (!(g.outDeg.get(start) > 0)) {
      consumed.add(start); // a pure sink — named by its predecessors, no sentence of its own
      continue;
    }
    // Coalesce a linear chain: single-out to a single-in successor, not yet consumed.
    const chain = [start];
    const labels = [];
    let cur = start;
    ...
    const multi = chain.length > 1; // a real chain precedes → attach with "which"
    if (outs.length >= 2) {
...
/** Grouped-adjacency fallback: one sentence per source node, out-edges coalesced. Neutral
 *  ("leads to") and order-faithful — safe on a cyclic/entry-less graph the flow walk bails on. */
