...
const { isValuePill, parseValue } = require('./chart-values');
...
function spokenPills(pills, names) {
  if (names && names.length === pills.length) {
    return { text: pills.map((p, i) => `${names[i]}, ${toSpokenText(p)}`).join('; '), named: true };
  }
  return { text: pills.map((p) => toSpokenText(p)).join(', '), named: false };
}
  ...
  if (table) {
    ...
    for (const row of table.rows) {
      const spoken = row.values
        .map((v, i) => `${table.columns[i]}, ${v.value ? toSpokenText(v.value) : 'no data'}${v.note ? ` (${v.note})` : ''}`)
        .join('; ');
...
function speakBulletRow(r) {
  ...
  const measure = r.measureRaw ? toSpokenText(r.measureRaw) : null;
  if (!at) {
  ...
  const target = toSpokenText(r.targetRaw);
  ...
  const base = r.floorRaw ? `, on a scale starting at ${toSpokenText(r.floorRaw)}` : '';
...
function speakTier(tier) {
  const at = toSpokenText(String(tier.weight));
...
function narrateWordCloud(markdown) {
  ...
  if (f.soleLeader && f.secondTier) {
    parts.push(terminate(openSentence(
      `${f.leader} is the biggest at ${toSpokenText(String(f.leaderWeight))}, ` +
      `${toSpokenText(String(f.leadMargin))} clear of ${speakTier(f.secondTier)}`,
    )));
    spoken = 2;
