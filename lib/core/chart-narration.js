 ...
 *  currently on the recursion stack is a back-edge. Removing exactly these yields a DAG the
 *  flow can walk — so a single retry/feedback edge no longer collapses the whole reading to a
 *  grouped dump (a Munger-inversion finding); the back-edges narrate as "loops back to". */
...
/**
 * Flow-path narrative: remove feedback arcs to get a DAG, walk it entry→terminal coalescing
 * linear chains and grouping fan-outs, then speak the feedback arcs as loops. Returns null
 * only on the (post-back-removal) impossible case of a non-DAG — the defensive grouped
...
function renderFlowNarrative(parsed) {
  ...
  // A terminal is a REACHED sink — original out-degree 0 AND in-degree > 0. An orphan
  // (in = out = 0, a floating box) is NOT a flow terminal (a red-team false-terminal find)
  // and is never introduced at all (its label lives only in the blanked fence).
  ...
  for (const start of topo) {
    ...
    if (orphan) { consumed.add(start); continue; }
    if (dagOutDeg(start) === 0) { consumed.add(start); continue; } // reached sink; named by predecessors

    // Coalesce a linear chain along the DAG: single-out to a single-in successor.
    const chain = [start];
    const labels = [];
    let cur = start;
    ...
    if (outs.length === 0) {
      ...
      // pure unlabeled: clean grouped fan-out / single continuation
      ...
      // ANY labeled branch → "From X: <clause>; <clause>." Each clause binds label→target
      // with a verb, so it can't be misheard as a flat comma list (the M3 fan-out ambiguity).
      if (multi) sentences.push(`${s}.`); // the chain clause has real content ending at X
      const clauses = outs.map((o) => (o.label ? `on ${o.label}, leads to ${lbl(o.to)}` : `leads to ${lbl(o.to)}`));
      sentences.push(`From ${lbl(last)}: ${clauses.join('; ')}.`);
    }
  }

  // Feedback arcs, spoken as loops (also restores the cycle signal a grouped dump lost).
