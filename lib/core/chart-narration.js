...
const { slideToSpeech, blankHtmlComments } = require('./slide-speech');
...
function withoutFences(markdown) {
  const out = [];
  // The OPENING character, not a boolean — a fence closes on a run of the same character
  // (CommonMark), so a `~~~` line inside a ``` block is content, not the close. Tracking a
  // bare in/out flag made a deck that mixes the two blank the wrong half of itself.
  let openChar = null;
  for (const line of markdown.split('\n')) {
    const m = /^(`{3,}|~{3,})/.exec(line.trim());
    if (m && (openChar === null || m[1][0] === openChar)) {
      openChar = openChar === null ? m[1][0] : null;
      out.push('');
      continue;
    }
    out.push(openChar ? '' : line);
  }
  return out.join('\n');
}
 ...
 * shared `mermaid-fences` matcher `preprocessMermaid` renders, so narrator ⇔ render — one
 * module now rather than two copies of one regex, which is what makes that claim checkable),
 * but the
