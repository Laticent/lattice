...
function renderFlowNarrative(parsed) {
  ...
  // ④ GIST (layer 1), gated: on a non-trivial shape, open with a one-line TOPOLOGICAL
  // characterization — the SHAPE the walk buries (depth, fan-out width, where paths converge),
  // plus the one structural fact worth flagging (a shared node). A pure linear chain stays
  // SILENT (the walk IS the gist). Purely topology — no inferred meaning. It replaces the old
  // thin "It begins at X and ends at Y" overview: the shape is strictly more orienting. "It"/
  // the frame is the "A flowchart[, title]." the caller (narrateDiagram) prepends.
  ...
  let maxOut = 0;
  let hub = null;
  for (const id of g.order) if (dagOutDeg(id) > maxOut) { maxOut = dagOutDeg(id); hub = id; }
  const memoLP = new Map();
  const longest = (u) => { if (memoLP.has(u)) return memoLP.get(u); let best = 0; for (const o of dagOut(u)) best = Math.max(best, 1 + longest(o.to)); memoLP.set(u, best); return best; };
