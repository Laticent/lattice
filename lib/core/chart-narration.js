...
function parseDataRows(markdown) {
  ...
  markdown.split('\n').forEach((raw, i) => {
    ...
    if (!current || !isNestedBullet(raw)) return;
    // ANYTHING DEEPER THAN THE LAST NESTED LINE SITS INSIDE ITS `<li>`, which is the
    // one thing the transform's structural test turns on and a line scanner cannot
    // otherwise see. markdown-it nests a deeper bullet inside the list item above it,
    // so `- Target \`4\`` followed by `  - note` flattens to `Target <code>4</code>
    // <ul>…</ul>` — the code span is no longer last, and `parseBullet` files the whole
    // thing as mark DETAIL rather than reading a target off it. Recorded here, where
    // the indents are still in hand; `parseBulletRow` refuses such a child.
    {
      const last = current.nested[current.nested.length - 1];
      if (last && leadingSpaces(raw) > last.indent) last.hasDeeper = true;
    }
    const { isData, nextDataIndent } = classifyDepth(leadingSpaces(raw), dataIndent);
    ...
    if ((!label && !keyInPills) || !pills.length) {
    ...
    dataIndent = nextDataIndent;
    current.nested.push({ label: cleanLabel(label), pills, index: i, indent, hasDeeper: false });
  });
  ...
  const nestedIsData = rows.some((r) => r.pills.length === 0 && r.nested.length > 0);
  // `.filter(n => n.label)` — a code-spanned KEY line rides in `nested` for
  // `parseBulletRow` and must never become a data ITEM, which is read out by name.
  // Filtered rather than consumed, so `speakLeftover` still reads it on a slide
  // whose narrator does not absorb it.
  if (nestedIsData) for (const row of rows) row.items = row.nested.filter((n) => n.label);
...
function parseBulletRow(row) {
  ...
  for (const child of row.nested) {
    // THE TRANSFORM'S RULE, NOT AN APPROXIMATION OF IT. `parseBullet` flattens the
    // child's `<li>` and matches `^(everything) <code>(value)</code>$`: the code span
    // must be the LAST THING IN THE ITEM, and the key is EVERYTHING BEFORE IT with
    // its own tags stripped by `plainText`. Two clauses mirror that here.
    //
