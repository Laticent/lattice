 ...
 *  verb or a verb+preposition phrase — otherwise the appositive is the safe, grammatical reading. */
...
function renderFlowNarrative(parsed) {
  ...
  // ④ Overview, gated: emit ONLY when the shape can't be reconstructed from a single walk
  // sentence — multiple entry points, or a graph that both branches AND re-converges/loops.
  // A linear chain or a lone fan-out is fully described by its own walk, so an overview there
  // is pure boilerplate that restates the next sentence (a Munger-inversion finding). "It"
  // refers to the "A flowchart[, title]." frame the caller (narrateDiagram) prepends.
  ...
  for (const key of back) {
    ...
    if (l && isCondition(l)) {
      ...
      sentences.push(`${lbl(u)}, ${l}, loops back to ${target}.`); // appositive (noun/code label)
    } else {
