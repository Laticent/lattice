...
// entries below match a dot-run (`-\.+->`), and the inline-text dotted form requires its
// label to START with a non-dot — so a dot-lengthened UNLABELED arrow (`-...->`) falls
// through to the plain dotted rule (no label) instead of the inline rule fabricating a
// "." label out of the arrow's own dots (a red-team finding).
const CONNECTORS = [
  { re: /^(?:-{2,}>|=+>|-\.+->)\|([^|]*)\|/, label: 1 },        // -->|x|  ==>|x|  -.->|x| / -..->|x|
  { re: /^--\s*([^|>]+?)\s*-{2,}>/, label: 1 },                  // -- x -->
  { re: /^==\s*([^|>=]+?)\s*=+>/, label: 1 },                    // == x ==>
  { re: /^-\.\s*([^.\s|][^|]*?)\s*\.+->/, label: 1 },            // -. x .-> (label starts non-dot)
