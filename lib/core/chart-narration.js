...
function parseBulletRow(row) {
  ...
  if (row.structurallyAmbiguous && (row.nestedHasValue || row.rowLineAmbiguous)) {
    return {
      label: [row.label, ...words].filter(Boolean).join(' '),
      measureRaw, targetRaw: null, floorRaw: null,
      measure: measureRaw == null ? NaN : parseValue(measureRaw),
      target: NaN, floor: 0, bands: [], structural,
    };
  }
...
function narrateBullet(markdown) {
  ...
  const rows = raw.map(parseBulletRow);
  ...
  const consumed = new Set(raw.map((r) => r.index));
