...
function parseBulletRow(row) {
  ...
  for (const child of row.nested) {
    // EXACTLY ONE PILL, because that is what the transform requires and a second
    // pill changes the answer. `parseBullet`'s regex takes everything before the
    // LAST code span as the key, so `- Band \`99.5%\` \`99.7%\`` has the key
    // "Band 99.5%", matches no keyword, and becomes mark DETAIL — the chart derives
    // its zones and shows that line in the reveal. Reading `pills[0]` instead
    // recorded a band the picture never drew AND swallowed the author's line.
 ...
 */
const CLEARING_VERB = /\b(?:clear(?:ed|s)?|beat|beats|hit|met|made|above|ahead|behind|short|miss(?:ed|es)?|on plan|off plan|over plan|under plan|at plan)\b/;
...
function headingStatesTally(h, cleared, scored) {
  ...
  // A NUMBER ALONE IS NOT A TALLY. When every row cleared, `cleared` and `scored`
  // are the same value, so both probes read the SAME token and any heading that
  // merely counts the rows ("Three regions, one plan") suppressed the verdict —
  // which on an all-clear slide is the one summary fact there is. So the heading
  // must also say something about CLEARING. "Two of five KPIs cleared the plan
  // line" and "All three cleared their target" both trip it; "Three regions, one
  // plan" and "Stress test — seven rows, one runaway" do not.
  return mentions(cleared) && mentions(scored) && CLEARING_VERB.test(text);
}
...
function speakBulletRow(r) {
  ...
  const grade = zone?.name && zone.index < 2 ? `, ${zone.name}` : '';
...
function narrateBullet(markdown) {
  ...
  if (!headingStatesTally(h, tally.cleared, tally.scored)) {
...
function narrateWordCloud(markdown) {
  ...
  const drawn = f.count + f.unweighted.length;
