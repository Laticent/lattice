...
function narrateMachineShape(parsed) {
  ...
  if (f.unreachable.length) {
    hazardClauses.push(`nothing leads to ${joinWithAnd(f.unreachable.map((s) => s.label))}`);
  }
  ...
  if (f.deadEnds.length) {
    hazardClauses.push(`${joinWithAnd(f.deadEnds.map((s) => s.label))} stops without being marked an ending`);
  }
...
function parseDataRows(markdown) {
  ...
  markdown.split('\n').forEach((raw, i) => {
    ...
    if (raw.trim()) {
      const indentNow = leadingSpaces(raw);
