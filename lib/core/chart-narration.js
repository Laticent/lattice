 ...
 * the consequence: "Two terms" over a picture showing three, and "one of two cleared
 * the plan line" over a chart scoring three rows.
 *
...
function narrateWordCloud(markdown) {
  ...
  // THE FRAME. "Sized by how often each came up" is what the slide itself claims —
  // the key in the right rail reads `size = frequency` and the SVG `<desc>` opens
  // "Sized by frequency" — so the voice and the picture say the same thing. A
  // weight that is really a rating is the author's to label; narration follows the
  // slide rather than second-guessing it.
  // EVERY WORD THE PICTURE DRAWS, not every word with a readable weight. A
  // non-finite weight is sized to the MIDDLE of the scale rather than dropped
  // (`word-cloud.docs.md` calls that silence "the actual hazard"), so the word IS
  // on the slide — and counting only the priced ones said "two terms" over a
  // picture showing three, then named the third one sentence later.
  // EVERY TOP-LEVEL BULLET, because that is what the packer places. An earlier fix
  // added `unweighted` to the weighted count and still under-reported: the parser
  // also drops a bullet with no pill at all, and one whose whole label sits inside
  // its code span, and the cloud draws both.
  const drawn = Math.max(drawnRowCount(md), f.count + f.unweighted.length);
  const frame = frameFor('word-cloud');
  parts.push(terminate(openSentence(
    `${numberToWords(drawn)} terms${frame ? `, ${frame}` : ''}`,
  )));
  ...
  const namedCount = f.tiers.slice(0, spoken).reduce((n, t) => n + t.words.length, 0);
  ...
  if (f.count <= CLOUD_ENUMERATE_CAP) {
    ...
    // PAST THE CAP. COUNT the remainder rather than dropping it — the summary names
    // itself as a count, so nothing is silently truncated (§7) — and name the tail's
    // weight, because "the rest are mentioned once" is a real fact about the
    // silhouette the eye reads straight off the picture.
    const remaining = f.count - namedCount;
    const tailTier = rest[rest.length - 1];
    const tail = tailTier ? `, down to ${speakTier(tailTier)}` : '';
    parts.push(terminate(openSentence(
      `${numberToWords(remaining)} more follow${tail}`,
    )));
  }
