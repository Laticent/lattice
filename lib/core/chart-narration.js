...
function parseDataRows(markdown) {
  ...
  markdown.split('\n').forEach((raw, i) => {
    if (isTopLevelBullet(raw)) {
      ...
      if (rowMarker && rowMarker[1].length > 1) {
        current.structurallyAmbiguous = true;
        // AND THE ROW'S OWN PILLS ARE UNREADABLE TOO, which a child's ambiguity does
        // not imply. The wide gap makes the row's CONTENT an indented code block, so
        // the chart prints `\`5\` \`4\`` as literal backticks and scores nothing —
        // reading two pills off that line is the same guess as reading a child.
    ...
    if (raw.trim()) {
      ...
      if (enclosing && indentNow >= enclosing.contentCol) {
        if (enclosing.child) enclosing.child.hasDeeper = true;
      } else {
        enclosing = null;
      }
    ...
    if (current.bareNestedIndent != null && indent > current.bareNestedIndent) current.threeLevel = true;
    if (keyInPills) {
      current.bareNestedIndent = Math.min(current.bareNestedIndent ?? indent, indent);
      return;
    }
