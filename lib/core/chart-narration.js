...
function narrateMachineShape(parsed) {
  ...
  const deadEndLabels = new Set(f.deadEnds.map((s) => s.label));
  const strandedBoth = f.unreachable.filter((s) => deadEndLabels.has(s.label));
  const strandedIn = f.unreachable.filter((s) => !deadEndLabels.has(s.label));
  ...
  const strandedOut = f.deadEnds.filter((s) => !strandedBoth.some((x) => x.label === s.label));
...
function parseDataRows(markdown) {
  ...
  if (rows.some((r) => r.threeLevel)) return { rows: [], consumed: new Set(), dropped: rows };
  ...
  return { rows: kept, consumed, dropped: rows.filter((r) => !kept.includes(r)) };
}
...
function parseBulletRow(row) {
  ...
  for (const child of row.nested) {
    ...
    if (row.structurallyAmbiguous || child.hasDeeper) continue;
  ...
  // AN AMBIGUOUS ROW IS READ AS A BARE BAR, not as a row with its children ignored.
  // Refusing only the children was the first cut and it was not enough: a child can
  // carry a `Target` or `Floor` that changes whether the chart scores the row AT ALL.
  // Measured — `- Row0 \`2\` \`2\`` over `- Target \`4\`` / `- Floor \`80%\`` renders
  // UNSCORED, because the child's floor lands above the child's target; narration
  // dropped the children, kept the row's own second pill, and said "exactly at plan"
  // over a bar the chart draws with no plan line. The relationship is what cannot be
  // trusted here, so the relationship is what goes.
  if (row.structurallyAmbiguous) {
    return {
...
function narrateBullet(markdown) {
  ...
  const { rows: raw, dropped } = parseDataRows(md);
