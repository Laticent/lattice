 ...
 */
const CLEARING_VERB =
  /\b(?:clear(?:ed|s)?|beat|beats|exceed(?:ed|s)?|surpass(?:ed|es)?|miss(?:ed|es)?|pass(?:ed|es)?|on plan|off plan|over plan|under plan|at plan|on track|behind plan|ahead of plan)\b/;
 ...
 *
 * It is a heuristic and it fails SAFE in the only direction that matters: a miss
 * costs one short clause a listener has already heard, and a false positive costs
 * a clause nobody hears. Both are cheaper than the alternative, which is a
 * narrator that recites the slide's own headline back at it on every deck whose
 * author followed the `title` slot's instruction to put the verdict there.
 */
function headingStatesTally(h, cleared, scored) {
  ...
  // TWO DISTINCT NUMBERS ARE A TALLY ON THEIR OWN. "Three of five exceeded plan"
  // states it; so do "delivered against plan", "passed their target", "reached
  // plan", "are on track" and "came in over target". Requiring a verb here was a
  // regression I introduced and a checker measured: 7 of 12 realistic verdict
  // headings started saying the tally twice, where before the fix they had not.
  // The heading named both numbers — that IS the tally, whatever verb carries it.
  if (cleared !== scored) return true;
  // ONE NUMBER IS NOT. When every row cleared, both probes read the SAME token, so
  // "Three regions, one plan" looked identical to "All three cleared their target"
  // and suppressed the verdict — which on an all-clear slide is the only summary
  // fact there is. Only here does a verb decide, and the list is deliberately tight:
  // bare `short`, `hit`, `above` and `behind` were in it once and matched
...
/**
 * HOW MANY ROWS THE PICTURE DRAWS — every top-level bullet, which is what the
 * transforms parse (`ul > li`).
 *
 * `parseDataRows` returns fewer. Its `kept` filter drops a row with no pills and no
 * children, and a row whose whole label sits inside a code span (`- \`residency\`
 * \`1\``) — both deliberate there, and both DRAWN by the chart. A checker measured
 * the consequence: "one of two cleared the plan line" over a chart scoring three rows.
 *
 * So the count a narrator states comes from here, and a narrator that cannot parse
 * every drawn row says less rather than something wrong.
 *
 * ONE CALLER, AND THE SECOND ONE IS GONE ON PURPOSE. `narrateWordCloud` used this to
 * open on "Thirteen terms", and a word cloud is the one chart in the family whose
 * picture can hold FEWER things than its source lists — the packer seats words on a
 * spiral and drops what will not fit, which `packPortraitLadder`'s own docblock calls
 * "dropped harder than anything else in the family". This function reads markdown, so
 * it cannot see that, and the name is a promise it cannot keep for that component.
 * `narrateBullet` is safe: a bullet chart draws every row it is given.
 */
function drawnRowCount(markdown) {
  return markdown.split('\n').filter((line) => isTopLevelBullet(line)).length;
}
...
function speakBulletRow(r) {
  ...
  if (at.cleared) {
    if (!at.over) return `${against} — exactly at plan`;
    return echoes ? `${against} — above plan` : `${against} — ${pct} of plan`;
  }
...
function narrateBullet(markdown) {
  ...
  const { rows: raw } = parseDataRows(md);
  ...
  //
  // AND ONLY WHEN EVERY DRAWN ROW WAS SCORED. A row this narrator could not parse is
  // still drawn, still carries a tick, and is still counted by the `<desc>` — so
  // "one of two cleared the plan line" over a three-row chart is a confidently wrong
  // number, which is the whole defect class this work exists to remove. Better to
  // say nothing: the per-row readings below are all still true, and `speakLeftover`
  // reads the row the parser could not.
  const n = numberToWords(tally.scored);
  const wholeChart = tally.total === drawnRowCount(md);
...
//
// WHAT THIS SPEAKS, AND WHAT IT DELIBERATELY DOES NOT. Rank facts only: how many
// terms, which is biggest and by how much, the tiers under it, the quiet tail.
//
