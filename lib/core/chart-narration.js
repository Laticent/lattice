...
function narrateFunnel(markdown) {
  ...
  parts.push(terminate(openSentence(frameFor('funnel'))));
  stages.forEach((s, i) => {
...
function narrateRadar(markdown) {
  ...
  // computes a fact the Markdown never states; it still owes the listener the sentence
  // that says what the fact is ABOUT.
  parts.push(terminate(openSentence(frameFor('radar'))));
...
function narrateQuadrant(markdown) {
  ...
  // computes a fact the Markdown never states; it still owes the listener the sentence
  // that says what the fact is ABOUT.
  parts.push(terminate(openSentence(frameFor('quadrant'))));
...
function narrateMachineShape(parsed) {
  ...
  if (f.selfLoops.length) {
    const where = [...new Set(f.selfLoops.map((e) => parsed.states[e.from - 1]?.label).filter(Boolean))];
    if (where.length) {
...
function parseBulletRow(row) {
  ...
  for (const child of row.nested) {
    if (!child.pills.length || !isValuePill(child.pills[0])) continue;
...
function headingStatesTally(h, cleared, scored) {
  ...
  return mentions(cleared) && mentions(scored);
}
...
function speakBulletRow(r) {
  ...
  if (echoes) return `${against} — short of plan${grade}`;
...
function narrateBullet(markdown) {
  ...
  const { rows: raw, consumed: rowLines } = parseDataRows(md);
  ...
  const consumed = new Set(rowLines);
  ...
  parts.push(terminate(openSentence(frameFor('bullet'))));
...
function narrateWordCloud(markdown) {
  ...
  const { rows, consumed: rowLines } = parseDataRows(md);
  if (rows.length < 2) return null;
  ...
  const n = numberToWords(f.count);
  parts.push(terminate(openSentence(`${n} terms, ${frameFor('word-cloud')}`)));
