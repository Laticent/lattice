...
function parseDataRows(markdown) {
  ...
  markdown.split('\n').forEach((raw, i) => {
    ...
    dataIndent = nextDataIndent;
    current.nested.push({ label: cleanLabel(label), pills, index: i });
  });
  ...
  if (nestedIsData) for (const row of rows) row.items = row.nested;
...
function parseBulletRow(row) {
  ...
  for (const child of row.nested) {
    // EXACTLY ONE PILL. `parseBullet`'s regex takes everything before the LAST code
    // span as the key, so `- Band \`99.5%\` \`99.7%\`` keys as "Band 99.5%", matches
    // no keyword, and becomes mark DETAIL — the chart derives its zones and shows
    // that line in the reveal. Reading `pills[0]` instead recorded a band the
    // picture never drew AND swallowed the author's line.
    //
    // THIS IS A CLOSE MATCH TO THE TRANSFORM, NOT AN EXACT ONE, and the difference
    // is worth stating rather than implying. The transform's real requirement is
    // that the code span be the LAST THING IN THE `<li>`, which a pill count only
    // approximates. Two shapes still diverge, both pre-existing and both measured
    // by a checker against the rendered chart:
    //   · a structural line with its own nested child (`- Target \`6\`` over
    //     `  - note`) — markdown-it puts the child inside the same `<li>`, so the
    //     code span is no longer last and the transform takes it as detail, while
    //     this reads a target the picture never drew;
    //   · a key written as a code span (`- \`Target\` \`80%\``) — `plainText`
    //     strips the tags for the transform, which sets the target, while this sees
    //     two pills and skips it.
    // Both need the HTML the transform parses, which narration does not have.
    // Tracked rather than guessed at.
    if (child.pills.length !== 1 || !isValuePill(child.pills[0])) continue;
    const key = child.label;
    const raw = child.pills[0];
