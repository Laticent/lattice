...
function parseDataRows(markdown) {
  ...
  if (rows.some((r) => r.threeLevel)) return { rows: [], consumed: new Set() };
...
function speakBulletRow(r) {
  ...
  if (rounded >= 100) return `${against} — just short of plan${grade}`;
  return `${against} — ${pct} of plan${grade}`;
}
