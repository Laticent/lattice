...
function narrateMachineShape(parsed) {
  ...
  // arrives.
  // The SHAPE is one sentence (size, endpoints, where it decides, what loops) and the
  // HAZARDS are another. Splitting further would chop; not splitting at all was the
  // 39-word cue.
  const groups = [shapeClauses.join('; '), hazardClauses.join('; ')];
  return groups.filter(Boolean).map((g) => terminate(g)).join(' ');
}
