...
function narratePie(body) {
  ...
  for (const rawLine of pre.lines) {
    ...
    // Mermaid REQUIRES every slice value > 0 (a 0/negative errors the whole chart, so there is no
    // rendered pie to narrate). Guard each row, not just the sum — else we'd speak a derived % for
    // content Mermaid refuses to draw (an adversarial-check finding).
...
function narrateClass(body) {
  ...
  const sentences = rels.map((r) => {
    ...
    if (r.label && !r.def.assoc) s += `, ${r.label}`; // a label on a typed relationship is a role note
    if (r.multA && r.multB) {
...
function narrateState(body) {
  ...
  for (const rawLine of pre.lines) {
    ...
    // final-check finding). `end` is not a stateDiagram keyword, so it stays a valid id.
...
// gamble a single direction (design §8). Entities may be `"quoted"` or contain hyphens.
const ER_REL_RE = /^("[^"]*"|[\w-]+)\s+(\|o|\|\||\}o|\}\|)(--|\.\.)(o\||\|\||o\{|\|\{)\s+("[^"]*"|[\w-]+)\s*(?::\s*(.+))?$/;
...
function narrateEr(body) {
  ...
  for (const rawLine of pre.lines) {
    ...
    if (block) {
      ...
      // attribute: `type name [PK|FK|UK[, …]] ["comment"]` — key tags may be comma- AND/OR
      // space-separated (`PK, FK`, `PK,FK`, `PK FK`), the documented composite-key form (a check
      // finding: comma keys used to bail the whole diagram).
    ...
    // entity block open: `ENTITY {`  or  `ENTITY["alias"] {`
    const bm = t.match(/^("[^"]*"|[\w-]+)\s*(?:\[[^\]]*\])?\s*\{\s*$/);
    if (bm) { const e = stripQuotes(bm[1]); note(e); block = e; continue; }
    ...
    if (/^("[^"]*"|[\w-]+)\s*$/.test(t)) { note(stripQuotes(t)); continue; }
    return null; // unrecognized line → bail
  }
...
function narrateC4(body) {
  ...
  const boundaries = []; // { label, members: [alias] }
  const rels = [];
  const topLevel = []; // aliases declared outside any boundary, in order
  const stack = []; // boundary NESTING stack — top is the current container (a check finding: a
  // scalar flattened nested boundaries and leaked an element back to the top level on the inner close)
 ...
 *  slice #2. Every other type → null (→ heading caption), pending its own slice (design 2026-07-14). */
