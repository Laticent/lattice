...
function narrateMachineShape(parsed) {
  ...
  clauses.push(`A ${size} machine${from}${to}`);
  ...
  if (f.branchPoints.length === 1) {
    ...
    clauses.push(`${b.state.label} is where it decides, with ${numberToWords(b.ways)} ways out`);
    ...
    clauses.push('it runs as a straight chain with no forks');
  }
  ...
  if (loops.length) clauses.push(joinWithAnd(loops));
  ...
  if (f.traps.length) {
    clauses.push(`${joinWithAnd(f.traps.map((s) => s.label))} ${f.traps.length === 1 ? 'has' : 'have'} no way out but to stay`);
  }
  if (f.unreachable.length) {
    clauses.push(`nothing leads to ${joinWithAnd(f.unreachable.map((s) => s.label))}`);
  }
  if (f.deadEnds.length) {
    clauses.push(`${joinWithAnd(f.deadEnds.map((s) => s.label))} stops without being marked an ending`);
  }

  return terminate(clauses.join('; '));
}
