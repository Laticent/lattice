...
const SEQ_CLOSE = ' seq-close'; // sentinel: a block just closed (rendered as a resume cue only if more follows)
...
/** Render a frame + spoken-event list, resolving the `SEQ_CLOSE` marker: a block that just closed
 *  becomes an "Afterwards:" resume cue ONLY when more content follows (so the next message doesn't
 *  read as if still inside the block — a trailing close speaks nothing). */
 ...
 * slice #1). Reads participant display labels, then each message in source order as a neutral
 * "‹A› sends to ‹B›[: text]", single-line notes ("Note: …"), and SINGLE-LEVEL structured blocks
 * (loop/alt/opt/par/critical/break) as a spoken connective. Bails to null (→ heading caption) on
 * a NESTED block, a multiline `note … end note`, an unrecognized arrow/line, or a malformed
 * message — never a guessed reading. Past a message cap it summarizes (count + participants)
 * rather than reading a wall. The neutral "sends to" never voices the arrow's sync/async/reply
 * convention (§3.4). Returns null for a non-sequence fence.
 */
function narrateSequence(body) {
  ...
  for (; i < lines.length; i++) {
    ...
    if (/^end\b/i.test(t)) { if (depth > 0) { depth--; events.push(SEQ_CLOSE); } continue; }
    ...
    if (nm) { events.push(`Note: ${scrubLabel(nm[1])}`); continue; }
    if (/^note\b/i.test(t)) return null; // multiline note block → bail (v1)
    const bm = t.match(/^(loop|alt|opt|par|critical|break)\b\s*(.*)$/i);
    if (bm) { if (++depth > 1) return null; events.push(seqBlockPhrase(bm[1].toLowerCase(), scrubLabel(bm[2]))); continue; }
    const cm = t.match(/^(else|and|option)\b\s*(.*)$/i);
    if (cm && depth > 0) { events.push(seqBlockPhrase(cm[1].toLowerCase(), scrubLabel(cm[2]))); continue; }
  ...
  // Past the cap, read the first `SEQ_MSG_CAP` messages (with their in-flight blocks/notes) and
  // append the remainder count — a faithful prefix beats discarding the whole script for a bare
  // total, and beats reading a wall.
 ...
 *  topology walk + gist; sequenceDiagram → the message script (slice #1). Every other type → null
 *  (→ heading caption), pending its own first-wave slice (design 2026-07-14). */
