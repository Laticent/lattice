...
function parseJourneySections(markdown) {
  ...
  markdown.split('\n').forEach((raw, i) => {
    ...
    for (const tok of tokens) {
      if (tok.startsWith('@')) actors.push(tok.slice(1).trim());
      else if (/^:\d+(?:\.\d+)?$/.test(tok)) {
        const m = Number.parseFloat(tok.slice(1));
        if (Number.isFinite(m)) mood = m;
      }
    }
    for (const tok of tokens) {
...
const STATE_TRANSITION_TOKEN = /^`\s*([^`=]*?)\s*=>\s*(\d+|self)\s*`$/; // 1 = event (may be empty), 2 = target
