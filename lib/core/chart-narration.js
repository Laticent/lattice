...
// 1 = event (may be empty), 2 = target. The optional `:::class` tail is Mermaid's style hook —
// it decorates the transition, it is not part of the target. Without it in this pattern a
// styled pill matched neither the narrator NOR the suppression filter, so a chart mixing the
// two forms read half its transitions by name and the other half as raw `accept => 3:::state-pass-hue`
// glyphs — worse than main, which was at least uniformly raw.
const STATE_TRANSITION_TOKEN = /^`\s*([^`=]*?)\s*=>\s*(\d+|self)\s*(?::::?[\w-]+\s*)?`$/;
...
function narrateStateChart(markdown) {
  ...
  const withoutHeading = md
    ...
    .filter((raw, i) => {
      ...
      // A transition pill is spoken by name above; reading it again as `submit => 2` is
      // worse than either alone. A nested bullet that is NOT a transition is authored
      // content and still reads.
      return !(nested && STATE_TRANSITION_TOKEN.test(nested[1].trim()));
    })
    .join('\n');
