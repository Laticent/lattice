...
function spokenValue(raw, toSpokenText) {
  ...
  const value = parseValue(s);
  if (!Number.isFinite(value) || value === 0) return toSpokenText(s);

  const magnitude = SPOKEN_MAGNITUDES.find(([n]) => Math.abs(value) >= n);
  if (!magnitude) return toSpokenText(s);

  // THE SIGN RIDES IN FRONT OF EVERYTHING, including a currency prefix. Rebuilding
  // it after the prefix produced the literal string "$-800k", which `toSpokenText`
  // cannot read and passed through as glyphs. `-$0.8M` is an author saying "down",
  // and `parseValue` already carries all four spellings of that.
  const prefix = body.slice(0, numeric.index).trim();
  const sign = value < 0 ? '-' : '';
  // Trim the float noise `value / 1e6` introduces (1.2000000000000002) without
  // inventing precision the author did not type.
  const shown = Number((Math.abs(value) / magnitude[0]).toPrecision(12));
  return toSpokenText(`${sign}${prefix}${shown}${magnitude[1]}`);
}
