 ...
 * Harvesting before any structural transform means the rebuilders never see the
 * beats at all — they scan `[...sec.children]` and the beats are already inside a
 * cell those scans skip (`isFrameCell`, below).
 *
 * ── WHY IT DOCKS BY STRUCTURE, NOT BY COMPONENT ───────────────────────────────
 *
 * A section's outer structure is the only thing that decides how a full-width
 * band beneath the body has to be placed, and there are exactly three shapes in
 * the whole catalog (measured, not assumed — 57 column, 3 row, 1 grid). So the
...
const { mapSections, readClassAttr } = require('./section-walk');
...
function harvestBody(inner, cls) {
  ...
  // The claimed element stays exactly where it is (its own transform still finds
  // it); only the cell's INSERTION POINT moves to the end of the body, so the band
  // is still the last thing on the slide.
  const taken = [];
  let i = last;
  ...
  const at = (() => {
    ...
    return j >= 0 ? tail[j].end : out.length;
  })();
...
function applyToHtml(html) {
  ...
  return mapSections(html, (openTag, _cls, inner) =>
    // `readClassAttr` on the OPEN TAG, never the resolved `cls` mapSections
    // hands back: the section carries `data-class="<raw _class: payload>"`
    // before the resolved `class`, and a leftmost-match read of the raw payload
    // misses a deck-wide `class:` token (#1358).
    harvestBody(inner, readClassAttr(openTag)));
}
