...
/**
 * Does this OPEN TAG carry `token` as a whole CLASS TOKEN?
 *
 * The name says TAG on purpose. Handed an element's outer HTML instead — which INCLUDES
 * its descendants — it answers about the whole subtree, so any block that merely
 * CONTAINS a `.below-note` reads as one. That is not hypothetical: the first cut of
 * split-envelope.js's caller passed `el.outer` and a `.panel-right` slot came back as
 * the trailing note in full. Slice the open tag first, as `isChrome` and `isCodaCell`
 * in that file already do.
 *
 ...
 *
 * EXPORTED because this kernel is not the only module that has to recognize a wrapper
 * it did not write. `split-envelope.js` and `carousel.js` both matched `.below-note` by
 * EXACT STRING (`/^<div class="below-note"/`), which silently stopped matching the moment
 * this kernel began stamping a second class (`.is-annotation`) — measured: an annotation
 * note on a splitting slide was no longer recognized as a trailing note. An exact-string
 * class test is a latent break waiting for anyone to add a class; this is the fix, and it
 * lives here so all four call sites share one answer (HARD RULE #1).
 */
function tagHasClass(tag, token) {
  ...
  while ((m = CLASS_ATTR_RE.exec(tag))) {
    if (m[1].split(/\s+/).includes(token)) return true;
  }
  return false;
}

function hasCodaClass(tag) {
  return tagHasClass(tag, CODA_CLASS);
}
...
/**
 * Is this note's paragraph an ANNOTATION — the register that draws a ✦ and a dotted
 * rule instead of the below-note's accent hairline?
 *
 * THE KERNEL DECIDES THIS, and it has to, because CSS provably cannot. The register
 * used to be selected by `p:has(> em:only-child)`, which reads as "a paragraph whose
 * only content is an em" and is not: `:only-child` counts ELEMENTS, so
 * `<p>The figure excludes <em>pro forma</em> adjustments.</p>` has exactly one element
 * child and matches. Measured on a real render, that ordinary note came out at
 * `--fs-meta` in secondary ink with a spark on it — an author who italicizes one phrase
 * mid-sentence loses their below-note. There is no selector for "no text nodes", so the
 * shape test cannot be repaired in CSS; it can only be moved to the one place that has
 * the parse. Same reason the beats themselves are harvested here rather than matched by
 * position (HARD RULE #1: one predicate, every caller).
 *
 * The test is EXACT: strip the wrapping `<p>`, and what remains must be a single `<em>`
 * element with nothing beside it but whitespace. `<strong><em>x</em></strong>`, an `<i>`,
 * and `_source_<br>line two` are all deliberately NOT annotations — they are shapes the
 * register never claimed, and widening it here would repeat the mistake above in the
 * other direction.
 *
 * @param {string} html  the note's `<p>…</p>`, as authored
 * @returns {boolean}
 */
function isAnnotationHtml(html) {
  const inner = String(html || '').replace(/^\s*<p\b[^>]*>/i, '').replace(/<\/p>\s*$/i, '');
  return /^\s*<em>[\s\S]*<\/em>\s*$/i.test(inner) && !/<\/em>[\s\S]*<em>/i.test(inner);
}

/** The DOM arm of the same question. One predicate, two shapes of input. */
function isAnnotationEl(el) {
  if (!el || el.tagName !== 'P') return false;
  const kids = [...(el.childNodes || [])];
  let em = null;
  for (const n of kids) {
    if (n.nodeType === 3) { if (n.textContent.trim()) return false; continue; } // stray text
    if (n.nodeType !== 1) continue;
    if (em || n.tagName !== 'EM') return false;
    em = n;
  }
  return Boolean(em);
}

/** The class the register keys on. Stamped by the kernel, never inferred by CSS. */
const ANNOTATION_CLASS = 'is-annotation';

/**
 * Wrap a note's `<p>` in the `.below-note` element the hairline treatment keys on, stamping
 * the ANNOTATION register when the paragraph is em-only.
 *
 * EXPORTED because the harvest is not the only place a note gets wrapped. The split envelope
 * composes a run's CLOSING page from beats it found in a layout's own content slot
 * (`closingPageFromMaterial`, lib/core/split-envelope.js), and those arrive UNWRAPPED — a
 * layout that CLAIMS `trailing-paragraph` keeps the harvest away from it, and `split-panel`
 * claims it and then renders nothing, so the author's sentence reaches the closing page as a
 * bare `<p>`. Rendered beside four other closing pages whose notes came through the harvest,
 * it was the only one with no hairline rule under the panel. Wrapping it here rather than in
 * the envelope keeps ONE answer to "what does a below-note look like" (HARD RULE #1); the
 * annotation test especially, which CSS provably cannot make (see `isAnnotationHtml`).
 */
function wrapBelowNote(html) {
  const mark = isAnnotationHtml(html) ? ` ${ANNOTATION_CLASS}` : '';
  return `<div class="below-note${mark}">${html}</div>`;
}
 ...
 */
/** Is a coda Cell already among the section's OWN children? The idempotence guard, and
 *  the depth matters. Scanning the whole body at any depth — which this used to do —
 *  let `- One <span class="cell-coda">x</span>` inside a list item abort the harvest, so
 *  the author's `> Key insight:` printed as plain body text with no panel: the exact
 *  silent failure this kernel exists to end (#1651), reintroduced by the guard meant to
 *  protect it. It was also an arm divergence: `applyToDom` has always scoped its guard to
 *  `:scope > .cell-coda`, so the same slide rendered WITH a panel on the runtime path and
 *  WITHOUT one on the engine/export path (HARD RULE #1). Both arms ask the same question
 *  now — is one of MY children a cell — and a stray class in author content is just
 *  content. */
function hasOwnCodaCell(inner) {
  if (!hasCodaClass(inner)) return false; // cheap negative before the structural walk
  return topLevelElements(inner).some((el) => {
    const openTag = inner.slice(el.start, inner.indexOf('>', el.start) + 1);
    return hasCodaClass(openTag);
  });
}

function harvestBody(inner, cls) {
  if (typeof inner !== 'string' || !inner) return inner;
  if (hasOwnCodaCell(inner)) return inner; // idempotent
  ...
  const body = taken
    .map(({ beat, el }) => {
      ...
      if (beat !== 'below-note') return html;
      return wrapBelowNote(html);
    })
    .join('');
  ...
  for (const section of scope.querySelectorAll('section:not(section section)')) {
    ...
    for (const { beat, el } of taken) {
      if (beat === 'below-note') {
        ...
        wrap.className = isAnnotationEl(el) ? `below-note ${ANNOTATION_CLASS}` : 'below-note';
