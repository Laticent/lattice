...
function resolveFlatPalette(cssText, isDark) {
  ...
  for (const name of Object.keys(vars)) {
    ...
    // Resolve the token's OWN value, not `var(--name)`, so a modern fn EMBEDDED in a larger value
    // (a gradient/shadow: `linear-gradient(…, color-mix(…), …)`) is scanned + flattened, not passed
    // through verbatim and then dropped by the guard below. resolveDeclarationValue follows var()
    // chains too, so `--x: var(--y)` still resolves.
    const flat = resolveDeclarationValue(vars[name], vars, isDark);
