...
// The characters a run can start on: a comment's `/`, either quote, a url-token's `u`.
// `lastIndex` is set before every search, so a nested walk from `onRun` cannot desync it.
const RUN_OPENER = /[/"'uU]/g;
...
export function eachCssRun(css, onRun) {
  ...
  while (i < text.length) {
    // Jump to the next character that can open a run. Every other character is plain
    // code and only advanced `i`, one call to `urlTokenStart` at a time: measured, that
    // per-character walk was ~4 ms of every cold render, over a 2.3 MB bundle.
    RUN_OPENER.lastIndex = i;
    if (!RUN_OPENER.exec(text)) break;
    i = RUN_OPENER.lastIndex - 1;
    const ch = text[i];
    if (ch === '/' && text[i + 1] === '*') {
      flushCode(i);
