...
/** Matches an existing block (with its note), so re-baking REPLACES rather than stacks. */
const BLOCK_RE = new RegExp(
  `(?:${BLOCK_NOTE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\n)?`
  + `<script type="${FRONT_MATTER_TYPE}">[\\s\\S]*?<\\/script>\\n?`,
  'g',
);
...
function frontMatterBlock(deckSource, { localAssets = true } = {}) {
  ...
  if (!fm.trim()) return '';
  const payload = JSON.stringify(fm).replace(/</g, '\\u003c');
  return `${BLOCK_NOTE}\n<script type="${FRONT_MATTER_TYPE}">${payload}</script>\n`;
}
...
function readBakedFrontMatter(doc) {
  ...
  // The LAST block wins, and every one is removed. `withRuntimeScripts` replaces
  // an existing block rather than appending a second, so a document should only
  // ever hold one — but a deck that was hand-edited, or concatenated from two
  // exports, can hold more, and in that case the newest snapshot is the one to
  // trust. Taking the FIRST would have preferred the stalest.
  const nodes = [...doc.querySelectorAll(`script[type="${FRONT_MATTER_TYPE}"]`)];
...
/**
 * Return `deckSource` with `key: value` set in its front matter — replacing the
 * existing line if there is one, appending it just inside the closing fence if
 * there is not, and creating a whole front-matter head if the deck has none.
 *
 * The export uses this to write the resolved `overflow-marker:` into the emitted
 * `.md`, which is what makes the bundle SELF-DOCUMENTING: the recipient reads the
 * policy in the deck they were handed and can change it by editing one line, and
 * the same key is what `frontMatterBlock` then bakes for the runtime — one value,
 * one grammar, no second channel that can disagree with the visible front matter.
 *
 * COLUMN 0 only — that is the whole deck-register vocabulary, and it is what keeps
 * the write safe: an indentation-tolerant match would rewrite a same-named key
 * NESTED under a mapping (`meta:` → `  overflow-marker:`), silently editing
 * unrelated config. Matches how lib/core/resolve-overflow-marker.js reads it back.
 */
function withFrontMatterKey(deckSource, key, value) {
  ...
  const body = head[1];
  const nl = /\r\n/.test(head[0]) ? '\r\n' : '\n';
  const keyRe = new RegExp(`^${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}:[ \\t]*.*$`, 'gm');
  // Collapse DUPLICATES to one line. Editing only the first left the second behind,
  // so the emitted front matter carried two values of the same key — and since YAML
  // reads the last, the artifact stated a policy it did not enact. First occurrence
  // keeps its position (so the key does not migrate on a re-export); the rest go.
  let seen = false;
  const replaced = body.replace(keyRe, () => {
    if (seen) return '\u0000';
    seen = true;
    return line;
  });
  const nextBody = seen
    ? replaced.split(nl).filter((l) => l !== '\u0000').join(nl)
    : `${body}${nl}${line}`;
  // The head is anchored at index 0, so the tail is everything past the match.
  return `---${nl}${nextBody}${nl}${head[2]}${head[3]}` + src.slice(head[0].length);
}
