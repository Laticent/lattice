 ...
 *
 * ONE CALLER TODAY, and that is worth saying out loud given the name. The PDF
 * path calls it; the preview does not, because it reads tokens through
 * `getComputedStyle(section)` and CSS inheritance hands it the band implicitly.
 * What the preview still gets wrong is GRANULARITY — it configures Mermaid once
 * per document, from the first `<section>` — and that is #1332 step 3, which
 * needs an export sign-off and is not this module's to fix yet.
 */

const { COLOR_MODE_TOKENS } = require('./color-mode');
const { deckPrintBand, frontMatterBody, colorModeClass } = require('./resolve-color-mode');
...
function deckDarkBand(frontMatter) {
  ...
  if (!fm) return false;
  const colorModeToken = colorModeClass(frontMatterValue(fm, 'color-mode') || '');
  if (colorModeToken) return colorModeToken === 'dark';
  return (frontMatterValue(fm, 'class') || '').split(/[ \t]+/).filter(Boolean).includes('dark');
}
