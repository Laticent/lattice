 ...
 */
/**
 * The class token that keeps the sketch FINISH and drops the sketch BODY FACE. Named
 * once, read by both the type resolver and its deck-level half.
 */
const CLEAN_BODY_TOKEN = 'sketch-clean-body';

/**
 * Registers whose body TYPE is the hand face — `MODE_WANTS_HAND` minus the ones that
 * stamp the clean-body token. Derived from `MODE_REGISTER` rather than listed, so a new
 * register lands in the right set by declaring its classes and nothing else.
 */
const MODE_WANTS_HAND_TYPE = Object.freeze(new Set(
  Object.entries(MODE_REGISTER)
    .filter(([, classes]) => {
      const t = String(classes).split(/\s+/);
      return t.includes('sketch') && !t.includes(CLEAN_BODY_TOKEN);
    })
    .map(([name]) => name),
));
...
function resolveDiagramLook({
  ...
  // NOT `resolveDiagramHandType`. An earlier cut of #1674 expressed this by calling it,
  // on the reasoning that rules 2 and 3 were the same question — and they are not.
  // `sketch-clean` wants hand SHAPES and a clean body FACE, so delegating gave a
  // sketch-clean deck machine-drawn nodes. The two resolvers share `classTokens`,
  // `CLASSIC_TOKENS` and `HAND_TOKENS`; where they differ, they differ on purpose.
...
/**
 * THE TYPE QUESTION, and it is not the shape question (#1674).
 *
 * `resolveDiagramLook` answers "are the node SHAPES drawn by rough.js". This answers
 * "does this slide's `--font-body` resolve to the sketch hand face" — which is the only
 * thing the diagram map actually needs, because `MERMAID_VAR_MAP.fontFamily` reads
 * `--font-body` and `base.sketch.css` re-points it.
 *
 * THE TWO ANSWERS COME APART IN BOTH DIRECTIONS, which is why this is a separate
 * function rather than an alias:
 *
 *   - TEXTURE and PRINT take the SHAPE away and leave the TYPE. A pattern cannot survive
 *     being painted through a hachure stroke (rule 1 of `resolveDiagramLook`), but that
 *     reasoning is about the redundant-encoding channel and says nothing about type. An
 *     `a11y-*` / `onyx` / `concrete` sketch deck writes in the hand face everywhere else
 *     on the slide, and its diagram labels should match.
 *   - `sketch-clean` takes the TYPE away and leaves the SHAPE. It is the sketch finish
 *     with a legible body face for dense slides: `MODE_REGISTER` stamps
 *     `sketch sketch-clean-body`, and `base.sketch.css` answers by GUARDING the
 *     re-point — `section.sketch:not(.sketch-clean-body) { --font-body: … }` — so the
 *     clean-body slide keeps the theme's stack.
 *
 * That second case is why this cannot be `deckWantsHandDrawn`. It reads
 * `MODE_WANTS_HAND`, which contains `sketch-clean` — correctly, because sketch-clean
 * DOES want hand-drawn shapes. Using it here baked the hand face into a sketch-clean
 * deck's exported diagram labels while the preview's cascade resolved the clean one: a
 * HARD RULE #1 split, caught by the adversarial trio's checker before merge.
 *
 * @param {object}  input
 * @param {string}  input.frontMatter  The deck's raw front-matter block.
 * @param {string}  input.slideClass   The slide's own class tokens.
 * @returns {boolean}
 */
function resolveDiagramHandType({ frontMatter = '', slideClass = '' } = {}) {
  const tokens = classTokens(slideClass);
  // A slide naming the clean-body token opts its TYPE out while keeping the finish —
  // checked before the hand token, because `mode: sketch-clean` stamps BOTH and the
  // clean one has to win.
  if (tokens.includes(CLEAN_BODY_TOKEN)) return false;
  // Rule 2 — a slide naming any mode token owns its answer.
  if (tokens.some((t) => CLASSIC_TOKENS.has(t))) return false;
  if (tokens.some((t) => HAND_TOKENS.has(t))) return true;
  // Rule 3 — otherwise inherit the deck.
  return deckWantsHandType(frontMatter);
}

/**
 * The deck-level half of the type question. The sibling of `deckWantsHandDrawn`, and it
 * differs from it in exactly one register — see above.
 */
function deckWantsHandType(frontMatter) {
  const fm = frontMatterBody(frontMatter);
  if (!fm) return false;
  const mode = frontMatterName(fm, 'mode');
  if (mode) return MODE_WANTS_HAND_TYPE.has(String(mode).trim().toLowerCase());
  // A legacy deck-wide `class:` names TOKENS, not a register, so the same token rules
  // that govern a slide govern it.
  const tokens = classTokens(frontMatterValue(fm, 'class'));
  if (tokens.includes(CLEAN_BODY_TOKEN)) return false;
  return tokens.some((t) => HAND_TOKENS.has(t));
}
...
module.exports = {
  resolveDiagramLook,
  resolveDiagramHandType,
  MODE_WANTS_HAND_TYPE,
