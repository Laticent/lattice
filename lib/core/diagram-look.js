...
function resolveDiagramLook({
  ...
  // Rules 2 and 3 are exactly `resolveDiagramHandType` — expressed by CALLING it
  // rather than by repeating it, so the shape answer and the type answer cannot
  // drift apart on a slide the way band and chip did (#1326, #1340).
  return resolveDiagramHandType({ frontMatter, slideClass }) ? 'handDrawn' : 'classic';
}

/**
 * Does this slide's diagram TEXT speak in the hand face? (#1674)
 *
 * The sibling of `resolveDiagramLook`, and deliberately NOT the same question.
 * `look` is about SHAPE — whether Mermaid draws nodes through rough.js — and rule 1
 * takes it away from any deck whose palette carries categories by pattern, because a
 * pattern cannot survive being painted through a hachure stroke. That reasoning is
 * about the redundant-encoding channel and says nothing about TYPE: an `a11y-*`,
 * `onyx` or `concrete` sketch deck still writes in the hand face everywhere else on
 * the slide, and its diagram labels should match. Same for the print band.
 *
 * So this answers rules 2 and 3 only — a slide naming a mode token owns its answer,
 * otherwise it inherits the deck — and takes no `paletteUsesTexture` or `band`. A
 * texture deck in `mode: sketch` therefore gets hand LABELS inside machine-drawn
 * nodes, which is the combination `engineering/mermaid.md` §5.3e already describes as
 * the intent ("those decks still get the hand type everywhere else").
 *
 * @param {object}  input
...
function resolveDiagramHandType({ frontMatter = '', slideClass = '' } = {}) {
  ...
  return deckWantsHandDrawn(frontMatter);
}
