 ...
 * another slide's baked ink, because everything that survives normalization is still
 * compared exactly. So the list may be incomplete without being unsafe; add to it
...
function isRuntimeStampedProperty(prop) {
  return prop.startsWith('--_') || prop.startsWith('--logo-');
}

/**
 * The palette-relevant half of an inline style attribute, in a form two readers at
 * different points in the runtime's pass both compute identically: declarations
 * trimmed, whitespace collapsed, runtime-stamped properties dropped, and the result
 * SORTED so an appended `setProperty` cannot reorder the key.
 *
 * A declaration this cannot parse is kept verbatim rather than dropped — an
 * unparseable chunk is a difference between two sections, and losing it would be the
 * one direction that aliases two slides onto one key.
 */
