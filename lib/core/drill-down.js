...
// index correspondence between a source slide and its section. A DOM
// textContent count is a close-enough approximation for an already-hedged
// signal (§3: "not a geometric certainty") — with one universal exclusion:
// `.chart-status` (the shared status-pill class every chart-family component
// — kanban, timeline-list, — renders from a trailing backtick status like
// `at-risk`) is OPTIONAL per item, so unlike a fixed-cost badge it isn't a
// uniform bias across a collection; a card with a pill and an otherwise
// terser body could otherwise outrank a genuinely longer, pill-less card on
// word count alone (maker-checker finding, 2026-07-11). Any other component
// chrome not carrying this shared class is accepted as the roughly-uniform
// bias described above.
function domWordCount(el) {
  if (!el) return 0;
  const total = String(el.textContent || '').trim().split(/\s+/).filter(Boolean).length;
  if (typeof el.querySelectorAll !== 'function') return total;
  let chrome = 0;
  for (const pill of el.querySelectorAll('.chart-status')) {
    chrome += String(pill.textContent || '').trim().split(/\s+/).filter(Boolean).length;
  }
...
function findDensityOutlier(items, budget) {
  if (!items || !items.length || !budget || !Number.isInteger(budget.hard)) return null;
  let worst = null;
