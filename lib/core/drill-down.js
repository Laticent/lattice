 ...
 * can't tell the genuine culprit from an innocently-stretched neighbor.
 *
...
// ── Case B — the density-budget fallback (§12) ─────────────────────────────
// Word count off the LIVE rendered element, not the markdown source: the
// runtime never has the deck's markdown source at hand (only the rendered
// document), and re-deriving per-slide source to correlate against rendered
// <section>s the way lib/authoring/review-core.js does hits a real landmine
// — headingSplit/focusSteps (lib/integrations/markdown-it/plugins.js) can
// expand one AUTHORED slide into several RENDERED ones, breaking any naive
// index correspondence between a source slide and its section.
//
// A single `el.textContent.split(/\s+/)` read is NOT safe, and shipped
// broken (found by an independent checker in a post-merge adversarial
// review, 2026-07-11): chart-family builds markup via plain string
// concatenation with NO whitespace between adjacent sibling tags —
// `<div class="timeline-title">Migrate to Argo</div><span
// class="chart-status">at-risk</span><div class="timeline-body">Some
// notes here</div>` — so `.textContent` glues them into
// "Migrate to Argoat-riskSome notes here". Splitting THAT on whitespace
// merges "Argo"+"at-risk"+"Some" into one token, undercounting real prose
// by 2 words before the pill's own count is even subtracted — a silent
// FALSE NEGATIVE exactly where Case B exists to fire. The fix: walk the
// DOM tree and tokenize each TEXT NODE independently (using ITS OWN
// surrounding whitespace), then concatenate the resulting token arrays —
// never join raw strings across element boundaries first. `.chart-status`
// (the shared status-pill class every chart-family component renders from
// a trailing backtick status like `at-risk`) is excluded entirely by
// skipping its subtree during the walk, rather than counting it and
// subtracting afterward — the subtraction approach is what silently
// double-counted the gluing bug above.
function collectWordTokens(node, out) {
  if (!node) return;
  const type = node.nodeType;
  if (type === 3) { // Node.TEXT_NODE
    const s = String(node.textContent || '').trim();
    if (s) out.push(...s.split(/\s+/).filter(Boolean));
    return;
  }
  if (type !== 1) return; // Node.ELEMENT_NODE only past this point
  if (node.classList?.contains('chart-status')) return;
  for (const child of node.childNodes || []) collectWordTokens(child, out);
}

function domWordCount(el) {
  if (!el) return 0;
  if (!el.childNodes) {
    // Plain {textContent} fakes with no real DOM tree (unit tests only) —
    // no adjacent-element boundary to lose, so a flat split is exact.
    return String(el.textContent || '').trim().split(/\s+/).filter(Boolean).length;
  }
  const tokens = [];
  collectWordTokens(el, tokens);
  return tokens.length;
}

// The single worst item in `items` by live word count, if — and only if —
// it's past `budget.hard`. Unlike findCulprits (Case A), there is no
// row-grouping test here: density is an independent per-item content
// measure, not a stretched-geometry comparison, so the highest count past
// budget IS the signal (mirrors review-core.js's own
// `worst = Math.max(...counts)`, HARD RULE 15). Returns null when there are
// no items, no `hard` budget to compare against, or nothing crosses it.
function findDensityOutlier(items, budget) {
  if (!items?.length || !budget || !Number.isInteger(budget.hard) || budget.hard < 1) return null;
  let worst = null;
  for (const el of items) {
    const words = domWordCount(el);
    if (!worst || words > worst.words) worst = { el, words };
  }
  return worst && worst.words > budget.hard ? worst : null;
}

module.exports = {
  contentSlack, findCulprits, componentNameFor, outliersWithinGroup,
  domWordCount, findDensityOutlier,
};
