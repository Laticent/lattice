 ...
 *
 * Four empty, out-of-flow elements land as the last children of every slide
 * `<section>` — three `display: none` markers, and the slide's edge:
 *
 ...
 *   .slide-edge     the 1px keyline a HOST turns on by stating its scale; it paints
 *                   nothing otherwise (base.modifiers.css, "The slide's EDGE")
 *
 ...
 * hidden empty divs, no ink, no layout, no measurable box — and one full-slide
 * edge box that paints nothing until a host sets `--slide-edge-k`.
 *
...
// `slide-edge` is not a marker: it is the element the engine draws the slide's 1px EDGE on
// (base.modifiers.css, "The slide's EDGE"), berthed here because this is the one kernel both
// render paths already run on every slide, and because a berth is excluded from the fit
// probes by name. It paints nothing until a host hands the engine its scale.
const BERTHS = ['overflow-tab', 'illegible-tab', 'fixme-tab', 'slide-edge'];
...
/** The block the engine emitted before the edge berth joined it. A slide string berthed then
 *  gains ONLY the edge, never a second set of markers. */
const PRE_EDGE_BERTH_HTML = BERTH_HTML.slice(0, BERTH_HTML.length - tag('slide-edge').length);
...
function applyToHtml(html) {
  ...
  return pieces.map((p) => {
    ...
    const inner = hasBerth(p.inner)
      ? p.inner
      : p.inner.endsWith(PRE_EDGE_BERTH_HTML)
        ? p.inner + tag('slide-edge')
        : p.inner + BERTH_HTML;
 ...
 *
 * IT SLICES THE HEAD PREFIX OFF FIRST, and the reason has CHANGED — the prefix
 * slice is now belt-and-braces rather than the load-bearing fix it was written
 * as. It was written because an assembled document carries embedded `<script>` /
 * `<style>` chrome whose comments mention `<section …>` as prose, and those
 * substrings derailed the depth-aware section walker — it never balanced a close
 * and bailed, returning no sections at all (observed: `splitSections` on a real
 * exported sidecar found zero). As of 2026-09-21 `splitSections` walks a real
 * tokenizer, so a `<section …>` inside a comment or inside `<style>`/`<script>`
 * RAWTEXT is text and no longer derails it. The slice stays because scoping the
 * rewrite to the slide region is independently right — the head prefix is not
 * ours to rewrite — and because `lib/core/auto-split.js` scopes the same way.
 */
