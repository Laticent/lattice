 ...
 *         take colours, so an unquoted hex is a deck someone would plausibly write
 ...
 * than inventing a new behaviour. `frontMatterValue` and `parseFrontMatter` used to
...
function frontMatterScalar(raw) {
  ...
  const quote = t[0];
  if (quote === '"' || quote === "'") {
    // Find the CLOSING quote, honouring `\"` inside a double-quoted scalar the way YAML
    ...
    // replaces returned it — unfolding would be a second behaviour change riding along.
