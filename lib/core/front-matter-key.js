...
/** The keys a preset may set — the accent + surface family, in panel order. */
const PRESET_KEYS = Object.freeze([
  'spectrum', 'spectrum-edge', 'spectrum-card', 'spectrum-card-edge', 'spectrum-trim',
 ...
 * look clearly different, and each of these changes something visible on EVERY slide (the bar,
 * the heading rule, the kicker) rather than a detail only some slides carry.
 */
const PRESETS = Object.freeze({
  classic: Object.freeze({
    label: 'Classic',
    desc: 'The house default — rainbow bar, quiet trim, a hairline rule, flat cards.',
    values: Object.freeze({}),
  }),
  editorial: Object.freeze({
    label: 'Editorial',
    desc: 'A short rule, a bar on the kicker, a single-hue trim and lifted cards.',
    values: Object.freeze({ rule: 'short', eyebrow: 'bar', 'spectrum-trim': 'restrained', lift: 'on' }),
  }),
  brand: Object.freeze({
    label: 'Brand-forward',
    desc: 'The accent everywhere — a solid bar, card rails, full trim and an accent rule.',
    values: Object.freeze({ spectrum: 'solid', 'spectrum-card': 'auto', 'spectrum-trim': 'on', rule: 'accent', eyebrow: 'dot', lift: 'on' }),
  }),
  minimal: Object.freeze({
    label: 'Minimal',
    desc: 'No bar, no rule, rounded corners — the content and nothing else.',
    values: Object.freeze({ spectrum: 'off', rule: 'none', corners: 'rounded' }),
  }),
});
...
function frontMatterValue(fm, key) {
  ...
  if (m) return frontMatterScalar(m[1]);
  // An ABSENT accent/surface register falls back to the deck's `preset:` — the one place
  // a preset resolves, so every reader (plugins, runtime, linter, Studio) agrees on it.
  // An explicit key never reaches here, which is what makes the dials overrides.
  // lib/core/resolve-preset.js.
  if (!isPresetKey(key)) return null;
  const preset = frontMatterValue(fm, 'preset');
  return preset === null ? null : presetValue(preset, key);
}
