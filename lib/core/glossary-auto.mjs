...
function readFrontMatterGlossary(md) {
  ...
  if (!m) return null;
  // MUST stay identical to `frontMatterScalar` + `frontMatterName`
  // (lib/core/front-matter-key.js). It cannot IMPORT them — this module is ESM the docs
  // site imports directly (`render-engine.ts renderMarkdown`), and Rollup will not resolve
  // named exports off a CJS file outside its root. Same constraint, same remedy, as
  // resolve-pace.mjs: a sync-gated mirror, pinned by `front-matter-scalar-parity`.
  //
  // The `$`-anchored capture this replaces meant `glossary: auto  # for the appendix`
  // matched nothing, so the deck silently got NO glossary while the author had asked for one.
  const line = m[1].match(/^[ \t]*glossary:[ \t]*(.*)$/m);
  if (!line) return null;
  const cut = line[1].search(/[ \t]#/);
  const raw = (cut === -1 ? line[1] : line[1].slice(0, cut))
    .trim()
    .replace(/^['"]/, '')
    .replace(/['"]$/, '');
  return /^[A-Za-z0-9_-]+$/.test(raw) ? raw.toLowerCase() : null;
}
