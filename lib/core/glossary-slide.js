...
const { GLOSSARY_TABLE_CLASS } = require('./glossary-table-class');
...
/**
 * The GLOSSARY tables in a section — the ones a render path GENERATED, identified
 * by the marker class both paths stamp (lib/core/glossary-table-class.js).
 *
 * Neither position nor header text can identify them: an author's own table sits
 * in the same place, and an author can write `| Term | Definition |` themselves. A
 * marker can, and it is the same marker the token path's range rule now selects
 * on, so the two cannot disagree about which table sets the range.
 *
 ...
 * table (a summary, a source note) skipped the list→table conversion entirely AND
 * drew its range pill from that table's first column — a pill reading `X` where
 * the engine read `A`.
 */
function glossaryTables(section) {
  return [...section.querySelectorAll(`table.${GLOSSARY_TABLE_CLASS}`)];
}
  ...
  for (const section of scope.querySelectorAll('section.glossary')) {
    for (const list of section.querySelectorAll(':scope > ul, :scope > .cell-stage > ul')) {
      ...
      if (body.childElementCount) {
        ...
        table.className = GLOSSARY_TABLE_CLASS;
