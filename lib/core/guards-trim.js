...
function planBox(box) {
  ...
  // THE TAIL — content the walk could not model, sitting below the deepest text
  // block. This is the defect that shipped, and it is not a tolerance problem.
  //
  // `measureTrim` classifies INNERMOST TEXT BLOCKS. A card's own bottom padding and
  // border, a grid row gap, a rule under the last line — none of them is a text
  // block, so none of them appears in `blocks`, and a `contentBottom` taken as a max
  // over `blocks` alone reports the box as ending where its last PARAGRAPH ends. The
  // box's real content bottom is measured (`box.contentBottom`, from `scrollHeight`),
  // and on page 2 of the demo deck the two differ by 17px: model 1531.5, measured
  // 1548.8. The planner cut against the smaller number, so its clamp landed 17px
  // short of what the cell needed and the card shipped sheared by 8px — with the
  // overflow warning silenced, because 8px is inside the alarm's own slack.
  //
  // The tail travels with whatever block is deepest (it is that block's container's
  // own chrome), so the honest way to plan is against an EFFECTIVE limit: the real
  // limit less the tail. Everything below — the fit test, the line budget, rule 4 —
  // uses it, which is why it is computed once here rather than sprinkled.
  //
  // When the tail is NOT attached to the deepest block — an absolutely positioned
  // decoration pinned to the cell's bottom, say — this over-reserves and the planner
  // cuts one line more than strictly needed. That is the safe direction: the slide
  // still fits, and rule 5's fit-or-nothing is preserved. The unsafe direction is
  // what we had.
  const deepest = blocks.reduce((m, b) => Math.max(m, b.bottom), -Infinity);
  const measured = Number.isFinite(box.contentBottom) ? box.contentBottom : deepest;
  const tail = blocks.length ? Math.max(0, measured - deepest) : 0;
  const limit = box.limit - tail;

  const contentBottom = () => blocks.reduce((m, b) => Math.max(m, bottomOf(b)), limit);
  ...
  for (let guard = 0; guard < blocks.length + 1; guard++) {
    ...
    const next = blocks.find((b) => !recovered.has(b.id) && bottomOf(b) > limit + FIT_EPSILON);
    ...
    const lines = Math.max(1, Math.floor((limit - textTop - padBottom) / lh));
...
function trimRoleOf(el) {
  ...
  if (/^\s*[\u2014\u2013-]/.test(text) && el.closest('blockquote, figure, .quote')) return 'attribution';
  // An aside / note block. `note: 'trim'` was the other dead entry.
  if (tag === 'ASIDE' || el.closest('aside, .note')) return 'note';

  // A MIXED run — prose carrying operative inline text. Checked before the prose
  // tags below, because the tag says `<p>` and the content says otherwise.
...
function measureTrim(section, clipSelector, tolerance, idPrefix) {
  ...
  // Synthetic ids are unique per SECTION by construction (one counter below). That
  // is enough while they are only ever resolved within a section — and it stopped
  // being enough the moment the player's Read view concatenated every slide into
  // one scrolling document, where slide 1's `tb2` and slide 2's `tb2` coexist.
  ...
  var f, b, c, box, rect, limit, boxOver, blocks, pending, contentBottom, stack, node, child, r;
