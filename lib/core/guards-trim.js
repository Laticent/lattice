...
function verifyTrim(root, plan, opts) {
  ...
  var planned = {};
  var i, boxes, stillOver = [], over, all, undone, r;
  ...
  //
  // A MISSING PROBE COUNTS AS OVER, exactly like a throwing one. Written as
  // `if (!over && o.probe)`, omitting the option silently deleted arm 2 and left this
  // function running the per-box half alone — which is verbatim the policy the runtime
  // used to have and that this whole function exists to end. A guarantee that can be
  // switched off by forgetting an argument is not a guarantee, and the failure was
  // silent in the dangerous direction: trims would be KEPT under half the policy.
  // Failing toward "do not cut" makes the omission visible instead (trims stop), which
  // is the same disposition as rule 5 itself.
  // AND A PROBE THAT ANSWERS NOTHING COUNTS AS OVER TOO. Guarding only `typeof` closed
  // the omission and left the same door open one layer in: a probe returning `{}`,
  // `false`, `0` or `''` switched arm 2 off exactly as thoroughly as leaving it out —
  // `!!undefined` is false, so "not over", so the cut stands unverified. Measured, and
  // the old shape was incoherent about it: `null` and `undefined` threw on the property
  // access and failed safe while `{}` sailed through. The answer has to BE an answer —
  // a boolean `over` — or this is not a verification. Found by a checker reading the
  // commit that closed the first door.
  over = stillOver.length > 0;
  if (!over) {
    if (typeof o.probe !== 'function') over = true;
    else {
      try {
        r = o.probe(root, o.clipSel, tol, o.ignoreSel);
        over = (!r || typeof r.over !== 'boolean') ? true : r.over;
      } catch (_e) { over = true; }
    }
  }
