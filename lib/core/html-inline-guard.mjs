 ...
 *
 * Installed on the engine's parser only. `boundary-parser.mjs` never runs an inline rule —
 * every caller uses `md.block.parse` — so a guard there would be dead bytes in the Studio's
 * eager bundle (it was, briefly: +242 B gz). ESM, so any ESM caller can import it under the
 * docs site's Rollup build; the CommonJS engine `require`s it.
 */
