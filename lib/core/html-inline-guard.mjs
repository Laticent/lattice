 ...
 *
 * Installed on the engine's parser and on `boundary-parser.mjs`. An earlier commit removed the
 * second on the claim that every boundary-parser caller runs only `md.block.parse`; three run the
 * full `md.parse` (section-source-split, bake-splits, slide-class-spans), and 273 KB of `a <!--`
 * lines then took 32.8 s through `splitSourceToSections` on the CLI's narration export. Found by
 * the independent check. ESM, so `boundary-parser.mjs` can import it under the docs site's Rollup
 * build; the CommonJS engine `require`s it.
 */
