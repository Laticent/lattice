 ...
 * @param {Array}    opts.rows        `[{ stateClass, markState?, label, detail? }]`, in render
 *                                    order. `markState` appends classes to that row's SWATCH
 *                                    (rather than its `<li>`), which is how a component whose
 *                                    disc recipe is already keyed on the cell's own classes
 *                                    reuses that recipe instead of restating it.
...
function buildHtmlLegend({
  ...
  const items = list.map((r) => {
    ...
    // The swatch's own classes. `roadmap` keys its disc off the <li> and passes
    // nothing here; `obligation-matrix` keys off the cell's `state {sem} {shape}`
    // triple, so the swatch carries the same triple and the ONE disc recipe in
    // its stylesheet paints both. A key painted from a different rule than the
    // cells it names is the worst version of a stale legend.
    const markState = r.markState ? ` ${esc(r.markState)}` : '';
    return `<li class="${esc(itemClass)}${stateClass}"${titleAttr}>` +
      `<span class="${esc(markClass)}${markState}" aria-hidden="true"></span>` +
