 ...
 *
 * THE GRAMMAR and THE CHANNEL RULE live in `./hub-spoke-grammar.js`, which this module
 * re-exports whole, so the kernel and the linter still import one module. The voice
 * requires the grammar file directly: it needs what the slide says, not how it fits.
 *
 * Pure: no fs, no DOM. Safe in every browser bundle.
 */

const grammar = require('./hub-spoke-grammar');
const { spokenStatus } = require('./chart-status');

const { LIMITS, ALARM, groupsOf, channelOf, directionOf } = grammar;
...
module.exports = {
  ...grammar,
  NAME_ENVELOPE, LEAF_NAME_ENVELOPE, HUB_RATIO_MIN, HUB_RATIO_MAX, LADDER, STAGE_H, TIER_BRANCH_R,
  ...
  BRANCH_NAME_ENVELOPE, lintModel, spokenStatus,
  SIZED_BOOST, sizedRadiusMax, sizedDiscs,
};
