 ...
 *   4. a pill that carries a digit but is not a value is refused (`$18M+`, `1.2e6`):
 *      it is not a value the chart can read, and a group must be non-numeric;
 *   5. anything else is the group (a second one is linted, then dropped).
...
function hubCeiling(n, { tiered = false, sized = false, stage = 'landscape' } = {}) {
  ...
  const rs = ladderFor(n)[2] * (sized ? 1.18 : 1);
...
function readPills(pills) {
  ...
  for (const raw of pills || []) {
    ...
    if (/\d/.test(p)) { out.issues.push({ kind: 'numeric-group', pill: p }); continue; }
 ...
 */
function lintModel(model, { hand = false, stage = 'landscape' } = {}) {
  const out = [];
  ...
  for (const it of items) {
    for (const is of it.issues) {
      if (is.kind === 'extra-value') {
        ...
        add('hub-spoke-numeric-group', 'warning', `\`${is.pill}\` on "${it.label}" is neither a value the chart can read nor a group (a group name carries no digits), so it is dropped.`,
          'Write the value plainly (`$18M`, `1.2M`, `62%`), or spell the group without digits.', it.line);
      } else if (is.kind === 'extra-status') {
        add('hub-spoke-status-group', 'warning', `\`${is.pill}\` on "${it.label}" is a status word, so it cannot name a group — and a spoke takes one status.`,
          `Rename the group so it is not a status word (${['live', 'pilot', 'done', 'decision'].join(', ')} are statuses).`, it.line);
  ...
  if (C.channel === 'size') {
    const rmax = ladderFor(sp.length)[2] * 1.18;
    const clamped = sizeRadii(sp.map((s) => s.num), rmax, sizeFloor(rmax)).filter((x) => x.clamped).length;
    if (clamped > 1) {
  ...
  for (const f of crowding(model, stage)) add('hub-spoke-crowded', 'warning', f, 'Shorten the names, drop a pill, or cut a spoke; put the long form in a detail sublist. On a portrait deck the figure is narrower, so the limits are lower.', hub.line);
  return out;
}
 ...
 */
function crowding(model, stage = 'landscape') {
  const out = [];
  const sp = model.spokes;
  ...
  if (model.mods.tiered) {
    ...
    if (!tall && leaves.length > WIDE_TIER_LEAVES) {
      out.push(`\`tiered\` is proven clean up to ${WIDE_TIER_LEAVES} leaves; this slide draws ${leaves.length}, so some leaf labels may touch.`);
    }
...
/**
 * The first bullet list of a slide's Markdown, as a tree. Each node:
 * `{ label, pills, children, line }` — `label` with inline-code pills stripped from the
 * END (the kernel's `stripTrailingPills` rule), `line` the trimmed source line.
 */
function markdownTree(slide) {
  ...
  for (const raw of lines) {
    ...
    if (!m) {
      if (started && raw.trim() && !/^\s/.test(raw) && !/^<!--/.test(raw.trim())) break;
      continue;
    }
    started = true;
    ...
    const pills = [];
    let pm;
    while ((pm = text.match(/^([\s\S]*?)\s*`([^`]+)`\s*$/))) { pills.unshift(pm[2].trim()); text = pm[1]; }
    const label = text.replace(/\*\*|__|\*|_(?=\S)|(?<=\S)_/g, '').replace(/\[([^\]]*)\]\([^)]*\)/g, '$1').trim();
    const node = { label, pills, children: [], line: raw.trim(), indent };
...
module.exports = {
  ...
  sizeRadii, sizeFloor, nameEnvelope, envelopeRow, labelLoad, crowding, ENVELOPE, TALL_TIER, WIDE_TIER_LEAVES,
  BRANCH_NAME_ENVELOPE, lintModel, markdownTree, spokenStatus,
};
