 ...
 * 250-odd of them, 7,600-odd inline-code spans — the two grammars compare like this:
 *
 ...
 * fails on a dispatching span in any deck but those two, naming the file and the line.
 *
 * WHY THE CORPUS SIZE IS APPROXIMATE HERE AND EXACT THERE. The 27 / 36 / 22 / 14 figures
 * are properties of what the decks SAY and they have survived every re-measurement; the
 * corpus SIZE is a property of how many decks exist that day, and it moves whenever anyone
 * adds one. An exact "249 decks, 6,775 spans" was written into this comment and was false
 * eleven commits later, having been overtaken by a routine rebase — which is the defect
 * this paragraph was rewritten to fix, committed inside the fix. So the size is stated as
 * a floor and the test carries the exact number, because the test is re-run and a comment
 * is not.
 *
