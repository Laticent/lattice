 ...
 * `((X))` circle. Measured against all 12,551 single-backtick spans of the DECK CORPUS in this
 ...
 * code. One brace pair plus the guard below measures ZERO collisions IN THE DECK CORPUS
 * (examples/, lib/, docs/src, test/integration — 12,551 spans by the method above).
 * Repo-wide it is ~30, all in engineering prose that never renders — `{children}`,
 * `{staged_files}`, `{ok:true}`. An earlier draft of this line said "repo-wide" of a
 * number measured on 12% of the tree, which is the claim-names-its-surface failure HARD
 * RULE #23 exists to stop. The tilde is deliberate: two independent counts of the same
 * thing differed (30 and 31) because the file glob and fence handling differ, so the
 * honest form is the method plus an order of magnitude, not a false precision.
 ...
 *
 * METHOD, because the count moves with it: `git ls-files '*.md'` (1,364 files), fenced
 * blocks stripped, single-backtick spans only — 104,601 spans. A different glob or fence
 * rule gives a different total; an independent checker measuring this got 111,805 with its
 * own method. That is why the method is written down here rather than just the number.
 *
 * `parse` rejects on one `charCodeAt` before allocating, so 99.75% of spans (all but the
 * 258 that open with `{`) never reach a slice, a trim or a split.
