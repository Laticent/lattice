 ...
 * @param {object} [deck.readAlong]       read-along section — `{version, audioMode, voice?, timing?}`,
 *                                        built by `buildReadAlong` below. It records WHAT
 *                                        NARRATED the deck and where its timing track is, and
 *                                        nothing else: the LTT and the clips ride in their own
 *                                        inert blocks beside this envelope, NOT in it
 *                                        (player-core.mjs `narrationPayload` explains why — the
...
// The pair itself lives in one kernel, shared with the functionplot/anima fences and the
// runtime's decoder (lib/core/base64-utf8.js).
const { toBase64, fromBase64 } = require('./base64-utf8');
/** UTF-8 byte length. */
 ...
 *
 * `2.0` since LTT step 2 (engineering/decisions/2026-09-24-lattice-timing-track.md §9, fork D):
 * the section now points at the deck's embedded timing track — `timing`, below — and a file
 * that carries captions but no voice still gets a section, so the pointer is never lost. A
 * MAJOR bump because `voice` stopped being always present, and a 1.x reader relied on it.
 *
 * `1.1` was the section with no pointer: the audio rode in one inert block per slide and the
 * caption track travelled beside it, out of band. `1.0` was the 2026-07-08 ADR's shape, which
 * carried a per-slide `track` and never shipped.
 */
const READ_ALONG_VERSION = '2.0';

/** Where an export's timing track rides: the MIME type of its one inert block (the packed LTT,
 *  engineering/ltt.md §Encodings). Mirrors player-core.mjs's `LTT_BLOCK_MIME`. */
const LTT_BLOCK_MIME = 'application/lattice+ltt';

/**
 * Build the manifest's `readAlong` section — what NARRATED this deck, in terms a document
 * format can carry, and where its timing track is.
 *
 ...
 *
 * `timing` (2.0) says where the deck's LTT is: `{ format: 'ltt', version, encoding, mime }`,
 * naming the one inert block of that MIME type. Present only when the export embeds one.
 *
 ...
 * with the first). The timing itself is not copied either: it is in the LTT block.
 *
 * @param {object|null|undefined} voice  the Studio's voice object, or none (a captions-only export)
 * @param {{hasAudio?: boolean, ltt?: {version: string}|null}} [opts]
 * @returns {object|undefined} undefined when there is neither a voice nor a timing track
 */
function buildReadAlong(voice, opts = {}) {
	const hasVoice = !!voice && typeof voice === 'object';
	if (!hasVoice && !opts.ltt) return undefined;
	const section = {
	...
	if (opts.ltt) section.timing = { format: 'ltt', version: String(opts.ltt.version), encoding: 'packed', mime: LTT_BLOCK_MIME };
	return section;
}
