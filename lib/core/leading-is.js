...
// The prelude class excludes `{`, so the trailing `\{` can only match one way —
// there is no backtracking to exploit. The length bound is belt-and-braces for
// the browser producer, which runs this over CSS it just fetched: the longest
// real prelude in dist/lattice.css is ~4 KB (a long comma list), so 64 KB is far
// above the corpus while still being bounded.
const RULE_PRELUDE = /(^|[{};])([^{}@;]{1,65536})\{/g;

/**
 * Rewrite every leading-`:is()` selector in one COMMENT-FREE span of stylesheet.
 * `^` counts as a rule boundary, which is what makes the comment-skipping caller
 * below correct: a rule that follows a comment block starts its own span.
 */
function rewriteSpan(span) {
  return span.replace(RULE_PRELUDE, (whole, lead, prelude) => {
 ...
 * at this level. Declarations, at-rule preludes, and `@keyframes` frame selectors
 * are left alone (none can start with `:is(`).
 *
 ...
 *     617 chart rules survived the first attempt at this).
...
function distributeLeadingIs(css) {
  ...
  while (i < text.length) {
    ...
    if (open < 0) {
      out += rewriteSpan(text.slice(i));
      break;
    }
    out += rewriteSpan(text.slice(i, open));
    const close = text.indexOf('*/', open + 2);
    if (close < 0) {
      out += text.slice(open); // unterminated comment — copy the rest verbatim
      break;
    }
