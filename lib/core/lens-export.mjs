...
/** A MARKDOWN chunk, before anything has rendered — the projection's own marker. */
export const isHoleSourceChunk = (chunk) => /<!--\s*_class:\s*lens-hole\s*-->/.test(String(chunk ?? ''));
...
export function emptyHolePositions(source) {
	...
	const want = HOLE_BODY.trim();
	return entries.flatMap((e, i) => (e.chunk.trim() === want ? [i] : []));
}
