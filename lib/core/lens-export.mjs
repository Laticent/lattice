 ...
 * consistent for free: a `--lens brief` PDF paginates 1..4 because the four
 * slides it ships are the only ones that carry a visible page number.
 *
 * NOT "because it really is a four-slide deck" — that sentence stood here until
 * it was measured, and it is the exact false premise that produced six
 * authored-vs-shipped index bugs. Under position-holding projection the deck is
 * the AUTHORED length with holes in it: eight sections for a four-slide view.
 * Every stage still agrees, but they agree on TWO numbers rather than one — the
 * structural position, which every slot holds and CSS counts, and the visible
 * number, which ranks only what ships. That is why `SHIPPED_SLIDE_AT` and
 * `asShippedSlides` exist; a stage that assumes the single shorter deck pairs an
 * authored-length array against a shipped-length one and is off by the number of
 * preceding holes. Filtering later (at the
...
import { applyTag, lensEligibility, parseLensRegistry, parseSlideTags, readerLenses, stripExtraLensTags, upsertLensRegistry } from '@laticent/lente';
import { boundaryParser } from './boundary-parser.mjs';
import { readGlobalStyle } from './front-matter-key.js';
import { blockLines, splitFlowPairs } from './resolve-captions.mjs';
 ...
 * @param {import('@laticent/lente').LensDef[]} defs the exported views' definitions, in order
...
/**
 * A `_lens` tag the author QUOTED — inside a code fence, an indented block, or backticks.
 *
 * NOT a directive: the engine never reads it, and the prune deliberately leaves it alone (a slide
 * teaching the syntax had its example gutted to two bare backticks once, and there is a test pinning
 * that it must not happen again). But it IS text printed on the slide, so if it names a view this
 * export does not carry, the recipient reads the id of a view they were not given — measured on a
 * real PDF, twice on one deck, at exit 0.
 *
 * It WARNS rather than refusing, and the split from `unprunable` is deliberate. Every other
 * placement is a directive the author cannot see rendered; this one is prose they wrote and can read
 * on their own slide, and the natural deck to hit it is a deck teaching reader views — which HARD
 * RULE #29 says to coach rather than refuse. The author is told which id, and decides.
 *
 * @param {string} slideSrc a single slide's markdown
 * @returns {string[]} the raw bodies of every QUOTED `_lens` tag on the slide
 */
export function quotedDirectiveBodies(slideSrc) {
	const out = [];
	const scan = (text) => {
		for (const m of String(text).matchAll(/<!--\s*_lens:([\s\S]*?)-->/g)) out.push(m[1]);
	};
	for (const tok of boundaryParser.parse(String(slideSrc ?? ''), {})) {
		if (tok.type === 'fence' || tok.type === 'code_block') scan(tok.content);
		if (tok.type !== 'inline' || !tok.children) continue;
		for (const child of tok.children) if (child.type === 'code_inline') scan(child.content);
	}
	return out;
}

/**
 * Which views a projection would only ever NAME, never carry — the quoted ids above, filtered to the
 * ones this export does not offer. The CLI warns with these; nothing refuses on them.
 *
 * @param {string} source full deck source
 * @param {string[]} exportedIds the view ids this export carries
 * @returns {string[]} view ids quoted on a shipped slide that this export does not carry
 */
export function quotedForeignViews(source, exportedIds) {
	const src = normalizeSourceText(String(source ?? ''));
	const fm = frontMatterBlockOf(src);
	const have = new Set([...(exportedIds ?? []), FULL_VIEW]);
	const seen = new Set();
	for (const { chunk } of chunksWithSeparators(src.slice(fm.length)).entries) {
		if (isHoleSourceChunk(chunk)) continue;
		for (const body of quotedDirectiveBodies(chunk)) {
			for (const raw of body.split(/\s+/).filter(Boolean)) {
				const id = raw.replace(/^[+-]/, '');
				if (id && !have.has(id)) seen.add(id);
			}
		}
	}
	return [...seen];
...
export function projectForExport(source, requestedIds, options = {}) {
	...
	const reducing = kept.length < slides.length;
	const prunedAcronyms = [];
	const prunedLexicon = [];
	// FAIL CLOSED ON A SPELLING WE CANNOT PRUNE. The two prune branches read a plain `key:` header
	// opening an indented block, and a whole flow map on ONE line. Four other legal YAML spellings
	// reached neither and shipped every entry verbatim — measured on real artifacts: a multi-line
	// `captions: {`, a single-line map with a trailing `# comment`, a quoted `"captions":` key, and a
	// bare header with a comment after the colon.
	//
	// Refusing is the posture the rest of this file already takes for a channel it cannot verify, and
	// it is the right one here: the alternative is a THIRD parser for the same grammar, and the first
	// two disagreeing about where a block ends is what produced this class in the first place. The
	// author gets a message naming both forms that work. Only under a reducing projection — nothing
	// is withheld otherwise, so there is nothing to prune and no reason to refuse.
	if (reducing) {
		const fmBody = /^(?:---(?:\r\n|\r|\n))([\s\S]*?)(?:(?:\r\n|\r|\n)---)/.exec(fm);
		if (fmBody) {
			for (const line of fmBody[1].split(/\r?\n/)) {
				const key = /^(?:(")?(captions|acronyms|lexicon)\1?)\s*:(.*)$/.exec(line);
				if (!key) continue;
				const quoted = Boolean(key[1]);
				const rest = key[3].trim();
				// A bare header (block form) or a complete one-line flow map, and nothing else.
				const readable = !quoted && (rest === '' || /^\{[^{}]*\}$/.test(rest));
				if (!readable) return { ok: false, reason: 'unprunable-narration' };
			}
		}
	}
	// BOTH TERM-KEYED NARRATION BLOCKS, not just the one. `parseNarrationFrontMatter` returns three
	// blocks — `acronyms`, `captions`, `lexicon` — and the projection pruned two. `lexicon:` maps a
	// word taken OFF A SLIDE to how it should be spoken, so its KEY is withheld-slide content by
	// construction; it rode out whole into the envelope, the `--embed-source` attachment and the
	// manifest's `config` echo. Measured: a codename written on exactly one slide, that slide
	// withheld, recovered verbatim from a `--lens brief --embed-source` PDF at exit 0, and still
	// present under `--strip-notes --strip-captions` together.
	//
	// Same oracle, same machinery, different key — a lexicon entry has no expansion, so the term is
	// the whole of the question.
	const prunedFm = pruneTermBlock(
		pruneTermBlock(pruneCaptions(pruneRegistry(fm, keptDefs, defaultId), kept), body, reducing, 'acronyms', prunedAcronyms),
		body, reducing, 'lexicon', prunedLexicon,
	);
	...
	if (identity && tagsUnchanged && prunedFm === fm) return { ok: true, source: src, views, kept, total: slides.length, default: defaultId, prunedAcronyms, prunedLexicon };

	return { ok: true, source: prunedFm + body, views, kept, total: slides.length, default: defaultId, prunedAcronyms, prunedLexicon };
}

/**
 * Front-matter `acronyms:` pruned to the terms a recipient can actually see.
 *
 * THE SEVENTH INDEX-INDEPENDENT DISCLOSURE CHANNEL, and the only one that leaks without any index
 * arithmetic at all. `glossary: auto` appends a glossary slide built by `glossaryEntries`
 * (`lib/core/glossary-auto.mjs`), which walks the deck-wide `acronyms:` registry and emits EVERY
 * entry carrying a `definition` — it never asks whether the term appears in the deck. So a term
 * named on exactly one slide, with that slide withheld, still had its definition printed verbatim
 * on a page that ships. Measured end to end on the real CLI: `GLOSSLEAK the acquisition target.`
 * on the appended page of a `--lens brief` PDF that withheld the only slide naming ACME.
 *
 * This PR CREATED that channel, which is why it is fixed here rather than warned about. Before
 * `--lens` there was no withheld slide for a definition to be about; the glossary could only
 * describe material the reader already had. HARD RULE #18: a window you open, you close.
 *
 * THE ORACLE IS THE PROJECTED BODY, not the authored one — precisely the text a recipient can read,
 * holes included (they carry nothing). An entry survives if its TERM or its EXPANSION appears there;
 * otherwise its definition is about withheld material and the entry goes. Matching the expansion too
 * matters because an author may write "Acme Corporation" on the kept slide and never the acronym, and
 * dropping that entry would be a REGRESSION in the glossary rather than a disclosure fix.
 *
 * ONLY WHEN THE PROJECTION REDUCES. Under `--lens full` (and any view keeping every slide) the body
 * is the whole deck, so this would be a no-op by construction — except for a term defined and never
 * mentioned anywhere, which today still gets a glossary row. Gating on `reducing` keeps that existing
 * behavior exactly as it is: this function changes what ships ONLY on the exports that withhold
 * something, which is the whole of its remit.
 *
 * A term is matched on a WORD BOUNDARY and case-sensitively. `ARR` must not be kept alive by the word
 * "carry", and an acronym registry is case-bearing by nature — `IT` and `it` are not the same entry.
 */
function pruneTermBlock(fm, body, reducing, key, dropped) {
	if (!fm || !reducing) return fm;
	const m = /^(---(?:\r\n|\r|\n))([\s\S]*?)((?:\r\n|\r|\n)---[ \t]*(?:\r\n|\r|\n)?)$/.exec(fm);
	if (!m) return fm;
	const entries = blockLines(m[2], key);
	if (!entries.length) return fm;

	/**
	 * Does `needle` appear in `body` as a WHOLE WORD? Plain `indexOf` plus a character test on each
	 * side — deliberately NOT a regex.
	 *
	 * The needle is AUTHOR INPUT: an acronym or a lexicon key straight out of the deck's front
	 * matter. Building a `RegExp` from it is regular-expression injection by construction, and an
	 * escape does not stop a scanner reading it that way — this file's sibling
	 * `docs/src/lib/lente/tags.ts` makes exactly this argument for exactly this reason ("string
	 * scanning sidesteps the query entirely and is genuinely O(n)"). It is also the only version
	 * that is obviously correct for a term like `C++` or `N.A.S.A`, where the escape was the single
	 * thing standing between a term and a pattern that matches text nobody wrote.
	 *
	 * A boundary is anything that is not a letter, a digit or an underscore, tested with a one-char
	 * regex over a CONSTANT — no author text reaches a pattern.
	 */
	const WORD_CHAR = /[\p{L}\p{N}_]/u;
	// THE HAYSTACK IS EVERYTHING THE RECIPIENT GETS, not only the slide bodies. A caption is read
	// ALOUD over a slide they keep, so a term named there is a term they encounter — and pruning its
	// say-as leaves the narration mispronouncing a word the author took the trouble to teach.
	// Measured before this line existed: a deck whose slide 1 body said "Revenue grew" and whose
	// caption said "ARR grew forty percent" shipped the caption and dropped the ARR expansion.
	//
	// `fm` here has ALREADY been through `pruneCaptions`, so the captions in it are exactly the ones
	// that survive — a withheld slide's caption is gone by now and cannot keep a term alive. Only
	// the caption VALUES are taken; the registry blocks themselves are excluded, or every term would
	// trivially find itself.
	const captionText = (() => {
		const lines = blockLines(fm, 'captions');
		if (lines.length) return lines.map((l) => l.text).join('\n');
		const flow = /^[ \t]*captions:[ \t]*\{([^\n]*)\}[ \t]*$/m.exec(fm);
		return flow ? flow[1] : '';
	})();
	const haystack = captionText ? `${body}\n${captionText}` : body;
	const seen = (needle) => {
		const t = String(needle ?? '').trim();
		if (!t) return false;
		for (let at = haystack.indexOf(t); at !== -1; at = haystack.indexOf(t, at + 1)) {
			const before = at === 0 ? '' : haystack[at - 1];
			const after = at + t.length >= haystack.length ? '' : haystack[at + t.length];
	...
	// GROUPED FIRST, DECIDED SECOND. An entry is a header line plus the deeper lines under it, and
	// both forms the registry allows put the expansion somewhere different:
	//
	//   INLINE        ACME: { expansion: Acme Corporation, definition: "…" }
	//   BLOCK-OBJECT  ACME:
	//                   expansion: Acme Corporation
	//                   definition: "…"
	//
	// The first version of this walked the lines and treated anything deeper as "a stray line",
	// which dropped a block-object header and ORPHANED its children — they re-parented under the
	// previous surviving term and shipped the withheld definition attributed to IT. Measured:
	// `EBITDA  SECRETDEF we expect to lose the Acme suit.` on the glossary page, beside a run that
	// said it had dropped ACME. That is the same re-parenting defect `pruneCaptions` was fixed for
	// one commit earlier.
	//
	// The second version carried a flag per line and could not rescue a block-object entry at all,
	// because it stopped reading children the moment the header looked droppable. Grouping removes
	// the ordering question entirely: collect the entry, then ask one question about the whole of it.
	const groups = [];
	for (const line of entries) {
		if (line.indent === entryIndent) {
			const kv = line.text.match(/^([^:]+?)\s*:\s*([\s\S]*)$/);
			groups.push({ at: line.at, term: kv ? kv[1].trim() : null, rest: kv ? kv[2] : '', children: [] });
		} else if (groups.length) {
			groups[groups.length - 1].children.push(line);
		}
	}

	/** Every place this entry could name its expansion — inline on the header, or on a child line. */
	const expansionsOf = (g) => {
		const out = [];
		const inline = g.rest.match(/\bexpansion\s*:\s*("([^"]*)"|'([^']*)'|([^,}]*))/);
		if (inline) out.push((inline[2] ?? inline[3] ?? inline[4] ?? '').trim());
		for (const c of g.children) {
			const child = c.text.match(/^expansion\s*:\s*("([^"]*)"|'([^']*)'|(.*))$/);
			if (child) out.push((child[2] ?? child[3] ?? child[4] ?? '').trim());
		}
	...
	for (const g of groups) {
		if (!g.term) continue; // not an entry we can read; leave it exactly as written
		if (seen(g.term) || expansionsOf(g).some(seen)) { anyKept = true; continue; }
		drop.add(g.at);
		for (const c of g.children) drop.add(c.at);
		dropped?.push(g.term);
	}
	if (!drop.size) return fm;
	// A header whose every entry was dropped is left dangling, so it goes too.
...
function pruneCaptions(fm, kept) {
	...
	// left to decide is only whether an entry survives at all. Hoisted above both the flow branch
	// and the block branch, because each has to answer that same question.
	const survives = new Set(kept.map((authored) => authored + 1));
	// THE INLINE FLOW FORM OPENS NO BLOCK, so `blockLines` returned nothing and the whole map — every
	// withheld slide's caption, verbatim — rode into the envelope AND into the manifest's `config`
	// echo, where the engine's shallow front-matter parse surfaces it as a string. `--strip-captions`
	// already drops that key for exactly this reason; `--lens` did not. Pruned here with the
	// resolver's own `splitFlowPairs`, so the projection and the reader agree on where a pair ends.
	const flow = /^([ \t]*captions:[ \t]*)\{([^\n]*)\}[ \t]*$/m.exec(m[2]);
	if (flow) {
		const keptPairs = splitFlowPairs(flow[2]).filter((pair) => {
			const kv = /^\s*(?:"(\d+)"|'(\d+)'|(\d+))\s*:/.exec(pair);
			// A pair whose key cannot be read is not provably about a kept slide, so it goes. Failing
			// closed is the only safe direction for a channel that carries author text to a recipient.
			return kv ? survives.has(Number(kv[1] ?? kv[2] ?? kv[3])) : false;
		});
		const line = keptPairs.length ? `${flow[1]}{${keptPairs.join(',')} }` : null;
		const rebuilt = m[2]
			.split('\n')
			.map((l) => (l === flow[0] ? line : l))
			.filter((l) => l !== null)
			.join('\n');
		return m[1] + rebuilt + m[3];
	}
	...
	const drop = new Set();
	let anyKept = false;
	// ONE SPELLING WAS PRUNED AND THREE SHIPPED. This loop matched `^(\d+)\s*:` at exactly the entry
	// indent, so a caption written any other legal way rode into the recipient's envelope verbatim:
	//
	//   · a QUOTED KEY — `"4": "board only"` — failed the digit match, `continue`d, and survived;
	//   · a BLOCK SCALAR — `2: |` with an indented body — matched on its KEY line, which was dropped,
	//     while the deeper body lines were skipped as "a deeper stray line" and stayed. That is worse
	//     than a leak: the orphaned text RE-PARENTED onto the previous surviving entry, so a withheld
	//     slide's caption was spoken over a KEPT one. Measured: `1: One.` came out of the projection
	//     carrying two lines of the withheld slide's caption underneath it.
	//
	// A dropped entry now takes its continuation lines with it, and a surviving one keeps them.
	let carrying = null; // the entry line currently owning any deeper continuation lines
	for (const line of entries) {
		if (line.indent !== entryIndent) {
			// A continuation of whatever entry opened above it — it shares that entry's fate.
			if (carrying !== null) drop.add(line.at);
			continue;
		}
		// The key may be bare, "double-quoted" or 'single-quoted'; YAML permits all three and the
		// caption resolver accepts all three, so the prune has to read all three.
		const kv = line.text.match(/^(?:"(\d+)"|'(\d+)'|(\d+))\s*:\s*([\s\S]*)$/);
		if (!kv) { carrying = null; continue; }
		const num = Number(kv[1] ?? kv[2] ?? kv[3]);
		if (!survives.has(num)) { drop.add(line.at); carrying = line; }
		else { anyKept = true; carrying = null; }
	}
	if (!drop.size) return fm;
...
/**
 * IS THIS A HOLE? Once per representation, and nowhere else.
 *
 * A hole is asked about in four different shapes as it moves down the pipeline — a live DOM node, a
 * rendered `<section …>` open tag, a whole rendered section as a string, and a markdown chunk before
 * anything has rendered — and each shape needs a different test. That is fine. What is not fine is
 * what we had: SIX call sites, each with its own hand-written predicate, in FIVE different spellings.
 *
 * The cost of that was measured, twice. Five of the twelve findings in the seventh adversarial round
 * were a consumer pairing an authored-indexed array against a shipped-length one — the same class of
 * mistake, five times, because nothing named the boundary. And the ownership gate caught a SIXTH in
 * one of these very predicates: `\bclass="` instead of `\sclass="`, which also matches
 * `data-class="<raw _class payload>"` and would have read the directive an author typed instead of
 * the class list the engine resolved (#1358).
 *
 * The deeper point is that "authored index" and "shipped index" are two different types wearing the
 * same `number`. A language with newtypes would have found all five by compiling. We have JS, so the
 * substitute is: exactly one definition of the boundary, and `asShippedSlides` in the CLI as the one
 * function that crosses it. Anything hand-rolling a sixth predicate is the bug coming back.
 */
export const SHIPPED_SLIDES_SELECTOR = `section[data-lattice-slide]:not(.${HOLE_CLASS_NAME})`;
/**
 * A rendered `<section …>` OPEN TAG.
 *
 * Two things are load-bearing and each was got wrong once. `\sclass="` — the leading space — is the
 * #1358 guard: `\bclass="` also matches `data-class="<raw _class payload>"`, which is the directive
 * an author TYPED rather than the class list the engine resolved. And the class value is matched as
 * a whole TOKEN via the shared `HOLE_CLASS_ATTR`, not as a substring: `\blens-hole\b` matches across
 * a hyphen, so an ordinary author class like `lens-hole-note` read as a hole here while `holeDrift`
 * and the CSS (both exact-token) read it as content — three disagreeing answers to one question,
 * which is the exact defect this block of predicates exists to prevent. Demonstrated: a 3-slide deck
 * with one `lens-hole-note` slide shipped an `.html` claiming 2 slides, dropped that slide's PDF note
 * annotations, and mislabeled it in the sidecar — at exit 0, past `holeDrift`.
 */
export const isHoleOpenTag = (openTag) => {
	const cls = String(openTag ?? '').match(/\sclass="([^"]*)"/);
	return Boolean(cls) && HOLE_CLASS_ATTR.test(cls[1]);
};
/**
 * A whole rendered SECTION, as a string.
 *
 * NOT an alias for the open tag, and the difference is a measured defect rather than tidiness. This
 * string carries the slide's whole body, and author markup survives the engine — so the first
 * `class="…"` in it is the SECTION's class only because the engine happens to always emit one. On a
 * section without it, `<div class="badge lens-hole">` in ordinary author markup answered yes: the
 * notes join dropped one slide's speaker note and bound the next slide's note to it, in a sidecar
 * written one line under the CLI's own "3 slides with speaker notes". So the open tag is CUT out
 * here rather than hoped for, which makes the stated reason the actual reason.
 */
export const isHoleSectionHtml = (html) => {
	const open = /^\s*<section\b[^>]*>/.exec(String(html ?? ''));
	return Boolean(open) && isHoleOpenTag(open[0]);
};
/**
 * A MARKDOWN chunk, before anything has rendered — the projection's own hole body, exactly.
 *
 * Equality, not "contains the marker". Containment is unanchored and fence-blind: a slide that
 * DOCUMENTS this marker inside a ```md fence answered yes, and one deck doing that stood chart
 * narration down for the whole deck — under a message blaming the deck for a divergence this filter
 * had just introduced. Equality also makes this the same question `emptyHolePositions` asks, which
 * is the point of the block: a hole is a slide the projection emptied, and nothing else is.
 */
export const isHoleSourceChunk = (chunk) => String(chunk ?? '').trim() === HOLE_BODY.trim();

/**
 * WHICH SLIDES OF THIS SOURCE ARE ALREADY EMPTY HOLES — the export's own output, read back.
 *
 * A projected deck is markdown, and the obvious thing to do with markdown is render it again. That
 * has to work: the source in a shared player's envelope is a projected deck, and a tool whose output
 * its own input rejects is broken. Before this existed the CLI refused every re-export, under a
 * message that was FALSE for that file — "its text still ships in the .html" — when the projection
 * had already removed the text.
 *
 * What it must NOT do is weaken the check into "any hole is fine". The measured defect is a hole
 * with CONTENT under it: an author types `<!-- _class: lens-hole -->` on a real slide, the PDF drops
 * it, and its text ships anyway in the `.html` DOM and the envelope's verbatim source. So the test
 * is byte equality against the body the projection emits — nothing but the directive and an empty
 * comment. A chunk carrying a heading, a paragraph, a speaker note or a background image is not this
 * and is not accepted, which keeps every case `holeDrift` was written for.
 *
 * Byte equality rather than "looks empty" is deliberate. "Strip the comments and see if anything is
 * left" reads a speaker note — `<!-- note: … -->` — as emptiness, and a note is exactly the kind of
 * text a withheld slide is withheld for.
 *
 * @param {string} source full deck source, front matter included
 * @returns {number[]} authored indices whose chunk IS the projection's hole body
 */
export function emptyHolePositions(source) {
	const src = normalizeSourceText(String(source ?? ''));
	const fm = frontMatterBlockOf(src);
	const { entries } = chunksWithSeparators(src.slice(fm.length));
	return entries.flatMap((e, i) => (isHoleSourceChunk(e.chunk) ? [i] : []));
}
 ...
 *
 * A section with no `data-authored-slide` is skipped rather than failed. The written reason used to
 * be "an appended slide (the auto-glossary) carries no authored number" — measured false: the engine
 * stamps every top-level section it cuts, and the glossary page of a 3-slide deck reads
 * `data-authored-slide="3"`. The real reason is narrower and worth stating as such: the engine
 * stamps every section, `splitSections` is depth-aware and never surfaces a nested one as a piece,
 * and this runs before the measured split pass — so no reachable top-level section lacks the
 * attribute, and the branch is an unreachable fail-closed guard rather than a case being handled.
 *
 ...
 * same slide in the projection as in the full deck. "That family" is load-bearing: it is UNFILTERED
 * slot counting, not "positional CSS", and the difference is two more families below.
 *
 * A CSS COUNTER DOES NOT. `nth-of-type` is a STRUCTURAL selector and counts a `display:none` element;
 * counters live on the BOX tree, and a hidden element generates no box, so it does not increment.
 * Measured on two real PDFs of one deck: a `counter()` heading reads `#4` under `--lens full` and `#2`
 * under `--lens brief`. `counters()`, a `counter-reset` chain and `::marker` numbering are the same
 * shape, and so — by design rather than by accident — is the visible page number.
 *
 * AND NEITHER DOES ANYTHING THAT READS THE WITHHELD SLIDE, which is a THIRD family and the one the
 * two above hid. The hole holds the withheld slide's POSITION and carries nothing else — not its
 * class list, not a byte of its content — so a selector that counts slots holds and a selector that
 * asks a QUESTION ABOUT the slot does not. Measured in real Chrome on real exported files, an
 * 8-slide deck keeping 1/4/6/8:
 *
 *   · `:nth-child(3 of .kpi)` landed on slide 6 in the whole deck and matches NOTHING here — a
 *     FILTERED count is not a slot count, and the hole carries neither the class nor the content;
 *   · `section:has(blockquote) + section` matched slides 4 and 6, and matches NOTHING here;
 *   · `section:not(:has(blockquote)) + section` matched only kept slide 8 in the whole deck and
 *     matches 4, 6 and 8 here — it GAINS two slides, which is the direction that matters: a rule
 *     that HID something in the deck the sender previewed unhides it in the file they send.
 *
 * Those are the numbers `test/integration/export/lens-artifact-channels.test.js` measures, on its own
 * fixture, and they are asserted there. An earlier draft of this list quoted a DIFFERENT fixture's
 * numbers — "slide 8", "seven slides", "matched none" — because the fixture was rewritten in the same
 * commit and the prose was not re-derived. In a file whose subject is claims nobody re-derives.
 *
 * AND IN A `--player` CARRIER NONE OF IT APPLIES, which is a FOURTH thing to know and not a fourth
 * selector family — it is a format. The player wraps every slide in its own `.lp-frame`, so each
 * `section` is the only one in its parent and every cross-slide selector matches nothing, the
 * slot-counting ones included. Measured: `section:nth-of-type(3) .secret { display: none }` hides a
 * span in the PDF and the plain `.html`, and that span renders in the carrier. Two things make it
 * worth the space: it is the same UNHIDE direction as the third family above, and the player is the
 * ONLY format that can carry several views — so it is exactly where a multi-view sender ends up.
 * The frame-wrapping predates reader views and is not changed here; what was wrong was this file
 * claiming the slot-counting family holds without naming the format where it does not.
 *
 * This family is not closable by trying harder either, and the reason is worth stating: the only way
 * a hole could answer these the way the withheld slide did is by CARRYING the withheld slide's class
 * list and content, which is the disclosure the projection exists to prevent. A class name is author
 * text (`_class: acquisition-terms` names the thing), so even the class list alone is a leak. The
 * boundary is real, the warning states it, and the artifact-channel test pins the three breaking
 * families beside the six that hold so the record cannot drift back to "positional CSS is safe".
 *
...
export const REFUSAL_REASONS = {
	...
	'unprunable-narration': 'a narration block in the front matter (`captions:`, `acronyms:` or `lexicon:`) is written in a form this export cannot prune, so it would ship every entry — including entries about slides this view withholds. Put the block on its own line as a plain `key:` header with one entry per line beneath it, or write the whole map on ONE line as `key: { … }`, with nothing after it',
