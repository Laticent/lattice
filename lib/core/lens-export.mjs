...
export function projectForExport(source, requestedIds, options = {}) {
	...
	const prunedFm = pruneAcronyms(pruneCaptions(pruneRegistry(fm, keptDefs, defaultId), kept), body, reducing, prunedAcronyms);
	...
	if (identity && tagsUnchanged && prunedFm === fm) return { ok: true, source: src, views, kept, total: slides.length, default: defaultId, prunedAcronyms };

	return { ok: true, source: prunedFm + body, views, kept, total: slides.length, default: defaultId, prunedAcronyms };
}
 ...
 */
function pruneAcronyms(fm, body, reducing, dropped) {
	if (!fm || !reducing) return fm;
	const m = /^(---(?:\r\n|\r|\n))([\s\S]*?)((?:\r\n|\r|\n)---[ \t]*(?:\r\n|\r|\n)?)$/.exec(fm);
	if (!m) return fm;
	const entries = blockLines(m[2], 'acronyms');
	if (!entries.length) return fm;

	const seen = (needle) => {
		const t = String(needle ?? '').trim();
		if (!t) return false;
		return new RegExp(`(?:^|[^\\p{L}\\p{N}_])${t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:[^\\p{L}\\p{N}_]|$)`, 'u').test(body);
	};
	...
	for (const line of entries) {
		...
		if (!kv) continue;
		const term = kv[1].trim();
		// The expansion, when the entry uses the `{ expansion, definition }` form. A bare
		// `TERM: text` entry is the caption spoken-form and appends no glossary row at all.
		const exp = kv[2].match(/\bexpansion\s*:\s*("([^"]*)"|'([^']*)'|([^,}]*))/);
		const expansion = exp ? (exp[2] ?? exp[3] ?? exp[4] ?? '').trim() : '';
		if (seen(term) || seen(expansion)) anyKept = true;
		else { drop.add(line.at); dropped?.push(term); }
	}
	if (!drop.size) return fm;
	// An `acronyms:` header whose every entry was dropped is left dangling, so it goes too.
