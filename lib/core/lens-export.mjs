 ...
 *
 *   · `:nth-child(3 of .kpi)` landed on slide 6 in the full deck and slide 8 in the projection — a
 *     FILTERED count is not a slot count, and the hole matches no filter;
 *   · `section:has(blockquote) + section` matched seven slides in the full deck and NONE here;
 *   · `section:not(:has(blockquote)) + section` matched none in the full deck and THREE KEPT SLIDES
 *     here — the gain direction, where a rule that HID something in the deck the sender previewed
 *     unhides it in the file they send.
 *
