 ...
 * below slot there would EAT a coda that happened to be bracketed. Opting in
 * per slot keeps the lift from consuming a construct the component does not
 * have. Both default to on, because a component that owns both is the normal
 * case and forgetting a flag should not silently disable a feature.
 *
 * @param {string} html  the section's html
 * @param {{above?: boolean, below?: boolean}} [slots]  which slots to consume
 ...
 */
function liftBracketSpans(html, slots) {
  const wantAbove = !slots || slots.above !== false;
  const wantBelow = !slots || slots.below !== false;
  ...
  while ((m = re.exec(src)) !== null) {
    ...
    if (isAbove && wantAbove && above === null) above = text;
    else if (!isAbove && wantBelow && below === null) below = text;
    else continue;
