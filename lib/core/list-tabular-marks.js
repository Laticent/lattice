...
/**
 * The block-level tags a marks bullet may not contain.
 *
 * A marks cell is a status: one line of inline content, laid out as a flex row in a
 * capped trailing column. Anything block-level in there is prose, and the CSS has no
 * good answer for it — `> p { display:contents }` flattens whatever follows the
 * marker into that flex row, so a fenced code block or a nested list arrives as
 * uppercase mono squeezed beside the pill.
 *
 * The list is shared because the two paths kept disagreeing about it in DIFFERENT
 * directions. First the engine promoted a bullet with a second paragraph and the DOM
 * mirror did not (the engine stripped the marker, a Marp render kept it). Then a
 * token-side fix over-corrected: it counted only `*_open` tokens, so `fence`,
 * `code_block`, `html_block` and `hr` — which have no `_open` form — still passed,
 * while a NESTED LIST now made the engine reject an item the mirror still promoted,
 * reversing the drift. One list, checked on both sides, is the only shape that holds.
 */
const BLOCK_TAGS = new Set([
  'P', 'UL', 'OL', 'PRE', 'BLOCKQUOTE', 'DIV', 'HR', 'TABLE', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'DL', 'FIGURE',
]);

/**
 * Is this bullet a single block of inline content? The DOM half of the rule above.
 *
 * `contentHost` has already unwrapped a lone `<p>`, so anything block-level still
 * reachable from the host is a second block — including one nested inside that `<p>`,
 * which markdown-it does not produce but a hand-authored HTML bullet can.
 */
function isSingleBlock(li, host) {
  const blocks = [...li.querySelectorAll('*')].filter((el) => BLOCK_TAGS.has(el.tagName));
  // The unwrapped `<p>` is the host itself, not a second block.
  return blocks.filter((el) => el !== host).length === 0;
}

module.exports = {
  ROW_MARKER, MARKS_CLASS, MARKER_STATES, BLOCK_TAGS,
  readMarker, isPillsOnly, stateHtml, stateNode, contentHost, isSingleBlock,
};
