...
function readLegacyBlocks(html) {
  ...
  const slides = [];
  // A literal, not built from LEGACY_BLOCK_MIME: the MIME is a fixed constant, and a literal needs
  // no escaping step to get wrong. The unit tests pin LEGACY_BLOCK_MIME to the export's
  // AUDIO_BLOCK_MIME and run this pattern over a real export.
  const re = /<script type="application\/lattice\+audio" data-lp-audio="(\d+)">([\s\S]*?)<\/script>/g;
  for (const m of src.matchAll(re)) {
    const i = Number(m[1]);
    // The export escapes every `<` as the JSON escape for it; JSON.parse decodes it back. A block
    // that does not parse is a SILENT slide, which is what the player's `loadCues` makes of it —
    // one damaged block must not lose the whole deck.
    try {
...
function lttFromLegacy(opts) {
  ...
  for (let i = 0; i < slideCount; i++) {
    ...
    const tailMs = Math.max(0, Math.round(cues[cues.length - 1]?.g || 0));
    segments.push({ id: `d${i + 1}`, kind: 'slide', at, hash: segmentHash(text, inputs), basis: 'legacy', holdMs, track: trackFromBlock(cues), tailMs });
  }
