 ...
 */
function withRuntimeScripts(deckSource, { localAssets = true, overflowMarker } = {}) {
  ...
  const deck = withoutExportSettingsBlock(withoutFrontMatterBlock(deckSource))
    .replace(RUNTIME_SCRIPTS_RE, '')
    .replace(/\s*$/, '');
  const settings = exportSettingsBlock({ overflowMarker });
  return `${deck}\n${RUNTIME_SCRIPTS}${frontMatterBlock(deck, { localAssets })}${settings}`;
}
...
scrollable and it was not printed. Rather than lose it quietly, this deck tags such
a slide **"Content clipped"** along its bottom edge. The tag is drawn over the slide
and never changes its layout; it is a label on a loss, not a way to recover the
content. Trimming it, or moving it to a layout with more room, is the only fix.

The tag's wording was chosen when this deck was exported, and it is recorded in the
generated settings block at the end of the file — not in the front matter, so
editing the top of the deck will not change it. Re-export from Lattice to choose
differently: \`--overflow-marker=author\` for the full editing signal (a red ring and
per-cell "Fix Me" tags), or \`--overflow-marker=off\` for no marker at all.
...
function resolveExportOverflowMarker({ chosen = null, workspace = null } = {}) {
  ...
  if (isKnownOverflowMarker(chosen)) return { marker: norm(chosen), source: 'this export', ignored: null };
  if (isKnownOverflowMarker(workspace)) return { marker: norm(workspace), source: 'workspace setting', ignored: null };
  // A stored value that is no longer a level is REPORTED, not swallowed. It fell
  // back silently in the deck-key version, so an author whose setting had gone
  // stale got the default with nothing telling them why.
  const ignored = workspace != null && String(workspace).trim() !== '' ? String(workspace) : null;
