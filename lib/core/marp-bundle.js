...
const STATIC_ASSETS = Object.freeze([
  ...
  // eager path (26.5 KB gzipped on every reader of every deck, for something only a
...
const RUNTIME_SCRIPTS = [
  ...
  // BEFORE the runtime tag, and that order is the mechanism: both are classic
  // scripts, so they execute in document order, and the runtime's state-chart pass
  // reads `globalThis.__latticeDagre` synchronously on its first draw. After it, the
  // engine would arrive too late and every branching machine would paint as a column.
...
const RUNTIME_SCRIPTS_RE = new RegExp(
  '\\n*<!-- markdownlint-disable MD033 -->'
  + '(?:\\n<script src="(?:' + RUNTIME_SCRIPT_SRCS.map((f) => f.replace(/\./g, '\\.')).join('|') + ')"></script>)+'
  + '\\s*$',
);
