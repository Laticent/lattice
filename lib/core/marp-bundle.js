...
/** The exact tag block above, for stripping it back off a deck we already baked. */
const RUNTIME_SCRIPTS_RE = new RegExp(
  RUNTIME_SCRIPTS.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\n/g, '\\n'),
  'g',
);
 ...
 * `assets/…` while the in-browser producer passes `localAssets: false`.
 *
 ...
 */
function withRuntimeScripts(deckSource, { localAssets = true } = {}) {
  ...
  const deck = withoutFrontMatterBlock(deckSource)
    .replace(RUNTIME_SCRIPTS_RE, '')
    .replace(/\s*$/, '');
  return `${deck}\n${RUNTIME_SCRIPTS}${frontMatterBlock(deck, { localAssets })}`;
}
...
which would turn this deck's trailing \`<script>\` tags into visible text and
...
\`\`\`sh
npx @marp-team/marp-cli ${file}.md --theme-set lattice.css themes \\
  --html --allow-local-files -o ${file}.pdf
\`\`\`
...
scrollable and it is not printed. So the deck says so rather than losing it
quietly: an overflowing slide is tagged **"Content clipped"** along its bottom edge.

\`overflow-marker:\` in the front matter (and the snapshot block) decides who that
signal is for:

| value | what a slide that overflows shows |
|---|---|
| \`reader\` | a small "Content clipped" pill along the bottom edge — the export default |
| \`author\` | a red ring, an "Overflows" corner flag, per-cell "Fix Me" overlays, and the small-type alarm — for fixing the deck, not for delivering it |
| \`off\` | nothing. The clip is silent, so only use it once you know the deck fits |

The marker is drawn over the slide and never changes its layout. It is a label on a
loss, not a way to recover it — the clipped content is not scrollable and is not
printed, so trimming it, or moving it to a layout with more room, is the only fix.
...
/**
 * Resolve the bundle's `overflow-marker:` policy and WRITE it into the deck — the
 * one place both producers agree, so the CLI and the in-browser Studio cannot ship
 * different defaults (HARD RULE #1).
 *
 * That split is not hypothetical: before this lived here, only the CLI wrote the
 * key, and the Studio's export inherited the runtime's fallback instead. Because
 * `frontMatterBlock` bakes nothing for a deck with NO front matter, a
 * front-matter-less deck exported from the Studio carried no baked block at all, so
 * the runtime read it as an authoring surface and drew the red ring and the "FIX
 * ME" overlays — the originally reported defect, unfixed, on the export path a
 * non-CLI user actually uses. Writing the key manufactures the front matter when
 * absent, which closes that hole for both producers at once.
 *
 * @param {string} deckSource the deck markdown (front matter still attached)
 * @param {string|null} override a validated CLI/UI level, or null to use the deck's
 * @returns {{deck: string, marker: string, source: string, ignored: string|null}}
 *   `ignored` is the deck's raw value when it was present but unrecognized — the
 *   caller is expected to SAY so, because the write below overwrites it.
 */
function withResolvedOverflowMarker(deckSource, override = null) {
  const deckValue = readOverflowMarker(readFrontMatterBlock(deckSource));
  const deckKnown = isKnownOverflowMarker(deckValue);
  const marker = override !== null && isKnownOverflowMarker(override)
    ? override.trim().toLowerCase()
    : (deckKnown ? deckValue.trim().toLowerCase() : EXPORT_DEFAULT_MARKER);
  const source = override !== null && isKnownOverflowMarker(override)
    ? 'override'
    : (deckKnown ? 'deck front matter' : 'default');
  return {
    deck: withFrontMatterKey(deckSource, 'overflow-marker', marker),
    marker,
    source,
    ignored: override === null && deckValue !== null && !deckKnown ? deckValue : null,
  };
}
