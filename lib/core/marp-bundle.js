...
const { FRONT_MATTER_TYPE, readFrontMatterBlock, frontMatterBlock } = require('./deck-front-matter');
 ...
 * stylesheet dist/ ships, so this pass is a semantic no-op on a current dist and
 * exists as belt-and-braces for a bundle built against an older one. (It is NOT
 * byte-for-byte on a minified sheet: it re-joins comma lists with `, `, which
 * adds ~262 bytes. Harmless, but don't assert byte equality.)
 */
 ...
 * two bundles carry a byte-identical deck.
 *
...
function withRuntimeScripts(deckSource) {
  return `${deckSource.replace(/\s*$/, '')}\n${RUNTIME_SCRIPTS}${frontMatterBlock(deckSource)}`;
}
...
function readme({ name, palette, themes, agent }) {
  // Prose keeps the deck's own title; every PATH and COMMAND uses the safe slug,
  // which is the name both producers write the deck file under.
  const file = safeName(name);
...
Marp tool lets a plugin in — and math is typeset by a different library. They
degrade rather than break, and this is the complete list:

${fidelityNotes().map((line) => line).join('\n')}

Everything else — every component layout, the palette, the typography, Mermaid,
and the deck-wide registers — renders the same as it does in Lattice.
...
| Path | What |
|---|---|
| \`${file}.md\` | the deck — splits baked to \`---\`, image paths localized, runtime \`<script>\` tags appended |${agent ? `
