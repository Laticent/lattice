...
//
// THE FILENAMES ARE THE ONE LIST, and the tag block is derived from them. They used
// to be two hand-maintained arrays — this block, and RUNTIME_SCRIPT_SRCS below — that
// had to agree for the strip regex to find what the bake wrote. Nothing checked that
// they did. The same shape had already gone wrong one level up, where the agent kit's
// README said "the two runtime <script> tags" while this list emitted three: dagre was
// split out of the runtime bundle and the second copy stayed behind. Deriving one from
// the other means a fourth engine is added in exactly one place.
//
// ORDER IS THE MECHANISM, so this array's order is load-bearing: dagre sits BEFORE the
// runtime tag because both are classic scripts, so they execute in document order, and
// the runtime's state-chart pass reads `globalThis.__latticeDagre` synchronously on its
// first draw. After it, the engine would arrive too late and every branching machine
// would paint as a column.
const RUNTIME_SCRIPT_SRCS = ['mermaid-v11.min.js', 'lattice-dagre.min.js', 'lattice-runtime.min.js'];
...
const RUNTIME_SCRIPTS_RE = new RegExp(
  '\\n*<!-- markdownlint-disable MD033 -->'
  + '(?:\\n<script src="(?:' + RUNTIME_SCRIPT_SRCS.map(reEscape).join('|') + ')"></script>)+'
  + '\\s*$',
);
...
function withRuntimeScripts(deckSource, { localAssets = true, overflowMarker } = {}) {
  ...
  const deck = withSanitizedDeckClass(withoutExportSettingsBlock(withoutFrontMatterBlock(deckSource)))
