 ...
 * (`applyFormToHtml`, `applyBackdropToHtml`, `applyDeckLogoToHtml`,
...
const LEDGER = Object.freeze([
  ...
  {
    plugin: 'tableRowLabels',
    what: 'the `lat-row-label` stamp that turns first-column row-label emphasis on',
    coverage: 'mirrored',
    via: 'transformTableRowLabels',
  },
  {
  ...
  {
    plugin: 'inlinePills',
    what: 'the `{LABEL}:shape:c4` inline-pill grammar',
    coverage: 'mirrored',
    via: 'transformInlinePills',
  },
  {
    plugin: 'proseCodeMark',
    what: 'the `data-prose` mark that keeps a sentence holding one code span out of the eyebrow/subtitle rules',
    coverage: 'mirrored',
    via: 'transformProseCode',
  },
  {
    plugin: 'escapeMarks',
    what: 'the mark on a backslash-escaped arrow character (`\\->`, `\\&`) that a flowchart reads as text',
    coverage: 'unmirrored',
    note: 'the character shows as typed; a flowchart there reads an escaped `->` as an arrow',
  },
  {
  ...
  {
    plugin: 'registerShellHljs',
    what: "Lattice's augmented shell grammar — modern CLI tools as built-ins and "
      + '`--flags` as params, on ```bash / ```sh / ```zsh',
    coverage: 'unmirrored',
    note: 'A Marp render highlights with marp-core\'s OWN bundled highlight.js, '
      + 'which Lattice never touches, so a shell fence falls back to stock bash. '
      + 'Observed on a real marp-cli 4.5.0 render of the bundle: a command list '
      + '(npm / docker / kubectl) comes out with ZERO tokens. Scripts are barely '
      + 'affected — stock bash still colors keywords, strings and POSIX built-ins; '
      + 'what a recipient loses is the modern-command and flag color.',
  },
