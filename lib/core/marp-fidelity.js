 ...
 *   - `test/unit/core/marp-fidelity.test.js` — the gate. Every `function x(markdown)`
 *     in lib/integrations/markdown-it/plugins.js must appear here, every entry must
 *     still exist there, and every claim of coverage must point at code that really
 *     exists. Add a plugin without classifying it and the gate fails.
...
const LEDGER = Object.freeze([
  ...
  // Not a plugin — a renderer difference, and the one gap that is a difference in
  // HOW something renders rather than WHETHER it does.
  {
