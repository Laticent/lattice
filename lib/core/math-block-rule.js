...
/**
 * Install the rule on a markdown-it instance. `.before('fence')` matters: a
 * ```` ``` ```` fence and a `$$` block are both "opaque run of lines", and the
 * fence rule would otherwise claim a `$$` block whose body contains one.
 */
