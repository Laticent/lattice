 ...
 *
 * Reason: `canvas` places its body with `grid-template-areas` on the SECTION.
 *
 * THREE ENTRIES CAME OFF THIS LIST DURING THE MIGRATION — `derivation`, `theorem` and
 * `stats` — and that is the more useful lesson than any entry left on it. All three
 * said "already single-column", which answers the wrong question.
 *
 * (`theorem` is back, on a different and measured reason: its card padding was re-tuned
 * for the stage at every size once a committed deck showed the old rhythm did not fit at
 * `hd` either, and that re-tune covers the tall box with room to spare. An entry earned
 * by measurement, not by counting columns.) A family reflow is
 * not only for collapsing columns; it is for anything that has to give when the box
 * changes shape, and every one of the three had something. Two paid for the Form
 * masthead, an IN-FLOW band where the sovereign arm's title was absolutely positioned
 * and cost nothing (271px at portrait, on a wrapped title). The third, `stats`, was
 * over for an unrelated reason its sovereign arm had always carried: a `max-width`
 * measure cap that helps on a 1920-wide slide and hurts in a 972-wide one.
 *
 * So: MEASURE the migrated variant at every registered size before writing an entry
 * here. An entry is a claim about a render, not about a column count.
 *
...
const MIGRATED = new Set(['feature', 'compare', 'derivation', 'theorem', 'stats', 'matrix', 'decompose', 'canvas']);
