 ...
 *
 * Reasons: `theorem` and `stats` are already single-column, so a tall box needs
 * nothing collapsed. `canvas` places its body with `grid-template-areas` on the
 * SECTION.
 *
 * `derivation` WAS on this list and came off it, which is the more useful lesson than
...
const MIGRATED = new Set(['feature', 'compare', 'derivation']);
