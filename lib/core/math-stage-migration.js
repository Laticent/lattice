 ...
 *
 * `decompose` is deliberately checked BEFORE `matrix`: it is authored as
 * `math matrix decompose`, a compound of `matrix`, so a first-match scan over the
 * token list would call it `matrix` and migrate it a commit early.
 *
...
function mathVariantOf(cls) {
  ...
  if (tokens.includes('decompose')) return 'decompose';
...
module.exports = { MATH_VARIANTS, MIGRATED, mathVariantOf, mathSectionMigrated };
