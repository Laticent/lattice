 ...
 *
 * Reasons are the stylesheet's own (the `Family reflow` block's header comment):
 * `derivation`, `theorem` and `stats` are already single-column, so a tall box needs
 * no collapse; `compare` uses multi-`column-count` and `canvas` uses
 * `grid-template-areas`, both properties on the SECTION element that a container
 * query cannot reach, so they stay landscape by design.
 */
const NO_FAMILY_REFLOW = Object.freeze({
  derivation: 'already single-column — nothing to collapse in a tall box',
  theorem: 'already single-column — stacked blockquote cards',
  stats: 'already single-column — vertical estimate/interval scaffold',
  compare: 'multi-`column-count` is a section-element property a container query cannot reach',
  canvas: '`grid-template-areas` is a section-element property a container query cannot reach',
});

const MIGRATED = new Set(['feature']);
