 ...
 *
 * WHAT IT STILL IS NOT, AND WILL NOT BE: a CommonMark parser. Two shapes the engine draws
 * are deliberately not substituted, and the decision was taken on measurement rather than
 * left open (#2092's branch):
 *
 *   · a fence indented more than three spaces INSIDE A LIST. CommonMark allows a fence up to
 *     the list item's content indent plus three, so seeing it means modelling list markers,
 *     nested items, lazy continuation and blank-line termination — a block parser. Getting
 *     that wrong turns an INDENTED CODE BLOCK, which an author wrote as literal text, into a
 *     picture: an OVER-match, the one thing the conformance corpus budgets at zero. The miss
 *     only bites at the SECOND list level — a top-level bullet's content indent is 2, and a
 *     fence at 2 already substitutes.
 *   · a BLOCKQUOTED fence. Beyond stripping the markers, `preprocessMermaid` splices an SVG
 *     into the SOURCE, and an HTML block inside a blockquote needs every one of its lines
 *     re-prefixed or the quote ends at the first bare line. So a 12KB SVG would have to be
 *     re-quoted, and getting the container model wrong breaks the slide rather than the
 *     diagram.
 *
 * In both, a miss means the author sees their own source — the safe direction, and the
 * standing argument for leaving them. That argument was checked rather than repeated, and it
 * was INCOMPLETE for the blockquote: the export shows source, but `createFenceReader` trimmed
 * whitespace and not quote markers, so the NARRATOR read the definition aloud —
 * "mermaid. flowchart LR. A[\"Alpha\"] --> B[\"Beta\"]" into the .vtt, while the slide showed the
 * picture. That half is fixed (the reader is blockquote-aware now; blanking cannot over-match,
 * which is why the two halves are decided differently). The deep-list shape narrates cleanly
 * already, because the reader accepts any indent.
 *
 * So what is left is one divergence, on one surface: a preview draws these two shapes and the
 * PDF prints their source. Reopen it only with a CommonMark block model, and keep the corpus's
 * over-match budget at zero while you do.
 */
