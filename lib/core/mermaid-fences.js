 ...
 * reaches across a `---` line deletes it. Measured on the real CLI, twice: a three-slide deck
 * exported as ONE page with a slide of prose gone, and a substituted block torn in half across
 * a boundary, leaving an unclosed `<pre>` on one slide and a stray `</pre>` inside the author's
 * code block on the next. Both came from a fence the walker thought was still open — an
 * indented closer it did not recognize, or a fence left open inside a list item, which is a
 * block context this walker deliberately does not model.
 *
 * Rather than try to be right about those, refuse: a body containing a bare `---` line is not
 * substituted at all. The author sees their source, which is recoverable; a destroyed slide is
 * not.
 *
 * THE ONE EXCEPTION IS MERMAID'S OWN FRONT MATTER, which is a `---` block at the very start of
 * the definition (`---\nconfig:\n  theme: base\n---`). That is legitimate, common, and inside
 * a fence the engine never splits on — so it is allowed, and only there.
 *
...
function bodyCrossesSeparator(lines) {
	...
	let from = 0;
	// A leading front-matter block: `---` on the first body line, closed by the next `---`.
	if (lines.length && isRule(lines[0])) {
		const close = lines.findIndex((l, i) => i > 0 && isRule(l));
		if (close !== -1) from = close + 1;
	}
	return lines.slice(from).some(isRule);
}

/**
 * Strip inline-code spans, so a `<!--` an author is WRITING ABOUT does not open a comment.
 *
 * Measured: one slide explaining that "every speaker note begins with the `<!--` marker"
 * turned comment state on for the rest of the deck, and every diagram after it printed as
 * source. Backticked runs only — a bare `<!--` in prose really does open a comment.
 */
...
function matchMermaidFences(source) {
	...
	for (const line of src.split('\n')) {
		...
		if (!open) {
			...
			const scan = withoutInlineCode(line);
			for (let k = 0; k < scan.length; ) {
				if (!inComment) {
					const o = scan.indexOf('<!--', k);
					if (o === -1) break;
					inComment = true;
					k = o + 4;
				} else {
					const c = scan.indexOf('-->', k);
					if (c === -1) break;
					inComment = false;
					k = c + 3;
				}
