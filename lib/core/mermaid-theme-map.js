 ...
 *
 * It used to be two copies — 166 keys in lattice-emulator.js, 132 in
 * lib/runtime/index.js, the runtime a strict subset — kept in sync BY COMMENT.
 * The runtime's copy literally said "see the `clusterBkg` note in the emulator's
 * MERMAID_VAR_MAP": a cross-file prose pointer where an import belonged. Missing
 * on the preview path were `cScale1/3/5/7/9/11`, eight `cScaleLabel` slots, the
 * upper half of the pie cycle, and 20 more — a gap nobody could see, because
 * nothing compared the two. Now the gap is unrepresentable: there is one map, so
 * a key exists on both paths or on neither.
 *
