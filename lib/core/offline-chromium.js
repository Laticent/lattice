 ...
 *   - the resolver rule fails every host, so no DNS query leaves either, and it fails the
 *     proxy's own address, so nothing reaches even a live listener on the proxy port.
 ...
 *
 * Port 9 is the discard port and normally nothing listens there. If something DOES listen on
 * 127.0.0.1:9, it still sees nothing: the resolver rule maps EVERY host, the proxy's own
 * 127.0.0.1 included, so the browser never opens a connection to the proxy at all. Measured on
 * Chromium 131 with a listener on port 9: 0 connections with the rule, and without it every
 * request, IP literals and host names alike, arrived there as a proxy request naming its target
 * (test/integration/export/export-remote-subresource.test.js pins both). So the resolver rule is
 * the layer that holds if the dead proxy turns out to be alive.
 *
...
const OFFLINE_CHROMIUM_ARGS = Object.freeze([
  ...
  // A second layer: every host fails to resolve, so nothing reaches DNS, and `MAP *` maps IP
  // literals too, the proxy's 127.0.0.1 included, so nothing reaches the proxy port either.
  // file:, data: and blob: need no resolver.
