...
//
// THE RUNNING-FOOTER BAND IS NOT IN THIS LIST — and, as of the second trio round, has
// no exemption anywhere (see `probeContentClipped`). It was named here for one commit,
// because reporting the footer's accepted ellipsis made the new channel shout about a
// dated decision on most of its first hits. Two things were wrong with that. The narrow
// one: `IGNORED_CLIP_SELECTOR` is matched with `closest()`, so naming the band silenced
// its whole SUBTREE, while `2026-07-27-footer-band-allocation.md` prices exactly one
// thing — the HORIZONTAL `text-overflow: ellipsis` tail of the footer's own TEXT. The
// band also carries `height: 1.6em; overflow: hidden` (a VERTICAL clip) and `footer:`
// passes raw HTML, so the blanket entry swallowed a sliced confidentiality mark and an
// 80px footer logo, losses nobody has ever priced and that reported correctly before
// this branch touched them. The broad one: the same doc asks to be TOLD about the tail
// it prices, so there should be no exemption at all. Both corrections came from the
// HARD RULE #25 passes, one round apart.
...
function probeContentClipped(s, ignoreSelector, TOL) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    while (anc) {
      ...
      // NO EXEMPTION FOR THE RUNNING FOOTER — deliberately, after shipping one and
      // taking it back out. The argument for it was that
      // `2026-07-27-footer-band-allocation.md` §"What it costs, accepted deliberately"
      // has already priced the footer's ellipsis, so re-reporting it is noise: it was
      // 11 of this channel's first 13 corpus hits. But that doc does not ask for
      // silence, it asks for the opposite, twice: "Anyone whose footer is legally
      // operative should keep it short enough to fit, and THAT IS NOT ENFORCED OR
      // WARNED ABOUT", and then, naming the fix, "the cheapest opening is option (d)
      // — route over-subscription into the existing alarm so the author is told —
      // which stands on its own again now that truncation, unlike the overprint it
      // replaced, is invisible … the only option that closes the CLASS rather than an
      // instance." This probe IS option (d). Exempting the footer would have shipped
      // the mechanism that closes the class with the class carved out of it.
      // "Cry wolf" is a rule about FALSE positives. These are true: a legally
      // operative confidentiality line is being deleted from the exported text layer,
      // and in portrait roughly three quarters of it. The corpus noise was real
      // signal about our own decks, and it was answered where it belongs — by
      // shortening the footers, not by muting the alarm.
      if (vCut || hCut) {
