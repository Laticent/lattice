...
// signal runs long ("Option 1 of 6 · comparing …") the label ellipsises BY RULE — a pill that
...
function probeSectionOverflow(s, clipSelector, TOL, ignoreSelector) {
  ...
  // clipping box? Such a box is not failing to hold its content; ellipsising is the
  ...
  // portrait that fired on 24 of 71 pages — `_footer:` values generated from the
  ...
  function truncatesOwnLine(el) {
    ...
    // inline end, and a visible-overflow box paints past the edge instead of ellipsising.
    ...
    // …AND THE BOX MUST ACTUALLY BE ABLE TO ELLIPSISE. `text-overflow` acts on a block
    ...
    // a block container with a text node ellipsises; flex and grid containers hard-clip
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    // See `truncatesOwnLine`: an ellipsising nowrap box is handling this axis by design,
