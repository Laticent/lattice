...
function probeFigureLegibility(s, floorRatio) {
  ...
  //
  // It also lands the report in the units the deck's OWN type is documented in --
  // lib/typography/scale.js states the landscape roles as HD points (meta 11.25pt, body
  // 16pt) -- so "5pt" is directly comparable to the smallest type the deck sets anywhere.
  // On a non-landscape preset the pt is therefore "as if printed at HD" rather than a
  // measurement of that deck's own PDF page; that is the trade an invariant floor makes,
  // and it is the same trade the ratio itself makes.
  //
