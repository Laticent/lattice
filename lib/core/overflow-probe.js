...
const { BERTHS, BERTH_ATTR, RAIL_CLASS } = require('./fit-berth');
...
/**
 * THE FRAME TOLERANCE, in layout px — the noise budget every fit verdict is read
 * against, and the single source for it (HARD RULE #1). It used to be a bare `12`
 * written out four times, with the reasoning in one of them.
 *
 * WHAT IT IS FOR. `scrollHeight - clientHeight` is not zero on a slide that fits:
 * nested flex/grid borders and shadows push it a few px past `clientHeight` with
 * nothing visibly cut, so a verdict taken at zero would ring on layouts that are
 * fine.
 *
 * WHY 12, AND WHY THAT NUMBER IS NOW OVERSTATED. The original justification was
 * "the smallest real bug observed in the gallery was a 211px overshoot", so any
 * value under 211 was free. That has not been true for a long time. #2252 measured
 * a gantt painting 10.2px outside `.cell-stage`, which is `overflow: clip`, with
 * the export reporting nothing — and a sweep of all 335 shipped decks (4043 slides,
 * real Chromium, each rendered AS AUTHORED, 2026-09-21) puts real numbers on the
 * window. Measured the way the advisory reads it — the VERTICAL excess, `scrollH -
 * clientH` — so the buckets sum to their own total:
 *
 *   excess past the frame   slides the export says nothing about
 *   ─────────────────────   ───────────────────────────────────
 *   0                       3955   ← nothing to say, BY THIS MEASURE
 *   (0, 2]                  22     ← mostly phantom: 14 are ONE split-panel slide
 *                                    shape at exactly 2px (13 of them the same page
 *                                    across the token-contrast decks), and
 *                                    probeContentClipped answers cut:false on 19 of
 *                                    the 22 — which is NOT the same as "no ink
 *                                    outside a box": 5 of those 19 paint outside
 *                                    .cell-stage without losing a bearer across the
 *                                    edge, up to 1.75px
 *   (2, 3]                  2
 *   (3, 12]                 34     ← REAL, and silent (18 of them cut:true)
 *
 * 27 of those slides, across 20 decks, are ones `probeContentClipped` calls a cut
 * at zero tolerance, and 26 are BODY content rather than caption-footer chrome:
 * five `authority-chain` pages lose "$245M consent order — operationalized …", a
 * kanban card loses "Palette tune,", an actors table loses "Row one".
 *
 * SO WHY IS IT STILL 12? Because this constant is not only a report threshold — it
 * feeds `buildSplitVerdict`, and therefore AUTOSPLIT. Lowering it re-decides which
 * slides split across the whole corpus, which moves committed PDFs and page-count
 * assertions, and it adds 34 slides to the `overflow:check` ratchet that nobody has
 * fixed yet. That is its own change with its own remediation, not a constant edit.
 * The recommendation, the full distribution and what it would cost are recorded in
 * `engineering/decisions/2026-09-21-frame-tolerance-silent-window.md`.
 *
 * What this file does instead is STOP THE WINDOW BEING SILENT: `NEAR_MISS_FLOOR`
 * below, and the emulator line that reads it.
 */
const FRAME_TOLERANCE = 12;

/**
 * The floor for "close enough to the tolerance to be worth saying out loud".
 *
 * Taken from the distribution above, not from taste: the (0, 2] band is dominated by
 * a single slide shape that `probeContentClipped` answers cut:false on, and above 3
 * the measured ink tracks the number (excess 4 -> 4.02px of a `<strong>` outside its
 * box, 6 -> 5.58, 11 -> 10.64). So 3 is where THIS MEASURE stops being noise.
 *
 * "cut:false" IS NOT "no ink outside any box", and an earlier draft of this line said
 * the second thing — the same equivalence the FRAME_TOLERANCE docblock above retracts,
 * left standing 25 lines lower. Two separate reasons it is wrong. First, 5 of the 19
 * cut:false slides in (0, 2] DO paint outside `.cell-stage`, up to 1.75px, without
 * losing a bearer across the edge. Second, the sweep's paint metric only ever walks
 * `.cell-stage` descendants, while CLIP_CELL_SELECTOR names four boxes — and the
 * dominant shape here, the `split-panel metric` slide, carries NO `.cell-stage` at all
 * (checked on a real render: its clip boxes are `.panel-left` and `.panel-right`). So
 * its paint:0 is vacuous rather than reassuring. Nothing here claims ink escapes those
 * panels; the claim is that this dataset cannot answer that question, and a floor read
 * off it is a floor on the geometric near-miss, never an audit of loss.
 *
 * WHAT IT IS NOT is "below 3 nothing is lost", and an earlier version of this comment
 * said exactly that — "probeContentClipped answers cut:false on every slide in this
 * band" — which the sweep it cites refutes. Three slides in (0, 2] lose author body
 * content, two of them at 0.02 and 0.03px of section excess: a `regulatory-update`
 * diff band drops "Added", and `list.gallery.md` p9 loses a stress line. That is the
 * whole reason `probeContentClipped` exists — a formatter-truncated line crosses a box
 * edge without moving the section's scroll height at all, so SECTION EXCESS IS
 * UNCORRELATED WITH LOSS at the low end and no floor on it can be a completeness claim.
 *
 * Measured consequence, stated rather than implied: of the 27 slides the truthful probe
 * calls a silent cut, this advisory names 18. Nine stay silent, eight of them body
 * content, and they are listed by name in the decision note. Lowering the floor would
 * not reach them either — six sit at excess 0.
 *
 * The advisory is therefore a floor on the geometric near-miss, not an audit of loss.
 *
 * It costs 34 slides across 21 of 335 decks — an advisory on 6.3% of the corpus,
 * which is a signal rather than a wall of text.
 */
const NEAR_MISS_FLOOR = 3;

/**
 * The advisory's TEXT, so a test can read the real thing.
 *
 * It lives here rather than inline in `lattice-emulator.js` for one reason, and it is
 * not tidiness: THREE tools harvest page numbers out of the export's buffer with
 * regexes, and two of them cross newlines and one ignores case —
 *
 ...
 *
 * — so a reword of this line using the word "overflow" or writing `page 3` instead of
 * `p3` would be harvested as if it were real clipping, and an export carrying an
 * advisory and NO other warning is a buffer shape where "first match wins" protects
 * nobody. `test/unit/export/near-miss-advisory.test.js` guards that.
 *
 * ITS FIRST CUT GUARDED THE FORMAT STRING, WHICH DOES NOT WORK, and a HARD RULE #25
 * checker mutation-proved it: the page marker sits inside a nested template literal
 * (`${quiet.map((o) => `p${o.slide}…`)}`), so substituting every `${…}` for a digit
 * deletes the word beside it too, and `page ${o.slide}` rendered as "page 3" while the
 * guarded string never contained "page" followed by a digit. A function returning the
 * finished lines is the fix: the test renders the real text from real data and matches
 * the harvesters' own regexes against it.
 *
 * It reads FRAME_TOLERANCE directly rather than taking it as a parameter: a first cut
 * had one, no caller ever passed it, and a defaulted parameter nothing exercises is a
 * documented seam that does not exist.
 *
 * @param {{slide:number, px:number}[]} quiet  slides in the band nothing else reports
 * @returns {string[]} the lines to print, in order
 */
function formatNearMissAdvisory(quiet) {
  const many = quiet.length > 1;
  const list = quiet.map((o) => `p${o.slide} (${o.px}px)`).join(', ');
  return [
    `  \u24d8 INSIDE THE FIT TOLERANCE \u2014 ${quiet.length} slide${many ? 's' : ''} ${many ? 'paint' : 'paints'} `
      + `past a box that crops by less than the ${FRAME_TOLERANCE}px budget every check above is read `
      + `against, so nothing reports ${many ? 'them' : 'it'}: ${list}.`,
...
// The Fix-Me signal lost its `.lattice-fixme-*` overlay layer when it became an
// `outline` on the culprit plus a `.fixme-tab` berth (base.modifiers.css "THE
// FIX-ME SIGNAL"). `.fixme-tab` is named here for the same reason its two
// siblings are — the marker's own chrome must never be the evidence that content
// was cut, or the marker justifies itself: draw a tab, measure the tab, keep the
// tab. The OUTLINE needs no entry: it is drawn outside the box model, so it
// contributes no rect and moves no edge either probe reads.
// DERIVED from the berth list, not re-typed beside it. This name has to stay in
// lockstep with what `fit-berth.js` actually emits, and the cost of it drifting is
// that the marker's own chrome becomes evidence that content was cut — draw a tab,
// measure the tab, keep the tab. A fourth register added to BERTHS is covered here
// automatically instead of waiting for someone to remember this line.
// Qualified on `[data-lattice-berth]`: only what fit-berth EMITTED is chrome. An
// author's own `<div class="overflow-tab">` is content and must stay measurable —
// otherwise typing that class into a slide is a way to hide content from both
// probes, which is the author-reachable kill switch the two-list split above
// exists to prevent.
// THE RAIL IS CHROME TOO, and it is not in `BERTHS`. The two capsule segments are
// wrapped in `.marker-rail`, which is the positioned box — so a probe that excluded only
// the segments would walk the rail itself, measure an engine-drawn marker as author
// content, and let the marker manufacture the overflow it is reporting. That is the exact
// failure this module's header names, one element up from where it was fixed last time.
const MARKER_CHROME_SELECTOR = [...BERTHS, RAIL_CLASS].map((c) => `.${c}[${BERTH_ATTR}]`).join(', ');
// The split pointer's LABEL is chrome whose truncation is the design, so its ellipsis must not be
// reported as content loss. The pill names the page the run goes to next; when a `comparison`
// signal runs long ("Option 1 of 6 · comparing …") the label ellipsizes BY RULE — a pill that
// wraps has stopped being a pill. Two things make this an exemption rather than a silencing.
// First, the string is ENGINE-GENERATED: `relationship.js` joins it from the member's own badges,
// so the diagnostic's advice — "Shorten the copy or give that box more room" — names an edit the
// author cannot make. Second, the probe firing here trains authors to ignore the one signal the
// export sign-off reads, which is the failure HARD RULE #29 names in as many words: a gate that
// cries wolf is one somebody switches off. Found by the #25 red-team pass, on a plain six-option
// `verdict-grid` — no shipped deck reaches it, but an author's deck does.
//
// THE SPLIT CAPTION IS DELIBERATELY *NOT* HERE, and the line between them is who can act. The
// caption is the AUTHOR'S `_footer:` words; when it exceeds the band the reader loses text that
// appears nowhere else, and "shorten the copy" is exactly the right advice. The owner ruled the
// ellipsis itself acceptable (2026-09-03) — that ruling is about what the page DRAWS, not about
// whether the exporter should mention it. Silencing that one would hide a real content loss
// behind a design decision.
const IGNORED_CLIP_SELECTOR = `div.watermark, div.tile-watermark, div.split-feat-wm, div.split-feat-bleed, div.image-scrim, span.katex-mathml, span.lat-split-label, ${MARKER_CHROME_SELECTOR}`;
 ...
 *   grow-to-fit grid card, it never pushed a neighbor — so this is a safe
...
function probeSectionOverflow(s, clipSelector, TOL, ignoreSelector) {
  ...
  // Does `el` TRUNCATE ITS OWN LINE — `text-overflow: ellipsis` on a non-wrapping,
  // clipping box? Such a box is not failing to hold its content; ellipsizing is the
  // DESIGNED answer to a line that runs long, and the box applies it itself. Its
  // children's HORIZONTAL spill is therefore the mechanism working, not evidence that
  // the SLIDE overflows — so it raises `clipSuspect` (there really is hidden text, and
  // `probeContentClipped` classifies it, `chromeOnly` and all) but never `over`.
  //
  // WHY THIS IS NEEDED AT ALL, measured. The standard frame's footer cell is exactly
  // this box. A plain text footer is invisible to discovery — no element children, so
  // `flowedSpill` has nothing to measure. Put ONE inline element in it (a `<code>` from
  // a backticked word, a `<strong>`, an `<em>`) and the element's rect lands past the
  // footer's right edge, `flowedSpill` reads it as geometric spill, and `over` comes
  // back true for a slide whose body fits perfectly. On split-panel.gallery.md at
  // portrait that fired on 28 of 71 pages — `_footer:` values generated from the
  // manifest's variant summaries, which carry `` `proof` `` in backticks (#2138).
  //
  // WHAT `over` ACTUALLY DRIVES, corrected. The first version of this note said "it drives
  // AUTOSPLIT, so those pages were candidates to be cut into a run because a word in the footer
  // was in backticks." That has not been true since 2026-09-01: the split is STRUCTURAL and
  // consults no measurement (`lattice-emulator.js` says so at the `SPLIT_VERDICT_SRC` import;
  // `over` fed `resplitDoc` until then). No slide was ever cut for a backticked footer word, and
  // a reviewer should not accept a weakened probe for a benefit that does not exist.
  //
  // The three consumers it does have are enough on their own:
  //   · the author's overflow RING and the export's "⚠ OVERFLOW … CLIPPED" line — 24 false
  //     alarms on one gallery is the cry-wolf case this exists to end;
  //   · `frameOver()` (lattice-emulator.js), the `guards: strict` trim revert — a cut whose only
  //     residual spill is a footer's designed ellipsis is now KEPT rather than thrown away;
  //   · `tools/check-overflow-corpus.js`'s ratchet, whose floor these pages were inflating.
  // (Found by the HARD RULE #25 inversion, which read the emulator rather than the commit.)
  //
  // WHY NOT `IGNORED_CLIP_SELECTOR`. That list is the escape hatch for a DECORATIVE
  // clip — a watermark, a scrim, a bleed — and the footer's clip is not decorative: it
  // really is hiding text a reader would want. Listing the footer there switches off the
  // `clipSuspect` → `probeContentClipped` path that reports the ellipsis correctly
  // today. This exemption is narrower in what it removes (one axis of one contribution)
  // and wider in what it covers (any box that truncates its own line, not just a footer).
  //
  // VERTICAL SPILL STILL COUNTS. `text-overflow` says nothing about height: a nowrap
  // ellipsis box whose children spill BELOW it is overflowing in an axis it does not
  // handle, and that is a real cut.
  function truncatesOwnLine(el) {
    if (typeof getComputedStyle !== 'function') return false;
    const st = getComputedStyle(el);
    if (!st.textOverflow || st.textOverflow === 'clip') return false;
    // `text-overflow` is INERT without both of these — a wrapping box never reaches its
    // inline end, and a visible-overflow box paints past the edge instead of ellipsizing.
    // Testing the declaration alone would exempt a box that is really spilling.
    if (!st.overflowX || st.overflowX === 'visible') return false;
    const ws = st.whiteSpace;
    if (ws !== 'nowrap' && ws !== 'pre') return false;
    // …AND THE BOX MUST ACTUALLY BE ABLE TO ELLIPSIZE. `text-overflow` acts on a block
    // container's own inline formatting context, so it does nothing on a FLEX or GRID container —
    // this repo's own CSS says so twice (`lib/forms/cell/stage/stage.css`,
    // `lib/base/base.modifiers.css`). Measured in Chrome 131 across five identically-sized boxes:
    // a block container with a text node ellipsizes; flex and grid containers hard-clip
    // mid-glyph, losing text with no ellipsis at all. Exempting those from `over` would trade a
    // false positive for a silent content loss, which is the worse direction.
    //
    // No corpus instance today — every one of the 29 exempted boxes across five galleries at
    // portrait is a `display: block` footer with a genuine ellipsis — so this is a pin, not a
    // fix. Found by the HARD RULE #25 red team.
    const d = st.display;
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    for (let k = 0; k < kids.length; k++) {
      ...
      if (!r) continue;
      // …AND ONLY AGAINST A SIBLING THIS CHILD COULD ACTUALLY COVER: one that OVERLAPS IT
      // HORIZONTALLY and STARTS BELOW IT. Both halves, and it keeps looking until it finds
      // one — a sibling that fails either test is skipped, not treated as the answer.
      //
      // "The next flowed sibling's top" assumes the cell STACKS its children in one
      // column. A cell that lays them out in COLUMNS — `display: grid` with two tracks,
      // `flex-direction: row` — puts the next sibling BESIDE this one, and its top is then
      // not a line this child can cover by growing. Subtracting it reports the child's own
      // height as overflow. Multi-column clip cells are not exotic: `citation-card`
      // (split / margin / triptych), `inventory` (editorial), `math` (feature, matrix),
      // `redline` (split / three-col), `split-panel` and `team-profile` (sides) all build
      // one, so this was reachable long before math arrived. What it took to surface was a
      // child that reports PHANTOM inner overflow.
      //
      // TWO MEASURED CASES, one phantom and one real, and the real one is the better
      // argument:
      //
      //   · `team-profile sides` at hd — a genuinely squeezed 9-person roster `<ul>` at
      //     y=219.7..616.0 x=64..640 hiding 261px, whose next DOM sibling is the RIGHT
      //     column's `<h3>` at y=177.8 x=640..1216, 396px above its bottom. Measured
      //     against that, `squeezed` came out 699.2 — 2.7x the truth — and `scrollH` feeds
      //     autosplit, so the slide was a candidate to be split on a number that was
      //     mostly fiction. It now reports 261, exactly the hidden amount.
      //   · `math matrix` on the Form frame — stage 1152x438 holding a 672x206 equation at
      //     y=294..500 x=64..736 and a 448x319 legend at y=237..556 x=768..1216, side by
      //     side, both comfortably inside the cell, nothing clipped. The equation's `<p>`
      //     reports 2px of inner overflow because KaTeX builds display math from
      //     negative-offset `vlist` struts, so every box around it reports phantom hidden
      //     pixels (the `.katex-display` span reports 10; an inner `<span>` reports 280 at
      //     clientHeight 0). Against the legend's top that 2px became dy=265, and the
      //     author saw a "Content clipped" badge on a slide with nothing clipped.
      //
      // WHY BOTH AXES, AND WHY KEEP SCANNING — each half was wrong alone, and an
      // independent checker measured both:
      //
      //   · Vertical test only, breaking on the first sibling: a child that overprints a
      //     LATER same-column sibling is silently missed, because the loop stops at the
      //     one beside it. Reproduced on a 2x2 auto-placed grid — 280px of overprint
      //     reported as 0. That is the under-report direction the note above calls "worse".
      //   · Vertical test only, but scanning past a rejected sibling: the scan then reaches
      //     a DIFFERENT COLUMN's box that merely happens to start lower, and takes its top.
      //     Measured on `citation-card margin`: a column-1 paragraph at x=64..294 would
      //     have been limited by a column-3 `<ul>` at x=934..1216, 640px to its right. The
      //     `break` was accidentally shielding that.
      //
      // Horizontal overlap is what "could cover" actually means, so it is the test.
      //
      // TOLERANCES ARE IN LAYOUT PX, SCALED BY `K`. `r` and `nr` are visual px while
      // `hidden` and `TOL` are layout px — the same distinction line ~510 already handles.
      // A bare `- 1` here is `1/K` layout px, which makes the verdict depend on the host's
      // scale: the same slide answered `over: true` at K=1 and `over: false` at K=1.5 on a
      // successor 0.8 layout px above the crushed child's bottom. That contradicts the
      // scale-independence this suite asserts elsewhere. In real column flow adjacent boxes
      // meet exactly, so the tolerance only ever absorbs sub-pixel noise.
      let limit = cRect.bottom;
      const EPS = K;
      for (let n = k + 1; n < kids.length; n++) {
        const nr = flowedRect(kids[n]);
        if (!nr) continue;
        const sharesColumn = nr.left < r.right - EPS && nr.right > r.left + EPS;
        if (sharesColumn && nr.top >= r.bottom - EPS) { limit = nr.top; break; }
      }
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    // …and the horizontal half only when the box is NOT already truncating its own line.
    // See `truncatesOwnLine`: an ellipsizing nowrap box is handling this axis by design,
    // so its children's overhang is the mechanism, not a slide that overflows (#2138).
    if (spill.overH > 0 && !truncatesOwnLine(el)) scrollW = Math.max(scrollW, clientW + spill.overH);
  }
 ...
 * @returns {{under:boolean, minPx:number, floorPx:number, minPt:number, floorPt:number, pct:number, count:number, unmeasured:number}|null}
 ...
 *   `minPt`/`floorPt` are the same two sizes in POINTS on a standard 960x540pt slide page —
 *   preset-invariant (the floor is 5.4pt everywhere) and directly comparable to the deck's own
 *   type roles, which lib/typography/scale.js documents in those points. See REF_PT below.
 */
function probeFigureLegibility(s, floorRatio) {
  const figs = s.querySelectorAll ? s.querySelectorAll('svg[viewBox]') : [];
  // A viewBox is not the only way a figure becomes container-responsive. A box carrying
  // `data-fit-k` has been letterboxed by a CSS `scale()` — same bargain, same blind spot:
  // its box never overflows because it shrank instead, so `probeSectionOverflow` cannot
  // see it. The FIRST version of state-chart's inline fit had exactly this hole and it is
  // why this arm exists: that variant emits NO `<svg>` at all, so `figs` was empty, this
  // probe returned null ("nothing to judge", which reads downstream as "legible"), and a
  // 24-state machine shipped at 3.42px against a 7.2px floor with no channel saying a
  // word — trading a REPORTED shear for an unreported shrink. Measured, not reasoned.
  // (HARD RULE #25 inversion.)
  //
  // Only boxes with no viewBox figure inside: where both are present the SVG arm already
  // accounts for the CSS scale, because `getBoundingClientRect` is post-transform, and
  // counting it here too would double it.
  // Guarded at every hop: this function is `.toString()`-injected into browser strings AND
  // exercised against the fake DOMs the unit suite builds, which implement only the handful
  // of methods each test needs. An unguarded `querySelector` here threw on a fixture that
  // predates this arm.
  const scaled = [];
  if (typeof s.querySelectorAll === 'function') {
    const cand = s.querySelectorAll('[data-fit-k]') || [];
    for (let i = 0; i < cand.length; i++) {
      const el = cand[i];
      if (!el || typeof el.querySelectorAll !== 'function' || typeof el.getAttribute !== 'function') continue;
      if (!el.querySelectorAll('svg[viewBox]').length) scaled.push(el);
    }
  }
  if ((!figs.length && !scaled.length) || !(floorRatio > 0)) return null;
  if (typeof getComputedStyle !== 'function') return null;
  const slideH = s.clientHeight || 0;
  if (!slideH) return null;
  const floorPx = slideH * floorRatio;
  // POINTS, for the number a human is shown. `floorPx` is the honest measure and it is
  // also unreadable as a report: it moves with the preset (7.2px at `hd`, 10.8px at
  // `4K`), so an author who read one number could not carry it to another deck.
  //
  // 540 is the height of the standard 16:9 slide page in points -- 960x540pt, what
  // PowerPoint and Keynote use and what this engine's own PDF page measures at `hd`
  // (lattice-emulator.js `PT = 0.75`, applied to a 720px slide). Dividing by the slide's
  // own height first and scaling by that reference makes the pt PRESET-INVARIANT, which
  // is the property FIGURE_TEXT_FLOOR_RATIO was chosen for in the first place: the floor
  // is 5.4pt on every preset, where in px it is four different numbers for one rule.
  //
  // TWO CAVEATS, both measured, because an earlier draft of this comment oversold the
  // second one and a reviewer had to catch it.
  //
  // 1. IT IS NOT THE DECK'S OWN PRINTED POINT SIZE off 16:9. `PT = 0.75` scales BOTH page
  //    axes, so a `mobile` deck (1080x2340) prints a 1755pt-tall page and a glyph this
  //    function reports as 5.3pt really measures 17.4pt on that sheet. That is the trade an
  //    invariant floor makes, and it is the same trade the ratio itself makes -- what an
  //    author needs is the glyph's size RELATIVE to the frame, because a deck is displayed
  //    scaled-to-fit and nobody prints a 40-inch page. Worth knowing before quoting this
  //    number as a physical length.
  //
  // 2. THE COMPARISON TO `--fs-*` HOLDS NEAR 16:9 AND NOT ELSEWHERE, because the two are
  //    relative to DIFFERENT AXES. lib/typography/scale.js sizes every role in `cqi`, which
  //    is one hundredth of the slide's WIDTH; this is a fraction of its HEIGHT. They agree
  //    only where the aspect does. Expressing the `meta` role in this function's own pt:
  //
  //      hd 11.23 · 4K 11.23 · square 11.07 · portrait 12.01   <- the documented ~11.25 holds
  //      standard 8.42 · story 8.44 · mobile 6.93              <- 25-38% below it
  //
  //    So "4.9pt against a documented meta of 11.25pt" is a fair reading on a 16:9 deck and
  //    a misleading one on `mobile`, where that deck's own meta sits at 6.93 in these units.
  //    The floor stays height-relative regardless: making it width-relative to match the type
  //    scale would make `floorPt` move with the aspect ratio, which is the one property this
  //    whole conversion exists to buy. (Re-derive: `SCALES[category].meta * width/100`, then
  //    `/ height * 540`.)
  //
  // Declared INSIDE the function, not at module scope: this function is
  // `.toString()`-injected verbatim into the emulator's inline watcher, so it can close
  // over nothing (the same reason `floorRatio` arrives as a parameter).
  const REF_PT = 540;
  ...
  // The scaled-HTML arm. `font-size` from computed style is already LAYOUT px (unlike the
  // SVG arm's rect-derived scale, which needs the K normalization above), so the glyph size
  // on the page is simply `font-size x k`. Leaves only — an ancestor's font-size is
  // inherited by its children, so measuring both would report the same glyph twice and a
  // container's larger size would mask a small child.
  for (let g = 0; g < scaled.length; g++) {
    const box = scaled[g];
    const k = parseFloat(box.getAttribute('data-fit-k'));
    if (!(k > 0)) continue;
  ...
  if (!count) {
    return unmeasured
      ? { under: false, count: 0, unmeasured, floorPx: Math.round(floorPx * 10) / 10, floorPt: Math.round(floorRatio * REF_PT * 100) / 100 }
      : null;
  }
  // Report minPx rounded DOWN, so a flagged figure never prints as EQUAL to the floor it missed
  // ("8px vs an 8px floor" reads like a bug in the check). `minPt` rounds down for the same
  // reason, and independently rather than by converting the already-rounded px.
  //
  // THE pt PAIR IS NON-CONTRADICTORY BECAUSE OF THE CONSTANT, NOT BECAUSE OF THE ROUNDING,
  // and whoever changes the constant needs to know that. `minPt` FLOORS at 1dp while
  // `floorPt` ROUNDS at 2dp, so the two cannot be reasoned about as a pair in general. With
  // FIGURE_TEXT_FLOOR_RATIO = 0.01 the floor lands on exactly 5.4, and a brute-force sweep of
  // slide heights 100..3000 against minPx up to the last ulp below the floor finds no case of
  // `under === true` with `minPt >= floorPt`. Move the ratio into (0.01, 0.0100093) --
  // floorRatio * 540 in (5.400, 5.405) -- and `floorPt` rounds DOWN to 5.4 while a `minPt`
  // just under the true floor also floors to 5.4: the tab would read "Text too small · 5.4pt"
  // beside a hint saying "below the 5.4pt minimum". If the ratio moves, re-run that sweep or
  // make both sides round the same way. (The px pair has the same latent shape and always
  // has: slideH 713 prints minPx 7.1 against floorPx 7.1.)
  return {
    under: minPx < floorPx,
    minPx: Math.floor(minPx * 10) / 10,
    floorPx: Math.round(floorPx * 10) / 10,
    minPt: Math.floor((minPx / slideH) * REF_PT * 10) / 10,
    floorPt: Math.round(floorRatio * REF_PT * 100) / 100,
...
function probeContentClipped(s, ignoreSelector, TOL, bearerSelector) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    while (anc) {
      ...
      // THE RUNNING FOOTER IS DETECTED LIKE ANYTHING ELSE, AND LABELED. This one line
