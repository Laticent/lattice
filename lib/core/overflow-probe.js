...
//
// THE RUNNING-FOOTER BAND IS NOT IN THIS LIST, and that is a correction. It was, for
// one commit, because reporting the footer's accepted ellipsis made the new channel
// shout about a dated decision on most of its first hits. But `IGNORED_CLIP_SELECTOR`
// is matched with `closest()`, so naming the band silenced its whole SUBTREE — and
// `2026-07-27-footer-band-allocation.md` prices exactly one thing: the HORIZONTAL
// `text-overflow: ellipsis` tail of the footer's own TEXT. The band also carries
// `height: 1.6em; overflow: hidden` (a VERTICAL clip) and `footer:` passes raw HTML,
// so the blanket exemption also swallowed a sliced confidentiality mark and an 80px
// footer logo — losses nobody has ever priced, silent at every marker level, and
// reported correctly before this branch touched it. The accepted case is now excluded
// where it is actually accepted: inline-axis, text only, in `probeContentClipped`.
// (HARD RULE #25 red team.)
const IGNORED_CLIP_SELECTOR = 'div.watermark, .tile-watermark, .split-feat-wm, .split-feat-bleed, .katex-mathml, .image-scrim, .overflow-tab, .illegible-tab, .lattice-fixme-tab, .lattice-fixme-box, .lattice-fixme-overlay';
...
function probeSectionOverflow(s, clipSelector, TOL, ignoreSelector) {
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    if (ignoreSelector && typeof el.closest === 'function' && el.closest(ignoreSelector)) continue;
 ...
 * @param {string} ignoreSelector  IGNORED_CLIP_SELECTOR — clip boxes and content that
 *   are never evidence of loss (decorative bleeds, invisible a11y mirrors, our own chrome)
 * @param {number} TOL       same layout-px tolerance the geometry probe used
 * @returns {{cut:boolean, first:string|null}}  `first` is a short label for the
 *   first cut node, for the console line — never a DOM ref (it must cross the
 *   page.evaluate boundary).
 */
function probeContentClipped(s, ignoreSelector, TOL) {
  if (!s || typeof s.querySelectorAll !== 'function') return { cut: false, first: null };
  ...
  function skipped(el) {
    ...
    if (typeof el.closest === 'function' && ignoreSelector && el.closest(ignoreSelector)) return true;
    return false;
  }
  ...
  function clipBox(el) {
    ...
    if (el.namespaceURI !== 'http://www.w3.org/2000/svg'
      && !(ignoreSelector && typeof el.closest === 'function' && el.closest(ignoreSelector))
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    while (anc) {
      ...
      else if (!escaped) escaped = atSection ? '' : escapeKind(anc);
      ...
      // THE ONE TRUNCATION THIS REPO HAS ALREADY PRICED, and only in the shape it
      // priced it: the running footer's own TEXT running off the end of its band.
      // `2026-07-27-footer-band-allocation.md` §"What it costs, accepted deliberately"
      // measures that tail and takes it. It does NOT cover the band's vertical clip
      // (`height: 1.6em; overflow: hidden`) or the images `footer:` can carry, so a
      // sliced confidentiality mark and a lost logo still report. Keyed on the axis and
      // the bearer kind rather than on the selector, which is why this lives here and
      // not in IGNORED_CLIP_SELECTOR — `closest()` there took the whole subtree.
      // Matched on the EXACT STRUCTURAL SHAPE the decision doc describes, never on a bare
      // class. `html: true` means an author can type `<div class="cell-footer">`, and for
      // one commit that was a kill switch for the whole register — wrap a slide in it and
      // the ring, the pill, the console line and autosplit all went quiet. `matches()` on
      // the emitted `section.form > .cell-footer > footer` cannot be reached that way, and
      // scopes the allowance to the ONE box the doc priced.
      // BREAK, not continue: the allowance has to end the climb for this bearer. Layout
      // is UNCLIPPED, so a rect the footer already cut still reads as overflowing every
      // box above it — skipping one box just handed the same loss to the section, which
      // reported it anyway. (Measured on examples/portrait-journey.md p3: footer clips at
      // x=992, the text lays out to x=1115, the section edge is 1080.) Once the innermost
      // clip box has accounted for a bearer, an outer box is re-reporting the same
      // already-clipped glyphs, not finding a second loss.
      if (hCut && !vCut && items[i].kind === 'text'
        && typeof box.matches === 'function'
        && box.matches('section.form > .cell-footer > footer')) break;
      if (vCut || hCut) {
        ...
        return { cut: true, first: t };
      }
    }
  }
  return { cut: false, first: null };
}
