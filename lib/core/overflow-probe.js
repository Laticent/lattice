 ...
 * a single slide shape with no ink outside any box, and above 3 the measured ink
 * tracks the number (excess 4 -> 4.02px of a `<strong>` outside its box, 6 -> 5.58,
 * 11 -> 10.64). So 3 is where THIS MEASURE stops being noise.
 *
 ...
 * @param {number} tolerance                   the budget they came in under
 * @returns {string[]} the lines to print, in order
 */
function formatNearMissAdvisory(quiet, tolerance = FRAME_TOLERANCE) {
  ...
  return [
    `  \u24d8 INSIDE THE FIT TOLERANCE \u2014 ${quiet.length} slide${many ? 's' : ''} ${many ? 'paint' : 'paints'} `
      + `past a box that crops by less than the ${tolerance}px budget every check above is read `
