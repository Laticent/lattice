 ...
 * fine.
 *
 * WHY 12, AND WHY THAT NUMBER IS NOW OVERSTATED. The original justification was "the
 * smallest real bug observed in the gallery was a 211px overshoot", so anything under
 * 211 was free. That stopped being true long ago: #2252 measured a gantt painting
 * 10.2px outside `.cell-stage`, which is `overflow: clip`, with the export silent.
 *
 * SO WHY IS IT STILL 12? Because it is not only a report threshold — it feeds
 * `buildSplitVerdict`, and therefore AUTOSPLIT. Lowering it re-decides which slides
 * split corpus-wide, moving committed PDFs and page-count assertions, and it banks 34
 * unfixed slides in the `overflow:check` ratchet. That is its own change with its own
 * remediation.
 *
 * The measured case for lowering it, the full distribution over all 335 shipped decks,
 * and what it would cost are in
 * `engineering/decisions/2026-09-21-frame-tolerance-silent-window.md`. What this file
 * does instead is STOP THE WINDOW BEING SILENT: `NEAR_MISS_FLOOR` below, and the
 * emulator line that reads it.
 */
 ...
 *
 * Taken from the corpus distribution, not from taste: below 3 the band is dominated by
 * one slide shape the content probe answers `cut: false` on, and above it the measured
 * ink tracks the number. It costs an advisory on 34 slides across 21 of 335 decks.
 *
 * IT IS A FLOOR ON THE GEOMETRIC NEAR-MISS, NEVER AN AUDIT OF LOSS, and the printed
 * text has to say so — this is the one thing to keep if the rest of this comment is
 * ever trimmed. Section excess is UNCORRELATED with loss at the low end: a
 * formatter-truncated line crosses a box edge without moving scroll height at all, so
 * slides at 0.02px of excess do lose body content while slides at 8px lose nothing but
 * empty space. Of the 27 silent cuts the truthful probe finds corpus-wide this names
 * 18; nine sit below the floor and lowering it would not reach six of them, which sit
 * at excess 0. All of it, with the slides named, is in the decision note.
 */
...
//
// SCREEN-READER-ONLY TEXT clips by definition — `.chart-sr-only` is the 1px clip that keeps a
// chart's hidden data table and its spoken-only words off the page while leaving them in the
// accessibility tree. Its content is never painted, so its clip is never a loss; measured, a
// heatmap's hidden table (`cartesian.buildSrDataTable`) made every heatmap slide report
// "OVERFLOW … CLIPPED" and would have fed autosplit. Scoped to the chart frame, the only place
// the class takes effect, as `span.katex-mathml` is KaTeX's own accessibility copy.
const IGNORED_CLIP_SELECTOR = `div.watermark, div.tile-watermark, div.split-feat-wm, div.split-feat-bleed, div.image-scrim, span.katex-mathml, span.lat-split-label, .chart-frame .chart-sr-only, ${MARKER_CHROME_SELECTOR}`;
...
function probeFigureLegibility(s, floorRatio) {
  ...
  for (let f = 0; f < figs.length; f++) {
    const fig = figs[f];
    // A figure that renders NO box has no on-page glyph size to judge — skip it rather than
    // measure it. The case is not hypothetical: Anima's in-place chart motion hides the static
    // poster SVG (`display: none`) and draws a live clone beside it (docs/src/lib/anima/hydrate.ts
    // § mount). The poster's rect is 0x0, so the scale guard below left `scale = 1` and its
    // 7-unit heatmap labels were read as 7px — "Text too small · 5.2pt" on a slide whose live
    // chart set them at ~17px. The clone is a second `svg[viewBox]` and is measured on its own.
    // `getClientRects` is guarded because the unit suite's fake DOMs do not all implement it.
    if (typeof fig.getClientRects === 'function' && fig.getClientRects().length === 0) continue;
  ...
  for (let g = 0; g < scaled.length; g++) {
    const box = scaled[g];
    // Same skip as the SVG arm: a box that renders nothing has no on-page glyph size, and
    // computed `font-size` resolves even under `display: none`, so without this a hidden
    // scaled box would still be judged.
