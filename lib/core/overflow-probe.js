...
//
// CALIBRATED by rendering every shipped chart gallery through the emulator — the surface this gate
// actually runs on, so the numbers are the ones a deck really gets. Two galleries have figures
// below 8px and five have none: `radar` (page 7, the `small-multiples` variant) at 6.8px, and
// `state-chart` at 5.3 / 5.8 / 7.8 / 7.9 / 7.9px across five slides. `quadrant`, `funnel`,
// `piechart`, `word-cloud` and `map` are all clear.
//
// 8px is set just above those two ON PURPOSE: they are the rule's own subject. A figure whose
// labels render at 5.3px is exactly the "ships silently at 6px type" this rule was written about,
// and the point is to stop that being invisible. Everything the catalog renders at a legible size
// passes untouched.
//
// (Measure it the same way if you re-calibrate: through `node lattice-emulator.js`, not by loading
// the exported HTML in a bare browser. The figure's box — and therefore its scaled text — depends
// on the viewport, so a bare page load reports different px for the same slide.)
