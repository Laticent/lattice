...
//     parent, so a 955px Range rect qualifies as a cut bearer: 23 such boxes
//     across the corpus, every one of them a phantom (#1300 item 4). Generalized
//     to `[aria-hidden="true"]`, which is the standards-level statement of the
//     same fact and also covers the image scrim.
...
const IGNORED_CLIP_SELECTOR = 'div.watermark, .tile-watermark, .split-feat-wm, .split-feat-bleed, .katex-mathml, [aria-hidden="true"], .overflow-tab, .illegible-tab, .lattice-fixme-tab, .lattice-fixme-box, .lattice-fixme-overlay';
...
function probeSectionOverflow(s, clipSelector, TOL, ignoreSelector) {
  ...
  //
  // Cost, measured on the 117-slide gallery (the largest deck in the repo, and the
  // filmstrip renders all of it at once): 7.4ms for the whole document, ~0.06ms per
  // section, against 4.7ms for the probe as it stood. Childless elements are skipped
  // because `flowedSpill` reads element children and a box with none can never
  // contribute — that alone is half the cost (4706 elements down to 1761).
  const seen = [];
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    if (!spill) continue;
    // `clipSuspect` is deliberately looser than `over` — see the return note.
    ...
    if (spill.overV > TOL || spill.overH > TOL || raw > TOL) clipSuspect = true;
  ...
  // can decide whether running `probeContentClipped` (a full text-node walk, ~0.2ms
  // a section) is worth it. That inversion is what lets the two probes cover each
 ...
 * Live instance the corpus still carries: a docked `footer` at `text-overflow:
 * ellipsis` cuts 43px of real title with ZERO geometric spill — no measure the
 * geometry probe owns can see it.
 *
 * ONE tree walk, not one per box. The obvious shape — for each box, walk its text —
 * re-walks shared subtrees once per nesting level, and discovery multiplied the boxes
 * by ~4. Bearers are collected once for the whole section and tested against the boxes
 * that `contains()` them, which is the same set of (bearer, clipping-ancestor) pairs at
...
function probeContentClipped(s, ignoreSelector, TOL) {
  ...
  // and labelled as such: it can only ever be as good as the box the pseudo lives in.
  ...
  function bearers(box) {
      ...
      while (n) {
        ...
        if (v?.trim() && !skipped(n.parentElement)) {
          ...
          for (let i = 0; i < rects.length; i++) {
            if (rects[i].height > 0 || rects[i].width > 0) out.push({ n: n.parentElement, r: rects[i], t: v.trim() });
          }
        }
        n = walker.nextNode();
      }
    ...
    if (typeof box.querySelectorAll === 'function') {
      ...
      for (let i = 0; i < reps.length; i++) {
        ...
        const r = el.getBoundingClientRect();
        if (r.width === 0 && r.height === 0) continue;
        out.push({ n: el, r, t: (el.tagName || 'element').toLowerCase() });
      }
  ...
  // Every clipping box in play: the section itself, plus every DISCOVERED clipping
  // descendant. Not an allowlist -- see the header. The exceptions are named instead
  ...
  const boxes = [];
  if (clips(s)) boxes.push(s);
  const maybeBoxes = s.querySelectorAll('*');
  for (let i = 0; i < maybeBoxes.length; i++) {
    const el = maybeBoxes[i];
    if (el.namespaceURI === 'http://www.w3.org/2000/svg') continue;
    if (ignoreSelector && typeof el.closest === 'function' && el.closest(ignoreSelector)) continue;
    if (clips(el)) boxes.push(el);
  }

  // ONE bearer walk for the whole section, then each bearer against the boxes that
  // CONTAIN it -- same (bearer, clipping-ancestor) pairs as a per-box walk, at a
  // quarter of the tree traversal now that discovery multiplied the boxes.
  ...
  for (let b = 0; b < boxes.length; b++) {
    ...
    for (let i = 0; i < items.length; i++) {
      const r = items[i].r;
      if (box !== s && typeof box.contains === 'function' && !box.contains(items[i].n)) continue;
      ...
      if (r.bottom > br.bottom + tol || r.top < br.top - tol
        || r.right > br.right + tol || r.left < br.left - tol) {
