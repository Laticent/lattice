...
//   · A TRUNCATION THAT WAS ALREADY DECIDED — the running-footer band.
//     `section.form > .cell-footer > footer` is `height: 1.6em; overflow: hidden;
//     text-overflow: ellipsis`, and its tail really is absent from the exported text
//     layer. That is not news: `engineering/decisions/2026-07-27-footer-band-allocation.md`
//     §"What it costs, accepted deliberately" measures the loss, names it, and takes it,
//     eight days before this probe learned to see it. Reporting it now would make the
//     new channel shout about a dated decision on three shipped decks — the corpus's
//     single largest source of CONTENT CLIPPED hits, and every one of them a cost the
//     repo already priced. "A warning that cries wolf teaches its reader to silence it"
//     is the governing sentence of the register this probe serves; the first thing the
//     widened probe must not do is prove it. Excluded at the BAND, not at `footer`
//     generally, so a footer outside the Form band still reports.
const IGNORED_CLIP_SELECTOR = 'div.watermark, .tile-watermark, .split-feat-wm, .split-feat-bleed, .katex-mathml, .image-scrim, .cell-footer, .overflow-tab, .illegible-tab, .lattice-fixme-tab, .lattice-fixme-box, .lattice-fixme-overlay';
...
function probeContentClipped(s, ignoreSelector, TOL) {
  ...
  //
  // The rule is CSS's own: an absolutely-positioned element is clipped by an ancestor
  // only if that ancestor is its containing block, i.e. is itself positioned. So the
  // climb does not stop at an out-of-flow ancestor — it RAISES THE BAR, considering
  // only clip boxes from there up that establish a containing block. (HARD RULE #25
  // red team; the docstring had claimed this class was closed when only the ignore
  // clause had gained the ancestor walk.)
  ...
  for (let i = 0; i < items.length; i++) {
    const r = items[i].r;
    let anc = items[i].n;
    let escaped = false;   // passed through an out-of-flow ancestor (see outOfFlow)
    while (anc) {
      const br = clipBox(anc);
      const atSection = anc === s;
      const skipThis = !br || (escaped && !positioned(anc));
      if (!atSection && outOfFlow(anc)) escaped = true;
