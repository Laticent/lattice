 ...
 * sections that probe has already flagged, and answers the narrower question. Keeping
 * them separate is what lets `author` stay exactly as loud as it was.
 *
 ...
 *
 * @param {Element} s        the section already known to overflow geometrically
 * @param {string} clipSelector  CLIP_CELL_SELECTOR
 ...
 */
function probeContentClipped(s, clipSelector, TOL) {
  if (!s || typeof s.querySelectorAll !== 'function') return { cut: false, first: null };
  const tol = typeof TOL === 'number' ? TOL : 0;
  ...
  // Content-bearing LEAVES only. A wrapper's rect is the union of its children, so
  // counting wrappers would report a cut whenever any descendant was cut — and also
  // whenever only its padding was. Text is read off leaves; replaced elements are
  // leaves by nature.
  function bearers(root) {
    const out = [];
    const all = root.querySelectorAll('*');
    for (let i = 0; i < all.length; i++) {
      const el = all[i];
      const replaced = typeof el.matches === 'function' && el.matches(REPLACED);
      if (!replaced) {
        if (el.children?.length) continue;                    // not a leaf
        const t = (el.textContent || '').trim();
        if (!t) continue;                                     // no readable content
      } else if (el.closest && el.closest('svg') !== el && el.closest('svg')) {
        continue;                                             // inner svg nodes ride on the root <svg>
      }
      if (typeof getComputedStyle === 'function') {
        const st = getComputedStyle(el);
        if (st.position === 'absolute' || st.position === 'fixed') continue; // placement, not flow
        if (st.display === 'none' || st.visibility === 'hidden') continue;
        // The overflow marker's own tab is chrome we just added; it must never be the
        // evidence that content was cut.
        if (el.classList && (el.classList.contains('overflow-tab') || el.classList.contains('illegible-tab'))) continue;
      }
      const r = el.getBoundingClientRect();
      if (r.width === 0 && r.height === 0) continue;
      out.push({ el, r });
  ...
  function label(el) {
    ...
    // Concatenation, never a template literal: this function's SOURCE is interpolated
    // into a template literal in lattice-emulator.js, so a backtick or a dollar-brace
    // ANYWHERE in here (comments included) terminates that literal and breaks the whole
    // injected watcher. The unit test asserts neither appears.
    return t ? tag + ': ' + t : tag;
  }

  // Every clipping box in play: the section itself, plus the bounded content cells.
  // A decorative clip box is not in clipSelector by design (doc §4c), so an
  // intentional bleed still cannot manufacture a cut.
  const boxes = [];
  if (clips(s)) boxes.push(s);
  const cells = s.querySelectorAll(clipSelector);
  for (let i = 0; i < cells.length; i++) if (clips(cells[i])) boxes.push(cells[i]);

  for (let b = 0; b < boxes.length; b++) {
    const box = boxes[b];
    const br = box.getBoundingClientRect();
    // Rect space throughout — both sides come from getBoundingClientRect, so no
    // layout-vs-visual px conversion is needed and a scaled slide is handled for free.
    const items = bearers(box);
    for (let i = 0; i < items.length; i++) {
      const r = items[i].r;
      if (r.bottom > br.bottom + tol || r.right > br.right + tol || r.top < br.top - tol) {
        return { cut: true, first: label(items[i].el) };
      }
    }
  }
