...
function probeContentClipped(s, clipSelector, TOL) {
  ...
  function skipped(el) {
    ...
    if (typeof el.closest === 'function' && el.closest('.overflow-tab, .illegible-tab')) return true;
    return false;
  }
  ...
  function bearers(box) {
      ...
      while (n) {
        ...
        if (v?.trim() && !skipped(n.parentElement)) {
          ...
          for (let i = 0; i < rects.length; i++) {
            if (rects[i].height > 0 || rects[i].width > 0) out.push({ r: rects[i], t: v.trim() });
          }
        }
        n = walker.nextNode();
      }
    ...
    if (typeof box.querySelectorAll === 'function') {
      ...
      for (let i = 0; i < reps.length; i++) {
        ...
        const r = el.getBoundingClientRect();
        if (r.width === 0 && r.height === 0) continue;
        out.push({ r, t: (el.tagName || 'element').toLowerCase() });
      }
    }
    return out;
  }
  ...
  for (let b = 0; b < boxes.length; b++) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (r.bottom > br.bottom + tol || r.top < br.top - tol
        ...
        // Concatenation, never a template literal: this function's SOURCE is
        // interpolated into a template literal in lattice-emulator.js, so a backtick
        // or a dollar-brace ANYWHERE in here (comments included) terminates that
        // literal and breaks the whole injected watcher. A unit test asserts neither
        // appears -- it has already caught this twice.
