  ...
  lines.forEach((raw, n) => {
    ...
    if (html) {
      if (!blank && indent < html.col) html = null;
        ...
        else if (html.end && html.end.test(raw)) html = null;
        return;
      }
    }
    if (blank) {
    ...
    if (itemOk) {
      ...
      if (restFence && !(restFence[1][0] === '`' && restFence[2].includes('`'))) {
        ...
        para = false;
        if (!(h.end && h.end.test(rest.trim().replace(h.open, '')))) html = { end: h.end, col };
    ...
    if (htmlType >= 0) {
      ...
      if (!(h.end && h.end.test(line.replace(h.open, '')))) html = { end: h.end, col: base };
      return;
    }
    para = opensPara(line);
  });
...
function paneVocab(ms) {
  ...
  for (const m of ms) {
    if (!m.pane) continue;
    paneSpec[m.name] = { fit: m.pane.fit, ...(m.pane.stack === false ? { stack: false } : {}) };
    if (m.pane.budget) {
      const { axis, min, side, stack } = m.pane.budget;
      paneBudget[m.name] = { axis, ...(min ? { min } : {}), side: { ...side }, ...(stack ? { stack: { ...stack } } : {}) };
    }
  }
...
module.exports = {
  ...
  parseLayout, specOf, paneFitWarning, paneBudgetLimit, splitPaneMarkdown, paneVocab,
};
