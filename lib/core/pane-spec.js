 ...
 * included). Line-level, for the linter's instant feedback; the carve works on the parsed
 * tokens instead. So this follows the block rules that decide whether markdown-it sees a
 * marker as a TOP-LEVEL html block at all — and test/unit/core/pane-contract.test.js renders
 * each edge case through the engine to prove they agree:
 *   - a fence closes only on the same character, at least as long, with no info string
 *     (a ```` block holding a ``` block is one fence);
 *   - at most three spaces of indent (four is an indented code block);
 *   - not inside an HTML block (a `<div>` run with no blank line yet);
 *   - not inside a list item's content (indented to the item's content column).
...
function splitPaneMarkdown(slide) {
  ...
  lines.forEach((raw, n) => {
    ...
    const open = indent < 4 && raw.match(/^ {0,3}(`{3,}|~{3,})(.*)$/);
    if (open && !(open[1][0] === '`' && open[2].includes('`'))) {
      fence = { ch: open[1][0], len: open[1].length };
      return;
    }
    // A list item opens a content column; a later line indented to it is the item's content.
    const item = raw.match(/^( {0,3})([-*+]|\d{1,9}[.)])( +)\S/);
    if (item) {
      listCol = item[1].length + item[2].length + Math.min(item[3].length, 4);
      return;
    }
    if (listCol >= 0 && indent >= listCol) return;
    if (indent < listCol) listCol = -1;
    if (inHtml) return;
    const m = indent < 4 && line.match(PANE_RE);
    const l = indent < 4 && line.match(PANES_RE);
    if (m) markers.push({ cls: m[1], line: n });
    else if (l) spec = l[1].trim();
    // Any other HTML block (not a whole comment) runs to the next blank line.
    else if (indent < 4 && /^<(?:[A-Za-z/]|![A-Z]|\?)/.test(line) && !/^<!--[\s\S]*-->$/.test(line)) inHtml = true;
  });
