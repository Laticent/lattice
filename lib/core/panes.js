...
function parseLayout(spec) {
  ...
  for (const word of words) {
    ...
    } else out.error = `panes: unknown word "${word}" — use a ratio like 40/60, optionally with "stack"`;
  }
  return out;
}
...
function carveSlide(tokens, from, to, lines, section, nonce, env) {
  ...
  placeholder.content =
    `<div class="lat-panes" data-panes="${layout.direction}" data-pane-slot="${slot}"` +
...
function embed(html, carved) {
  ...
  return html.replace(
    /<div class="lat-panes" data-panes="(\w+)" data-pane-slot="([\w-]+)" style="([^"]*)"><\/div>/g,
