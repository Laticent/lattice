...
function carveSlide(tokens, from, to, lines, section, nonce, env) {
  ...
  // Coda blocks that STAYED in pane B — the ones it claims, and any the peel could not reach
  // past a claimed one — take their height there, so the pane's chart canvas is smaller by
  // them (lib/engine/index.js `renderPane`).
  const kept = [];
  for (let n = last.length - 1; n > 0; n--) {
    const b = last[n];
    if (isComment(tokens, b) || isEmpty(tokens, b)) continue;
    if (isBlockquote(tokens, b)) kept.push('insight');
    else if (isNoteParagraph(tokens, b)) kept.push('note');
    else break;
  }
  const claimed = { insight: kept.includes('insight'), note: kept.includes('note') };
  ...
  const chrome = {
    eyebrow: mast.some((b, n) => isCodeOnlyParagraph(tokens, b) && (titleAt < 0 || n < titleAt)),
    subtitle: titleAt >= 0 && mast.some((b, n) => n > titleAt && isCodeOnlyParagraph(tokens, b)),
    insight: coda.some((b) => isBlockquote(tokens, b)),
    note: coda.some((b) => isNoteParagraph(tokens, b)),
  };
  ...
  env.latticePanes.push({
    ...
    panes: markers.slice(0, 2).map((m, index) => ({ index, cls: cls[index], authored: m.cls, markdown: source(pane[index]), claimed: index === 1 ? claimed : null })),
  });
