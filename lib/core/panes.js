...
function paneForm(cls) {
  return Object.hasOwn(PANE_FORM, cls) ? PANE_FORM[cls] : cls;
}

const PANE_RE = /^<!--\s*pane:\s*([a-z][\w-]*)\s*-->$/;
// No quantifier here may overlap its neighbor: `\s*([^<>]*?)\s*` backtracked cubically on
// an unclosed comment (3 KB of spaces took 3.8s in a render). The capture is trimmed instead.
...
module.exports = {
  installPanes, embed, parseLayout, paneParts, paneForm,
