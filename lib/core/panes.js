...
/**
 * On a deck whose family is not `wide` (square, portrait, story, mobile), turn each panes slide
 * into one ordinary slide per pane. It runs before the engine forms slides, so every rule
 * after it sees plain slides: the first page takes the masthead and the first pane, each later
...
function installPaneSplit(md, { family } = {}) {
  ...
  md.core.ruler.before('lattice_slide', 'lattice_panes_split', (state) => {
    ...
    for (const group of groupOnSlideRule(state.tokens)) {
      ...
      const lead = group.slice(0, at[0]).filter((t) => !isLayout(t));
      at.forEach((start, k) => {
        pushHr();
        ...
        for (const t of lead) {
          ...
          if (k > 0 && isComment && !SPOT_DIRECTIVE_RE.test(t.content.trim())) continue;
          const copy = cloneToken(state, t);
          const spot = copy.type === 'html_block' && copy.content.trim().match(SPOT_CLASS_RE);
          if (spot) {
            copy.content = `<!-- _class: ${cls} ${spot[1]} -->\n`;
            classed = true;
          }
          out.push(copy);
        }
        if (!classed) {
          ...
          d.content = `<!-- _class: ${cls} -->\n`;
          d.block = true;
          out.push(d);
        }
        const end = k + 1 < at.length ? at[k + 1] : group.length;
        for (const t of group.slice(start + 1, end)) if (!isLayout(t)) out.push(t);
      });
    }
    state.tokens = out;
  });
...
module.exports = {
  installPanes, installPaneSplit, embed, parseLayout, paneParts, paneForm, paneFit,
