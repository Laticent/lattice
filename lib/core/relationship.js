...
// KaTeX's CLASSED failure rendering — a ParseError. Anchored to the span's own opening tag so a
// `.katex` span merely CONTAINING the string cannot trip it.
const ERROR_SPAN = /^<span[^>]*\sclass="[^"]*(?<![\w-])katex-error(?![\w-])/;
...
const stripMathMirror = (html, sink) => {
  ...
  for (let from = 0; from < s.length;) {
    ...
    const errored = ERROR_SPAN.test(span) || (mathml ? /mathcolor="#cc0000"/.test(span) : false);
