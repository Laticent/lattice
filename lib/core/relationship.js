 ...
 *     a hand-written six, which is the same failure mode one rung down: a fourth checker swept all
 *     292 `defineMacro` names and found `∹` missing, so `- $a \eqcolon b$ — the description` still
...
/**
 * A `.katex` span ANYWHERE — a SCAN, because every regex written for this job has been quadratic.
 *
 ...
 * at its `>`, and hand that one bounded string to the same `isKatexSpanTag` the tag walk uses.
 *
 * AND ADVANCE PAST THE `>`, WHICH IS WHAT MAKES IT ONE PASS. The first cut advanced by 5 and was
 * quadratic on `'<span x='.repeat(n) + '>'`: every candidate found the SAME far-away `>` and sliced
 * the whole window to test it — 4x per doubling, 422ms at 16,000 repeats. A tag cannot contain a
 * `<`, so when the window holds one, the only tag that can start inside it starts at the LAST `<`
 * before the `>`; test that, then resume after the `>`. Each character is looked at a bounded
 * number of times, so the walk is linear in the input rather than in the input squared. Measured on
 * that same shape: 422ms → 0.1ms. (HARD RULE #25 checker, fourth pass, which also measured that the
 * commit claiming this file's remaining regexes were "linear at 40,000 characters" had measured the
 * wrong shape — one long tag, not many.)
 *
 * Returns the INDEX of the span's `<`, or -1. `>` inside an attribute value would end the tag
 * early — the same limit every `[^>]*` form here has always had, and not reachable from KaTeX's
 * own output, which is what this reads.
 */
function findSpanByClass(html, token, from = 0) {
  ...
  for (let at = s.indexOf('<span', from); at >= 0; ) {
    ...
    if (close < 0) return -1;
    const inner = s.lastIndexOf('<', close);
    const start = inner > at ? inner : at;
    if (spanTagHasClass(s.slice(start, close + 1), token)) return start;
