 ...
 * rather than in the input squared: 422ms → 0.003ms on that shape.
 *
...
// `[\s>]` so `<spanfoo>` is not a span. Defense in depth: deleting it changes nothing observable,
// because `findMatchingClose` will not pair `<spanfoo` with a `</span>` and the caller breaks out.
// Measured by a HARD RULE #25 checker, and stated here because no test can pin it.
