...
const stripMathMirror = (html) => {
  ...
  for (let from = 0; from < s.length;) {
    ...
    const chars = mathml ? stripTags(mathml[1].replace(ANNOTATION, ''), '').trim() : '';
...
function labelOf(memberOuter) {
  ...
  const lead = textOf(dropLeadEquation(html.replace(TAG_OPEN, '').split(/<(?:ul|ol)\b/)[0]));
