...
const stripMathMirror = (html) => {
  ...
  //
  // The annotation stays as the FALLBACK, and the whole thing stays optional: the emulator's PDF
  // path can render KaTeX with `output: 'html'`, which emits no MathML and so neither source —
  // replacing the span with an empty string there would delete every inline symbol from a pointer
  // instead of merely setting it badly, so the span is left alone and the mirror-strip below plus
  ...
  for (let from = 0; from < s.length;) {
    ...
    const chars = mathml ? mathml[1].replace(ANNOTATION, '').replace(/<[^>]*>/g, '').trim() : '';
...
function dropLeadEquation(html) {
  const h = String(html);
  const isKatex = (at) => /^<span[^>]*\sclass="[^"]*(?<![\w-])katex(?![\w-])/.test(h.slice(at));
  ...
  for (;;) {
    if (isKatex(at)) break;
    const tag = /^\s*<[a-zA-Z][\w-]*[^>]*>\s*/.exec(h.slice(at));
    if (!tag) return h;
    at += tag[0].length;
  }
  ...
  const rest = h.slice(0, at) + h.slice(end);
...
function labelOf(memberOuter) {
  ...
  const lead = textOf(dropLeadEquation(html.replace(/^<[a-zA-Z][\w-]*[^>]*>/, '').split(/<(?:ul|ol)\b/)[0]));
