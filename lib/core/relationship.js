 ...
 *
 * Five readers in this file were `<h[3-6][^>]*>`-shaped, and every one of them put ATTRIBUTE TEXT
 * on a rendered chip: `<h3 title="a>b">Ridge regression</h3>` labeled a page `b">Ridge regression`.
 * The tag is bounded once, here, and the caller gets the index its CONTENT starts at.
 *
...
function firstElement(html, names, from = 0) {
  ...
  const end = findMatchingClose(html, name, at);
  if (end < 0) return null;
  return {
    tag: html.slice(at, contentAt),
    inner: html.slice(contentAt, end - name.length - 3),
    at,
    end,
  };
}
...
const xTexOf = (html) => {
  const ann = firstElement(html, ['annotation']);
  return ann && X_TEX.test(ann.tag) ? ann : null;
};
/**
