 ...
 *
 * The mirror is removed rather than the visual half kept, because the visual half is what a
 * reader sees and it is the only one of the three that is not a duplicate. Depth-aware: the
 * mirror contains nested `<span>`s, so a lazy `[\s\S]*?</span>` stops at the first inner close
 * and leaves the rest of the MathML behind.
 */
const stripMathMirror = (html) => {
  if (!html.includes('katex')) return html;
  let s = html;
  // PREFER THE AUTHOR'S OWN TeX. Removing the a11y mirror stopped `$X$` reading "X X X", but what
  // was left is KaTeX's VISUAL half — one `<span>` per glyph box, plus the zero-width space it
  // pads with — so a tag strip turned `$y_i$` into "y i ␀" and `$X^\top X$` into "X ⊤ X". Those
  // shipped in the forward pointer of a legend page.
  //
  // The MathML mirror carries `<annotation encoding="application/x-tex">` — the source the author
  // typed — so the whole `.katex` span is replaced by that instead. `y_i` in a wayfinding line is
  // both readable and honest; per-glyph boxes are neither. (Found by the HARD RULE #25 checker,
  // on the shipped demo PDF.)
  // ONLY where the source is actually recoverable. The emulator's PDF path renders KaTeX with
  // `output: 'html'`, which emits no MathML and so no annotation at all — replacing the span with
  // an empty string there would delete every inline symbol from a pointer instead of merely
  // setting it badly. With no annotation the span is left alone and the mirror-strip below plus
  // the caller's tag walk handle it exactly as they did.
  ...
  for (let from = 0; from < s.length;) {
    ...
    if (end < 0) break;
    const tex = ANNOTATION.exec(s.slice(at, end));
    if (!tex) { from = at + 1; continue; }
    const head = `${s.slice(0, at)} ${tex[1]} `;
    s = head + s.slice(end);
    from = head.length;
  }
...
const textOf = (html) => {
  let s = stripMathMirror(String(html));
