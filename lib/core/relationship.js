...
function dropLeadEquation(html) {
  ...
  for (;;) {
    ...
    // limit step</td>` labelled a page `b"> f(x)=y the limit step`, raw attribute text plus the
...
function safeName(text) {
  ...
  const head = textOf(dropped).split(CLAUSE_BREAK)[0].replace(/[.:;,]+$/, '').trim();
  if (!head || head.length > LABEL_MAX) return '';
  const safe = typeSafe(mathSafe(head, dropped), dropped);
  return safe ? trimTail(safe) : '';
}

function labelOf(memberOuter) {
  return typeSafe(labelOfIn(memberOuter), memberOuter);
}
 ...
 *
 * It escapes `<` and `>` ONLY, and only when they are bare. Escaping more would be wrong: the
