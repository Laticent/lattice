...
const stripMathMirror = (html, sink) => {
  ...
  for (let from = 0; from < s.length;) {
    ...
    const mathml = firstElement(span, ['math']);
    ...
    // The CLASSED failure first: a ParseError span has no MathML at all, so a test that reaches
    // for the mirror cannot see it.
    const errored = isErrorSpan(span) || (mathml ? /mathcolor="#cc0000"/.test(span) : false);
    const ann = mathml ? xTexOf(mathml.inner) : null;
    const bare = ann ? mathml.inner.slice(0, ann.at) + mathml.inner.slice(ann.end) : mathml?.inner;
    const chars = mathml && !errored
      ? stripTags(bare, '').replace(INVISIBLE, '').trim() : '';
    ...
    const tex = chars ? null : xTexOf(span);
    const label = chars || (tex ? tex.inner : null);
...
/**
 * The value of a tag's `class` attribute — a quote-aware WALK over the attributes, not a regex.
 *
 * `/\sclass="([^"]*)"/` was here, and it is DOUBLE-QUOTE-ONLY in both directions. Chromium reads
 * `class='katex-error'` and `class=katex-error` as the class `katex-error`; this file read neither,
 * so `isErrorSpan` returned false and the author's raw `\frac{a` flattened into the pointer pill —
 * character-for-character the blocker the commit before this one closed, one quoting style over.
 * And it matched INSIDE another attribute's value: `<span title=' class="katex-error"'>` has no
 * class at all to Chromium, but was classified as the error rendering here, so its visible words
 * were dropped from the label instead.
 *
 * Both rendered, on a real deck, before the fix. Reachable from `html: true`, which is the same
 * reachability this file has three times accepted as reason enough.
 *
 * The walk follows HTML's own rule — a quote delimits a value only directly after `=`, which is
 * what `tagEnd` already encodes — and returns the FIRST `class`, because that is the one Chromium
 * keeps when a tag carries two. (HARD RULE #25 checker, twelfth pass, with the render.)
 */
function classOfTag(tag) {
  const end = tag.length - 1;
  let i = 1;
  while (i < end && !WS.test(tag[i])) i += 1;
  while (i < end) {
    while (i < end && (WS.test(tag[i]) || tag[i] === '/')) i += 1;
    const from = i;
    while (i < end && !WS.test(tag[i]) && tag[i] !== '=') i += 1;
    const name = tag.slice(from, i).toLowerCase();
    while (i < end && WS.test(tag[i])) i += 1;
    if (tag[i] !== '=') {
    ...
    let value = '';
    if (tag[i] === '"' || tag[i] === "'") {
      const close = tag.indexOf(tag[i], i + 1);
      value = tag.slice(i + 1, close < 0 ? end : close);
...
/**
 * Drop a leading separator — whitespace and tags, then one of `— – : ; ,` and the space after it —
 * replacing the lot with a single space. `''` in, `''` out; no separator, no change.
 *
 * A regex anchored at `^` over `(?:\s | <[^>]*> )*` then a separator was here, and its `<[^>]*>`
 * is the same tag bound as everywhere else
 * in this file: `$X^\top X$ <span title="a>b"></span> — the Gram matrix` kept its em dash, so the
 * chip shipped `— the Gram matrix of the design` — character-for-character the defect the comment
 * three lines above records as fixed for `L \dashv R`. Rendered before the fix.
 * (HARD RULE #25 checker, twelfth pass.)
 */
function dropLeadSeparator(s) {
  let i = 0;
  for (;;) {
    if (i < s.length && WS.test(s[i])) { i += 1; continue; }
    if (s[i] === '<') {
  ...
  // `i < s.length` FIRST. `'—–:;,'.includes(s[i] ?? '')` was here, and `String.includes('')` is
  // TRUE, so a walk that reached end-of-string read the absence of a character as a separator and
  // returned a bare space: `dropLeadSeparator('')` gave `' '`. No caller could observe it — the
  // `hasVisibleText` guard above rejects every input that gets there — so it was a false docblock
  // and a trap for the next caller rather than a live defect. (HARD RULE #25 checker, 13th pass.)
  if (i >= s.length || !'—–:;,'.includes(s[i])) return s;
  i += 1;
  while (i < s.length && WS.test(s[i])) i += 1;
  return ` ${s.slice(i)}`;
}
/**
 * The inner HTML of a `<strong>` reached through up to three leading open tags, or `null`.
 *
 * `^(?:<[a-zA-Z][^\\s>]*(?:\\s[^>]*)?>\\s*){1,3}<strong>([\\s\\S]*?)</strong>` was here. Its `[^>]*`
 * bounds a tag at the first `>`, so `<p title="a>b"><strong>Ridge.</strong>` did not match at all.
 *
 * A first reading called that harmless — the label fell through to a later reader and came back
 * right. It is not. When the run does not match, THE FIGURE RULE NEVER RUNS: the two branches
 * below are what stop a bare numeral naming a page, and skipping them let
 * `<p title="a>b"><strong>31</strong> deals closed this quarter` label its page
 * `31 deals closed this quarter` — the whole run WITH the numeral, which is precisely what
 * `isFigure` exists to refuse. Measured against both implementations before the fix, because the
 * arm written for the harmless shape SURVIVED the mutation and the harmless shape was all it had.
 * (HARD RULE #25 checker, twelfth pass.)
 */
function leadingStrong(html) {
  let at = 0;
  // `{1,3}` — at LEAST one leading tag, which is the `^(?:TAG\s*){1,3}` the regex asked for. A
  // first cut let the run be empty, so an `html` that STARTS with `<strong>` newly matched. No
  // caller passes one (a member arrives wrapped in its own `<li>`/`<td>`), which is exactly why a
  // widening like that ships unnoticed; this is a bound fix, not a behavior change.
  for (let taken = 0; taken < 3; taken += 1) {
    if (html[at] !== '<' || !/[a-zA-Z]/.test(html[at + 1] || '')) return null;
    const close = tagEnd(html, at);
    if (close < 0) return null;
    at = close + 1;
    while (at < html.length && WS.test(html[at])) at += 1;
    if (html.startsWith('<strong>', at)) {
      const end = html.indexOf('</strong>', at);
      return end < 0 ? null : html.slice(at + '<strong>'.length, end);
    }
  }
  return null;
}
/**
 * The first `<name …>` open tag from `at` whose name is in `names` — bounded by `tagEnd`, so a `>`
 * inside a quoted attribute value does not end it.
 *
 * Eight readers in this file were `<h[3-6][^>]*>`-shaped, and four of them put ATTRIBUTE TEXT on a
 * rendered chip: `<h3 title="a>b">Ridge regression</h3>` labeled a page `b">Ridge regression`. The
 * tag is bounded once, here, and the caller gets the index its CONTENT starts at.
 *
 * The name test is EXACT and case-folded, which is the half `findMatchingClose` does not do — see
 * `closeOf` below.
 *
 * Returns `[tagStart, contentStart]`, or `[-1, -1]`.
 */
function firstOpenTag(html, names, from = 0) {
  for (let at = html.indexOf('<', from); at >= 0; at = html.indexOf('<', at + 1)) {
    const close = tagEnd(html, at);
    if (close < 0) return [-1, -1];
    let n = at + 1;
    while (n <= close && !WS.test(html[n]) && html[n] !== '>' && html[n] !== '/') n += 1;
    if (names.includes(html.slice(at + 1, n).toLowerCase())) return [at, close + 1];
    at = close;
  }
  return [-1, -1];
}
/**
 * The index of the FIRST `</name>` at or after `from`, matching the name EXACTLY and case-folded.
 * `-1` when there is none.
 *
 * NOT `findMatchingClose`, and the two reasons are the two defects a first cut of this shipped.
 *
 * IT PREFIX-MATCHES. `findMatchingClose` opens with `'<' + tagName` and tests by prefix, so `<li`
 * counts `<line>`, `<link>` and `<listing>` as nested list items — and `<annotation>` counts
 * `<annotation-xml>`, which is a real MathML element, not a hypothetical. One `<svg><line/></svg>`
 * inside a metric name made the reader run past its own `</li>` and swallow the NEXT item, so a
 * chip read `Keep whole Second item entirely`.
 *
 * IT IS DEPTH-AWARE, and every regex it replaced was LAZY — `([\s\S]*?)</li>`, to the first close.
 * On a well-formed nested list those differ, and the difference is not academic: a `stats` member
 * whose first sub-bullet carries its own sub-list AND further text handed the whole item to the
 * label, blew the 42-character budget and DECLINED, so the pointer stopped naming the next page.
 * That is straight authored markdown with no raw HTML in it — the one shape the commit that
 * introduced it claimed could not be affected. Depth-aware may well be the better read; changing
 * to it silently, in a commit whose subject is tag bounds, is not the way to find out.
 * (HARD RULE #25 checker, thirteenth pass, with the rendered chip.)
 */
function closeOf(html, name, from) {
  const want = `</${name.toLowerCase()}`;
  for (let at = html.indexOf('</', from); at >= 0; at = html.indexOf('</', at + 1)) {
    if (!html.slice(at, at + want.length).toLowerCase().startsWith(want)) continue;
    const after = html[at + want.length];
    if (after === '>' || after === undefined || WS.test(after)) {
      const close = tagEnd(html, at);
...
/**
 * The first element from `from` whose tag name is in `names`, as `{ tag, inner, at, end }` —
 * `at` is its `<`, `end` the index just past its close tag. `null` when there is none, which is
 * the same "nothing to read" signal the regexes it replaces gave through a null match.
 */
function firstElement(html, names, from = 0) {
  const [at, contentAt] = firstOpenTag(html, names, from);
  if (at < 0) return null;
  const name = html.slice(at + 1, contentAt).match(/^[a-zA-Z][^\s/>]*/)?.[0] ?? '';
  const end = closeOf(html, name, contentAt);
  if (end < 0) return null;
  return {
    tag: html.slice(at, contentAt),
    inner: html.slice(contentAt, html.lastIndexOf('</', end)),
    at,
    end,
  };
}
const innerOf = (html, names, from = 0) => firstElement(html, names, from)?.inner ?? '';
// KaTeX's own annotation, in either quoting style. The tag is bounded before this reads it, so
// this is a test on ONE tag rather than a scan that has to skip arbitrary text.
const X_TEX = /encoding=(["'])application\/x-tex\1/;
// …and it SEARCHES, rather than testing the first annotation and giving up. The regex it replaced
// scanned FOR the x-tex one; a first cut of this took `firstElement(html, ['annotation'])` and
// tested that, so a non-x-tex annotation sitting before it (`application/mathml`, which is legal)
// left the author's `\sigma` unstripped and riding into the text — raw TeX on a chip, the defect
// this file exists to close, reintroduced by the commit closing another one.
// (HARD RULE #25 checker, thirteenth pass.)
const xTexOf = (html) => {
  for (let from = 0; ; ) {
    const ann = firstElement(html, ['annotation'], from);
    if (!ann) return null;
    if (X_TEX.test(ann.tag)) return ann;
    from = ann.end;
  }
};
/**
...
// BOTH of KaTeX's renderings, not just the successful one. A ParseError renders as
// `<span class="katex-error" title="ParseError: …" style="color:#cc0000">\frac{a</span>` — no
// `.katex` wrapper and no MathML anywhere — so a scan for `katex` alone walked straight past it,
// `stripMathMirror` never touched it, and `stripTags` flattened the author's raw TeX into the
// chip: a rendered slide read `\frac{a →`. `mathSafe`'s early exit keys on the same scan, so the
// HARD RULE #29 shape-glyph guard was switched off for those members too.
//
// `lib/engine/math.js` ALREADY LEARNED THIS — its docblock says "KATEX HAS TWO FAILURE RENDERINGS
// AND ONLY ONE OF THEM CARRIES A CLASS" and it tests both. This file learned half of it, twice:
// first by reading the error mirror as symbols, then by refusing that mirror while leaving the
// classed rendering invisible. (HARD RULE #25 checker, on the fold that fixed the other half.)
const findKatexSpan = (html, from = 0) => {
  const ok = findSpanByClass(html, 'katex', from);
  const err = findSpanByClass(html, 'katex-error', from);
  if (ok < 0) return err;
  return err < 0 ? ok : Math.min(ok, err);
};
...
const spanTagHasClass = (tag, token) => /^<span[\s>]/.test(tag)
  && classOfTag(tag).split(/\s+/).includes(token);

/**
 * Is this span KaTeX's CLASSED failure rendering — a ParseError?
 *
 * `tagEnd` + `spanTagHasClass`, NOT a regex. The first cut was
 * `/^<span[^>]*\sclass="[^"]*…katex-error…/`, and `[^>]*` before a quoted attribute is the exact
 * shape this file spent three commits removing: it bounds the tag at the first `>`, which
 * Chromium does not. A `katex-error` span carrying `title="a>b"` BEFORE its `class` was found by
 * the scan and then not classified by the test, so `errored` came back false and the author's raw
 * `\frac{a` flattened into the pointer pill — the very defect the commit that added it claims to
 * close. Not reachable from KaTeX (it escapes `>` in `title`); reachable from `html: true`, which
 * is the same reachability this file has twice accepted as reason enough to fix.
 *
 * Two tag-bounding implementations that disagree is a defect this file has now had three times.
 * There is one bound, and it is `tagEnd`. (HARD RULE #25 checker, on the fold that introduced it.)
 */
const isErrorSpan = (span) => {
  if (span[0] !== '<') return false;
  const close = tagEnd(span, 0);
  return close >= 0 && spanTagHasClass(span.slice(0, close + 1), 'katex-error');
};
const isKatexSpanTag = (tag) => spanTagHasClass(tag, 'katex') || spanTagHasClass(tag, 'katex-error');
function dropLeadEquation(html) {
  ...
  // AN UNREADABLE LEADING SPAN DROPS TOO, and it has to drop through the branch BELOW so its
  // separator goes with it. `decoded` is '' for a failed render (nothing readable came out), and
  // returning here left the em dash leading: the chip shipped `— a description of it`, which is
  // character-for-character the defect this function's own comment records as fixed for
  // `L \dashv R`. (HARD RULE #25 checker, on the commit that introduced it.)
  if (decoded !== '' && !RELATION.test(decoded) && !UNTYPEABLE.test(decoded)) return h;
  ...
  const rest = h.slice(0, at) + dropLeadSeparator(h.slice(end));
...
function labelOfIn(memberOuter) {
  ...
  // TRANSFORM can wrap a member's whole text in `<strong>`, and `list-criteria` (retired
  // 2026-09-25; any slot-lift layout still can) did exactly that. So the named path clipped full sentences: the shipped `examples/split-structure.pdf`
  ...
  if (slotAt >= 0) {
    ...
    if (slotEnd >= 0) {
      const from = tagEnd(html, slotAt) + 1;
      return safeName(html.slice(from, slotEnd - '</span>'.length));
    }
  }
  const strong = leadingStrong(html);
  if (strong !== null) {
    const lead = textOf(strong).trim();
    if (!isFigure(lead)) return safeName(strong);
    ...
    const list = firstOpenTag(html, ['ul', 'ol'])[1];
    const nested = list < 0 ? '' : innerOf(html, ['li'], list);
    const after = html.split(/<\/strong>/)[1] || '';
    const source = nested || after.split(/<(?:ul|ol)\b/)[0];
    ...
    return textOf(source).trim() ? '' : safeName(strong);
  }
  const head = firstElement(html, ['h3', 'h4', 'h5', 'h6']);
  if (head) return safeName(head.inner);
