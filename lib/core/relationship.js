...
const stripMathMirror = (html) => {
  ...
  // The `.katex` search is `findKatexSpan` (above) — it was `<span[^>]*\sclass="…` here, the same
 ...
 * One pass over the input, one bounded test per candidate, and one definition of "is this katex".
 *
 ...
 */
function findKatexSpan(html, from = 0) {
  const s = String(html);
  for (let at = s.indexOf('<span', from); at >= 0; at = s.indexOf('<span', at + 5)) {
    const close = s.indexOf('>', at);
    if (close < 0) return -1;
    if (isKatexSpanTag(s.slice(at, close + 1))) return at;
  }
  return -1;
}
