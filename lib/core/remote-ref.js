...
const FETCH_ATTRS = new Set([
  'src', 'srcset', 'href', 'xlink:href', 'poster', 'background', 'data', 'action',
...
function attrIsRemote(tag, name, value) {
  ...
  if (FETCH_ATTRS.has(name) && !isLink) {
    ...
    const targets = name === 'srcset' ? value.split(',').map((c) => c.trim().split(/\s+/)[0]) : [value];
...
function remoteRefsInElements(elements) {
  ...
  for (const el of elements) {
    ...
    if (/(?:^|\s)(?:language-)?mermaid(?:\s|$)/.test(get('class'))) {
      out.push(...remoteCssRefs(el.text));
      for (const m of String(el.text || '').matchAll(/(?:[a-z][a-z0-9+.-]*:)?[/\\]{2}[^\s"'<>)\]]+/gi)) if (isRemoteUrl(m[0])) out.push(m[0]);
    }
