...
function attrIsRemote(tag, name, value) {
  ...
  if (NESTED_DOC.has(tag) && (name === 'src' || name === 'data') && /^\s*data:/i.test(value) && !/^\s*data:image\/(?:png|jpe?g|gif|webp|avif)[;,]/i.test(value)) return true;
...
function mermaidDecoded(text) {
  ...
  for (let pass = 0; pass < 2; pass++) {
    t = t
      .replace(/&?#(?:x([0-9a-f]+)|(\d+));/gi, (m, hex, dec) => {
...
function remoteRefsInElements(elements) {
  ...
  for (const el of elements) {
    ...
    if (/(?:^|\s)(?:language-)?mermaid(?:\s|$)/.test(get('class'))) {
      const text = mermaidDecoded(el.text);
      out.push(...remoteCssRefs(text));
      for (const m of text.matchAll(/(?:[a-z][a-z0-9+.-]*:)?[/\\]{2}[^\s"'<>)\]]+/gi)) if (isRemoteUrl(m[0])) out.push(m[0]);
    }
...
function galleryRefusal(url) {
  return `its sample slide loads ${String(url).slice(0, 80)} from the network — a remote image or url() in gallery.md is a beacon to whoever sent it (only relative paths and data: URIs are allowed).`;
}

module.exports = { isRemoteUrl, remoteCssRefs, attrIsRemote, remoteRefsInElements, galleryRefusal, FETCH_ATTRS };
