...
function blockWebImages(html, allowed = []) {
  ...
  for (const m of html2.matchAll(/<(?:code|pre|div)\b[^>]*\sclass\s*=\s*"[^"]*mermaid[^"]*"[^>]*>([\s\S]*?)<\/(?:code|pre|div)>/gi)) {
    ...
    // Text for a URL SCAN only, never output: tags dropped, then any stray `<` too.
    for (const u of new Set(mermaidRemoteRefs(decodeEntities(m[1].replace(/<[^>]*>/g, '').replace(/</g, ''))))) if (!u.startsWith('policy:')) refuse(u, 'diagram');
  }
