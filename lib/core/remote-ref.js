...
function remoteRefsInElements(elements) {
  ...
  for (const el of elements) {
    ...
    // attribute shows (WebRTC's UDP, a DNS lookup). A sample slide may load a script by a
    // RELATIVE path with an empty body — the diagram gallery loads the vendored Mermaid that way,
    // and a package cannot supply such a file, since any script file makes it a code package —
    // but carries no inline script, no `data:`/`javascript:` source, no event handler and no
    // `javascript:` URL. An SVG `<script>` names its file in `href` or `xlink:href`, not `src`,
    // so all three are read.
    if (el.tag === 'script') {
      if (String(el.text || '').trim()) out.push('policy:an inline <script>');
      for (const k of ['src', 'href', 'xlink:href']) {
        if (/^(?:data|javascript|vbscript|blob):/.test(urlAsParsed(get(k)))) out.push('policy:a <script> with a data: or javascript: source');
      }
    }
...
module.exports = { isRemoteUrl, remoteCssRefs, attrIsRemote, remoteRefsInElements, mermaidRemoteRefs, frontMatterRemoteRefs, frontMatterOf, galleryRefusal, FETCH_ATTRS };
