...
function mermaidRemoteRefs(text) {
  ...
  // `ht#9;tp:`) is `http:host`. Read the text again with those removed; with no word boundary
  // to lean on once lines are joined, any special scheme counts.
  const joined = t.replace(/[\t\n\r]/g, '');
  if (joined !== t) {
    for (const m of joined.matchAll(/(?:https?|wss?|ftp|file):[^\s"'<>)\]]*|[/\\]{2}[^\s"'<>)\]]+/gi)) {
      if (isRemoteUrl(m[0])) out.push(m[0]);
    }
  }
  return out;
