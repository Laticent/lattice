 ...
 *   band.
 * NO `finishTheme` PORT (#1674). There used to be one: the single place a path was
 *   licensed to alter the palette the map built, and its only sanctioned use was
 *   `DIVERGENT_KEYS` — `fontFamily`, because a hyphenated font stack could not survive
 *   mermaid's directive sanitizer on the export path. That path renders in an
 *   engine-owned page now and configures Mermaid through `initialize`, so the two
 *   palettes are equal key-for-key and the hook has nothing left to do.
 *   `test/unit/core/diagram-theme-parity.test.js` asserts that equality outright, with
 *   no exception set — a port kept "just in case" is how the next divergence arrives
 *   pre-authorized.
 *
 ...
 */
function renderDiagrams(deck, { readToken, renderOne, scopeKey = String, beginRun } = {}) {
  ...
  for (let slideIndex = 0; slideIndex < slides.length; slideIndex++) {
    ...
    for (let diagramIndex = 0; diagramIndex < diagrams.length; diagramIndex++) {
      results.push(renderOne(diagrams[diagramIndex], themeVars, {
        scope: slide.scope,
        scopeKey: key,
        // The slide's node LOOK ('handDrawn' | 'classic'), resolved by the caller
        // via lib/core/diagram-look.js. Carried per SLIDE beside `scope` for the
        // same reason `scope` is: both are baked into the SVG at render time, so
        // a path that resolved one per-slide and the other per-deck would emit a
        // diagram whose shape and palette answered different questions. Undefined
        // for a caller that does not resolve a look — `engineInitConfig` then
        // emits no `look` key and Mermaid keeps its `classic` default.
        look: slide.look,
        slideIndex,
        diagramIndex,
