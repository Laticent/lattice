...
let slide = null;
// A PANE (lib/core/panes.js) renders as its own one-slide document, whose transforms call
// `setRenderSection(0)`. Pinning holds the slide scope at the HOST section instead, so a pane's
// ids are numbered as the slide they sit on — the same ids whether that slide is rendered in its
// deck or alone at its offset — and continue that slide's per-family sequence.
let pinned = null;

// The families that mint document-scoped `<defs>` ids through this module. Keep this in sync
...
const FAMILIES = /pie-wedge|radar-area|chart-spine|gantt-fill|gantt-barclip|q-tint|cart-fill/;
...
/**
 * Hold the slide scope at section `i` of the document (0-based) until `pinRenderSlide(null)`.
 * `setRenderSection` calls made while pinned are ignored.
 */
function pinRenderSlide(i) {
	if (i === null) {
		pinned = null;
		slide = null;
		return;
	}
	pinned = slideBase + i + 1;
	slide = pinned;
}

/**
 * Start a fresh id space for ONE slide transformed on its own, with the position and prefix the
 * deck render gave it: a code package runs one slide at a time in a sandbox, far from the render
 * that chose both (lib/packages/code-bundle.js). `idPrefix` is that render's `renderIdPrefix()` and
 * only a prefix this module could have minted is taken; `index` is the slide's 0-based position in
 * the deck. The transform's own section walk then enters section 0, which is slide `index + 1`, the
 * number the deck render used.
 */
function enterSlideIds(idPrefix, index) {
	resetRenderIds(undefined, index);
	prefix = typeof idPrefix === 'string' && /^lat-r(?:0|[1-9]\d*)-$/.test(idPrefix) ? idPrefix : '';
}
...
module.exports = { nextRenderSeq, resetRenderIds, enterSlideIds, renderIdPrefix, setRenderSection, pinRenderSlide };
