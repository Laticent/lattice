 ...
 * guard returns before any interpolation site is reached — so this is unreachable today.
 * It is here because an unescaped key inside a pattern is the kind of asymmetry that
 * survives a refactor: the day this family is built from anything but a literal, that guard
 * is the only thing between a key and the regex engine, and nothing would fail to say so.
 */
 ...
 * `*.md`, via `git ls-files "*.md"` — 905 carry front matter, 3 opt a render target in, and
 ...
 * found. The denominator is stated here so the next reader can re-run it.) It is contrived
...
function renderTargetKeyState(fm, key) {
  ...
  // `Object.hasOwn`, not a truthiness test: `LEGACY_ON` is a plain object, so `toString`,
  // `constructor` and `__proto__` all came back truthy and then threw on `.test`. The
  // earlier guard claimed to have fixed the throw and had fixed one third of it.
