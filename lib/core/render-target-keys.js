 ...
 * is stated rather than buried. A nested key is read by the scalar arm when it comes
 * first (`frontMatterValue` is `^[ \t]*`, and it takes the FIRST match), and the two
 * widenings make a nested line readable that the `$`-anchored legacy regex could not
 * read:
 *
 *     nest:
 ...
 *
 * The old export said OFF here; the union says ON. Plain nested shadowing
 * (`fluid: true` with no quotes or comment) already did this and is pre-existing — the
 * widenings extend it to two more spellings. Measured across the 901 committed decks
 * carrying front matter: ZERO instances, so this is contrived rather than live, and
 * narrowing the scalar arm to top-level-only would re-break the six parity rows above.
 * If it ever does bite, the fix is a linter warning on a nested render-target key, not a
...
function renderTargetKeyState(fm, key) {
  ...
  if (ON_WORDS.includes(word) || pattern.test(String(fm ?? ''))) {
