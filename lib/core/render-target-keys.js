 ...
 *
 * The four RENDER-TARGET front-matter keys — `fluid:`, `player:`, `present:`, `read:` —
 * read in ONE place (HARD RULE #1). `read:` joined in #2246; the prose below said
 * "three" for a while after it did, which is how a documented key stayed a no-op long
 * enough to ship.
 *
 * WHAT MAKES THESE A FAMILY, and why they are not registers. Every other
 * front-matter key describes the DECK: its palette, its Form, its rhythm. These
 * four describe the ARTIFACT a particular render should emit — a fluid-box
 * viewer, a self-contained player, a PDF flagged to open full-screen, a reading
 * article. They exist
 ...
 * — it decides them all at export time (ShareSheet) and offers no control.
 ...
 *
 * WHY A KERNEL FOR FOUR BOOLEANS. `lattice-emulator.js` hand-rolled the same
 * pattern three times:
 *
 ...
 * through the shared scalar rule fixes it for all of them at once.
 *
 ...
 * the vocabulary is DATA here and not one inline regex per key: a warning and a
...
const RENDER_TARGET_KEYS = Object.freeze({
  ...
  read: 'Emit the .html as the deck\'s reading article — the prose instead of the slides (--read). PDF/PPTX/PNG are unchanged.',
});
 ...
 * corrupts the deck; nothing writes these keys, and tightening the read would
