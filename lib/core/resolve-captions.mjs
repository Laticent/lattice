 ...
 *  EXPORTED so the reader-view projection can prune an inline `captions: { … }` with the same
 *  parse the resolver reads it with — a second copy of this quote state machine is exactly the
 *  duplication that let three caption spellings ship unpruned. */
export function splitFlowPairs(inner) {
  const parts = [];
  let buf = '';
  let quote = '';
