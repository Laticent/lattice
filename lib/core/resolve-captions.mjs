 ...
 *
 * The sibling `captions:` key (Layer 1 — a slide's read-as text) is a tracked follow-up
 * (#907); its parser lands with the producer that reads it, so nothing dead ships here.
...
/**
 * Parse a deck source's narration front-matter. Returns the acronym registry Map (empty
 * when the key is absent). The single entry point both producers call. NOTE: the sibling
 * `captions:` block (Layer 1 — a slide's read-as text) is a tracked follow-up (#907); its
 * parser lands with the producer that consumes it, so no dead branch ships here.
 */
export function parseNarrationFrontMatter(md) {
  return { acronyms: parseAcronyms(frontMatterBody(md)) };
}
...
export default { parseNarrationFrontMatter, acronymSpokenMap };
