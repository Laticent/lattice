...
/**
 * Parse the `symbols:` block → Map<glyph, spoken>. A flat map of a display GLYPH → its spoken
 * form; an empty value SILENCES the glyph. Author overrides beat the built-in Speech Symbol
 * Commons (cadenza/symbols.ts). The key is a single glyph, quoted or bare:
 *
 *   symbols:
 *     "→": leads to        # override the built-in "to"
 *     "≈": roughly
 *     "🎯": ""             # silence a decorative glyph the commons would otherwise keep
 *
 * Later duplicate keys win (last-wins). A missing/empty key is skipped; an empty VALUE is kept
 * (it's the deliberate "silence this glyph" signal).
 */
function parseSymbols(body) {
  const out = new Map();
  const lines = blockLines(body, 'symbols');
  if (!lines.length) return out;
  const entryIndent = Math.min(...lines.map((l) => l.indent));
  for (const line of lines) {
    if (line.indent !== entryIndent) continue; // flat map — ignore stray deeper lines
    // key: value — the key is a quoted glyph, or a bare run up to the first unquoted colon.
    const m = line.text.match(/^(?:"([^"]+)"|'([^']+)'|([^\s:]+))\s*:\s*([\s\S]*)$/u);
    if (!m) continue;
    const key = m[1] ?? m[2] ?? m[3];
    if (!key) continue;
    out.set(key, unquote(m[4])); // empty value = silence (deliberate)
  }
  return out;
}
 ...
 * front-matter captions Map (Layer 1, slide-number keyed), and the symbol-override Map — each
 * empty when its key is absent. The single entry point both producers call, so they can't drift
 * (#904).
 */
export function parseNarrationFrontMatter(md) {
  const body = frontMatterBody(md);
  return { acronyms: parseAcronyms(body), captions: parseCaptions(body), symbols: parseSymbols(body) };
}

/** The cadenza-ready symbol-override map: glyph → spoken form (author beats the built-in commons).
 *  Symmetric with `acronymSpokenMap`, so both producers thread overrides through one call. */
export function symbolOverrideMap(md) {
  return parseNarrationFrontMatter(md).symbols;
}
...
export default { parseNarrationFrontMatter, acronymSpokenMap, frontMatterCaptions, frontMatterLang };
