 ...
 * it. Beats the built-in Speech Symbol Commons (cadenza/symbols.ts). Read from the `lexicon:`
 * block; the older `symbols:` key is still honored as a deprecated alias (shipped in #949), with a
 * `lexicon:` entry winning if a deck has both:
 *
 *   lexicon:
 ...
 *
 * Later duplicate keys win (last-wins). A missing key is skipped; an empty VALUE is kept.
 */
function parseLexicon(body) {
  const out = parseTokenMap(body, 'symbols'); // deprecated alias, parsed first…
  for (const [k, v] of parseTokenMap(body, 'lexicon')) out.set(k, v); // …lexicon wins
  return out;
}
