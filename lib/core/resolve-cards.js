 ...
 *   3. the manifest: `withCoda`, then `byClass[variant]`, then `byFamily[family]`, then `default`
 *
 ...
 * does today. Opting one in is a manifest field PLUS the one declaration that reads the
 * token (`align-content: var(--cards-align)`); a unit test fails a component that declares a
 * composition its CSS never consumes, because that opt-in would otherwise be a silent no-op.
 *
 ...
 *  `byClass[variant]`, which beats `byFamily[family]`, which beats `default`. Null when it
 *  declares nothing. `byClass` is for a component whose variants are different shapes (a
 *  split-panel `proof` fills its column, the plain evidence list spreads); its keys are
 *  checked in the catalog's (sorted) order, so two matching variants resolve the same way
 *  on every path. */
function componentCardsAlign(component, { family, hasCoda, classes } = {}) {
  const entry = CARDS_CATALOG[component];
  if (!entry) return null;
  if (hasCoda && entry.withCoda) return entry.withCoda;
  if (entry.byClass && Array.isArray(classes)) {
    for (const [cls, v] of Object.entries(entry.byClass)) if (classes.includes(cls)) return v;
  }
...
function resolveCardsAlign({ classes, family, hasCoda } = {}) {
  ...
  return componentCardsAlign(component, { family: family || 'wide', hasCoda, classes });
}
