 ...
 * declaration is what lets a per-SHAPE default survive, and two shapes use it: cards-grid at
 * tall/strip, where the cards go full-width and it is a single column rather than a grid, so
 * it paces them down the frame; and any slide ending in a Key Insight coda, which keeps
 * stretching because that panel's step is measured from the stage and shrinking the cards
 * would widen it (48px -> 239px centered, 431px topped). `cards: center` therefore stamps nothing and resolves to each rule's
