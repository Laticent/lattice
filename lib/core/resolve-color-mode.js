 ...
 *
 * Tolerates a leading BOM and a padded opening fence (`--- `) — the same two
 * shapes `withPrintColorMode` below and `resolve-pace.mjs` already tolerate.
 * Author text is normalized at ingest (`SANCTIONED_EOL_BOUNDARIES`), so this
 * is belt-and-braces on the render path; what it actually buys is that the
 * linter's parse can be character-for-character identical to this one without
...
function deckPrintBand(md, flagPrint = false) {
  ...
  // Top-level, matching the kernel: a `class:` nested under another key, or sitting
  // inside a `style: |` block scalar, is not the deck register.
