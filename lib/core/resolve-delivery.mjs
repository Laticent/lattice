 ...
 * slide get a gesture), says where overlay ink and the cursor still appear, and picks the spark's
 * color, pulse, tempo and the caption's look. It never decides which moments matter: that is a fact about the content,
 ...
 *
 *   Every gestured moment SPARKS: the named bullet, row, cell, column, chart mark or line changes
 *   color in place (`sparkContent`, `lib/base/base.focus.css`). Color only, so no box moves. The
 *   fields below say how the spark and the overlay read, and they are what tells the three
 *   presets apart at a glance (§6):
 *
 ...
 *   spark    the spark's color: `'accent'` (the accent over an accent wash) or `'muted'` (the
 *            heading ink over a gray wash — no accent anywhere).
 *   pulse    one brief glow as the spark arrives. Reduced motion drops it.
 *   fade     how long the spark lingers after the narration moves on, in ms. (The color
 *            switches in one step: animating it produced garbage colors in Chromium.)
 *   hold     `'aside'` keeps the spark through a short aside ("Thank you.") that follows it;
 ...
 *   wordSpark inside a sparked text element, light the word being spoken (on the caption's
 *            clock). Off for somber: a moving highlight is exactly the motion somber refuses.
...
export const DELIVERY_PRESETS = Object.freeze({
  restrained: Object.freeze({ budget: 2, floor: 1, ink: 'none', spark: 'accent', pulse: true, fade: 320, hold: 'none', caption: 'crawl', wordSpark: true, strength: 'quiet', motion: 'legible' }),
  expressive: Object.freeze({ budget: 4, floor: 0, ink: 'top', spark: 'accent', pulse: true, fade: 240, hold: 'none', caption: 'crawl', wordSpark: true, strength: 'notable', motion: 'full' }),
  somber: Object.freeze({ budget: 1, floor: 1, ink: 'none', spark: 'muted', pulse: false, fade: 900, hold: 'aside', caption: 'still', wordSpark: false, strength: 'quiet', motion: 'legible' }),
});
 ...
 * @returns {{ name: string, budget: number, floor: number, ink: 'none'|'top', spark: 'accent'|'muted', pulse: boolean, fade: number, hold: 'none'|'aside', caption: 'crawl'|'still', wordSpark: boolean, strength: 'quiet'|'notable', motion: 'full'|'legible' }}
 */
