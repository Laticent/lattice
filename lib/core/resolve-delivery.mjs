 ...
 * slide get a gesture), says where overlay ink and the cursor still appear, and sets how deep the
 * rest recedes, the handoff's tempo and the caption's look. It never decides which moments matter: that is a fact about the content,
 ...
 *
 *   Every gestured moment FOCUSES: the named bullet, row, cell, column, chart mark, line or point
 *   keeps its own colors and everything else in its group recedes — the chart hover's emphasis
 *   (`focusContent`, `lib/base/base.focus.css`). One lever, opacity only, so no box moves. The
 *   fields below are how the presets differ, and they are ALL the difference (§6):
 *
 *   ink      where the Guide ALSO draws overlay ink (underline, wash, ring…) and shows its cursor:
 *            `'none'`, or `'top'` for the slide's top-ranked moment only.
 *   dim      the opacity the rest recedes to. `0.45` is the chart hover's own value.
 *   dimInner the opacity a walked line's OTHER points recede to. Deeper than `dim`: the line's own
 *            stroke stays whole and carries the shape, so its dots can drop further and the point
 *            being read stands out (at a gentler 0.62 it barely read — seen in the Studio).
 *   fade     the crossfade of one handoff, in ms: the old focus fades down as the new one fades
 *            up, in the same frame. Never two foci.
 *   hold     `'aside'` keeps the focus through a short aside ("Thank you.") that follows it;
 *            `'none'` releases it the moment the narration moves on.
 *   caption  `'crawl'` lights each word in the caption as it is said; `'still'` shows the line
 *            being read in one quiet ink.
 *   wordFocus with the captions OFF, light the word being spoken inside the focused text (on the
 *            caption's clock). With captions on the caption already reads along, and a second copy
 *            on the slide is redundancy (Mayer). Off for somber: a moving highlight is the motion
 *            somber refuses.
 *   strength `quiet` everywhere, or `notable` on the top-ranked moment's ink.
 *   motion   the Vetrina motion tier the stage runs at (`full` or `legible`).
 */
export const DELIVERY_PRESETS = Object.freeze({
  restrained: Object.freeze({ budget: 2, floor: 1, ink: 'none', dim: 0.45, dimInner: 0.3, fade: 200, hold: 'none', caption: 'crawl', wordFocus: true, strength: 'quiet', motion: 'legible' }),
  expressive: Object.freeze({ budget: 4, floor: 0, ink: 'top', dim: 0.3, dimInner: 0.2, fade: 160, hold: 'none', caption: 'crawl', wordFocus: true, strength: 'notable', motion: 'full' }),
  somber: Object.freeze({ budget: 1, floor: 1, ink: 'none', dim: 0.62, dimInner: 0.5, fade: 600, hold: 'aside', caption: 'still', wordFocus: false, strength: 'quiet', motion: 'legible' }),
});
 ...
 * @returns {{ name: string, budget: number, floor: number, ink: 'none'|'top', dim: number, dimInner: number, fade: number, hold: 'none'|'aside', caption: 'crawl'|'still', wordFocus: boolean, strength: 'quiet'|'notable', motion: 'full'|'legible' }}
 */
