 ...
 *
 * `center` / `right` are a deliberate follow-up — box-aligning capped/inset framing on the
 * full frame (esp. against a masthead bay) needs a design pass of its own, so this ships the
 * rock-solid left/auto core first. See engineering/decisions/2026-07-20-mass-head-alignment.md
 * (§ "Scoped to auto + left").
 *
...
// carries a `head-left` class. (`center`/`right` are a deferred follow-up.)
const HEADLINE_NAMES = Object.freeze(['auto', 'left']);
/** The class tokens — the per-slide override set (everything except the `auto` baseline). */
const HEADLINE_TOKENS = Object.freeze(['head-left']);
