 ...
 * deck. Measured across this repo's 12,551 deck spans it captures zero an author meant
 * literally — but that is OUR corpus. A deck written elsewhere, against plain Marp,
 ...
 *
 * THE CLASS IS THE CONTRACT, NOT THE REGISTER — and that is what makes this work on the
 * one surface we cannot otherwise reach. Three render paths carry the deck:
 *
 *   1. the ENGINE (markdown-it) — reads the front matter directly, and stamps
 *      `inline-code-literal` onto every section like every other deck-wide register;
 *   2. the STUDIO / Playground — the engine renders, the runtime mirrors over its output,
 *      so the class is already on the section when the mirror runs;
 *   3. a RAW MARP PREVIEW — marp-core parses the front matter and Lattice never sees it.
 *      Nothing of ours can read `inline-code:` there, the same wall `form: off` hits
 *      (lib/runtime `deckFormMode`).
 *
 * Because the runtime gates on the CLASS rather than the register, path 3 has a real
 * answer instead of a caveat: an author writes Marp's own global `class:` directive and
 * marp-core puts the token on every section itself. Verified against real marp-cli 4.x —
 * `class: inline-code-literal` emits
 * `<section id="1" data-class="inline-code-literal" … class="inline-code-literal">`.
 *
 *     ---
 *     marp: true
 ...
 *     ---
 *
 * KNOW WHAT THAT COSTS on path 3: Marpit's per-slide `_class:` REPLACES the global
 * `class:` for that slide, so a raw-Marp deck that also sets `_class:` somewhere loses the
 * token on those slides and the grammar comes back. A Lattice deck is unaffected — the
 * engine appends the token to the class list it builds, rather than competing for it.
 *
