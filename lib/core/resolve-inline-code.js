 ...
 * deck. Measured across the deck corpus #2066 defines — `examples/`, `lib/`, `docs/src`,
 * `test/integration`, 12,551 spans on 2026-09-04 — it captures zero an author meant
 ...
 *
 * (An earlier version of this note justified the class by claiming "nothing of ours can
 * read `inline-code:` on a raw Marp preview, the same wall `form: off` hits". That is
 * FALSE and worth recording: `lib/runtime` `deckFrontMatterSource()` fetches the sibling
 * `.md` and applies eighteen deck registers from it, which is how `eyebrow: dot` reaches a
 * section there. Only `form:` is limited to a BAKED block, and the note generalized from
 * the one exception. Believing it is plausibly what left this register out of the
 * runtime's own register mirror — the checker's first and worst finding.)
 *
 *     ---
 *     marp: true
