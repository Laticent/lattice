 ...
 * has no CSS of its own. `frontMatterValue` (lib/core/front-matter-key.js) is the one reader
 * every register goes through — the markdown-it plugins, the browser runtime, the linter and
 * the Studio's resolvers — and when a key in `PRESET_KEYS` is ABSENT from the front matter it
 * answers with the preset's value instead of null. So `preset: editorial` reads exactly as
