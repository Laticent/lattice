...
// The opt-in "spectrum accent on card surfaces" control. OFF by default — a card carries no
// spectrum rail unless the deck sets `spectrum-card: on` (stamps `spectrum-card` on every
// section) or a slide opts in with `_class: spectrum-card`. A slide drops out of a deck-wide
// `on` with `_class: spectrum-card-off`. Mirrors the `lift:` register. The painted rail reads
// the same `--spectrum` token, so it inherits the deck's STYLE choice.
const SPECTRUM_CARD_NAMES = Object.freeze(['on', 'off']);
/** The per-slide override tokens: `spectrum-card` opts a slide IN, `spectrum-card-off` OUT. */
const SPECTRUM_CARD_TOKENS = Object.freeze(['spectrum-card', 'spectrum-card-off']);
...
/** Map a spectrum-card value to its deck-wide class token. `on` → `spectrum-card`; `off`,
 *  empty, and unknown → `''` (off is the default, so no token is stamped). */
function spectrumCardClass(value) {
  if (typeof value !== 'string') return '';
  return value.trim().toLowerCase() === 'on' ? 'spectrum-card' : '';
}
...
/** True if a class token is a spectrum-card override token (`spectrum-card` / `-off`). Used by
 *  the render paths to detect a per-slide card override independently of STYLE / EDGE. */
function isSpectrumCardToken(token) {
  return token === 'spectrum-card' || token === 'spectrum-card-off';
}
