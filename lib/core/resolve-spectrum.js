...
function readFrontMatterSpectrum(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  const s = m[1].match(/^[ \t]*spectrum:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
...
function readFrontMatterSpectrumEdge(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  const s = m[1].match(/^[ \t]*spectrum-edge:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
  return s ? s[1].trim() : null;
}
...
function readFrontMatterSpectrumCard(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  const s = m[1].match(/^[ \t]*spectrum-card:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
  return s ? s[1].trim() : null;
}
...
function readFrontMatterSpectrumCardEdge(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  const s = m[1].match(/^[ \t]*spectrum-card-edge:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
  return s ? s[1].trim() : null;
}
...
function readFrontMatterSpectrumTrim(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  const s = m[1].match(/^[ \t]*spectrum-trim:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
  return s ? s[1].trim() : null;
}
