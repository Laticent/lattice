 ...
 *
 *   STYLE  (`spectrum:`)      — the gradient IDENTITY. Sets `--spectrum-style` /
 *                               `--spectrum-style-vertical` at the section level; the bar
 *                               reads `--spectrum-bar`, so EVERY spectrum-derived accent
 *                               follows as one system (`--spectrum` stays the theme's):
...
function readFrontMatterSpectrum(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  // The shared scalar rule + the SAME bare-name constraint this line used to fold into
  // one pattern. The old `$` anchor made a trailing YAML comment fail the whole
  // match, so an annotated register silently resolved to nothing.
  return frontMatterName(m[1], 'spectrum');
}
...
function readFrontMatterSpectrumEdge(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  return frontMatterName(m[1], 'spectrum-edge'); // shared rule; see resolve-finish.js
}
...
//     auto     → spectrum-card         rail follows the deck's section-bar STYLE (--spectrum-bar)
...
function readFrontMatterSpectrumCard(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  return frontMatterName(m[1], 'spectrum-card'); // shared rule; see resolve-finish.js
}
...
function readFrontMatterSpectrumCardEdge(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  return frontMatterName(m[1], 'spectrum-card-edge'); // shared rule; see resolve-finish.js
}
...
// The STRUCTURE tier control. By default the spectrum lives on the brand BAR alone (section edge
...
// accent-tint hairline (`--spectrum-structure` defaults to `--spectrum-quiet`, base.variants.css),
// so a default deck stays elegant and low-noise. Three tiers of increasing structural presence:
//   off        → (no token)                 the quiet accent-tint hairline — the default
//   restrained → spectrum-trim-restrained   a single-hue accent ramp (the two-tier look), held
//                                           constant regardless of the bar's STYLE (never clashes)
//   on         → spectrum-trim              the deck's full spectrum flows onto the structure
// Per-slide `_class: spectrum-trim` / `spectrum-trim-restrained` opts one slide in;
// `spectrum-trim-off` drops one out of a deck-wide setting. OFF by default. See
// 2026-07-16-spectrum-structure-default.md.
const SPECTRUM_TRIM_NAMES = Object.freeze(['off', 'restrained', 'on']);
/** The per-slide override tokens: `spectrum-trim` (full) / `spectrum-trim-restrained` opt a
 *  slide IN at a tier; `spectrum-trim-off` OUT. */
const SPECTRUM_TRIM_TOKENS = Object.freeze(['spectrum-trim', 'spectrum-trim-restrained', 'spectrum-trim-off']);
...
function readFrontMatterSpectrumTrim(md) {
  ...
  const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return null;
  return frontMatterName(m[1], 'spectrum-trim'); // shared rule; see resolve-finish.js
}
...
/** Map a spectrum-trim value to its deck-wide class token. `on` → `spectrum-trim`; `restrained`
 *  → `spectrum-trim-restrained`; `off`, empty, and unknown → `''` (off is the default, so no
 *  token is stamped). */
function spectrumTrimClass(value) {
  ...
  return ''; // off / empty / unknown → the quiet default
}
...
/** True if a class token is a spectrum-trim override token (`spectrum-trim` / `-restrained` /
 *  `-off`). Used by the render paths to detect a per-slide trim override independently of the
 *  other spectrum axes. Exact-set so it never matches an unrelated `spectrum-*` token. */
function isSpectrumTrimToken(token) {
  return token === 'spectrum-trim' || token === 'spectrum-trim-restrained' || token === 'spectrum-trim-off';
}
