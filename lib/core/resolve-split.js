 ...
 *                       Opt back into the classic separator-only behavior.
 ...
 *                       never orphans. `---` is ALSO honored (hybrid), so an
...
// back to the Marp-compatible separator-only behavior.
...
function readFrontMatterSplit(md) {
  ...
  return frontMatterName(m[1], 'split');
}
