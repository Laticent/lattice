...
function resolveTokenExpr(expr, vars, isDark, seen = new Set()) {
  ...
  // light-dark(light, dark) — resolve the selected arm through the EMBEDDED scanner, not
  // resolveTokenExpr, so an arm that is a COMPOUND value (a box-shadow whose colour trails
  // bare lengths — `0 0.1cqi 0.2cqi color-mix(…)` — or an `inset …` layer) still has its
  // color-mix()/light-dark() flattened. resolveTokenExpr alone only collapses an arm that IS,
  // as a whole, one colour call; a compound arm would fall through verbatim (an unflattened
  // color-mix → old-engine break). The cycle guard threads through.
  const ld = matchCall(s, 'light-dark');
  if (ld && ld.length === 2) {
    return resolveDeclarationValue(isDark ? ld[1] : ld[0], vars, isDark, seen);
  }
  ...
  if (cm && cm.length === 3) {
    ...
    // This evaluator implements ONLY the two mix spaces the palette recipe uses: `oklab`
    // and `srgb`. A different space (`oklch`, `hsl`, `lab`, …) would silently be computed
    // with the WRONG math and ship a literal that diverges from the browser's — a parity
    // break invisible on modern too. Refuse it: return verbatim so the compiler's
    // flatten-or-throw guard (build-chart-palette-css.js) fails the build loudly. (A mix
    // with `transparent`, handled below, is pure alpha-reduction and space-agnostic.)
    const spaceOk = space === 'oklab' || space.startsWith('srgb');
    const isPct = (v) => String(v).match(/^(-?\d*\.?\d+)\s*%$/);
    const readStop = (arg) => {
      // A stop is `<color> [<percentage>]`. EITHER may be a var() — the canonical
      // chart-fill gradient writes the ratio as a token
      // (`var(--fill-hue) var(--chart-fill-top-l)`). Only a TRAILING percentage is
      // the ratio; the colour is everything before it, resolved WHOLE — so a
      // multi-token modern colour (`rgb(255 0 0)`, `oklch(0.7 0.1 200)`) is not
      // split apart. The percentage may itself be a var() that resolves to `N%`.
      const s2 = arg.trim();
      const toks = s2.split(/\s+/);
      let pct = null;
      let colorStr = s2;
      if (toks.length > 1) {
        const lastPm = isPct(resolveTokenExpr(toks[toks.length - 1], vars, isDark, seen));
        if (lastPm) { pct = parseFloat(lastPm[1]); colorStr = toks.slice(0, -1).join(' '); }
      } else {
        const pm = isPct(s2);
        if (pm) { pct = parseFloat(pm[1]); colorStr = ''; }
      }
      return { color: colorStr ? resolveTokenExpr(colorStr, vars, isDark, seen) : '', pct };
    };
    ...
    if (h1 && h2 && spaceOk) return space.startsWith('srgb') ? mixSrgb(h1, h2, p2 / sum) : mixOklab(h1, h2, p2 / sum);
    return s; // unsupported space, or a non-resolvable stop (e.g. currentColor) — pass through (the compiler then throws)
  }
 ...
 */
function resolveDeclarationValue(value, vars, isDark, seen = new Set()) {
  ...
  while (i < s.length) {
    ...
    if (m && atBoundary) {
      ...
      out += resolveTokenExpr(call, vars, isDark, seen);
      i = j;
    } else {
      out += s[i];
      i++;
