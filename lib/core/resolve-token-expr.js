...
function resolveTokenExpr(expr, vars, isDark, seen = new Set()) {
  ...
  if (cm && cm.length === 3) {
    ...
    const readStop = (arg) => {
      // A stop is `<color> [<percentage>]`, where EITHER may be a var() — e.g.
      // `var(--fill-hue) var(--chart-fill-top-l)` (the canonical chart-fill
      // gradient uses a token for the mix percentage). Resolve each
      // whitespace-separated token, then classify: a token that resolves to a
      // bare `N%` is the percentage; the other is the colour.
      let color = null;
      let pct = null;
      for (const tok of arg.trim().split(/\s+/)) {
        const r = resolveTokenExpr(tok, vars, isDark, seen);
        const pm = String(r).match(/^(-?\d*\.?\d+)\s*%$/);
        if (pm) pct = parseFloat(pm[1]);
        else color = r;
      }
      return { color: color == null ? '' : color, pct };
    };
