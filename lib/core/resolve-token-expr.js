 ...
 * `themeVariables`, which wants literal colors, not CSS expressions. The
 ...
 * Anything else (a hex, a named color, rgb()/rgba(), a non-color length)
 ...
 *
 * Pure: reuses lib/theme/color.js for the color math (share, never
...
/** Named colors used as color-mix() stops in the palette/base expressions. */
...
/** Component-wise gamma-sRGB mix of two hex colors; `t` is the weight of `b`. */
...
/** Read a value as a hex color (#abc / #aabbcc / white / black); null if not. */
function asHex(v) {
...
function resolveTokenExpr(expr, vars, isDark, seen = new Set()) {
  ...
  if (cm && cm.length === 3) {
    ...
    // A transparent stop → the other color carried at the reduced alpha.
  ...
  return s; // literal hex / named color / rgb()/rgba() / non-color value
}
