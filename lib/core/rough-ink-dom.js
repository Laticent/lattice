 ...
 *        hLines, vLines, stroke, strokeWidth, inset}` — see `pathsForPlan`.
 */
function measureRoughInk(structures) {
  ...
  for (const spec of structures) {
    ...
    for (let n = 0; n < els.length; n++) {
      ...
      const declared = cs.getPropertyValue('--rough-ink-stroke').trim();
      if (!declared) continue;                     // not enrolled by the cascade

      // THE STROKE MUST BE A COLOR, AND NOTHING ELSE.
      //
      // `--rough-ink-stroke` is read out of the cascade and written straight
      // into an SVG `stroke` attribute, and a deck author's own `<style>`
      // block participates in that cascade. `stroke` takes a `<paint>`, which
      // includes `url(…)` — so an authored
      // `--rough-ink-stroke: url(https://evil/x.svg#p)` reaches the attribute
      // and the browser fetches it. Measured, in a `--fluid` export opened in
      // Chromium: one outbound request per opened copy. That is the beacon
      // HARD RULE #22 names as the off-docs-site harm.
      //
      // The capability is NOT ours to close: an author who can write `<style>`
      // can already write `background-image: url(https://evil/…)` and get the
      // identical request — verified the same way — and
      // `lib/core/sanitize-style-text.mjs` says in terms that CSS-as-CSS
      // fetching is intended ("it is not a CSS sanitizer and must not become
      // one"). What this file must not do is add a SECOND route to it. Taking
      // a value out of the cascade and re-injecting it as a DOM attribute is
      // exactly the third shape #22's docblock warns about — the one no gate
      // sees — so the value is validated here, at the crossing.
      //
      // `CSS.supports` is the browser's own parser, which is the only thing
      // that can be trusted to agree with what the attribute will accept.
      // Where it is unavailable (jsdom in the unit tests) the fallback is a
      // conservative reject of the funciri form rather than a pass-through.
      const isColor = typeof CSS !== 'undefined' && typeof CSS.supports === 'function'
        ? CSS.supports('color', declared)
        : !/url\s*\(/i.test(declared);
      // `currentColor` rather than skipping: a hostile value should cost the
      // deck its ink COLOR, not its rules. Skipping would leave the structure
      // with no lines at all whenever some other element on the same slide
      // inked and tripped the `:has(> svg…)` fallback gate.
      const stroke = isColor ? declared : 'currentColor';
      ...
      // `--rough-ink-inset` is registered as a `<length>` (base.sketch.css), so the
      // computed value is already px — no token or calc() left to resolve here.
      const insetRaw = parseFloat(cs.getPropertyValue('--rough-ink-inset'));
      const plan = {
        ...
        // Where the interior rules START, from the structure's left edge — a row
        // rule that begins at the text column rather than under the counter.
        inset: Number.isFinite(insetRaw) && insetRaw > 0 ? Math.min(insetRaw, w) : 0,
      };
