 ...
 *              (table / glossary / obligation-matrix). Column rules
 ...
 *   rows       interior row boundaries only, no frame (list.principles); the
 *              rules start at `--rough-ink-inset` (the text column), not at 0
 ...
 * components, and none of them rules columns: table and glossary
...
const ROUGH_INK_STRUCTURES = Object.freeze([
  {
    id: 'table',
    kind: 'grid',
    // Mirrors the CSS enrollment (FRAME) selector in base.sketch.css — the ink
    // painter and the fallback rules must agree on exactly which tables are
    // enrolled, or a table draws the wave strip AND the ink, or neither. Names
    // no component on purpose (sketch decorates whatever table it lands on);
    // the four exclusions are the tables that are structurally something else.
    // The CHILD combinators are load-bearing and match base.elements.css: a
    // descendant combinator enrolls a table sitting in a side frame
    // (split-panel, compare-code, image), which base excludes and which would
    // gain an ink overlay on a slide that never had one.
    sel:
      'section.sketch:where(:not(.matrix-grid):not(.math.derivation.form)' +
      ':not(.roadmap):not(.statute-stack.lane)) > table,' +
      'section.sketch:where(:not(.matrix-grid):not(.math.derivation.form)' +
      ':not(.roadmap):not(.statute-stack.lane)) > :where(.cell-stage) > table',
  },
  {
    id: 'tabular',
    kind: 'ledger',
 ...
 *   {number} strokeWidth
 *   {number} [inset]    where the horizontal rules start, structure-relative x
 *                       (`--rough-ink-inset`; 0 when absent)
...
function pathsForPlan(plan) {
  ...
  const x0 = plan.inset || 0;
  plan.hLines.forEach((y, i) => {
    emit(
      gen.line(x0, y, plan.w, y, {
        ...base,
