...
const ROUGH_INK_STRUCTURES = Object.freeze([
  {
    id: 'table',
    kind: 'grid',
    // Mirrors the CSS enrollment selector in base.sketch.css — the ink painter
    // and the fallback rules must agree on exactly which tables are enrolled,
    // or a table draws the wave strip AND the ink, or neither. Names no
    // component on purpose (sketch decorates whatever table it lands on); the
    // four exclusions are the tables that are structurally something else.
    sel:
