...
//
// `lat-pane` is the PANE Cell (lib/core/panes.js): a slide whose body holds two components
// wraps each component's body in one. DOMPurify drops unknown element names but keeps their
// children, so without this entry a Studio or Playground preview flattened both panes into one
// unstyled column while the CLI export (which does not sanitize) drew them. It is a plain
// container with no behavior of its own — no content model, no URL or script grammar, never
// registered as a custom element — and its attributes pass the same ALLOWED_ATTR filter as a
// `<div>`'s. A `<div class="lat-pane">` would not do: the component CSS reaches a pane through
// `:is(section,lat-pane)`, which only keeps its specificity because `lat-pane` is a TYPE.
export const ADD_TAGS = ['semantics', 'annotation', 'lat-pane'];
...
// the reasoning. Measured against the installed DOMPurify through `createSlideSanitizer`:
//
//   dominant-baseline="javascript:alert(1)"       → attribute STRIPPED
//   dominant-baseline="url(javascript:alert(1))"  → survives verbatim
//   vector-effect="url(javascript:alert(1))"      → survives verbatim
//
// So URI validation IS still applied to an `ADD_ATTR` attribute — a bare `javascript:` is
// caught. What gets through is the `url(…)` wrapper, because DOMPurify's allowed-URI test
// accepts a leading `[a-z+.-]+` run followed by a non-scheme character, and `url(` satisfies
// it. Those values are inert here for a SECOND, independent reason: both properties are
// enumerated-keyword-only, so the CSS parser discards anything else without fetching.
//
// The safety therefore rests on the GRAMMAR, with URI validation as a partial backstop, not
// the reverse. Adding an attribute whose grammar genuinely admits `url()` — a real fetch —
// would be a live vector that the `url(` gap in the URI test would NOT catch.
//
...
export function createSlideSanitizer(DOMPurifyFactory, win) {
	...
	// CONFIGURE ONCE, not per call. `dp.sanitize(html, cfg)` re-runs DOMPurify's
	// `_parseConfig` on every call, which rebuilds the ALLOWED_TAGS / ALLOWED_ATTR sets from
	// its defaults (~250 tags, ~300 attributes) and folds our FORBID/ADD lists in — fixed work,
	// paid per slide, that produces the same two sets every time. `setConfig` does it once and
	// the sanitize path then skips `_parseConfig` entirely.
	//
	// MEASURED by `tools/bench-sanitize.mjs` — real Chromium (the docs preview's own host), real
	// engine output, batch-timed p50 at the 4x CPU throttle `studio-preview-perf.spec.ts` uses:
	//   prose slide (723 B)             2.01ms -> 1.21ms   (-40%)
	//   gallery slice (1.5 KB)          2.36ms -> 1.57ms   (-33%)
	//   heaviest gallery slice (2.5 KB) 3.23ms -> 2.40ms   (-26%)
	//   whole 40-slide render (62 KB)  30.76ms -> 31.14ms  (the control: at 62KB the config parse
	//                                                       is a rounding error against the parse)
	// ~0.8ms off every call, on every route — including the keystroke path, where the input html is
	// new by construction and no cache can help. Output is byte-identical, and that is a sweep
	// rather than an argument: `--verify` sanitizes every section of every committed deck both ways
	// on BOTH hosts (281 decks / 3145 strings / 13.0 MB, zero differences in Chromium AND in
	// Node+jsdom, which is the export assembler's own host).
	//
	// SAFE ON THIS INSTANCE, and the reasons are worth keeping. (1) The instance is OURS —
	// `DOMPurifyFactory(win)` mints a fresh one per call to this function, so a persistent config
	// cannot reach another consumer's sanitize. (2) DOMPurify 3.4.11 captures pristine
	// ALLOWED_TAGS / ALLOWED_ATTR bindings at `setConfig()` time and restores them on every
	// sanitize, precisely so the per-call clone our `uponSanitizeAttribute` hook forces cannot
	// carry a widened allowlist into the next call. That guard is what makes this path equivalent
	// rather than merely faster, so it is pinned by test rather than trusted: both suites assert a
	// killer payload still dies on the Nth call, not just the first.
	// IF DOMPURIFY IS UPGRADED and those tests go red, restore the per-call `cfg` argument — the
	// speed is not worth a config that silently stops applying.
	dp.setConfig({ FORBID_TAGS, FORBID_ATTR, ADD_TAGS, ADD_ATTR });
	return (html) => (!html ? html : dp.sanitize(String(html)));
}
