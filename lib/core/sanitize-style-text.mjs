 ...
 *   - inside an unquoted `url()` — likewise a valid escape, and inert;
 *   - inside a comment — inert, but for a DIFFERENT reason worth not blurring: CSS comments
 *     have no escape processing at all, so the backslash is a literal text mutation that
 *     simply nothing reads. That holds for today's readers, not by syntax — and
 *     `lib/theme/serialize.js` does write structured data (`label`, `description`) into a
 *     comment header, so a future reader of that text would see the extra character;
 *   - in code position — `<` was never valid CSS there, so there is nothing to preserve.
 *
 * WHAT THIS DOES NOT SURVIVE. Inert is not permanent: a CSS **serializer** is entitled to
 * normalize the escape away, and css-tree does exactly that on a parse→generate —
 * `content:"<\/style>"` comes back as `content:"</style>"`. So any code that takes guarded
 * CSS out of a document, runs it through css-tree (`prunePlayerCss`), and re-wraps it in a
 * fresh `<style>` owes this call AGAIN at the re-wrap; the upstream guard does not travel
 * with the string. That is gated by `checkCssTreeRewrapSinks`, per site.
 *
