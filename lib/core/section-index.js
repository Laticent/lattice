 ...
 * see; a wrong one is misinformation the room cannot check.
 *
 * MARP-CLI IS NOT ON THAT SIDE OF THE LINE — both its outputs number correctly.
 * Measured on 4.5.0 against a 13-slide deck with four dividers: HTML and PDF both
 * carry 01/02/03/04, the PDF's in its text layer. An earlier revision of this
 * comment claimed the PDF path executes no deck script and therefore drew nothing.
 * That was measured against a build still carrying the section-vs-heading bug
 * below, so it recorded a broken render as a policy; a clean probe (an inline
 * `<script>` rewriting a heading) lands in the PDF. The surface still genuinely
 * unverified is the marp-vscode PREVIEW, where whether `<script>` runs is
 * contested in this repo (engineering/gotchas/vscode.md) and untested either way.
 *
 * The runtime stamps at BOOT as well as in the transform tail, and that is what
 * makes the PDF work: a consumer that captures the page before the heavy pass
 * finishes still gets the number, because the number needs no transform to run.
 */
 ...
 * this scopes to the slide region rather than calling `applyToHtml` on the whole
 * document — the head prefix is not ours to rewrite. (It was originally written
 * because embedded chrome DERAILED the walker; that stopped being true on
 * 2026-09-21, when splitSections moved onto a real tokenizer.) */
