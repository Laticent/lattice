 ...
 *   2. markdown-it (lib/core/boundary-parser.js — the ONE instance configured from the
 *      engine's own options, shared with the baker) tokenizes the baked body; its
 *      `hr` tokens ARE the engine's `splitOnHr`
 ...
 *
 * Pure (markdown-it only, no fs); SHARES `bake-splits.js`'s parser instance rather than
 * mirroring its config, so the two cannot reason over different token streams.
 */
...
function splitSourceToSections(source) {
  // `bakeSplits` normalizes at its own door, so `baked` is already LF and BOM-free;
  // stated rather than assumed, because this function's whole contract is that its
  // block COUNT matches the engine's section count, and a lone CR moved that by one.
  const baked = bakeSplits(normalizeSource(source)); // headings boundaries → `---`; now split: rule
  ...
  const hrLines = tokens
    .filter((t) => isSlideRule(t) && Array.isArray(t.map))
    .map((t) => t.map[0]);
