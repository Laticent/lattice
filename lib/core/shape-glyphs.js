 ...
 * `--icon-*`) precisely so the shape is IDENTICAL on every machine. A typed
...
const NOT_SHAPES = Object.freeze([
  ...
  { glyph: '›', why: 'single angle quote — punctuation, NOT the chevron icon (use --icon-chevron-right for chrome)' },
...
const SHAPE_GLYPHS = Object.freeze([
  ...
  // ── arrows
  {
    ...
    authorFix: 'In prose, write the word — "to", "becomes", "leads to". In a `quadrant` or ' +
      '`matrix-2x2` axis eyebrow the arrow is a PARSE-TIME delimiter that never reaches the ' +
      'slide, and the same parser already accepts ASCII `->`. In a gantt span the delimiter ' +
      'is `..`.',
    note: 'quadrant.transform.js splits the eyebrow on /(?:→|->)/ and consumes it, so `->` renders identically — which is to say, not at all.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-arrow-up', authorFix: 'Write the word. Emoji-presented on most systems.',
  },
  {
    glyph: '⬇', name: 'DOWNWARDS BLACK ARROW', role: 'arrow',
    token: '--icon-arrow-down', authorFix: 'Write the word. Emoji-presented on most systems.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-chevron-down', authorFix: 'Let the layout draw its own chrome.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-triangle-right', authorFix: 'Layout chrome — let the component draw it.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-triangle-down', authorFix: 'Layout chrome — let the component draw it.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-pause', authorFix: 'A transport control is `scene` chrome the engine draws.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-spark', authorFix: 'Let the layout draw its own ornament.',
  },
  {
  ...
  },
  {
    ...
    token: '--icon-spark', authorFix: 'Say what is new in words. Emoji — full color, palette-blind.',
  },
  {
...
/** The variation selector that forces emoji presentation — carried along with
 * the glyph it follows so a message can say "and it is asking for emoji". */
const EMOJI_VS = '️';
