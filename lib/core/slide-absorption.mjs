 ...
 *  sr-only description and KaTeX's `.katex-mathml` — both are the SAME CONTENT AGAIN for a screen
 *  reader, not more to absorb. Missing the KaTeX one measured a math slide at 111 prose words where
 *  it holds 66: 41% of the count was one formula counted three times, which under the picked model
 *  saturated BOTH beats and wrapped the slide in 18 s of silence. */
const SKIP_SELECTOR =
	'header, footer, .cell-footer, .masthead-bay, .lat-pagination, aside, script, style, .lattice-notes, .lattice-description, .katex-mathml';
...
function countWords(stage) {
	...
	if (!stage?.ownerDocument || typeof stage.ownerDocument.createTreeWalker !== 'function') {
		return { proseWords, labelWords, proseText: '' };
	}
	...
	for (let node = walker.nextNode(); node; node = walker.nextNode()) {
		...
		if (el.closest(PROSE_SELECTOR) && !el.closest('svg')) {
			proseWords += n;
			prose.push(node.nodeValue.trim());
		} else labelWords += n;
	}
	// Joined with a SPACE, which is the whole point: the role probe below matches phrases
	// ("thank you", "q&a"), and a separator-free join invents and destroys them at block seams.
	return { proseWords, labelWords, proseText: prose.join(' ') };
}
...
function itemElements(root) {
	...
	const list = root.querySelector(':scope > ul, :scope > ol, :scope > * > ul, :scope > * > ol');
	if (!list) return 0;
	let n = 0;
...
export function measureSlide(section, ctx = {}) {
	...
	const { proseWords, labelWords, proseText } = countWords(stage);
	...
	const items = declared?.domSelector ? countContent(stage, declared.domSelector) : itemElements(stage);
	...
	// The mark sets are DISJOINT BY CONSTRUCTION, and each exclusion is load-bearing:
	//   · `svg text` is NOT a mark — its words are already charged as `labelWords`, so counting the
	//     element too billed 9 quadrant labels as 25 things to look at (160 such elements in the
	//     bake-off corpus).
	//   · `tr` is NOT an item — its `td`/`th` are already `cells`, so a 2x2 table measured 2 rows +
	//     4 cells + 4 label words = 10 marks for 4 pieces of content.
	//   · `figure` is NOT an image — it WRAPS one, so a `figure > img` was charged twice at IMAGE_MS.
	//     A figure's caption is prose and is counted there.
	const svgMarks = countContent(stage, 'svg path, svg rect, svg circle, svg ellipse, svg polygon, svg polyline, svg line');
	const images = countContent(stage, 'img, video');
	...
	if (stage && typeof stage.querySelectorAll === 'function') {
		for (const el of stage.querySelectorAll('code[class*="language-mermaid"], pre.mermaid, .mermaid')) {
			...
			// A wrapper AND its inner `code` can both match (`<pre class="mermaid"><code
			// class="language-mermaid">`), which counted one diagram twice. The innermost match owns
			// it. `.mermaid` is the class the BROWSER render path uses, so this shape is reachable
			// there even though `lib/engine` emits only `<pre><code class="language-mermaid">` today.
			if (el.querySelector?.('code[class*="language-mermaid"], pre.mermaid, .mermaid')) continue;
	...
	// The closing-phrase probe reads the PROSE, not `stage.textContent`. `countWords` was rewritten
	// to walk text nodes precisely because textContent concatenates adjacent elements with no
	// separator — and this probe had kept the defect: `<p>data</p><p>Thank you</p>` read as one run,
	// while chrome the word count excludes (a masthead, a footer) still reached the regex. No role
	// flipped across 1602 slides today, so this closes a latent defect, not a live one.
	const role = roleOf(component, index, total, proseText, bucketOf);
 ...
 *
 * BOUNDED on purpose. The first draft used `={2,}>`, whose open quantifier backtracks quadratically
 * on a run of `=` with no `>` — measured 15 ms at 4k characters, 303 ms at 16k, 1.0 s at 32k, 2.3 s
 * at 50k — and the input is a markdown fence, i.e. author-controlled text from a shared deck. That
 * is the exact ReDoS shape the house pattern already bans (`edgeTrim`,
 * docs/src/lib/cadenza/normalize.ts). Every quantifier here has an upper bound.
 */
const EDGE_OP =
 ...
 *  literally `---`, which the link alternation reads as two plain edges — on the largest diagram in
 *  the corpus that inflated the cost by 1.8 s, and it reaches six shipped decks including the
 *  gallery. Stripped before anything else runs. */
 ...
 * text and no marks, so a DOM-only visual model scores every diagram in the tree at ZERO and
 * concludes diagrams need no time.
 *
 * NODES ARE DERIVED FROM EDGES, NOT PARSED AS IDENTIFIERS. Reading the identifiers either side of
 * an arrow is the obvious approach and it is a trap: an unspaced link (`Server--xClient`, which
 * shipped decks use) has no whitespace to split on, so the operator's characters fuse into the
 * name. A three-participant sequence diagram yielded the "nodes" `Server--`, `xClient` and
 * `Worker--` — not one of them real, with the actual participants dropped — and separately, comment
 * words and `click` handler names became nodes.
 *
 * A connected diagram with E edges has between E and E+1 nodes, so `edges + 1` bounds it with no
 * identifier parsing at all and cannot invent a name. Explicit shape declarations (`A[label]`,
 * `B(label)`, `C{label}`) are counted when present and win when higher, which covers the declared
 * but unlinked node a pure edge count would miss. The result feeds a knee curve, so being out by
 * one node moves the beat by tens of milliseconds; being out by "there is a diagram here at all"
 * moves it by seconds.
 */
export function diagramMarksOf(source) {
	const src = String(source ?? '');
	if (!src.trim()) return { nodes: 0, edges: 0 };
	// Frontmatter first, then `%%` comments — both carry punctuation the link alternation would
	// otherwise read as diagram structure.
	const body = src.replace(MERMAID_FRONTMATTER, '').replace(/^[ \t]*%%.*$/gm, '');
	const edges = (body.match(EDGE_OP) || []).length;
	...
	const declared = new Set(body.match(/\b[A-Za-z_][A-Za-z0-9_]*(?=[[({])/g) || []);
	const nodes = Math.max(declared.size, edges > 0 ? edges + 1 : 0);
	return { nodes, edges };
}

/** A measure field as a usable number. Every model reads its inputs through this, so a partial or
 *  hand-built measure yields a real cost instead of NaN. It mattered: `scanTimeMs` guarded only
 *  `labelWords`, so `visualCost` and `hybrid` returned NaN on a partial measure — and `spend`'s own
 *  finiteness guard then turned that into cost 0, silently giving the slide NO absorption rather
 *  than failing loudly. */
function num(v) {
...
export const ABSORPTION_MODELS = {
	...
	roleWeighted: {
		...
		cost: (m) => Math.max(0.4, (0.6 + num(m.words) / 45) * (ROLE_MULT[m.role] || 1)) * 1600,
	},
...
export function spend(costMs, opts = {}) {
	...
	// function advertises. Unreachable at today's 1400 ms SLIDE_PAUSE_MS; a slower future preset
	// would reach it. Likewise a non-finite `arriveShare` must not zero the beat.
	const floor = num(floorMs);
	const share = Number.isFinite(arriveShare) && arriveShare >= 0 ? arriveShare : 0.35;
	...
	const arriveMs = Math.max(floor, clampBeat(Math.max(floor, cost * share)));
	...
	const credit = num(narrationMs) * (isMedia ? num(mediaNarrationCredit) : 1);
	const residual = cost - arriveMs - credit;
	const exitMs = residual > MIN_BEAT_MS ? clampBeat(residual) : 0;

	// `costMs` is reported clamped like the beats it produced — an unclamped 1e12 in a diagnostic
	// row reads as a measurement rather than as the saturated value the beats actually used.
