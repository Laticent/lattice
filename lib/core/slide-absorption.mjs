...
function itemElements(root) {
	...
	if (!list || list.closest?.(SKIP_SELECTOR)) return 0;
	let n = 0;
...
function walkSlide(root, isHidden) {
	...
	for (let node = walker.nextNode(); node; node = walker.nextNode()) {
		...
		if (node.nodeType === 1) {
			...
			else if (tag === 'IMG' || tag === 'VIDEO') images += 1;
...
function stripDiagramText(src) {
	return src
		.replace(/"[^"\n]*"/g, '""') // quoted labels
		.replace(/'[^'\n]*'/g, "''")
		.replace(/%%.*$/gm, '') // comments, leading or trailing
...
export function diagramMarksOf(source) {
	...
	if (edges === 0 && isRowGrammar) {
		// Rows minus the type declaration itself; each row is a datum to take in.
		const rows = Math.max(0, lines.length - 1);
		return { nodes: rows, edges: 0 };
	}
...
export function spend(costMs, opts = {}) {
	...
	const readResidual = num(ch.read) * scale - narr;
