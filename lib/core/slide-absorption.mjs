...
export function roleOf(component, index, total, text, bucketOf) {
	...
	// `math` is in VISUAL_BUCKETS, so a math slide was `isVisual: true` with role `body` — scored
	// as visual by the metric and weighted as prose by the multiplier. One list, one answer.
	if (VISUAL_BUCKETS.has(bucket)) return 'visual';
