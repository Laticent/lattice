...
function isTableDelimiterRow(rest) {
	...
	//
	// WHAT IT IS AND IS NOT. The pattern has ambiguous quantifiers (`[ \t]*` and `-+` inside a
	// `*` group) — the classic polynomial-ReDoS shape. Measured, V8 runs it LINEARLY: 64,000
	// dashes with a failing suffix cost 0.70ms, and a million cost proportionally more, not
	// quadratically. So this fixes no observed hang, and no analyzer asked for it: it was added
	// on a WRONG guess about which CodeQL alert this file had raised (the real one was
	// `js/bad-tag-filter`, on the comment terminator above). It is kept because it is free —
	// measured at 0.678ms vs 0.734ms over the 60KB gallery deck, i.e. inside the noise — and
	// because this function runs on every keystroke over text an untrusted shared deck controls
	// (HARD RULE #22's threat model), where an input class that is provably unambiguous beats
	// one that is empirically fast on today's engine.
	let bar = false;
...
// The only terminator that is genuinely a pattern rather than a substring — a
// case-insensitive alternation, exactly as markdown-it spells it.
const HTML_RAW_TEXT_END_RE = /<\/(?:script|pre|style|textarea)>/i;
/**
 ...
 *
 * `end` is a PREDICATE that answers "does this line close the block"; `blankCloses`
 * marks the kinds (types 6 and 7) that run to the next BLANK LINE rather than to a
 * marker. That distinction is not academic — `<div>` / `x` / `</div>` / `---` with
 * no blank line anywhere renders ZERO slide breaks, because the `---` is still
 * inside the HTML block. A splitter that stops at `</div>` cuts a slide the engine
 * does not.
 *
 * `interrupts` is false only for type 7, which CommonMark forbids from breaking
 * an open paragraph.
 *
 * FOUR OF THE FIVE TERMINATORS ARE SUBSTRING SEARCHES, and they are written as
 * such rather than as regexes. markdown-it stores them as patterns and calls
 * `.test()`, which for `-->`, `?>`, `]]>` and `>` is a substring search wearing a
 * regex costume; only the raw-text terminator needs a real pattern (a
 * case-insensitive alternation). Spelling them as `includes` says what they are,
 * skips the regex engine, and — the reason it changed — stops `js/bad-tag-filter`
 * flagging `/-->/` as a "bad HTML filtering regexp".
 *
 * THAT ALERT IS CORRECT ABOUT HTML AND WRONG ABOUT THIS CODE, which is worth
 * stating rather than silently working around. Its point is that an HTML comment
 * can also end with `--!>`, so a SANITIZER keying on `-->` alone can be walked
 * past. This is not a sanitizer: it models markdown-it's block grammar, whose
 * comment terminator is `/-->/` and nothing else. Verified against the real
 * parser — `<!-- x --!>` / `---` renders ZERO slide breaks, because the block is
 * still open — so teaching this `--!>` would make the scanner disagree with the
 * engine, which is the entire defect class this module exists to end. The
 * behavior is deliberately unchanged; only the spelling is.
 */
function htmlBlockOpener(rest) {
	...
	// THE SAME LINEAR PRE-FILTER, on the same terms. `HTML_OPEN_CLOSE_TAG_RE` is transcribed from
	// markdown-it verbatim — it has to be, that is the contract — and markdown-it's own attribute
	// loop is ambiguous: the `\s+` separating attributes and the `\s*` before the closing `>` can
	// split a run of spaces between them N ways. A type-7 block is a COMPLETE tag alone on its
	// line, so it must end with `>`, and a line that does not is rejected here in one pass without
	// ever reaching the pattern.
	//
	// Measured through `slideBoundaries` on `'<a' + ' b'.repeat(64000) + '!'`: 3.26ms before,
	// 0.12ms after — 27x, and below what an ordinary deck of the same length costs. The unguarded
	// pattern was linear on V8 too (1,000,000 repeats in 116ms), so like the guard above this
	// hardens a shape rather than rescuing a hang. The transcription itself is untouched, so the
	// test that pins it against markdown-it's `.source` still holds.
