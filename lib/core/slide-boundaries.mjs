...
// A complete open or close tag, alone on its line — markdown-it's HTML_OPEN_CLOSE_TAG_RE
// anchored to end-of-line. This is CommonMark's html-block type 7, the one kind that
// cannot interrupt a paragraph.
const HTML_ANY_TAG_RE =
	/^(?:<[A-Za-z][A-Za-z0-9-]*(?:\s+[^\s"'/>=]+(?:\s*=\s*(?:[^\s"'=<>`]+|'[^']*'|"[^"]*"))?)*\s*\/?>|<\/[A-Za-z][A-Za-z0-9-]*\s*>)[ \t]*$/;
...
export function slideBoundaries(body) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (!htmlOpen && rest[0] === '<' && !paraOpen) {
			// A `<`-led line the tag grammar above does not recognize. It is almost certainly
			// inline HTML inside a paragraph — but "almost certainly" is the register this
			// module refuses to answer in, because the difference decides whether the deck's
			// next `---` is inside an HTML block or is a slide boundary.
			unsure('a line opening with `<` that is not a recognized HTML block');
		}
