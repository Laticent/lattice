...
import { boundaryParser as md, normalizeSource } from './boundary-parser.js';
...
/**
 * A ONE-ENTRY MEMO on the parse, because the editor asks this question several
 * times per keystroke.
 *
 ...
 * for several parses of the same string; with it, one. Single-entry on purpose:
 * the deck being edited is the only string asked about repeatedly, and an
 * unbounded cache keyed on every intermediate document state is a leak, not a
 * cache — the same reasoning, and the same boundedness property, as the
 * whole-deck render memo in `docs/src/lib/single-slide-render.ts`.
 */
let memoKey = null;
let memoValue = null;
