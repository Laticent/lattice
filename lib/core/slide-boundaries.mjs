 ...
 *
 * WHY IT IS NO LONGER SINGLE-ENTRY. It was, and "the deck being edited is the
 * only string asked about repeatedly" turned out to be false of the preview
 * path in a way that made the single entry THRASH — every eviction paying a
 * whole-deck parse to re-derive an answer it had just discarded. One keystroke
 * asks about several NEARLY-identical strings, and they are distinct for
 * reasons that are each deliberate:
 *
 *   · the editor asks about `source`; the preview asks about `editorSample`,
 *     which `StudioShell` REBUILDS from trimmed chunks, so the two differ by
 *     whitespace and can never share an entry;
 *   · `deckSectionFor` asks about the body AND about a code-blanked copy of
 *     the body, alternating, inside one derivation.
 *
 * Measured by counting real `md.block.parse` calls (not by timing): with one
 * entry a 40-slide gallery deck pays 3 parses per keystroke where 2 are
 * distinct, and a deck with code but no `_class: divider` parses ONE
 * byte-identical string twice. Which case loses depends only on call ORDER,
 * so reordering the callers moves the waste rather than removing it — the
 * cache has to hold both.
 *
