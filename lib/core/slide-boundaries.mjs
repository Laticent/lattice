 ...
 * TYPING. False, and false for a reason worth keeping — it assumed the editor and
 * the preview ask about the SAME string, which the paragraph directly above says
 * they never do.
 *
 * MEASURED ON THE CODE THAT SHIPS, by wrapping `boundaryParser.block.parse` and
 * replaying the real caller order on the perf spec's 40-slide gallery deck, varying
 * ONLY `MEMO_MAX` so nothing else is credited to it:
 *
 *     MEMO_MAX = 1   typing 2 parses   navigation 2
 *     MEMO_MAX = 6   typing 2 parses   navigation 0
 *
 * So on TYPING this buys nothing, and on NAVIGATION — where the deck string does not
 * change at all — it removes both parses. Navigation is the interaction a memo can
 * help by construction and typing is the one it cannot: the same asymmetry the
 * whole-deck render memo has in `docs/src/lib/single-slide-render.ts`, and the same
 * one #1273 turned on.
 *
 * TWO EARLIER NUMBERS HERE WERE WRONG, and both erred the same way — by crediting
 * this cache with savings that belong to its neighbors. "Three parses either way"
 * was measured on the intermediate tree, before the divider derivation stopped
 * parsing a code-blanked copy; "5 → 0 on navigation" is the single-entry cost on the
 * BASE tree, where neither the derivation memo nor the raw-chunk fix exists. Quoting
 * it here stacks three changes' wins onto one — the exact error Amendment 7 says it
 * corrected, committed inside the correction. Numbers above are `MEMO_MAX`-only.
 *
 ...
 *
 * WHY SIX. The typing working set is TWO distinct strings: the editor's body and
 * the preview's `editorSample` body, which `StudioShell` rebuilds from trimmed
 * chunks so it is never byte-equal. It was THREE — the divider derivation used to
 * parse a code-blanked copy of the body as well — until that derivation was fixed
 * to chunk the raw body and blank per chunk, which removed the third parse
 * entirely. (An earlier version of this note still said three, and its own
 * parenthesis named the caller this branch had already deleted. Recorded because
 * a sizing rationale that outlives its measurement is exactly the "reasoned rather
 * than measured" failure this file keeps catching.)
 *
 * The largest configuration I could ENUMERATE — not measure — across every
 * `<DeckPreview>` call site is five: Present with a reader lens open beside a live
 * editor preview. Six is that plus one. Overflow degrades gracefully (extra parses,
 * never a wrong answer), so the constant is a comfort margin over an enumeration,
 * not a threshold. Both the floor and the eviction ORDER are pinned in
 * `test/unit/core/slide-boundaries.test.js` — a bare size constant with no test is
 * how this drifts back to thrashing silently.
 */
const MEMO_MAX = 6;
