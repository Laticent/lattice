...
// Panes: a panes slide the engine SPLITS (lib/core/panes.js `installPaneSplit`) renders as one
// slide per pane, so the source-side answer splits it at the same markers.
const paneSpec = require('./pane-spec');
...
function slideClassSpans(source) {
  ...
  // NORMALIZATION IS READ, NOT APPLIED, and that asymmetry is forced. Every other
  // boundary module folds CRLF / lone CR / a BOM at its door the way the engine
  // does; this one CANNOT, because it returns offsets into the string the CALLER
  // holds, and rewriting the source moves every one of them. So it reads a
  // normalized copy where a normalized copy is what a reader needs, and counts
  // lines against the ORIGINAL bytes.
  //
  // Both halves matter. A BOM is invisible to the `^---` anchor, so front matter
  // reads as body (and `split: rule` silently becomes the default `headings`). And
  // markdown-it folds `\r` internally, so its `token.map` counts NORMALIZED lines
  // while a `split('\n')` over raw CR-delimited text sees one enormous line — the
  // two desync and every boundary lands in the wrong place.
  const bomLen = src.charCodeAt(0) === 0xfeff ? 1 : 0;
  const fmMatch = src.slice(bomLen).match(FRONT_MATTER);
  const bodyStart = bomLen + (fmMatch ? fmMatch[0].length : 0);
  const body = src.slice(bodyStart);

  // Full-source offset of the first character of each body line, counted over the
  // REAL line terminators, so line k here is line k in markdown-it's folded view.
  const lineOffset = [bodyStart];
  for (let i = bodyStart; i < src.length; i++) {
    if (src[i] === '\n') lineOffset.push(i + 1);
    else if (src[i] === '\r') lineOffset.push(src[i + 1] === '\n' ? (i += 1) + 1 : i + 1);
  }
  const lineCount = lineOffset.length;

  const tokens = md.parse(body, {});
  const boundaries = [];
  for (const t of tokens) {
    if (isSlideRule(t) && Array.isArray(t.map)) {
      boundaries.push({ line: t.map[0], consumed: true });
    }
  }
  if (resolveSplitMode(normalizeSource(src)) === 'headings') {
  ...
  // exists to close.
  const bodyOpensWithRule = isSlideRule(tokens[0]);
  const startLines = bodyOpensWithRule ? [] : [0];
  for (const b of ordered) startLines.push(b.consumed ? b.line + 1 : b.line);
  const uniqueStarts = [...new Set(startLines)].sort((a, b) => a - b).filter((n) => n <= lineCount);
  const offsetOf = (line) => (line < lineCount ? lineOffset[line] : src.length);
  ...
  for (const dir of directives) {
    ...
    if (span) {
      span.slideClass = dir.value;
      span.spot = dir.value;
    }
  }

  // ── Panes the engine splits into slides ───────────────────────────────────
  //
  // `installPaneSplit` turns a panes slide into one ordinary slide per pane when the deck is
  // not 16:9-family, or when its two components do not fit either way (`arrangePanes`). Each
  // page's class is its pane's component — a SPOT directive, so it replaces the running
  // global — merged with the slide's own spot `_class` when it has one, wherever on the slide
  // the author wrote it. The second marker STARTS the second page's span.
  const size = (fmMatch ? fmMatch[0] : '').match(/^\s*size:\s*["']?([\w:/.-]+)/m)?.[1];
  const geom = sizeFor(size);
  const alwaysSplit = familyFor(parseFloat(geom.width) / parseFloat(geom.height)) !== 'wide';
  const out = [];
  for (const span of spans) {
    const inSpan = (t) => Array.isArray(t.map) && t.level === 0 && t.type === 'html_block'
      && (lineOffset[t.map[0]] ?? bodyStart) >= span.start && (lineOffset[t.map[0]] ?? bodyStart) < span.end;
    const markers = tokens.filter((t) => inSpan(t) && paneSpec.PANE_RE.test(t.content.trim()));
    if (markers.length < 2) {
      out.push({ start: span.start, end: span.end, slideClass: span.slideClass });
      continue;
    }
    const classes = markers.map((t) => t.content.trim().match(paneSpec.PANE_RE)[1]);
    const layoutTok = tokens.find((t) => inSpan(t) && paneSpec.PANES_RE.test(t.content.trim()));
    const layout = paneSpec.parseLayout(layoutTok ? layoutTok.content.trim().match(paneSpec.PANES_RE)[1].trim() : null);
    const splits = alwaysSplit || paneSpec.arrangePanes(PANE_CATALOG, classes.slice(0, 2), layout).as === 'split';
    // A component class on a panes slide never runs: the carve takes it off the host
    // (lib/core/panes.js `markHost`), and a split gives each page its pane's component instead.
    const keep = (list) => (list || '').split(/\s+/).filter((c) => c && !isComponentToken(c)).join(' ');
    if (!splits) {
      out.push({ start: span.start, end: span.end, slideClass: keep(span.slideClass) });
      continue;
    }
    // Two pages: a third marker folds into the second pane's page, as the carve folds it.
    const cut = lineOffset[markers[1].map[0]];
    const extra = keep(span.spot);
    [[span.start, cut], [cut, span.end]].forEach(([start, end], k) => {
      out.push({ start, end, slideClass: extra ? `${classes[k]} ${extra}` : classes[k] });
    });
  }
