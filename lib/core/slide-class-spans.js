...
function slideClassSpans(source) {
  ...
  // global — merged with the slide's own spot `_class` when it has one. A pane's content lies
  // after its marker, so each marker after the first STARTS the next page's span.
  ...
  for (const span of spans) {
    ...
    const cuts = markers.slice(1).map((t) => lineOffset[t.map[0]]);
    const bounds = [span.start, ...cuts, span.end];
    classes.forEach((cls, k) => {
      out.push({ start: bounds[k], end: bounds[k + 1], slideClass: span.spot ? `${cls} ${span.spot}` : cls });
    });
  }
