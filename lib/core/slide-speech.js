 ...
 *
 * The rules are CommonMark's, and the same three the Mermaid matcher uses: a run of three
 * or more of one character opens; a run of the SAME character, at least as long and alone
 * on its line, closes; and a ``` fence's info string may not itself contain a backtick, so
 * one line of `` `inline code` `` cannot open a fence that swallows the rest of the slide.
 *
