 ...
 *
 * `inventory`, `matrix-2x2` and `verdict-grid` joined for exactly that reason.
 * Measured on a probe deck, dropping the asterisks left all three with ZERO
 * `<strong>` elements — the header existed only because the author typed it.
 * `inventory` needed its authoring shape moved from the inline `- **Lead.** body`
 * to the nested `- Lead` / `  - body` at the same time: the lift takes the whole
 * lead up to the first nested list, so an inline body has nothing to delimit it and
 * would be swallowed into the label. That is not hypothetical — it is what a bare
 * `redline` row did, and why redline's decks moved to the nested shape too.
 */
const SLOT_LAYOUTS = Object.freeze([
  ...
  'inventory', 'matrix-2x2', 'verdict-grid',
]);
