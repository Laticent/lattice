...
const { hasCodaClass, tagHasClass, STRUCTURAL, CODA_CLASS, wrapBelowNote } = require('./coda');
 ...
 */
const SPLIT_ROLES = Object.freeze(['cover', 'body', 'closing', 'insight']);
...
const isChrome = (el) => {
  ...
  if (new RegExp(`\\s${BERTH_ATTR}(?=[\\s=>/])`).test(openTag)) return true;
 ...
 *
 *   · `insightPage` removes just the note half, which strips the cell's closing
 *     `</div>` and leaves its open tag — the exported section had `.cell-stage`
 *     unclosed, so in the browser the stage swallowed the marker berths and the
 *     k-of-N rail (4 unbalanced sections in one real deck; 0 before the coda).
 *   · `insightPageFrom` keys `keep` on a span's `start:end`, so a half-span never
 *     matched the cell's real top-level range and the WHOLE cell was dropped — the
 *     insight page shipped blank, losing the author's Key Insight outright.
 *
...
function trailingSpansIn(inner, collEnd, cellEnd) {
  ...
  while (first > 0 && (isCodaCell(els[first - 1]) || isKeyInsightEl(els[first - 1]) || isTrailingNoteEl(els[first - 1]))) first -= 1;
...
function trailingMaterialOf(inner) {
  const stage = extractStage(inner);
  const cell = stage ? { start: stage.bodyStart, end: stage.bodyEnd } : { start: 0, end: inner.length };
  const trailing = trailingSpansIn(inner, cell.start, cell.end);
...
function insightPageFrom(openTag, inner) {
  ...
  const stage = extractStage(inner);
  const cell = stage ? { start: stage.bodyStart, end: stage.bodyEnd } : { start: 0, end: inner.length };
  // The insight's span is the WHOLE coda cell (codaSpans), so it matches a real
  // top-level element and survives the `keep` test — and it is REPLACED by the
  // insight-only rewrite, which is what drops a co-resident note from this page.
  // Keying `keep` on a half-span matched nothing here, so the cell joined `drop`
  // and the page shipped with an empty stage.
  const keep = new Map(insight.map((s) => [`${s.start}:${s.end}`, s]));
  // The edit list must span every element that could be dropped OR rewritten. The
  // stage's children are dropped; the coda Cell is REWRITTEN insight-only — and it
  // now sits beside the stage, outside `cell`, so it has to be added by hand or the
  // page ships with a co-resident NOTE leaking onto it. (An earlier comment here said
  // "the body dropped and no insight to show for it" — mutation-tested, that is not what
  // happens: without the sibling in the list `spliceSpans` simply leaves the cell alone,
  // so the insight survives and the note rides along. The arm is right; the reason was
  // not, and a wrong reason in a comment is the kind of thing a reader trusts.)
  const cellEls = topLevelElements(inner.slice(cell.start, cell.end))
    .map((el) => ({ start: cell.start + el.start, end: cell.start + el.end, name: el.name }));
  const sibEls = stage
    ? topLevelElements(inner)
        .filter((el) => isCodaCell(el))
        .map((el) => ({ start: el.start, end: el.end, name: el.name }))
    : [];
  const edits = [...cellEls, ...sibEls]
    .filter((el) => !isChrome(el))
    .map((el) => keep.get(`${el.start}:${el.end}`) ?? el)
    .sort((a, b) => a.start - b.start);
  const tag = withRole(addClass(openTag.replace(/\s+id="[^"]*"/, ''), 'lat-split-insight'), 'insight');
  return `${tag}${spliceSpans(inner, edits)}</section>`;
}
 ...
 *
 * Simpler than `insightPage` by construction: that one had to surgically drop a co-resident
 * note out of a shared coda cell, which is where its half-span hazards came from. This page
...
function insightPage(openTag, inner, regions) {
  ...
  // The note and the insight share one range when both live in the same coda cell,
  // so REMOVING the note would take the insight's markup with it (and, when they were
  // half-spans, leave `.cell-stage` unclosed). Replace that range with the
  // insight-only rewrite instead; a note that is NOT in a cell still just goes away.
  const insightByRange = new Map(regions.insight.map((s) => [`${s.start}:${s.end}`, s]));
  const noteEdits = regions.note.map((n) => insightByRange.get(`${n.start}:${n.end}`) ?? { start: n.start, end: n.end });
  const insightInner = spliceSpans(inner, [...regions.lede, regions.coll, ...noteEdits]);
