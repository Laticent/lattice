...
function deriveAxis(inner) {
  ...
  for (;;) {
    ...
    // container-responsive. A bare inline `<svg>` glyph (an icon in a list item) is not the
    // slide's collection, so keep looking.
    if (tag === 'svg' && !/\bviewBox=/i.test(m[2])) {
      rest = rest.slice(m.index + m[0].length);
      continue;
    }
    return AXIS_OF_TAG[tag] || null;
  }
}
