...
// The first non-closing element walking back therefore ends the run — whatever sits before
// it is the component's own structure, left in place (and still repeating, as before).
function closingSpansIn(inner, collEnd, cellEnd) {
  const region = inner.slice(collEnd, cellEnd);
  const els = topLevelElements(region);
