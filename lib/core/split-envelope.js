...
function ledeSpansIn(inner, cellStart, collStart) {
  ...
  const direct = top.filter((el) => isLedeP(el) && el.start > h2At);
  ...
  return topLevelElements(sub)
    .filter((el) => isLedeP(el) && el.start > subH2)
    .map(at(bounds.start));
}
...
function splitEnvelope(openTag, inner, chrome, opts) {
  ...
  const parts = [coverSection(openTag, chrome, { ...cover, intro: introOf(recipe, count, firstMember) }, layoutName)];
  ...
  for (const [k, frag] of pages.entries()) {
    parts.push(`${bodyTag}${k === 0 ? frag : withCont(frag)}</section>`);
  }
