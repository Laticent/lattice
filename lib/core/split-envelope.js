...
/**
 * Decompose a `.cell-coda` into the two spans this envelope needs, WITHOUT leaving
 ...
 * So the cell is cut at the boundary BETWEEN them and each half is re-wrapped in a
 * cell of its own — each `outer` is a complete, well-formed coda cell ready to be
 * injected, and the two spans are CONTIGUOUS, so removing both takes the original
 * cell's open and close tags with them. A cell holding only one beat yields one
 * span covering the whole cell, unchanged.
 */
function codaSpans(cellEl, base) {
  ...
  // Both present. The cut point is the end of whichever comes first in the cell.
  const cut = start + openEnd + Math.max(insightKid.end, 0);
  return {
    insight: [{ start, end: cut, outer: wrap(body.slice(insightKid.start, insightKid.end)), name: 'div' }],
    note: [{ start: cut, end, outer: wrap(body.slice(noteKid.start, noteKid.end)), name: 'div' }],
  };
}
...
function insightPageFrom(openTag, inner) {
  ...
  const drop = topLevelElements(inner.slice(cell.start, cell.end))
    ...
    .filter((el) => !isChrome(el) && !keep.has(`${el.start}:${el.end}`));
