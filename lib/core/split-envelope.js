...
const { hasCodaClass, hasClassToken } = require('./coda');
...
const isTrailingNoteEl = (el) => el.name === 'p' || (el.name === 'div' && hasClassToken(el.outer, 'below-note'));
...
function trailingSlotMaterialOf(inner) {
  ...
  const note = [...found.note, ...found.insight]
    .filter((el) => el.name === 'blockquote' || (el.name === 'div' && hasClassToken(el.outer, 'below-note')))
