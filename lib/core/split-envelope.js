...
function injectTrailing(pageInner, noteHtml) {
  ...
  // No stage (a STAGE_DEFERRED / non-Form layout) — insert right before the TRAILING Marp
  // <footer>, found deterministically (lastIndexOf + a literal check that only whitespace
  // follows its close), not a `$`-anchored lazy regex: a lazy `[\s\S]*?` backtracks to the
  // EARLIEST `<footer` that can still reach end-of-string, so a page whose CONTENT happens
  // to mention a literal `<footer>` (however unlikely) would misplace the note inside that
  // content instead of after it (found by the second maker-checker pass).
  const footerAt = pageInner.lastIndexOf('<footer');
  if (footerAt >= 0) {
    const closeAt = pageInner.indexOf('</footer>', footerAt);
    const tailStart = closeAt >= 0 ? closeAt + '</footer>'.length : -1;
    if (tailStart >= 0 && /^\s*$/.test(pageInner.slice(tailStart))) {
      return pageInner.slice(0, footerAt) + noteHtml + pageInner.slice(footerAt);
    }
  }
...
function insightPage(openTag, inner, regions) {
  const insightTag = addClass(openTag.replace(/\s+id="[^"]*"/, ''), 'lat-split-insight');
