...
const TREATMENTS = Object.freeze({
  ...
  //
  // `progress` and `timeline-list` were BOTH enrolled on 2026-09-01 and both backed out the
  // same day, on the render. Their transforms rewrite the authored list into a custom DOM
  // (`.chart-body`, `.timeline-spine`), so `deriveAxis` resolves no `item` collection on the
  // rendered page and a declared `capacity.axis` can never fire — an inert contract, which is
  // the §8 rule 1 authoring/render mismatch seen from the other side (glossary authors a list
  // and renders a table; these author a list and render a diagram). Splitting either needs a
  // carousel STRATEGY that re-authors the transformed shape, the way `roadmap-horizons` does.
