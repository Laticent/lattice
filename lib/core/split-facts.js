...
const TREATMENTS = Object.freeze({
  ...
  //
  // `progress`, `timeline-list` and `journey` were ALL enrolled on 2026-09-01 and all backed
  // out on the render. Their transforms rewrite the authored list into a custom DOM
  // (`.chart-body`, `.timeline-spine`, `.journey-board`), so `deriveAxis` resolves no `item`
  // collection on the rendered page and a declared `capacity.axis` can never fire — an inert
  // contract, which is the §8 rule 1 authoring/render mismatch seen from the other side
  // (glossary authors a list and renders a table; these author a list and render a diagram).
  //
  // WHAT IS MISSING IS A READER, NOT A SEAM. Measured on a real render, each of them keeps its
  // members as clean repeated blocks in the rendered DOM — `.progress-row` (label · track ·
  // fill), `.timeline-item` (dot · pill · title · body), `.journey-vtask` nested inside
  // `.journey-vstage`. `axis` cannot see them because it reads the authored collection; a
  // carousel STRATEGY can, which is exactly what `kanban-lanes` and `roadmap-horizons` already
  // do for their own post-transform shapes. So the work is one reader per shape plus a recipe,
  // not a new mechanism. `split-compare` is the fourth of the same kind (2 × `.option`).
  // Contrast `matrix-grid` and `gantt`, which really are ONE figure over a shared axis: cutting
  // those leaves rows with nothing to read them against, and no reader fixes that.
  //
  // `journey` is the one that shows why "inert" is the optimistic reading. It was enrolled
  // under "split what can be split", backed out on a bucket rule, then RE-ENROLLED when that
  // bucket rule was corrected — and never re-rendered in between. On the render it did not
  // decline to split: it produced a MULTI-PAGE run in which every body page carried the whole
  // five-stage board, identical (an earlier version of this comment said SIX; the count depends
  // on the deck). It also blamed the section band labels colliding with the rows on the run —
  // that collision is `journey`'s OWN, pixel-identical on the unsplit page against origin/main
  // (`examples/split-structure.md` §"Reading a split run", before this back-out). A component
  // whose members the splitter cannot reach still gets a page count, a rail and a "next:"
  // pointer — the envelope is built from the member COUNT, which `countAxis` reads from the
  // authored markup that the transform has already discarded. So the failure is not a
  // no-op; it is a run of duplicates that every gate passed.
  // Recorded here so the next sweep does not re-add the axis and call it enrolled.
  progress: 'needs-call',
});
...
const TREATMENT_LABELS = Object.freeze({
  ...
  'needs-call': '**Needs an opt-in call**',
});
...
const TREATMENT_NOTES = Object.freeze({
  ...
  progress: 'CSS bars, not a viewBox graphic — so NOT "scale like a graphic"; list-heavy if enrolled, else whole-slide',
});
