...
const TREATMENTS = Object.freeze({
  // Anchor — never splits.
  title: 'anchor', closing: 'anchor', divider: 'anchor', topic: 'anchor',
  ...
  slope: 'graphic',
  'stacked-bar': 'graphic',
  waterfall: 'graphic', map: 'graphic', diagram: 'graphic', scene: 'graphic',
  'state-chart': 'graphic',
  flowchart: 'graphic',
  ...
  'big-number': 'atomic', quote: 'atomic', 'citation-card': 'atomic',
  // A matrix is one scale: intensity only reads against the grid's own min and
  // max, so half a grid on one page is a chart that lies about its own range.
  heatmap: 'graphic',
  contact: 'atomic', wifi: 'atomic', gantt: 'atomic',
  ...
  agenda: 'list-light', inventory: 'list-light',
  ...
  // team-profile: list-heavy, NOT atomic — and the near miss is worth recording,
  // because it looks like `logo-wall` (a wall of marks whose claim is all of them
  // at once) and is not. A logo carries nothing to read one at a time, so a single
  // mark on a page asserts an endorsement the author never wrote; a single PERSON
  // on a page reads exactly as authored — a name, a role, and the line about what
  // they own, which is a title AND a body, the literal list-heavy member. It also
  // matches `actors`, the component this one is a portrait of. The split only fires
  // at the presentation @sizes anyway, and at those the wall read is already gone
  // to geometry: twelve faces on a phone are thumbnails, where one per page is the
  // better read, not a worse one. A long roster that wants to stay ONE slide has an
  // in-component answer — the `bench` variant drops the notes and packs six up.
  'team-profile': 'list-heavy',
  ...
  // and `pricing` carried no capacity at all (they rang) and received theirs with the
  // §0c "opt-in backfill" follow-on, which is where their treatment is decided. All three
  // have since been decided and none of them stayed here — `journey` to read-across,
  // `timeline-list` to atomic, `pricing` to read-across (each recorded at its own entry).
  'list-steps': 'connected', cycle: 'connected', 'authority-chain': 'connected',
  'verdict-grid': 'connected',
  // pricing MOVED connected -> read-across (2026-09-06, owner's call): "pricing is no
  // different than compare components. we accept the splitting."
  //
  // It sits in the `comparison` bucket beside `table` and `split-compare`, and it is
  // one: three tiers set side by side so a reader can run a feature down all of them.
  // 'connected' said the tiers follow one another, which they do not — nothing about tier
  // 2 comes after tier 1. So the placement is corrected to what the slide actually is.
  //
  // The second half of the ruling is the part that matters, because read-across normally
  // ARGUES AGAINST splitting: cutting a comparison into one member per page costs the
  // cross-reading, which is the whole point of the form. That cost is ACCEPTED here.
  // Not splitting was measured and it is worse — at portrait the un-split slide showed
  // ONE of three tiers with its text cut mid-word (`Volume dis`, `$0.14 / recor`) and
  // ~65% of the canvas blank (2026-09-05-auto-split-catalog-audit). A comparison the
  // reader can page through beats a comparison two thirds of which is off the page.
  //
  // Read-across requires a STRATEGY rather than a bare axis (treatmentViolations below),
  // and the strategy is `cover-paginate` rather than table's `cover-cards`:
  // cover-cards exists to transpose a wide TABLE into cards, and pricing's tiers are
  // already cards in a top-level <ul>, so there is nothing to transpose. cover-paginate
  // opens on the shared accent cover and then flows the component's OWN tier cards, which
  // is ruling 1 (reuse the component's styling, refine the shared envelope) exactly.
  // `capacity.relationship: comparison` is what keeps the run readable as one comparison
  // — every page carries "Option N of M · comparing …" derived at build time.
  pricing: 'read-across',
  ...
  // obligation-matrix MOVED atomic -> read-across (2026-09-13, owner's call).
  //
  // It is table's shape — a wide read-across grid — and it gets table's reader,
  // `cover-cards`, for table's reason: a wide table cannot paginate out of HORIZONTAL
  // overflow, so the reshape turns each regime row into a card whose column headers become
  // in-card labels. Landscape renders the native grid untouched.
  //
  // It declares NO `capacity`. The first cut invented a 4/5/6 band and the gallery contract caught
  // it in one run: the component's own sample and its stress doc both hold SIX regimes, which its
  // stress summary calls "the grid ceiling", so a band that admits the sample also admits the
  // ceiling and tells an author nothing. `cover-cards` reads `split.perPage`; the advisory is a
  // separate decision, exactly as it is for `code`.
  //
  // TWO THINGS WERE MEASURED ON THE WAY, and both are worth carrying because both looked right.
  //
  // A ROW SLICE was tried first — `cover-paginate` on a `row` axis, which repeats the `<thead>`
  // per page and keeps every state mark in its own component's CSS. It took the gallery from 9
  // bad pages to 43: slicing rows does not narrow six columns. That is the emulator's own rule
  // (a vertical paginator never fixes a wide table) met from the wrong end.
  //
  // And the transpose SHIPPED THE MARKS AND NOT THEIR STYLING. `cover-cards` carries each cell's
  // HTML, so `<span class="state pass state-full">` reached the card intact — but every rule
  // that paints it is scoped `section.obligation-matrix td .state`, and a card cell is a `<dd>`.
  // A GDPR card rendered as Notice / Consent / Retention / Breach / DSAR with nothing beside any
  // of them: the marks were in the DOM, unstyled, and therefore zero-sized. No gate could see it
  // — every word was conserved. The fix is in the component's own stylesheet and is a WIDENED
  // selector rather than a second copy of the block (`:is(td, .ct-card dd) .state`), which is
  // the CSS being reused rather than re-authored.
  //
  // The `atomic` placement was the honest one while no reader existed. The measurement that
  // moved it: 8 of 10 pages of the component's own gallery clip at portrait, including the
  // default — so "keep it whole and ring" was in practice "show part of the grid and say
  // nothing". A transposed card carries one regime's five duties whole, which is the read the
  // grid gives a row anyway.
  'obligation-matrix': 'read-across',
  // matrix-2x2 MOVED atomic -> read-across (2026-09-13, owner's call), REVERSING the 2026-07-22
  // resolution. That resolution is worth quoting because it is still half true: "a 2x2 quadrant
  // read is destroyed by splitting BETWEEN quadrants (you would see 2 of 4 cells with no axis
  // structure)". It is the #1193 defect class, and an unadorned slice still is.
  //
  // Two things make the enrollment survivable, and neither existed when that resolution was
  // written. The quadrant's own label IS its position on both axes — `High impact · Low effort`
  // — so a page says where it sits without the grid being present; and the carousel adornment
  // (§0b) puts an `Option N of 4` rail on every page, which is what tells a reader there are
  // four and this is one of them. The cost is accepted rather than argued away: a paged 2x2 is a
  // worse read than a 2x2 that fits. It is a better read than the 4 of 7 gallery pages that
  // clipped at portrait, where the reader saw part of the grid and was told nothing.
  //
  // 'read-across' is the treatment that REQUIRES a strategy rather than permitting a bare axis
  // (treatmentViolations below), which is the guarantee wanted: the quadrants can only paginate
  // through `cover-paginate`'s envelope, never through a bare axis that would emit four
  // unrelated pages.
  'matrix-2x2': 'read-across',
  // Read-across → keep whole / carousel.
  'compare-prose': 'read-across', 'table': 'read-across',
  ...
  premise: 'read-across',
  // math MOVED atomic -> read-across (2026-09-08, #2136, owner's call: "we split on structure,
  // unique structure gets a split preprocessor").
  //
  // It was placed 'atomic' as a "single text unit" — one hero equation, ring on overflow — and
  // that reading is right for exactly one of its eight variants. The other seven surround the
  // equation with structure: a legend, a step table, a stack of Definition/Theorem/Proof cards,
  // two labeled columns. Measured off the committed manifest samples rendered at `hd`, the eight
  // variants produce FOUR rendered structures, which is why the enrollment is a dispatch and not
  // a variant list (lib/core/carousel.js `splitMathStructures`).
  //
  // WHAT ATOMIC COST. The Fit Spine's ladder is collapse -> shed -> split -> NEVER scale, and
  // `atomic` + NEVER_SPLIT deleted the third rung — so #2129's portrait work reached for the
  // fourth at every turn (1.15em on the derivation table, 1.55em on decompose's equation,
  // tighter card padding on theorem), buying fit with legibility each time. On the `derivation`
  // sample at portrait every proof step wrapped mid-expression with no continuation indent. One
  // step per page sets each equation on one line at full size.
  //
  // 'read-across' RATHER THAN A LIST TREATMENT, for roadmap's and journey's reason: it is the
  // treatment that REQUIRES a strategy instead of permitting a bare axis (treatmentViolations
  // below), and that is precisely the guarantee wanted here. Two of math's four structures —
  // `stats`'s estimate/CI/reading scaffold and `canvas`'s equation-beside-plot — have no
  // repeating collection at all, and a bare `item` axis would let a later change paginate them
  // off the first `<ul>` it found. The strategy returns null for both, so they keep ringing.
  // It is also true of the content: an equation and its legend ARE read across, which is why
  // the equation repeats on every legend page rather than sitting on the cover alone.
  //
  // NO `capacity.relationship`, deliberately — which means math takes the DEFAULT, not silence.
  // The carousel signal is universal since 2026-09-01 (`applyRelationshipSignals`): every split
  // run points the reader at what is coming, and `relationship` only chooses the PHRASING, with
  // `sequence` ("-> the next page's first member") as the default. That default is the right one
  // here and no declared kind is: the four structures disagree about what the relationship IS —
  // a proof chain is a sequence, a symbol legend is a set — and `relationship` is a
  // COMPONENT-level fact with no per-structure form. `comparison` would print "Option 3 of 5"
  // over a legend; `cycle` and `hierarchy` are plainly wrong. So the honest declaration is none,
  // and every math run reads as what a split run always is: pages that were one slide.
  math: 'read-across',
...
const TREATMENT_LABELS = Object.freeze({
  ...
  code: '**Code → code-cards** (one `<pre>` per page when there are several; line-runs when a single block is over budget — BUILT 2026-09-13)',
...
const TREATMENT_NOTES = Object.freeze({
  ...
  flowchart: 'JS-scaled; no-JS UNVERIFIED',
  'logo-wall': 'by image',
  stats: 'tile — watch',
  'list-steps': '→next',
  cycle: '↻loop',
  ...
  'obligation-matrix': "table's shape and table's reader — the grid reshapes to one regime card per page at portrait, and renders natively at landscape",
  'matrix-2x2': 'ENROLLED 2026-09-13, reversing the 2026-07-22 atomic resolution — the quadrant label carries its own position on both axes, and the `Option N of 4` rail carries the rest',
  ...
  math: 'splits on STRUCTURE, not on variant — eight variants, four rendered structures; the `stats` and `canvas` scaffolds have no collection and ring',
...
/**
 * Strategies whose cover is the shared accent field, not a per-layout `<name>-cover`.
 *
 * `math-structures` is here for the reason `compare-options` was added: it calls `coverSection`
 * on every arm — the two paginated ones through `cover-paginate`, the two bespoke ones directly
 * — so deriving `math-cover` would publish a class that exists nowhere in the tree. Nothing
 * compares this field against emitted markup, so a wrong entry is a green oracle carrying a
 * false fact, in the record whose whole job is pinning derived facts against drift.
 *
 * `code-cards` is here on the same reading, reached independently on a parallel branch: both
 * of its seams call `coverSection` rather than minting a `code-cover`.
 */
const SHARED_COVER_STRATEGIES = new Set(['cover-paginate', 'cover-cards', 'compare-options', 'math-structures', 'code-cards']);
...
function splitFactsFor(m) {
  ...
  return {
    ...
    // Which strategies open on the SHARED accent cover (`coverSection`, split-envelope.js)
    // rather than minting a per-layout one. `compare-options` belongs here and was missed:
    // the derivation published `split-compare-cover`, a class that exists nowhere in the tree
    // except in the comment in carousel.js explaining why the strategy does NOT hand-roll one.
    // Nothing compares this field against emitted markup, so the oracle was green on a false
    // fact — in the record whose whole job (rule 5) is pinning derived facts against drift.
    // Found by the HARD RULE #25 checker.
    coverClass: !split || SHARED_COVER_STRATEGIES.has(split.strategy)
