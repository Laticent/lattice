...
// THE RULE — the repo's single statement of it. Given a slide's class string and
// the deck's running proof counter, return the tokens to ADD (`[]` for a slide the
// sequencer doesn't touch). Both consumers go through it: this module's streaming
// HTML pass (`sequenceProofPanels`) and the browser runtime's DOM walk
// (`lib/transformers/split-panels.js`), which hands over `sec.className` and
// applies the result to a classList. Only the traversal differs.
//
// It was extracted because the DOM walk carried a hand-kept SECOND copy of the
// logic, and every comment describing that arrangement was wrong: the kernel side
// said nothing, while the unit test claimed the walk was "covered alongside the
// other DOM parity cases" — it was not covered at all. Deleting the walk's entire
// sequencing loop broke nothing in a 4,700-test suite. See
// test/unit/transformers/split-panels-dom.test.js § "proof sequencing", which now
// includes a real kernel-vs-DOM parity case.
//
// One deliberate behavior change came with the unification: `HAS_CAT_RE` matches a
// `cat-N` anywhere in the string with non-word delimiters, where the DOM walk's old
// per-token `^cat-[1-8]$` test did not. A pathological class like `cat-1.5` now
// counts as pinned on BOTH paths instead of only the HTML one — the two paths
// agreeing is the point, and neither shape is reachable from `_class:` authoring.
function proofTokensFor(cls, seq) {
  const tokens = String(cls ?? '').trim().split(/\s+/);
  if (!tokens.includes('split-panel')) return [];
  const isCapstone = tokens.includes('capstone');
  if (!isCapstone && !tokens.includes('proof')) return [];
...
function sequenceProofPanels(openTag, cls, seq) {
  const add = proofTokensFor(cls, seq);
...
module.exports = {
  ...
  PANEL_VARIANTS,
  applyToRenderedHtml,
  // THE RULE, for the DOM walk in lib/transformers/split-panels.js. It traverses
  // classLists rather than class strings, so it used to carry its own hand-kept
  // copy — with, contrary to what the unit test claimed, no coverage holding it to
  // this one. Calling through leaves one statement of the rule (HARD RULE #1).
  proofTokensFor,
