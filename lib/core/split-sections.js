 ...
 *     rewrites pieces back into a document, and inventing a close tag for a
 *     malformed one would move an author's bytes.
...
function findClassAttr(openTag) {
  ...
  let i = t.indexOf('<') + 1;
  if (t[i] === '/') i += 1;
  while (i < t.length && !SPACE.test(t[i]) && t[i] !== '>' && t[i] !== '/') i += 1;  // tag name

  while (i < t.length) {
    while (i < t.length && (SPACE.test(t[i]) || t[i] === '/')) i += 1;
    if (i >= t.length || t[i] === '>') return null;
    const start = i;
    while (i < t.length && !SPACE.test(t[i]) && t[i] !== '=' && t[i] !== '>' && t[i] !== '/') i += 1;
    const name = t.slice(start, i).toLowerCase();
    while (i < t.length && SPACE.test(t[i])) i += 1;
...
function withClass(openTag, next) {
  ...
  let i = lt + 1;
  while (i < openTag.length && !SPACE.test(openTag[i]) && openTag[i] !== '>' && openTag[i] !== '/') i += 1;
  return `${openTag.slice(0, i)} class="${next}"${openTag.slice(i)}`;
}

function splitSections(html) {
  // NO `String(html ?? '')`. The old walk read `html.length` directly, so a
  // caller passing undefined, a Buffer or an object got a loud `TypeError`;
  // coercing here would turn that into a silent empty answer — a wrong render
  // instead of a crash, which is the exact trade `lib/engine/index.js` refuses
  // at its own door and for the same reason. A caller that wants leniency
  // coerces at ITS call site, as `applyDeckLogoToHtml` does.
  const src = html;
  const pieces = [];
  ...
  for (const t of scanTags(src)) {
    // `kind: 'inert'` — a comment, doctype or RAWTEXT span — is skipped here,
    // which is the whole point: it cannot open or close a section.
