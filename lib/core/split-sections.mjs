...
import { scanTags } from './top-level-h2.mjs';
...
// ── THE WALK IS MEMOIZED ON THE STRING, FOR THE LAST FEW DOCUMENTS ───────────
//
// A render hands the SAME document to this walk again and again: every pass that
// maps sections and changes nothing returns its input, and the next pass splits it
// once more. Measured on the bench decks: the jargon gallery made 22 calls over 13
// distinct strings, the chart gallery 16 over 6. The fast path above still scans the
// whole document for `DIRTY` (~260 MB/s, so ~10 ms of every chart-deck render), and
// a repeat paid that again for an answer it already had.
//
// Only the section TAG OFFSETS are kept — `{ isClose, start, end }`, a pure function
// of the string — never the pieces, which callers own and may mutate. The key is the
// string itself, so a hit is an identity or content comparison and a miss can never
// return another document's tags. Four entries covers the repeats a render makes
// (they are adjacent passes) and holds at most four documents alive between renders.
const WALK_MEMO_SIZE = 4;
const walkMemo = [];

function sectionTagsOf(src) {
  for (let k = 0; k < walkMemo.length; k++) {
    if (walkMemo[k].src === src) return walkMemo[k].tags;
  }
  const tags = [];
  walkFast(src, (t) => { tags.push({ isClose: t.isClose, start: t.start, end: t.end }); });
  walkMemo.unshift({ src, tags });
  if (walkMemo.length > WALK_MEMO_SIZE) walkMemo.pop();
  return tags;
}
...
function splitSections(html, { tokenizerOnly = false } = {}) {
  ...
  if (typeof src !== 'string' || tokenizerOnly) {
    ...
    for (const t of sectionTagsOf(src)) onSection(t);
  }
...
/**
 * The offset of the first TOP-LEVEL `<section` that never closes, or -1.
 *
 * `splitSections` surrenders such a section, and everything after it, to the trailing gap —
 * the right call for a caller that rewrites pieces back into a document. A caller that has to
 * line its pieces up with a browser's DOM needs to know it happened: a browser auto-closes the
 * section at end of file, so the DOM holds one more top-level section than the walk returned,
 * and every later slide sits inside it. An author mid-way through typing `<section class="x">`
 * is in exactly this state. Read by the same tokenizer, from the start of the trailing gap.
 */
function unclosedSectionAt(html, pieces = splitSections(html)) {
  const last = pieces[pieces.length - 1];
  if (!last || last.type !== 'gap') return -1;
  const from = html.length - last.text.length;
  for (const t of scanTags(html, from)) {
    if (t.kind === 'tag' && t.name === 'section' && !t.isClose) return t.start;
  }
  return -1;
}

export { findClassAttr, splitSections, unclosedSectionAt, withClass };
