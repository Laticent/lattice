 ...
 *
 * WHAT THIS NO LONGER FEEDS. Until 2026-09-01 `lib/core/auto-split.js` exposed a
 * measurement-FED `resplitDoc`, which took `{ slide, ratio, canSplit, splitRatio }` per
 * overflowing slide and did the cutting. That function is gone: the split is STRUCTURAL and
 * decides from the markup, so no verdict reaches it. What survives here feeds the overflow
 * RING — the marker on a page that still does not fit at one element per page — and the
 * `lint:deck` advisories. The measurement itself is two steps, and only the first was ever
 ...
 * because this function is `.toString()`-injected into the emulator's
 * `measureOverflow` `page.evaluate`. `SPLIT_VERDICT_SRC` is that injection form,
 * and today that is its ONE use site — the emulator's other, inline watcher
 * injects `PROBE_SRC`/`LEGIBILITY_SRC` only, because it answers "does this
 * overflow" rather than "should it be split". The runtime becomes the second use
 * site when Option B is built.
 *
...
function buildSplitVerdict(s, deps, opts) {
  ...
  // it). Mark it splittable and let the carousel branch own it (the ratio is
  ...
  // for the ring (this is the guard that lets a wide table / obligation-matrix
  ...
  // wins where it is tighter (the ring takes the tighter of the two), so a
