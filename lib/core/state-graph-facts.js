...
function summarizeGraph(parsed) {
  ...
  return {
    ...
    /**
     * Is this a straight chain — every state one way out, no branch, no loop
     * back? A TOPOLOGY claim and never a layout one; see the module docblock.
     */
    isChain:
      states.length > 1 &&
      states.filter((s) => (outForward.get(s.index) || 0) > 1).length === 0 &&
      edges.filter((e) => !e.isSelf && e.to < e.from).length === 0,
