...
function summarizeGraph(parsed) {
  ...
  // INCLUDING self-loops, because "nothing leads to X" is a claim about ARROWS and
  // a self-loop is one the picture draws. `incoming` excludes them (it answers "is
  // this state entered from elsewhere", which `isChain` wants); this answers "does
  // anything point at it at all". Reading the first as the second put a state whose
  // only inbound arrow is its own loop into the in-degree sentence — "Orphan loops
  // on itself … nothing leads to Orphan" — the same self-contradiction the
  // `unreachable`/`cutOff` split was made to remove, one door over.
  const incomingAll = new Map();
  for (const e of edges) {
    outAll.set(e.from, (outAll.get(e.from) || 0) + 1);
    incomingAll.set(e.to, (incomingAll.get(e.to) || 0) + 1);
    if (!e.isSelf) {
  ...
  return {
     ...
     *
     * IT IS SPLIT IN TWO BECAUSE THE TWO NEED DIFFERENT WORDS, and a checker caught
     * the first cut shipping the wider set under the narrower sentence. "Nothing
     * leads to X" is an IN-DEGREE claim, and on an island the picture draws the
     * arrow that refutes it — the narrator asserted nothing leads to Alpha and then
     * read "from Beta, y goes to Alpha" three clauses later, which is the same
     * self-contradiction the endpoint fix removed, moved one clause over.
     */
    unreachable: states.filter(
      (s) => s.index !== start?.index && !reachable.has(s.index) && !(incomingAll.get(s.index) > 0),
    ),

    /**
     * A state something DOES point at that the machine still cannot get to — an
     * island. Separate from `unreachable` so a caller can say "the machine never
     * gets to X" rather than "nothing leads to X", which its own edges deny.
     */
    cutOff: states.filter(
      (s) => s.index !== start?.index && !reachable.has(s.index) && incomingAll.get(s.index) > 0,
    ),
    ...
    isChain: (() => {
      ...
      // Dropping EITHER one changes nothing across every machine up to three states
      // with every choice of START — 4200 edge-set/order/start combinations, so a
      // mutation test on either finds nothing and reports the line as dead. Dropping
      // BOTH changes 16 of them. (An earlier revision of this comment quoted 132132
      // and 120, from a four-state sweep that hard-coded `states[0]` as the start —
      // a space the test itself now records as a slice rather than the whole. The
      // conclusions are the same in both; the numbers are not, and the ones here are
      // the ones the committed test pins.)
