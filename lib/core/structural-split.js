...
const { applyToRenderedHtml: liftMastheads } = require('../forms/cell/masthead/masthead.transform');
...
function structuralSplit(docHtml, { deckSource = '', width, height, capacity, firstSlide = 1, settle = false } = {}) {
  ...
  return { html: settle && r.changed ? liftMastheads(adorned) : adorned, changed: r.changed || 0, applies: true };
}
