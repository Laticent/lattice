...
const masthead = require('../forms/cell/masthead/masthead.transform');
...
// Run the masthead lift on the pages the split EMITTED, exactly where the live runtime's DOM
// mirror (lib/transformers/masthead-lift.js) would run it after first paint, and nowhere else.
// The mirror lifts a `.form` section only when it has a TOP-LEVEL h2 or its component wraps its
// stage; the string kernel alone is depth-blind for a sovereign component and would pull a
// nested title (split-compare's `.compare-left > h2`) into a band the export never draws. So
// that gate is applied here first, and a page outside it passes through untouched. Unsplit
// slides are never touched: the engine already lifted them at render time.
function settleSplitPages(docHtml) {
  return splitSections(docHtml)
    .map((p) => {
      if (p.type !== 'section') return p.text;
      const whole = `${p.openTag}${p.inner}${p.close}`;
      if (!/\sdata-split-role=/.test(p.openTag)) return whole;
      const cls = (p.openTag.match(/\sclass="([^"]*)"/) || [])[1] || '';
      if (!masthead.wrapsStageBody(cls) && !masthead.extractH2(p.inner, true).el) return whole;
      return masthead.applyToRenderedHtml(whole);
    })
    .join('');
}
 ...
 * @param {boolean} [o.settle=false] a LIVE preview passes true. The split composes its cover and
 *   closing pages AFTER the engine's render-time masthead lift ran, so they arrive unlifted, and the
 *   live runtime's DOM mirror (lib/transformers/masthead-lift.js) wraps them in `.cell-stage` about
 *   160 ms after first paint: the cover visibly jumps. `settle` runs the same lift, with the
 *   render-time string kernel the mirror copies, before the frame paints; the mirror then finds
 *   `.cell-stage` and leaves the page alone, and already-lifted pages pass through untouched. The
 *   CLI leaves it false: its export is captured after the runtime settles, so it never shows the
 *   jump, and its exported HTML stays byte-identical.
 ...
 */
function structuralSplit(docHtml, { deckSource = '', width, height, capacity, firstSlide = 1, settle = false } = {}) {
  ...
  const adorned = railRun(split, deckSource, capacity);
  return { html: settle && r.changed ? settleSplitPages(adorned) : adorned, changed: r.changed || 0, applies: true };
}
