 ...
 * @returns {{ html: string, changed: number }} `changed` is the count of authored slides split.
 */
function structuralSplit(docHtml, { deckSource = '', width, height, capacity } = {}) {
  const stamped = stampSlideNumbers(docHtml);
  if (!capacity || !Object.keys(capacity).length || !splitApplies(width, height)) {
    return { html: stamped, changed: 0 };
  }
  const r = splitDoc(stamped, capacity);
  const split = r.changed ? r.html : stamped;
  return { html: railRun(split, deckSource, capacity), changed: r.changed || 0 };
}
