 ...
 *
 * MEASURED IN ALL THREE ENGINE FAMILIES — Blink, Gecko and WebKit — because `file://` origin
 * rules are engine-specific and a Chromium-only measurement would not carry. WebKit is the
 * strict one and therefore the one that matters: it enforces `img-src` AND `font-src` on a
 * same-document `file://` load, and under this policy it still renders the deck's own image and
 * lays the math out in the real KaTeX faces. Gecko enforces neither on `file://` subresources,
 * so its run confirms the remote fetch is refused and nothing about the cost.
 * Pinned by `docs/e2e/export-subresource-engines.spec.ts` on the `gecko` and `webkit-tablet`
 * projects. Still UNVERIFIED on Safari ITSELF (HARD RULE #23) — Playwright's WebKit is a WebKit
 * build, not the shipping browser.
 *
 ...
 * @param {Iterable<string>} [opts.webOrigins] web origins the READER chose to load images and
 *   media from ("load them?", trio follow-up 11). Only these join `img-src` and `media-src`;
 *   anything not shaped exactly like an origin is dropped. Fonts, connections and the rest
 *   stay closed: the choice is about pictures.
 * @returns {string}
 */
// A web origin exactly as `new URL(…).origin` writes it, and nothing else: a value with a
// space, a `;` or a quote would add a source or a whole directive to the policy.
// An underscore is legal in a host name the URL parser accepts (`my_host.example`), and CSP
// source expressions allow it; dropping it would leave "Load them" saying loaded while blocked.
const ORIGIN = /^https?:\/\/(?:[a-z0-9_-]+(?:\.[a-z0-9_-]+)*|\[[0-9a-f:.]+\])(?::\d{1,5})?$/;

/** The allowed web origins that are well-formed, deduplicated, as a policy suffix (` a b`). */
function webSources(webOrigins) {
	const ok = [...new Set([...(webOrigins || [])].map(String))].filter((o) => ORIGIN.test(o));
	return ok.length ? ` ${ok.join(' ')}` : '';
}

export function subresourceCspPolicy({ katexUrl = '', webOrigins = [] } = {}) {
	const web = webSources(webOrigins);
	let fontOrigin = '';
	try {
	...
	return [
		`img-src ${SELF_SOURCES}${web}`,
		`media-src ${SELF_SOURCES}${web}`,
