...
function uniquePrefix(html) {
  ...
  //
  // THE LOOP MUST VALIDATE ITS OWN RESULT. The first version tested the PREVIOUS prefix
  // and bailed at a fixed count, so the last candidate it assigned was returned WITHOUT
  // ever being checked: seed 32 decoy tokens as ordinary text and squat the 33rd, and the
  // squat wins — the exact attack this function exists to stop, demonstrated on a real
  // Chrome accessibility tree (the chart announced the attacker's string). Now the guard
  // bounds the search and the condition is the invariant, so an exhausted search returns
  // a prefix that is genuinely free or nothing at all.
  let prefix = 'lat-svg';
  for (let guard = 0; html.includes(prefix); guard += 1) {
