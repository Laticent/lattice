...
function uniquePrefix(html) {
  ...
  // Test THREE spaces, and the third is the one that was missing. `ID_ATTR` above requires the value
  // to be QUOTED, so an UNQUOTED attribute with numeric character references —
  // `<span id=lat&#x2d;svgt&#x2d;2&#x2d;1 hidden>` — is invisible to the decoded id space, and
  // carries no literal `lat-svg` for the raw-text test either. It parses to exactly the id about to
  // be minted and wins by being first in tree order. Demonstrated on a real Chrome accessibility
  // tree: the chart announced the attacker's string with a pixel-identical render.
  //
  // The fix is the shape `render-ids.js::safePrefix` already uses — decode the WHOLE text, not only
  // the values a quote-aware scan could find. That needs no idea of where an id may appear, which is
  // the property the two earlier breaks of this guard both lacked.
  const decodedText = html.includes('&') ? decodeNumericRefs(html) : html;
  const taken = (candidate) => html.includes(candidate) || ids.includes(candidate) || decodedText.includes(candidate);
  let prefix = 'lat-svg';
...
/**
 * Rewrite every `role="img"` chart `<svg>` in `html` to reference its own `<title>`/`<desc>` by id.
 * Returns the html unchanged when there is nothing to do.
 *
 * SCOPED PER SLIDE, not per document. `slideOffset` is the caller-supplied `page.offset` — where
 * this document's first section sits in the real deck — so ids read `lat-svgt-<slide>-<n>` with
 * `<n>` counting within that slide. Without it the counter ran from the start of whatever document
 * was handed over, and the SAME chart was `lat-svgt-1` previewed alone and `lat-svgt-3` inside its
 * deck. Nothing announced wrongly (each document wires itself correctly), but a slice render and an
 * export render of one slide were never the same bytes.
 *
 * A count cannot be handed in — a lone slice would have to know how many named graphics the earlier
 * slides produced, which only rendering them reveals — but the slide's POSITION is metadata the
 * caller already holds, and scoping by it gives the same result. Uniqueness within the document is
 * preserved by construction: slide number × per-slide ordinal.
 *
 * The prefix is still probed against the WHOLE document (an author's squatting `id` can live on any
 * slide), so `uniquePrefix` is unchanged and runs once.
 */
function applyToHtml(html, slideOffset) {
  ...
  // No sections at all — a fragment, or a unit test. There is no slide to scope by, so number from
  // the document start exactly as before rather than inventing a position.
  if (html.indexOf('<section') === -1) return nameSvgsIn(html, prefix, null);
  const base = Number.isInteger(slideOffset) && slideOffset > 0 ? slideOffset : 0;
  let sectionIndex = -1;
  const walked = mapSections(html, (_openTag, _cls, inner) => {
    sectionIndex += 1;
    if (inner.indexOf('<svg') === -1) return null;
    const named = nameSvgsIn(inner, prefix, base + sectionIndex + 1);
    return named === inner ? null : named;
  });
  // AND THE GAPS. `mapSections` passes everything between and around the sections through
  // untouched — the `<article>` wrapper, a deck tail, anything after the last `</section>`.
  // Walking only the sections silently NARROWED this pass: an `<svg role="img">` outside a section
  // stopped being named at all, which is exactly the unnamed-graphic defect this module exists to
  // prevent. No committed deck has one, so no test caught it and the `indexOf('<svg')` guard above
  // read as an optimization rather than a scope decision. Those graphics have no slide to be scoped
  // by, so they keep the bare document-start ordinal — the same answer, for the same reason, as the
  // section-less branch above.
  return nameSvgsOutsideSections(walked, prefix);
}

/** Name any `<svg>` OUTSIDE every top-level `<section>`, leaving the sections themselves alone. */
function nameSvgsOutsideSections(html, prefix) {
  const spans = sectionSpans(html);
  if (!spans.length) return html;
  let out = '';
  let cursor = 0;
  let changed = false;
  // ONE counter across every gap. Each gap is a separate call, so a per-call counter restarted at 1
  // and two out-of-section graphics both took `lat-svgt-1` — a duplicate id, which is the single
  // failure this whole module exists to prevent. Caught by asserting uniqueness rather than by
  // reading the code.
  const counter = { n: 0 };
  const nameGap = (gap) => {
    if (gap.indexOf('<svg') === -1) return gap;
    const named = nameSvgsIn(gap, prefix, null, counter);
    if (named !== gap) changed = true;
    return named;
  };
  for (const span of spans) {
    out += nameGap(html.slice(cursor, span.start)) + html.slice(span.start, span.end);
    cursor = span.end;
  }
  out += nameGap(html.slice(cursor));
  return changed ? out : html;
}

/** Top-level `<section>` spans — the same walk `mapSections` does, because it IS that walk
 * (`splitSections`, lib/core/split-sections.js). A private `indexOf('<section')` copy lived here
 * and stopped at a `<section` quoted in a comment or in `<style>` text, so the two disagreed about
 * where the sections were on exactly the decks that quote one. */
...
/**
 * The naming loop itself, over one section's markup (or a whole section-less document).
 * `slide` null → the ids keep the bare `<n>` ordinal; a number → `<slide>-<n>`.
 */
function nameSvgsIn(html, prefix, slide, counter) {
  let out = '';
  let cursor = 0;
  let n = counter ? counter.n : 0;
  const seq = () => (slide === null ? String(n) : `${slide}-${n}`);
  ...
  while ((open = SVG_OPEN.exec(html))) {
    ...
    // string-level assertion sees it. Caught by a static analyzer flagging the dead
    ...
    const did = d && !d.hasId ? DESC_ID(prefix, seq()) : null;
  ...
  const used = counter ? n - counter.n : n;
  if (counter) counter.n = n;
  return used === 0 ? html : out + html.slice(cursor);
}
