...
function applyToHtml(html) {
  ...
  while ((open = SVG_OPEN.exec(html))) {
    ...
    // the real node rather than a duplicate — but ONLY if it is a well-formed id token.
    // This value is reflected into an attribute we emit, so anything else is both a
    // broken reference target and an injection surface (CodeQL: incomplete HTML
    // attribute sanitization). A malformed id is simply not reused; we mint our own.
    const SAFE_ID = /^[A-Za-z][\w:.-]*$/;
    const existingId = (tag) => {
      const v = tag.match(/\sid\s*=\s*["']([^"']*)["']/)?.[1];
