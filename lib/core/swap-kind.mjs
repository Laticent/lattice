...
export function deckContextKey(deckSource, slideIndex, deckId) {
	...
	// ONE MEMBER OF THAT CLASS SURVIVES: a leading BOM defeats the `^---` anchor, so a
	// BOM'd deck still folds its front matter into slide 0. It is not reachable through the
	// Studio — every ingest door runs `normalizeSourceText`, which strips the BOM before
	// this sees it — and it is named here rather than fixed because the fix belongs at the
	// door, not in a comparison function that would then disagree with every other reader.
	...
	// The shown slide is blanked with a TOKEN, not dropped: the key still records WHERE it
	// sits among the others, so moving it is visible even though its own text is excluded.
	// Drop the token and a deck of two identical slides keys the same at either index —
	// which is also what makes `swapKindForSlide`'s index check redundant, so the two edits
	// are a footgun in series. Both are pinned in test/unit/core/swap-kind.test.js.
