 ...
 * caller needs.
 *
 * THE GAP THAT SECOND CALLER WOULD CLOSE is real and open: markdown-it hands a
 * table an author wrote as RAW HTML through as an opaque `html_block`, so its rows
 * never enter the token stream the walker in
 * lib/integrations/markdown-it/plugins.js reads, and it goes unstamped on the
 * engine path. `compare-table` styled `td:first-child` unconditionally in
 * component CSS and caught raw HTML for free, so this is a REGRESSION against it,
 * measured on a two-slide probe through the real emulator.
 *
