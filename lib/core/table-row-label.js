 ...
 * `lib/components/` at the time of writing — 224 of them, of which 202 carry a
 * named label header, 22 an empty header cell over a label column, and ZERO
 * match any OFF shape. Widen the scope (to all of `lib/`, say) and the totals
 * move, because prose docs carry tables too; the corpus test in
 * test/unit/core/table-row-label.test.js walks exactly these four roots, which
 * is what makes the number re-derivable rather than folklore. So this reproduces the previous
