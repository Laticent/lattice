 ...
 * `lib/components/` — 242 of them, of which 216 carry a named label header, 22 an
 * empty header cell over a label column, and 4 resolve OFF. Do not retype these
 ...
 *
 * THE FOUR OFF CASES ARE DELIBERATE — two demonstration slides each in
 * examples/universal-table.md and examples/table-component.md, where a year
 * column shows the rule declining and `row-label` forces it back on over the same
 * data. They are the corpus's only
...
/** A cell that is only a state marker, nothing else. Built from the kernel's marker class
 *  (lib/core/state-marks.js). Case-insensitive on purpose: a `[X]` cell is still a status
 *  tick for the row-label bet, even though it does not draw a mark. */
const MARKER_CELL = new RegExp(`^\\[${require('./state-marks').MARKER_CLASS}\\]$`, 'i');
