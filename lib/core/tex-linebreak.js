 ...
 *   | …plus the `+` inside the bracket       |    1743px |               771  |
 *   | …plus the `\sum` prefix on its own line|    1397px |               425  |
 *
 ...
 * author's `+` actually lives. #2136's own table stopped one break short and concluded
 * line-breaking hits a floor at 1694px; it does not, it reaches 1397px.
 *
 * WHAT IT DOES NOT DO. It does not close the gap alone — 1397px still exceeds 972px, and the
 * remaining 425px is the tall families' 2.4em display scale-UP. That is the second lever
 ...
 *
 * Calibrated against the corpus rather than guessed. Every display equation in every
 * committed deck was measured for source length and rendered width; the shipped equations
 * that FIT their tall-family box top out at 60 characters (`math matrix`'s pmatrix is longer
 * but carries `\\`, so it is refused before this number is consulted), and the one that does
 * not fit is 122. 80 sits between them with room on both sides: a 20-character margin over
 * the widest fitting equation, so an author's slightly-longer-but-fine expression is still
 * left alone, and a 42-character margin under the one this exists for.
 */
 ...
 * 1397px. 0.75 sits between them, nearer the useless break than the useful one, because the
...
/** The closing delimiter token of a `\left…\right` group, e.g. `]` from `\right]`. */
function closerOf(src, g) {
  const at = src.lastIndexOf('\\right', g.end);
  return src.slice(at + '\\right'.length, delimEnd(src, at + '\\right'.length)).trim();
}
...
function descend(body, groups, relPt) {
  ...
  return [
    { op: null, text: relPt ? body.slice(0, relPt.at).trim() : '' },
    { op: relPt ? relPt.sym : '+', text: `${head} ${open} ${pieces[0].text}`.trim() },
    ...pieces.slice(1).map((p, k) => ({
      op: p.op,
...
function reflowDisplayTex(src, opts = {}) {
  ...
  // The author already chose a multi-line layout — an explicit `\\`, or any environment
  // (`aligned`, `cases`, `pmatrix`, `array`). Refuse: this pass has nothing to add and
