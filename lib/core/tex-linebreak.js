...
function descend(body, groups, relPt) {
  ...
  const g = cands[0];
  // A PLAIN `{…}` IS NOT A DELIMITER GROUP, and re-wrapping it as one emits `\Bigl{`, which KaTeX
  // rejects outright. `\frac{…}{…}` and `\sqrt{…}` put their arguments in exactly such a group at
  // depth 0, so when the operators live only inside it this was the ONLY candidate — and the pass
  // then produced TeX that failed to parse, `displayBlock` fell back to the author's source, and
  // the equation shipped on one line and CLIPPED. The Fit Spine's collapse rung did nothing for the
  // commonest wide-equation shape there is, silently. A brace group cannot span a `\\` row inside
  // `aligned` anyway, so refusing it is a refusal the design already owed.
  // (HARD RULE #25 red team, measured on a real render: 3 slides clipped, no `data-math-reflow`.)
  if (!g.left && g.open === '{') return null;
