...
// `src.slice(lt, lt + 64)` at every `<` in the document just to anchor it, which
// is one allocation per candidate tag on a hot path shared by every render.
 ...
 *
 * Without this the walk read `<ul><li>Cost<li><strong>Payback</strong></ul>` as
 * ONE item whose text is "CostPayback", so `markAuthored` found nothing wholly
 * bold and nothing matching the heading and marked nothing, while the DOM arm
 * saw two children and marked the second. markdown-it always writes `</li>`, so
 * this takes author raw HTML to reach — but it reaches, and it is a HARD RULE #1
 * split when it does.
 */
