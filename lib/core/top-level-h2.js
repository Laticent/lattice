...
// The characters that can end an open tag or open a quoted value inside it.
const ATTR_SIG = /[>"']/g;
// One close-tag search per RAWTEXT element, reused (`lastIndex` is set per use).
const RAW_TEXT_CLOSE = Object.fromEntries([...RAW_TEXT_TAGS].map((t) => [t, new RegExp(`</${t}\\b`, 'gi')]));
 ...
 *
 * `from` starts the walk at that offset, which must be a point outside any tag
 * or inert span (the start of the document, or the end of a tag this scan
 * yielded). `splitSections` uses it to tokenize only the stretches its literal
 * fast path cannot prove clean.
 *
...
function* scanTags(src, from = 0) {
  ...
  while (i < n) {
    ...
    //
    // ONLY WHEN IT ACTUALLY CLOSES — the rule the RAWTEXT branch below states for
    // an unclosed `<style>`, for the same reason. A browser reads an unterminated
    // `<!--` as a comment to EOF, and matching that here dropped every later
    // slide from the export: one `<div><!-- oops</div>` in a raw HTML block
    // exported a four-slide deck as one page, exit code 0. An unterminated
    // comment is read as text instead, so the walk goes on past it.
    //
    // `<!-->` and `<!--->` are complete, EMPTY comments to a browser. Searching for `-->` from
    // after the opener read them as a comment running to the next `-->` in the document, which
    // swallowed every slide in between (measured: 1 of 3 slides stamped).
    if (src.startsWith('<!--', lt)) {
      let stop;
      if (src.startsWith('>', lt + 4)) stop = lt + 5;
      else if (src.startsWith('->', lt + 4)) stop = lt + 6;
      else {
    ...
    // Unterminated, it is text, as above.
    if (src.startsWith('<!', lt) || src.startsWith('<?', lt)) {
      const end = src.indexOf('>', lt + 2);
      if (end < 0) { i = lt + 2; continue; }
      yield { kind: 'inert', start: lt, end: end + 1 };
      i = end + 1;
      continue;
    }
    ...
    // did the same.
    //
    // JUMPS rather than steps. The only characters that change the answer are
    // `>`, `"` and `'`, so a native regex finds the next one and the JS loop
    // runs once per such character instead of once per byte. This scan runs
    // over the whole document on every `mapSections` call (about 19 per deck
    // render), and the per-byte loop cost ~30 ms a render on the gallery deck.
    // The quote rule is unchanged: a quote opens a value only when the nearest
    // non-space character before it is `=`, and a quoted value is skipped whole.
    let j = lt + head[0].length;
    for (;;) {
      ATTR_SIG.lastIndex = j;
      const m = ATTR_SIG.exec(src);
      if (!m) { j = n; break; }
      const k = m.index;
      const c = src[k];
      if (c === '>') { j = k; break; }
      let b = k - 1;
      while (b > lt && (src[b] === ' ' || src[b] === '\t' || src[b] === '\n' || src[b] === '\r' || src[b] === '\f')) b -= 1;
      if (src[b] !== '=') { j = k + 1; continue; }  // a quote inside an UNQUOTED value
      const close = src.indexOf(c, k + 1);
      if (close < 0) { j = n; break; }                // an unterminated quoted value
      j = close + 1;
    }
    ...
    if (!isClose && !selfClose && RAW_TEXT_TAGS.has(name)) {
      // Searched in place with `lastIndex`, not over `src.slice(i)`: the slice
      // copied the rest of the document once per stylesheet or script.
      const closeRe = RAW_TEXT_CLOSE[name];
      closeRe.lastIndex = i;
      const hit = closeRe.exec(src);
      const close = hit ? { index: hit.index - i } : null;
