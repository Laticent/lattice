...
function* scanTags(src, from = 0) {
  const n = src.length;
  let i = from;
  // The next `-->` and the next `>` at or after the scan, found once and reused until
  // the scan passes them (-2 unknown, -1 none left). Searching afresh from every
  // UNCLOSED opener re-read the rest of the document each time: one 250 KB line of
  // `<!--` took 148 s to render. The answers are the same `indexOf` would give.
  let commentClose = -2;
  let bangClose = -2;
  const nextFrom = (cached, needle, at) =>
    (cached === -1 || cached >= at ? cached : src.indexOf(needle, at));
  while (i < n) {
    ...
    if (src.startsWith('<!--', lt)) {
      ...
      if (src.startsWith('>', lt + 4)) stop = lt + 5;
        ...
        const end = commentClose = nextFrom(commentClose, '-->', lt + 4);
    ...
    if (src.startsWith('<!', lt) || src.startsWith('<?', lt)) {
      const end = bangClose = nextFrom(bangClose, '>', lt + 2);
