 ...
 *
 * "`|` ALWAYS SEPARATES" IS TRUE OF THE DIRECTIVE, NOT OF THE ATTRIBUTE. The two
 * render arms read the same `data-track` from different sides of HTML decoding:
 * the string arm reads the attribute still ENCODED, the DOM arm reads what
 * `getAttribute` decoded. So a hand-written `data-track="A &#124; B | [C]"`
 * splits into two labels on one arm and three on the other — found by a 40,000
 * deck differential fuzz across both arms, as its single divergent atom.
 *
 * It is not reachable from markdown, and that is why it is recorded rather than
 * fixed: `token.attrSet` runs markdown-it's `escapeHtml`, so an author's
 * `&#124;` reaches the section as `&amp;#124;` and both arms then agree
 * (verified end to end through `render`, with `&`, `"`, `<`, `>`, `'` and
 * `-->` besides). One hand-written `<section data-track=…>`, or one producer
 * that is not markdown-it, is all it would take.
 *
