...
export function positionIsTrustworthy(deck, slideCount) {
	...
	if (splitsOnHeadings) {
		...
		for (const chunk of chunks) {
			// ATX tolerating markdown's 1–3 leading spaces. Anchoring at column 0 let `  ## Two` split
			// for the engine and stay invisible here. A heading nested in a list or blockquote is
			// indented FOUR or introduced by a marker, so it still does not divide.
			if ((chunk.match(/^[ \t]{0,3}#{1,2}[ \t]+\S/gm) || []).length > 1) return false;
		}
	}
	return true;
}
