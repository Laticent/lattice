 ...
 *
 * Extracted so it is testable. The same invariant lives in `narrowToSlide` (docs/src), and this is
 * now its third copy; the copies had already diverged once (this one was missing the raw `<section`
 * tally and skipped the count check when `slideCount` was absent), which is the argument for
 * reconciling the two slide splitters rather than growing a fourth. Until then, at least the copy
 * that an author-facing readout depends on is pinned by tests.
 *
 * Fails CLOSED: every "unsure" returns a reason, and the caller reports it instead of comparing a
 * pair it cannot prove is the right one.
 */
...
export function positionIsTrustworthy(deck, slideCount) {
	...
	// top-level h1/h2 after the first in a chunk. Count only headings at column 0 — one nested in a
	// list or blockquote does not divide (see the plugin's `token.level === 0` guard).
