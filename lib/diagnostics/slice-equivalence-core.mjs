...
export function positionIsTrustworthy(deck, slideCount) {
	...
	// `slideBoundaries` decides all three exactly as the engine does, so refusing on them
	// would now cost the fast path and buy nothing.
	//
	// What replaces them is the scanner's OWN verdict. It reports `certain: false` on the
	// shapes it cannot settle by scanning, and this is the caller that must honor it: an index
	// handed over on a boundary set nobody can vouch for is precisely the plausible lie the
	// rest of this function exists to prevent.
	const cut = splitSlideChunks(body);
	if (!cut.certain) return false;
