...
export function diffSections(got, want) {
	...
	// Anything left: the bodies differ but neither the section tag nor the words explain it (a nested
	// element's attribute, a reordered child). Fall back to the raw window rather than claim the list
	// above is complete — but compare WHITESPACE-COLLAPSED bodies first, because HTML collapses
	// whitespace when it paints, so a re-wrap is not a difference an author can see and reporting it
	// would be noise dressed as a finding.
	const ba = collapse(A.body);
	const bb = collapse(B.body);
	if (ba !== bb && (!out.length || ta === tb)) {
