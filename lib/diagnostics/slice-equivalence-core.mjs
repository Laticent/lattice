 ...
 * document on one sentence of prose that names the tag loses the whole deck. A `<section` inside a
 * quoted attribute value is NOT covered here, and still reads as a tag.
 */
function maskInert(html) {
	const blank = (m) => m.replace(/[^\n]/g, ' ');
	return html
		.replace(/<!--[\s\S]*?-->/g, blank)
		.replace(/(<(style|script)\b[^>]*>)([\s\S]*?)(<\/\2\s*>)/gi, (_m, open, _name, body, close) => open + blank(body) + close);
}
...
export function sectionsOf(html) {
	...
	const out = [];
	for (const m of maskInert(src).matchAll(/<section[\s\S]*?<\/section>/g)) out.push(src.slice(m.index, m.index + m[0].length));
	return out;
}
