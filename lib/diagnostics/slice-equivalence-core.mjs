...
// count-up-to-k, and the deck half is the expensive half: TWO whole-deck `md.block.parse`
// passes (`callerChunks` over the code-blanked source and again over the raw source), plus
...
// about each other — the preview's route gate (`DECK_DERIVED_FACTS`'s `ambiguous divider count`
// probes it at index 0 to find out whether the count can be trusted) and `supplyablePosition`
// (which needs the section for the shown slide). Measured on the perf spec's 40-slide gallery
// deck, Node 1x CPU, p50 of 60: 1.354ms EACH, so half of it was pure duplication — and at the
// 4x throttle the preview budget is written against, that is ~5.4ms of every keystroke.
// See engineering/decisions/2026-07-30-preview-deck-context-and-render-cost.md, Amendment 6.
//
...
function dividerDerivation(deck) {
	...
	// index space, so the two cannot desync. Per-chunk blanking is equivalent to whole-body
	// blanking here because no code construct spans a chunk boundary: a fence containing `---`
	// keeps it inside the fence (so it is not a boundary), indented code is line-scoped, and an
	// inline span cannot cross a newline.
	//
	// It also costs ONE whole-deck parse instead of two, because the second `callerChunks` — which
	// existed only to compare blanked and raw divider counts — is gone with the ambiguity it was
	// detecting.
	const flags = callerChunks(stripFm).map((chunk) => dividerTokens(blankCode(chunk)));
	const total = flags.filter(Boolean).length;

	// NO AMBIGUITY BAIL, and its removal is the point rather than a simplification. It existed
	// because counting over a whole-body-blanked copy could disagree with the raw reading, so a
	// `_class: divider` shown inside a code span risked painting an extra dot — "fail safe, not
	// fail wrong". Blanking per chunk removes the disagreement at its source: a divider inside code
	// is simply not a divider for that chunk, and the chunk it belongs to is the caller's own. So
	// there is no longer a reading to be unsure between, and a deck that teaches Lattice authoring
	// now gets a TRUE section on the cheap path instead of being pushed to the whole-deck route.
	sectionDerivation = { deck, flags, total };
...
export function deckSectionFor(deck, slideIndex) {
	const { flags, total } = dividerDerivation(String(deck ?? ''));
