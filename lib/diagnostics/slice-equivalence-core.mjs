...
/**
 * `html` with every span a browser reads as TEXT blanked to spaces of the same length: an HTML
 * comment, and the body of a CLOSED `<style>` or `<script>`. Offsets are preserved exactly, so a
 * match found in the masked copy slices the ORIGINAL string at the same place.
 *
 * Why: the section walk and the `<section` tally below are flat string scans, and a flat scan
 * reads `<!-- quoting <section class="title"> -->` as a real open tag. The tally then counted one
 * more section than the walk paired, `alignmentFailure` reported "nested or unbalanced", and the
 * Studio painted "This preview couldn't render" over an ordinary slide. The engine side already
 * walks with `scanTags` (lib/core/split-sections.js); that module is CommonJS and not on this
 * browser bundle, so this closes the two shapes an author reaches without the bundle change.
 * An UNCLOSED `<style>` is left alone for the reason split-sections gives: blanking to end of
 * document on one sentence of prose that names the tag loses the whole deck. NOT covered: a
 * `<section` inside an attribute value, or any `<` inside a tag, which still reads as a tag. Outside
 * those, the section count equals `scanTags`': 0 mismatches over 294,508 fuzzed documents (2026-09-24).
 */
// Whichever starts first wins, in ONE left-to-right pass, the way a tokenizer reads it: a `<!--`
// inside a stylesheet or a script is text, and a `<style>` inside a comment is comment. The first
// cut blanked every comment first and then every stylesheet, so a `content: "<!--"` in a `<style>`
// blanked through the next `-->` anywhere in the document, slides included. The rawtext names
// match `scanTags`' (script, style, textarea), and `(?![a-zA-Z0-9-])` keeps `<style-x>` a tag.
const INERT_START = /<!--|<[!?]|<(script|style|textarea)(?![a-zA-Z0-9-])/gi;

function inertRanges(html) {
	const ranges = [];
	const noCloseFrom = {}; // name -> an offset from which no close tag exists: keeps the pass linear
	let p = 0;
	for (;;) {
		INERT_START.lastIndex = p;
		const m = INERT_START.exec(html);
		if (!m) return ranges;
		const at = m.index;
		if (m[0] === '<!--') {
			// `<!-->` and `<!--->` are complete, EMPTY comments to a browser, not the start of one
			// that runs to the next `-->` in the document.
			let end;
			if (html.startsWith('>', at + 4)) end = at + 5;
			else if (html.startsWith('->', at + 4)) end = at + 6;
			else {
				const close = html.indexOf('-->', at + 4);
				if (close < 0) { p = at + 4; continue; } // unterminated: text, as `scanTags` reads it
				end = close + 3;
			}
			ranges.push([at, end]);
			p = end;
		...
		if (m[1] === undefined) {
			// A doctype, CDATA, processing instruction or bogus comment: inert through its `>`,
			// and text when there is none — `scanTags`' rule for the same shapes.
			const gt = html.indexOf('>', at + 2);
			if (gt < 0) { p = at + 2; continue; }
			ranges.push([at, gt + 1]);
			p = gt + 1;
			continue;
		}
		...
		if (gt < 0) return ranges;
		const body = gt + 1;
		let close = -1;
		if (!(noCloseFrom[name] <= body)) {
			const re = new RegExp(`</${name}(?![a-zA-Z0-9-])`, 'gi');
			re.lastIndex = body;
			const c = re.exec(html);
			if (c) close = c.index;
			else noCloseFrom[name] = body;
		}
		if (close < 0) { p = body; continue; } // unclosed: text (see the note above)
		ranges.push([body, close]);
		p = close;
	}
}
...
function maskInert(html) {
	let out = '';
	let p = 0;
	for (const [s, e] of inertRanges(html)) {
		out += html.slice(p, s) + html.slice(s, e).replace(/[^\n]/g, ' ');
		p = e;
	}
	return out + html.slice(p);
}

/** The rendered `<section>` elements of an engine render, in order — read past comments and
 * `<style>`/`<script>` text (see `maskInert`), and returned as slices of the original. */
export function sectionsOf(html) {
	const src = String(html ?? '');
	return sectionSpansOf(src).map(([s, e]) => src.slice(s, e));
}

/** The same sections as `[start, end)` offsets into `html`. A caller that has to FIND a section in
 * the document uses these, not `indexOf(section)`: a byte-identical copy of a later section inside
 * a comment would otherwise be found first, and narrowing would keep two live sections.
 * @param {string} html
 * @returns {[number, number][]} */
export function sectionSpansOf(html) {
	/** @type {[number, number][]} */
	const out = [];
	for (const m of maskInert(String(html ?? '')).matchAll(/<section[\s\S]*?<\/section>/g)) out.push([m.index, m.index + m[0].length]);
	return out;
}

/** How many `<section` open tags the document carries, read past comments and rawtext text. */
export function sectionOpenCount(html) {
	return (maskInert(String(html ?? '')).match(/<section\b/g) || []).length;
}
...
export function alignmentFailure(deckHtml, sections, slideCount, slideIndex) {
	const opens = sectionOpenCount(deckHtml);
...
export function classifyDivergence(got, want, facts) {
	...
	// A derived TOPIC TRACK on one side and not the other. Named
	// for the same reason "unclassified" is useless: it has a specific, statable meaning. A slice has
	// no section context — no preceding `divider`, no sibling `topic` slides — so
	// `lib/transformers/topic-track.js` has no headings to read and correctly emits nothing. The
	// slide then drops its band, seam and shelf reserve and composes as a single dark
	// canvas — it does NOT become `fact`, which this comment claimed until it was checked. This residual is
	// the component's DESIGNED degradation rather than a missing repair, so it belongs beside the
	// fail-closed guard above in kind: correct behavior, honestly reported.
	//
	// LAST, and that is the whole reason it sits here. A slice of a `topic` slide NEVER carries the
	// track, so this test is true on every such slide whatever else diverges on it — placed earlier
	// it swallowed the attribution for all of them, reporting `topic track absent` where the real
	// cause was the deck logo or the section index. Those two buckets exist to keep a HARD RULE #1
	// disagreement visible and countable, which is the principle the `section index` block above
	// states for itself; a catch-all inserted above them quietly spends it.
	if (/tile-track/.test(got) !== /tile-track/.test(want)) return 'topic track absent (no section context)';
	return 'unclassified';
}
 ...
 *   · `pie-wedge-N`, `radar-area-N`, `chart-spine-N`, `gantt-fill-pass-N`, `q-tint-N`,
 *     `cart-fill-1-N` — chart
 ...
 */
const GENERATED_ID = /\b(?:lat-svg[td]|pie-wedge|radar-area|chart-spine|gantt-fill|gantt-barclip|q-tint|cart-fill)[\w-]*-\d+\b/g;
