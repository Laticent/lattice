...
export function slideSeparatorRe(flags = '') {
	// CRLF-TOLERANT, because `splitSlides` in this same module is. Without the `\r?` a CRLF deck cut
	// to ONE chunk here while `splitSlides` cut it correctly, so `positionIsTrustworthy` saw a count
	// mismatch and refused every CRLF deck — fail-closed, so never a wrong number, but the
	// optimization silently off for a whole class of decks and the module disagreeing with itself
	// about what a separator is. That disagreement is the exact thing having ONE definition is for.
	return new RegExp('\\r?\\n-{3,}\\r?\\n', flags);
}
