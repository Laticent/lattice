 ...
 *
 * TWO IMPORTS: `lib/core/slide-boundaries.mjs`, and the shared section walker
 * `lib/core/split-sections.mjs` with its tokenizer `lib/core/top-level-h2.mjs`. All three are ESM
 * and import nothing else. Every other dependency is refused for the reason
 * lib/authoring/lint-core.js refuses them — this has to bundle for the browser. The engine's directive VOCABULARY lives in lib/engine/directives.js, which is CommonJS;
...
import { splitSections, unclosedSectionAt } from '../core/split-sections.mjs';
import { scanTags } from '../core/top-level-h2.mjs';
...
// ── THE SECTION WALK ─────────────────────────────────────────────────────────
// The engine's own walker, `splitSections` (lib/core/split-sections.mjs), over its tokenizer
// `scanTags` (lib/core/top-level-h2.mjs) — not a second reading of the document (HARD RULE #1).
// A `<section` a browser reads as TEXT (inside a comment, inside `<style>`/`<script>` text, or
// inside a quoted attribute value such as `<p title="<section>">`) neither opens nor closes a
// section here, exactly as it does not on the render path. A flat scan counted any of those as a
// real open tag; the tally then disagreed with the walk, `alignmentFailure` reported "nested or
// unbalanced", and the Studio painted "This preview couldn't render" over an ordinary slide.
//
// This module used to carry a local mask (`maskInert`) because the walker was CommonJS and off the
// browser bundle. The mask covered comments and closed rawtext only, and every rule in it restated
// one of `scanTags`' (the unterminated comment, the empty `<!-->`, the unclosed `<style>`), so the
// two could drift. The walker is ESM now and this module imports it.

/** The rendered `<section>` elements of an engine render, in order — top-level, as the engine's
 * walker reads them, and returned as slices of the original. */
...
export function sectionSpansOf(html) {
	return splitSections(String(html ?? ''))
		.filter((p) => p.type === 'section')
		.map((p) => [p.start, p.end]);
}

/** How many `<section` OPEN tags the document carries, nested ones included — read by the same
 * tokenizer, so text that only looks like a tag is not counted. A diagnostic count: the refusal
 * guards key on `unclosedSectionAt`, not on this, because balanced nesting walks fine. */
export function sectionOpenCount(html) {
	let n = 0;
	for (const t of scanTags(String(html ?? ''))) if (t.kind === 'tag' && t.name === 'section' && !t.isClose) n += 1;
	return n;
}
...
export function alignmentFailure(deckHtml, sections, slideCount, slideIndex) {
	// A section that never closes is the one shape the walk cannot place: a browser runs it to
	// the end of the document, so every later slide is inside it. Balanced NESTING is not refused:
	// the walk is depth-aware (`splitSections`), so a slide that holds a hand-authored `<section>`
	// is still one top-level section. The guard used to compare every open tag with the
	// top-level count, from when this module paired each open with the next close.
	if (unclosedSectionAt(String(deckHtml ?? '')) >= 0) return 'the deck contains a `<section>` that never closes, so the slides cannot be told apart';
