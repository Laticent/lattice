...
// (→ `div.lattice > section.x`), a `figure.y` arm is a slide DESCENDANT (→
// `div.lattice > section figure.y`). Marpit's prepend (step 2 below) only treats a
// LITERAL leading `section` as slide-targeting, so `:is(section.map, figure.chart-frame)
// .map-region` was scoped as a slide *descendant* (`div.lattice > section :is(section.map,
