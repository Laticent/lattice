 ...
 * export, PDF, PPTX, the HTML sidecar at ANY window size, marp, and the live
 * preview. The runtime stamp stays as an INLINE override, which is exactly the
 * right precedence: it only differs from this value when the box genuinely is
 * not the authored one (the `--fluid` viewer, which unpins the slide on purpose).
 *
 ...
 * Returns '' for a non-pixel geometry, leaving the `1cq*` fallback in place —
