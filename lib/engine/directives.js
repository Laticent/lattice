 ...
 * the deck-wide `class:` APPEND behavior Lattice wants is layered ON TOP by the
 * existing `deckClassPropagate` plugin (lib/integrations/markdown-it/plugins.js), which
 * the engine composes unchanged. We deliberately match marp-core's historical
 * directive semantics here so that plugin keeps doing exactly what it always
 * did, now on the owned engine.
 *
...
// The one front-matter VALUE rule, shared with `frontMatterValue` /
// `topLevelFrontMatterValue` so the engine's own parse and every `resolve-*`
// register clean a scalar identically. This file is bundled for the browser by
// tools/build-playground.js (esbuild), so a `lib/core` require resolves the same
// way `lib/engine/index.js`'s several already do.
const { frontMatterScalar } = require('../core/front-matter-key');
...
// comments under a recognized directive set.
const KNOWN_DIRECTIVES = new Set([
  ...
  'debug',
  // Reader lenses (engineering/decisions/2026-07-13-lente-reader-lenses.md).
  // `<!-- _lens: brief ask -->` on a slide carries its reader-lens membership. It is
  // KNOWN so it is STRIPPED from the body (never reaching markdown-it / the export),
  // but deliberately NOT in APPLIED_DIRECTIVES — it becomes no `<section>` attribute;
  // membership is read (from the source, not the DOM) by @laticent/lente. Registering
  // it here stops the tag leaking internal lens membership into shared/exported HTML,
  // which it would as an unknown comment. Lowercase-only, like every other directive.
  'lens',
  // The `topic` anchor's sibling-track override (lib/core/track-spec.js).
  // `<!-- _track: Cost to win | Lifetime value | [Payback] -->` names the scale
  // this slide sits in when the DERIVED one (every topic slide's own `<h2>`) is
  // not what the author wants — shorter labels, or topics not written yet. It is
  // KNOWN so it is stripped from the body, and APPLIED so it reaches both render
  // arms as one `data-track` string; left unknown it would ride into shared and
  // exported HTML as a stray comment — the leak `lens` above is registered to
  // avoid — and the note extractor would read it as speaker text.
  'track',
]);
...
// `lens` is a flag too, so the degenerate bare form `<!-- _lens -->` (no tokens) STRIPS as
// empty instead of leaking the literal comment into the export (a normal `_lens` always has
// a value; a bare one is meaningless but must not survive into shared HTML).
const FLAG_DIRECTIVES = new Set(['build', 'debug', 'lens']);
...
const APPLIED_DIRECTIVES = new Set([
  ...
  'debug',
  // `track` emits `data-track` on the section; `lib/transformers/topic-track.js`
  // reads it on both arms. No `--track` custom property — see NO_CUSTOM_PROP.
  'track',
]);

/**
 * Applied directives that get the `data-<kebab>` attribute and NOT the matching
 * `--<kebab>` custom property.
 *
 * Each of these is read by CODE rather than by CSS, so the property would be
 * dead bytes for the export pipeline to strip. A NAMED SET rather than the
 * `key !== 'build' && key !== 'debug'` chain it replaces: that chain sat at two
 * call sites in slides.js, so a third member was two edits with one easy to miss.
 */
const NO_CUSTOM_PROP = new Set(['build', 'debug', 'track']);
...
// Value cleaning is NOT local to this file. `frontMatterScalar` (lib/core/front-matter-key.js)
// is the one rule shared with `frontMatterValue` / `topLevelFrontMatterValue`, which is how
// every `resolve-*` register reads the same block. The local `stripQuotes` this replaced kept
// a trailing YAML comment as part of the value — `theme: cuoio  # brand` resolved to a palette
// name no theme file matches, so the deck fell back to the default here while Export-to-Marp
// (real YAML) kept `cuoio`. Two readers, one question, opposite answers.
function stripQuotes(v) {
  return frontMatterScalar(v);
}
