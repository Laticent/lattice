...
// Lattice's augmented shell grammar, required HERE rather than inside the plugins
// module and handed to registerShellHljs — plugins.js is on the browser runtime's
// bundle graph, and the runtime never highlights. See that function's docblock.
const shellLanguage = require('../integrations/highlight-js/shell.hljs');
const { resetRenderIds, pinRenderSlide } = require('../core/render-ids');
const { readClassAttr, mapSectionHtml } = require('../core/section-walk');
...
const panes = require('../core/panes');
const paneCss = require('../core/pane-css');
const { CHART_LAYOUTS } = require('../components/chart/_chart-family/chart-family');
...
const paginationTile = require('../forms/tile/pagination/pagination.transform');
...
const sectionIndex = require('../core/section-index');
const svgA11yNames = require('../core/svg-a11y-names');
const fenceLanguagesKernel = require('../core/fence-languages');
const { spansFromHighlightHtml } = require('../core/highlight-spans');
...
function applyImageStructure(html) {
  ...
  // quoted in a comment ended the slide there and split the image panel in two.
  return mapSectionHtml(html, (sec) => {
...
const LATTICE_PLUGINS = [
  ...
  checklistItemStates,
  inlinePills,
  // AFTER inlinePills, so a `{LABEL}` span already turned into a pill is read as the element it
  // now is rather than as code (#2308).
  proseCodeMark,
  slotLabelLift,
  glossaryListToTable,
  glossaryRange,
...
const languages = {
  ...
  /**
   * Tokenize `code` as `name` and report each token's RANGE in that string —
   * `[{ from, to, cls }]`, offsets into `code`, nesting preserved.
   *
   * READ-ONLY, and additive: it runs the same `hljs.highlight` call the fence
   * renderer runs, with the same `ignoreIllegals`, and reads the result back as
   * offsets instead of HTML. Nothing about a render changes.
   *
   * It exists for a surface that cannot take HTML — the Compose editor, whose
   * `code_block` is plain text in a ProseMirror document. Going through the
   * engine rather than standing a second highlight.js up inside the docs bundle
   * is the point: this singleton is the one carrying Lattice's OWN mermaid and
   * augmented-shell grammars, so the editor's tokens are the export's tokens
   * (HARD RULE #1). A grammar this build does not have yields `[]`, which is the
   * same silent, plain-text degradation the renderer already makes.
   */
  spans(code, name) {
    if (typeof code !== 'string' || !code || !name || !hljs.getLanguage(name)) return [];
    try {
      return spansFromHighlightHtml(hljs.highlight(code, { language: name, ignoreIllegals: true }).value);
    } catch {
      return [];
    }
  },
...
function createEngine(opts = {}) {
  ...
  function buildMd(globalBase, geom) {
    ...
    // `reflow` rides the SAME family gate autosplit uses (`AUTOSPLIT_APPLIES`): a display
    // equation is re-flowed onto `aligned` lines only at square/tall/strip, never at `wide`.
    // See `displayBlock` in lib/engine/math.js for why, and lib/core/tex-linebreak.js for
    // the rule itself.
    if (opts.math !== false) installMath(md, { output: opts.mathOutput, reflow: family !== 'wide' });
    registerMermaidHljs(md);
    registerShellHljs(md, shellLanguage);
    for (const plugin of LATTICE_PLUGINS) md.use(plugin);
    panes.installPanes(md);
    return md;
  }
  ...
  // THE PANES' BOX — the host's stage, less what the host keeps above and below it. Measured
  // in Chromium (engine sheet, `.lat-panes` box) on every registered size. The stage is 90% of
  // the slide's width. Its height is the slide's less a base band (the title and the stage's
  // own padding), less an eyebrow, a subtitle, a Key Insight or a below-note when the host has
  // one; the four add up (hd: 244px measured against 252 summed). Every figure is a fraction
  // of the slide WIDTH, and landscape is proportional (hd and 4K agree to 0.1%). Portrait and
  // square boost type and spacing by `--canvas-scale` (lib/engine/css.js `orientationFor`),
  // and the chrome does not grow in proportion to it, so each band is a MEASURED point per
  // scale, interpolated between them:
  //   scale  base    eyebrow subtitle insight note     measured on
  //   1      .2195   .0297   .0352    .0789   .0531    hd 1280x720, 4K, standard
  //   1.65   .2657   .0481   .0565    .1306   .0889    square 1080x1080
  //   1.95   .3102   .0722   .0833    .1750   .1222    portrait 1080x1350
  //   2.19   .3204   .0731   .0833    .1824   .1287    story 1080x1920
  //   2.29   .3250   .0722   .0833    .1852   .1315    mobile 1080x2340
  // Each band above is ONE line. A title, subtitle, Key Insight or note that wraps takes a
  // further line height per line, and the carve hands over each one's text length
  // (lib/core/panes.js `chrome`), so the lines are counted from a measured one-line capacity:
  //   scale  line height (fraction of width)     characters per line, per 1000px of width
  //          title  subtitle insight note         title subtitle insight note
  //   1      .0350  .0350    .0251   .0284        48.4  53.9     75.8    75.8
  //   1.65   .0534  .0560    .0419   .0476        38.0  44.4     50.9    50.9
  //   1.95+  .0846  .0831    .0653   .0740        25.0  31.5     31.5    31.5
  // (portrait, story and mobile measure the same: the type tokens top out.) The count takes
  // 90% of the capacity, because real text runs wider than the measured words and a model
  // that errs TALL is the costly direction: a chart canvas taller than its real box letterboxes
  // and shrinks its labels. The rest of the error is the runtime measurement,
  // followups.d/2376-p2-measure-pane-size-at-runtime.md.
  const STAGE_W = 0.9;
  const STAGE_BANDS = [
    // scale, base, eyebrow, subtitle, insight, note, then line height and characters per line
    // (per 1000px) for title, subtitle, insight, note
    [1, 0.2195, 0.0297, 0.0352, 0.0789, 0.0531, 0.035, 0.035, 0.0251, 0.0284, 48.4, 53.9, 75.8, 75.8],
    [1.65, 0.2657, 0.0481, 0.0565, 0.1306, 0.0889, 0.0534, 0.056, 0.0419, 0.0476, 38, 44.4, 50.9, 50.9],
    [1.95, 0.3102, 0.0722, 0.0833, 0.175, 0.1222, 0.0846, 0.0831, 0.0653, 0.074, 25, 31.5, 31.5, 31.5],
    [2.19, 0.3204, 0.0731, 0.0833, 0.1824, 0.1287, 0.0846, 0.0831, 0.0653, 0.074, 25, 31.5, 31.5, 31.5],
    [2.29, 0.325, 0.0722, 0.0833, 0.1852, 0.1315, 0.0846, 0.0831, 0.0653, 0.074, 25, 31.5, 31.5, 31.5],
  ];
  const BAND_KEYS = ['base', 'eyebrow', 'subtitle', 'insight', 'note',
    'lineTitle', 'lineSubtitle', 'lineInsight', 'lineNote', 'cplTitle', 'cplSubtitle', 'cplInsight', 'cplNote'];
  const WRAPS = { title: ['lineTitle', 'cplTitle'], subtitle: ['lineSubtitle', 'cplSubtitle'], insight: ['lineInsight', 'cplInsight'], note: ['lineNote', 'cplNote'] };

  /** The px a chrome element of `chars` characters takes beyond its one measured line, set in
   *  a measure `lineW` wide (the stage's; a pane's own, for a coda that stays in a pane). */
  function extraLines(W, bands, key, chars, lineW = W) {
    if (!chars || !WRAPS[key]) return 0;
    const [line, cpl] = WRAPS[key];
    const perLine = Math.max(1, bands[cpl] * (lineW / 1000) * 0.9);
    return (Math.ceil(chars / perLine) - 1) * W * bands[line];
  }
  // The gutter between panes as a fraction of the slide WIDTH — `--sp-2xl` (5cqi) side by
  // side, `--sp-xl` (3.75cqi) stacked, as lib/forms/cell/pane/pane.css sets them. Spacing
  // tokens carry `--canvas-scale`, so the gutter does too (measured: square 89px = .05 × 1080
  // × 1.65, mobile 124px = .05 × 1080 × 2.29).
  const PANE_GAP_FRAC = { side: 0.05, stack: 0.0375 };

  /** The measured bands for a geometry, as fractions of its width (see the table above). */
  function stageBands(geom) {
    const k = orientationFor(geom).scale;
    let lo = STAGE_BANDS[0];
    let hi = STAGE_BANDS[STAGE_BANDS.length - 1];
    for (let n = 0; n < STAGE_BANDS.length - 1; n++) {
      if (k >= STAGE_BANDS[n][0] && k <= STAGE_BANDS[n + 1][0]) [lo, hi] = [STAGE_BANDS[n], STAGE_BANDS[n + 1]];
    }
    const t = hi[0] === lo[0] ? 0 : Math.min(1, Math.max(0, (k - lo[0]) / (hi[0] - lo[0])));
    const out = { scale: k };
    BAND_KEYS.forEach((key, n) => { out[key] = lo[n + 1] + (hi[n + 1] - lo[n + 1]) * t; });
    return out;
  }

  function stageBox(geom, chrome = {}) {
    const W = parseFloat(geom.width);
    const H = parseFloat(geom.height);
    const bands = stageBands(geom);
    let h = H - W * bands.base - extraLines(W, bands, 'title', chrome.title);
    for (const k of ['eyebrow', 'subtitle', 'insight', 'note']) {
      if (chrome[k]) h -= W * bands[k] + extraLines(W, bands, k, chrome[k]);
    }
    return { w: W * STAGE_W, h: Math.max(h, H * 0.2) };
  }
  const paneMdMemo = new Map();

  function paneGeometry(geom, layout, index, chrome) {
    const { w, h } = stageBox(geom, chrome);
    const share = (index === 0 ? layout.a : layout.b) / 100;
    const gap = parseFloat(geom.width) * PANE_GAP_FRAC[layout.direction === 'stack' ? 'stack' : 'side'] * orientationFor(geom).scale;
    return layout.direction === 'stack'
      ? { width: `${w}px`, height: `${(h - gap) * share}px` }
      : { width: `${(w - gap) * share}px`, height: `${h}px` };
  }

  // A chart PANE's canvas, in the user units a full-slide chart is drawn in (cartesian.js
  // VIEW: 320x180 landscape, 320x300 portrait). One unit is the px a full slide's chart gets
  // per unit — the smaller of its stage-to-canvas ratios, since a slide chart letterboxes with
  // `meet` — so the pane's box divided by it is a canvas whose labels print at the slide's
  // size. The chart then lays out for the pane (a shorter plot, fewer ticks, wrapped labels)
  // instead of the slide's drawing scaled down into it. Stamped as `data-pane-view` on the
  // pane's section, where the chart family reads it (chart-family.js); no ordinary slide has it.
  function paneView(hostGeom, paneGeom) {
    // The FULL slide's chart scale is set by a title-only slide's stage: that is the box a chart
    // slide draws into, so its labels are the size a pane's labels should match.
    const { w: stageW, h: stageH } = stageBox(hostGeom);
    const view = orientationFor(hostGeom).name === 'portrait' ? { w: 320, h: 300 } : { w: 320, h: 180 };
    const unit = Math.min(stageW / view.w, stageH / view.h);
    const round = (n) => Math.round(n * 10) / 10;
    return `${round(parseFloat(paneGeom.width) / unit)} ${round(parseFloat(paneGeom.height) / unit)}`;
  }

  // A pane that CLAIMED the slide's Key Insight or note (lib/core/panes.js) draws it inside
  // itself, so its chart gets the pane's box less that band — the same measured heights the
  // host would have given up.
  function claimedBox(hostGeom, geom, claimed) {
    if (!claimed || !(claimed.insight || claimed.note)) return geom;
    const W = parseFloat(hostGeom.width);
    let h = parseFloat(geom.height);
    const bands = stageBands(hostGeom);
    // Set in the pane's width, which is the stage's share of the slide's (0.9 × W on a slide).
    const lineW = parseFloat(geom.width) / STAGE_W;
    if (claimed.insight) h -= W * bands.insight + extraLines(W, bands, 'insight', claimed.insight, lineW);
    if (claimed.note) h -= W * bands.note + extraLines(W, bands, 'note', claimed.note, lineW);
    return { ...geom, height: `${Math.max(h, parseFloat(geom.height) * 0.3)}px` };
  }

  function renderPane(pane, layout, globalBase, baseUrl, source, chrome) {
    const hostGeom = themes.geometryFor(globalBase.theme || 'lattice', globalBase.size);
    const geom = paneGeometry(hostGeom, layout, pane.index, chrome);
    // The deck-wide running chrome belongs to the host slide, never to a pane.
    const paneBase = { ...globalBase };
    for (const k of ['header', 'footer', 'paginate']) delete paneBase[k];
    // `buildMd` reads the geometry only to derive orientation + family, so a pane parser
    // is keyed on that pair: two panes of the same shape share one parser.
    const w = parseFloat(geom.width);
    const h = parseFloat(geom.height);
    const key = `${JSON.stringify(paneBase)}|${opts.math}|${opts.mathOutput}|${orientationFor(geom).name}|${familyFor(w / h)}`;
    let md = paneMdMemo.get(key);
    if (!md) {
      md = buildMd(paneBase, geom);
      // One entry per pane shape a deck uses; bounded by the four families × orientations.
      if (paneMdMemo.size > 16) paneMdMemo.clear();
      paneMdMemo.set(key, md);
    }
    // A pane is its component's BODY: no pagination, header or footer of its own.
    // The chart family's wrap frames its body against the slide's h2, so a CHART pane
    // carries a stand-in heading. It lands in the pane's masthead, which `panes.embed`
    // discards (handing back any paragraph the masthead lifted with it): the HOST owns the
    // only title. No other component gets one, so nothing else can be lifted away.
    const cls = panes.paneForm(pane.cls);
    const standIn = CHART_LAYOUTS.includes(cls) ? '## \u200b\n\n' : '';
    const src = `<!-- _class: ${cls}${pane.cards ? ` ${pane.cards}` : ''} -->\n\n${standIn}${pane.markdown}\n`;
    // The deck's front matter rides along, so a deck-wide register (`mode: sketch`, a
    // deck `class:`) reaches the pane's component as it reaches every slide.
    const fm = (source || '').match(/^---\r?\n[\s\S]*?\r?\n---\r?\n/);
    let html = md.render(bgImage.liftImageBgImages(src, baseUrl), { markdown: (fm ? fm[0] : '') + src });
    html = html.replace(/<section(?=[\s>])/, `<section data-pane-view="${paneView(hostGeom, claimedBox(hostGeom, geom, pane.claimed))}"`);
    html = applyFormToHtml(html);
  ...
  function renderHtml(body, globalBase, source, baseUrl, stats, page) {
    ...
    // `latticePanes` collects the slides the panes rule carved (lib/core/panes.js); a deck
    // with no `<!-- pane: -->` marker leaves it empty and renders byte-identical markup.
    const env = { markdown: source, page, latticePanes: [] };
    ...
    // PANES — render each carved pane as an ordinary one-slide deck of its component,
    // AFTER the host's transforms, with its SVG ids scoped to the host slide's number, so a
    // slide's ids never depend on the panes of a later slide.
    if (env.latticePanes.length) {
      for (const c of env.latticePanes) {
        pinRenderSlide(c.section);
        try {
          for (const p of c.panes) p.html = renderPane(p, c.layout, globalBase, baseUrl, source, c.chrome);
        } finally {
          pinRenderSlide(null);
        }
      }
      html = panes.embed(html, env.latticePanes);
    }
    ...
    html = applyDeckLogoToHtml(html, source, page?.offset);
    ...
    // The page number as a REAL element on every paginated frame (#2206). LATE, and
    // for the same reason the berth below is last: the span must be a DIRECT CHILD of
    // the section to take its padding box as the containing block, and every transform
    // above moves loose section children into a cell (`.cell-stage`, `.image-text`,
    // `.backdrop`). It is a no-op on a frame that already minted the element in its
    // footer Cell, so a chrome-hosting frame's markup is byte-identical to before.
    html = paginationTile.applyToHtml(html);
    ...
    // The running section number for `divider numbered`, stamped as an attribute the
    // stylesheet reads. It is not a CSS counter because a counter cannot cross Marpit's
    // per-slide `<foreignObject>` containers — see lib/core/section-index.js.
    html = sectionIndex.applyToHtml(html);
   ...
   * `opts.styles: 'scoped' | 'flat'` names the style-delivery mode; 'flat' adds
   * `flatCss` (see the comment where it is produced).
   */
  function render(markdown, theme, opts) {
    ...
    // A deck with panes composes the pane-widened sheet; every other deck, the plain one.
    const panesInDeck = paneCss.paneClasses(html);
    const css = themes.cssFor(themeName, sizeName, { panes: panesInDeck });
    const { width, height } = themes.geometryFor(themeName, sizeName);
    if (stats) stats.cssMs = performance.now() - tCss;
    const out = stats ? { html, css, width, height, stats } : { html, css, width, height };
    // `opts.styles` — the style-delivery MODE the host asks for, by name
    // (engineering/decisions/2026-09-24-one-style-delivery-spine.md §4):
    //   · 'scoped' (default) — `css` only: the pack scoped to the inside of a slide, for a
    //     host that shows slides and nothing else (the preview, the Playground, embeds).
    //   · 'flat' — ALSO `flatCss`: the same pack with every re-scoped arm shipped as
    //     written too, for a host whose content also sits OUTSIDE a slide (the exported
    //     player's Read · Article, the Studio's Reading view). `css` stays the scoped shape,
    //     because the same caller still mounts the deck in a preview-shaped capture frame.
    // The third mode, 'baked', is not a string: it reads computed styles, so it is the
    // browser-side `bakeSvg` (standalone-svg.js). Asking render() for it is a caller bug,
    // and an unknown name throws rather than silently falling back to 'scoped' — the
    // fallback is exactly how #2344 shipped black charts. See `packSelector` in ./css.js
    // for what "flat" changes and why only the pack can do it.
    const mode = opts?.styles ?? 'scoped';
    if (mode !== 'scoped' && mode !== 'flat') {
      throw new TypeError(
        mode === 'baked'
          ? "render(): styles 'baked' is not a stylesheet — call bakeSvg() on the rendered SVG in the browser"
          : `render(): unknown styles mode ${JSON.stringify(mode)} (expected 'scoped' or 'flat')`,
      );
    }
    if (mode === 'flat') out.flatCss = themes.cssFor(themeName, sizeName, { flat: true, panes: panesInDeck });
    return out;
  }
  ...
  return { render, geometry, addThemes, hasTheme, themes, languages, referenceTargets };
}

// A bare CommonMark parser for `referenceTargets`: a link reference definition is core
// CommonMark, and none of the engine's plugins changes how one is read.
let referenceParser = null;
/**
 * The target of every link reference definition in `markdown` (`[label]: target`), used or
 * not, as markdown-it normalizes it. A definition that renders nothing where it is written
 * still resolves an image elsewhere in the document, and the FIRST definition of a label
 * wins — so a component's sample slide inserted into a deck can hand the deck's own
 * `![logo]` a remote target. The package gallery gate refuses on any remote one
 * (lib/packages/gallery-gate.js, docs/.../library/gallery-gate.ts).
 * @param {string} markdown
 * @returns {string[]}
 */
function referenceTargets(markdown) {
  if (!referenceParser) referenceParser = new MarkdownIt('commonmark');
  const env = {};
  referenceParser.parse(String(markdown ?? ''), env);
  return Object.values(env.references || {}).map((r) => r.href);
}
