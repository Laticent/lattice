...
const LATTICE_PLUGINS = [
  ...
  // AFTER deckClassPropagate so a deck-wide `class: <component>` counts (#1292).
  defaultComponent,
  verdictGridBadges,
  obligationMatrixBadges,
  matrixGridCells,
...
function createEngine(opts = {}) {
  ...
  function render(markdown, theme, opts) {
    ...
    // LF, so no already-correct deck changes a byte — measured across all 118 committed example
    // decks. CRLF already rendered identically to LF (0 of 118 differed); a lone CR did NOT
    // (117 of 118 differed), so this closes a real gap rather than being belt-and-braces.
