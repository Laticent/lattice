...
function createEngine(opts = {}) {
  ...
  function renderPane(pane, layout, globalBase, baseUrl) {
    ...
    // Some kernels frame their body against the slide's h2 (the chart family's wrap
    // locates it), so a pane carries a stand-in heading. It lands in the pane's
    // masthead, which `panes.embed` discards: the HOST owns the only title.
    const src = `<!-- _class: ${panes.PANE_FORM[pane.cls] || pane.cls} -->\n\n## \u200b\n\n${pane.markdown}\n`;
    let html = md.render(bgImage.liftImageBgImages(src, baseUrl), { markdown: src });
  ...
  function renderHtml(body, globalBase, source, baseUrl, stats, page) {
    ...
    if (env.latticePanes.length) {
      for (const c of env.latticePanes) {
        ...
        for (const p of c.panes) p.html = renderPane(p, c.layout, globalBase, baseUrl);
        pinRenderSlide(null);
      }
  ...
  function render(markdown, theme, opts) {
    ...
    if (mode === 'flat') out.flatCss = themes.cssFor(themeName, sizeName, { flat: true });
    return out;
  }
