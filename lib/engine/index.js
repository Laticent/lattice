...
function createEngine(opts = {}) {
  ...
  // rebuilt on EVERY render. Measured (Node, warm, median): it is ~0.3ms of a 0.44ms one-slide
  // render, so it dominated the cost of rendering a single slide and was worth about ten prose
  // slides of content. Reusing it makes a one-slide render 0.44ms -> 0.15ms (2.9x) and a
  // 40-prose-slide deck 2.00ms -> 1.53ms.
  //
  ...
  function memoizedMd(globalBase) {
    // The key must cover everything `buildMd` reads. `globalBase` is the deck's directive base
    // (theme, size, and every front-matter directive); `opts.math`/`opts.mathOutput` gate the math
    // plugin's installation and output mode.
    const key = `${JSON.stringify(globalBase)}|${opts.math}|${opts.mathOutput}`;
    if (mdMemo && mdMemo.key === key) return mdMemo.md;
    mdMemo = { key, md: buildMd(globalBase) };
    return mdMemo.md;
  }

  function renderHtml(body, globalBase, source, baseUrl, stats) {
    ...
    // across calls — rendering one deck twice produced different bytes on 48 of 112 committed
    ...
    html = svgA11yNames.applyToHtml(html);
  ...
  function render(markdown, theme, opts) {
    ...
    let html = renderHtml(body, globalBase, markdown, opts?.baseUrl, stats);
