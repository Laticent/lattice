...
function createEngine(opts = {}) {
  ...
  function render(markdown, theme, opts) {
    ...
    // `opts.flatCss` — ALSO return the stylesheet packed for a FLAT document: the exported
    // player, which lays its slides out without the preview wrapper and whose Read · Article
    // view shows slide content outside any slide. `css` stays the preview shape, because the
    // same caller still mounts the deck in a preview-shaped capture frame (the diagram bake).
    // See `packSelector` in ./css.js for what "flat" changes and why only the pack can do it.
    if (opts?.flatCss) out.flatCss = themes.cssFor(themeName, sizeName, { flat: true });
    return out;
  }
