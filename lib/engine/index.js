...
function createEngine(opts = {}) {
  ...
  // in Chromium on a 1280x720 panes slide (the CLI export, indaco): a title alone leaves a
  // 1152x438 stage; an eyebrow takes 38px, a subtitle 45, a Key Insight 103, a below-note 68,
  // and they add up (all four measured 192 against 184 summed). Type and spacing scale with
  // the slide's WIDTH (`--_sec-1cqi`), so each deduction is a fraction of it, and the title-only
  // stage is the slide height less a width fraction. An estimate that errs TALL is the costly
  // direction — a chart canvas taller than its real box letterboxes and shrinks its labels — so
  // this is the measured size, not a round-up. A title long enough to wrap is not modelled yet
  // (the runtime measurement is followups.d/2376-p2-measure-pane-size-at-runtime.md).
  const STAGE = { w: 1152 / 1280, chrome: 282 / 1280, eyebrow: 38 / 1280, subtitle: 45 / 1280, insight: 103 / 1280, note: 68 / 1280 };
  ...
  function stageBox(geom, chrome = {}) {
    ...
    let h = H - W * STAGE.chrome;
    for (const k of ['eyebrow', 'subtitle', 'insight', 'note']) if (chrome[k]) h -= W * STAGE[k];
    return { w: W * STAGE.w, h: Math.max(h, H * 0.2) };
  }
  ...
  function claimedBox(hostGeom, geom, claimed) {
    ...
    if (claimed.insight) h -= W * STAGE.insight;
    if (claimed.note) h -= W * STAGE.note;
