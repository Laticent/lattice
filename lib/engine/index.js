...
const { guardHtmlInline } = require('../core/html-inline-guard.mjs');
...
function createEngine(opts = {}) {
  ...
  function buildMd(globalBase, geom) {
    ...
    // Upstream `html_inline` is quadratic on a paragraph of unclosed `<!--`; the guard
    // answers that case without its regex, with the same answer.
    guardHtmlInline(md);
    ...
    panes.installPaneSplit(md, { family });
  ...
  // THE PANES' BOX — the host's stage, less what the host keeps above and below it. Only a
  // `wide` deck lays panes out (a square, portrait, story or mobile deck splits them into slides,
  // lib/core/panes.js `installPaneSplit`), so every figure is at `--canvas-scale` 1. Measured in
  // Chromium (engine sheet, `.lat-panes` box): the stage is 90% of the slide's width; its height
  // is the slide's less a base band (the title and the stage's own padding), less an eyebrow, a
  // subtitle, a Key Insight or a below-note when the host has one. The four add up (hd: 244px
  // measured against 252 summed), and each is a fraction of the slide WIDTH, since type and
  // spacing scale with it (hd and 4K agree to 0.1%):
  //   base .2195 · eyebrow .0297 · subtitle .0352 · Key Insight .0789 · note .0531
  // Each band is ONE line. A title, subtitle, Key Insight or note that wraps takes a further line
  // per line, counted from its text length (the carve hands it over, lib/core/panes.js `chrome`)
  // against a measured one-line capacity, in characters per 1000px of width:
  //   title 48.4 (line .035) · subtitle 53.9 (.035) · Key Insight 75.8 (.0251) · note 75.8 (.0284)
  // The count takes 90% of the capacity, because real text runs wider than the measured words and
  // a model that errs TALL is the costly direction: a chart canvas taller than its real box
  // letterboxes and shrinks its labels. The rest of the error is the runtime measurement,
  // followups.d/2376-p2-measure-pane-size-at-runtime.md.
  const STAGE_W = 0.9;
  const BANDS = {
    base: 0.2195, eyebrow: 0.0297, subtitle: 0.0352, insight: 0.0789, note: 0.0531,
    lineTitle: 0.035, lineSubtitle: 0.035, lineInsight: 0.0251, lineNote: 0.0284,
    cplTitle: 48.4, cplSubtitle: 53.9, cplInsight: 75.8, cplNote: 75.8,
  };
  ...
  // side, `--sp-xl` (3.75cqi) stacked, as lib/forms/cell/pane/pane.css sets them.
  ...
  function paneGeometry(geom, layout, index, chrome) {
    ...
    const gap = parseFloat(geom.width) * PANE_GAP_FRAC[layout.direction === 'stack' ? 'stack' : 'side'];
