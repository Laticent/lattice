...
function createEngine(opts = {}) {
  ...
  // The stage box as a fraction of the slide, measured on a rendered 16:9 `list` slide
  // with a title (Chromium, 1280×720: .cell-stage 1164×488). A pane's family and
  // orientation are classified from ITS share of that box, so a chart in a pane lays out
  // for the pane, not for the whole slide. Type stays slide-sized: `--_sec-1cqi` is px.
  const STAGE_FRAC = { w: 1164 / 1280, h: 488 / 720 };
  ...
  function paneGeometry(geom, layout, index) {
    const w = parseFloat(geom.width) * STAGE_FRAC.w;
    const h = parseFloat(geom.height) * STAGE_FRAC.h;
  ...
  function renderPane(pane, layout, globalBase, baseUrl, source) {
    const geom = paneGeometry(themes.geometryFor(globalBase.theme || 'lattice', globalBase.size), layout, pane.index);
  ...
  function renderHtml(body, globalBase, source, baseUrl, stats, page) {
    ...
    if (env.latticePanes.length) {
      for (const c of env.latticePanes) {
        ...
        try {
          for (const p of c.panes) p.html = renderPane(p, c.layout, globalBase, baseUrl, source);
        } finally {
          pinRenderSlide(null);
        }
      }
