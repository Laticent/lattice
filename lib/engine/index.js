...
function createEngine(opts = {}) {
  ...
  function renderPane(pane, layout, globalBase, baseUrl) {
    ...
    if (process.env.PANES_DEBUG) console.error('PANE>>', pane.cls, html);
    return html;
  }
  ...
  function renderHtml(body, globalBase, source, baseUrl, stats, page) {
    ...
    // PANES — carve any slide whose body is split between two components, render each
    // pane as an ordinary one-slide deck of its component through a parser built for the
    // PANE's box, and embed the results after the host's transforms (lib/core/panes.js).
    // A deck with no `<!-- pane: -->` marker returns the same string: byte-identical.
    const carved = panes.extract(body);
    body = carved.body;
    const renderedPanes = carved.panes.map((p) => {
      const layout = carved.layouts.find((l) => l.slot === p.slot);
      return { ...p, html: renderPane(p, layout, globalBase, baseUrl) };
    });
    ...
    if (renderedPanes.length) html = panes.embed(html, renderedPanes);
