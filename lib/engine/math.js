 ...
 * `<span class="mord text" style="color:#cc0000">` with the command's literal source as its text
 * and no error class anywhere. Testing `katex-error` alone therefore certified as clean a render
 ...
 * `#cc0000` is KaTeX's default `errorColor`; nothing in this repo overrides it, and a false
 * positive costs only the reflow (the author's own source renders instead). Found by the HARD
...
function displayBlock(src, output, reflow) {
  if (reflow) {
    ...
    if (flow.lines > 1) {
      ...
      if (!/katex-error/.test(out) && !/color:#cc0000/.test(out)) {
...
// `opts.output` — KaTeX render mode. Default 'htmlAndMathml' (matches marp-core's historical
// default), and NO shipping path asks for anything else. This comment used to say the emulator's
// fixed-page PDF path passes 'html' because the hidden MathML inflates `scrollWidth` into a stale
// overflow ring; `lattice-emulator.js` refutes both halves in its own comment — it re-tested them
// on a real render, neither reproduced, and it sets `mathOutput: 'htmlAndMathml'` for
// accessibility. Two sibling files in one subsystem asserted opposite things until a HARD RULE #25
// checker read them side by side. The only callers left are TWO unit tests —
// `test/unit/core/relationship.test.js` and `test/unit/core/chart-narration-export-parity.test.js`.
// (A previous commit's message said it had corrected this line to say that; it had not, and a
// fifth checker caught the claim as well as the count.)
//
