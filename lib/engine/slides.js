...
// the engine's own CSS hides (`section.lens-hole` in lib/base/base.lens-hole.css — it moved out of
// the stage rule because a `.form`-scoped selector let `form: off` un-hide every hole). A whole
// class TOKEN rather than `includes`, so neither a deck's own `lens-holder` nor its `lens-hole-note`
// is mistaken for one: the same token test `HOLE_CLASS_ATTR` applies in the kernel, and the three
// answers disagreeing is a defect this feature has already shipped once.
