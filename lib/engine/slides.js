 ...
 * custom property (`--header`, `--footer`, …) and, for `color`/`backgroundColor`/
 * `backgroundImage`, also as a REAL declaration. Inline style is the highest cascade tier
 ...
 * reachable.
 *
 * THIS HAS BEEN FIXED THREE TIMES. The first two attempts both tried to decide whether a
 ...
 *
 * The lesson is that approximating the tokenizer is the wrong shape of solution. This does
 * not parse the value. It makes a value structurally incapable of ending its declaration:
 *
 ...
 *
 *   · REAL DECLARATIONS cannot be quoted — `color: "red"` is not a color — so they are
 *     VALIDATED instead, against a deliberately narrow allowlist: no `; { } < > \ " '`,
...
/** Is the value ALREADY exactly one well-formed CSS string? Then it is inert as written and
 *  re-quoting it would change what a consumer reads (`--theme:"lattice"` is emitted
 *  pre-quoted and must stay that way). Checked, not assumed: matching outer quotes with no
 *  unescaped instance of that quote inside. */
function isWellFormedCssString(v) {
  if (v.length < 2) return false;
  const q = v[0];
  if ((q !== '"' && q !== "'") || v[v.length - 1] !== q) return false;
  for (let i = 1; i < v.length - 1; i++) {
    if (v[i] === '\\') { i++; continue; }
    if (v[i] === q) return false;                 // closes early — not a single string
  }
  return v[v.length - 2] !== '\\' || v[v.length - 3] === '\\';
}

/** A custom-property value, emitted so it cannot terminate its own declaration.
 *  Anything not already inert becomes a CSS string token, which no construct can escape. */
function quoteDeclValue(value) {
  const v = String(value);
  if (isWellFormedCssString(v)) return v;
...
function realDeclValueIsSafe(value) {
  ...
  if (/^url\([^()"'\\{}<>]*\)$/i.test(v)) return true;                       // url(unquoted)
  if (/^url\(\s*"[^"'()\\{}<>]*"\s*\)$/i.test(v)) return true;                 // url("quoted")
  if (/^url\(\s*'[^"'()\\{}<>]*'\s*\)$/i.test(v)) return true;                 // url('quoted')
