 ...
 *
 *  BUT `;` is legal, common and harmless inside a `url()` or a quoted string — a data URI
 *  is `url(data:image/svg+xml;base64,…)`, and CSS tokenizes both as single tokens, so the
 *  browser never reads that `;` as a separator. Dropping on a naive `/;/` would silently
 *  break every data-URI background. This scans instead, tracking url() and quote depth, so
 *  only a `;` that would really terminate the declaration rejects the value. */
function sanitizeDeclValue(value) {
  const v = String(value);
  let quote = null;   // "'" or '"' when inside a string
  let url = 0;        // depth of unquoted url( … )
  for (let i = 0; i < v.length; i++) {
    ...
    if (c === '(') { url++; continue; }
    if (c === ')') { if (url) url--; continue; }
    if (url) continue;              // inside url()/any function — a `;` here is inert
...
class InlineStyle {
   ...
   * engineering/decisions/2026-08-12-slide-plane-model.md § The injection the planes armed. */
  constructor(initial) {
    this.map = new Map();
    if (initial) {
