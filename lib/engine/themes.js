 ...
 * base theme and honoring the `size:` directive's `@size` geometry. The
...
const { maskCssComments } = require('../core/leading-is');

/** Pane-scoped sheets kept per engine (see `cssFor`); each is a full composed sheet. */
const PANE_SHEETS_KEPT = 8;
...
// scaffold-only CSS. See THEME_IMPORT_RE in css.js for the same fix, and note
// that css.js resolves the BASE import this method hands off at BASE_THEME —
// a THIRD content-side reader, quoted-only, which this grammar must agree with.
...
class ThemeStore {
   ...
   * fall through to composeCss's URL-import hoisting, the pre-fix behavior).
   ...
   *
   * COMMENTS ARE NOT CODE. A theme's own prose mentioning another palette used to
   * RESOLVE: `lib/theme/serialize.js` interpolates a Studio user's free-text
   * description straight into the header comment, so a description reading "like
   * @import 'onyx'; but warmer" spliced onyx — and, through onyx's own
   * `@import 'lattice';`, the base scaffold behind it, 768 KB of composed output —
   * into a theme that declared no parent. And because `composeCss` strips comments
   * AFTER, the rest of that sentence was left as live CSS, read as a selector,
   * swallowing the rule behind it. The scan runs over `maskCssComments` (quote- and
   * escape-aware, so a `/*` inside a string is not an opener) while the splice
   * applies to the ORIGINAL.
   */
  resolveThemeImports(cssText, seen) {
    const text = cssText || '';
    // SCAN a comment-masked copy, SPLICE against the original. Masking (blanking to
    // spaces, length preserved) means every offset still points at the same byte, so
    // bytes outside a matched directive come back as they went in.
    //
    // One nuance: what the directive SPANS is decided on the masked copy, so a comment
    // sitting where the trailing `;` would be is inside the match and goes with it —
    // `@import 'onyx'\n/* notes */` loses the notes. No shipped palette hits this and
    // `serialize.js` always emits the semicolon; composeCss strips comments anyway, so
    // it is invisible in composed output. Recorded because "untouched" is not exact.
    const masked = maskCssComments(text);
    let out = '';
    let at = 0;
    for (const m of masked.matchAll(THEME_NAME_IMPORT_RE)) {
      const full = text.slice(m.index, m.index + m[0].length);
      const importName = m[2];
      out += text.slice(at, m.index);
      at = m.index + m[0].length;
      if (importName === BASE_THEME) { out += full; continue; } // composeCss resolves the base
      if (seen.has(importName) || !this.byName.has(importName)) { out += full; continue; }
      seen.add(importName);
      out += this.resolveThemeImports(this.byName.get(importName), seen);
    }
   ...
   * rather than throwing. `flat` packs it for a flat document (the exported
   * player) instead of the preview — see `packSelector` in ./css.js.
   */
  cssFor(name, sizeName, { flat = false, panes = null } = {}) {
    if (!this.byName.has(name)) return '';
    // `panes`: the classes the deck's panes carry (lib/core/pane-css.js `paneClasses`), so the
    // sheet carries pane twins for exactly those components. Empty or null: no pane, plain sheet.
    const paneKey = panes?.length ? panes.join(' ') : '';
    const key = `${name}\u0000${sizeName || ''}\u0000${flat ? 'flat' : ''}\u0000${paneKey}`;
    const cached = this._cssCache.get(key);
    if (cached !== undefined) {
      // A pane sheet in use moves to the back of the eviction queue: least recently USED goes.
      if (paneKey && this._paneKeys) this._paneKeys = [...this._paneKeys.filter((k) => k !== key), key];
      return cached;
    }
    ...
    const css = composeCss({
      ...
      sizeName,
      flat,
      panes: paneKey ? panes : null,
    });
    if (paneKey) {
      // Each pane pairing composes its own sheet, and an editor mints a new pairing per edit.
      // Keep the most recent few so a long Studio session cannot grow the cache without bound.
      this._paneKeys = (this._paneKeys || []).filter((k) => k !== key);
      this._paneKeys.push(key);
      while (this._paneKeys.length > PANE_SHEETS_KEPT) this._cssCache.delete(this._paneKeys.shift());
    }
...
// THEME_NAME_IMPORT_RE is exported for ONE reason: it is the grammar that decides
// whether an `@import` resolves against the registry or survives into the composed
// sheet as a live fetch, and `lib/theme/gate.js` has to allow a strict SUBSET of it.
// That gate cannot require this module (it would drag the engine into the browser
// theme bundle), so it carries a pinned copy — and `test/unit/palette/theme-gate.test.js`
// runs THIS regex over every statement the gate allows to prove the subset relation.
// It is a `/g` regex: reset `lastIndex` before an `exec`, or share nothing.
module.exports = { ThemeStore, THEME_NAME_IMPORT_RE };
