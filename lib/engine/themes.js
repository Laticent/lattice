...
const { stripCssComments } = require('../core/strip-css-comments.mjs');
...
class ThemeStore {
   ...
   * @import 'onyx'; but warmer" spliced all 768 KB of onyx into a theme that
   * declared no parent — and because `composeCss` strips comments AFTER, the rest of
   * that sentence was left as live CSS, read as a selector, swallowing the rule
   * behind it. The scan runs over `stripCssComments` (quote- and escape-aware, so a
   * `/*` inside a string is not an opener) while the splice applies to the ORIGINAL.
   */
  resolveThemeImports(cssText, seen) {
    ...
    // the returned text is untouched apart from the directives actually replaced.
