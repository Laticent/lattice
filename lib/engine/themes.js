...
class ThemeStore {
   ...
   */
  cssFor(name, sizeName, { flat = false, panes = false } = {}) {
    if (!this.byName.has(name)) return '';
    // `panes`: the deck holds a pane, so the sheet carries the pane twins (lib/core/pane-css.js).
    const key = `${name}\u0000${sizeName || ''}\u0000${flat ? 'flat' : ''}\u0000${panes ? 'panes' : ''}`;
