...
function splitDeck(source) {
  // LINE ENDINGS: LF, no leading BOM. `^---` is what decides whether this deck HAS front
  // matter at all, and a BOM defeats that anchor exactly as a CRLF does — the whole
  // frontmatter block would then be read as body. Same reason and same idiom as
  // lib/core/resolve-palette.js; listed in SANCTIONED_EOL_BOUNDARIES.
  const text = String(source == null ? '' : source).replace(/^\uFEFF/, '').replace(/\r\n?/g, '\n');
  let frontmatter = '';
  let body = text;

  // Leading YAML frontmatter: starts at the very first line `---`, ends at the
  // next standalone `---`. Only recognized at offset 0 (a `---` mid-deck is a
