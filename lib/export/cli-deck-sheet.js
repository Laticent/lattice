 ...
 *      wrapper. Unlike the Studio's Webpage player (`share-export.ts`), which deletes it,
 *      this keeps "a top-level slide" as a condition (`unwrap`), so a `<section>` an author
...
/**
 * Unwrap the pack for a document whose slides sit in `<main>` rather than
 * `article.lattice`, WITHOUT turning a child-only slide selector into an any-depth one.
 *
 * `article.lattice > section` meant "a slide". Deleting the prefix, as the Studio's Webpage
 * player does, leaves `section`, which also matches a raw `<section>` an author nests inside a
 * slide: measured, it took the scaffold's 1280×720 box, `overflow:hidden` and
 * `container-type:size`, and inside a `dark` slide the re-declared light tokens painted it as
 * a white panel, clipping both slides. So the prefix becomes a condition on the slide itself:
 * `section:where(:not(section *))` — a section with no section above it — whose specificity is
 * exactly plain `section`'s, so no rule moves against any other. The packed `:root` arm,
 * `:where(section)`, becomes `:where(section:not(section *))` and stays at zero.
 */
function unwrap(css) {
  return css
