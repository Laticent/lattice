...
// `transparent` keeps the marks as the slides rendered them and bakes no canvas (drops onto
// anything). `light`/`dark` render the chart/diagram in that scheme on a white / dark canvas.
// `print` renders it B&W-safe (grayscale + textures) on white — so you can export color
// slides but print-ready chart/diagram vectors for a document. Canvas fills are neutral
// literals (not theme tokens) so a detached file is self-describing.
const SVG_BACKGROUNDS = ['transparent', 'light', 'dark', 'print'];
const SVG_BACKGROUND_FILL = { transparent: null, light: '#ffffff', dark: '#111317', print: '#ffffff' };
// The render treatment a look implies (null = leave as the slide render). Surfaces apply this
...
const SVG_LOOK_MODE = { transparent: null, light: 'light', dark: 'dark', print: 'print' };
