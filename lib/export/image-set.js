...
// Charts that render as ONE self-contained <svg>, so a standalone vector file can be
// extracted for them. Projected from each manifest's `projection.figure === "svg"`
// (lib/core/projection-catalog.generated.mjs) — this list, CHART_TOKEN_COMPONENTS in
// the prose projection, CLEAN_SVG_LAYOUTS in the Studio export and a third copy inside
// a page.evaluate in tools/export-chart-svg.js were four hand-written copies of the
// same twelve names, and a chart missing from one silently exported as a raster PNG.
// `require()` of an .mjs is the same bridge lib/theme/edges.generated.mjs already uses
// from CJS (Node >= 22.12, pinned in package.json); the catalog is frozen data, so the
// module stays as pure and dependency-free as the header promises.
const { SVG_CHART_LAYOUTS } = require('../core/projection-catalog.generated.mjs');
const KEYED_CHART_LAYOUTS = SVG_CHART_LAYOUTS;
