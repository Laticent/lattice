...
// (drops onto anything) — the parallel to the color mode's `auto`. `light`/`dark` render the
...
/** PNG: insert a `pHYs` chunk (pixels-per-meter, unit=1) right after IHDR. */
...
function deckSlug(name) {
  ...
  // Windows reserved device names (CON, NUL, COM1, …) can't be a file/folder name there — prefix them.
...
function buildImageSetManifest(p) {
  ...
  // Thumbnail geometry (uniform across the set) — only when thumbnails were packed.
  if (opts.thumbnails && thumbs.length) {
    const ts = resolveThumbScale(opts.thumbWidth, geom.w);
