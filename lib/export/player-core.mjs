...
//
// These are (0,2,1) against the engine rule's (0,1,1), so they win in both directions. The
...
const SYSTEM_LOGO_SEL = 'section[data-lattice-slide].color-system:not(.print)';
...
// The bookends, as their own selector: dark in every scheme, print excepted (a printed
// bookend is paper — the engine says so with the same `:not(.print)`).
//
// SPELLED AS TWO SELECTORS, NOT `:is(.title, .closing)`. The first cut used `:is()` and this
// file's own test caught it: the selector-list forms are Safari-14-era, and an engine that
// cannot parse one drops the WHOLE rule — which for this rule means every bookend loses its
// dark tokens on exactly the old engines least able to recover. The comma-joined pair costs
// a few bytes and degrades per-arm instead of all at once.
const BOOKEND_SEL = BOOKEND_FORCES_DARK
	.map((b) => `section[data-lattice-slide]${b}:not(.print)`)
	.join(',');
