 ...
 */
export function playerCss(narration = false, captions = narration) {
	return `
...
var playBtn=document.querySelector('#lp-bar > #lp-play'),capBand=document.querySelector('#lp-app > #lp-caption');
...
export async function playerJs(animaJs = '', beats = null, captions = !!beats) {
	...
	const capKernel = captions ? `var makeCursor=${(await import('@slidewright/cadenza')).makeCursor.toString()};\n` : '';
	const kernel =
		capKernel +
