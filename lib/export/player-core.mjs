...
export async function assemblePlayer(data, caps) {
	...
	//
	// WHICH SECTIONS ARE HOLES COMES FROM THE PROJECTION, NOT FROM THE DOM. This filter used to ask
	// `classList.contains('lens-hole')` of a document that is a `cloneNode(true)` of the LIVE page —
	// so a deck whose own script strips the class after the visibility check has looked leaves no
	// holes to find, and every withheld slot became a real frame. Measured on the real CLI: a
	// `--player --lens brief` export of a six-slide deck keeping three shipped SIX `.lp-frame`s, all
	// six `data-authored-slide` stamps, and a Read·Article entry per withheld slot — at exit 0,
	// beside the run's own `brief — 3 of 6 slides ship`. No withheld body text escaped, but the
	// deck's real length and the exact withheld positions did, which is the whole reason the frames
	// are dense.
	//
	// `lensKept` is the authored index set from `LENS_PROJECTION`, handed in at build time and
	// deliberately NOT part of `lensProjection` — that object is serialized into the envelope, and
	// publishing the kept set there would disclose the withheld positions in the very file this
	// scrub exists to keep them out of.
	const keptSet = Array.isArray(data.lensKept) ? new Set(data.lensKept) : null;
	const authoredStamp = (el) => {
		const raw = el.getAttribute('data-authored-slide');
		return raw === null ? null : Number(raw);
	};
	const isHole = (el) => (keptSet
		// A section the render did not number cannot be matched against the projection, so it is
		// treated as a hole rather than shipped — the safe direction for a disclosure filter.
		? (authoredStamp(el) === null || !keptSet.has(authoredStamp(el)))
		: el.classList.contains('lens-hole'));
	const shipped = [...doc.querySelectorAll('section[data-lattice-slide]')].filter((el) => !isHole(el));
	for (const el of [...doc.querySelectorAll('section[data-lattice-slide]')].filter(isHole)) el.remove();
	...
	// AND THE STAMP IT WAS READ FROM COMES OFF, for the reason the rank is dense in the first place.
	// `data-lp-i` was made a dense rank precisely so a recipient could not count the deck — "measured
	// on an 8-slide deck carrying two views, the frames read `data-lp-i="0,2,6"` in plaintext, the
	// count and the exact withheld slots". The `<section>` inside each frame kept its own
	// `data-authored-slide`, so one `grep` on a real carrier read `0 2 3 5` beside frames stamped
	// `0 1 2 3`: the same disclosure, one element deeper, in a file the same function had carefully
	// scrubbed of holes.
	//
	// Removed only when this is a REDUCING projection — the number is not a disclosure when nothing
	// was withheld, and a full-deck player has no reason to lose it. Nothing downstream reads it:
	// `denseRank` and `authoredPerPage` are both derived above, `buildArticle` is handed the mapping
	// rather than the attribute, and `lib/runtime` never mentions it (grepped).
	//
	// GATED ON THE PROJECTION, NOT ON THE VIEW COUNT. This read `lensViews && data.lensProjection`,
	// and `lensViews` is non-null only past TWO views (see above) — so the ordinary single-view
	// export, `--lens brief --player`, kept the authored stamp on every frame and published the
	// withheld slots with one `grep`. The number is a disclosure whenever something was withheld,
	// which is what `lensProjection` already means; how many views ride along is beside the point.
	if (data.lensProjection) {
		for (const el of shipped) el.removeAttribute('data-authored-slide');
	}
