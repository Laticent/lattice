 ...
 * (roadmap, progress, …) sizes in `cqi` and a spatial one (word-cloud) in `cqi` against a
 * definite box; the slide's cell-stage gave them that container, and the article figure has
 * to give it back. `playerCss()` writes these under `#lp-article` (see the comments there
...
export function rehostContainerCss(root) {
	return [
		`${root} .lp-spatial{container-type:size;width:100%;position:relative;overflow:hidden;display:grid;grid-template-rows:minmax(0,1fr) auto}`,
		`${root} .lp-spatial>.chart-body{width:100%;height:100%;min-height:0}`,
		`${root} .lp-spatial.word-cloud{aspect-ratio:3/2}`,
...
/* word-cloud packs SVG text at viewBox coords + a %-positioned key, so it needs a DEFINITE
   ...
   height. The box is two grid rows: the re-hosted chart-body fills the first, and the
   figcaption takes the second. With the body filling the whole box, the caption sat below it
   and overflow:hidden clipped it, so no word cloud ever showed its caption. */
#lp-article .lp-spatial{container-type:size;width:100%;position:relative;overflow:hidden;display:grid;grid-template-rows:minmax(0,1fr) auto}
#lp-article .lp-spatial>.chart-body{width:100%;height:100%;min-height:0}
