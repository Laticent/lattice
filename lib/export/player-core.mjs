...
import { deckMotionScalars, playerMotionSuppressed } from '../core/resolve-motion.mjs';
...
${beats ? `${narrationJs(beats)}\n` : ''}setView('present');
	...
	if (chart?.js) {
		const deck = JSON.stringify(chart.deck ?? { motion: null, style: null, speed: null });
...
export async function assemblePlayer(data, caps) {
	...
	const playerMotionOff = playerMotionSuppressed(data.source || '');
	const hasChartMarks = /<(?:polygon|rect|path|circle|ellipse|line|polyline|g|text)\b[^>]*\sdata-anima-role=/.test(docHtml);
	// A per-slide `motion-*` class opts in without any deck-level key, so the markup is checked
	// too rather than trusting the front matter alone.
	const wantsChartMotion = chartMotion.motion === 'on' || /<section\b[^>]*\sclass="[^"]*\bmotion-(?:on|build|together|rise)\b/.test(docHtml);
	const hasChart = hasChartMarks && wantsChartMotion && !playerMotionOff;
