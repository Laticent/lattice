...
import { buildEnvelope, buildReadAlong, LTT_BLOCK_MIME as DOC_LTT_MIME } from '../core/lattice-doc.js';
...
import { slideEdgeKCss, slideFrameShadow } from '../core/slide-frame.mjs';
import { ANIMA_CHART_JS, ANIMA_PLAYER_JS } from './anima-player-bundle.generated.mjs';

/** The `<script>` type a narrated slide's CLIPS ride in. Not executable — the same
 *  inert-data-block idiom the manifest envelope uses. */
export const AUDIO_BLOCK_MIME = 'application/lattice+audio';
/** The `<script>` type the deck's timing track rides in: the packed LTT (engineering/ltt.md). */
export const LTT_BLOCK_MIME = DOC_LTT_MIME;

/** Inert JSON for a `<script>` block. Every `<` becomes the JSON escape `<`, so the HTML
 *  parser never sees one inside the block — neither `</script` nor the `<!--` that would flip it
 *  into script-data-escaped state can appear — while `JSON.parse` decodes it back exactly.
 *  HTML-entity escaping would be the WRONG tool: a `<script>` element's content is raw text, so
 *  `&amp;` would survive into the parsed JSON verbatim and corrupt every caption with an ampersand. */
function inertJson(value) {
	return JSON.stringify(value).replace(/</g, '\\u003c');
}

/** base64 → lowercase hex. `caps.sha256` returns base64 (the CSP wants it that way); an LTT hash is
 *  hex. `atob` is global in every runtime this module runs in (Node 16+, every browser). */
...
/**
 * Serialize a deck's baked narration: the deck's LTT, packed, in ONE inert block, and each narrated
 * slide's clips in an inert block of their own.
 *
 * WHY THE TIMING AND THE AUDIO RIDE APART. The LTT is small (4–10 KB packed for a typical deck) and
 * the transport needs all of it on the first Play: every slide's arrival hold and every cue's
 * timing. The clips are megabytes, and the player needs ONE slide's at a time. So the clips stay
 * split per slide, as they have been since #1393: a viewer who presses Play parses the LTT and the
 * first slide's clips, and a viewer who never presses it parses nothing. Each clip's `src` in the
 * LTT names its place in those blocks (`#lp-audio/<block>/<n>`, `exportClipSrc`).
 *
 * WHY NOT INSIDE THE MANIFEST ENVELOPE, which already has a `readAlong` slot. `lattice-doc.js`
 * base64s the WHOLE manifest, deliberately, so no deck content can terminate its script element.
 * Audio nested inside it would be base64'd twice (1.78x over raw), and reaching it would mean
 * decoding and parsing the entire manifest to play one sentence.
 *
 * THE BREAKOUT GUARD. Caption text is deck content, so both blocks are written with `inertJson`.
 * `src` admits ONLY a `data:` URI: a remote URL reaching here would quietly make a
 * "self-contained, offline" file phone home, which is the player's whole contract.
 *
 * @param {{captions?: boolean, slides?: Array<null | {text: string, track: object,
 *   clips?: Array<null | {audio?: string|null, clip?: string, leadMs?: number}>,
 *   emphasis?: Array<{start: number, end: number, weight: number}>}>,
 *   inputs?: {pace?: string, lang?: string, lexicon?: object, acronyms?: object},
 *   voice?: {model: string, voice: string, speed: number} | null} | null | undefined} narration
 *   per-slide narration, index-aligned to the deck's slides; `clips` index-aligned to the cues
 * @param {{slideCount: number, isSection: (i: number) => boolean, beats: {slide: number, section: number},
 *   deckPace?: string|null, id?: string, sha256: (s: string) => Promise<string>}} deck
 * @returns {Promise<{blocks: string, ltt: object|null, hasCaptions: boolean, hasAudio: boolean}>}
 *   `blocks` is '' and `ltt` null when nothing on any slide speaks
 */
export async function narrationPayload(narration, deck) {
	const none = { blocks: '', ltt: null, hasCaptions: false, hasAudio: false };
	// The pre-LTT shape was an array of per-slide cue lists. `lib/*` is a published export, so an
	// embedder still passing it gets a loud error rather than a deck that silently stops speaking.
	if (Array.isArray(narration)) {
		throw new TypeError('assemblePlayer: `narration` is { slides, inputs?, voice?, captions? } since LTT step 2 (engineering/ltt.md) — an array of cue lists is the old shape.');
	}
	const slides = Array.isArray(narration?.slides) ? narration.slides : [];
	if (!slides.some((s) => s?.track?.cues?.length)) return none;
	const [{ deckLtt, exportClipSrc }, { pack }, { ENGINE_HASH }] = await Promise.all([
		import('../core/ltt-deck.mjs'),
		import('@laticent/ltt'),
		import('@laticent/cadenza'),
	]);
	const uris = []; // per slide: the clips' data URIs, in the order the LTT lists them
	// Only what the LTT can place ships: a slide past the rendered deck, or a clip past its track's
	// last cue, would be megabytes nothing plays — and a `media-src` grant for them.
	const narrated = slides.slice(0, Math.max(deck.slideCount, 0)).map((s, i) => {
		if (!s?.track?.cues?.length) return null;
		let n = 0;
		const clips = (s.clips || []).slice(0, s.track.cues.length).map((c) => {
			if (!c || typeof c.audio !== 'string' || !c.audio.startsWith('data:')) return null;
			(uris[i] ||= []).push(c.audio);
			return { src: exportClipSrc(i, n++), clip: c.clip, leadMs: c.leadMs };
		});
		return { text: String(s.text ?? ''), track: s.track, clips, ...(Array.isArray(s.emphasis) && s.emphasis.length ? { emphasis: s.emphasis } : {}) };
	});
	const ltt = await deckLtt({
		id: deck.id,
		slides: narrated,
		slideCount: Math.max(deck.slideCount, 0),
		isSection: deck.isSection,
		beats: deck.beats,
		inputs: { ...(narration.inputs || {}), ...(deck.deckPace ? { deckPace: deck.deckPace } : {}) },
		voice: narration.voice || null,
		engine: ENGINE_HASH,
		sha256Hex: async (text) => base64ToHex(await deck.sha256(text)),
	});
	const out = [`<script type="${LTT_BLOCK_MIME}" data-lp-ltt="">${inertJson(pack(ltt))}</script>`];
	uris.forEach((list, i) => {
		if (list?.length) out.push(`<script type="${AUDIO_BLOCK_MIME}" data-lp-audio="${i}">${inertJson(list)}</script>`);
	});
	return {
		blocks: out.join('\n'),
		ltt,
		// CAPTIONS are the author's choice in the export panel, not a property of the payload any
		// more: every narrated segment carries its words, because the LTT's core requires them.
		// Captions off ships the voice and no band, and then no crawl code at all.
		hasCaptions: narration.captions !== false,
		hasAudio: uris.some((list) => list?.length),
	};
}
...
// The author-pinned-dark slide selector, with the pin set EXCLUDED — derived from
// PINNED_TO_LIGHT for the same reason the restore rules share it: so the two cannot drift.
//
// THE DRIFT ALREADY HAPPENED, and this is what it cost. The three emitters below each
// hand-wrote `:not(.print)`, so the rule fired on a slide the author had ALSO pinned light.
// The restore rules that would have corrected it are emitted only inside the
// `:root[data-lp-scheme=dark|system]` scopes, so in the player's LIGHT scheme no restore
// exists at all and `.dark` simply won. Measured on a real export, default scheme: a
// `dark light` slide and a `divider light dark` slide both came back `rgb(0,29,51)` where
// the PDF renders them `rgb(255,255,255)` (#2158).
//
// In the engine there is no contest — `section.light` follows `section.dark` in source
// order and wins on `color-scheme`. The player has no `color-scheme` to contest, only two
// flat token blocks, and until now only one of them was scoped.
//
// THE PIN SET IS NOT THE WHOLE STORY, AND THE FIRST CUT OF THIS SHIPPED A BLANK SLIDE.
// This selector gates the ENTIRE dark token block — `--bg`, every ink, every elevation —
// not just the two logo tokens #2158 set out to fix. The engine has a rule the player does
// not model: `section:is(.title, .closing):not(.print)` (base.modifiers.css, (0,2,1))
// forces `color-scheme: dark` and the on-dark panel EVEN WHEN `.light` is present, because
// a bookend IS a dark panel. Excluding `.light` wholesale therefore stripped the dark
// tokens from a `title light dark` slide while its ink stayed on-dark: white on white,
// 1.00:1, measured by this repo's own `check-player-contrast` as two NEW below-AA runs in
// the AS-EXPORTED view — the one the recipient opens.
//
// So the bookends are re-admitted — as separate ARMS, not inside `:where()`. A first cut
// wrote `:where(:not(.light):not(.color-light), .title, .closing)`, which is one compound and
// therefore suffixable, and which is also the exact selector-list form the `themeDualMode`
// comment below bars by name; the guard test could not see it, because its alternation listed
// `:is(` and a `:not(a,b)` shape and never `:where(`. Both are fixed: the arms are spelled out
// and the guard now checks the class it names.
//
// `keepAlwaysDark` LOST ITS `:where()` IN THE SAME PASS, and that is worth saying here because
// this comment is where a reader tracing a cascade surprise will look. Every restore rule moved
// (0,5,1) -> (0,7,1). Measured, nothing shifts: 7,680 synthetic cells across five emitter paths
// and 1,268,379 resolved values dumped from two real `--player` exports, zero differences.
//
// The arms measure (0,5,1) for the pin-excluded one and (0,4,1) for each bookend, against the
// (0,2,1) that draft claimed for the `:where()` form — which was itself wrong, since
// `[data-lattice-slide]`, `.dark` and the `:not(.print)` are three class-column entries before
// any carve-out. Nothing downstream shifts regardless: resolving `--bg` and
// `--deck-logo-filter` over all 256 class combinations x 3 player schemes in Chromium, the
// only cell that moves between the two spellings is the `.dark .color-system` one the
// `:not(.dark)` below fixes.
const BOOKEND_FORCES_DARK = ['.title', '.closing'];
//
// AND THE RESTORES NEED THE SAME CARVE-OUT, which is the half a first fix missed. Re-admitting
// bookends to the unconditional dark rule is not enough: on a `-dark` theme the player starts
// in its DARK scheme, where the restore-to-light rules fire — and those were still pinning a
// `title light dark` bookend to light while the engine's `section:is(.title,.closing):not(.print)`
// keeps it dark. Measured: the player rendered that slide on a white ground where the PDF
// renders it rgb(0,29,51).
//
// `.print` is deliberately NOT carved out. A printed bookend IS paper — the engine says so with
// the same `:not(.print)` on its own bookend rule — so the print band must keep restoring.
const ALWAYS_DARK = [
	{ cls: '.title', unless: '' },
	{ cls: '.closing', unless: '' },
	{ cls: '.divider', unless: ':not(.light)' },
];

// THE CARVE-OUT IS PER PINNED CLASS, because "always dark" is itself conditional. A
// `divider light` is NOT force-dark — the engine's own predicate says `divider:not(.light)`,
// and the bright variant replaces the canvas — so it must keep restoring to light. A
// `divider color-light` IS force-dark, so it must not.
//
// Driving both from ALWAYS_DARK is the point (HARD RULE #1). A first fix mirrored the engine
// predicate into ALWAYS_DARK_SEL and left THIS function on the two bookends, then claimed in
// its own message that the player "mirrors it whole". It did not, and a divider still
// rendered on-light ink on a permanently dark panel after a toggle to dark — 1.61:1, the same
// number and the same panel as the defect that commit closed.
//
// TWO NEARBY SELECTORS LOOK LIKE THE SAME MIRROR AND ARE NOT, which is worth saying so the
// next reader does not "fix" them. `BOOKEND_FORCES_DARK` answers *which classes stay dark
// DESPITE a light pin* — a `divider light` does not, it is the bright variant, so its absence
// there is correct. `SYSTEM_LOGO_SEL` answers *whose ground follows the receiver* — no divider
// does, in either variant, so excluding them all is correct. Only the two selectors that hand
// out dark TOKENS mirror the force-dark predicate itself.
//
// `.print` is deliberately NOT carved out. A printed panel IS paper — the engine says so with
// the same `:not(.print)` on its own rule — so the print band must keep restoring.
// THE FILTER ASSUMES A GRAMMAR, so the grammar is stated and pinned rather than left to be
// inferred from the one entry that currently has an `unless`. Each `unless` must be a chain of
// SINGLE-CLASS, SPACE-FREE `:not()` — `:not(.light)`, or `:not(.print):not(.light)`. The
// trailing `)` in the needle already defeats the substring hazard (`:not(.color-light)` is not
// dropped for the pin `.light`), but a re-spelling CSS would accept — `:not(.light, .bright)`
// or `:not( .light )` — fails the test in the UNSAFE direction: the divider stays in the
// `.light` carve-out, so a bright divider takes the dark token block in the player's dark
// scheme. Measured, that is a FIDELITY break rather than an illegibility one — the player
// renders 17.16:1 white-on-dark where the engine renders 18.13:1 dark-on-white, because
// `section.divider.light` sets `background: var(--bg)` and the ground moves with the ink.
// `test/unit/export/html-player.test.js` pins the grammar so the next entry cannot introduce
// that silently.
const keepAlwaysDark = (c) => {
  if (c === '.print') return `section[data-lattice-slide]${c}`;
  // An entry whose `unless` already excludes this pin cannot be force-dark under it.
  const carve = ALWAYS_DARK
    .filter(({ unless }) => !unless.includes(`:not(${c})`))
    .map(({ cls }) => `:not(${cls})`)
    .join('');
  return `section[data-lattice-slide]${c}${carve}`;
};

// `.color-system` FOLLOWS THE PLAYER'S SCHEME, NOT THE OS — and the engine rule for it
// follows the OS, which is why the player needs its own pair. (#2201)
//
// `base.modifiers.css` gives a `color-system` slide its inverse mark inside
// `@media (prefers-color-scheme: dark)`, which is right on every surface with no toggle:
// the ground follows the receiver and so does the mark. The player HAS a toggle, and it
// overrides the OS — so on the two cells where they disagree the engine rule is wrong, and
// one of those cells was CORRECT before that rule existed:
//
//   os=dark  + viewer picks light  ->  ground light, engine rule still fires  -> inverse on white
//   os=light + viewer picks dark   ->  ground dark,  engine rule never fires  -> light on dark
//
// These are (0,11,1) against the engine rule's (0,1,1), so they win in both directions — and
// counting them as (0,2,1), as a first draft did, is what hid the bookend collision above:
// at (0,2,1) they would merely TIE the engine's force-dark rule, not steamroll it. The
// `system` arm needs no light half: with the OS light the engine's media query does not
// fire and the mark is already light.
//
// `:not(.print)` for the reason every rule in this file carries it — a printed slide is
// paper, and these would otherwise outrank `section.print`'s own (0,1,1) reset.
// EVERY SLIDE WHOSE GROUND DOES NOT FOLLOW THE OS IS EXCLUDED, and the first cut excluded
// only `.print`. `color-system` is a DECK-WIDE stamp, so the engine puts it on the bookends
// and dividers too — and at (0,11,1) these rules steamroll the engine's own (0,2,1) force-dark
// rule. The result was #2156's defect rebuilt: on a `color-mode: system` deck the player's
// LIGHT scheme reset the mark to its light-canvas treatment on a panel that is permanently
// rgb(0,61,102), so the title, divider and closing slides looked as though they had no logo.
// Measured, mean |ΔL| of the mark against its ground collapsed 19.15 -> 2.07.
//
// The two pinned-light exclusions are a GUARD, not a reachable case, and saying which is the
// point: `slidePinEvictsDeckToken` (`lib/core/color-mode.js`) drops the deck-wide token from
// any slide that pins its own scheme, so `color-system` and `light` cannot co-occur from front
// matter. They are kept because they cost no specificity and this selector is also reached by
// hand-authored markup, but a comment that presented them as live cases would be describing
// something the propagation kernel already prevents.
//
// `.dark` IS ON THAT LIST FOR THE SAME REASON, and was missing from it until a checker asked
// which combination this selector still over-reaches on. `slidePinEvictsDeckToken` evicts
// `color-system` from a `.dark` slide exactly as it does from a `.light` one, so front matter
// cannot produce the pair — but hand-authored `<section class="dark color-system">` can, and
// there `DARK_SLIDE_SEL` hands the slide a permanently dark ground while this rule reset its
// mark to the light-canvas treatment. That is #2156's defect rebuilt one class over. Measured
// over the 256-combination matrix, it was the single disagreeing cell.
//
// What is left is the set whose ground really does follow the receiver.
const SYSTEM_LOGO_SEL = 'section[data-lattice-slide].color-system'
	+ ':not(.print):not(.light):not(.color-light):not(.dark)'
	+ ':not(.title):not(.closing):not(.divider)';
const SYSTEM_LOGO_INVERSE = '{--deck-logo-filter:var(--deck-logo-filter-inverse);'
	+ '--deck-logo-opacity:var(--deck-logo-opacity-inverse)}';
const SYSTEM_LOGO_RESET = '{--deck-logo-filter:initial;--deck-logo-opacity:initial}';
const SYSTEM_LOGO_RULES =
	`:root[data-lp-scheme=dark] ${SYSTEM_LOGO_SEL}${SYSTEM_LOGO_INVERSE}`
	+ `:root[data-lp-scheme=light] ${SYSTEM_LOGO_SEL}${SYSTEM_LOGO_RESET}`
	+ `@media (prefers-color-scheme:dark){`
	+ `:root[data-lp-scheme=system] ${SYSTEM_LOGO_SEL}${SYSTEM_LOGO_INVERSE}}`;

// THE ENGINE'S FORCE-DARK SET, MIRRORED WHOLE. `base.modifiers.css` forces
// `color-scheme: dark` on `section:is(.title, .closing):not(.print)` AND on
// `section.divider:not(.light):not(.print)` — a divider is a dark panel too, unless it is
// the bright `divider.light` variant, which REPLACES the canvas and keeps the light scheme
// it declares. The first cut of this block copied only the two bookends, so a `divider`
// kept the 1.61:1 ink this rule exists to fix. Mirroring two thirds of a predicate is how
// the class-vs-canvas defect keeps coming back.
//
// SPELLED AS SEPARATE SELECTORS, NOT `:is(.title, .closing)`. The first cut used `:is()` and
// this file's own test caught it: the selector-list forms are Safari-14-era, and an engine
// that cannot parse one drops the WHOLE rule — which for this rule means every bookend loses
// its dark tokens on exactly the old engines least able to recover. Comma-joined arms cost a
// few bytes and degrade one at a time instead of all at once.
const ALWAYS_DARK_SEL = ALWAYS_DARK
	.map(({ cls, unless }) => `section[data-lattice-slide]${cls}${unless}:not(.print)`)
	.join(',');

//
// SPELLED AS ARMS, AND EVERY CONSUMER DISTRIBUTES OVER THEM. The set this selects is a
// DISJUNCTION — `.dark` and not `.print`, and then either (not `.light` and not
// `.color-light`) or a bookend — which no single compound can express without a selector
// list. `:where(:not(.light):not(.color-light), .title, .closing)` did express it, in one
// compound, and that is why the first cut used it; it is also the exact banned form, in the
// one block whose sibling comment bars it by name.
//
// The cost of unpicking it is that a comma-joined LIST CANNOT CARRY A SUFFIX: `a,b .x`
// parses as `a` and `b .x`, so appending ` .lp-sd-0` or `.title.spectrum::before` to a
// joined form silently drops the other arms' suffix and leaves those arms selecting the
// bare section. Two tests in this file caught exactly that. So the arms stay an ARRAY and
// `scopeSelector`/`at()` cross-product over them; only a standalone rule may join.
const DARK_SLIDE_BASE = 'section[data-lattice-slide].dark:not(.print)';
const DARK_SLIDE_ARMS = [
	`${DARK_SLIDE_BASE}${PINNED_TO_LIGHT.filter((c) => c !== '.print').map((c) => `:not(${c})`).join('')}`,
	...BOOKEND_FORCES_DARK.map((cls) => `${DARK_SLIDE_BASE}${cls}`),
];
const DARK_SLIDE_SEL = DARK_SLIDE_ARMS.join(',');
...
export function hoistInlineLightDark(doc) {
	...
	const at = (prefix, arm) =>
		[prefix].flat().map((p) => rules.map((r) => `${p} .${r.cls}{${bang(r[arm])}}`).join('')).join('');
	const restore = (scope) => PINNED_TO_LIGHT.map((c) => at(`${scope} ${keepAlwaysDark(c)}`, 'light')).join('');
	return (
		at(':root[data-lp-scheme=dark]', 'dark') +
		restore(':root[data-lp-scheme=dark]') +
		at(DARK_SLIDE_ARMS, 'dark') +
...
const SCHEME_SCOPES = [
	...
	...PINNED_TO_LIGHT.map((c) => ({ root: ':root[data-lp-scheme=dark]', section: keepAlwaysDark(c), arm: 'light', media: '' })),
	{ root: '', section: DARK_SLIDE_ARMS, arm: 'dark', media: '' },
...
function scopeSelector(selectorList, scope) {
	// `scope.section` may be several compounds (a disjunction no single compound expresses —
	// see DARK_SLIDE_ARMS). Each one is spliced into each arm separately: joining them first
	// and splicing once would bind the arm's own suffix to the last compound alone.
	// `.length`, not truthiness: an empty array is truthy, and `sections = []` would make the
	// `flatMap` below return nothing, `scopeSelector` return '', and the emitter write a rule
	// with an empty prelude — invalid CSS that can swallow the rule after it. No SCHEME_SCOPES
	// entry can produce that today (DARK_SLIDE_ARMS always has arm 1), so this is a guard.
	const sections = [scope.section].flat().filter(Boolean);
	if (!sections.length) sections.push('');
	return selectorArms(selectorList)
		.flatMap((arm) => {
			const above = scope.root ? `${scope.root} ` : '';
			return sections.map((section) => {
				if (!section) return `${above}${arm}`;
				...
				if (/^section(?![\w-])/.test(arm)) return `${above}${section}${arm.slice('section'.length)}`;
				return `${above}${section} ${arm}`;
			});
		})
		.join(',');
}
...
export function themeDualMode(css) {
	...
	const pinned = (scope) =>
		PIN_LIGHT.map((c) => `${scope} ${keepAlwaysDark(c)}:not(.print)`).join(',');
	const darkBlock =
		...
		`${DARK_SLIDE_SEL}{${body}}` +
		// A BOOKEND IS A DARK PANEL IN EVERY SCHEME, and until now it only got dark TOKENS
		// when the whole root was dark. `section:is(.title, .closing):not(.print)` forces
		// `color-scheme: dark` in the engine and paints `--surface-inverse`, so after a
		// toggle to light the panel stayed dark while `:root` handed it light ink.
		// Measured on `examples/deck-logo-dark-theme.md` p1 (`title silent`) with
		// `check-player-contrast`: a `strong` came back rgb(10,22,40) on rgb(0,61,102),
		// 1.61:1, in the after-the-toggle view.
		//
		// This is the same shape #2158 fixed one level in — the restores live only inside
		// the dark scopes, so the player's LIGHT scheme had no rule re-asserting dark for a
		// slide that never stops being dark. DARK_SLIDE_SEL does not cover it: that one
		// needs an author's `.dark` class, and a plain `title` carries none.
		`${ALWAYS_DARK_SEL}{${body}}` +
...
/**
 * The container rules a RE-HOSTED chart figure needs, under any article root. A flow chart
 * (roadmap, progress, …) sizes in `cqi`, which the slide's cell-stage gave it and the article
 * figure has to give back; a spatial one (word-cloud) is one viewBox SVG that fills the
 * figure's width and takes its height from its viewBox. `playerCss()` writes these under `#lp-article` (see the comments there
 * for why each rule exists); the Studio's Reading view writes them under its own root, and
 * `check:render`'s Reading pass lays its article out with the same call. A unit test pins
 * that the player's copy matches this one, so the two hosts cannot drift apart.
 * @param {string} root - the article root selector, e.g. `.st-read-article`
 */
export function rehostContainerCss(root) {
	return [
		`${root} .lp-spatial{container-type:inline-size;width:100%;position:relative}`,
		`${root} .lp-spatial>.chart-body{width:100%;box-sizing:border-box;padding-inline:2%}`,
		`${root} .lp-spatial.word-cloud .word-cloud-canvas{width:100%;height:auto;max-width:none;max-height:none}`,
		`${root} .lp-spatial.word-cloud .wc-svg{height:auto}`,
		`${root} .lp-chart{container-type:inline-size;width:100%;overflow-x:auto}`,
		`${root} .lp-chart>.chart-body{width:100%}`,
	].join('\n');
}

export function playerCss(narration = false, captions = narration, canvas = PLAYER_CANVAS) {
	const CW = canvas.w;
	const CH = canvas.h;
	const ladder =
		CW === PLAYER_CANVAS.w
...
.lp-js [data-lp-view=present] .lp-frame.lp-active{display:block;
 ...
 box-shadow:${slideFrameShadow('stage')};--slide-edge-k:${slideEdgeKCss(`${CW} * var(--lp-fit-present,.5)`)}}
...
/* The frame carries the LIFT only; the EDGE is the engine's (base.modifiers.css, "The
   slide's EDGE"), drawn inside the slide in the deck's own --border. The frame's one job
   for it is --slide-edge-k: 100 / the slide's on-screen width, so the engine can keep
   its keyline at a true 1px through the scale (lib/core/slide-frame.mjs). Frame chrome
   drawn here broke three different ways: a border on the scaled section shrank to a
   sub-pixel hairline; a border + 12px radius on this frame rounded every slide of a
   SQUARE deck and cut the edge at each corner in the no-JS floor (the phone report); and
   a drop-shadow edge on this frame was clipped away by its own overflow:hidden in WebKit.
   The lift is a negative-spread box-shadow, which a box's own overflow never clips. */
...
[data-lp-view=read-slides] .lp-frame{flex:none;width:calc(${CW}px * var(--lp-fit,.28));height:calc(${CH}px * var(--lp-fit,.28));overflow:hidden;
 box-shadow:${slideFrameShadow('card')};--slide-edge-k:${slideEdgeKCss(`${CW} * var(--lp-fit,.28)`)}}
...
/* UN-TRIM the Read view. guards:strict clamps text so a SLIDE's fixed cell stops
   short of shearing; this column scrolls and has no fit problem, so the same clamp
   is pure content loss — a reader who switched to Read view precisely to get the
   full text was handed the seven-line truncation instead. The projection clones the
   live slide DOM, so the inline clamp rode in with it. Safe to undo here and only
   here: -webkit-line-clamp truncates VISUALLY and removes nothing, so the whole
   text is already in this DOM. Found by an independent review; the design note's
   open problem 8 predicted exactly this surface and nothing stripped it.
   display:revert, NOT display:block: list-item is one of the four trimmable
   roles, so this selector lands on <li>, and block demotes it out of
   display:list-item — the bullet vanishes and an <ol> stops incrementing, so a
   three-item list reads 1, blank, 2. Reproduced on this feature's own demo deck by
   a third review.
   NOTE: this block is inside a JS template literal — no backticks. */
#lp-article [data-lattice-trimmed]{display:revert!important;-webkit-line-clamp:none!important;overflow:visible!important}
/* The roster: a portrait beside the person's line, the words in their own wrapper.
   Which combinator each rule uses is load-bearing, and the split is the whole point.

   CHILD, for the row and the portrait. A roster row re-emits the author's own note
   markup verbatim, so a DESCENDANT form reached into it and turned a nested list under
   a person into flex items: markers gone, the sub-list floated beside the sentence.

   DESCENDANT, deliberately, for the two rules inside the wrapper — and the SHAPE that
   needs it is worth naming precisely, because an earlier version of this comment named
   the wrong one and a checker falsified it in a single deck. An image-only name
   (- ![Acme](wide.svg)) does NOT need it: with no text there is no <strong>, so the image
   is a direct child of the wrapper and >li>div>img already matches. What needs the
   descendant form is a name carrying TEXT AND an image (- Acme Corp ![logo](wide.svg)):
   that goes through the <strong> branch, the image becomes a grandchild, and with a child
   combinator it escapes every cap in the article — measured, a 900x600 source took the
   article's scrollWidth to 932 against a 390px column. Same for the list rule: a child
   combinator left a list nested one level deeper on the generic article margin, 19.8px of
   dead space inside the row. Inside the wrapper we WANT to reach the author's markup;
   outside it we must not. Re-testing only the image-only name will show the child form
   working and invite a "consistency" fix that restores the real defect.

   align-self is flex-start, not center. The wrapper makes rows as tall as their content,
   so a centered portrait sank with them: measured 31.9px below the name's center on a
   three-line row, 2.6px after. It costs NOTHING on a single-line row — there the 1.9em
   portrait is the tallest flex item, so center and flex-start place it identically.

   The list rule uses a margin, and that is deliberate rather than an oversight. It was
   briefly respelled margin:0 + padding-top to look like HARD RULE #20 compliance; that
   rule's substance is about layouts that MEASURE (padding is visible to
   getBoundingClientRect, margin is not), and this article is plain document flow with no
   fit pass. The respelling bought nothing and cost real geometry: padding does not
   collapse with the first list item's own .3em margin, so the gap above the first
   sub-bullet doubled 5.4px -> 10.8px and every row carrying a nested list grew 5.4px.
   Eight sibling declarations in this same stylesheet use margin shorthand for the same
   reason. (No backticks here: the stylesheet is a template literal.) */
#lp-article .lp-roster{list-style:none;padding-left:0}#lp-article .lp-roster>li{display:flex;align-items:baseline;gap:.6em}#lp-article .lp-roster>li>img{flex:none;width:1.9em;height:1.9em;border-radius:50%;object-fit:cover;align-self:flex-start}#lp-article .lp-roster>li>div{flex:1;min-width:0}#lp-article .lp-roster>li>div img{max-width:100%;height:auto}#lp-article .lp-roster>li>div :is(ul,ol){margin:.3em 0 0}
...
#lp-article .lp-subtitle{font-style:italic;color:var(--text-muted,#888);font-size:1.02em;margin:-.3em 0 1.1em}
#lp-article .lp-video{margin:0;padding:.2em 0 1.2em}
#lp-article .lp-video-link{display:flex;align-items:center;gap:.75em;width:100%;max-width:24em;box-sizing:border-box;padding:.65em 1.1em .65em .65em;border:1px solid var(--border,#ddd);border-radius:12px;background:var(--bg-alt,#f7f7f7);color:var(--text-heading,#111);font-weight:600;text-decoration:none}
#lp-article .lp-video-link:hover{border-color:var(--accent,#4338ca)}
#lp-article .lp-video-link:focus-visible{outline:2px solid var(--accent,#4338ca);outline-offset:2px}
#lp-article .lp-video-thumb{display:grid;place-items:center;flex:none;width:7.5em;max-width:40%;aspect-ratio:16/9;border-radius:7px;border:1px solid var(--border,#ddd);background:var(--bg,#fff) center/cover no-repeat}
#lp-article .lp-video-play{position:relative;flex:none;width:2em;height:2em;border-radius:50%;background:var(--accent,#4338ca)}
#lp-article .lp-video-play::after{content:"";position:absolute;left:54%;top:50%;width:.62em;height:.72em;transform:translate(-50%,-50%);background:var(--on-accent,#fff);clip-path:polygon(0 0,100% 50%,0 100%)}
#lp-article .lp-video figcaption{text-align:left;padding:.5em 0 0;font-size:.88em;color:var(--text-muted,#777)}
...
/* CHART SVGs are viewBox-only + preserveAspectRatio="meet": width:100% fills
...
#lp-article .lp-figure svg[aria-roledescription]{width:auto;max-width:100%}
...
#lp-article .lp-figure.lp-diagram{--lp-fig-w:100%;--lp-fig-min-w:0px;--lp-fig-ratio:auto;overflow-x:auto}
#lp-article .lp-figure.lp-diagram[style] svg[aria-roledescription]{width:clamp(var(--lp-fig-min-w),min(100%,78vh * var(--lp-fig-ratio)),var(--lp-fig-w));height:auto;aspect-ratio:var(--lp-fig-ratio);max-width:none;max-height:none}
...
@media print{#lp-article .lp-figure.lp-diagram{overflow:visible}#lp-article .lp-figure.lp-diagram[style] svg[aria-roledescription]{width:min(100%,var(--lp-fig-w))}}
#lp-article .lp-figure img{max-width:100%;height:auto;display:block;margin-inline:auto}
/* SPATIAL figure (word-cloud): the cloud, its size key and its spine are ONE viewBox SVG, so
   the figure is as tall as that SVG at the figure's width, plus the caption. Breakout supplies
   the width (up to 1200px); the SVG takes its height from its own viewBox (1100x320 landscape,
   taller in portrait), so no aspect-ratio is written here. The article-wide
   .lp-figure svg{max-height:78vh} still caps it: a portrait cloud in a short window draws
   narrower than the column, centered by preserveAspectRatio, with the caption right under it. On the slide the canvas is pinned at
   85.9375cqi x 25cqi of the SLIDE; re-hosted, that pin sized it against the figure while a 3:2
   box left the lower two-thirds empty above the caption, so the canvas fills the width instead.
   The packer keeps each word's ANCHOR in the viewBox, and a glyph may breathe a few units past
   its edge (.wc-svg overflow:visible); the 2% inline padding is the room the slide's side
   margins gave it, so the leftmost word no longer meets the figure's edge.
   inline-size, not size: nothing here reads cqh/cqb, and a size container needs a definite
   height, which is what forced the aspect-ratio. */
#lp-article .lp-spatial{container-type:inline-size;width:100%;position:relative}
#lp-article .lp-spatial>.chart-body{width:100%;box-sizing:border-box;padding-inline:2%}
#lp-article .lp-spatial.word-cloud .word-cloud-canvas{width:100%;height:auto;max-width:none;max-height:none}
#lp-article .lp-spatial.word-cloud .wc-svg{height:auto}
/* FLOW-HEIGHT figure (roadmap / progress / kanban / gantt / timeline-list): a WIDTH container
   ...
   with content-driven height — they have no cqh/cqb, so no aspect lock.
...
html:not(.lp-js) .lp-frame{width:calc(${CW}px * var(--lp-fit));height:calc(${CH}px * var(--lp-fit));overflow:hidden;box-shadow:${slideFrameShadow('card')};--slide-edge-k:${slideEdgeKCss(`${CW} * var(--lp-fit)`)}}
html:not(.lp-js) section[data-lattice-slide]{width:${CW}px!important;height:${CH}px!important;transform:scale(var(--lp-fit));transform-origin:0 0}
 ...
 */
// RENDER MODE, in the script below, is for video export only (lib/export/video.mjs;
// engineering/decisions/2026-09-25-video-export.md §3, §9). The capture sets window.__lpRender = {log: []}
// before the script runs and plays the file on a clock it owns, where media does not play. So a voiced
// cue lasts its clip's measured length minus its lead (the LTT's measuredMs, which the capture fills),
// re-timed exactly as the media anchor would re-time it, on the same timer path a cue with no clip
// takes; a clip with no measured length falls back as a clip that failed to decode (rule 4). Each cue
// logs its start so the capture can place the audio at the player's own times. A viewer's browser
// never sets the flag, and a deck cannot: an element with id="__lpRender" is a named window property,
// and one with name="log" beside it would make .log truthy, so only a real array counts (DOM
// clobbering). These notes live here, outside the template, because comments inside it ship in every
// narrated export.
function narrationJs() {
	return `
// ---- baked narration -------------------------------------------------------
// The deck's timing is its LTT (engineering/ltt.md), packed into one inert block: every slide's
// arrival hold, every cue, every breath. The clips ride in per-slide blocks beside it. This script
// is the TRANSPORT: WHEN to move between cues and slides (the spec's rules 1-7). It restates no
// timing rule of its own. Every hold, silent-cue length and breath it waits comes from positionAt,
// inlined above, and so does the crawl's "which word now", so the player and every other LTT
// reader lay a segment out with the same code. The one length it does not take from positionAt
// is a clip's: that one ends when the audio does (rule 3).
...
var au=null,narPlaying=false,beatT=null,autoNav=false;
// Render mode: video export only (see narrationJs's header).
var RENDER=window.__lpRender&&Array.isArray(window.__lpRender.log)?window.__lpRender:null;
...
function quietAudio(){if(au){try{au.pause();}catch(e){}au.onended=null;au.onerror=null;au.onloadedmetadata=null;au.ondurationchange=null;au.onplaying=null;try{au.removeAttribute('src');au.load();}catch(e){}}}
function stopAudio(){clearBeat();stopCrawlLoop();gen++;quietAudio();}
// ---- the timing track --------------------------------------------------------
// Parsed on the first Play, never at load: a viewer who never presses Play parses nothing.
var ltt=null;
function loadLtt(){if(ltt)return ltt;ltt={segments:[]};
 var n=document.querySelector('script[data-lp-ltt]');
 if(n){try{var p=JSON.parse(n.textContent||'{}');if(p&&p.segments)ltt=p;}catch(e){}}
 return ltt;}
// A FRESH copy of slide i's segment, with its track unpacked. Fresh because a clip that fails
// to decode is dropped from it (nextCue), and a replay should try that clip again.
function segFor(i){var s=loadLtt().segments[i];if(!s)return null;var o={};for(var k in s)o[k]=s[k];
 if(s.track)o.track=unpackTrack(s.track);
 if(s.audio)o.audio={voice:s.audio.voice,clips:(s.audio.clips||[]).slice()};
 return o;}
// Slide i's clips: its audio block, parsed only when the slide is about to speak.
function clipsFor(i){var n=document.querySelector('script[data-lp-audio="'+i+'"]');
 if(!n)return [];try{var p=JSON.parse(n.textContent||'[]');return p&&p.length?p:[];}catch(e){return [];}}
// A clip's bytes, from its src: the fragment #lp-audio/<block>/<n> names entry n of a slide's audio
// block (engineering/ltt.md §Encodings), and a data: URI is the clip itself. Anything else — a path,
// a remote URL — plays nothing: this file is self-contained and offline by contract.
function clipUri(src,i,uris){var m=/^#lp-audio\\/(\\d+)\\/(\\d+)$/.exec(src||'');
 if(m)return +m[1]===i?(uris[+m[2]]||null):null;
...
// is rebuilt here — but the TIMING is not reimplemented: makeCursor and positionAt above are the
// LTT's own kernels, inlined verbatim.
var seg=null,cursor=null,held={cueIndex:0,wordIndex:0},cueFrom=0,cueLead=0,cueClips={};
...
function paintCrawl(){
 if(!capBand||!cursor||!capBand.firstChild)return;
 var inner=capBand.firstChild;if(!inner)return;
 var ci=Math.min(cursor.track().cues.length-1,Math.max(0,held.cueIndex)),wi=held.wordIndex;
...
// positionAt holds the last word through every pause, so the crawl never lurches back to line
// one at a full stop.
function tickCrawl(localMs){
 if(!capBand||!cursor||!seg)return;
 var p=positionAt(seg,localMs,cursor);
 if(p.cueIndex>=0)held={cueIndex:p.cueIndex,wordIndex:p.wordIndex};
 paintCrawl();}
function clearCaption(){stopCrawlLoop();if(capBand)capBand.textContent='';cursor=null;held={cueIndex:0,wordIndex:0};}
// THE CLOCK, in ms from the start of the slide's segment: the cue's onset (from positionAt's
// layout) plus how far into the cue we are. A cue with a clip reads the media element, minus the
// encoder silence it skipped (rule 7); a cue with no clip reads the wall clock from when it
// started. So the crawl runs through a sentence the author never prepared, and through a
// captions-only export, instead of sitting frozen on word one.
var rafId=0,silentFrom=0;
function crawlClock(){return cueFrom+Math.max(0,silentFrom?Date.now()-silentFrom:(au?au.currentTime*1000-cueLead:0));}
...
// little here and only runs while something is actually being spoken.
function crawlLoop(){
...
function speakSlide(i){
 stopAudio();spokeSlide=i;seg=segFor(i);cueClips={};cursor=null;
 if(seg&&seg.audio){var uris=clipsFor(i);
  for(var c=0;c<seg.audio.clips.length;c++){var cl=seg.audio.clips[c];cueClips[cl.cue]={a:clipUri(cl.src,i,uris),l:cl.leadMs||0,m:cl.measuredMs||0};}}
 // One cursor per slide, over the slide's own track, re-anchored per clip below. It exists with or
 // without a caption band, because the transport times every cue through positionAt, which reads it.
 held={cueIndex:0,wordIndex:0};
 if(seg&&seg.track){cursor=makeCursor(seg.track);if(capBand){renderCrawl(cursor.track());paintCrawl();}}
 nextCue(0);}
// The slide's layout as it stands NOW — after whichever clips have re-timed the cursor, and without
// any clip that failed. Cheap: one pass over the slide's cues.
function layout(){return positionAt(seg,0,cursor);}
// A cue with no clip, or one whose clip failed: show its caption, crawl on the estimate, and hold
// for however long positionAt says a cue with no clip plays — the estimate, never under the floor
// (rule 4). The clip is dropped from this slide's segment first, so positionAt lays the cue out
// as silent.
function silentCue(k,after){
 if(seg&&seg.audio){seg.audio.clips=seg.audio.clips.filter(function(c){return c.cue!==k;});}
 cueLead=0;silentFrom=Date.now();startCrawlLoop();clearBeat();
 beatT=setTimeout(after,layout().played[k]);}
function nextCue(k){
 if(!narPlaying)return;
 var n=cursor?cursor.track().cues.length:0;
 if(k>=n){endSlide();return;}
 var lay=layout();
 cueFrom=lay.onsets[k];
 held={cueIndex:k,wordIndex:0};paintCrawl();
 // The BREATH after this sentence (positionAt's gaps: the track's gap to the next cue, and after
 // the last one the slide's tailMs), so the slide boundary does not land on the final syllable.
 var after=function(){clearBeat();beatT=setTimeout(function(){nextCue(k+1);},layout().gaps[k]);};
 var clip=cueClips[k];
 if(!clip||!clip.a){silentCue(k,after);return;}
 if(RENDER){if(!(clip.m>clip.l)){silentCue(k,after);return;}
  if(cursor)cursor.align(k,cursor.track().cues[k].startMs,Math.max(1,clip.m-clip.l));
  RENDER.log.push({slide:spokeSlide,cue:k,at:performance.now()});
  cueLead=0;silentFrom=Date.now();startCrawlLoop();clearBeat();
  beatT=setTimeout(after,layout().played[k]);return;}
 silentFrom=0;
 var a=audioEl();
 // The clip is done with once it ends: a late error event from it must not re-run this cue.
 a.onended=function(){a.onerror=null;after();};
 ...
 // sentences grows ~28%. The value is 0 for a clip that arrived already compressed.
 var lead=clip.l||0,anchored=false;cueLead=lead;
 // Both steps wait for a KNOWN duration longer than the lead. WebKit can fire loadedmetadata before
 // it knows an MP3's length (duration NaN), and a seek then makes it treat the clip as 1 ms long:
 // it fires ended at once and the sentence is skipped in silence (measured on WebKitGTK 2.52). So
 // a clip whose length arrives late is anchored on durationchange instead.
 var anchor=function(){
  if(anchored||!(isFinite(a.duration)&&a.duration*1000>lead))return;
  anchored=true;
  if(lead>0&&a.currentTime*1000<lead){try{a.currentTime=lead/1000;}catch(e){}}
  // RE-ANCHOR to the clip's REAL SPEECH duration — the decoded length MINUS the silence we just
  // skipped — the same call Present makes from Suono's measured onset. This is why nothing has to
  // be measured at bake time: the estimate ships, and the truth arrives with the audio.
  if(cursor){cursor.align(k,cursor.track().cues[k].startMs,Math.max(1,a.duration*1000-lead));}
 };
 a.onloadedmetadata=anchor;a.ondurationchange=anchor;
 a.onplaying=function(){startCrawlLoop();};
 // A CLIP THAT WILL NOT DECODE falls back to the estimate, exactly as a cue with no clip does
 // (rule 4). It arrives twice — the element's error event AND the play() promise rejecting with
 // NotSupportedError — in either order, so the fallback runs once per attempt. It used to stop
 // narration outright: the rejection ran the autoplay-refusal branch below, which cleared the
 // fallback the error event had just started.
 var g=++gen,fell=false;
 var fallback=function(){if(g!==gen||fell)return;fell=true;
  a.onended=null;a.onerror=null;a.onloadedmetadata=null;a.ondurationchange=null;a.onplaying=null;try{a.pause();}catch(e){}
  silentCue(k,after);};
 a.onerror=fallback;
 a.src=clip.a;
 ...
 // kept running (a double-click on Play was enough). The generation stamp ignores a rejection
 // that belongs to a cue we have already moved past. A CURRENT NotAllowedError means autoplay
 // was refused, so narration stops; any other current rejection is the clip itself failing.
 var pr=a.play();
 if(pr&&pr.catch)pr.catch(function(e){if(g!==gen)return;
  if(e&&e.name==='NotAllowedError'){setPlaying(false);stopAudio();clearCaption();return;}
  fallback();});
}
function endSlide(){
 clearCaption();
 if(t.index>=slides.length-1){setPlaying(false);return;}
 // Advance FIRST, then hold on the slide that arrived, then speak (rule 2). The hold is the
 // ARRIVING slide's — its segment's wait, as positionAt lays it out: a section opener waits the
 // deeper beat. The deck is anchored on the arriving slide from here, so navigating BACK during
 // the hold is a real move that speaks at once (rule 6) rather than a no-op that waits it out.
 autoNav=true;t.next();autoNav=false;spokeSlide=t.index;
 var s=segFor(t.index),hold=s?positionAt(s,0,s.track?makeCursor(s.track):undefined).waitMs:0;
...
// deck, so the narration follows them (rule 6). Two things must NOT re-anchor:
...
// A SILENT slide the viewer chose is one they want to look at, so the player STAYS on it, with
// narration still armed, and speaks again when they move to a narrated slide (rule 6, owner
// ruling 2026-09-25). Speaking it would reach endSlide at once, and the deck used to leave the
// slide as soon as the viewer arrived. A silent LAST slide ends narration instead, as reaching the
// end does: nothing after it can speak, so an armed Pause button would promise nothing. Play on a
// silent slide is rule 1: it advances at once and holds on the slide that arrives.
onSlideShown=function(i){if(narPlaying&&!autoNav){if(i!==spokeSlide){var s=loadLtt().segments[i];
 if(s&&s.kind!=='hold')speakSlide(i);else{stopAudio();clearCaption();spokeSlide=i;if(i>=slides.length-1)setPlaying(false);}}}else if(!narPlaying)clearCaption();};
...
// `_captions` keeps its place for positional callers, and no longer changes a byte: since LTT
// step 2 the timing kernels ship with any narration, captions or not (see the kernel block below).
export async function playerJs(animaJs = '', beats = null, _captions = !!beats, canvas = PLAYER_CANVAS, chart = null) {
	...
	// The LTT kernels, shared VERBATIM with every other LTT reader (docs/src/lib/ltt/) — the
	// same `unpackTrack` that decodes the packed track, the same cursor `at()` / `align()` that
	// drive Present's teleprompter, and the same `positionAt` the video renderer will lay frames
	// out with. Inlined for the same reason the transport is: this script is CSP-hashed and
	// cannot import. Each is self-contained by contract, pinned by
	// test/unit/export/inlinable-kernels.test.js and, against a MINIFIED build, by
	// test/unit/export/ltt-conformance.test.js. `positionAt`'s one module-scope reference is in
	// the branch that builds its own cursor, and narrationJs always passes one.
	// Loaded and inlined ONLY for a deck that ships narration. A silent export must stay
	// byte-identical to what it was before this feature existed — which the frozen-artifact
	// golden enforces, and which caught this line the first time it was written
	// unconditionally. All three ship with ANY narration, captions or not: the transport times
	// every cue and hold through positionAt, so an audio-only export needs them as much as one
	// with a band. (Until the step-2 inversion pass, an audio-only export restated the timing
	// rules by hand instead, which is exactly the second copy that drifts.)
	const ltt = beats ? await import('@laticent/ltt') : null;
	const capKernel = ltt
		? `var unpackTrack=${ltt.unpackTrack.toString()};\nvar makeCursor=${ltt.makeCursor.toString()};\nvar positionAt=${ltt.positionAt.toString()};\n`
		: '';
	const kernel =
		capKernel +
...
function setView(v){view=v;app.setAttribute('data-lp-view',v);
 // Mirror the CSS view-hide onto the HIDDEN ATTRIBUTE of the pane that is off.
 // This is for TEXT EXTRACTORS (reader mode, "summarize this page"), not for layout:
 // Readability's visibility check reads the hidden attribute, inline style and
 // aria-hidden — never COMPUTED style — so a pane hidden by a stylesheet rule alone is
 // still extracted. This document carries the deck TWICE (the slide stack in #lp-stage,
 // the prose article in #lp-doc), so every sentence reached the summarizer twice: 2255
 // extracted words for a 1080-word deck, which also halves the deck that fits under
 // Firefox's 3000-word summarize cap. Two properties make this safe. The attribute is
 // set ONLY where the CSS already hides the pane, so it is a provable visual no-op; and
 // it is set only from JS, so the shipped markup is unchanged and the NO-JS FLOOR in
 // playerCss still lays out every slide.
 var docEl=lpEl('lp-doc'),stageEl=lpEl('lp-stage');
 if(docEl)docEl.hidden=v!=='read-article';
 if(stageEl)stageEl.hidden=v==='read-article';
...
[].forEach.call((lpBar?lpBar.querySelectorAll(':scope > .lp-seg > [data-lp-btn]'):[]),function(b){b.onclick=function(){setView(b.getAttribute('data-lp-btn'));};});
...
${beats ? `${narrationJs()}\n` : ''}setView('present');
	...
	if (chart?.js) {
		...
		// escape is the same one `inertJson` above and `lib/core/data-block.js` use;
...
export async function assemblePlayer(data, caps) {
	...
	const styles = [...doc.querySelectorAll('head style, head link[rel="stylesheet"]')]
		.map((s) => {
			...
			// The embedded faces are FALLBACK, not swap, in the player. Every engine face is
			// `font-display: swap`, so a player opened cold laid its titles out in the fallback
			// face, painted, then re-laid them out when the real face decoded: the title text
			// visibly shifted a beat after the file opened (seen on an iPhone). Here every face
			// is a data URI already in the file, so it decodes well inside `fallback`'s ~100ms
			// block period and the first layout is the real one; it holds for the no-JS view
			// too, where no script can gate the reveal. Not `block`: its ~3s invisible period
			// is what a reader would get if a browser ever failed to decode a face, which is
			// worse than the shift it fixes. A swap exists for a slow NETWORK, which a
			// self-contained file never has.
			if (s.id === 'lattice-embedded-fonts') out = out.replace(/font-display:\s*swap/g, 'font-display:fallback');
	...
	// The deck's own rhythm, resolved HERE and baked into the LTT as each slide's arrival hold.
	// The player cannot resolve it for itself: the parse pass above strips every `<script>` that
	// is not the manifest envelope, so the `application/lattice-front-matter` block does not
	// survive into the file — and a shared artifact has no workspace preset to consult anyway.
	// `pace:` off the verbatim source is therefore both the only reachable answer and the right
	// one: it is the author's directorial choice, and honoring it is what makes a deck that
	// presents itself play the way they made it rather than the way the recipient's browser does.
	const paceName = frontMatterPace(source);
	const beats = { slide: paceBeatMs('slide', paceName), section: paceBeatMs('section', paceName) };
	// Which slides open a section, read off the rendered deck at export time. The player used to
	// read this from its live DOM on every advance; the LTT carries the answer as `holdMs` now, so
	// every reader of the file gets the same hold, not only this player.
	const slideEls = [...doc.querySelectorAll('section[data-lattice-slide]')];
	const isDivider = (i) => /(^|\s)divider(\s|$)/.test(slideEls[i]?.getAttribute('class') || '');
	// Baked narration (#1393, LTT step 2). Emitted only when the author opted in to captions,
	// audio, or both — a deck that opted into neither stays byte-identical to before this
	// existed: no blocks, no `media-src`, no caption band, and a player script that never grows
	// the transport. (It is NOT gated on the device holding prepared audio: a captions-only
	// export ships an LTT with no clips, and an audio export synthesizes what the device lacks.)
	const narration = await narrationPayload(data.narration, {
		slideCount: slideEls.length,
		isSection: isDivider,
		beats,
		deckPace: paceName,
		id: typeof title === 'string' && title ? title : 'deck',
		sha256: caps.sha256,
	});
	const hasNarration = !!narration.ltt;
	// CAPTIONS ship when the author asked for them; the band, its stylesheet and the inlined
	// cursor all key on this one flag. AUDIO ships iff a slide actually carries a clip — the rule
	// the CSP's `media-src` grant is keyed on. A captions-only export is a read-along on the
	// player's own wall clock: it has a transport and a band, and it must not be handed
	// permission to load media it does not contain.
	const hasCaptions = hasNarration && narration.hasCaptions;
	const hasAudio = narration.hasAudio;
	...
	// Laticent wrapper, which calls this and is not in this tree — deep-imports assemblePlayer,
	...
	const js = await playerJs(hasScene ? ANIMA_PLAYER_JS : '', hasNarration ? beats : null, hasCaptions, canvas, hasChart ? { js: ANIMA_CHART_JS, deck: chartMotion } : null);
	...
	//    carried or must be re-synthesized, the voice, and where the timing track rides — so a
	//    future player can migrate an old artifact instead of guessing at it. Shaped by
	//    `buildReadAlong` rather than written as a literal here, because the caller supplies a
	//    Studio-internal voice object and a document format must not carry the Studio's own
	//    ladder names (#1462 item 1). Neither the AUDIO nor the TIMING rides in here — see
	//    `narrationPayload` for why.
	const envelope = buildEnvelope(
		{ source, title, theme: data.theme, config: data.config, notes: data.notes, glossary: data.glossary, readAlong: buildReadAlong(data.readAlong?.voice, { hasAudio, ltt: narration.ltt }) },
...
${hasCaptions ? ' <div id="lp-caption" aria-hidden="true"></div>\n' : ''} <div id="lp-nav">
 ...
 <!-- SHIPPED hidden, and the attribute is load-bearing rather than cosmetic.
      A reader-mode extractor reaches this document by one of two paths, and they
      disagree about JavaScript. Firefox for iOS readerizes the LIVE DOM (it calls
      readerize() inside the webview), so the attribute setView maintains is what it
      sees. Firefox on the desktop reaches about:reader?url= by RE-FETCHING the URL and
      parsing the server HTML with no scripts run at all — measured: a paragraph added
      by page JS never appears, and a hidden attribute set by page JS is ignored. On that
      path nothing setView does is visible, so without this attribute in the SHIPPED
      markup the re-fetching reader extracts the slide stack AND the article and the deck
      is summarized twice over.
      Marking #lp-doc rather than #lp-stage is forced: the NO-JS FLOOR needs the slides to
      stay visible when no script runs, so the stack can never ship hidden. The cost is
      that a re-fetching reader always gets the slides rather than the richer article —
      one copy of a partial deck beats two copies of a whole one, and a live-DOM reader
      still gets whichever view is open. Visually this is a no-op in both worlds: the
      default view is present, where the CSS already sets #lp-doc to display:none. -->
 <div id="lp-doc" hidden>
...
<script>${js}</script>
${narration.blocks ? `${narration.blocks}\n` : ''}${envelope}
</body></html>`;
