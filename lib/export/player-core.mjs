...
const keepBookendsDark = (c) => (c === '.print'
  ? `section[data-lattice-slide]${c}`
  : `section[data-lattice-slide]${c}:where(${BOOKEND_FORCES_DARK.map((b) => `:not(${b})`).join('')})`);
...
//
// These are (0,5,1) against the engine rule's (0,1,1), so they win in both directions — and
...
//
// A pinned-light slide is excluded for the mirror reason: its ground is light whatever the OS
// says. So what is left is exactly the set whose ground really does follow the receiver.
...
export function hoistInlineLightDark(doc) {
	...
	const restore = (scope) => PINNED_TO_LIGHT.map((c) => at(`${scope} ${keepBookendsDark(c)}`, 'light')).join('');
	return (
...
const SCHEME_SCOPES = [
	...
	...PINNED_TO_LIGHT.map((c) => ({ root: ':root[data-lp-scheme=dark]', section: keepBookendsDark(c), arm: 'light', media: '' })),
...
export function themeDualMode(css) {
	...
	const pinned = (scope) =>
		PIN_LIGHT.map((c) => `${scope} ${keepBookendsDark(c)}:not(.print)`).join(',');
	const darkBlock =
