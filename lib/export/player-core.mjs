...
//
// So the bookends are re-admitted. `:where()` carries zero specificity, which keeps this
// selector at (0,2,1) — exactly what it was before the pin set was subtracted — so nothing
// downstream shifts rank.
...
const keepAlwaysDark = (c) => {
  ...
  const carve = ALWAYS_DARK
    ...
    .map(({ cls }) => `:not(${cls})`)
    .join('');
  return `section[data-lattice-slide]${c}:where(${carve})`;
};
...
//
// These are (0,10,1) against the engine rule's (0,1,1), so they win in both directions — and
...
// and dividers too — and at (0,5,1) these rules steamroll the engine's own (0,2,1) force-dark
...
const SYSTEM_LOGO_SEL = 'section[data-lattice-slide].color-system'
	+ ':not(.print):not(.light):not(.color-light):not(.title):not(.closing):not(.divider)';
...
const DARK_SLIDE_SEL = `section[data-lattice-slide].dark:not(.print):where(`
  + `${PINNED_TO_LIGHT.filter((c) => c !== '.print').map((c) => `:not(${c})`).join('')}`
  + `, ${BOOKEND_FORCES_DARK.join(', ')})`;
...
const SCHEME_SCOPES = [
	...
	{ root: '', section: DARK_SLIDE_SEL, arm: 'dark', media: '' },
