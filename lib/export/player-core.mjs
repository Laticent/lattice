...
/* The roster: a portrait beside the person's line, the words in their own wrapper.
   ...
   DESCENDANT, deliberately, for the two rules inside the wrapper. A child combinator on
   the image rule let an image that is the person's NAME (rather than the portrait) escape
   every cap in the article and render at natural size — a 900x600 source ran 542px past a
   390px column. A child combinator on the list rule left a list nested one level deeper
   falling back to the generic article margin, which put 19.8px of dead space inside the
   row. Inside the wrapper we WANT to reach the author's markup; outside it we must not.

   align-self is flex-start, not center: the wrapper makes rows as tall as their content,
   and a centered portrait drifted 64px below the name it captions on a three-line row.
   It costs 1.5px on a single-line row.

   HARD RULE #20: the list rule is spelled margin:0 + padding-top rather than a margin
   shorthand. It has to zero the generic article margin, so margin:0 is not optional — but
   the SPACE is padding. (The file comment above about margins is honest only if the rules
   below hold to it. No backticks here: the stylesheet is a template literal.) */
#lp-article .lp-roster{list-style:none;padding-left:0}#lp-article .lp-roster>li{display:flex;align-items:baseline;gap:.6em}#lp-article .lp-roster>li>img{flex:none;width:1.9em;height:1.9em;border-radius:50%;object-fit:cover;align-self:flex-start}#lp-article .lp-roster>li>div{flex:1;min-width:0}#lp-article .lp-roster>li>div img{max-width:100%;height:auto}#lp-article .lp-roster>li>div :is(ul,ol){margin:0;padding-top:.3em}
