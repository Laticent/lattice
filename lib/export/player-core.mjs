...
import { slideFrameFilter } from '../core/slide-frame.mjs';
...
/* The frame — NOT the scaled section — carries the edge + shadow that makes each
   slide a distinct card, and it carries them as the ONE slide-frame filter
   (lib/core/slide-frame.mjs) shared by every surface that shows a slide. On the scaled
   section a border shrank to a sub-pixel hairline. A border + 12px radius on this frame
   rounded every slide of a SQUARE deck, and cut the section's own border at each
   corner in the no-JS floor (the gapped corners seen on a phone). The drop-shadow
   filter traces whatever the section painted, so the edge follows the engine's corner
   (square, or corners-rounded) exactly, at true 1px, with no radius here to disagree. */
...
html:not(.lp-js) .lp-frame{width:calc(${CW}px * var(--lp-fit));height:calc(${CH}px * var(--lp-fit));overflow:hidden;filter:${slideFrameFilter('card')}}
