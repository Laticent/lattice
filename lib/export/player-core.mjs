...
export function themeDualMode(css) {
	...
	while ((m = declRe.exec(noComments))) {
		...
		// The mirror set, flattened the same way — what a section PINNED to light needs when
		// the rest of the player is in dark (see the per-section pins below). `deepFlatten` stops at a
		// `darkKeys` name, which is exactly right here too: a light value referencing another
		// dual-mode token must keep referring to the value in force on THIS element.
		lightDecls.push(`${m[1]}:${deepFlatten(resolveLightDark(m[2], 0).trim(), new Set())}`);
	}
	...
	const derivedDecls = [];
	{
		// SCOPED TO `:root`, and that scoping is load-bearing. This block hoists declarations
		// onto section scope WITHOUT their original selector, which is sound ONLY for tokens
		// that were declared at `:root` — exactly the caveat the docblock above states ("NOT
		// selector-general"). The engine and themes also declare custom properties inside
		// COMPONENT rules: `--elevation-card` on `section.lifted`, `--pill-border` on an
		// nth-child arm, the whole `--fs-*` type scale on size classes. Scanning the entire
		// sheet lifted those onto EVERY slide at (0,7,1) — outranking the very rules they were
		// read from, so `_class: flat` got the lifted card's shadow back and a pill's border
		// recolored to a categorical mark. Measured on `dist/lattice.css` + `themes/indaco.css`:
		// 281 of 435 candidate tokens have at least one declaration outside `:root`.
		//
		// A selector qualifies only when `:root` is its own SUBJECT — `:root`,
		// `:root[data-lp-scheme=dark]`, or a list with such an arm. `:root[…] .lattice-bg`
		// does NOT: its subject is a descendant, so its declarations land on that descendant.
		const isRootScope = (selectorList) =>
			selectorList.split(',').some((s) => /^:root(?:\[[^\]]*\]|:[a-z-]+(?:\([^()]*\))?)*$/.test(s.trim()));
		// LAST declaration wins, as the cascade says. The previous pass walked source order and
		// kept the FIRST, taking the losing value for the 203 tokens declared more than once.
		// (Emission order is irrelevant: custom properties resolve at computed-value time, not
		// in source order, so the fixed point below can emit in any order.)
		const pool = new Map();
		...
		while ((rule = ruleRe.exec(noComments))) {
			...
			while ((d = declAny.exec(block))) {
				if (darkKeys.has(d[1])) continue; // already re-emitted as a light/dark pair
...
export async function playerJs(animaJs = '', beats = null, captions = !!beats, canvas = PLAYER_CANVAS) {
