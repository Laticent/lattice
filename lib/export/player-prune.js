...
//
// OPT-IN (`legacyPseudoElements`). Every caller in the repo now opts in: the Reading-view fence
// (#2366), and both player prunes — the CLI's (`lattice-emulator.js` `prunePlayerCssInPage`,
// where KaTeX's one-colon rules sat in the unminified sheet) and its Studio twin
// (`player-prune-browser.ts`, whose minified sheet writes every `::before` that way). The two
// players moved together with the CLI's flat sheet (decision 2026-09-24 §8 step 4), under the
// owner's export sign-off. The default stays off for an external caller of this module.
 ...
 * 2, §8 step 3).
 *
 * - `css` is the engine's FLAT pack with the `article.lattice > ` wrapper already unwrapped
 *   (`unwrapFlatSheet`, lib/export/unwrap-flat-sheet.mjs).
