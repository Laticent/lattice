...
function opaqueOverrideDecls(r) {
  ...
  // A baked SPOTLIGHT flips to its hard-edged mirror in THIS finish's own export rules:
  ...
  // A baked CLEARANCE needs no flip: its content-box layer's export face is in base.finish.css.
  if (r.backdrop?.spotlight) out.push('--fin-backdrop-mask: var(--fin-backdrop-mask-opaque, none)');
  return out;
}
...
// one window) wins over clearance. A spotlight ships a rich feather and a hard opaque mirror;
// CLEAR BEHIND CONTENT is the frame's content box (heading + body), painted by the shared layer in
// base.finish.css, the same one a deck's `backdrop: clear` uses. It replaced a central ellipse
// (`--backdrop-clear-mask`, still defined for finishes saved before 2026-09-26) that missed the
// heading and only faded the body's edges. A baked mask also switches the strength from group
// opacity to the flat veil (base.finish.css § BACKDROP REGISTER): poppler draws a hard mask inside
// an opacity group as a wedge. It is emitted even at full strength, so a deck's `backdrop: 40` over
// this finish still dims through the veil. Clamped numbers and fixed tokens only (HARD RULE #22).
function backdropSlots(r) {
  ...
  const veil = ['--fin-backdrop-veil-weight: 1', '--fin-backdrop-dim-scrim: var(--backdrop-veil-fill)'];
  if (spot) {
    ...
    out.push('--fin-backdrop-clear-scrim: var(--backdrop-clear-fill)');
    out.push(...veil);
  }
  return out;
}
