 ...
 * keeps its direct-child `section.X > …` bodies untouched. (compare-code drives its
 * own title grid via `> h2` and is chrome-exempt in the toggle's skip set. `math` did
 * too until 2026-09; it is a strict canvas now and wraps like any other.) A Marp running <header>, if present, stays at section level before the band.
 *
...
const { commentCloser } = require('../../../core/closed-comments');
const { peelCoda } = require('../../../core/coda');
const {
  findTopLevelH2, findTopLevelTag, findFirstTag,
...
// than the sovereign-frame exemption `compare-code` uses, which opts a component out
// of the shared Frame model entirely. (`math` used that exemption until 2026-09.)
...
// STAGE_MIGRATED / STAGE_DEFERRED and the sovereign SOVEREIGN_FRAMES
...
//                 upstream (SOVEREIGN_FRAMES), never reaches the wrap decision.
...
/** Should this Form slide's body be wrapped into the `.cell-stage` cell?
 *
 * Decided by THE layout token — the first token in the class list that names a
 * known layout, which is the component the slide is. Given that token:
 *   · generic prose (no layout token at all) → wraps
 *   · `flow`                                 → wraps (bounded prose/structure body)
 *   · `canvas`                               → wraps only if `conformance:"strict"`
 *                                              (model-driven frame render §4: a strict
 *                                              component materializes its declared stage
 *                                              cell even when self-sizing)
 *   · `sovereign`                            → never; it owns its own grid
 * A `chart-frame` section always wraps, ahead of all of this.
 *
 * The first-match scan is load-bearing, not a tidy-up — see the body for the
 * variant/component name collision it fixes and the 273-combination measurement
 * that bounds it. */
function wrapsStageBody(cls) {
  ...
  // THE FIRST token naming a known layout decides — it is the component the slide
  // IS. Mirrors `layoutTokenFor` (lib/core/auto-split.js), which resolves the same
  // question with the same first-match scan.
  //
  // This used to be an ANY-token veto (`.some()`), and that was a real defect: the
  // layout-name namespace and the variant-modifier namespace are shared, so a
  // variant whose token happens to also name a component silently took over the
  // wrap decision. Five such collisions exist today —
  //   journey+heatmap · radar+quadrant · compare-prose+decision · list+bullet · math+stats
  // — and `list bullet` was the one that actually broke: `bullet` is the CHART
  // component, classified `canvas`, so a `<!-- _class: list bullet -->` slide lost
  // the `.cell-stage` clip cell its six sibling variants all get, and with it the
  // bounded box the overflow probe walls. Measured over every component × variant
  // combination the manifests declare (273 of them), first-match changes exactly
  // one answer: `list bullet`, false → true. The other four collisions resolve
  // identically under both rules.
  const token = cls.trim().split(/\s+/).find((t) => stageSizingFor(t) !== null);
  if (token === undefined) return true;              // generic prose → wraps
  const kind = stageSizingFor(token);
  if (kind === 'flow') return true;                  // bounded prose/structure body
  // A conformance:strict CANVAS wraps its self-sizing body in `.cell-stage` too, so
  // its model-declared stage cell is built. A SOVEREIGN owns its own grid.
  if (kind === 'canvas') return STRICT.has(token);
  return false;
}

// `VOID_TAGS` (HTML5 void elements never open a nesting level, so the top-level
// scans don't miscount depth on a stray <br>/<img> in pre-title prose) and the
// depth-aware `findTopLevelH2` both moved to lib/core/top-level-h2.js when
// lib/transformers/topic-track.js became their second caller — shared, never
// cloned (HARD RULE #1). The reasoning for why a plain first-`<h2>` regex is
// wrong here lives in that kernel's header; the wifi bug it prevents is pinned
// by test/integration/parity/wifi-overflow-preserved.test.js.
...
//
// It rides the SHARED walk rather than a private copy of it. While the title
// scan moved to the kernel and this one did not, the two disagreed about depth
// the moment a comment held a tag: `<!-- wrap this in a <div> later -->` left
// the title findable and this `<p>` counted as depth 1, so the band rendered
// WITH its title and the eyebrow stranded below the hairline — a state neither
// the old engine nor the runtime ever produced. Two walks over one document is
// how that happens (HARD RULE #1).
function findTopLevelEyebrow(scope) {
  return findTopLevelTag(scope, 'p', ({ start }) => {
    const pm = scope.slice(start).match(/^<p[^>]*>\s*<code[^>]*>[\s\S]*?<\/code>\s*<\/p>/);
    return pm ? { start, text: pm[0] } : null;
  });
}
...
function extractEyebrowP(html) {
  // Bound the scope at the title the BAND will actually lift — the depth-aware
  // one — not at the first `<h2>` anywhere. A component card that emits its own
  // `<h2>` before the eyebrow (a QR card's `.qr-head > h2`, the wifi bug this
  // tree already hit) truncated the scope to nothing, and the eyebrow was then
  // left stranded in `.cell-stage` below the hairline while the band carried the
  // title alone. `findFirstTag` remains the fallback for a section whose only
  // headings are nested, where the legacy scan below picks one of them.
  const top = findTopLevelH2(html);
  const h2Idx = top ? top.start : findFirstTag(html, 'h2');
...
//   · false (other UNWRAPPED canvases — a non-strict canvas outside `chart-frame`,
//     whose title placement has not yet converged onto the model; `video` left this
//     set on 2026-09-24 when it went strict) → legacy depth-blind first-match, preserving
...
// Returns `at` — the offset the h2 occupied in the RETURNED html, i.e. where its next
// sibling now begins. extractSubtitleP anchors there (see its header).
function extractH2(html, topLevelOnly = true) {
  if (topLevelOnly) {
    const found = findTopLevelH2(html);
    if (!found) return { el: '', html, at: -1 };
    return {
      el: found.text,
      html: html.slice(0, found.start) + html.slice(found.start + found.text.length),
      at: found.start,
    };
  }
  // Depth-blind, deliberately — but no longer COMMENT-blind. A drafting comment
  // holding an `<h2>` used to become the band's title while the real heading
  // stayed in the body, and once the eyebrow scan above moved to the shared walk
  // the two disagreed hard enough to put the eyebrow beside that phantom title.
  // "Preserving pre-migration behavior EXACTLY" stopped being available the
  // moment its neighbor changed, so the pair is made consistent instead.
  const at = findFirstTag(html, 'h2');
  if (at === -1) return { el: '', html, at: -1 };
  const close = /<\/h2\s*>/i.exec(html.slice(at));
  if (!close) return { el: '', html, at: -1 };
  const el = html.slice(at, at + close.index + close[0].length);
  return { el, html: html.slice(0, at) + html.slice(at + el.length), at };
}
...
//
// Anchored at `at` — the offset extractH2 just vacated — so this is the string
// equivalent of the DOM kernel's `h2.nextElementSibling` (masthead-lift.js). A
// subtitle further down the body is real content, not this chrome, so the anchor is
// load-bearing; what it must NOT be is the START of the string. It used to be, and then
// ANY markup preceding the heading defeated it and the subtitle stayed in the body,
// below the hairline — out of the band, and so out of `headline:` control: with
// `headline: center` the title centered while the subtitle sat left-pinned.
//
// The common trigger is a speaker note. NOT the `<aside class="lattice-notes">` — that
// does not exist yet at lift time; notes are materialized AFTER the render returns
// (lattice-emulator.js `slidesWithNotes`, and share-export.ts `materializeNotes` on the
// Studio path). At lift time the note is still its raw `<!-- … -->` comment node, and
// that is what sat in front of the heading. Prose or raw HTML authored above the heading
// did the same thing, so the anchor fixes a broader class than notes alone.
//
// The DOM kernel was always immune (element siblings, not string offsets), so this was
// also a live HARD RULE #1 engine↔web split of the kind that function's own comment
// warns about — live but unexercised: every deck in the shipped corpus places its notes
// after the heading.
function extractSubtitleP(html, at = 0) {
  ...
  //
  // The paragraph must be a `<p>` holding ONE code span and nothing else — what the DOM twin
  // tests as `tagName === 'P'` with a single child. `<p[^>]*>` also matched `<pre>`, and the
  // lazy body then ran on to the NEXT code-only paragraph: a heading followed by a code block
  // (an unrendered mermaid fence) and a closing note had the fence, the note and the whole
  // stage pulled into the band. Tag name closed by whitespace or `>`; no `</code>` inside.
  const m = rest.match(/^\s*<p(?:\s[^>]*)?>\s*<code[^>]*>(?:(?!<\/code>)[\s\S])*<\/code>\s*<\/p>/)
    || rest.match(/^\s*<p class="chart-subtitle">[\s\S]*?<\/p>/);
  if (!m) return { el: '', html };
  return { el: m[0], html: html.slice(0, from) + rest.slice(m[0].length) };
}
...
// Is an element carrying the `cell-footer` class a DIRECT child of this section's inner HTML?
// The string twin of the DOM mirror's `:scope > .cell-footer` (lib/transformers/masthead-lift.js):
// depth is counted over EVERY element (void elements and self-closing tags do not open a level),
// and the class is matched as a whole TOKEN in either quote style, so `cell-footer-x` is not a
// footer cell and `class='x cell-footer'` is.
const VOID_TAGS = new Set(['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'source', 'track', 'wbr']);
function hasTopLevelFooterCell(innerHtml) {
  // A comment is skipped through its `-->` from a cursor, not by a `<!--[\s\S]*?-->` arm: that
  // arm rescans to the end from every UNCLOSED `<!--`, which made a pasted line of them
  // quadratic (lib/core/closed-comments.js). An unclosed one is text, as the arm read it.
  const re = /<!--|<(\/?)([a-zA-Z][\w-]*)\b((?:[^>"']|"[^"]*"|'[^']*')*)>/g;
  const commentEnd = commentCloser(innerHtml);
  let depth = 0;
  let m;
  while ((m = re.exec(innerHtml)) !== null) {
    ...
    const attrs = m[3] || '';
    if (depth === 0) {
      const cm = attrs.match(/\sclass\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/i);
      const classes = cm ? (cm[1] ?? cm[2] ?? cm[3] ?? '').split(/\s+/) : [];
      if (classes.includes('cell-footer')) return true;
    }
    if (!VOID_TAGS.has(tag) && !/\/\s*$/.test(attrs)) depth++;
  }
  return false;
}
...
function transformMastheadSection(innerHtml, cls, pagination = '') {
  ...
  // A composed frame (its footer cell already a top-level child — the split envelope's cover and
  // closing pages) is not a flow body; mirrors the DOM walk in lib/transformers/masthead-lift.js.
  if (hasTopLevelFooterCell(innerHtml)) return innerHtml;
  ...
  // Other UNWRAPPED canvases keep the legacy depth-blind lift, PENDING the
  ...
  let band = '';
  let rest = r0;
  // Both arms read through the shared walk now: a `<h2>` a browser never parses
  // (inside a comment, inside `<style>`) must not decide that a band exists.
  const hasTitle = depthAware ? Boolean(findTopLevelH2(r0)) : findFirstTag(r0, 'h2') !== -1;
  if (hasTitle) {
    const { el: eyebrow, html: r1 } = extractEyebrowP(r0);
    const { el: h2, html: r2, at: h2At } = extractH2(r1, depthAware);
    const { el: subtitle, html: r3 } = OWN_TRAILING_LABEL.test(cls)
      ? { el: '', html: r2 }
      : extractSubtitleP(r2, h2At);
    band =
      '<div class="cell-masthead">' +
  ...
  if (wraps) {
    ...
    // The coda Cell is the FRAME's, exactly like the running footer above, so it
    // is peeled out of the body for the same reason and re-seated as the stage's
    // SIBLING. Inside the stage it was a flex item of the COMPONENT's column and
    // therefore inherited the component's own vertical alignment: on `cycle` the
    // band sat 171px above the floor at `align-top`, 85px at `align-middle` and on
    // the floor at `align-bottom` — the frame's editorial band moved because the
    // author re-aligned the body. As a sibling it is the section column's last row, the
    // stage keeps `flex: 1 1 auto` and absorbs the slack, and the band lands at
    // content height above the footer on every layout — no margin, no positioning
    // (HARD RULE #20). engineering/decisions/2026-08-24-universal-coda-cell.md §7.
    const { rest: stageBody, coda } = peelCoda(body);
    return `${header}${band}<div class="cell-stage">${stageBody}</div>${coda}${buildFooterCell(footer, pagination)}`;
  }
