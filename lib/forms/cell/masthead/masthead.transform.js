...
//   · true  (WRAPPED components — strict/flow, whose body hoists into `.cell-stage`)
//     → depth-aware (findTopLevelH2): a nested in-card <h2> (e.g. a QR card's
//     `.qr-head > h2`) is LEFT in place, so a canvas that owns its title inside a
//     card is never mastheaded. This matches the DOM mirror's `:scope > h2`.
//   · false (UNWRAPPED canvases — e.g. charts, whose title nests in `.chart-header`
//     before mastheadLift) → legacy depth-blind first-match, preserving their
//     pre-migration masthead band EXACTLY. Their engine↔runtime title-lift
//     disagreement (the engine builds a band from the nested h2; the DOM mirror's
//     `:scope > h2` does not) is a PRE-EXISTING parity gap tracked in
//     engineering/decisions/2026-07-15-model-driven-frame-render.md §6 — a separate,
//     deliberate decision (with export sign-off), NOT changed as a wifi side-effect.
...
function transformMastheadSection(innerHtml, cls, pagination = '') {
  ...
  // prose slide just has no band.) The h2-lift is depth-aware ONLY for a WRAPPED
  // component (`wraps`): its body hoists into `.cell-stage`, so a title nested in
  // its own rebuilt card (a QR card's `.qr-head > h2`) stays put instead of being
  // yanked into an empty band. An UNWRAPPED canvas (charts, whose h2 nests in
  // `.chart-header`) keeps the legacy depth-blind lift, so its masthead band is
  // byte-identical to main (see extractH2's note — the chart parity gap is a
  // separate, tracked decision, not a wifi side-effect).
  const wraps = wrapsStageBody(cls);
  let band = '';
  let rest = r0;
  const hasTitle = wraps ? Boolean(findTopLevelH2(r0)) : /<h2[^>]*>/.test(r0);
  if (hasTitle) {
    const { el: eyebrow, html: r1 } = extractEyebrowP(r0);
    const { el: h2, html: r2 } = extractH2(r1, wraps);
