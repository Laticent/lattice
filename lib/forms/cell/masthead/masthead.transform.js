...
const { mathSectionMigrated } = require('../../../core/math-stage-migration.js');
...
function wrapsStageBody(cls) {
  ...
  // Same temporary per-variant switch as the class toggle in plugins.js, and it MUST
  // read the same predicate: a section that takes the `form` class but does not wrap
  // would get chrome cells built around a body that never moved into them.
  if (mathSectionMigrated(cls)) return true;
