...
// see engineering/decisions/2026-07-14-frame-species-one-model.md, step A). This
...
/** The stage-cell species for one layout token — `'flow' | 'canvas' | 'sovereign'`,
 * or `null` when the token is not a known layout. The single classifier the three
 * Sets used to encode; wrapsStageBody is defined in its terms. */
 ...
 * sovereign). Equivalent to the pre-catalog `ALL_LAYOUTS.has(t) && !STAGE_MIGRATED
 * .has(t)` test — the drift test pins that equivalence. */
function wrapsStageBody(cls) {
  return !cls.trim().split(/\s+/).some((t) => {
    const species = stageSizingFor(t);
    return species !== null && species !== 'flow';
  });
}
