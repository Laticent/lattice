...
const OPT_IN = 'form';
// A sovereign Frame declares ONE Cell (`stage`) and no chrome Cells, so the band,
// the bay and the footer Cell are not its to build. Until 2026-09-20 this was
// expressed by denying those slides the `form` class entirely; they carry it now
// (Form is universal) plus `frame-sovereign`, and this is the token that says so.
const SOVEREIGN_CLASS = 'frame-sovereign';

function hasOptIn(cls) {
  const tokens = cls.trim().split(/\s+/);
  // Every slide carries `form`; only a CHROME-HOSTING Frame gets a band + footer
  // Cell out of this transform. A sovereign Frame owns its own grid.
  return tokens.includes(OPT_IN) && !tokens.includes(SOVEREIGN_CLASS);
}
