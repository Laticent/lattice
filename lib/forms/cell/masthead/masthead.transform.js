...
function extractH2(html, topLevelOnly = true) {
  ...
  // moment its neighbour changed, so the pair is made consistent instead.
...
module.exports = {
  OPT_IN,
  hasOptIn,
  // Exported so the SPLIT path can build the same Cell rather than hand-roll one.
  // A split body page of a sovereign frame has no footer Cell of its own (the frame
  // suppresses it), and every piece of run wayfinding docks INTO that Cell:
  // `footer-dock.js` puts the forward pointer there, `progress.transform.js` the
  // k-of-N rail, and the page number is built here. Without the Cell all three
  // degrade to section level — which is how the pointer became a third column of
  // `split-panel`'s panel row. One builder, one shape (HARD RULE #1).
  buildFooterCell,
  OWN_TRAILING_LABEL,
  wrapsStageBody,
  stageSizingFor,
