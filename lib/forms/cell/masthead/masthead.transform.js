...
// Is a `.cell-footer` a DIRECT child of this section's inner HTML? Depth-counted over `<div>`, so
// a footer cell nested inside some other block does not count.
function hasTopLevelFooterCell(innerHtml) {
  ...
  while ((m = re.exec(innerHtml)) !== null) {
    ...
    if (depth === 0 && /\bclass="cell-footer\b/.test(m[1] || '')) return true;
    depth++;
  }
  return false;
}
