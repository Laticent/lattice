...
/**
 * Plain text of an HTML fragment, for an `aria-label`.
 *
 * TWO SEPARATE JOBS, and conflating them is the bug CodeQL names. Stripping tags is for
 * READABILITY — a caption's `<strong>` should not be announced as "strong". Escaping is
 * for SAFETY, and it is the escaping alone that must be sound: a single-pass
 * `replace(/<[^>]*>/g, '')` is defeatable by construction (remove one tag and the
 * remaining fragments can close up into another), so it can never be the thing standing
 * between input and an attribute.
 *
 * So: strip to a FIXED POINT, then escape every character that means anything in an
 * attribute value. `&` is deliberately NOT escaped — the input is already HTML, so its
 * entities are encoded, and re-escaping would announce a caption's `&amp;` as the literal
 * text "&amp;". The DOM mirror reaches the same result the easy way, via `textContent` +
 * `setAttribute`; this is the string path doing it by hand, so it must be conservative.
 */
const ATTR_ESCAPES = { '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' };
function labelText(html) {
  let text = String(html);
  let prev;
  // Tags collapse to NOTHING, not to a space: the DOM mirror derives this label from
  // `textContent`, which joins across element boundaries, and the two paths must produce
  // the same string (HARD RULE #1). Replacing with a space read better in isolation and
  // silently made every marked-up caption announce differently on the two paths.
  do { prev = text; text = text.replace(/<[^>]*>/g, ''); } while (text !== prev);
  return text.replace(/\s+/g, ' ').trim().replace(/[<>"']/g, (c) => ATTR_ESCAPES[c]);
}
