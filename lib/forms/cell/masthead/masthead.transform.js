...
/**
 * THE STAGE'S TAG — `<figure>` when the cell holds a captioned graphic, else `<div>`.
 *
 * `<figure>` is the one element that says "this graphic and this caption belong to
 * each other". Without it a screen reader gets the chart, then — some arbitrary
 * distance later in the reading order — a paragraph, with nothing tying them
 * together; the association a sighted reader gets for free from position and
 * typography is simply absent.
 *
 * WHY THE STAGE AND NOT A NEW WRAPPER. `.cell-stage` is ALREADY the common parent of
 * exactly `{.chart-body, .chart-caption}` — chart-family emits them as adjacent
 * siblings and the stage wrap collects them. So this is a RETAG, not an added box:
 * zero new elements, zero layout change, and every `.cell-stage` selector in the
 * engine is class-keyed (there is no `div.cell-stage` anywhere), so the cascade
 * doesn't move. The UA's `figure { margin: 1em 40px }` is killed by the bare
 * `margin: 0` on `section.form > .cell-stage` — HARD RULE #20 permits a zero reset
 * precisely because it adds no space.
 *
 * WHY IT KEYS ON THE CAPTION AND NOT ON `chart-frame`. A `<figure>` whose only
 * content is a graphic adds a boundary and no information — five of the seven charts
 * in the component gallery carry no caption, and wrapping those buys a landmark-ish
 * announcement for nothing. The association is the whole point, so the tag follows
 * the thing being associated. A stage that also carries `.chart-details` or a
 * `.state-legend` (the state-chart shape) still qualifies: those are part of the
 * graphic's own content, not unrelated prose, and `<figure>` explicitly admits more
 * than one child.
 *
 * DEPTH-AWARE, because the DOM mirror is. `masthead-lift.js` inspects the cell's DIRECT
 * children; a bare `includes()` here would also match a `<figcaption>` nested inside
 * authored raw HTML, and the two render paths would disagree about the tag on the same
 * deck (HARD RULE #1). Escaped markup in a `<pre>` sample can't reach either check, but
 * "can't happen today" is how the last several of these started.
 */
function stageTag(bodyHtml) {
  if (bodyHtml.indexOf('<figcaption') === -1) return 'div';
  ...
  while ((m = tagRe.exec(bodyHtml))) {
    ...
    if (depth === 0 && openName.toLowerCase() === 'figcaption' && /\bclass="chart-caption"/.test(full)) {
      return 'figure';
    }
    depth++;
  }
...
function transformMastheadSection(innerHtml, cls, pagination = '') {
  ...
  if (wraps) {
    ...
    const tag = stageTag(body); // `figure` when the cell holds a captioned graphic
    return `${header}${band}<${tag} class="cell-stage">${body}</${tag}>${buildFooterCell(footer, pagination)}`;
  }
