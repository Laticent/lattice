...
function applyFormDefaultToDom(root) {
  ...
  for (const sec of root.querySelectorAll('section:not(section section)')) {
    ...
    // The model, on every slide — the DOM twin of the engine's `stampFormAttrs`.
    if (!sec.hasAttribute('data-form')) sec.setAttribute('data-form', FORM_MEDIUM);
    if (!sec.hasAttribute('data-frame')) sec.setAttribute('data-frame', frameIdFor(sec.className));
