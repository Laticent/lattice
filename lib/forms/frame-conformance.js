 ...
 *
 * `math` (2026-09) DEPARTED FROM THAT PRECONDITION AS WRITTEN, and the departure is
 * recorded here rather than left for the next migration to discover as precedent.
 * "Verdicts unchanged" was read as "verdicts PROVEN", because math could not satisfy
 * the literal version and should not have:
 *
 *   · Its verdicts changed on purpose. `matrix` and `decompose` clipped at three
 *     registered sizes each on the sovereign frame and clip at none now; the bare
 *     hero's leftover title clearance was 13px over and is not. Holding "unchanged"
 *     would have meant preserving four measured content-loss defects to satisfy a
 *     gate about measurement.
 *   · The probe kernel changed in the SAME branch, because materializing math's stage
 *     is what exposed a false positive in it (a clip cell laid out in columns). The
 *     precondition assumes the probe is a fixed reference; here it was the thing under
 *     test. That change carries its own evidence: 232 rendered slides A/B'd, exactly
 *     one verdict moved, and that one was the false positive.
 *
 * What was actually delivered instead: every variant rendered at all five registered
 * sizes and read through this probe, before and after, with each moved verdict named
 * and attributed. If the precondition is to keep meaning what it says, it needs
 * amending to the proven form — that is a decision for whoever migrates next, not
 * something this branch was entitled to make silently.
 */
 ...
 */
const NON_MATERIALIZING_CELLS = new Set(['overlay', 'logo', 'watermark']);
