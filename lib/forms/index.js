 ...
 * claim a stage kind no component declares, no ROOT Frame may leave one out, and
 * no stage kind in the generated catalog may be left with nowhere to compose.
 * This is what stops `admits` from being a field nobody reads — it is checked
 * against lib/forms/cell/masthead/stage-catalog.generated.js, the same source
 * masthead.transform.js uses to decide the stage wrap.
 *
 ...
 * · UNDER-claiming is caught for a ROOT Frame, because there the correct value is
 *   DERIVABLE: a root Frame is the fallback host for every component that is not
 *   its own sovereign, so it must admit every non-sovereign kind the catalog
 *   declares. The arm keys on `exemptFromChrome` — the property frameToggleSkip()
 *   actually reads — NOT on the `kind` label, which a frame declares about itself.
 *   Keying on the label let a frame opt OUT of this check by calling itself
 *   sovereign while still hosting chrome; validateFrame now also rejects that
 *   contradiction, so the two guards are independent. Those `flow`/`canvas` values come from each component manifest's
 *   own `stage` field (tools/build-stage-catalog.js `build()`), which is a source
 *   INDEPENDENT of any frame — so this arm is a real check, not a restatement of
 *   the frame catalog against itself.
 * · UNDER-claiming by a SOVEREIGN Frame is still not caught, and cannot be from
 *   here: the catalog's `sovereign` values are built FROM frameToggleSkip(), i.e.
 *   from the frame manifests' own exemptFromChrome. validateFrame pins the shape
 *   (exemptFromChrome ⇔ admits === ["sovereign"]), which is as far as a
 *   same-source check can go. */
