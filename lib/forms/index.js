...
const CELL_REGIONS = Object.freeze([
  ...
  'overlay',
  // A PANE sits inside the stage: a slide whose body holds two components gives each
  // its own clipped Cell (lib/core/panes.js, lib/forms/cell/pane/pane.css).
  'pane',
  // The two FRAME-ANCHORED regions: one Tile each, positioned against the section
  // box rather than docked in a band — which is what the render has always done.
  // Two regions, not one shared "slide" region: the Tiles agree on nothing but
  // being un-docked (different kind, different plane, different anchor), and a
  // second Cell claiming an existing region would make a slicing relocation
  // target arbitrary — see checkSlicingIntegrity's first-wins map below.
  // This list is kept in step with the `region` enum in schema/cell.schema.json
  // AND the slicing-relocation enum in schema/frame.schema.json by arm (f) of
  // test/unit/forms/forms-manifest.test.js; arm (l) pins region 1:1 with Cell id.
  'logo',
  'watermark',
]);
...
function validateFrame(f, source) {
  ...
  // property. sovereignFrames() reads exemptFromChrome, so that lie was invisible at
 ...
 *   The arm keys on `exemptFromChrome` — the property sovereignFrames() actually
 ...
 *    `sovereign` values are built FROM sovereignFrames(), i.e. from the frame
 ...
 *    alone: `build()` opens with sovereignFrames() to decide which components get
...
/**
 * The engine's SOVEREIGN_FRAMES set, DERIVED from the frame manifests: the
 ...
 */
function sovereignFrames(frames) {
