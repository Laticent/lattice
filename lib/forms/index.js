 ...
 * claim a stage kind no component declares, and no stage kind in the generated
 * catalog may be left with nowhere to compose. This is what stops `admits` from
 * being a field nobody reads — it is checked against
 * lib/forms/cell/masthead/stage-catalog.generated.js, the same source
...
function checkFrameAdmits({ frames }, errors) {
  ...
  if (!declared) return; // catalog not built yet (bootstrap) — the build regenerates it
  for (const f of frames) {
...
/** The stage kinds the shipped components actually declare, or null if the
 * generated catalog is not built yet. */
function declaredStageKinds() {
  try {
    return new Set(Object.values(require('./cell/masthead/stage-catalog.generated.js')));
  } catch (_e) {
    return null;
  }
}
