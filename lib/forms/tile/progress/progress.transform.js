 ...
 *
 * Ten is a width budget expressed as a count. The dots are sized in `cqi` (0.85cqi
 * each, 2.6cqi for the current one, `--sp-sm` = 0.9375cqi between), so ten of them
 * occupy ~19.6cqi — about a fifth of the band — at EVERY canvas size. That is what
 * makes a fixed count the responsive answer here: the rail's share of the band is
 * constant, so nothing has to be measured at render time.
 */
const MAX_DOTS = 10;
...
function railHtml(idx, total) {
  ...
  return `<nav class="tile-progress" aria-hidden="true">${dots}</nav>`;
}
