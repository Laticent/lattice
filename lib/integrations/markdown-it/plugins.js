 ...
 * Cells, and each of the nine SOVEREIGN Frames declares exactly one (`stage`).
...
/** The class every slide carries: this slide composes as Form. */
const FORM_CLASS = 'form';
/** The class a slide carries when its Frame is one of the nine sovereign ones. */
 ...
 * split-panel, split-compare, compare-code)?
 *
 * This is the question every chrome injector actually wants to ask. It used to ask
 * "does the section carry `form`?", which worked only because sovereign slides were
 * denied the class — encoding "which Frame" as "is this Form at all".
 */
 ...
 * footer Cell and the progress rail? True for the `standard` Frame, false for the
 * nine sovereign Frames. The predicate every chrome injector gates on.
 */
function hostsChromeCells(classAttr) {
  const tokens = (classAttr || '').split(/\s+/).filter(Boolean);
  return tokens.includes(FORM_CLASS) && !isSovereignFrame(classAttr);
}

/**
 * Resolve one section's class list under Form. EVERY slide composes as Form, so
 * every slide gets the class — including a sovereign one, which then also carries
 * `frame-sovereign` so the chrome injectors and the handful of universal
 * `section.form` CSS rules can tell the two Frames apart POSITIVELY.
 *
 * Sovereignty stopped being spelled as an absent class on 2026-09-20. It is a
 * property of the Frame: the chrome-hosting `standard` Frame declares nine Cells,
 * each sovereign Frame declares one. Withholding `form` made "which Frame" read as
 * "is this Form", which is the confusion this removes.
 *
 * Idempotent, and there is no opt-out token: a deck cannot refuse Form.
 */
function formToggleClass(classAttr) {
  const tokens = (classAttr || '').split(/\s+/).filter(Boolean);
  const sovereign = isSovereignFrame(classAttr);
  if (sovereign && !tokens.includes(SOVEREIGN_CLASS)) tokens.push(SOVEREIGN_CLASS);
  if (!tokens.includes(FORM_CLASS)) tokens.push(FORM_CLASS);
  const next = tokens.join(' ');
  return next === (classAttr || '').trim() ? classAttr : next;
}

/**
 * HTML-stage helper: apply Form to rendered HTML by adding the `form` class to
 * every eligible TOP-LEVEL `<section>`. Runs FIRST, before the registry, so
 * masthead-lift et al. see the class. Uses the depth-aware splitSections walker,
 * so it only rewrites real slide sections — a raw `<section>` in slide content is
 * nested and left untouched. (lib/engine — which the emulator runs — applies this
 * same function in its renderHtml, before its registry pass.)
 *
...
function applyFormToHtml(html) {
  return splitSections(html).map((p) => {
    if (p.type === 'gap') return p.text;
    const next = formToggleClass(p.cls);
