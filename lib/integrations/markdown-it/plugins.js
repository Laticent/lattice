...
function formToggleClass(classAttr, mode = 'standard') {
  ...
  // A math variant that has MIGRATED to the Form frame is no longer sovereign, even
  // though the bare `math` token is still in the skip set — sovereignty is per-layout
  // and this migration lands per-variant. Temporary; see lib/core/math-stage-migration.js,
  // which is deleted along with this line when the last variant moves.
  if (!mathSectionMigrated(classAttr) && tokens.some((t) => FORM_TOGGLE_SKIP.includes(t))) return classAttr;
  tokens.push('form');
  return tokens.join(' ');
}
