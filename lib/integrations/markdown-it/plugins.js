 ...
 *   [ ] → fail + state-empty (verdict-grid) · todo + state-todo (everywhere else)
 *   [/] → skip + state-slashed
 */
 ...
 *
 * `[ ]` decodes by layout. verdict-grid is the ONE place it means "not met" (red
 * ✕); pricing takes the universal neutral ring every other consumer draws —
 * checklist, obligation-matrix, `state-cells`, an inline `` `[ ]` ``. Pricing used
 * to inherit verdict-grid's reading with the decoder, so an unchecked feature row
 * drew a red ✕ on a pricing card and an open ring everywhere else.
 */
