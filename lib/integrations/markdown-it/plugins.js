 ...
 *
 * A KNOWN GAP, AND A REGRESSION: a table the author wrote as RAW HTML is not
 * reached. markdown-it hands it through as an `html_block`, so its rows never
 * enter this token stream and the table goes unstamped — where `compare-table`
 * styled `td:first-child` unconditionally in component CSS and caught raw HTML
 * for free. Measured on a two-slide probe through the real emulator: the old
 * component bolds the raw table's first column, the renamed one does not.
 *
 * `applyToDom` in lib/core/table-row-label.js WOULD stamp it, because a raw
 * table and a pipe table are the same element by the time it runs. That does
 * not make the gap a cross-path SPLIT on every surface, and an earlier draft of
 * this block said it did: the CLI `.html` export never boots the runtime at all
 * (measured in a real browser — `window.Lattice` undefined, raw cell at weight
 * 400), so it is plain there too. The surfaces the DOM pass actually runs on are
 * the VS Code Marp preview and a Marp render of an exported bundle, and neither
 * was driven for this claim. See the decision record's § "The raw-HTML
 * regression" for every route considered — the CSS one the ownership gate
 * closes, the `lib/engine` one the browser bundle closes, and the one that
 * survives: let the CLI judge in the Chromium it already opens for the render.
 * That last one changes exported bytes, which is a human's call to sign off.
 *
