...
const { fitClassFromFrontMatter, isGuardsToken } = require('../../core/resolve-guards');
...
const { backdropClassesFromFrontMatter, isBackdropStrengthToken, isBackdropMaskToken } = require('../../core/resolve-backdrop');
const { venueClass, isVenueToken } = require('../../core/resolve-venue');
...
function deckClassPropagate(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'deck_class_propagate', (state) => {
    ...
    // Deck-wide FIT register (`fit: report|heal|trim`, old spelling `guards:`) — what the
    // engine may do to make a slide fit. `heal`/unknown map to no token, the baseline every
    // deck rendered at before the register existed. A per-slide `_class: fit-<level>` wins.
    // See lib/core/resolve-guards.js.
    const fitToken = fitClassFromFrontMatter(fmMatch[1]);
    ...
    const venueName = frontMatterName(fmMatch[1], 'venue') || '';
    ...
    // Deck-wide BACKDROP restraint (`backdrop: 40 clear` → `backdrop-40 backdrop-clear`) —
    // dims / masks whatever finish the deck wears, over the finish's own baked value. Two
    // axes, each overridden per-slide on its own. See lib/core/resolve-backdrop.js.
    const backdropTokens = backdropClassesFromFrontMatter(fmMatch[1]);
    ...
    for (const token of state.tokens) {
      ...
      // A per-slide backdrop token wins over the deck's on ITS axis only — a slide can keep
...
/**
 * markdown-it plugin: keep a backslash escape of an ARROW character visible in the HTML.
 *
 * markdown-it consumes `\->` into a plain `->` (its `text_join` core rule folds escape
 * tokens into text), so a kernel reading the rendered HTML cannot tell an escaped
 * arrow from a real one. The flowchart's grammar gives `\-` `\=` `\<` `\>` `\&` a meaning
 * — "this is text, not an arrow or a fan-out" — and its Markdown reader (the linter)
 * sees the backslash, so without this the two readers disagree on the same source.
 * Wrapping only those five characters, only when escaped, as `<span data-esc>`, lets
 * the HTML reader put the backslash back (`lib/core/flowchart-grammar.js`,
 * `outlineFromHtml`). No deck in the repository escapes any of the five, so no existing
 * render changes; the span has no style.
 */
const ESCAPE_MARKED = new Set(['-', '=', '<', '>', '&']);
function escapeMarks(markdown) {
  markdown.core.ruler.before('text_join', 'lattice_escape_marks', (state) => {
    for (const block of state.tokens) {
      if (block.type !== 'inline' || !block.children) continue;
      for (const t of block.children) {
        if (t.type !== 'text_special' || t.info !== 'escape' || !ESCAPE_MARKED.has(t.content)) continue;
        t.type = 'html_inline';
        t.content = `<span data-esc>${markdown.utils.escapeHtml(t.content)}</span>`;
      }
    }
  });
}
