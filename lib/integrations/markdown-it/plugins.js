...
function registerShellHljs(marp, shellLanguage) {
  try {
    ...
    // runs per markdown-it build, and compiling bash's ~250-entry built_in list
    // into a regex on every rebuild is real work on the Studio's typing path.
    // `getLanguage` returns the COMPILED language, which keeps `name`; stock bash
    // answers 'Bash', so this only short-circuits once ours is installed.
