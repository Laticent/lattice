...
function verdictGridBadges(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'verdict_grid_badges', (state) => {
    ...
    for (const token of state.tokens) {
      ...
      // `m[2]` is DECODED text (a typed `&lt;` is `<` by now), and this token is raw
      // HTML, so it is escaped here. The runtime mirror writes the same text through
      // `textContent`; without the escape the two paths disagreed on any `<` or `&`.
