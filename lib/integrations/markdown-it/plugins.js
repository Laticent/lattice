 ...
 *
 * A KNOWN, DELIBERATE GAP: a table the author wrote as RAW HTML is not reached.
 * markdown-it hands it through as an `html_block`, so its rows never enter this
 * token stream, while the DOM mirror sees an ordinary `<table>` and stamps it —
 * so such a table reads plain in the PDF and emphasized in the HTML export.
 *
