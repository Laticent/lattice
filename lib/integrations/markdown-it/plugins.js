...
const { stateClassesFor } = require('../../core/state-marks.js');
const { ROW_LABEL_CLASS, resolveRowLabel } = require('../../core/table-row-label.js');
const {
 ...
 *   · compare-code — it drives its own `> h2` title grid (an explicit four-row
 *     section grid, not the absolutely-positioned arithmetic `math` used to have;
 *     math left this set in 2026-09 and takes `form` like any other component),
...
/**
 * The same verdict, for a table an author wrote as RAW HTML.
 *
 * markdown-it hands raw HTML through as an `html_block` token, so the table
 * walker below never sees its rows — but the DOM mirror does, because by then it
 * is an ordinary `<table>` element. That asymmetry made one deck render two
 * ways: the PDF (engine) left the table unstamped and the HTML export (runtime)
 * stamped it. HARD RULE #1 says the two paths share one source of truth, so the
 * engine has to reach this shape too.
 *
 * Deliberately a narrow reader, not an HTML parser: it wants the header row's
 * cells and the first cell of each body row, which is what the kernel takes.
 * Tag stripping plus the five XML entities is enough for that, and anything it
 * cannot read confidently it declines by returning null, which leaves the table
 * unstamped exactly as before. A table split across several `html_block` tokens
 * (a blank line inside the markup) is one of those cases.
 */
// The five entities plus `&#39;`, decoded in ONE pass. Decoding them in
// SEQUENCE is the "double unescaping" defect: `&amp;` -> `&` first turns
// `&amp;lt;` into `&lt;`, which the next replace then turns into `<`. A single
// regex cannot do that, because scanning resumes AFTER each match.
const RAW_HTML_ENTITIES = Object.freeze({
  '&nbsp;': ' ',
  '&lt;': '<',
  '&gt;': '>',
  '&quot;': '"',
...
/**
 * Strip tags until the string STOPS changing.
 *
 * One pass is not enough and the counterexample is the reason CodeQL calls this
 * "incomplete multi-character sanitization": `<<script>script>` loses the inner
 * `<script>` and the halves close up into a live `<script>`. Looping to a fixed
 * point cannot leave one behind.
 *
 * The text this produces is only ever CLASSIFIED — numeric, marker, placeholder
 * — and never inserted into a document, so nothing here is a sanitizer for
 * output (that is HARD RULE #22's job, in the modules it names). It is written
 * to the safer standard anyway, because the next person to reach for a helper
 * called "strip tags" will not read this paragraph first.
 */
...
function rawHtmlTableCells(html) {
  if (!/<table[\s>]/i.test(html) || !/<\/table>/i.test(html)) return null;
  const rows = [...html.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)].map((m) => m[1]);
  if (rows.length === 0) return null;
  const cellsOf = (row, tag) =>
    [...row.matchAll(new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`, 'gi'))].map((m) =>
      stripTagsToFixedPoint(m[1])
        .replace(/&(?:nbsp|lt|gt|quot|#39|amp);/g, (ent) => RAW_HTML_ENTITIES[ent])
        .replace(/\s+/g, ' ')
        .trim(),
    );
  // The header is the first row carrying `th` — the same rule the DOM mirror
  // uses, so a table with no `thead` resolves identically on both paths.
  const headRow = rows.find((r) => /<th[\s>]/i.test(r));
  const headers = headRow ? cellsOf(headRow, 'th') : [];
  const firstColumn = rows
    .filter((r) => r !== headRow && /<td[\s>]/i.test(r))
    .map((r) => cellsOf(r, 'td')[0] ?? '');
 ...
 * refuses this bet by default and 187 plain tables in this repo have never
 * carried it. `row-label` is how a plain table opts IN.
 *
...
function tableRowLabels(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'table_row_labels', (state) => {
    ...
    for (const token of state.tokens) {
      ...
      if (token.type === 'lattice_slide_close') { classTokens = null; continue; }
      ...
      switch (token.type) {
        ...
        case 'inline':
          ...
          break;
        case 'html_block': {
          // Raw-HTML tables never enter the token table stream; read them here so
          // the engine and the DOM mirror reach the same verdict on one deck.
          const cells = rawHtmlTableCells(token.content || '');
          if (!cells) break;
          if (!resolveRowLabel(classTokens, cells)) break;
          token.content = token.content.replace(/<table([^>]*)>/i, (full, attrs) =>
            /\bclass\s*=/.test(attrs)
              ? full.replace(/\bclass\s*=\s*(['"])(.*?)\1/i, (_m, q, v) => `class=${q}${v} ${ROW_LABEL_CLASS}${q}`)
              : `<table${attrs} class="${ROW_LABEL_CLASS}">`,
          );
          break;
        }
        case 'table_close':
          if (open && resolveRowLabel(classTokens, { headers, firstColumn })) {
            const prev = open.attrGet('class');
            open.attrSet('class', prev ? `${prev} ${ROW_LABEL_CLASS}` : ROW_LABEL_CLASS);
          }
