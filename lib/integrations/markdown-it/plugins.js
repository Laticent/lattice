...
const { COLOR_MODE_TOKENS: COLOR_MODE_TOKEN_LIST } = require('../../core/color-mode');
const { colorModeClass } = require('../../core/resolve-color-mode');
...
function deckClassPropagate(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'deck_class_propagate', (state) => {
    ...
    // legacy `class: dark`/`class: light` color axis. When it is set it WINS: drop any
    // color-axis token the `class:` alias carried so the two can't conflict. When it is
    // absent, the `class: dark`/`class: light` alias still resolves (back-compat).
    const cmm = fmMatch[1].match(/^\s*color-mode:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const colorModeToken = colorModeClass(cmm ? cmm[1] : '');
    ...
    const clm = fmMatch[1].match(/^\s*claim:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const claimTokens = claimClasses(clm ? clm[1] : '').split(/\s+/).filter(Boolean);
    ...
    const pm = fmMatch[1].match(/^\s*stamp:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const stampTokens = stampClass(pm ? pm[1] : '') ? [stampClass(pm[1])] : [];
    const tsm = fmMatch[1].match(/^\s*tone:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const toneStyleTokens = toneStyleClass(tsm ? tsm[1] : '') ? [toneStyleClass(tsm[1])] : [];
    ...
    const spm = fmMatch[1].match(/^\s*spectrum:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const spectrumTokens = spectrumClass(spm ? spm[1] : '') ? [spectrumClass(spm[1])] : [];
    ...
    const sem = fmMatch[1].match(/^\s*spectrum-edge:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const spectrumEdgeTokens = spectrumEdgeClass(sem ? sem[1] : '') ? [spectrumEdgeClass(sem[1])] : [];
    ...
    const scm = fmMatch[1].match(/^\s*spectrum-card:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const spectrumCardTokens = spectrumCardClass(scm ? scm[1] : '') ? [spectrumCardClass(scm[1])] : [];
    const sce = fmMatch[1].match(/^\s*spectrum-card-edge:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const spectrumCardEdgeTokens = spectrumCardEdgeClass(sce ? sce[1] : '') ? [spectrumCardEdgeClass(sce[1])] : [];
    ...
    const stm = fmMatch[1].match(/^\s*spectrum-trim:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const spectrumTrimTokens = spectrumTrimClass(stm ? stm[1] : '') ? [spectrumTrimClass(stm[1])] : [];
    ...
    // resolve-eyebrow.js.
    const rlm = fmMatch[1].match(/^\s*rule:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const ruleTokens = ruleClass(rlm ? rlm[1] : '') ? [ruleClass(rlm[1])] : [];
    const ebm = fmMatch[1].match(/^\s*eyebrow:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const eyebrowTokens = eyebrowClass(ebm ? ebm[1] : '') ? [eyebrowClass(ebm[1])] : [];
    ...
    const hlm = fmMatch[1].match(/^\s*headline:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const headlineTokens = headlineClass(hlm ? hlm[1] : '') ? [headlineClass(hlm[1])] : [];
    ...
    const lfm = fmMatch[1].match(/^\s*lift:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
    const liftTokens = liftClass(lfm ? lfm[1] : '') ? [liftClass(lfm[1])] : [];
    ...
    // When `color-mode:` is set it SUPERSEDES the legacy `class:` color axis — but the
    // deck-wide `class: dark`/`light` is applied to every section by the engine's native
    // directive handling, BEFORE this rule runs. So filtering it out of `deckTokens` (above)
    // isn't enough: we must also STRIP the alias's color tokens the engine already stamped,
    // else a half-migrated deck (`color-mode: light` + a leftover `class: dark`) keeps the
    // stale `dark` and the guard below then suppresses the real `color-light`. A per-slide
    // `_class:` token is NOT in the deck-wide `class:`, so only the deck-wide alias is removed.
    ...
    for (const token of state.tokens) {
      ...
      let cur = (token.attrGet('class') || '').split(/\s+/).filter(Boolean);
...
function matrixGridCells(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'matrix_grid_cells', (state) => {
    ...
    for (const token of state.tokens) {
      ...
      const m = /^\[([x\- ])\]\s*(.*)$/.exec(text);
      if (!m) continue;
      const shape = m[1] === 'x' ? 'cell-filled' : m[1] === '-' ? 'cell-outlined' : 'cell-empty';
      // Only a filled cell's label slot holds a title ([x] Senior) — [-] / [ ]
      // are pure position markers, so any trailing text there is authoring
      // debris, not a slot. But the shape+hue alone carries NO accessible text
      // for "reachable"/"not applicable" (no WORD_MAPS entry keys on this
      // component — lib/transformers/prose-projection.mjs has none), so a
      // visually-hidden span names the state for anything reading the DOM
      // text rather than looking at the rendered shape.
      const stateLabel = { 'cell-filled': '', 'cell-outlined': 'reachable', 'cell-empty': 'not applicable' }[shape];
      const label = shape === 'cell-filled' ? m[2] : '';
      const srSpan = stateLabel ? `<span class="cell-sr-label">${stateLabel}</span>` : '';
      const htmlToken = new token.children[0].constructor('html_inline', '', 0);
      htmlToken.content = `<span class="cell ${shape}">${label}${srSpan}</span>`;
      token.children = [htmlToken];
    }
  });
}
...
/**
 * markdown-it plugin: on a `premise` slide, each top-level `<li>` in the
 * ledger list is authored as one line with four inline segments —
 * `` `01` **Remember** Recall facts, syntax, rules *How is this done?* `` —
 * a number (code), a verb (strong), a description (bare text), and a
 * framing question (em). `<code>`/`<strong>`/`<em>` already exist as
 * distinct elements the CSS can grid-position directly; only the bare
 * description text has no element to hang a grid-column on, so this wraps
 * exactly that span (between `</strong>` and `<em>`) in
 * `<span class="premise-desc">`. Scoped to `list_item_open`..
 * `list_item_close` so it never touches the heading/lede's own inline
 * content even if that prose happens to contain a strong+em pair.
 */
function premiseRows(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'premise_rows', (state) => {
    let inPremise = false;
    let inLi = false;
    for (const token of state.tokens) {
      if (token.type === 'lattice_slide_open') {
        inPremise = /\bpremise\b/.test(token.attrGet('class') || '');
        inLi = false;
        continue;
      }
      if (token.type === 'lattice_slide_close') {
      ...
      if (!inPremise) continue;
      if (token.type === 'list_item_open') { inLi = true; continue; }
      if (token.type === 'list_item_close') { inLi = false; continue; }
      if (!inLi || token.type !== 'inline' || !token.children) continue;
      const children = token.children;
      let strongCloseIdx = -1;
      let emOpenIdx = -1;
      for (let i = 0; i < children.length; i++) {
        if (children[i].type === 'strong_close') strongCloseIdx = i;
        if (children[i].type === 'em_open' && strongCloseIdx !== -1 && emOpenIdx === -1) emOpenIdx = i;
      }
      if (strongCloseIdx === -1 || emOpenIdx === -1 || emOpenIdx <= strongCloseIdx + 1) continue;
      const Ctor = children[0].constructor;
      const closeTok = new Ctor('html_inline', '', 0);
      closeTok.content = '</span>';
      const openTok = new Ctor('html_inline', '', 0);
      openTok.content = '<span class="premise-desc">';
      children.splice(emOpenIdx, 0, closeTok);
      children.splice(strongCloseIdx + 1, 0, openTok);
    }
  });
}
...
function slotLabelLift(markdown) {
  ...
  // (`section.list-steps.chevron` …) so the two implementations stay equivalent.
  const LIST_STEPS_HOST = /(?<![\w-])list-steps(?![\w-])/;
  const LIST_STEPS_LIFT_VARIANTS = /(?<![\w-])(chevron|converge|ghost)(?![\w-])/;
