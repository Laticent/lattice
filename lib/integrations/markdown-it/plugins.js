...
const listTabularMarksKernel = require('../../core/list-tabular-marks');
...
/**
 * markdown-it plugin: on a `list-tabular` slide, promotes a trailing sublist
 * bullet that holds only a state marker and/or inline pills into the row's MARKS
 * cell — `class="marks [state {sem} {shape}]"` on the nested `<li>`, marker
 * stripped. CSS then lands it right-aligned in the trailing column and draws the
 * marker as a disc instead of leaving a typed `[x]` on the slide (HARD RULE #29).
 *
 * Which bullets qualify comes from the shared kernel (lib/core/list-tabular-
 * marks.js), so this path and the runtime's DOM mirror cannot drift.
 */
// markdown-it block tokens with no `_open` form. Kept beside `singleBlockItem`
// because they are the reason it exists in the shape it does.
const LEAF_BLOCK_TOKENS = new Set(['fence', 'code_block', 'html_block', 'hr']);

/**
 * Does this `list_item_open` at index `i` hold exactly ONE content block?
 *
 * A bullet with a marker AND a following paragraph is prose, not a status, and it is
 * the shape that made the two render paths disagree: this plugin consumes the FIRST
 * `inline` token after the item opens and is blind to a second paragraph, so it
 * stripped the `[x]`, while the DOM mirror's `contentHost` — which requires a lone
 * `<p>` — left the item alone and a Marp render kept the marker on the reader's
 * slide. The CSS made it worse still, flattening the second paragraph into the
 * cell's flex row. So the marker-bearing bullet must be a single block on both
 * paths, and this is where the engine says so.
 *
 * Counts only blocks at the ITEM's own level: a nested list inside the bullet raises
 * the depth and its paragraphs are not this item's.
 */
function singleBlockItem(tokens, i) {
  let depth = 0;
  let blocks = 0;
  for (let j = i + 1; j < tokens.length; j++) {
    ...
    // A NESTED LIST counts, wherever it sits: `bullet_list_open` at depth 0 is a
    // second block of this item, and everything under it belongs to that list.
    if (depth > 0) continue;
    if (t.level !== tokens[i].level + 1) continue;
    // `endsWith('_open')` alone misses every LEAF block — `fence`, `code_block`,
    // `html_block` and `hr` have no opening form — and those were exactly the four
    // shapes that kept slipping through while the DOM mirror rejected them.
    const isBlock = t.type.endsWith('_open') || LEAF_BLOCK_TOKENS.has(t.type);
    if (!isBlock) continue;
    blocks++;
...
function listTabularMarks(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'list_tabular_marks', (state) => {
    let inTabular = false;
    let listDepth = 0;
    let outerIsOrdered = false;
    let innerIsBullet = false;
    let pendingItemOpen = null;
    for (let i = 0; i < state.tokens.length; i++) {
      const token = state.tokens[i];
      if (token.type === 'lattice_slide_open') {
        inTabular = /(?<![\w-])list-tabular(?![\w-])/.test(token.attrGet('class') || '');
        listDepth = 0;
        outerIsOrdered = false;
        innerIsBullet = false;
        pendingItemOpen = null;
        continue;
      }
      ...
      if (!inTabular) continue;
      if (token.type === 'bullet_list_open' || token.type === 'ordered_list_open') {
        listDepth++;
        // The ledger is keyed to `ol > li` — a `ul` at the top level produces no
        // rows at all (it is the component's second documented mistake). Promoting
        // a bullet inside one would tag markup no CSS renders, and the runtime's
        // DOM walk, which selects `ol > li > …`, could never agree with it.
        if (listDepth === 1) outerIsOrdered = token.type === 'ordered_list_open';
        // Depth 2 must be a BULLET list. A nested `<ol>` cannot carry a marks cell
        // because it is not a sublist as far as this component is concerned: the row
        // selector is `ol > li`, which an inner `ol`'s items match too, so they
        // render as extra ROWS — counter, hairline and all — before any marks rule
        // is reached. That shape is pre-existing and off this change's path; what
        // belongs here is simply not promoting into it.
        if (listDepth === 2) innerIsBullet = token.type === 'bullet_list_open';
        continue;
      }
      if (token.type === 'bullet_list_close' || token.type === 'ordered_list_close') {
        listDepth--;
      ...
      // Depth 2 only — the row itself (depth 1) keeps its name and its inline
      // meta `code`; only a NESTED bullet can become the marks cell.
      if (token.type === 'list_item_open' && listDepth === 2 && outerIsOrdered && innerIsBullet) {
        pendingItemOpen = singleBlockItem(state.tokens, i) ? token : null;
        continue;
      }
      if (token.type !== 'inline' || !pendingItemOpen || !token.children) continue;
      const itemOpen = pendingItemOpen;
      pendingItemOpen = null;
      // The LEADING child only, never a `.find` over the whole inline array: a
      // scan reaches inside author markup and strips a marker that is part of a
      // bold run or a link label (`- **[x] shipped**`), which is not a leading
      // marker at all — and the DOM mirror, which stops at the first element,
      // could never agree with it.
      const firstText = token.children[0]?.type === 'text' ? token.children[0] : null;
      const marker = firstText ? listTabularMarksKernel.readMarker(firstText.content) : null;
      // A pills-only bullet qualifies too, so `- \`stable\` \`v2\`` needs no
      // placeholder checkbox to reach the trailing column.
      const parts = token.children.map((c) => ({ code: c.type === 'code_inline', text: c.type === 'text' ? c.content : '' }));
      if (!marker && !listTabularMarksKernel.isPillsOnly(parts)) continue;
      const cur = itemOpen.attrGet('class');
      const cls = listTabularMarksKernel.MARKS_CLASS;
      itemOpen.attrSet('class', cur ? `${cur} ${cls}` : cls);
      if (!marker) continue;
      // Strip the typed marker and PREPEND the drawn disc (HARD RULE #29). The
      // span is an `html_inline` token rather than an attribute on the <li>,
      // because the cell also holds pills — the disc needs its own box.
      firstText.content = firstText.content.slice(marker.consumed);
      const disc = new token.children[0].constructor('html_inline', '', 0);
      disc.content = listTabularMarksKernel.stateHtml(marker);
      token.children.unshift(disc);
    }
  });
}
