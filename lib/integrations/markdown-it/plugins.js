...
const { pillHtml: inlinePillHtml } = require('../../core/inline-pills.js');
const { stateClassesFor, stateHtml: inlineStateHtml } = require('../../core/state-marks.js');
 ...
 *
 * `markup === '`'` IS THE ESCAPE, and it is why this reads the token rather than
 * the source text. CommonMark preserves the backtick run on the token, so an
 * author who needs the literal writes it in DOUBLE backticks — ``{LIVE}`` — and
 * this rule never sees it. That is the ADR's resolved decision #2, and it is the
 * only per-occurrence opt-out, so it must stay exact: widening this to any
 * `code_inline` would take the escape hatch away.
 */
  ...
  markdown.core.ruler.push('lattice_inline_pills', (state) => {
    for (const token of state.tokens) {
      ...
      for (const child of token.children) {
        if (child.type !== 'code_inline' || child.markup !== '`') continue;
        // State mark FIRST: `[x]` and `{LABEL}` are disjoint by their opening
        // character, so the order is not a precedence rule — it is just the cheaper
        // test first, and either returning null falls through to the other.
        const html = inlineStateHtml(child.content) || inlinePillHtml(child.content);
