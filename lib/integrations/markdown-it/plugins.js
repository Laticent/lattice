...
/**
 * Resolve one section's class list under Form. Appends `form` unless the slide
 * already carries it (idempotent) or its layout is a sovereign Frame. Returns
 * the class string.
 *
 * There is no mode argument and no opt-out token: a deck cannot refuse Form.
 */
function formToggleClass(classAttr) {
  const tokens = (classAttr || '').split(/\s+/).filter(Boolean);
  if (tokens.includes('form')) return classAttr;
  if (tokens.some((t) => SOVEREIGN_FRAMES.includes(t))) return classAttr;
  tokens.push('form');
  return tokens.join(' ');
}
