 ...
 * false for the nine sovereign Frames. The predicate every chrome injector wants.
 */
 ...
 * and the split envelope, and universalising it moved real decks three separate
...
function formToggleClass(classAttr) {
  ...
  if (isSovereignFrame(classAttr)) return classAttr;
 ...
 *     `premise`, `scene`, `split-panel`, `split-compare`, `compare-code`).
 *
 ...
 *
 * Idempotent: a tag that already carries them is returned unchanged.
 */
function stampFormAttrs(openTag, classAttr) {
  let tag = openTag;
  if (!/\sdata-form="/.test(tag)) {
    tag = tag.replace(/^<section\b/, `<section data-form="${FORM_MEDIUM}"`);
  }
  if (!/\sdata-frame="/.test(tag)) {
    tag = tag.replace(/^<section\b/, `<section data-frame="${frameIdFor(classAttr)}"`);
  }
  return tag;
}
