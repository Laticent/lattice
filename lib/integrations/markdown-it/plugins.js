...
const { cardsClass, CARDS_TOKENS } = require('../../core/resolve-cards');
...
function deckClassPropagate(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'deck_class_propagate', (state) => {
    ...
    // Deck-wide CARD-ROW alignment (`cards: center|top|spread` → `cards-*`) — where a
    // sparse card row puts the height it does not need, overridden per-slide
    // (`_class: cards-*`). See lib/core/resolve-cards.js. `stretch`/unknown map to no
    // token, which is the default and leaves every component's own value in place.
