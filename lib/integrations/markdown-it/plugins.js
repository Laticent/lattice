...
const { inlineCodeClass, isLiteralFromSource } = require('../../core/resolve-inline-code');
...
function inlinePills(markdown) {
  markdown.core.ruler.push('lattice_inline_pills', (state) => {
    // `inline-code: literal` turns the whole grammar off for this deck — every
    // single-backtick span stays a `<code>`, INCLUDING one carrying a backslash, which is
    // deliberate: with no grammar running there is nothing to escape FROM, and stripping
    // the backslash would silently edit an author's text. The same source idiom the other
    // front-matter readers in this file use. See lib/core/resolve-inline-code.js.
    const src = (state.env && (state.env.markdown || state.env.source)) || state.src || '';
