...
function deckClassPropagate(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'deck_class_propagate', (state) => {
    ...
    if (!fmMatch) return;
    const classTokensRaw = (frontMatterValue(fmMatch[1], 'class') || '').split(/[ \t]+/).filter(Boolean);
    ...
    const cmm = fmMatch[1].match(/^[ \t]*color-mode:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const fm = fmMatch[1].match(/^[ \t]*finish:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    const finishTokens = finishClasses(fm ? fm[1] : '').split(/\s+/).filter(Boolean);
    const sm = fmMatch[1].match(/^[ \t]*mode:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const clm = fmMatch[1].match(/^[ \t]*claim:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const pm = fmMatch[1].match(/^[ \t]*stamp:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    const stampTokens = stampClass(pm ? pm[1] : '') ? [stampClass(pm[1])] : [];
    const tsm = fmMatch[1].match(/^[ \t]*tone:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const spm = fmMatch[1].match(/^[ \t]*spectrum:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const sem = fmMatch[1].match(/^[ \t]*spectrum-edge:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const scm = fmMatch[1].match(/^[ \t]*spectrum-card:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    const spectrumCardTokens = spectrumCardClass(scm ? scm[1] : '') ? [spectrumCardClass(scm[1])] : [];
    const sce = fmMatch[1].match(/^[ \t]*spectrum-card-edge:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const stm = fmMatch[1].match(/^[ \t]*spectrum-trim:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    // resolve-eyebrow.js.
    const rlm = fmMatch[1].match(/^[ \t]*rule:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    const ruleTokens = ruleClass(rlm ? rlm[1] : '') ? [ruleClass(rlm[1])] : [];
    const ebm = fmMatch[1].match(/^[ \t]*eyebrow:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const hlm = fmMatch[1].match(/^[ \t]*headline:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
    ...
    const lfm = fmMatch[1].match(/^[ \t]*lift:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
 ...
 *
 * Ordered AFTER `deck_class_propagate` deliberately: the deck-wide `class:`
 * register can name the component for every slide (`class: content`), and that
 * has to count as "this slide has a component" or a whole deck would carry the
 * token twice. Unlike propagation, this rule has nothing to read from front
 * matter, so it runs unconditionally — a deck with no front matter at all is
