...
function deckClassPropagate(markdown) {
  markdown.core.ruler.after('lattice_slide_containers', 'deck_class_propagate', (state) => {
    ...
    // Deck-wide SPECTRUM CARD toggle (`spectrum-card: on` → spectrum-card) — the opt-in
    // spectrum rail on card surfaces. `off`/unknown map to no token (the default). Per-slide
    // `_class: spectrum-card` / `spectrum-card-off` overrides. See lib/core/resolve-spectrum.js.
    ...
    for (const token of state.tokens) {
      ...
      // A per-slide spectrum-card token (`spectrum-card` in / `spectrum-card-off` out) wins
      // over the deck-wide `spectrum-card: on`.
