...
function formToggleClass(classAttr) {
  ...
  if (isSovereignFrame(classAttr)) {
    ...
    return tokens.includes(FORM_CLASS) ? tokens.filter((t) => t !== FORM_CLASS).join(' ') : classAttr;
  }
...
const ATTR_VALUE = "(?:\"[^\"]*\"|'[^']*'|[^\\s>]*)";
const STRIP_FORM = new RegExp(`\\s+data-form=${ATTR_VALUE}`, 'g');
// No lookbehind. An earlier version guarded with `(?<![-\w])` against a prefixed sibling
// like `data-lattice-data-frame`, which was inert — the pattern requires WHITESPACE
// immediately before `data-frame=`, and a prefixed name has none — and actively wrong:
// once `data-form` is stripped first, `data-frame` follows `<section` directly, the
// lookbehind sees the `n`, the strip is skipped and a second pass emits the attribute
// TWICE. Idempotency is asserted below and is what caught it.
const STRIP_FRAME = new RegExp(`\\s+data-frame=${ATTR_VALUE}`, 'g');
...
function stampFormAttrs(openTag, classAttr) {
  ...
  return openTag
    .replace(STRIP_FORM, '')
    .replace(STRIP_FRAME, '')
    .replace(/^<section\b/, `<section${attrs}`);
}
...
function applyFormToHtml(html) {
  return splitSections(html).map((p) => {
    ...
    let openTag = next === p.cls
      ? p.openTag
      : (/\sclass="/.test(p.openTag)
