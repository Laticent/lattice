 ...
 * `marginx/marginy: 8` on the sub-graph mermaid hands to dagre; no config
 * reaches it, and `flowchart.padding` provably does not move it (raising it from
 * 8 to 24 leaves cluster-minus-node constant at 70 x 100 user units).
 *
 * 10 is `--sp-sm` RENDERED. The value cannot read the token: this is a number in
 * the diagram's own viewBox space, and the diagram is then fitted to the slide —
 * at 1.40x-2.17x on examples/containment-tier.md, median 1.64. 10 x 1.64 = 16.4px
 * against `--sp-sm`'s 16px at hd.
 *
 * ONE constant, both render paths (HARD RULE #1). They disagreed before: this
 * path never set it, taking mermaid's built-in 8, while lib/runtime/index.js set
 * 15 — so the same deck's nodes were one size in preview and another in the
 * exported PDF.
 */
...
function engineInitConfig(themeVars) {
  return {
    ...
    // The node label inset, from Lattice's spacing scale rather than mermaid's
    // built-in 8 — see DIAGRAM_NODE_PADDING. An author's own `flowchart` block
    ...
    // config that reaches the cluster's INTERNAL space, and any non-zero value
    // shifts the subgraph title out of the box that mermaid already sized — the
    // title renders clipped in half. Verified by rendering top/bottom at 5/10,
    // 6/12 and 10/100; all three clip. The runtime carried 10/100 and did exactly
    // that in live preview.
    flowchart: {
