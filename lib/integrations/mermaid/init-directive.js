...
// NO `\s*` AROUND THE PAYLOAD GROUP, and the group is greedy, not lazy. Both are
// ReDoS guards, not style. `\s*(…[\s\S]…*?)\s*` makes whitespace matchable by two
// adjacent quantifiers, so a directive with a long whitespace run and no closing
// `}%%` sends the engine into quadratic backtracking — an author can hang the
// build with one malformed fence (measured: 2 000 spaces did not finish).
// Bounding the group with only the literal terminator leaves exactly one way to
// match any given end position, which is linear. The payload is trimmed in code
// instead — see readAuthorInit.
const INIT_DIRECTIVE_RE = /%%\{\s*(?:init|initialize)\s*:((?:(?!\}%%)[\s\S])*)\}%%/g;
...
function readAuthorInit(definition) {
  ...
  let m;
  while ((m = INIT_DIRECTIVE_RE.exec(definition)) !== null) payloads.push(m[1].trim());
  if (!payloads.length) {
    ...
    // Case-sensitive for the same reason as INIT_DIRECTIVE_RE above.
    return /%%\{\s*(?:init|initialize)\s*:/.test(definition)
