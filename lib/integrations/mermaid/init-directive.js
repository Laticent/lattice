 ...
 * never again.
 *
 * IT IS EMITTED UNCONDITIONALLY, and the "only under `handDrawn`" it replaced was
 * wrong — measured, not reasoned. The old rule rested on "rough.js is the hand-drawn
 * renderer, so a classic deck cannot reach it." Mermaid does not honor that
 * boundary: `classBox` (Mermaid 11's class-node shape) builds its box through
 * rough.js on EVERY render and merely flattens the wobble afterwards —
 * `if (node.look !== 'handDrawn') { options.roughness = 0 }`, and `roughness` is a
 * multiplier applied AFTER the draws are taken. rough.js's `_line` spends one draw
 * on `divergePoint = 0.2 + random(o) * 0.2`, which positions the two bezier control
 * points ALONG the segment and is never scaled by roughness. So a classic
 * `classDiagram` still consulted `handDrawnSeed`, still found Mermaid's default `0`,
 * and still fell through to `Math.random()`.
 *
 * The result is invisible and real at once. Both control points stay ON the straight
 * line between the endpoints, so the cubic IS that segment however far along it they
 * land: two renders rasterize pixel-identical and emit DIFFERENT `<path d>` text.
 * That is the whole of the `classDiagram` byte-irreproducibility — measured on a
 * two-line classic deck as 12 differing `<path d="…">` lines out of 161, all of them
 * the rough.js double-stroke, and zero changed pixels.
 *
 * The old rule's stated cost ("emitting it on a classic deck would change that deck's
 * directive bytes for no behavioral gain") had the trade backwards: the directive
 * grows by one stable key, and what it buys is that every classic diagram deck stops
 * re-writing its own goldens. Classic decks that never touch rough.js are unaffected
 * either way — a key nobody reads changes no pixels.
 */
...
function engineInitConfig(themeVars, { look, omitPalette = false } = {}) {
  return {
    ...
    // …and the TOP-LEVEL `fontFamily`, which is a separate key from
    // `themeVariables.fontFamily` and does NOT follow it: measured, after
    // `mermaid.initialize` with a hand face in themeVariables, `getConfig().fontFamily`
    // was still `"trebuchet ms", verdana, arial, sans-serif`. It reaches the SVG as
    // `--mermaid-font-family`, which nothing in mermaid 11.14's emitted CSS currently
    // consumes — so leaving it stock changes no pixel today. It is set anyway because
    // "no consumer today" is not a property of the key, and because the alternative was
    // an exemption in the parity gate justifying a hole nobody could see.
    ...(omitPalette || !themeVars.fontFamily ? {} : { fontFamily: themeVars.fontFamily }),
    // ── Mermaid SECURE KEYS ───────────────────────────────────────────────────
    ...
    // `look` is spread rather than assigned so a classic render emits no key.
    // The SEED does not ride with it — it is stated on every render, classic
    // included, because `classBox` draws through rough.js whatever the look and an
    // unset seed is `Math.random()`. See DIAGRAM_HAND_DRAWN_SEED for the measurement.
    ...
    // OMITTED WITH THE PALETTE. A `theme:` pin is an opt-out of the deck's palette, and
    // the font is part of that palette — it rides `themeVariables.fontFamily` for every
    // other family. Sending these while standing `themeVariables` down gave a pinned
    // diagram stock colors wearing the deck's type, which is neither answer: before
    // #1674 a pinned diagram got no engine config at all and used mermaid's own font.
    // The stand-down is all-or-nothing or it is not a stand-down.
    ...(omitPalette ? {} : perDiagramFonts(themeVars.fontFamily)),
    ...
    c4: {
      ...(omitPalette ? {} : perDiagramFonts(themeVars.fontFamily).c4),
      c4ShapeInRow: 3,
      c4BoundaryInRow: 1,
    },
  };
