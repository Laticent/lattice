...
function tagMermaidMotion(svg) {
  ...
  if (svg.querySelector('g.nodes')) {
    ...
    let last = 0;
    const cols = svg.querySelectorAll('rect.actor-top, line.actor-line, rect.actor-bottom');
    for (let i = 0; i < cols.length; i++) {
      const el = cols[i];
      if (el.hasAttribute('data-anima-role')) continue;
      const cls = String(el.getAttribute('class') || '');
      const center = num(el, 'x') + num(el, 'width') / 2;
      const key = el.tagName.toLowerCase() === 'line' ? num(el, 'x1') : cls.indexOf('actor-top') !== -1 ? num(el, 'x') : center + 0.5;
