...
async function renderAll(job) {
  ...
  const launchOpts = {
    // `chromeArgs` carries the CLI's network containment (lib/core/offline-chromium.js).
    args: ['--no-sandbox', '--disable-setuid-sandbox', ...(Array.isArray(job.chromeArgs) ? job.chromeArgs : [])],
