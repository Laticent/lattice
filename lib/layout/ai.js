...
const { NAME_RE, FUNCTIONS, BUCKETS, FORMS, CSS_ONLY_SUBSTANCES, findUnscopedSelectors, splitSelectorList } = require('./gate.js');

// The legible-ceiling + reflow shapes from manifest.schema.json (§6 audit).
// Derived from the manifest schema (the contract's source of truth) — was a
// hand-mirror. Order-insensitive use only (snapEnum membership test).
const ADAPT_MODES = require('../components/manifest.schema.json').properties.adapt.properties.mode.enum;
...
const COMPONENT_CANON =
  ...
  'leading bracket marker on a list item OR a table cell is AUTO-CHROME the engine decodes into a color-blind-safe status disc ' +
  '(a shape + a state token). SIX markers, each with ONE meaning in every layout: `[x]` = yes/done/met, `[-]` = partly, ' +
  '`[!]` = no/failed/not met, `[?]` = unknown (looked at, cannot be settled), `[ ]` = open (not started / not assessed), ' +
  '`[/]` = does not apply/out of scope. A criterion that FAILED is `[!]`, never `[ ]`. Author `- [x] Item` (or ' +
  '`| Clause | [x] | [!] |`); do NOT style the `.state` disc ' +
  ...
  '`--text-secondary`, `--text-muted` (de-emphasized TEXT, AA), `--muted-mark` (de-emphasized DECORATION — rules, hairlines, ' +
  'empty marks; 3:1, never text); accent `--accent`, `--accent-soft`, `--on-accent`, `--on-accent-soft`; state `--pass`, ' +
  ...
  '\n' +
  '• FAMILY REFLOW for portrait/tall frames: attach the family filter to the SECTION compound — ' +
  '`section.<name>.<name>:where([data-family="square"], [data-family="tall"], [data-family="strip"]) > .cell-stage > ul > li { width:100% }` — to drop to a single column. The engine stamps ' +
  '`data-family` ON the `section` from the deck geometry, so the filter must sit on that same compound: a LEADING ' +
  '`:where([data-family=…]) section.<name>` is a DESCENDANT combinator and matches nothing. `wide` (>1.05 aspect) carries NO stamp, ' +
  '`square` / `tall` / `strip` do — so name the families the rule applies to and omit `wide`. `:where()` adds ZERO specificity, so ALSO ' +
  'use the DOUBLED-CLASS trick (`section.<name>.<name>`) so the reflow rule outranks the base multi-column rule (the `width:calc(50% …)` ' +
  'on the flex cards, or a `flex-direction` on an even split). This is the single most error-prone idiom; copy it exactly. Do NOT use ' +
  '`@container … aspect-ratio` — a container query measures the section CONTENT box, which drifts off the deck aspect and silently ' +
  'fails to match (issue #1218).\n' +
  '\n' +
  ...
  '(photo-led) · chart · diagram · math · code · legal · connect (contact / QR / wifi cards). Pick the `function`/`form`/`substance` that ' +
  'matches; for a transform-free CSS component stay in inventory/comparison/evidence/legal/connect (+ code/math prose).\n' +
  '\n' +
  ...
  'TALL/strip one, because the same Tile is dropped into landscape decks and portrait/social frames. Bake in the ' +
  '`section.<name>.<name>:where([data-family="square"], [data-family="tall"], [data-family="strip"])` single-column drop (the FAMILY REFLOW idiom above) as part of the FIRST draft, not a ' +
  ...
  'concept earns it. The neutral surface/border/`--muted-mark` tokens carry most decorative work without touching the accent\n' +
  ...
  'scaling + the `data-family` reflow above; `{"mode":"reflow"}` once it ships per-family layouts) and `capacity` (its legible ceiling — ' +
...
const EXAMPLE_REUSE =
  ...
  '  box-sizing:border-box; width:calc(50% - var(--sp-md) / 2);\n' +
  '  background:var(--bg-alt); border:1px solid var(--border); border-left:3px solid var(--muted-mark);\n' +
  ...
  'section.status-cards.status-cards:where([data-family="square"], [data-family="tall"], [data-family="strip"])\n' +
  '  > .cell-stage > ul > li { width:100%; }\n' +
  'skeleton:\n' +
  '<!-- _class: status-cards -->\n\n## Capability status\n\n- Ingest pipeline\n- Scoring engine\n- Audit trail\n';
...
const EXAMPLE_COMPARE =
  ...
  '}\n' +
  'section.build-buy.build-buy:where([data-family="tall"], [data-family="strip"])\n' +
  '  > .cell-stage > ul { flex-direction:column; }\n' +
  'skeleton:\n' +
...
/** A gate-clean TERMINAL/CONSOLE panel — the exact shape a model botches with raw
 *  #00ff00 hex. The dark "screen" is NOT a hardcoded green-on-black: it INVERTS the
 *  tokens (background:var(--text-heading); color:var(--bg)) and uses the state tokens
 *  for status. ZERO hex, ZERO margin — a concrete instance of the DARE bullet's rule. */
const EXAMPLE_TERMINAL =
  'WORKED EXAMPLE F — a terminal/console STATUS panel ("a sci-fi diagnostic console", "a hacker-green status readout"). The ' +
  'dark "screen" is NOT `#00ff00` on `#000` — that is hex, which the gate REJECTS. INVERT the tokens: the panel background is ' +
  '`var(--text-heading)` (darkest ink), its text is `var(--bg)` (lightest surface); the status chips are ' +
  '`var(--pass)`/`var(--warn)`/`var(--fail)` (green/amber/red). Spacing is gap/padding (NEVER margin); a monospace look is a ' +
  '`font-family` only.\n' +
  'manifest: {"name":"status-console","function":"inventory","form":"panel","substance":"structure","bucket":"inventory",' +
  '"tags":["console","status","terminal"],"description":"A dark diagnostic console — systems each with a status chip.",' +
  '"adapt":{"mode":"native"},"capacity":{"sweet":4,"soft":6,"hard":8},"density":{"axis":"item","soft":8,"hard":14}}\n' +
  'css:\n' +
...
const ASK_SYSTEM =
  ...
  // A terse, load-bearing restatement at the VERY END (recency) of the exact rules the
  // deterministic gate hard-rejects on — the ones models most often miss (raw hex for a
  // "terminal green", a stray margin). SCAN-BEFORE-RETURN framing so the model self-checks.
  '\n\nHARD CONSTRAINTS — the gate REJECTS a draft that breaks ANY of these. Scan your own CSS before returning:\n' +
  '  1. ZERO hex colors — every color is a `var(--…)` token. There is a token PATH for every color you want, so a hex is never ' +
  'needed. A "terminal/console/hacker-green" or any dark panel is NOT `#00ff00`/`#000`: INVERT the tokens ' +
  '(`background:var(--text-heading); color:var(--bg)`), status green/amber/red = `var(--pass)`/`var(--warn)`/`var(--fail)`, a ' +
  'material tint = `color-mix(in srgb, var(--cat-3-mark) 12-22%, var(--bg))`, neutrals = `var(--text-body)`/`var(--muted-mark)`/`var(--border)`.\n' +
  '  2. NO `margin` except a bare `margin:0` reset — space with `gap` (flex/grid children) or `padding` (inside a box); every ' +
  'decorative offset/tilt/scatter is a `transform`. A non-zero margin (incl. on a `::before`/`::after`) is rejected.\n' +
...
// ── Silent gate-repair ────────────────────────────────────────────────────────
// When a generated draft fails the gate, we don't dead-end on the findings — we feed
// the draft + the EXACT violations back to the model to fix, silently, before the user
// sees it (up to a small pass budget; the caller loops + re-gates). This is the
// prompt: fix ONLY the listed violations, keep the design, return the same JSON shape —
// with the concrete remediation for each gate rule (the token map, margin→gap) so the
// model can't repeat the "lack of understanding" that caused the violation. Mirrors the
// model-proposes / gate-disposes contract: the repair is re-gated, never trusted blind.
const REPAIR_SYSTEM =
  'You are fixing a Lattice component draft that FAILED the deterministic gate. Return the SAME compact JSON object shape ' +
  '(name, description, function, form, substance, bucket, tags, adapt, capacity, density, css, skeleton) with ONLY the listed ' +
  'violations corrected — preserve the design, structure, copy, and intent; change nothing the gate did not flag. No prose, no ' +
  'markdown fences — JSON only.\n\n' +
  'HOW TO FIX EACH RULE:\n' +
  '• no-hex — replace the hex with the token that matches its INTENT (never invent a value): a terminal/console/dark panel ' +
  'INVERTS the tokens (`background:var(--text-heading); color:var(--bg)`, inset bar `var(--bg-alt)`/`var(--border)`); status ' +
  'green/amber/red = `var(--pass)`/`var(--warn)`/`var(--fail)`; the brand accent = `var(--accent)`/`var(--accent-soft)`; a ' +
  'material/paper tint = `color-mix(in srgb, var(--cat-3-mark) 12-22%, var(--bg))`; neutrals = `var(--text-body)`/' +
  '`var(--text-secondary)`/`var(--muted-mark)`/`var(--border)`. Categorical hues are `var(--cat-1-mark) … var(--cat-12-mark)`.\n' +
  '• no-margin — margin is BANNED (it corrupts the height math a measuring layout depends on). Replace every non-zero margin ' +
  '(a bare `margin:0` reset is fine) with `gap` (between flex/grid children) or `padding` (inside a box); a decorative offset or ' +
  'tilt becomes a `transform`. This includes margins on a `::before`/`::after`.\n' +
  '• fs-token — `font-size` must be a `--fs-*` role token (`var(--fs-meta)`/`var(--fs-body-compact)`/`var(--fs-body)`/' +
  '`var(--fs-h2)`/…), never a raw px/rem/cqi.\n' +
  '• scope — prefix every selector with `section.<name>` so styles can\'t leak onto other slides.\n' +
  '• skeleton-html / skeleton-script — the skeleton is PURE MARKDOWN; delete any raw `<tag>`/script (show code as `` `inline` `` ' +
  'or a ```fence). Keep the `<!-- _class: <name> -->` directive.\n' +
  '• manifest:* — correct the flagged field to a valid enum/shape (see the field list above).\n' +
  'Return ONLY the corrected JSON.';

/**
 * Messages for ONE silent repair pass: the failing draft + the exact gate findings.
 * `draft` is the coerced component ({name, …manifest, css, skeleton}); `findings` is
 * the ERROR-level gate list ([{rule, line?, message}]). Pure + testable.
 */
function askRepairMessages(draft, findings) {
  const d = draft && typeof draft === 'object' ? draft : {};
  const payload = {
    name: d.name, description: d.description, function: d.function, form: d.form,
    substance: d.substance, bucket: d.bucket, tags: d.tags, adapt: d.adapt,
    capacity: d.capacity, density: d.density, css: d.css, skeleton: d.skeleton,
  };
  const list = (Array.isArray(findings) ? findings : [])
    .map(f => `- ${f.rule}${f.line ? ` (line ${f.line})` : ''}: ${f.message}`)
...
// ── Design self-refine (the "effort" dial) ────────────────────────────────────
// SEPARATE from gate-repair: repair fixes VIOLATIONS (hex/margin/scope); this raises
// DESIGN QUALITY. The caller runs N rounds (effort low/medium/high/maximum → 0/1/2/3),
// each critiquing the current best against the boardroom rubric and returning an
// IMPROVED version + an honest self-rating; the caller keeps the highest-rated (and
// gate-repairs each candidate, so it compares COMPLIANT designs). Same model-proposes /
// code-disposes contract — the rating steers selection but the gate still disposes.
// See 2026-07-19-component-effort-dial.md.
const REFINE_DESIGN_SYSTEM =
  'You are improving the DESIGN QUALITY of a Lattice component that already renders. Critique it hard against the boardroom ' +
  '10/10 rubric, then return an IMPROVED version plus an honest rating.\n\n' +
  ...
  '• HIERARCHY — six distinct levels of emphasis; if two adjacent levels look the same, it is broken. Size to the ROLE: a single ' +
  'number or a one-word verdict is MONUMENTAL (`--fs-hero`/`--fs-emphasis`), never a lonely `--fs-body` line.\n' +
  '• FIT — the structure must FILL the bounded 16:9 stage handsomely: no sparse dead-space, no lonely tile adrift in empty. Cards ' +
  'GROW into the space (`flex:1; min-height:0` on the list so rows distribute), they do not float at intrinsic size.\n' +
  ...
  'HARD RULES you must NOT break (a change that breaks the gate is NOT an improvement): colors are `var(--…)` tokens only (ZERO ' +
  'hex; a dark panel INVERTS tokens — `background:var(--text-heading); color:var(--bg)`; status = `--pass`/`--warn`/`--fail`); ' +
  'NO `margin` (use `gap`/`padding`; offsets are `transform`); `font-size` is a `--fs-*` role token; every selector starts ' +
  '`section.<name>`; the skeleton is PURE MARKDOWN.\n\n' +
  'KEEP THE COMPONENT\'S IDENTITY — the same name, bucket, core idea, and skeleton CONTENT. Improve the CRAFT (hierarchy, fit, ' +
  'restraint, sizing), do NOT swap it for a different component. If it is already excellent and you cannot honestly improve it, ' +
  'return it UNCHANGED with your rating.\n\n' +
  'Return ONLY a compact JSON object — no prose, no fences — the SAME component shape (name, description, function, form, ' +
  'substance, bucket, tags, adapt, capacity, density, css, skeleton) PLUS two integer keys, both 1-10 and on the SAME scale: ' +
  '"baselineRating" — your honest score of the component AS GIVEN TO YOU, before any change; and "rating" — your honest score of ' +
  'the improved design you are returning. Be critical and consistent between them (reserve 9-10 for genuinely boardroom-ready): ' +
  'raise "rating" above "baselineRating" ONLY if you genuinely improved the craft, and if you could not honestly improve it, return ' +
  'it UNCHANGED with "rating" equal to "baselineRating".';

/**
 * Messages for ONE design self-refine round: the current best draft to critique +
 * improve. The reply is coerced by `coerceRefinement` (the component shape + a rating).
 * Pure + testable.
 */
function askDesignRefineMessages(draft) {
  const d = draft && typeof draft === 'object' ? draft : {};
  const payload = {
    name: d.name, description: d.description, function: d.function, form: d.form,
    substance: d.substance, bucket: d.bucket, tags: d.tags, adapt: d.adapt,
...
/**
 * Coerce a design-refine reply: the component (via `coerceComponent`) PLUS a clamped
 * integer `rating` (1-10; defaults to 5 when the model omits or garbles it, so a
 * missing rating never wins or loses by accident). Returns the coerceComponent shape
 * with a `rating` field.
 */
function coerceRefinement(raw) {
  const obj = (typeof raw === 'string' ? safeParse(raw) : raw) || {};
  const c = coerceComponent(obj);
  const r = Number(obj.rating); // obj is always an object here (|| {} above)
  const rating = Number.isFinite(r) ? Math.max(1, Math.min(10, Math.round(r))) : 5;
  // baselineRating: the model's honest score of the ORIGINAL (input) design, so the
  // caller can require a refinement to STRICTLY beat it (the effort-regression guard).
  // null when the model omits it — the caller then falls back to its ungraded bar, so a
  // model that doesn't report a baseline never has its improvements silently rejected.
...
// ── Manual refine (the author's directed nudge) ───────────────────────────────
// DISTINCT from the effort dial's self-refine (askDesignRefineMessages, no
// instruction): here the AUTHOR gives a DIRECTION ("simpler", "make the cards
// bolder", "tighter copy") and the model applies THAT change to the current draft,
// keeping everything else. The Motion faculty's refine, ported to components. The
// reply is coerced by the SAME coerceComponent + gate-repaired by the caller, so a
// nudge can never smuggle a violation past the gate. See 2026-07-19-component-refine.md.
const REFINE_NUDGE_SYSTEM =
  'You are REFINING an existing Lattice component per the author\'s instruction. Apply ONLY the change they ' +
  'ask for and keep EVERYTHING else — the name, bucket, the core idea, and the parts the instruction does not ' +
  'touch. Do not rebuild it from scratch or swap it for a different component; make the smallest change that ' +
  'satisfies the instruction well.\n\n' +
  'PRESERVE THE GATE INVARIANTS (a refinement that breaks the gate is wrong): colors are `var(--…)` tokens only ' +
  '(ZERO hex; a dark panel INVERTS tokens — `background:var(--text-heading); color:var(--bg)`; status = ' +
  '`--pass`/`--warn`/`--fail`); NO `margin` (use `gap`/`padding`; offsets are `transform`); `font-size` is a ' +
  '`--fs-*` role token; every selector starts `section.<name>`; the skeleton is PURE MARKDOWN.\n\n' +
  'Return ONLY the full corrected component as a compact JSON object — the SAME shape (name, description, ' +
  'function, form, substance, bucket, tags, adapt, capacity, density, css, skeleton). No prose, no fences.';

/**
 * Messages for ONE author-directed refine: the current draft + the instruction
 * ("simpler", "make the cards bigger"). Pure + testable. The reply is coerced by
 * `coerceComponent` (the caller then gate-repairs it).
 */
function askComponentRefineMessages(draft, instruction) {
  const d = draft && typeof draft === 'object' ? draft : {};
  const payload = {
    name: d.name, description: d.description, function: d.function, form: d.form,
    substance: d.substance, bucket: d.bucket, tags: d.tags, adapt: d.adapt,
...
function addScopePrefix(css, name) {
  ...
  // Cap the at-rule nesting the rewriter recurses through — the CSS is untrusted,
  // AI-generated, so a pathologically deep nest must not blow the JS stack. Past the
  // cap we bail the whole fix (ok=false → the original stands and the gate flags the
  // leak) rather than descend further. Real component CSS nests one or two levels.
  const MAX_NEST = 32;
  const rewrite = (str, depth = 0) => {
    ...
    while (pos < str.length) {
      ...
      if (depth >= MAX_NEST) { ok = false; return str; }
      if (prelude.startsWith('@')) {
        ...
        // `splitSelectorList` is nesting- and quote-aware, so a comma inside
        // `:is()`/`:where()`/`[attr="a,b"]` never splits a selector in half. The
        // family-reflow idiom writes `section.x:where([data-family="tall"], …)`,
        // which a naive split would corrupt into two broken parts.
        const parts = splitSelectorList(prelude).map(p => p.trim()).filter(Boolean);
...
function coerceComponent(raw) {
  ...
  // LINE ENDINGS: normalize the skeleton HERE, at the coercion, because this is the funnel every
  // model-proposed component crosses (`coerceRefinement` delegates to this function) and the
  // skeleton is markdown that gets spliced verbatim into deck source by `addSlideAfter`. A model
  // that emits CRLF — they do — otherwise produced a mixed-EOL deck that was persisted and shared
  // out that way. `fixCardNesting` also splits on lines, so normalizing first keeps it from
  // seeing a trailing `\r` as part of a card title. `\r\n?` covers CRLF and classic-Mac lone CR;
  // it is a no-op on LF. CSS is left alone: it is never spliced into markdown.
...
module.exports = {
  ASK_SYSTEM, REPAIR_SYSTEM, REFINE_DESIGN_SYSTEM, REFINE_NUDGE_SYSTEM, COMPONENT_CANON,
  askComponentMessages, askRepairMessages, askDesignRefineMessages, askComponentRefineMessages,
  coerceRefinement, coerceComponent, rankSimilar, auditComponentDesign, addScopePrefix,
  slugifyName, fixCardNesting, MAX_CSS_BYTES,
};
