...
/** The names of the component blocks a deck already embeds, in order. */
function embeddedComponentNames(source) {
  const out = [];
  for (const m of String(source || '').matchAll(new RegExp(`/\\* ${COMPONENT_BLOCK_MARK} "([^"]*)"`, 'g'))) out.push(m[1]);
  return out;
}
...
// #22 stylesheet channel — a library component's CSS is model-authored by construction
// (the AI component generator; 2026-06-29-component-transformer-threat-model.md), and this
// splices it into a `<style>` in the markdown handed to a RECIPIENT. A `</style>` in it ends
// the element on their machine and the remainder is parsed as markup. The whole element BODY
// is guarded, not just `css`, because `name` rides in the comment header and is free text too.
// Same shape as `share-export.ts`'s finish block and `deck-export.js`'s theme block.
const { sanitizeStyleText } = require('../core/sanitize-style-text.mjs');
  ...
  return (
    ...
    sanitizeStyleText(
      `/* ${COMPONENT_BLOCK_MARK} "${String(name || 'component')}" (self-contained:\n` +
        ...
        '   Palette-blind — every color is a token. Generated on export. */\n' +
        String(css || '').trim(),
    ) +
    '\n</style>\n'
  );
 ...
 * @returns {string}
 */
function embedComponentsInMarkdown(source, components, { keepExisting = false } = {}) {
  // `keepExisting` ADDS blocks beside the ones the deck carries instead of replacing them —
  // the CLI's installed components, which must never delete CSS a deck brought with it.
  const src = keepExisting ? String(source || '') : stripEmbeddedComponents(source);
