 ...
 *      This is the runtime analog of the `section.<name> …` convention every
...
// DERIVED from lib/components/manifest.schema.json — the manifest contract's
// source of truth. A JSON require is fs-free at runtime (esbuild inlines it
// into the layout-core browser bundle), so this module no longer carries the
// hand-synced copy that had already drifted (it was missing the `connect`
// bucket the schema and loader both knew).
const MANIFEST_SCHEMA = require('../components/manifest.schema.json');
// Same deep-freeze index.js applies: the browser bundle inlines a FRESH copy
// of the JSON (esbuild), so without this the Studio's copy of the contract
// would be mutable while the Node copy is frozen.
...
const FUNCTIONS = Object.freeze(MANIFEST_SCHEMA.properties.function.enum);
const BUCKETS = Object.freeze(MANIFEST_SCHEMA.properties.bucket.enum);
const FORMS = Object.freeze(MANIFEST_SCHEMA.properties.form.enum);
const SUBSTANCES = Object.freeze(MANIFEST_SCHEMA.properties.substance.enum);
...
const NAME_RE = new RegExp(MANIFEST_SCHEMA.properties.name.pattern);
// A hex color literal: #rgb / #rgba / #rrggbb / #rrggbbaa.
const HEX_RE = /#[0-9a-fA-F]{3,8}\b/g;

// The CSS SAFETY scanners live in lib/core/css-scan.js, not here. They are the one
// part of this gate the THEME faculty also needs (lib/theme/gate.js), and reaching
// them through this module drags the 41 KB component manifest schema into the theme
// browser bundle — for two regex scanners that know nothing about components. They
// also now read comments through the canonical `lib/core/css-comments.mjs` walk
// rather than a naive regex, which closed a live bypass: a `content: "/*"` string
// paired with the next real closer and blanked a remote `@import` + `url()` beacon
// out of this gate entirely (measured: `gateCss` returned ZERO findings). See that
// file's header. Re-exported below so this module's public API is unchanged.
const { stripComments, lineAt, findCssExfil, findCssImports } = require('../core/css-scan.js');
 ...
 *
 * A STATEMENT ENDS AT `;`, AND IT USED NOT TO. The scan only ever reset its
 * selector chunk at a `}`, so a `;`-terminated top-level at-rule stayed glued to
 * the rule after it: `@import 'x';\n:root { … }` read as ONE head starting with
 * `@`, which the at-rule branch then discarded — selector and all. Exactly one
 * rule after every such at-rule was invisible, and `findUnscopedSelectors` reported
 * nothing at all for a file whose first rule sat there (measured: 0 findings on the
 * four `themes/a11y-*.css` variants, which are `@import 'a11y-base';` + one `:root`
 * block). `@charset`, `@layer a;` and `@namespace` reach it the same way. A gate that
 * silently stops reporting is worse than no gate — the objection `splitSelectorList`
 * already records, arriving through a different door.
 *
 * Quotes are tracked for the same reason they are tracked there: introducing a `;`
 * boundary without them would cut a selector in half at `[data-x=";"]` and report the
 * remainder as unscoped. (The `{`/`}` matching below is still quote-blind — a brace
 * inside a string mis-slices the scan. Pre-existing, off-path, and left as it is
 * rather than widened into: no shipped or generated CSS carries one.)
 *
 * AND A BACKSLASH ESCAPES THE NEXT CHARACTER OUTSIDE A STRING TOO, which the first
 * cut of that quote tracking missed and which turned it into a FAIL-OPEN. `\'` is a
 * legal escaped character in a class name — `.a\'b` selects the class `a'b` — so
 * reading it as a string opener starts a string that never closes, the walk runs to
 * end of input, and every rule after it is invisible. Measured against real Chromium:
 * `.a\'b { … }` followed by an unscoped `.leaky { … }` made `findUnscopedSelectors`
 * return `[]` and `gateCss` return `ok: true`, while the browser applied `.leaky` to
 * every slide. Escapes are honored first, everywhere, exactly as CSS honors them.
 */
function eachRule(css, visit) {
  const src = stripComments(css);
  let i = 0;
  const n = src.length;
  // Cap the `@media`/`@supports` descent depth — the input is untrusted, AI-generated
  // CSS, so a pathologically nested block must not blow the JS stack (a tab crash).
  // Real component CSS nests one, maybe two levels; 32 is unreachably generous. Beyond
  // it we stop descending (the outer regex gates on hex/margin still apply to the whole
  // string, so nothing structural slips — only selector-scoping of a 32-deep nest is skipped).
  const MAX_NEST = 32;
  function block(end, depth) {
    ...
    while (i < end) {
      const ch = src[i];
      // An escape makes the NEXT character literal wherever it appears — inside a
      // string and out. `.a\'b` is a class named `a'b`, not a string opener.
      if (ch === '\\') {
        i += 2;
        continue;
      }
      ...
      if (ch === ';') {
        // Statement boundary: whatever preceded it was a `;`-terminated at-rule
        // (`@import`, `@charset`, `@layer a;`), not the head of the rule that
        // follows. Drop it so the next selector starts clean.
        i++;
        chunkStart = i;
        continue;
      }
      if (ch === '{') {
        ...
        if (head.startsWith('@')) {
          ...
          // else: opaque at-rule (keyframes/font-face) or past the depth cap — no descent
        } else if (head) {
...
/** Blank the quoted runs in a selector, preserving length. */
function maskSelectorStrings(part) {
  return String(part).replace(/"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'/g, m => ' '.repeat(m.length));
}

/** Does one comma-part of a selector reference `.<name>` as a class token? */
function partScoped(part, name) {
  // A `.name` inside a quoted ATTRIBUTE VALUE is TEXT, not a class token, and reading
  // it as one certifies a selector that leaks: `ul[data-state="} section.x "]` scopes
  // nothing and claimed to scope to `.x`. Mask the quoted runs first — a class token
  // can never live inside them, so nothing legitimate is lost.
  // (Reachable only since `eachRule` became string-aware; before that the brace inside
  // the string ended the "selector" early and the leak was caught by accident. Fixing
  // the walker without fixing this would have traded one blindness for another.)
  const bare = maskSelectorStrings(part);
 ...
 * `:where()`, `:not()`, or an attribute value belongs to that construct, not to
 * the list — `section.foo:where([data-family="tall"], [data-family="strip"]) > .x`
 * is ONE scoped selector, and a naive `split(',')` reads it as two, the second of
 * which (`[data-family="strip"]) > .x`) looks unscoped. The family-reflow idiom
 * (lib/adaptive/families.js) writes exactly that shape, so the split has to see
 * nesting.
 */
function splitSelectorList(selector) {
  ...
  for (const ch of selector) {
    if (quote) {
      cur += ch;
      // A CSS string escapes its own delimiter with `\`, so `[t="a\", b"]` is
      // ONE attribute value. Without this the `\"` closed the string, the rest
      // of the selector ran at depth 1 forever, and every comma-part merged into
      // one — which `partScoped` then read as scoped, so a genuinely unscoped
      // part went unreported. A gate that silently stops reporting is worse than
      // no gate; the input here is AI/Studio-authored CSS, so it is adversarial
      // by construction.
    ...
    // Clamp at 0: an unbalanced `)` in malformed input must not drive depth
    // negative, where no later comma can ever be "top-level" again.
...
function findSkeletonHtml(skeleton) {
  ...
  // `replaceClosedComments` is `.replace(/<!--[\s\S]*?-->/g, …)` without the quadratic scan
  // from every unclosed `<!--` (lib/core/closed-comments.js) — a skeleton is model output.
  const src = replaceClosedComments(stripMarkdownCode(String(skeleton || '')), blankRun)
...
module.exports = {
  ...
  findHexLiterals, findCssExfil, findCssImports, findMargins, findRawFontSize, findUnscopedSelectors, findSkeletonHtml,
  splitSelectorList,
