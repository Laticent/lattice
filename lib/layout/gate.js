 ...
 * the list — `:where([data-family="tall"], [data-family="strip"]) section.foo` is
 * ONE scoped selector, and a naive `split(',')` reads it as two, the first of
 * which looks unscoped. The family-reflow idiom (lib/adaptive/families.js) writes
 * exactly that shape, so the split has to see nesting.
 */
