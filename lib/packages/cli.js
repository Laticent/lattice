...
function cmdList(flags, log) {
  ...
  const rows = [
    ...
    ...listInstalled(root).map((p) => {
      ...
      const note = !p.ok ? `unreadable: ${p.errors[0]}` : refused ? `refused, not used: ${refused}` : hidden ? `hidden by the shipped ${p.type} of this name; re-add it to install as ${p.name}-custom` : '';
 ...
 */
async function main(argv, { log: rawLog = (s) => console.log(s), err: rawErr = (s) => console.error(s) } = {}) {
