...
const { remoteRefsInElements, mermaidRemoteRefs, frontMatterRemoteRefs, frontMatterOf, isRemoteUrl, galleryRefusal } = require('../core/remote-ref.js');
const { matchMermaidFences } = require('../core/mermaid-fences.js');
  ...
  for (const child of node.childNodes || []) {
    if (child.tagName) {
      yield {
        ...
        text: child.tagName === 'style' || child.tagName === 'script' || /mermaid/i.test(child.attrs.find((a) => a.name === 'class')?.value || '') ? textOf(child) : '',
      };
 ...
 * @returns {string[]}
 */
function galleryRemoteRefs(source) {
  // Line endings as the CLI reads a deck (lattice-emulator.js readFileOrDie): a CRLF fence
  // the fence walker would not match raw is still drawn once the deck is exported.
  const md = String(source || '').replace(/^\uFEFF/, '').replace(/\r\n?/g, '\n');
  if (!md.trim()) return [];
  ...
  // Every fence the CLI itself hands to Mermaid, read off the SOURCE, because one inside an
  // HTML block renders as plain text in the page yet is still drawn by the export.
  const fenced = matchMermaidFences(String(md)).flatMap((f) => mermaidRemoteRefs(f.body));
  // Read it twice: alone, as the gate always did, and AS INSERTED. Insert splices the gallery
  // after a slide and a `---`, so its front matter stops being front matter and renders.
  const inserted = engine.render(`# x\n\n---\n\n${md.trim()}\n`).html;
  const fm = frontMatterOf(md);
  return [
    ...new Set([
      ...remoteRefsInElements(elements(parse5.parse(html, { scriptingEnabled: false }))),
      ...remoteRefsInElements(elements(parse5.parse(inserted, { scriptingEnabled: false }))),
      ...defined,
      ...fenced,
      ...(fm == null ? [] : frontMatterRemoteRefs(fm)),
    ]),
  ];
}
