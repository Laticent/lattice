...
const RESERVED_CLASSES = require('./reserved-classes.generated.js');
...
function createRegistry(index = { packages: [] }) {
  ...
  // The `finish:` register's own words (the same five RESERVED_FINISH_NAMES carries in the
  // Studio's finish-library.ts): a finish package under one of them would shadow the register.
  const engine = new Set(['theme/lattice', ...['boardroom', 'sketch', 'sketch-clean', 'none', 'preview'].map((n) => `finish/${n}`)]);
  // The slide classes the engine owns (`finish`, `print`, `dark`, the modifiers and the
  // `tint-*`/`mark-*`… families): a component under one of those names would restyle every
  // slide the engine stamps with it. Generated by tools/build-packages-index.js.
  for (const n of RESERVED_CLASSES.names) engine.add(`component/${n}`);
  // A `-custom` name is the escape hatch itself (no engine class ends in it), so reserving is
  // idempotent: `finish` → `finish-custom`, which the `finish-*` family must not claim again.
  const reservedByPrefix = (type, name) => kindOf(type).type === 'component' && !name.endsWith(RESERVED_SUFFIX) && RESERVED_CLASSES.prefixes.some((p) => name === p || name.startsWith(p.endsWith('-') ? p : `${p}-`));
  ...
  const isReserved = (type, name) => shipped.has(key(type, name)) || engine.has(key(type, name)) || reservedByPrefix(type, name);
