...
function createRegistry(index = { packages: [] }) {
  ...
  const reservedNames = (type) => new Set([...shipped.values()].filter((p) => p.type === type).map((p) => p.name));
  const unreservedName = (type, name) => (shipped.has(key(type, name)) ? `${name}${RESERVED_SUFFIX}` : name);
