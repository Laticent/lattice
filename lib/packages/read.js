...
const { themeDirectiveName } = require('../theme/directive.js');
 ...
 */
/** A file a runtime would execute: JavaScript and TypeScript in every module flavor, and WebAssembly. */
...
function readPackage(files, { type, folder, strict = false } = {}) {
  ...
  let manifest;
  try {
    manifest = parseJsonCapped(asText(files[manifestFile]) ?? '', TOO_MANY);
  } catch (e) {
    errors.push(e.message === TOO_MANY ? `${manifestFile} is too large to read (${TOO_MANY})` : `${manifestFile} is not valid JSON: ${e.message}`);
    return fail();
  }
  ...
  if (typeof name !== 'string' || !/^[a-z0-9][a-z0-9-]*$/.test(name)) {
    errors.push(`${manifestFile}: "name" must be a lowercase slug (got ${JSON.stringify(name)})`);
    return fail();
  }
  ...
  if (!(kind.nameRe || NAME_RE).test(name)) {
    errors.push(`${manifestFile}: a ${resolved} name must start with a letter (got ${JSON.stringify(name)})`);
    return fail();
  }
  ...
  // CODE is any script in the folder, not only the named `transform.js` role: a
  // `x.transform.JS` (the same file on a case-insensitive disk), a `.mjs`, `.cjs` or `.ts`
  // would otherwise ride along as an "asset" and be exported again. Nothing runs them today;
  // a code package waits on consent and a sandbox (§3.5), and until then it is refused whole.
  const code = kind.code.some((r) => roles[r]) || Object.keys(files).some((f) => SCRIPT_RE.test(f));
  return {
    ok: true,
    errors,
    renames,
