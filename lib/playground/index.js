 ...
 *   splitForPreview(html, source, width, height)
...
function render(markdown, theme, opts) {
  ...
  // `flatCss` (the exported player's stylesheet) when the caller asked for it.
 ...
 */
function splitForPreview(html, source, width, height) {
  return structuralSplit(html, { deckSource: source, width, height, capacity: SPLIT_CAPACITY });
}
