const { BERTHS } = require('../core/fit-berth');
// Every berth, as a selector — derived, so a fourth register cannot be added to
// BERTHS and silently escape the `off` sweep.
const BERTH_SELECTOR = BERTHS.map((c) => `.${c}`).join(', ');
...
function sweepOverflowMarkers(root) {
  ...
  for (const t of scope.querySelectorAll(BERTH_SELECTOR)) {
...
/**
 * The LEGIBILITY tab's label (§8 rule 8). The ring is color-only, so the
 * condition is named in text (WCAG 1.4.1) — and named with a NUMBER, because
 * "too small" is only actionable next to a size.
 *
 * ── IT USED TO READ `Type 6.6px · floor 7.2px`, AND NOBODY COULD READ IT ────
 *
 * That label named a measurement and a threshold with no verb and no subject.
 * It asked the author to already know the rule before it would tell them
 * anything — which is backwards, because the author who needs it most is the one
 * meeting it for the first time. Reported from the Studio preview by the person
 * who wrote the rule ("what is it saying, it makes no sense even to me").
 *
 * Two things changed. The condition is now stated in PLAIN WORDS, matching the
 * clip register beside it — "Content clipped" is understood on sight, and this
 * tab had no equivalent. And the size is reported in POINTS rather than px:
 *
 *   · px moves with the preset. The same figure is 6.6px at `hd` and 19.8px at
 *     `4K`, so the number could not be carried between two decks, and neither
 *     could the floor it was printed beside.
 *   · pt (on the standard 960x540pt slide page — see `REF_PT` in
 *     lib/core/overflow-probe.js) is preset-invariant: the floor is 5.4pt on
 *     every deck, and the measured size means the same thing everywhere.
 *   · pt is the unit an author already thinks in for type, and near 16:9 it is
 *     the unit this engine documents its own roles in (lib/typography/scale.js:
 *     `meta` 11.25pt, `body` 16pt), so "5pt" places itself against those. That
 *     comparison is aspect-bound and the probe says why — `--fs-*` is relative to
 *     slide WIDTH and this is relative to HEIGHT, so on `standard` / `story` /
 *     `mobile` the deck's own `meta` sits 25-38% below its documented number in
 *     these units. See the REF_PT block in lib/core/overflow-probe.js.
 *
 * THE FLOOR IS NO LONGER ON THE TAB, and that is what buys the plain words
 * without making the tab wider: once the number is preset-invariant the floor is
 * the constant 5.4pt, so repeating it in the corner of every ringed slide costs
 * width and tells the author nothing that varies. It moves to the hint below,
 * where it is one hover away and sits next to the fix. `Text too small · 4.9pt`
 * is 22 characters against `Type 6.6px · floor 7.2px`'s 24 — rendered into the
 * live tab at hd, 269.1px against 290.7px. (An earlier draft of this line said
 * "20 against 24", true only of a single-digit integer pt and not of the example
 * sitting beside it.)
 *
...
function legibilityTabText(leg) {
  return 'Text too small · ' + leg.minPt + 'pt';
}

/**
 * The LEGIBILITY tab's HINT — the `title` the label does not have room for.
 *
 * Two sentences and no more: what was measured against what, then what to do
 * about it. The tab answers "what is wrong and how far off"; this answers "what
 * do I change", which is the half an author cannot derive from a number.
 *
 * "minimum", not "floor". `floor` is this codebase's word for the rule, not the
 * author's word for a limit, and a hint written in the vocabulary of the check it
 * came from is the failure this whole change is repairing.
 *
 * The fix names both directions because the probe cannot tell which applies: a
 * figure below the floor is either carrying too much or sitting in too small a
 * box, and only the author knows which. Same wording as the emulator's stderr
 * warning (lattice-emulator.js § TYPE FLOOR), so the two channels teach one fix.
 *
 * Injected into the export watcher verbatim alongside `legibilityTabText`, for
 * the same reason (HARD RULE #15).
 */
function legibilityTabHint(leg) {
  return 'Figure text renders at ' + leg.minPt + 'pt — below the ' + leg.floorPt
    + 'pt minimum. Simplify the figure (fewer labels, shorter text) or give it a bigger box.';
}
...
module.exports = {
  ...
  LEGIBILITY_TAB_HINT_SRC: legibilityTabHint.toString(),
};
