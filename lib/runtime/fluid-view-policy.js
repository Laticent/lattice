 ...
 *   reader → the same tab, restyled by CSS into a calm "More below ↓" pill, and no
 *            ring. No QA chrome; the type floor has no reader answer (a reader
 ...
 *
 * `off` is not "skip the watcher" — it is "leave no marker", and there are two
 * ways one can already be in the DOM when the watcher starts: lattice-emulator.js
 * stamps `.overflow` into exported HTML at BUILD time, and a re-render can hand
 * the watcher a document an earlier, louder pass already marked. Skipping without
 * sweeping would leave exactly the chrome `off` was asked to remove.
 *
...
function sweepOverflowMarkers(root) {
  ...
  for (const t of scope.querySelectorAll('.overflow-tab, .illegible-tab, .lattice-fixme-tab')) t.remove();
}
