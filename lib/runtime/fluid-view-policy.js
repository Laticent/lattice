...
function sweepOverflowMarkers(root) {
  ...
  // `.overflow-silent` was missing here while the docstring above promised the list was
  // "EXHAUSTIVE by contract" — so `off` cleared the ring and left the class that modifies
  // it stamped. Inert today (the class only gates a reader tab `off` removes anyway), and
  // fixed rather than documented-around because a stale-state carrier is a defect waiting
  // for its second reader, and a docstring that reads false about its own list is worse
  // than no docstring (#1300 item 3).
  for (const s of scope.querySelectorAll('section.overflow-silent')) s.classList.remove('overflow-silent');
  for (const s of scope.querySelectorAll('section.content-clipped')) s.classList.remove('content-clipped');
