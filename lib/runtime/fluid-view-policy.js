...
function sweepOverflowMarkers(root) {
  ...
  // `.content-cut` was missing here while the docstring above promised the list was
  // "EXHAUSTIVE by contract" — so `off` cleared the ring and left the class that gates the
  // reader tab stamped. Fixed rather than documented-around because a stale-state carrier
  // is a defect waiting for its second reader, and a docstring that reads false about its
  // own list is worse than no docstring (#1300 item 3).
  for (const s of scope.querySelectorAll('section.content-cut')) s.classList.remove('content-cut');
