...
(() => {
  ...
  function buildMermaidThemeVars() {
    ...
    // SENTINEL. `ensureConfigured` checks this same var before it calls us, but it
    // can also proceed WITHOUT a resolved palette on its `force` path (some preview
    // hosts never expose theme vars to JS). Answer that case with nothing rather
    // than colors read off an unstyled element: `vc()` resolves an undefined var to
    // a real `rgb(0, 0, 0)`, so the diagram would bake solid black. An empty set
    // lets Mermaid use its own defaults — which is exactly what that path's
    // "proceeding with Mermaid defaults" warning already claims happens.
  ...
  function ensureConfigured(mermaid, { force = false } = {}) {
    ...
    mermaid.initialize({
      ...
      // that names layout/curve/renderer keeps every colour it did not set — the
