...
(() => {
  ...
  function transformListTabularMarks() {
    ...
    for (const section of document.querySelectorAll('section.list-tabular')) {
      for (const li of section.querySelectorAll(':scope ol > li > ul > li, :scope ol > li > ol > li')) {
        if (li.classList.contains('marks')) continue;
        // The leading text node (an element before any text means no marker).
        const firstText = (() => {
          for (const node of li.childNodes) {
        ...
        if (!marker) {
          const parts = [...li.childNodes].map((n) => ({
        ...
        li.insertBefore(listTabularMarksKernel.stateNode(document, marker), li.firstChild);
      }
    }
  }
