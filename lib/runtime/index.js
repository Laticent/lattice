...
(() => {
  ...
  /**
   * The wait for everything else — the exact debounce this work removed.
   *
   * 150 is not tuned, it is inherited: it is what a costly diagram waited before any of this,
   * so a costly diagram cannot be made worse by any of this.
   */
  const DIAGRAM_DEBOUNCE_MS = 150;
  /**
  ...
  function diagramBackoffMs() {
    return diagramIsCheap() ? 0 : DIAGRAM_DEBOUNCE_MS;
  }
  //
  // ARRIVAL IS NOT SPECIAL-CASED, and that is a decision rather than an omission. Navigating
  // to a costly diagram waits this same 150ms — exactly what it waited before this work, so
  // it cannot have regressed. Two attempts to make arrival FASTER than that both cost more
  // than they bought: asking the DOM whether any fence lacked a drawing disabled the back-off
  // on every host that does not stamp `data-lattice-swap`, and again on any slide holding a
  // fence in `error`; clearing the cost record when the fence set turned over could not tell
  // an edit of the ONLY fence on a slide from an arrival, because editing it changes the one
  // source there is, so it cleared on every keystroke and drew eight times where it should
  // have drawn once. Both were the same error: inferring the author's intent from the
  // document. The cost record is the only thing here that is actually known.

  /**
   * NO CEILING, AND ITS REMOVAL IS THE POINT.
   *
   * There was one — 1200ms, so that steady typing into a large fence could not starve the
   * redraw the way an uncapped trailing debounce does. It was asked at the arm site rather
   * than inside the timer, which was itself a real fix: tested inside the callback it was
   * never reached, because every pass that re-arms cancels that callback first, and a
   * 64-node diagram sat frozen for 14166ms against a nominal 1200ms.
   *
   * It worked, and it cost more than it bought. Firing mid-burst means a ~376ms blocking
   * `mermaid.render` while the author is still typing, three times over a 24-character
   * burst — which is where the 30% main-thread figure and the 24% slower typing recorded
   * above both come from. There is no cheap version: the cost IS the render, the main thread
   * is serial, and `mermaid.render` cannot be moved off it (it needs `document` and throws
   * `ReferenceError` in a worker — measured).
   *
   * Starving the redraw mid-burst is what the old build does, so matching it is parity
   * rather than a regression, and the diagram is redrawn the moment the author pauses. The
   * ceiling can come back the day a render stops blocking the main thread.
   */
  /** Trailing-edge timer; re-armed by every pass while the source keeps moving. */
  let diagramBackoffHandle = null;

  /** Draw now, or hold for the source to settle? */
  function shouldDispatchDiagrams() {
    return diagramBackoffMs() <= 0;
  }

  /**
   * Hold this pass and come back when the source has settled.
   *
   * Re-arming restarts the wait, which is what a trailing debounce MEANS — and with the
   * ceiling gone that is the whole of it: a burst that keeps arriving keeps pushing the
   * render out, exactly as the build before this one did.
   */
  function armDiagramBackoff() {
    if (diagramBackoffHandle) clearTimeout(diagramBackoffHandle);
    diagramBackoffHandle = setTimeout(() => {
      diagramBackoffHandle = null;
      // `backoffElapsed` is what stops the re-entered pass taking this branch again and
      // arming another timer forever. Dropping it is a total wedge: the diagram never
      // redraws, and it is invisible to any test that does not drive this callback.
      initAndRun({ backoffElapsed: true });
    }, diagramBackoffMs());
  }

  /**
   * Should this pass proceed to draw, or be held?
   *
   * The whole gate, not just its parts, because when the two lines that USE the policy sat
   * outside these sentinels a checker found mutations that survived the suite — including
   * discarding this function's return value entirely, which turns the feature off.
   */
  function admitDiagramPass(backoffElapsed) {
    if (!backoffElapsed && !shouldDispatchDiagrams()) {
      armDiagramBackoff();
      return false;
    }
    clearDiagramBackoff();
  ...
  /** Called once a pass gets through, so no stale timer re-enters behind it. */
  function clearDiagramBackoff() {
    if (diagramBackoffHandle) {
      clearTimeout(diagramBackoffHandle);
      diagramBackoffHandle = null;
    }
  }
  // ── END BACK-OFF PORT
...
/**
  ...
  function initAndRun({ force = false, backoffElapsed = false } = {}) {
    // The transforms run FIRST and unconditionally — they are what keeps the rest of the
    // slide alive while a diagram is drawing, and they are idempotent.
    ...
    // THE BACK-OFF, and it sits here rather than in `scheduleRun` for the reason written
    // over that function: this is the diagram dispatch, and everything above it — the Form
    // composition, the masthead, the charts, the fit berth — has already run and must keep
    // running per keystroke whatever the diagrams are doing.
    //
    // Re-armed, so it is a trailing edge: while the author keeps typing the timer keeps
    // moving and no render starts. It costs nothing until a render has proved expensive,
    // because `diagramBackoffMs()` is 0 until then.
    // NOT keyed on `force`. That flag means "the theme sentinel may be bypassed" and is
    // threaded into `themeSettled` below; a forced pass still owes the author an unblocked
    // editor, so it takes the same back-off as any other.
    if (!admitDiagramPass(backoffElapsed)) return RUN_DEFERRED;
    const mermaid = globalScope.mermaid;
  ...
  function renderDiagramJob(mermaid, scopeKey, job) {
    ...
    if (!diagramIsCheap()) return renderDiagramNow(mermaid, scopeKey, job, false);
    ...
    return new Promise((resolve) => {
      ...
      Promise.resolve()
        ...
        .then((result) => {
          ...
          // A LOCAL, not a module-level variable. An earlier revision kept the last cost in
          // one, then the policy moved to a median over the last three samples and the
          // declaration went with the block that was replaced — leaving a bare assignment
          // that creates an implicit global in sloppy mode and throws `ReferenceError` in a
          // strict or module context, which would lose every diagram on the spot. Nothing
          // needs the value beyond this line: `recordRenderCost` owns the samples.
