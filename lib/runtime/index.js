...
(() => {
  ...
  function configureForScope(mermaid, themeVars) {
    if (mermaidConfiguredVars === themeVars) return;
  ...
  function initAndRun({ force = false } = {}) {
    ...
    try {
      renderDiagrams(deck, {
        ...diagramThemePorts(),
        beginRun: ({ themeVars }) => beginDiagramRun(mermaid, themeVars),
  ...
  function beginDiagramRun(mermaid, themeVars) {
    const jobs = [];
    const fences = [];
    currentRunJobs = { jobs, fences };
    diagramQueue = diagramQueue
      .then(() => {
        configureForScope(mermaid, themeVars);
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        // been right first. `author` short-circuits before it, so that register stays
        // purely geometric and exactly as cheap as it was.
        const tell = (over && (policy.authorTags || squeezed > TOL))
          || ((over || clipSuspect) && probeContentClipped(s, IGNORED_CLIP_SELECTOR, TOL).cut);
        if (s.classList.contains('overflow-silent') !== (over && !tell)) {
          s.classList.toggle('overflow-silent', over && !tell);
        }
        // …and its MIRROR. `tell` can now be true while `over` is false — an ellipsed
        // label, a line-clamped card, a sheared panel head all cut content without
        // growing the section's own extent. `.overflow` stays geometric (autosplit and
        // the console report key off it), so the pill's VISIBILITY needs its own class:
        // base.modifiers.css hides any tab under `section:not(.overflow)`, which
        // silently swallowed exactly this new population. Idempotent, like the toggles
        // around it, so the attribute observer does not churn.
        // `policy.mark` is false only at `off`, which promises to leave nothing — and the
        // sweep that clears this class runs once at boot while the watcher re-stamps on
        // every settle, so an unguarded toggle loses that race.
        const clipOnly = policy.mark && tell && !over;
        if (s.classList.contains('content-clipped') !== clipOnly) {
          s.classList.toggle('content-clipped', clipOnly);
        }
