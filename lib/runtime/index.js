...
const { planFitSweep } = require('../../lib/core/fit-sweep');
...
(() => {
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    const sweep = () => {
      ...
      for (const m of plan.measure) fitState.set(m.section, { gen: fitGeneration, w: m.w, h: m.h });
      check(plan.measure.map((m) => m.section));
      return plan;
    };
    ...
    const scheduleSweep = ({ fresh = true } = {}) => {
      ...
      sweepHandle = setTimeout(() => { sweepHandle = null; sweep(); }, DEBOUNCE_MS);
    };
    ...
    // CONTENT. The same observer that already drives the debounced content
    // transforms (`scheduleRun`, 150ms) — childList and subtree only, NOT
    // attributes, which is precisely why this one cannot see its own writes and
    // the old one could. Registered as a listener rather than a second observer
    // so an edit costs one settle window, not two.
    ...
    // The host's own hook. The Studio and the Playground patch slide DOM
    // directly (docs/src/playground/deck-preview.js, docs/src/lib/single-slide-
    // render.ts) and know exactly when a render has landed — better than any
    // observer can infer. Exposed so they can say so, rather than relying on the
    // 150ms fallback. `sweep` is the synchronous form for a caller that needs
    // the verdict now (a test, an export handoff).
    globalScope.latticeFit = {
      sweep: () => { invalidate(); return sweep(); },
