...
(() => {
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        if (over && !tab) {
          ...
          // calm, honest "content continues" cue instead of a QA banner. Both
          // are text (WCAG 1.4.1 — never color alone). Styling: the loud red
          // is base.modifiers.css; the reader restyle is base.fluid-view.css.
          tab.textContent = authorTags ? 'Overflows' : 'More below';
          s.appendChild(tab);
        } else if (!over && tab) {
          tab.remove();
        }
  ...
  // The default view fills within a tolerance band around the authored aspect.
  // Ultrawide (aspect > this) has no edge cap yet — that is the P2 slice — so it
  // keeps the authored fixed deck rather than filling into a dead band. This is a
  // provisional numeric bound; P2 replaces it with a families.js upper-wide
  // boundary + the cap. engineering/decisions/2026-07-20-adaptive-viewport-fill.md.
  ...
  function initFluidView() {
    ...
    if (wants("fixed")) apply(false);
      ...
      // Default fill within the band: portrait / square / standard / 16:10 / 16:9
      // all fill; ultrawide keeps the authored fixed deck until the P2 cap. (Was
      // phone-only — `innerHeight > innerWidth`; now any non-ultrawide screen.)
      const aspect = window.innerWidth / Math.max(1, window.innerHeight);
      apply(aspect <= FILL_DEFAULT_MAX_ASPECT);
    }
  }
