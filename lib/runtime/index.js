...
(() => {
   ...
   * measured 210ms once — and a policy reading only the latest number then throttled the
   * whole burst that followed.
   *
   * The median takes the LOWER of two, deliberately. `Math.floor(len/2)` picks the upper,
   * which kept a cold 210ms sample answering for a 36ms diagram across two renders — so the
   * first two edits of a session, and the first two after arriving from a big diagram, paid
   * a back-off the diagram did not deserve.
   */
  const renderCosts = [];
   ...
   *           fence stretched 1086ms -> 1130ms rendering on every keystroke — 44ms across
   *           eight, invisible — while the same burst on a ~70ms fence stretched to 1493ms.
   *           Below the floor the diagram redraws live as you type, which is the whole feel
   *           of editing a small diagram, and it costs nothing.
   *
   *   COST    Twice the median render, so the share of the author's time spent re-rendering
   *           is bounded by construction: `c` out of every `c + 2c` is a third.
   *
   *   CAP     150ms, and it is not a round number — it is exactly the fixed debounce this
   *           work removed. A fence already showing a drawing always waits, so the wait is
   *           what the author feels after their LAST keystroke, and any value above 150
   *           makes that worse than the build we replaced. THIS CAP MUST BIND
   *           UNCONDITIONALLY. An earlier revision added a second term for the author's
   *           typing cadence and applied it with `Math.max` OUTSIDE the cap, which made the
   *           sentence above false as shipped: an observed gap over ~120ms put the wait past
   *           150, and the post-keystroke redraw went behind `main` in every cell measured
   *           (610ms against 281, 696 against 522, 899 against 525, 1111 against 903).
   *           That term is gone. It was reasoning about the right thing — a wait shorter
   *           than the typing gap fires between keystrokes — but it was estimated from
   *           mutation timing the runtime cannot attribute (it counts its own DOM writes as
   *           the author's), and it bought slower redraws plus the stall below.
   */
  function diagramBackoffMs() {
    const cost = medianRenderCostMs();
    if (cost <= RENDER_COST_FLOOR_MS) return 0;
    return Math.min(cost * 2, RENDER_COST_CAP_MS);
  }

  /**
   * THE CEILING, and it is asked HERE rather than inside the timer — which is the whole bug.
   *
   * A trailing debounce with no ceiling is only as reliable as the quietest moment in the
   * document, and someone typing steadily into a large fence never provides one. The ceiling
   * existed for that, and it did not work: it was tested only inside the timer callback,
   * while every pass that re-arms cancels that callback before it can run. The runtime's own
   * transform writes echo back through the observer about four times per keystroke, so the
   * timer was cancelled and reset faster than it could fire and the ceiling was never
   * reached. Measured on the built Studio: a 64-node diagram sat frozen for 14166ms against
   * a nominal 1200ms ceiling, scaling with how long the author typed, where the build before
   * this work redrew on every keystroke.
   *
   * Asked at the ARM SITE, every pass tests it, so no amount of re-arming can outrun it.
   */
  ...
  function backoffExpired() {
    return backoffStartedAt !== 0 && nowMs() - backoffStartedAt >= DIAGRAM_MAX_WAIT_MS;
  }

  /**
   * DOES THIS HOST HAND THE PREVIOUS DRAWING FORWARD? Set the first time a host stamps
   * `data-lattice-swap`.
   *
   * The leading edge below asks whether a fence already has a drawing on screen, and treats
   * "no" as an arrival worth rendering at once. On a host that stamps — the Studio and the
   * Playground — that is exactly right, because adoption transplants the outgoing SVG into
   * the arriving fence, so a slot is empty only when there genuinely is nothing to look at.
   *
   * ON A HOST THAT DOES NOT STAMP, adoption returns early and the slot is ALWAYS empty, so
   * the question answers "arrival" on every keystroke and the back-off never engages. That
   * is not hypothetical: driven against the DOM contract of a non-stamping host, an
   * 8-character burst on a 64-node fence cost 8 renders and 2603ms against 1 render and
   * 357ms on the build this replaces, and stretched the typing itself from 1006ms to
   * 3333ms. The same page WITH the stamp cost 1 render — the attribute was the only
   * difference. Those are the hosts the runtime ships to beyond this repo: marp-vscode,
   * third-party embedders, the `marp --html` bundle.
   *
   * So the leading edge is consulted only where it can be answered. Where it cannot, the
   * cost back-off applies unconditionally — which is what the build before this work did
   * for every host, so those consumers are no worse off. First paint stays immediate there
   * regardless, because `diagramBackoffMs()` is 0 until a render has been timed.
   */
  let hostStampsSwaps = false;

  /**
   * IS ANY FENCE ABOUT TO DRAW ONE THE READER CANNOT SEE YET?
   *
   * Asking about INK rather than about a CLOCK is what makes the leading edge right: an
   * empty slot means the reader has nothing, a slot holding an SVG means they have something
   * valid. The sibling is checked for BEING the slot, not merely for containing an `<svg>` —
   * `adoptWithinNode` guards the same question that way, and two derivations of one question
   * disagreeing is how a false answer here silently restores the regression this exists to
   * fix (a false "no ink" dispatches every keystroke immediately).
   */
  function anyFenceWithoutInk() {
    ...
    for (const preEl of document.querySelectorAll(PENDING_FENCE_SELECTOR)) {
      ...
      if (!slot?.classList?.contains('mermaid')) return true;
  ...
  /**
   * WHAT SHOULD THIS PASS DO ABOUT DIAGRAMS — go now, or wait for the source to settle?
   *
   * Three ways to go now, and each is a different reason: the diagram is cheap enough that
   * waiting buys nothing; somebody is looking at an empty slot; or we have held the render
   * as long as we are allowed to. Anything else waits.
   *
   * It lives here, beside the numbers it reads and inside the lifted block, because when it
   * lived inline in `initAndRun` a checker mutated it eight ways — including inverting the
   * leading edge, which silently restores the very regression it exists to fix — and all
   * 9179 tests stayed green.
   */
  function shouldDispatchDiagrams() {
    if (diagramBackoffMs() <= 0) return true;
    if (hostStampsSwaps && anyFenceWithoutInk()) return true;
   ...
   *
   * Re-arming restarts the wait, which is what a trailing debounce MEANS; `backoffStartedAt`
   * deliberately survives the re-arm, because it is what `backoffExpired` measures and
   * resetting it per pass is how the ceiling was outrun in the first place.
   */
  function armDiagramBackoff() {
    ...
    diagramBackoffHandle = setTimeout(() => {
      ...
      // redraws, and it is invisible to every test that does not drive this callback.
   ...
   *
   * The whole gate, not just its parts. An earlier round moved the decision, the arm and the
   * ceiling inside these sentinels and left the two lines that USE them outside — and a
   * checker then found eight mutations that survived all 9211 tests, including dropping the
   * `backoffElapsed` argument here (which stops the held pass ever being re-admitted) and
   * deleting the `clearDiagramBackoff()` below (which leaves the ceiling measuring a stretch
   * that already ended). Both are one line, both are invisible to any arm that can only
   * reach the functions they call.
   */
  ...
  /** Called once a pass gets through, so the ceiling measures one held stretch. */
...
/**
  ...
  function adoptOutgoingDiagrams(records) {
    ...
    // REMEMBER THAT THIS HOST SPEAKS. The back-off's leading edge asks whether a fence has
    // a drawing on screen, and that question is only MEANINGFUL where a host hands the
    // previous drawing forward. See `hostStampsSwaps`.
    if (kind) hostStampsSwaps = true;
