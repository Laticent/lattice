...
(() => {
  ...
  // Unlike a grow-to-fit grid card — which grows and pushes a NEIGHBOUR past
  // the frame, so pinpointing "the biggest box" flags the wrong element (see
  // startOverflowWatcher below) — a clip cell (overflow:clip) that overflows
  // genuinely clipped its OWN content. It never pushed anything. That makes
  // it a safe, geometrically-certain "cause" signal, unlike the section-level
  // ring. See engineering/decisions/2026-07-10-overflow-cause-highlighting.md
  // (§3 Case A). §3 Case B — the grow-to-fit fallback, keyed off the
  // prose-density word budget — is deliberately NOT built here (follow-up).
  //
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        if (over && overCells?.length) {
          ...
          for (const oc of overCells) {
            ...
            if (cellEl) fixMeTargets.push(cellEl);
          }
        }
      }
