...
(() => {
  ...
  function patchSectionGeometry() {
    ...
    const patch = (s) => {
      ...
      // path is unchanged).
      const vh = (s.offsetHeight / 100).toFixed(3) + 'px';
      if (s.offsetHeight && s.style.getPropertyValue('--_sec-1cqh') !== vh) {
        s.style.setProperty('--_sec-1cqh', vh);
      }
    };
  ...
  function startOverflowWatcher({ authorTags = true } = {}) {
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        // treatment — `overflowTabText(authorTags)` turns "Overflows" into a calm "More
        ...
        const leg = authorTags ? probeFigureLegibility(s, FIGURE_TEXT_FLOOR_RATIO) : null;
