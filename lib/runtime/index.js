...
(() => {
  ...
  function buildMermaidThemeVars() {
    ...
    // The containment box's own edge — see the clusterBorder note in the
    // emulator's MERMAID_VAR_MAP for why this is not --diagram-stroke.
    const containEdge = vc('c-container-edge');
  ...
  //
  // It is ~6px tall and permanently empty in a static render (nothing ever hovers
  // a slide in a PDF), but it is real layout AFTER the last slide. In print that
  // makes document height exceed the deck's own height by those few pixels, and
  // Chrome spills onto ONE MORE SHEET: every export of a deck containing a
  // flowchart ended on a blank page. Measured on dist/marp-kit's sample deck —
  // 13 sections, body 7800px, document 7806px, 14 pages.
  //
  // This CANNOT be fixed from theme CSS, which is the obvious first instinct and
  // is why the bug survived: Marpit SCOPES theme rules to the deck root, so an
  // unscoped `.mermaidTooltip{display:none}` in lattice.css is rewritten to a
  // selector that can never match a body-level node. The runtime is the only
  // surface here that reaches it. Idempotent — safe to call after every render.
  function dropMermaidTooltip() {
    if (typeof document === "undefined") return;
    for (const el of document.querySelectorAll("body > .mermaidTooltip")) el.remove();
  }
