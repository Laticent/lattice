...
// The flowchart's browser pass and the shared router it lays out with. Same delivery as
// the state chart above: dagre arrives as `globalThis.__latticeDagre`, from the sibling
// script, never from this bundle; without it the harness tiles stay up.
const { installFlowchartLayout } = require('../../lib/components/chart/flowchart/flowchart.layout');
const { graphLayoutKernel } = require('../../lib/components/chart/_chart-family/graph-layout');
 ...
 * (single-slide-render.ts reads the sanitized HTML for `data-sc-transitions` /
 ...
 * closes that the way `ensureFunctionPlot` does for plots: the sibling file, one filename
...
function ensureDagre(onReady) {
  if (globalThis.__latticeDagre || dagreLoad) return;
  try {
    if (!document.querySelector('.flowchart-figure[data-fc-model], .state-chart-figure[data-sc-transitions]')) return;
  } catch (_e) { return; }
  ...
  const attempt = (i) => {
    ...
    tag.src = urls[i];
    tag.onload = () => {
      dagreLoad = globalThis.__latticeDagre ? 'loaded' : 'failed';
      if (dagreLoad === 'loaded' && typeof onReady === 'function') { try { onReady(); } catch (_e) { /* the next pass redraws */ } }
      if (dagreLoad === 'failed') warnMissingDagre();
    };
    tag.onerror = () => {
      tag.remove();
...
function warnMissingDagre() {
  ...
  try {
    ...
    if (!document.querySelector('.state-chart-figure[data-sc-transitions], .flowchart-figure[data-fc-model]')) return;
    dagreWarned = true;
    console.warn(
      '[lattice] a state chart or flowchart is present but lattice-dagre.min.js did not load — a machine '
      + 'that branches will fall back to the chain grid, drawing each branch as a skip, and a flowchart shows its '
      + 'shapes as tiles with no lines. Load it with a <script src> before lattice-runtime.min.js.');
...
const { fitFunctionPlotSvg } = require('../../lib/core/function-plot-viewbox');
const {
...
// STEP — the projection font scale's Fit-Ladder move (lib/core/scale-fit.js). The same
// kernel the export injects, so the live preview and the PDF step the same slides.
const { fitScaleStep, levelScaleSteps } = require('../../lib/core/scale-fit');
const {
  stateClassesFor, LEADING_MARKER_RE, LEADING_MARKER_PREFIX_RE,
} = require('../../lib/core/state-marks.js');
const {
...
(() => {
  ...
  function drawFreshStateCharts() {
    try {
      ...
      // tilt, a degenerate canvas) never gains `data-sc-svg`, and without the mark
      // every later burst would redraw the whole deck in this microtask. The
      // debounced pass still owns every retry.
      const fresh = document.querySelectorAll('.state-chart-figure[data-sc-transitions]:not([data-sc-svg])');
      let drawable = false;
      for (const f of fresh) {
        if (f.offsetWidth > 0 && !f.__scMicrotaskTried) { f.__scMicrotaskTried = true; drawable = true; }
      }
      if (drawable) stateChartLayout.installStateChartLayout(document, { onlyFresh: true });
   ...
   * finally turns up.
   *
   * EXCEPT WHEN AN EXPORT FINALIZED IT, which is what `[data-mermaid-final]` marks. An export
  ...
  const RELEASED_FENCE_SELECTOR =
    'pre[data-mermaid-state="unavailable"]:not([data-mermaid-final]),marp-pre[data-mermaid-state="unavailable"]:not([data-mermaid-final])';
  ...
  function runAllContentTransforms() {
    ...
    if (dagreLoad !== 'loading') warnMissingDagre();
   ...
   * grammar and its meaning live in lib/core/state-marks.js (LEADING_MARKER_RE,
   * stateClassesFor), the same kernel the engine imports, so the two paths cannot
   * disagree about which characters are markers or what each one means.
   */
...
function transformProseCode() {
  ...
  function transformChecklistItemStates() {
    ...
    for (const section of document.querySelectorAll('section.checklist')) {
      for (const li of section.querySelectorAll(':scope > ul > li, :scope > ol > li')) {
        ...
        const m = LEADING_MARKER_PREFIX_RE.exec(firstText.nodeValue);
        if (!m) continue;
        const { sem, shape } = stateClassesFor(m[1]);
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    // The slides LEVEL moved outside the batch `check` measured. `sweep` forgets their fit
    // record and schedules a pass WITHOUT a new generation, so only they are probed again
    // (their ring stays true meanwhile: LEVEL only moves a slide to a rung it fits, but a
    // `guards: strict` trim was planned at the old rung).
    let scaleLevelMovedOutside = [];
    const check = (sections) => {
      ...
      const measured = [];
      // STEP, then LEVEL, before any probe below — so every channel measures each slide
      // at the size it actually renders, and every slide that asked for one scale shares
      // one rung (lib/core/scale-fit.js rule 7). STEP measures only this batch; LEVEL
      // reads what STEP recorded across the WHOLE deck and never measures, so a slide
      // outside the batch can move rung here. That is safe for its ring (it only ever
      // lands on a rung it fits), and `sweep` re-sweeps when it happens. A trim from
      // the PREVIOUS pass is cleared first, or the step would measure a slide the old
      // clamp had already shortened and keep a rung it cannot hold.
      // A slide whose STEP threw is left UNMEASURED this pass (skipped below), so the
      // next sweep retries it rather than recording a half-stepped slide as current.
      const stepFailed = new Set();
      for (const s of sections) {
        try {
          if (guardsEnabled(s.className)) clearTrim(s);
          fitScaleStep(s, { probeSectionOverflow, probeContentClipped }, {
      ...
      try {
        const inBatch = new Set(sections);
        scaleLevelMovedOutside = levelScaleSteps([...document.querySelectorAll('section[data-lattice-slide]')])
          .changed.filter((s) => !inBatch.has(s));
      } catch (err) {
        reportProbeFailure(err);
      }
      for (const s of sections) {
        if (stepFailed.has(s)) continue;
        try {
        ...
        // STEP and LEVEL already ran above, before TRIM — STEP loses no content, TRIM
        // cuts text (lib/core/scale-fit.js rules 1, 4, 6).
    ...
    const sweep = ({ bandRatio } = {}) => {
      ...
      // LEVEL moved a slide this sweep did not measure (an edit changed the rung the
      // deck shares). Re-probe just those slides; it settles in one more pass, because
      // the fits LEVEL re-reads are the ones that produced this rung.
      if (scaleLevelMovedOutside.length) {
        for (const s of scaleLevelMovedOutside) fitState.delete(s);
        scaleLevelMovedOutside = [];
        scheduleSweep({ fresh: false });
      }
      return plan;
    };
  ...
  // surface (the `--fluid` viewer, the Studio/Playground preview) this function is
  ...
  //
  // LOADED ON DEMAND, BESIDE THIS RUNTIME, when the host did not load it. The CLI emulator
  // injects function-plot.js into its page, so `window.functionPlot` is already there. The
  // docs-site hosts (the Studio preview and its capture frame, which bakes the Read pane and
  // the webpage export, the Playground, the landing previews) load only this runtime, and
  // until 2026-09-24 not one of them drew a curve: every ```functionplot fence was an empty
  // stage. `sync-playground-assets` stages the same function-plot.js the CLI uses (the version
  // package-lock.json locks) as a SIBLING of
  // lattice-runtime.js, so the URL is this script's own, one filename over, and no host has
  // a URL to thread. A deck without a plot never fetches it.
  //
  // Only after parsing has finished. Before then an emulator page's own function-plot tag
  // may simply not have run yet, and loading a second copy would race it.
  //
  // A LOAD THAT FAILS HANDS THE AUTHOR THEIR CONFIG, never a blank, for the reason
  // `releaseUnrenderableFences` does it for Mermaid (#2092): the Studio capture bakes whatever
  // is there when it stops waiting, and `data-fp-state="unavailable"` is what tells it the
  // plot is settled. A runtime with no `<script src>` (inlined, as `--fluid` does) has no
  // sibling to ask for, so it keeps the old behavior and does nothing.
  let functionPlotLoad = '';   // '' | 'loading' | 'loaded' | 'failed'
  function releaseFunctionPlots() {
    for (const div of document.querySelectorAll('div.functionplot[data-fp-config]:not([data-fp-inflated])')) {
      if (div.dataset.fpState === 'unavailable') continue;
      div.dataset.fpState = 'unavailable';
      try { div.textContent = fromBase64(div.getAttribute('data-fp-config')); } catch (_e) { /* keep it empty */ }
    }
  }
  function ensureFunctionPlot() {
    if (functionPlotLoad || document.readyState === 'loading') return;
    const src = OWN_SCRIPT?.src;
    if (!src) return;
    let url = '';
    try { url = new URL('function-plot.js', src).href; } catch (_e) { return; }
    functionPlotLoad = 'loading';
    const tag = document.createElement('script');
    tag.src = url;
    tag.onload = () => {
      functionPlotLoad = typeof window.functionPlot === 'function' ? 'loaded' : 'failed';
      if (functionPlotLoad === 'loaded') inflateFunctionPlots(); else releaseFunctionPlots();
    };
    tag.onerror = () => { functionPlotLoad = 'failed'; releaseFunctionPlots(); };
    (document.head || document.documentElement).appendChild(tag);
  }
  function inflateFunctionPlots() {
    if (typeof window === 'undefined' || typeof document === 'undefined') return;
    const divs = document.querySelectorAll('div.functionplot[data-fp-config]:not([data-fp-inflated])');
    if (!divs.length) return;
    if (typeof window.functionPlot !== 'function') {
      if (functionPlotLoad === 'failed') releaseFunctionPlots(); else ensureFunctionPlot();
      return;
    }
    divs.forEach((div) => {
      if (div.dataset.fpInflated === '1') return;
      // A plot released while the library was missing, which has since arrived — unless an
      // export capture released it (`data-fp-final`), which is terminal, as `data-mermaid-final` is.
      if (div.hasAttribute('data-fp-final')) return;
      if (div.dataset.fpState === 'unavailable') { div.textContent = ''; delete div.dataset.fpState; }
      try {
        // UTF-8, not a bare atob: the config was packed as UTF-8 bytes, and atob hands each
        // byte back as its own character, so an axis label `x²` drew as `XÂ²`
        // (lib/core/base64-utf8.js).
        const cfg = JSON.parse(fromBase64(div.getAttribute('data-fp-config')));
        ...
        // Settled, so the selector above skips it. Without the mark, the text write is a
        // mutation, the observer re-runs this pass, and the pass writes the text again,
        // once per frame for as long as the page is open.
        div.dataset.fpInflated = 'error';
      }
    });
  }
