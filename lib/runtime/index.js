...
(() => {
  ...
  function configureForScope(mermaid, key, themeVars) {
  ...
  function initAndRun({ force = false } = {}) {
    ...
    for (const preEl of document.querySelectorAll(FENCE_SELECTOR)) {
      ...
      // `querySelectorAll` returns document order, so consecutive fences on one
      // section are one deck entry — which is exactly the grouping the kernel's runs
      // are built on.
      if (!deck.length || sectionEl !== currentSection) {
        currentSection = sectionEl;
        deck.push({ scope: sectionEl, diagrams: [] });
      }
      deck[deck.length - 1].diagrams.push({ preEl, target, source });
    }
    ...
    try {
      renderDiagrams(deck, {
        ...diagramThemePorts(),
        beginRun: ({ scope, scopeKey, themeVars }) => beginDiagramRun(mermaid, scopeKey, themeVars, scope),
        renderOne: (job, _themeVars, meta) => enqueueDiagramJob(mermaid, meta.scopeKey, job),
      });
    } finally {
  ...
  function beginDiagramRun(mermaid, scopeKey, themeVars, _sectionEl) {
    const jobs = [];
    currentRunJobs = jobs;
    diagramQueue = diagramQueue
      .then(() => {
        configureForScope(mermaid, scopeKey, themeVars);
        return Promise.all(jobs.map((run) => run()));
      })
      // A run must not be able to poison the chain for later runs: every job already
      // handles its own failure, and `configureForScope` throwing (a stub mermaid
      // slipping past the guard) would otherwise leave every later band permanently
      // unrendered.
      .catch(() => {})
  ...
  function enqueueDiagramJob(mermaid, scopeKey, job) {
    // Defensive: a `renderOne` with no run open would render against whatever config
    // happened to be live. The kernel always calls `beginRun` first, so this is a
    // guard against a future edit, not a live path.
    if (!currentRunJobs) beginDiagramRun(mermaid, scopeKey, {}, null);
    currentRunJobs.push(() => renderDiagramJob(mermaid, scopeKey, job));
  }
