...
(() => {
  ...
  function diagramBackoffMs() {
    return medianRenderCostMs() <= CHEAP_RENDER_MS ? 0 : DIAGRAM_DEBOUNCE_MS;
  }
  //
  ...
  /**
   * THE CEILING, asked HERE rather than inside the timer — which is the whole bug it fixes.
   *
   * A trailing debounce with no ceiling is only as reliable as the quietest moment in the
   * document, and someone typing steadily into a large fence never provides one. Tested
   * inside the timer callback it was never reached, because every pass that re-arms cancels
   * that callback first: a 64-node diagram sat frozen for 14166ms against a nominal 1200ms.
   * Asked at the arm site, every pass tests it, so no amount of re-arming outruns it.
   *
   * This is the one place the new behavior is better than the old rather than equal to it:
   * the debounce it restores had no ceiling, so steady typing starved the redraw entirely.
   */
...
/**
  ...
  function attachErrorSafely(preEl, target, err) {
    try {
      ...
      const errKey = fenceSourceOf(preEl);
  ...
  function sweepDeferredFences() {
    ...
    for (const preEl of waiting) {
      ...
      if (waited >= ERROR_QUIET_MS) {
        forceRender.add(fenceSourceOf(preEl));
  ...
  function renderDiagramJob(mermaid, scopeKey, job) {
    ...
    if (forceRender.has(fenceSourceOf(job.preEl))) return renderDiagramNow(mermaid, scopeKey, job);
