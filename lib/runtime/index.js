...
(() => {
  ...
  function applyCachedDeckClass() {
    ...
    // The suppression set is the deck's mode tokens from EITHER spelling. `modeTokens`
    // holds what `mode:` stamped; a legacy deck-wide `class: sketch` puts the same token
    // in `classTokens`, and leaving it out meant a per-slide `_class: boardroom` opted the
    ...
    const modeSet = new Set([...modeTokens, ...classTokens.filter((t) => MODE_TOKENS.includes(t))]);
