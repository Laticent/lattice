...
(() => {
  ...
  /**
   * WHAT A RENDER OF THIS DIAGRAM LAST COST, and why the coalescing above needs it.
   *
   * Coalescing on completion assumes a keystroke can ARRIVE while a render is in flight —
   * that is the only state there is anything to coalesce. Measured on the built Studio, it
   * cannot: `mermaid.render` occupies the main thread, so the author's next keystroke is
   * blocked behind it and lands only once the render is done, finding no run in flight. The
   * queue is therefore one render per keystroke at every typing speed. Driving a 64-node
   * fence with an 8-character burst: 8 renders and 1899ms of render on this branch against
   * 1 render and 252ms on the version before it, and the burst itself stretched from 1101ms
   * to 2726ms because the keystrokes were waiting on the renders. That is the jank the
   * change set out to remove, moved from the diagram into the editor.
   *
   * So the back-off is sized by the diagram instead of by a constant — which is the same
   * argument that retired `DEBOUNCE_MS`, applied to a number we can now MEASURE rather than
   * guess: after each render this holds what it cost, and the next dispatch waits that long
   * for the author to stop typing. A cheap diagram keeps rendering per keystroke, because at
   * ~20-36ms that is invisible and it is what makes an edit feel live; an expensive one backs
   * off to roughly one render per render, which is what completion-coalescing was supposed to
   * deliver and could not.
   *
   * `RENDER_COST_FLOOR_MS` is the cost below which no back-off is worth having, and the two
   * measurements that bracket it are the reason it is 50 rather than a round number: an
   * 8-character burst on a fence costing ~30ms per render stretched from 1086ms to 1130ms
   * with a render on every keystroke — 44ms across eight, which nobody can feel — while the
   * same burst on a fence costing ~70ms stretched to 1493ms. Below the floor the live
   * per-keystroke redraw is free and worth having; above it, it is not. 50ms is also about
   * half a typing gap, so a render under it cannot swallow a whole keystroke.
   *
   * The cost is ONE NUMBER for the document, not one per fence, because the dispatch point
   * runs before the walk knows which fences it is about to draw. The cross-talk is real and
   * bounded: editing a cheap diagram just after a costly one waits the costly one's back-off
   * ONCE, then the next render overwrites the figure and the wait goes away. `RENDER_COST_CAP_MS` bounds
   * what this can ADD to the wait after the last keystroke, and 150 is not a round number:
   * it is exactly the fixed debounce this work removed. Since a fence that already holds a
   * drawing now ALWAYS waits (see `anyFenceWithoutInk`), a wait above 150 would make a
   * single keystroke on a large diagram slower than the build we replaced — 150 + the render
   * either way — and being slower than what came before is the one outcome none of this is
   * allowed to have. Above ~75ms of render the cap binds before the doubling does, so the
   * one-third share is a bound on the cheap and middling diagrams, not a promise about the
   * pathological ones.
   *
   * It also lands where it needs to for a BURST: 150ms exceeds a brisk typing gap, so while
   * someone is typing the trailing timer keeps being re-armed and no render starts at all.
   * That is the same reason the old fixed debounce looked good on a 120ms cadence — the
   * difference is that this one is only paid by diagrams expensive enough to be worth it,
   * where the fixed one charged every diagram, every time, including a first paint.
   */
  // ── BEGIN BACK-OFF PORT — lifted verbatim by test/unit/runtime/diagram-backoff.test.js.
  /**
   * THE LAST FEW RENDER COSTS, and the MEDIAN of them rather than the latest.
   *
   * One sample is not a cost, it is a sample. The first `mermaid.render` of a session also
   * pays Mermaid's own initialization, so a 4-node fence that costs ~36ms every other time
   * measured 200ms+ once — and a policy reading only the latest number then throttled the
   * whole burst that followed, turning a diagram that should update live on every keystroke
   * into one render. A median over three is enough to ignore that and still track a genuine
   * change in the diagram's size within a couple of renders.
   */
  const renderCosts = [];
  ...
  /**
   * THE AUTHOR'S OWN CADENCE — the input neither this branch nor the build before it used,
   * and the reason both of them were bad at some typing speed.
   *
   * A back-off only helps if it is LONGER than the gap between two keystrokes: longer, and
   * the trailing timer keeps being re-armed while somebody types and no render starts at
   * all; shorter, and it fires in every gap and buys a blocking render per character. So any
   * FIXED wait is good at one cadence and bad at another, which is exactly what the numbers
   * show. The 150ms debounce this work removed collapsed an 8-character burst to 1 render at
   * a 120ms cadence and to 8 renders at 200ms, on the same diagram — it was never a better
   * policy, it just happened to exceed the gap it was measured at.
   *
   * Only gaps that look like typing count, and both ends of that are load-bearing.
   */
  const sourceGaps = [];
  const SOURCE_GAP_SAMPLES = 4;
  const TYPING_GAP_MAX_MS = 600;
  /**
   * The shortest gap that counts as a separate change, and it is 100 for a reason that is
   * about arithmetic rather than about typing.
   *
   * One keystroke does not produce one mutation batch. Measured on the built Studio, a single
   * character in a fence produces four, at gaps of roughly `36, 0, 183, 234` — the host's
   * write, the runtime's own transforms echoing back through the observer, then the next
   * keystroke. Counting all of them put the median at ~110ms for an author typing at 200ms,
   * under the 150ms cost cap, so the cadence term could never raise the wait and the trailing
   * timer fired in every gap.
   *
   * 100ms is where the noise stops mattering rather than where it stops: a cadence under
   * 100ms is already beaten by the cost cap, so those samples cannot change the answer and
   * only drag the median toward the echo.
   */
  const TYPING_GAP_MIN_MS = 100;
  /**
   * WHAT COUNTS AS A PAUSE SCALES WITH THE RENDER, because a render is blocked time and not
   * idle time.
   *
   * A render occupies the main thread, so the author's next keystroke is delivered late and
   * the raw gap between two source changes is their real cadence PLUS the render sitting
   * between them. At a 350ms cadence with a 260ms render that reads as ~610ms — past a flat
   * 600ms threshold, so every sample was thrown away as "a pause" and the rhythm never
   * reached two samples.
   *
   * SUBTRACTING the render from the gap was tried first and measured worse (8 renders,
   * 4107ms against 3 and 2445ms). One keystroke produces four mutation batches, so the
   * discount was consumed by the first tiny one, and `36 - 260` is negative — which read as
   * "no gap at all" and cleared the rhythm on every render. Moving the threshold needs no
   * bookkeeping about which sample owns the render, and cannot go negative.
   */
  function pauseThresholdMs() {
    return TYPING_GAP_MAX_MS + medianRenderCostMs();
  }
  function recordSourceGap(gapMs) {
    // Below the minimum is the same edit still settling, or the runtime's own echo. Ignored
    // rather than treated as a pause: forgetting the rhythm on our own write would throw away
    // the sample that matters two batches later.
    if (gapMs >= 0 && gapMs < TYPING_GAP_MIN_MS) return;
    if (!(gapMs > 0) || gapMs > pauseThresholdMs()) {
      // Not typing. Forget the rhythm rather than averaging a pause into it.
      sourceGaps.length = 0;
      return;
    }
    sourceGaps.push(gapMs);
    if (sourceGaps.length > SOURCE_GAP_SAMPLES) sourceGaps.shift();
  }
  /** The typing gap to beat, or 0 when there is no evidence anyone is typing. */
  function typingGapMs() {
    // TWO samples, not one. A single gap is also what you see when somebody types one
    // character and stops, and holding a render on that evidence is the lone-keystroke case
    // this must never make slower.
    if (sourceGaps.length < 2) return 0;
    const sorted = [...sourceGaps].sort((a, b) => a - b);
    return sorted[Math.floor(sorted.length / 2)];
  }

  let lastRenderCostMs = 0;
  const RENDER_COST_FLOOR_MS = 50;
  const RENDER_COST_CAP_MS = 150;
  const TYPING_GAP_MARGIN = 1.25;
  /**
   * How far past the OBSERVED gap the wait must reach.
   *
   * A margin is needed because the observed gap is NOT the author's gap: the runtime cannot
   * tell its own DOM writes from the host's, so one keystroke arrives as about four mutation
   * batches and the pause is split across them. Instrumented on the built Studio at a 350ms
   * cadence the batches land at `36, 1, 185, 263`, so the median sample is ~227 for somebody
   * typing every 350ms — an under-report of about 1.5x, structurally rather than noisily.
   *
   * 1.75 was tried, to cover that factor, and REVERTED: it fixed the slow-cadence burst and
   * cost 1232ms on `edit-burst` cold, because a stale rhythm from an earlier burst held the
   * first render of the next one for most of a second. Raising this constant trades the case
   * an author is in constantly for the case they are in occasionally. The real fix is to stop
   * counting the runtime's own writes as source changes — there is precedent in
   * `burstIsMarkerChromeOnly`, which already drops bursts that are entirely marker chrome —
   * and until that exists the margin stays conservative and one slow-cadence cell stays
   * behind the old build. See §8 of the decision note.
   */
  /**
   * How long to let the source settle before dispatching a diagram run.
   *
   * Three terms, and each is there because dropping it was measured to break something:
   *
   *   FLOOR   A render under 50ms is not worth delaying. An 8-character burst on a ~30ms
   *           fence stretched 1086ms -> 1130ms rendering on every keystroke, which is 44ms
   *           across eight and invisible; the same burst on a ~70ms fence stretched to
   *           1493ms. Below the floor the diagram redraws live as you type, which is the
   *           whole feel of editing a small diagram, and it costs nothing.
   *
   *   COST    Twice the median render, so the share of the author's time spent re-rendering
   *           is bounded by construction: `c` out of every `c + 2c` is a third. CAPPED at
   *           150 — the exact debounce this work removed — because a fence already showing a
   *           drawing now always waits, and a wait above 150 would make a LONE keystroke on
   *           a large diagram slower than the build we replaced. Being slower than what came
   *           before is the one outcome this is not allowed to have.
   *
   *   CADENCE Beat the author's own typing gap, so the trailing timer is re-armed rather
   *           than firing between two keystrokes. This is the term that makes the policy
   *           cadence-independent instead of accidentally tuned to one speed, and it is
   *           allowed past the cost cap because by then two rapid changes have PROVEN
   *           somebody is typing — which is precisely when a blocking render is worst and
   *           when delaying it costs nothing, since another keystroke is already coming.
   *
   * `DIAGRAM_MAX_WAIT_MS` is the ceiling on all of it, so sustained typing still gets its
   * diagram back on a schedule rather than never.
   */
  function diagramBackoffMs() {
    const cost = medianRenderCostMs();
    if (cost <= RENDER_COST_FLOOR_MS) return 0;
    const byCost = Math.min(cost * 2, RENDER_COST_CAP_MS);
    const byCadence = typingGapMs() * TYPING_GAP_MARGIN;
    return Math.min(Math.max(byCost, byCadence), DIAGRAM_MAX_WAIT_MS);
  }

  /** Trailing-edge timer for the back-off above; re-armed by every pass while it is open. */
  let diagramBackoffHandle = null;
  /** When the observer last saw the document change — the author's own clock. */
  let lastSourceChangeAt = -Infinity;
  /** When the current back-off began, so continuous typing cannot starve the render. */
  let backoffStartedAt = 0;
  /**
   * The longest a diagram may be held back while the author keeps typing.
   *
   * A trailing debounce with no ceiling is only as reliable as the quietest moment in the
   * document, and someone typing steadily into a large fence never provides one — the
   * diagram would stop updating for as long as they wrote. The same argument, and the same
   * shape, as the settle backstop's own max wait further down.
   */
  const DIAGRAM_MAX_WAIT_MS = 1200;
  /**
   * IS ANY FENCE ABOUT TO DRAW ONE THE READER CANNOT SEE YET?
   *
   * This is the leading edge, and asking about INK rather than about a CLOCK is what makes
   * it right. The question a back-off has to answer is "does the author have something
   * valid to look at while we wait?" — and the honest way to answer it is to look.
   *
   *   empty slot  → nothing on screen. Waiting buys the author nothing and costs them the
   *                 whole wait. This is arriving at a slide, first paint, a cleared fence.
   *   holds a SVG → the previous drawing is up and correct-looking. Redrawing it is worth
   *                 doing, but never worth blocking a keystroke for.
   *
   * The clock version this replaces could not tell those apart: it asked "has the author
   * been idle?", which is TRUE at the first keystroke of a burst exactly as it is on a
   * click, so it fired a 248ms blocking render right as someone started typing — 3 renders
   * and 1508ms for an 8-character burst on a 64-node fence, against `main`'s 1 and 1101ms.
   * That was the one measurement where this branch was worse than the build before it.
   */
  function anyFenceWithoutInk() {
    if (typeof document === 'undefined') return false;
    for (const preEl of document.querySelectorAll(PENDING_FENCE_SELECTOR)) {
      const slot = preEl.nextElementSibling;
      if (!slot?.querySelector('svg')) return true;
    }
    return false;
  }
  // ── END BACK-OFF PORT
...
/**
  ...
  function scheduleRun() {
    ...
    // The author's rhythm, sampled where every source change already passes through.
    const changedAt = nowMs();
    recordSourceGap(changedAt - lastSourceChangeAt);
    lastSourceChangeAt = changedAt;
  ...
  function initAndRun({ force = false, backoffElapsed = false } = {}) {
    ...
    const backoff = diagramBackoffMs();
    // LEADING EDGE. A back-off is for a BURST, and the first change after a pause is not one:
    // clicking onto a cold diagram slide is a single interaction the author is waiting on, and
    // charging it a back-off put `nav` cold back to 195ms from 94ms. So dispatch at once when
    // the previous dispatch is further back than the back-off itself — the author has paused
    // for at least as long as a render costs — and throttle only while changes keep landing
    // inside that window. The first keystroke of a burst is therefore instant too, and the
    // rest of the burst still collapses to one render.
    if (backoff > 0 && !backoffElapsed && !anyFenceWithoutInk()) {
      if (!diagramBackoffHandle) backoffStartedAt = nowMs();
      if (diagramBackoffHandle) clearTimeout(diagramBackoffHandle);
      diagramBackoffHandle = setTimeout(function fire() {
        diagramBackoffHandle = null;
        // RE-CHECK AT FIRE TIME, because the window can elapse while the author is still
        // typing: a render blocks the main thread, so no pass runs to re-arm this timer for
        // the render's whole duration, and on a 64-node fence that handed back a render
        // every ~450ms mid-burst. Asking the AUTHOR'S clock rather than the pass clock is
        // what makes "do not draw while they are typing" actually hold.
        const quietFor = nowMs() - lastSourceChangeAt;
        const heldFor = nowMs() - backoffStartedAt;
        if (quietFor < backoff && heldFor < DIAGRAM_MAX_WAIT_MS) {
          diagramBackoffHandle = setTimeout(fire, Math.min(backoff - quietFor, DIAGRAM_MAX_WAIT_MS - heldFor));
          return;
        }
  ...
  function deferUntilQuiet(preEl) {
    ...
    if (!deferredSince.has(src)) deferredSince.set(src, nowMs());
    armErrorSurface();
  }
  ...
  function renderDiagramJob(mermaid, scopeKey, job) {
    ...
    if (forceRender.delete(job.preEl)) return renderDiagramNow(mermaid, scopeKey, job);
    ...
    return parsesCleanly(mermaid, job.source).then((ok) => {
      ...
      return renderDiagramNow(mermaid, scopeKey, job);
    });
  }

  // ── END PARSE-GATE PORT

  function renderDiagramNow(mermaid, scopeKey, { preEl, target, source }) {
    ...
    return new Promise((resolve) => {
      ...
      Promise.resolve()
        ...
        .then((result) => {
          ...
          lastRenderCostMs = nowMs() - startedAt;
          recordRenderCost(lastRenderCostMs);
