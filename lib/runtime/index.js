...
(() => {
  ...
  function transformSlotLabels() {
    if (typeof document === 'undefined') return;
    const SELECTOR = 'section.compare-prose, section.decision, section.split-panel, section.split-compare, section.statute-stack, section.regulatory-update, section.authority-chain, section.redline, section.timeline, section.list-criteria, section.actors, section.kpi, section.stats, section.list-steps.chevron, section.list-steps.converge, section.list-steps.ghost';
  ...
  function applyDeckLogoFromFrontMatter() {
    ...
    fetch(mdUrl)
      ...
      .then((src) => {
        ...
        // Cache the parsed config (not just the raw fetch result) so
        // applyCachedDeckLogo can re-inject on every later
        // runAllContentTransforms pass without re-fetching — see the cache
  ...
  function applyMastheadMetaFromFrontMatter() {
    ...
    fetch(mdUrl)
      ...
      .then((src) => {
        ...
        // path and this fetch-wrapper parse it ONE way. Cached so
        // applyCachedMastheadMeta can re-fill a fresh masthead-bay on every
        // later runAllContentTransforms pass without re-fetching.
        cachedMastheadMeta = metaTile.readFrontMatter(src) || '';
        applyCachedMastheadMeta();
      })
      .catch(() => { /* fetch blocked / 404 / sandbox — no-op */ });
  }
  ...
  function applyDeckClassFromFrontMatter() {
    ...
    fetch(mdUrl)
      ...
      .then((src) => {
        ...
        const classTokensRaw = cm ? cm[1].trim().split(/\s+/).filter(Boolean) : [];
        ...
        const fmColorMode = fm[1].match(/^\s*color-mode:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        const fmClaim = fm[1].match(/^\s*claim:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        const fmStamp = fm[1].match(/^\s*stamp:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        const stampTokens = stampClass(fmStamp ? fmStamp[1] : '') ? [stampClass(fmStamp[1])] : [];
        const fmTone = fm[1].match(/^\s*tone:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        const fmSpectrumTrim = fm[1].match(/^\s*spectrum-trim:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        const fmRule = fm[1].match(/^\s*rule:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        const ruleTokens = ruleClass(fmRule ? fmRule[1] : '') ? [ruleClass(fmRule[1])] : [];
        const fmEyebrow = fm[1].match(/^\s*eyebrow:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        const fmHeadline = fm[1].match(/^\s*headline:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        const fmLift = fm[1].match(/^\s*lift:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
        ...
        // runAllContentTransforms pass without re-fetching.
  ...
  // authorTags: draw the per-cell "Fix Me" culprit overlays (an AUTHORING
  // affordance for the live preview / Studio). The fluid VIEWER passes false —
  // a reader gets the honest overflow ring + "Overflows" marker (content is
  // clipped, not silently lost) but not the author-facing "fix this" tags.
  function startOverflowWatcher({ authorTags = true } = {}) {
    if (typeof document === "undefined") return;
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        if (tabAction === 'add') {
          ...
          tab.textContent = overflowTabText(authorTags);
          s.appendChild(tab);
        ...
        // illegible while its box fits, and it can be both — so this runs unconditionally
        // beside the overflow branch, never instead of it. Same probe and same floor as
        // the export watcher (lib/core/overflow-probe.js; the emulator injects the very
        // same function source), so the live preview and the PDF cannot disagree about
        // what "too small" means.
        const leg = probeFigureLegibility(s, FIGURE_TEXT_FLOOR_RATIO);
  ...
  function boot() {
    ...
    if (document.documentElement.hasAttribute("data-lattice-fluid-capable")) {
      ...
      try { startOverflowWatcher({ authorTags: false }); } catch (_e) { /* watcher is best-effort */ }
      return;
    }
