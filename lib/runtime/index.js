...
(() => {
  ...
  function ensureConfigured(mermaid, { force = false } = {}) {
    ...
    mermaid.initialize({
      ...
      flowchart: {
        ...
        // (HARD RULE #1). This path used to set 15 while the PDF path set nothing
        // and took mermaid's built-in 8, so the same deck's nodes came out one
        // size in preview and another in the export.
        padding: DIAGRAM_NODE_PADDING,
        // `subGraphTitleMargin: { top: 10, bottom: 100 }` was here. It is the only
        // mermaid config that reaches the cluster's INTERNAL space, and it is not
        // usable: any non-zero value shifts the subgraph title out of the box
        // mermaid has already sized, so the title renders CLIPPED IN HALF, and the
        // `bottom` term adds that many user units of dead space inside the box —
        // 100 of them here. Rendered at 5/10, 6/12 and 10/100; all three clip.
        // Dropped rather than re-tuned, which also puts this path back in step
        // with the PDF path, where it was never set.
  ...
  function bootstrap() {
    ...
    afterDeckFrontMatter(() => { if (applyDefaultComponent()) runAllContentTransforms(); });
