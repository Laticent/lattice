...
function warnMissingDagre() {
  ...
  try {
    ...
    console.warn(
      ...
      + 'that branches will fall back to the chain grid, drawing each branch as a skip. Load it with a <script src> '
...
(() => {
  ...
  function drawFreshStateCharts() {
    try {
      ...
      if (!drawable) return;
      stateChartLayout.installStateChartLayout(document, { onlyFresh: true });
