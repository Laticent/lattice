...
(() => {
  ...
  function runAllContentTransforms() {
    ...
    if (deckFrontMatterSettled || bakedFrontMatter !== null) transformInlinePills();
...
function transformInlinePills() {
  ...
  function bootstrap() {
    ...
    // synchronously inside pass 1, so both signals are false and a deck pays
    // exactly nothing — which is the property the old gate was reaching for and
    // the reason not to simply drop it. Only the FETCH FALLBACK, where the answer
    // genuinely cannot arrive before first paint, buys another pass — measured at
    // 2 for a plain deck and 3 for one carrying a chart, the third being the
    // chart rebuild's own mutation returning through the observer below.
    //
    ...
    afterDeckFrontMatter(() => {
      ...
      if (stamped || deckClassStampedSincePass || inlinePillsAwaitingFrontMatter) runAllContentTransforms();
    });
