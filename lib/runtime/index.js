...
(() => {
  ...
  function runAllContentTransforms() {
    ...
    // Resolve the deck's front matter first — memoized, so this is a no-op after
    // the first pass. The point of doing it HERE is the baked block: resolving it
    // takes the block out of the DOM, and nothing below may measure a slide that
    // still contains it (an inert zero-height element still takes a `gap` in a
    // flex column). bootstrap() already resolves it earlier; this keeps the
    // ordering true even if it didn't.
  ...
  function applyMastheadMetaFromFrontMatter() {
    ...
    withDeckFrontMatter((fm) => {
      ...
      cachedMastheadMeta = metaTile.readFrontMatter(`---\n${fm}\n---\n`) || '';
      applyCachedMastheadMeta();
    });
  }
  ...
  function applyDeckClassFromFrontMatter() {
    if (typeof document === 'undefined') return;
    withDeckFrontMatter((fm) => {
      const cm = fm.match(/^\s*class:\s*["']?(.*?)["']?\s*$/m);
      ...
      const fmColorMode = fm.match(/^\s*color-mode:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmFinish = fm.match(/^\s*finish:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmMode = fm.match(/^\s*mode:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmClaim = fm.match(/^\s*claim:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmStamp = fm.match(/^\s*stamp:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      const stampTokens = stampClass(fmStamp ? fmStamp[1] : '') ? [stampClass(fmStamp[1])] : [];
      const fmTone = fm.match(/^\s*tone:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmSpectrumTrim = fm.match(/^\s*spectrum-trim:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmRule = fm.match(/^\s*rule:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      const ruleTokens = ruleClass(fmRule ? fmRule[1] : '') ? [ruleClass(fmRule[1])] : [];
      const fmEyebrow = fm.match(/^\s*eyebrow:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmHeadline = fm.match(/^\s*headline:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
      ...
      const fmLift = fm.match(/^\s*lift:\s*["']?([A-Za-z0-9_-]+)["']?\s*$/m);
