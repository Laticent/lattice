...
(() => {
  ...
  function applyCachedDeckLogo() {
    ...
    // them by inheritance. Mirrors deckLogoVars / deckLogoInCorner in plugins.js;
    // the three render paths must agree (HARD RULE #1).
    const applyLogoPlacement = (section) => {
      if (cfg.logoScale != null) section.style.setProperty('--logo-scale', String(clamp(cfg.logoScale, 0.2, 3)));
      if (cfg.logoX != null && cfg.logoY != null) {
        section.style.setProperty('--logo-x', `${clamp(cfg.logoX, 0, 100)}%`);
        section.style.setProperty('--logo-y', `${clamp(cfg.logoY, 0, 100)}%`);
        section.style.setProperty('--logo-anchor-right', 'auto');
        section.style.setProperty('--logo-nudge', 'translate(-50%, -50%)');
      } else {
        // Not repositioned, so it sits in the top-right corner the marker tabs share.
        section.setAttribute('data-logo-corner', '');
      }
    };
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        // `policy.mark` gates the TAB too, not just the class. It used to gate only the
        // class, so at `off` this watcher appended a tab that survived purely because
        // CSS hid it — while the emulator's inline watcher skips its whole tab branch at
        // `off`. Two watchers producing different DOM for the same level is the HARD
        // RULE #1 divergence the shared kernel exists to prevent, and `off` promises to
        // leave nothing, not to leave something invisible. Folding it into `over:` here
        // rather than skipping the branch also REMOVES a tab an earlier pass left.
        ...
        // channel uses.
        const tabText = policy.authorTags && !over ? policy.tabTextCut : policy.tabText;
        const tabAction = overflowTabAction({ over: drawTab, hasTab: !!tab });
        if (tabAction === 'add') {
          ...
          tab.remove();
        } else if (tab && tab.textContent !== tabText) {
