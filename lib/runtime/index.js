...
(() => {
  ...
  function ensureConfigured(mermaid, { force = false } = {}) {
    ...
    mermaid.initialize({
      ...
      // CSS custom properties. Mermaid merges an author's in-source `%%{init}%%`
      // OVER this siteConfig per render (`updateCurrentConfig`), so a directive
      // that names layout/curve/renderer keeps every color it did not set — the
      // #1311 guarantee, for free, with no per-diagram injection. The PDF path
      // cannot do this (mmdc is a separate process, so its config has to travel in
      // the diagram source); that difference is delivery, not policy. What is
      // shared is the token→variable MAP, not the plumbing.
  ...
  function runAllContentTransforms() {
    ...
    // The DEFAULT-component rule (#1292) belongs to that same family and was the one
    // member missing from it: it ran ONCE, from bootstrap's `afterDeckFrontMatter`
    // continuation, so a surface that REPLACES `<section>` elements on re-render — the
    // marp-vscode webview being the whole reason this DOM mirror exists — served every
    // later render with the fresh sections unstamped and therefore unstyled, which is the
    // exact symptom #1292 was filed about. Re-stamping here is idempotent by construction:
    // `withDefaultComponent` returns the SAME array when the list already names a
    // component, so a stamped section is not rewritten and this cannot loop.
    //
    // ORDER IS LOAD-BEARING and is why this sits immediately after applyCachedDeckClass()
    // rather than anywhere else in the pass: the rule reads the RESOLVED class list, so a
    // deck-wide `class: kpi` must already be on the section. Stamped first, an un-classed
    // slide on such a deck would come out `content kpi` — two components — which is the
    // ordering bug bootstrap's comment above `afterDeckFrontMatter` describes.
    //
    // GATED on the front matter having settled, for that same reason: the first transform
    // pass runs SYNCHRONOUSLY, before the deck-class fetch resolves, and stamping then
    // would lose the same contest. Before it settles this is a no-op and the behavior is
    // exactly what bootstrap already produced.
