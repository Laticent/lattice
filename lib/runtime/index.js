...
(() => {
  ...
  function drawFixMeTags(targets) {
    ...
    // when the PAINTED signature would actually differ: same target elements (by
    // identity), same label/hint, same ROUNDED rect (sub-pixel float jitter between
    // otherwise-identical measurements must not itself be treated as a change, or
    // this guard would just relocate the oscillation one level up).
    const sig = targets
      .map((t) => {
        const r = t.el.getBoundingClientRect();
        return [t.el, t.label || 'Fix Me', t.hint || '', Math.round(r.top), Math.round(r.left), Math.round(r.width), Math.round(r.height)].join('|');
      })
      .join(';;');
