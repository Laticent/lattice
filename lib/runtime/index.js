...
(() => {
  ...
  // …and 0 for a fence with NOTHING ON SCREEN YET. The debounce buys one thing: not
  // re-rendering a diagram the author is in the middle of editing. A fence whose slot is
  // EMPTY has no such diagram — there is nothing to protect and nobody to spare, and the
  // 150ms is simply added to the wait. Measured on the built Studio, navigating onto a
  // diagram slide for the first time: 214ms of empty slot, of which 150 was this timer.
  //
  // It is still a timeout rather than a direct call, so a mutation BURST still coalesces
  // into one pass — every record in the burst clears the pending handle and re-arms.
  const COLD_MS = 0;
  ...
  /**
   * @param {{ delay?: number }} [opts] `delay` 0 for a pass that has an empty diagram slot
   *   to fill (see COLD_MS). A burst that asks for the shorter delay wins — a single fence
   *   showing nothing is the case that decides how long the author waits.
   */
  ...
  // A pending fence whose SVG target is empty — the slide is showing nothing where a
  // diagram goes. `:empty` is exact here: the target holds an `<svg>` or it holds nothing,
  // because every writer of that div assigns whole markup (`innerHTML`) or moves one
  // element into it.
  const EMPTY_PENDING_SELECTOR =
    'pre[data-mermaid-state="pending"] + .mermaid:empty,marp-pre[data-mermaid-state="pending"] + .mermaid:empty';
  ...
  function adoptOutgoingDiagrams(records) {
    ...
    // Every fence `<pre>` in a subtree, in document order. Both sides are indexed the
    // same way so position means the same thing on each — which is the whole basis of
    // the match, and why the counts have to agree before any of it is trusted.
    const fencePres = (node) => {
      if (node.nodeType !== 1 || typeof node.querySelectorAll !== 'function') return [];
      const out = [];
    ...
    for (const record of records) {
      ...
      // POSITION, so one un-rendered diagram on a multi-diagram slide costs only its
      // own hold instead of refusing the whole slide.
      const donors = [];
      for (const node of record.removedNodes) {
        for (const preEl of fencePres(node)) {
          const sib = preEl.nextElementSibling;
          const svg = sib?.classList.contains('mermaid') ? sib.querySelector('svg') : null;
          donors.push(svg ? { svg, sectionEl: preEl.closest?.('section') || null } : null);
        }
      }
      if (!donors.some(Boolean)) continue;
      const arrivals = [];
      for (const node of record.addedNodes) arrivals.push(...fencePres(node));
      if (arrivals.length !== donors.length) continue;
  ...
  function bootstrap() {
    ...
    if (typeof MutationObserver !== "undefined") {
      new MutationObserver((records) => {
        ...
        // A fence with an EMPTY slot after both of those is one the author is watching
        // nothing in — no cached SVG, no diagram held over. Nothing is gained by making
        // them wait out a debounce that exists to protect a diagram they can see.
        scheduleRun({ delay: document.querySelector(EMPTY_PENDING_SELECTOR) ? COLD_MS : DEBOUNCE_MS });
