...
const {
  AUTHORING_DEFAULT_MARKER, EXPORT_DEFAULT_MARKER, overflowMarkerFrom,
...
(() => {
  ...
  /**
   * The deck's `overflow-marker:` level, or `fallback` when nothing says.
   *
   * Same BAKED-only rule as `deckFormMode` above, and for the same reason: the
   * watcher starts during boot, and only the baked block is readable that early
   * (the `fetch` fallback resolves a tick later, and not at all over `file://`).
   *
   * The fallback differs by SURFACE, and when the caller does not name one the
   * PRESENCE of a baked block decides it. That is not a heuristic: the block is
   * written by exactly one function (`frontMatterBlock` in lib/core/marp-bundle.js),
   * so a document carrying one IS an Export-to-Marp bundle and a document without
   * one is a live preview / Studio / published page. Authoring surfaces default to
   * `author` (ring + "Overflows" + the Fix-Me overlays); an exported artifact
   * defaults to `reader`. Getting this backwards is the defect the register exists
   * to fix — the bundle renders through THIS runtime inside marp-cli's browser, so
   * it inherited the authoring default and shipped "Fix Me" tags to recipients.
   *
   * The fluid viewer names its fallback explicitly, because it is a reader surface
   * that carries no baked block.
   */
  function deckOverflowMarker(fallback) {
    const base = fallback
      || (bakedFrontMatter === null ? AUTHORING_DEFAULT_MARKER : EXPORT_DEFAULT_MARKER);
    return overflowMarkerFrom(bakedFrontMatter || '', base);
  }
  ...
  function bootstrap() {
    ...
    // No explicit fallback: a document with no baked block is a live preview and
    // defaults to `author`; a bundle carries one and defaults to `reader`. The
    // deck's own `overflow-marker:` overrides either.
  ...
  // `level` is the deck's `overflow-marker:` register — who the signal is
  ...
  // type-floor alarm); `reader` keeps the honest ring but calls it "More below"
  ...
  function startOverflowWatcher({ level = AUTHORING_DEFAULT_MARKER } = {}) {
    if (typeof document === "undefined") return;
    ...
    // red ring + "OVERFLOWS" flag, reader = no ring and a calm "More below ↓" pill.
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        const { over, overCells } = probeSectionOverflow(s, CLIP_CELL_SELECTOR, TOL);
        ...
        const tabAction = overflowTabAction({ over, hasTab: !!tab });
        if (tabAction === 'add') {
          ...
          // restyle is base.fluid-view.css.
        ...
        // treatment — `overflowMarkerPolicy(level).tabText` turns "Overflows" into a calm "More
        // below", and base.fluid-view.css restyles the ring into a pill — because a reader
  ...
  function boot() {
    ...
    if (document.documentElement.hasAttribute("data-lattice-fluid-capable")) {
      ...
      // The fluid viewer is a READER surface by construction, so that is its
      // fallback — but resolve the deck's own register first, so an author who
      // asked for `off` gets `off` here too. deckFrontMatterSource() has to run
      // before the read: this path never calls runAllContentTransforms(), which
      // is what primes the baked block on the live-preview path.
      try { deckFrontMatterSource(); } catch (_e) { /* no block to read */ }
