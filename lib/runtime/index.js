...
(() => {
  ...
  // nothing. Mermaid is the only third-party library those artifacts need,
  // because a ```mermaid fence becomes `<pre><code class="language-mermaid">`
  // in ANY markdown renderer — so the runtime can find it without a Lattice
  // plugin having run. That is the test for whether a library belongs in a
  // hand-off artifact at all: does the DOM node it needs survive a PLAIN parse?
