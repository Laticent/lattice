...
(() => {
  ...
  let exportSettings;
  /**
   * Is this document a THUMBNAIL — a miniature of a slide, rendered in a grid of its
   * peers, that nobody authors from?
   *
   * Opt-in via `<html data-lattice-thumbnail>`, the same root-attribute idiom as
   * `data-lattice-fluid-capable`: one producer sets it (the docs site's shared
   * thumbnail face, `SlideThumbFace`), nothing else does, and every other surface —
   * the VS Code preview, the HTML player, an Export-to-Marp bundle, the Studio's own
   * full-size preview — is byte-identical to before.
   *
   * WHY A THUMBNAIL WANTS NOTHING WATCHED. Two reasons, and the second is the one that
   * scales:
   *
   *   · The signal has no addressee. It is unreadable at ~260px, and in the add-slide
   *     gallery it describes a CATALOG SAMPLE the author neither wrote nor can fix —
   *     measured on the shipped gallery, the `image` tile painted an "Overflows" tab
   *     and `state-chart` / `quadrant` painted type-floor alarms.
   *   · The cost is per-DOCUMENT, so it multiplies by the grid. The watcher arms a
   *     permanent MutationObserver over `document.body` (subtree + childList +
   *     characterData + attributes) plus a resize listener, and rAF-dispatches a
   *     full-document scan whose probes force layout — once per frame, in every one of
   *     the ~33 frames a scrolled gallery holds open (the budget in slide-thumb.tsx).
   *
   ...
   * container-query reflow, so a thumbnail that skipped them would render DIFFERENTLY
   * from the slide it depicts, which is a worse defect than the one being fixed.
   */
  function isThumbnailDocument() {
    return typeof document !== 'undefined' && !!document.documentElement?.hasAttribute('data-lattice-thumbnail');
  }
  ...
  function bootstrap() {
    ...
    //
    // …unless this document is a THUMBNAIL, which has no addressee for the signal and
    // pays the watcher's cost once per frame across a whole grid. `off` is not a
    // quieter marker here — it is the level that installs NOTHING (see
    // startOverflowWatcher: "Sweep once, install nothing: no probe, no observer, no
    // resize handler"), which is exactly what a thumbnail wants, and it sweeps + stamps
    // on the way out where a bare `return` would not. See isThumbnailDocument().
    startOverflowWatcher({ level: isThumbnailDocument() ? 'off' : deckOverflowMarker() });
  }
