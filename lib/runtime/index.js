...
(() => {
  ...
  const PREVIEW_ONLY_CONFIG = {
    ...
    //
    // WHAT THIS KEY DOES AND DOES NOT COVER, measured rather than assumed (#1246). It gates
    // TWO things: URL handling (`formatUrl` runs `sanitizeUrl`, and `setClickFun` refuses to
    // bind a click callback, unless the level is `loose`), and a WHOLE-SVG DOMPurify pass in
    // `render()` (`else if (!isLooseSecurityLevel) svgCode = purify.sanitize(svgCode, …)`).
    // It does NOT solely govern NODE LABELS — `sanitizeText` runs DOMPurify on those
    // UNCONDITIONALLY and consults this key only for an extra `removeScript` pass. So a
    // label payload has belt and braces and a `click … javascript:` payload has only this
    // line: measured, flipping to `loose` moves the click arm and leaves nine label arms
    // green. Every one of those contracts is THIRD-PARTY, so they are pinned behaviorally in
    // docs/e2e/mermaid-post-sanitize.spec.ts against the real Playground and the CDN's real
    // Mermaid (`mermaid@11`, currently 11.16.1 — not the 11.14.0 in node_modules). A source
    // assertion here could see none of it.
    ...
    for (const section of document.querySelectorAll('section.verdict-grid, section.pricing')) {
      for (const outerLi of section.querySelectorAll(':scope > ul > li')) {
        ...
        for (const li of badgeItems) {
          ...
          li.replaceChildren(badgeSpan(`badge ${sem} ${shape}`, m[2]));
        }
      }
    }
  }
