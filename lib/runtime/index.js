...
(() => {
   ...
   * what this can ADD to the wait after the last keystroke; past it the author is already
   * waiting on the render itself and making them wait longer first buys nothing. Above about
   * 100ms of render the cap binds before the doubling does, so the one-third share is a
   * bound on the cheap and middling diagrams, not a promise about the pathological ones.
   */
  ...
  const RENDER_COST_CAP_MS = 200;
  /**
   * How long to let the source settle before dispatching a diagram run.
   *
   * TWICE the last cost, so the share of the author's typing time this spends re-rendering
   * is bounded by construction rather than tuned: rendering for `c` out of every `c + 2c`
   * is one third, whatever the diagram costs. A back-off of `c` alone was measured and
   * rejected — at 16 nodes it lands at 70ms, under a 120ms typing gap, so it still renders
   * per keystroke and stretched an 8-character burst from 1087ms to 1493ms.
   */
  function diagramBackoffMs() {
    if (lastRenderCostMs <= RENDER_COST_FLOOR_MS) return 0;
    return Math.min(lastRenderCostMs * 2, RENDER_COST_CAP_MS);
  }
  /**
   * How long idle counts as "the author has stopped", for the leading edge below.
   *
   * The UNCAPPED doubled cost, and it is deliberately a different number from the wait. They
   * answer different questions — "has this person paused?" against "how long may we make
   * them wait?" — and using the capped wait for both leaked the whole throttle: on a 64-node
   * fence the cap holds the wait at 200ms while a render costs ~248ms, so the gap between two
   * keystrokes measured from the end of a render was 240ms, read as idle, and fired a render.
   * Every ~240ms, which is 4 renders across an 8-character burst.
   */
  function diagramIdleMs() {
    if (lastRenderCostMs <= RENDER_COST_FLOOR_MS) return 0;
    return lastRenderCostMs * 2;
  }
  // ── END BACK-OFF PORT
  ...
  /**
   * When the last diagram run FINISHED, so the leading edge can tell a burst from a click.
   *
   * Finished, not dispatched, and the difference is the whole test. Measured from dispatch,
   * the interval includes the render itself — 232ms on a 64-node fence, which is longer than
   * the 200ms cap the back-off is clamped to, so the "has the author paused?" question
   * answered YES on every keystroke of a burst and the throttle never engaged (8 renders,
   * 2674ms, straight back to the regression). Measured from completion, a burst reads ~0ms
   * idle because the next pass is already queued behind the render, and a click after a
   * pause reads seconds.
   */
...
/**
  ...
  function initAndRun({ force = false, backoffElapsed = false } = {}) {
    ...
    const idleFor = nowMs() - lastRenderEndedAt;
    if (backoff > 0 && !backoffElapsed && idleFor < diagramIdleMs()) {
  ...
  function beginDiagramRun(mermaid, themeVars, look) {
    ...
    diagramQueue = diagramQueue
      ...
      .then(() => {
        ...
        // The idle clock the leading edge reads. Set here, at the END of the run, so a burst
        // whose next pass is already queued reads as not-idle.
        lastRenderEndedAt = nowMs();
