...
const { pillElement: inlinePillElement } = require('../../lib/core/inline-pills.js');
const { stateClassesFor, stateElement: inlineStateElement } = require('../../lib/core/state-marks.js');
 ...
 * `<code>{LIVE}</code>`.
 *
 * THE DOUBLE-BACKTICK ESCAPE SURVIVES HERE FOR FREE, and not by accident: markdown
 * renders `` ``{LIVE}`` `` to exactly the same `<code>{LIVE}</code>` as the single
 * form, so the DOM cannot tell them apart — but by the time this runs, the escape has
 * already done its job upstream on the path that has the token. On a path with no
 * token there is nothing to escape FROM, because that path never interprets at parse
 * time either. Both ends stay consistent; a deck that wants the literal on BOTH paths
 * uses the double backticks and the engine leaves it, while Marp never had a pill to
 * suppress. This asymmetry is the reason the escape is checked on `markup` in the
 * plugin and not re-derived from text anywhere.
 */
function transformInlinePills() {
  for (const code of document.querySelectorAll('section code')) {
    ...
    const el = inlineStateElement(document, text) || inlinePillElement(document, text);
