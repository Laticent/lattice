...
/**
   * `initAndRun` handed this pass to machinery that will re-run it — a diagram run already
   * in flight, or the back-off timer below. NOT an init failure, and telling the two apart
  ...
  function renderDiagramJob(mermaid, scopeKey, job) {
    ...
    // The one path allowed to REMEMBER its failure: this fence failed `mermaid.parse` and
    // then sat out a whole quiet window, so the text is known bad rather than unlucky.
    if (forceRender.has(fenceSourceOf(job.preEl))) return renderDiagramNow(mermaid, scopeKey, job, true);
...
function transformInlinePills() {
  ...
  function bootstrap() {
    ...
    const tick = () => {
      ...
      // A deferred pass is not a stalled one: the diagram run's completion tail or the
      // back-off timer will call back. Stop polling rather than spending theme-wait frames
      // on a condition that has nothing to do with the theme.
