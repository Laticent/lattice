...
// Imported for its SIDE EFFECT: it installs `globalThis.__latticeDagre`, which
// the state-chart's browser pass reads. The pass is serialised through
// `.toString()` on the export path, so it cannot import a layout engine itself
// and can only reach one through a global. esbuild inlines dagre here (this
// bundle has no externals), so nothing is fetched at run time.
require('../core/dagre-layout.js');
...
(() => {
  ...
  function replayCachedFences() {
    ...
    // bierner.markdown-mermaid exposes a render-blocks-only shim with no `.render`. The
  ...
  // Did a pass SKIP the inline-directive transform because the front matter had not
  // settled? The re-run in `afterDeckFrontMatter` is conditional — it fires only when a
  // deck class or a default component was stamped — so on the plainest deck there is
  // (`theme:` and nothing else) a skipped pass would never be made up and the deck would
  // render no pills at all. Measured: the control arm of
  // test/unit/core/inline-code-register-paths.test.js caught exactly that, one edit after
  // the deferral was added.
  let inlinePillsAwaitingFrontMatter = false;
  // A DEADLINE on that wait, because the thing being waited for can never arrive: nothing
  // bounds `deckFrontMatterSource`'s fetch, so a sibling `.md` request that hangs (rather
  // than rejecting or 404-ing, both of which settle correctly) would leave this transform
  // permanently unrun and the deck would draw NO pills or marks at all. Measured on the
  // real bundle: a never-settling fetch gave `pills 0 marks 0` where the base gave 1 and 1.
  //
  // Degrading toward DRAWING rather than toward silence is the deliberate half. Both
  // outcomes are wrong renders, but they are not equally wrong: drawing late affects only a
  // deck that actually set `inline-code: literal`, while never drawing affects every deck
  // whose fetch hangs — and the second is the author's own content failing to appear.
  let inlineCodeDeadlinePassed = false;
  ...
  function runAllContentTransforms() {
    ...
    // DEFERRED UNTIL THE FRONT MATTER SETTLES, unlike its neighbours, because this
    // transform is the one that CANNOT BE UNDONE. Every other pass here is idempotent and
    // re-runnable, so the file's "immediate paint, then correct" philosophy costs them
    // nothing. This one replaces a `<code>` with a `<span>`: once a pill is drawn, the
    // later pass that learns the deck said `inline-code: literal` has nothing left to put
    // back. Measured before this gate: the class arrived correctly on pass 2 and the
    // pills — drawn on pass 1 — stayed. The cost is that a deck served without a baked
    // block shows its literal text for one fetch round trip; a brief literal is
    // recoverable, a wrong pill is not.
    // `bakedFrontMatter` is primed SYNCHRONOUSLY by `applyCachedDeckClass()` at the top of
    // this same pass, so on an Export-to-Marp bundle the answer is already known here and
    // the transform runs on pass 1 — which is what keeps that path at exactly ONE pass
    // (`runtime-mode-geometry.test.js` pins it; gating on `deckFrontMatterSettled` alone
    // made every exported bundle pay for a second full pass). Only the FETCH fallback,
    // where nothing is knowable this early, defers and buys itself the correction pass.
    if (deckFrontMatterSettled || bakedFrontMatter !== null || inlineCodeDeadlinePassed) transformInlinePills();
    else inlinePillsAwaitingFrontMatter = true; // and force the re-run below
...
function transformInlinePills() {
  ...
  function bootstrap() {
    ...
    //
    // THERE ARE THREE SIGNALS HERE NOW, not two — `owedAPass` joined them when the
    // inline directive grammar became the one transform that cannot be re-run into
    // correctness (it replaces a `<code>` with a `<span>`; a later pass has nothing
    // left to put back). It is false on the baked path by construction, because
    // `bakedFrontMatter` is non-null before the transform is reached, so the
    // no-extra-pass property above still holds. A checker measured that property
    // broken once, when the gate keyed on `deckFrontMatterSettled` alone and every
    // exported bundle paid for a second full pass; re-measured on the real bundle
    // after the fix, the baked path runs ONE pass for a plain deck, for
    // `class: dark`, and for `inline-code: literal`.
    //
    ...
    // Arm the deadline above. It exists for a fetch that NEVER settles — pathological and
    // rare — so it must not fire on one that is merely SLOW, which is common. Measured
    // against the real bundle with the fetch settling at a controlled delay:
    //
    //   settles  100ms  → 2 passes, literal   (ordinary path, deadline never reached)
    //   settles 2500ms  → 2 passes, literal
    //   settles 2950ms  → 2 passes, literal   (races a 3s deadline; the guard holds)
    //   settles 3200ms  → 4 passes, PILLS DRAWN against `inline-code: literal`
    //
    // The last row is why this is 10s and not the 3s it was first written as: past the
    // deadline the transform has already replaced the `<code>`, and the front matter
    // arriving afterwards has nothing left to put back. A sibling `.md` is same-origin and
    // a few KB, so 10s is far outside a real fetch and still bounded; the cost is that a
    // genuinely hung fetch shows literal text for 10s instead of 3, which is recoverable
    // where a wrong render is not.
    if (typeof setTimeout === 'function') {
      setTimeout(() => {
        if (deckFrontMatterSettled || !inlinePillsAwaitingFrontMatter) return;
        inlineCodeDeadlinePassed = true;
        runAllContentTransforms();
      }, 10000);
    }
    afterDeckFrontMatter(() => {
      ...
      const owedAPass = inlinePillsAwaitingFrontMatter;
      // Clear the latch BEFORE the re-run, not after: it is read once here, and leaving it
      // set would make the next reader re-derive the once-only property of
      // `afterDeckFrontMatter` to convince themselves the pass cannot repeat.
      inlinePillsAwaitingFrontMatter = false;
      if (stamped || deckClassStampedSincePass || owedAPass) runAllContentTransforms();
    });
    ...
    if (typeof MutationObserver !== "undefined") {
      new MutationObserver((records) => {
        ...
        // BEFORE the debounce, and in this microtask: tag the swapped-in fences and
        // give back any SVG we are already holding, so the first frame the host's
        // write produces carries the diagram instead of its source. Everything this
        // cannot settle stays pending for the debounced pass below.
        replayCachedFences();
        scheduleRun();
