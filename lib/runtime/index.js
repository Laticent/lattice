...
(() => {
   ...
   *   [ ] → fail + state-empty    (red ✕; verdict-grid only)
   *   [ ] → todo + state-todo     (neutral open ring; every other layout)
  ...
  function transformVerdictGridBadges() {
    ...
    for (const section of document.querySelectorAll('section.verdict-grid, section.pricing')) {
      // `[ ]` is "not met" (red ✕) in verdict-grid only; pricing takes the universal
      // neutral ring — the engine's `verdictGridBadges` makes the same call.
      const neutralEmpty = !section.classList.contains('verdict-grid');
      ...
      for (const outerLi of section.querySelectorAll(':scope > ul > li, :scope > ol > li')) {
        ...
        for (const li of outerLi.querySelectorAll('li')) {
          ...
          if (!m) continue;
          const { sem, shape } = stateClassesFor(m[1], neutralEmpty);
