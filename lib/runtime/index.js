...
const { ROW_LABEL_CLASS, resolveRowLabel } = require('../../lib/core/table-row-label.js');
...
function transformInlinePills() {
   ...
   * the DOM mirror of the markdown-it `table_row_labels` plugin. Both ask the
   * same kernel (lib/core/table-row-label.js) so the two render paths cannot
   * disagree about the same table (HARD RULE #1); all this one does differently
   * is read the cells out of the DOM instead of out of tokens.
   *
   * Reaches a slide carrying `table`, `row-label` or `no-row-label` only —
   * base refuses this bet by default, and `row-label` is how a plain table opts
   * in. Idempotent: a table already carrying the class is skipped.
   */
  function transformTableRowLabels() {
    if (typeof document === 'undefined') return;
    for (const section of document.querySelectorAll('section.table, section.row-label, section.no-row-label')) {
      ...
      for (const table of section.querySelectorAll('table')) {
        ...
        // The header row is `thead > tr`, or a first row made of `th` — the
        // same two shapes prose-projection's speakTable accepts, because a
        // table built by a transform does not always carry a `thead`.
        ...
        if (resolveRowLabel(classTokens, { headers, firstColumn })) table.classList.add(ROW_LABEL_CLASS);
      }
    }
  }
