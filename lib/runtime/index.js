...
const { readFormMode } = require('../../lib/integrations/markdown-it/plugins');
...
const { COLOR_MODE_TOKENS: COLOR_MODE_TOKEN_LIST } = require('../../lib/core/color-mode');
const { colorModeClass } = require('../../lib/core/resolve-color-mode');
// THE token → Mermaid themeVariables map, shared with the PDF path so the two
// renderers cannot drift (#1332 step 2, HARD RULE #1). This path supplies only
// the reader; see buildMermaidThemeVars below.
...
const { diagramScopeKey, diagramCacheKey } = require('../../lib/core/diagram-scope');
// Diagram GEOMETRY that both render paths must agree on. Only the node label
// inset today; the cluster's own inset is hardcoded inside mermaid's dagre
// wrapper and no config reaches it.
const { DIAGRAM_NODE_PADDING } = require('../../lib/integrations/mermaid/init-directive');
...
(() => {
  ...
  //
  // WHICH variables, and which token feeds each, is NOT decided here: this
  // function supplies the reader and the shared map does the rest
  // (lib/core/mermaid-theme-map.js, #1332 step 2, HARD RULE #1). It used to hold
  // a second copy of that map, kept in sync by a comment that pointed at the other
  // file. Both copies exposed the same 166 slots — a key-set gate has watched that
  // since #511 — but 38 of the VALUES had drifted apart, which that gate could not
  // see. test/unit/core/diagram-theme-parity.test.js now compares the values, and
  // fails on any divergence outside DIVERGENT_KEYS.
  //
  // `scopeEl` IS THE PER-SLIDE PORT (#1332 step 3). It used to be resolved inside
  // this function as `document.querySelector('section')` — ALWAYS slide 1 — so a
  // deck whose first slide is light baked LIGHT ink into every diagram in the
  // deck, including slide 9's `_class: dark` one. The chip is per-section CSS and
  // the ink is baked, so that was the last surviving instance of the #1326 bug
  // class: ink and chip describing different slides.
  //
  ...
  // `_class: dark` / `light` / `print`. CSS inheritance already answers the
  // question `resolveDiagramBand` has to answer offline, which is why this is a
  // parameter rather than a rewrite.
  function buildMermaidThemeVars(scopeEl) {
    ...
    // theme tokens, so a caller with no section to hand falls back to it only to
    // stay defined — it will read empty and hit the retry budget below.
    if (!scopeEl) scopeEl = document.querySelector('section') ?? document.documentElement;
    ...
    // THE map, shared with the PDF path (lib/core/mermaid-theme-map.js, #1332
    // step 2). `vc` IS this path's half of the port: getComputedStyle on the
    // section means CSS inheritance has already applied that slide's own classes,
    // so no band has to be resolved here — and a miss returns the empty string
    // rather than a black sentinel, because an unresolved theme on a slow webview
    // must fall through to the retry budget below, not paint the deck black.
    const result = buildDiagramTheme(vc);

    // The ONE sanctioned divergence (DIVERGENT_KEYS). The PDF path bakes a
    // monospace stack because mermaid's `sanitizeDirective` allow-list has no
    // hyphen, so `system-ui`/`sans-serif` is blanked the moment it rides in a
    // %%{init}%% directive — and a blank font makes mermaid MEASURE labels in one
    // font while the page RENDERS them in another, clipping them mid-word. This
    // path goes through `mermaid.initialize`'s far more permissive `sanitize`, so
    // it can carry the deck's real body font. A pre-existing WYSIWYG gap
    // (engineering/mermaid.md §5.3), tracked separately and deliberately left
    // alone here: closing it changes how every preview renders, which is not the
    // same change as unifying the map.
    result.fontFamily = v('font-body') || "'Outfit', system-ui, sans-serif";
  ...
  // (see buildMermaidThemeVars). It lives in lib/core/diagram-scope.js — pure and
  ...
  // Resolved themeVariables per scope key. Bounded by the number of distinct
  // class signatures in the deck, so no eviction policy is needed for an editor
  // session — same reasoning as mermaidSvgCache below.
  //
  // A live PALETTE switch is not invalidated here, exactly as it is not for
  // mermaidSvgCache: a Mermaid SVG bakes its colors, so a theme change needs a
  // preview reload either way (see the theme-change caveat on that cache).
  const diagramThemeCache = new Map();
  function themeVarsForScope(key, sectionEl) {
    let vars = diagramThemeCache.get(key);
    if (!vars) {
      vars = buildMermaidThemeVars(sectionEl);
      diagramThemeCache.set(key, vars);
    }
    return vars;
  }
  ...
  // reading the PALETTE from slide 1 (buildMermaidThemeVars), not probing
  // readiness there. Latching means the probe cost is paid once, not per pass.
  ...
  // The scope key whose palette is CURRENTLY loaded into mermaid's global config,
  // or null before the first `initialize`. This replaced the `__llMermaidConfigured`
  // one-shot: that flag existed because `mermaid.initialize` is global and
  // re-running it was assumed to be the only alternative to configuring once — but
  // "once per document" is exactly what baked slide 1's ink into slide 9's diagram
  // (#1332 step 3). Re-initializing per SCOPE keeps the call count at one per band
  // instead of one per diagram, and the render queue below is what makes it safe.
  let mermaidConfiguredScope = null;

  function configureForScope(mermaid, key, sectionEl) {
    if (mermaidConfiguredScope === key) return;
    mermaid.initialize({
      ...
      theme: "base",
      // STRICT — Mermaid's own default, and the PDF path (mmdc) has always used it
      // because it never overrides. The runtime was the one surface that opted OUT,
      // and that opt-out was a live XSS: with `loose`, a `click X "javascript:…"`
      // directive renders as `<a xlink:href="javascript:…">` inside the SVG, which
      // the runtime assigns straight to `innerHTML` below — clicking the node then
      // executes it, in the docs Studio's SAME-ORIGIN, un-sandboxed preview frame,
      // which renders shared and AI-generated decks (the HARD RULE #22 threat model:
      // XSS there is OpenRouter-key theft). Verified on the real Playground: the
      // payload fired under `loose` and does not under `strict`.
      //
      // The old comment claimed `loose` was "required to allow HTML (e.g. <br/>) in
      // node labels". That is not true on Mermaid 11 — `strict` still renders
      // `<p>first line<br>second line</p>` in a label (verified the same way). What
      // `strict` actually costs is Mermaid's `click` interactivity, which is the
      // attack vector itself and which no deck in this repo uses.
      ...
      // The palette lives HERE, on the global config, read from the active theme's
      // CSS custom properties AS THAT SLIDE RESOLVES THEM. Mermaid merges an
      // author's in-source `%%{init}%%` OVER this siteConfig per render
      // (`updateCurrentConfig`), so a directive that names layout/curve/renderer
      // keeps every color it did not set — the #1311 guarantee, for free, with no
      // per-diagram injection. The PDF path cannot do this (mmdc is a separate
      // process, so its config has to travel in the diagram source); that
      // difference is delivery, not policy. What is shared is the token→variable
      // MAP, not the plumbing.
      themeVariables: themeVarsForScope(key, sectionEl),
      quadrantChart: {
        titleFontSize: 24,
        pointTextPadding: 20,
      ...
      flowchart: {
        ...
        useMaxWidth: false,
        htmlLabels: true,
        // The node label inset, from ONE constant shared with the PDF path
        // (HARD RULE #1). Same value this path always used — mermaid's schema
        // default is 15, so the export was already rendering at 15 too and the
        // two never actually diverged here. Sharing the number keeps it that way.
        padding: DIAGRAM_NODE_PADDING,
        // `subGraphTitleMargin: { top: 10, bottom: 100 }` was here. It is the only
        // mermaid config that reaches the cluster's INTERNAL space, and it breaks
        ...
        // measured at 10/100, the inner rect starts 10 units into the outer label.
        // `bottom` also adds that many user units of dead space inside the box.
        // Verified on the real Playground before and after. Dropped rather than
        // re-tuned, which also puts this path back in step with the PDF path,
  ...
  // Has the deck's front matter resolved (or failed to) at least once? This is
  // exactly the point at which "does a deck-wide `class:` name the component for
  // every slide?" has an answer, and so the point from which the DEFAULT-component
  // rule may safely stamp. It is NOT the same question as
   ...
   * the engine path emits just `kpi`. Source order is not execution order, so the
  ...
  function runAllContentTransforms() {
    ...
    // comment gives: the default-component rule reads the RESOLVED class list, so a deck-wide
    // `class: kpi` must be on the section before it decides the slide names no component.
  ...
  function initAndRun({ force = false } = {}) {
    ...
    // GROUP BY CASCADE SCOPE (#1332 step 3). `mermaid.initialize` is global, so a
    // deck spanning two bands has to configure, render, reconfigure, render — and
    // that ordering only holds if the renders in between are not interleaved. The
    // grouping is what keeps the reconfigure count at one per band; the queue below
    // is what keeps the ordering true.
    ...
    for (const preEl of document.querySelectorAll(FENCE_SELECTOR)) {
      ...
      const scopeKey = diagramScopeKey(sectionEl);
      ...
      // here rather than in the queue so an unchanged deck never enqueues anything
      // and never reconfigures mermaid.
      const cachedSvg = mermaidSvgCache.get(diagramCacheKey(scopeKey, source));
      if (cachedSvg) {
      ...
      if (!group) {
        group = { scopeKey, sectionEl, jobs: [] };
        groups.set(scopeKey, group);
      }
      group.jobs.push({ preEl, target, source });
    }

    for (const group of groups.values()) enqueueDiagramGroup(mermaid, group);
    return true;
  }
  ...
  // render was dispatched concurrently; now the configure is per band, and
  // concurrent dispatch across bands would let a render read the NEXT band's
  // palette. Serializing per GROUP rather than per DIAGRAM keeps the concurrency
  // that mattered: within a group the config is identical, so its diagrams still
  // render in parallel exactly as they always did, and a single-band deck (almost
  // every deck) is dispatched in one batch just like before.
  let diagramQueue = Promise.resolve();
  function enqueueDiagramGroup(mermaid, group) {
    diagramQueue = diagramQueue
      .then(() => {
        configureForScope(mermaid, group.scopeKey, group.sectionEl);
        return Promise.all(group.jobs.map((job) => renderDiagramJob(mermaid, group.scopeKey, job)));
      })
      // A group must not be able to poison the chain for later groups: every job
      // already handles its own failure, and `configureForScope` throwing (a stub
      // mermaid slipping past the guard) would otherwise leave every later band
  ...
  function applyCachedDeckLogo() {
    ...
    const applyLogoPlacement = (img) => {
      if (cfg.logoScale != null) img.style.setProperty('--logo-scale', String(clamp(cfg.logoScale, 0.2, 3)));
      if (cfg.logoX != null && cfg.logoY != null) {
        img.style.setProperty('--logo-x', `${clamp(cfg.logoX, 0, 100)}%`);
        img.style.setProperty('--logo-y', `${clamp(cfg.logoY, 0, 100)}%`);
   ...
   * run AFTER it, though, so a deck-wide `class: <component>` has already landed
   * and counts as the slide's component — hence `afterDeckFrontMatter` at the call
   * site rather than a plain statement, which would run first and lose the race.
   *
  ...
  function applyDeckClassFromFrontMatter() {
    if (typeof document === 'undefined') return;
    withDeckFrontMatter((fm) => {
      const classTokensRaw = (frontMatterValue(fm, 'class') || '').split(/[ \t]+/).filter(Boolean);
      // The first-class `color-mode:` key (light/dark/system/inherited) supersedes the
      // legacy `class: dark`/`class: light` color axis: when set it WINS (drop any color
      // token the alias carried); when absent, the `class:` alias still resolves.
      // Mirrors lib/integrations/markdown-it/plugins.js.
      const fmColorMode = fm.match(/^[ \t]*color-mode:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const colorModeToken = colorModeClass(fmColorMode ? fmColorMode[1] : '');
      const colorAxis = new Set(COLOR_MODE_TOKEN_LIST);
      const classTokens = colorModeToken ? classTokensRaw.filter((t) => !colorAxis.has(t)) : classTokensRaw;
      ...
      const fmFinish = fm.match(/^[ \t]*finish:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      ...
      const fmMode = fm.match(/^[ \t]*mode:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      ...
      const fmClaim = fm.match(/^[ \t]*claim:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const claimTokens = claimClasses(fmClaim ? fmClaim[1] : '').split(/\s+/).filter(Boolean);
      // Deck-wide stamp / tone STYLE registers (resolve-stamp.js / resolve-tone-style.js).
      const fmStamp = fm.match(/^[ \t]*stamp:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const stampTokens = stampClass(fmStamp ? fmStamp[1] : '') ? [stampClass(fmStamp[1])] : [];
      const fmTone = fm.match(/^[ \t]*tone:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const toneStyleTokens = toneStyleClass(fmTone ? fmTone[1] : '') ? [toneStyleClass(fmTone[1])] : [];
      ...
      const fmSpectrum = fm.match(/^[ \t]*spectrum:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const spectrumTokens = spectrumClass(fmSpectrum ? fmSpectrum[1] : '') ? [spectrumClass(fmSpectrum[1])] : [];
      const fmSpectrumEdge = fm.match(/^[ \t]*spectrum-edge:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const spectrumEdgeTokens = spectrumEdgeClass(fmSpectrumEdge ? fmSpectrumEdge[1] : '') ? [spectrumEdgeClass(fmSpectrumEdge[1])] : [];
      const fmSpectrumCard = fm.match(/^[ \t]*spectrum-card:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const spectrumCardTokens = spectrumCardClass(fmSpectrumCard ? fmSpectrumCard[1] : '') ? [spectrumCardClass(fmSpectrumCard[1])] : [];
      const fmSpectrumCardEdge = fm.match(/^[ \t]*spectrum-card-edge:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const spectrumCardEdgeTokens = spectrumCardEdgeClass(fmSpectrumCardEdge ? fmSpectrumCardEdge[1] : '') ? [spectrumCardEdgeClass(fmSpectrumCardEdge[1])] : [];
      const fmSpectrumTrim = fm.match(/^[ \t]*spectrum-trim:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const spectrumTrimTokens = spectrumTrimClass(fmSpectrumTrim ? fmSpectrumTrim[1] : '') ? [spectrumTrimClass(fmSpectrumTrim[1])] : [];
      ...
      const fmRule = fm.match(/^[ \t]*rule:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const ruleTokens = ruleClass(fmRule ? fmRule[1] : '') ? [ruleClass(fmRule[1])] : [];
      const fmEyebrow = fm.match(/^[ \t]*eyebrow:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const eyebrowTokens = eyebrowClass(fmEyebrow ? fmEyebrow[1] : '') ? [eyebrowClass(fmEyebrow[1])] : [];
      ...
      const fmHeadline = fm.match(/^[ \t]*headline:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const headlineTokens = headlineClass(fmHeadline ? fmHeadline[1] : '') ? [headlineClass(fmHeadline[1])] : [];
      // Deck-wide LIFT toggle — opt-in card elevation (resolve-lift.js).
      const fmLift = fm.match(/^[ \t]*lift:[ \t]*["']?([A-Za-z0-9_-]+)["']?[ \t]*$/m);
      const liftTokens = liftClass(fmLift ? fmLift[1] : '') ? [liftClass(fmLift[1])] : [];
      ...
      // `color-mode:` supersedes the legacy `class:` color axis — strip the alias's color
      // tokens the (non-engine) webview already stamped from the deck-wide `class:`, so a
      // half-migrated deck (`color-mode: light` + a leftover `class: dark`) doesn't keep the
      // stale color token. A per-slide `_class:` is not in the deck-wide class:, so it's kept.
      // Mirrors lib/integrations/markdown-it/plugins.js.
      const aliasColorTokens = colorModeToken ? classTokensRaw.filter((t) => colorAxis.has(t)) : [];
      if (!deckTokens.length && !aliasColorTokens.length) return;
      ...
      cachedDeckClassConfig = { deckTokens, modeTokens, aliasColorTokens };
      applyCachedDeckClass();
    });
  }
  ...
  function applyCachedDeckClass() {
    ...
    if (!cfg || typeof document === 'undefined') return;
    const { deckTokens, modeTokens, aliasColorTokens = [] } = cfg;
    ...
    for (const section of document.querySelectorAll('section')) {
      let cur = section.className.split(/\s+/).filter(Boolean);
      if (aliasColorTokens.length) cur = cur.filter((c) => !aliasColorTokens.includes(c));
      let changed = aliasColorTokens.length ? cur.length !== section.className.split(/\s+/).filter(Boolean).length : false;
      ...
      for (const t of deckTokens) {
        ...
        if (slideHasOwnColorMode && colorModeSet.has(t)) continue;
  ...
  function bootstrap() {
    ...
    // bare call here would run BEFORE it and stamp `content` on a slide the deck's
    // own `class: <component>` is about to claim (#1292).
    //
    ...
    startOverflowWatcher({ level: deckOverflowMarker() });
  }
  ...
  //
  // Drawn as a zero-flow JS overlay mirror layer, never a DOM child of the
  // cell: appending even a `position:absolute` child shifts `nth-child`
  // indices for every sibling selector inside it and risks HARD RULE #20
  // (no margin / no layout-shift in a measured layout). Same technique as the
  // debug bounding-box overlay (2026-07-01-debug-bounding-boxes.md §6) —
  // one `position:fixed` container holding boxes positioned from
  // `getBoundingClientRect()`. `position:fixed` is pinned to the VIEWPORT,
  // not the document, so it goes stale on scroll same as any other fixed
  // layer — drawFixMeTags binds a `scroll` listener below (matching the
  // debug overlay's own) for exactly that reason. (An earlier version of
  // this comment claimed same-document execution made scroll-tracking
  // unnecessary — that was wrong, caught in review; same-document only
  // removes the CROSS-IFRAME complexity the debug overlay's OTHER,
  // document-coordinate technique had to solve, not the need to track
  // scroll at all.)
  const FIXME_YELLOW = '#f0e442'; // Okabe-Ito CVD-safe yellow — matches the
  // debug overlay's grid/flex/flow hue family (docs/src/playground/
  // debug-overlay.js), deliberately distinct from the overflow ring's fixed
  // danger red (#d4351c, base.modifiers.css). Not a theme token on purpose —
  // same exception the ring itself takes: an authoring alarm must read
  // identically loud in every palette.
  const FIXME_INK = '#1a1300'; // near-black on the yellow chip, ~15.8:1 contrast.

  function ensureFixMeStyle() {
    if (document.getElementById('lattice-fixme-style')) return;
    const el = document.createElement('style');
    el.id = 'lattice-fixme-style';
    el.textContent =
      '.lattice-fixme-overlay{position:fixed;inset:0;pointer-events:none;z-index:2147483000;}' +
      '.lattice-fixme-box{position:fixed;outline:3px solid ' + FIXME_YELLOW + ';outline-offset:-3px;}' +
      '.lattice-fixme-tab{position:fixed;background:' + FIXME_YELLOW + ';color:' + FIXME_INK + ';' +
      'font-family:var(--font-mono,monospace);font-size:11px;font-weight:700;letter-spacing:0.08em;' +
      'text-transform:uppercase;line-height:1;padding:3px 6px;border-top-left-radius:4px;}';
    (document.head || document.documentElement).appendChild(el);
  }

  let fixMeOverlay = null;
  let lastFixMeTargets = [];
  let fixMeScrollBound = false;
  // The signature drawFixMeTags most recently PAINTED (not just requested) — what
  // the guard below compares a new call against, so an unchanged draw request is a
  // no-op. See the guard's own comment for why this is load-bearing, not cosmetic.
  let lastFixMeSignature = '';
  // A real per-element identity for the signature below — `String(el)` collapses
  // every element of the same tag to the identical `"[object HTMLDivElement]"`, which
  // would make two DIFFERENT culprit elements with the same tag/label/hint/rect
  // indistinguishable (rare, but the comment on the guard below promises identity, so
  // it must actually be one). `WeakMap` keys don't pin the element from GC. No
  // fallback needed: every environment this runtime targets (an evergreen preview
  // webview) has WeakMap.
  let fixMeElSeq = 0;
  const fixMeElIds = new WeakMap();
  function fixMeElId(el) {
    let id = fixMeElIds.get(el);
    if (id === undefined) {
      id = ++fixMeElSeq;
      fixMeElIds.set(el, id);
    }
    return id;
  }
  // Redraws every Fix-Me box + tab from scratch each call — cheap (a handful
  // of cells at most) and avoids tracking per-node state across re-renders,
  // matching the debug overlay's own "no orphaned labels" approach.
  // `targets` is [{el, label, hint?}] — `label` is the tab's short text
  // (Case A: "Fix Me", geometric fact; Case B: "Likely fix", an editorial
  // guess, never the same unhedged string — HARD RULE 23) and `hint`, when
  // present, becomes the tab's native tooltip (`title`) carrying the fuller,
  // still-hedged detail ("Nw, over budget") without widening the chip.
  function drawFixMeTags(targets) {
    if (typeof document === 'undefined') return;
    lastFixMeTargets = targets;
    // `position:fixed` boxes are pinned to the VIEWPORT, not the document —
    // they go stale the instant the page scrolls without a coincident DOM
    // mutation (the every-preview-surface reality: the srcdoc/webview
    // document itself is the scroller, per the debug overlay's own root-cause
    // notes — 2026-07-01-debug-bounding-boxes.md). Re-run this same function
    // against the LAST known targets on scroll — cheap (just re-reads their
    // rects), and matches the debug overlay's own `scroll` listener
    // (docs/src/playground/debug-overlay.js), which this technique is
    // otherwise a straight port of.
    if (!fixMeScrollBound && typeof window !== 'undefined' && window.addEventListener) {
      fixMeScrollBound = true;
      window.addEventListener('scroll', () => drawFixMeTags(lastFixMeTargets), { passive: true });
    }
    // IDEMPOTENCY GUARD (the fix for a real infinite loop): this function is called
    // from check() below, which is itself scheduled to re-run on EVERY DOM mutation
    // (schedulePostMutation's shared observer). Before this guard, an overflowing
    // slide WITH an identifiable culprit (any Case A clip-cell, or a Case B density
    // outlier) redrew unconditionally on every call — clearing + re-appending
    // `fixMeOverlay`'s children, itself a childList mutation the SAME observer is
    // watching → re-triggers check() → redraws again → forever. Measured on a real
    // overflowing slide: a steady 60fps DOM-churn loop (destroy+rebuild every single
    // animation frame, confirmed by object-identity tracking), for as long as the
    // slide stays in view. Every sibling write in this file (patchSectionGeometry's
    // `--_sec-1cqi`, the `.overflow` class toggle just above) is already explicitly
    // change-gated for exactly this reason — this was the one omission. Only redraw
    // when the PAINTED signature would actually differ: same target elements (a real
    // per-element id, not `String(el)` — every element of the same tag stringifies to
    // the identical "[object HTMLDivElement]", which would make two DIFFERENT culprits
    // with the same tag/label/hint/rect indistinguishable), same label/hint, same
    // ROUNDED rect (sub-pixel float jitter between otherwise-identical measurements
    // must not itself be treated as a change, or this guard would just relocate the
    // oscillation one level up). The tab is positioned from `window.innerWidth`/
    // `innerHeight` too (below), NOT just the target's rect — a resize that changes the
    // viewport while a top-anchored target's rect stays put must still redraw, or the
    // tab's `bottom`/`right` (computed from the OLD viewport) drifts away from the box
    // it labels. Both are read once per call so every target shares one measurement.
    ...
    const sig =
      ...
      targets
        .map((t) => {
          ...
          return [fixMeElId(t.el), t.label || 'Fix Me', t.hint || '', Math.round(r.top), Math.round(r.left), Math.round(r.width), Math.round(r.height)].join('|');
        })
        .join(';;');
    if (sig === lastFixMeSignature) return;
    lastFixMeSignature = sig;
    if (!targets.length) {
      if (fixMeOverlay) fixMeOverlay.textContent = '';
      return;
    }
    ensureFixMeStyle();
    if (!fixMeOverlay) {
      fixMeOverlay = document.createElement('div');
      fixMeOverlay.className = 'lattice-fixme-overlay';
      (document.body || document.documentElement).appendChild(fixMeOverlay);
    }
    fixMeOverlay.textContent = '';
    for (const target of targets) {
      const cell = target.el;
      const r = cell.getBoundingClientRect();
      if (!r.width || !r.height) continue;
      const box = document.createElement('div');
      box.className = 'lattice-fixme-box';
      box.style.top = r.top + 'px';
      box.style.left = r.left + 'px';
      box.style.width = r.width + 'px';
      box.style.height = r.height + 'px';
      fixMeOverlay.appendChild(box);
      // Bottom-right corner, not top-right: the SECTION's own "Overflows" tab
      // already claims the section's top-right, and a clip cell's top edge
      // often sits close to it (no masthead above), which visually collided
      // the two tags into an unreadable stack.
      const tab = document.createElement('div');
      tab.className = 'lattice-fixme-tab';
      tab.textContent = target.label || 'Fix Me';
      if (target.hint) tab.title = target.hint;
      tab.style.bottom = (window.innerHeight - r.bottom) + 'px';
      tab.style.right = (window.innerWidth - r.right) + 'px';
      fixMeOverlay.appendChild(tab);
    }
  }
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    // the exact pages.
    const check = () => {
      // Fix-Me targets accumulate across every slide on the page (the
      // filmstrip previewers render many <section>s at once), then draw in
      // one pass at the end — mirrors how `check()` already scans the whole
      // document per tick.
      const fixMeTargets = [];
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        if (s.getAttribute(MARKER_ATTR) !== level) s.setAttribute(MARKER_ATTR, level);
        const { over, overCells, squeezed } = probeSectionOverflow(s, CLIP_CELL_SELECTOR, TOL);
        ...
        // `.overflow-silent` in base.modifiers.css.
        ...
        const tell = over && (policy.authorTags
          || squeezed > TOL
          || probeContentClipped(s, CLIP_CELL_SELECTOR, TOL).cut);
        if (s.classList.contains('overflow-silent') !== (over && !tell)) {
        ...
        // honest marker, never a silent clip. Presence-guarded so it still
        // settles (add once / remove once; the tab is position:absolute → it
        // never changes the measured height, so no measure→mutate→measure loop).
        let tab = s.querySelector(':scope > .overflow-tab');
        const tabAction = overflowTabAction({ over: tell, hasTab: !!tab });
        if (tabAction === 'add') {
          tab = document.createElement('div');
          tab.className = 'overflow-tab';
          // Author preview names the defect ("Overflows"); the reader gets a
          // calm "content continues" cue instead of a QA banner (both are text —
          // WCAG 1.4.1). Styling: the loud red is base.modifiers.css; the reader
          // restyle is base.modifiers.css.
          tab.textContent = policy.tabText;
          s.appendChild(tab);
        } else if (tabAction === 'remove') {
          tab.remove();
        } else if (tab && tab.textContent !== policy.tabText) {
          // A tab ANOTHER producer already drew, whose wording belongs to a
          // different level. lattice-emulator.js's inline watcher runs first in a
          // `--fluid` export and writes its own text; this watcher then stamps the
          // resolved level for the CSS but used to leave the text alone, so
          // `--overflow-marker=author` shipped a calm reader pill reading
          // "Overflows" — author wording in reader styling, the hybrid the level
          // normalization exists to prevent. Guarded on inequality, so it settles.
          tab.textContent = policy.tabText;
        }
        ...
        let legTab = s.querySelector(':scope > .illegible-tab');
        const legAction = legibilityTabAction({ under, hasTab: !!legTab });
        if (legAction === 'add') {
          legTab = document.createElement('div');
          legTab.className = 'illegible-tab';
          legTab.textContent = legibilityTabText(leg);
          s.appendChild(legTab);
        ...
        if (authorTags && over && overCells?.length) {
          ...
          for (const oc of overCells) {
            ...
            if (culprits.length) fixMeTargets.push(...culprits.map((el) => ({ el, label: 'Fix Me' })));
            else fixMeTargets.push({ el: cellEl, label: 'Fix Me' });
          }
          ...
          if (outlier) {
            fixMeTargets.push({ el: outlier.el, label: 'Likely fix', hint: `Likely cause — ${outlier.words} words, over budget` });
          }
        }
      }
      if (authorTags) drawFixMeTags(fixMeTargets);
    };
    check();
    // The immediate check() above can measure a not-yet-rendered slide's
    // text against FALLBACK font metrics — the browser lazy-loads a
    // @font-face only when text using it is first painted, so
    // document.fonts.ready can resolve before every font this document
    // will ever need has actually loaded (mirrors the identical race
    // lattice-emulator.js's embedded export watcher had, issue #894). Every
    // later edit already re-triggers check() via schedulePostMutation, so
    // this mostly self-corrects — but a slide nobody touches again after
    // initial paint could keep a false ring forever without this. Cheap
    // and idempotent (check() only mutates a class on an actual state
    // flip), so a redundant extra pass costs nothing.
    if (typeof document.fonts !== 'undefined') {
      try { settleFonts(document.fonts, 2000).then(check, check); }
      catch (_e) { /* fonts API present but unusable — the immediate check() above stands */ }
    }
