...
(() => {
   ...
   *
   * Called from `initAndRun`, which has already established that `window.mermaid` is
   * real — so this cannot resurrect a fence into a document that still cannot draw it.
   * Without it, a slow script that lands after the deadline would leave the author
   * looking at source for the rest of the session on a host that does not replace its
   * section DOM (the give-up is one-shot: `tick` has returned by then).
   */
  function reclaimReleasedFences() {
  ...
  function initAndRun({ force = false } = {}) {
    ...
    // Mermaid is real HERE, whatever the bootstrap deadline concluded earlier. Anything
    // that deadline handed back to the author goes back in the queue before the walk.
...
function transformInlinePills() {
  ...
  function bootstrap() {
    ...
    const giveUp = (why) => {
      ...
      // AND DROP THE DOCUMENT'S PROMISE, because it has just been falsified.
      // `[data-lattice-diagrams]` is what `mermaid.css` withholds an UNTAGGED fence's ink
      // on — a host earns that rule by committing to inject a Mermaid renderer. Leaving it
      // standing after we have concluded no renderer is coming makes every fence the
      // author types AFTER this point invisible rather than merely un-drawn: the same harm
      // this whole function exists to end, arriving one step later. Only the three watched
      // preview builders stamp it, so this is a no-op on every export document.
      if (typeof document !== 'undefined') {
        document.documentElement?.removeAttribute('data-lattice-diagrams');
      }
     ...
     * `defer` — and writes it BEFORE the runtime's own tag (deck-preview.js buildSrcdoc,
     * single-slide-render.ts, stage-window.js all assemble the pair in that order). A
     * classic parser-inserted script that is already in the DOM when this code runs has
     * therefore already had its turn: it executed, or it failed. So if one is there and
     * `window.mermaid` is still not real, waiting is not going to change the answer.
     *
     * That distinction is worth the twelve lines because of WHERE it is read. The Studio's
     * desktop print document waits `load` + 450ms and nothing else — no diagram wait at
     * all — so a ten-second deadline is far past the moment `print()` captures the page,
     * and the fence would still be hidden. This arm fires on the first tick instead, which
     * is before `load`, so the print document carries the author's source. It also spares
     * the webpage export's 12s bake budget the full wait.
     *
     * Two deliberate narrowings, both of which fall back to the deadline rather than
     * guessing:
     *   · a document with NO mermaid tag never promised a renderer, and may still install
     *     one by a route we cannot see (VS Code's plain markdown preview and its stub);
     *   · an `async` tag, or a `defer` one while the parser is still running, has NOT had
     *     its turn yet — so it is not settled and this stays silent.
     */
    const mermaidScripts = typeof document !== 'undefined' && document.querySelectorAll
      ? [...document.querySelectorAll('script[src]')].filter((el) => /mermaid/i.test(el.src || el.getAttribute('src') || ''))
      : [];
    const hasHadItsTurn = (el) => !el.async && (!el.defer || document.readyState !== 'loading');
