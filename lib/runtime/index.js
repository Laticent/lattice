...
const { measureTrim, planTrim, applyTrim, clearTrim } = require('../../lib/core/guards-trim');
    ...
    const check = (sections) => {
      ...
      for (const s of sections) {
        ...
        if (guardsEnabled(s.className)) {
          clearTrim(s);
          const trimModel = measureTrim(s, CLIP_CELL_SELECTOR, TOL);
          if (trimModel.boxes.length) {
            ...
            if (plan.actions.length) {
              ...
              // exists to prevent.
              if (measureTrim(s, CLIP_CELL_SELECTOR, TOL).boxes.length) clearTrim(s);
            }
          }
        }
