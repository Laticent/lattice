...
const { fitScaleStep } = require('../../lib/core/scale-fit');
const {
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    const check = (sections) => {
      ...
      for (const s of sections) {
        ...
        // STEP (`scale-l/xl/2xl`) runs FIRST — before TRIM, because it loses no content —
        // and before the probe, so every channel below measures the slide at the size it
        // actually renders. It undoes its own previous step on entry, so an edit that
        // shortens a slide gets the full scale back on the next sweep; below scale 1.01 it
        // reads one computed value and writes nothing (lib/core/scale-fit.js rules 1, 4).
        // A trim from the PREVIOUS pass is cleared first, or the step would measure a
        // slide the old clamp had already shortened and keep a rung it cannot hold.
