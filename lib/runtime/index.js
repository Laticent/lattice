...
const {
  AUTHORING_DEFAULT_MARKER, EXPORT_DEFAULT_MARKER, overflowMarkerFrom, resolveOverflowMarker,
...
(() => {
   ...
   * written by exactly one function (`frontMatterBlock` in lib/core/deck-front-matter.js),
  ...
  function boot() {
    ...
    if (document.documentElement.hasAttribute("data-lattice-fluid-capable")) {
      ...
      // export bakes no front-matter block (only the Marp producers do), so
      // `bakedFrontMatter` is always null here and the deck's own key is not
      // reachable. An earlier comment claimed an author's `off` was honored on this
      // path; it is not. The priming call stays because it costs nothing and is
      // correct the day a block does appear, but it is a no-op today — do not read
      // it as the register working here.
