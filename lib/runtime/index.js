...
const { withEngineInit, engineInitConfig } = require('../../lib/integrations/mermaid/init-directive');
...
(() => {
  ...
  function initAndRun({ force = false } = {}) {
    ...
    for (const preEl of document.querySelectorAll(FENCE_SELECTOR)) {
      ...
      // (#1311). It also makes the cache key below palette-aware: a live palette
      // swap now re-renders the diagram instead of serving the old bake.
      const source = withEngineInit(
        reorientMermaidForPortrait((codeEl.textContent || "").trim(), orientation),
        engineInitFor(),
      );
