 ...
 * it is null. It is the anchor for the ONE question the Mermaid give-up asks synchronously:
...
// `globalThis.__latticeDagre` — it is serialised through `.toString()`, so it
...
// A state chart whose engine did not arrive falls back to the NUMBERED COLUMN —
...
//
// Keyed on `[data-sc-transitions]`, which only the DEFAULT variant emits — the
...
function warnMissingDagre() {
  ...
  try {
    if (!document.querySelector('.state-chart-figure[data-sc-transitions]')) return;
    dagreWarned = true;
    console.warn(
      '[lattice] state chart present but lattice-dagre.min.js did not load — a machine '
      + 'that branches will fall back to the numbered column. Load it with a <script src> '
      + 'before lattice-runtime.min.js.');
...
const { guardsClass, isGuardsToken, guardsEnabled } = require('../../lib/core/resolve-guards');
const { measureTrim, planTrim, applyTrim, clearTrim, clearTrimBoxes, FIT_EPSILON: TRIM_FIT_EPSILON } = require('../../lib/core/guards-trim');
...
const { isLiteralElement: isLiteralInlineCode, inlineCodeClass } = require('../../lib/core/resolve-inline-code');
...
const { textValueTokens } = require('../../lib/core/mermaid-theme-map');
...
(() => {
  ...
  // …and the frame-level floor the diagram pass actually waits, which is a different
  // question from what the 150ms above is for.
  //
  // WHAT THE FIXED TIMER COST. Measured on the built Studio, a single keystroke inside a
  // fence took 209ms end to end: 150ms of it was this timer, ~22ms was `mermaid.render`
  // for a four-node flowchart, and the rest was the engine, the sanitize and the patch. A
  // timer larger than the work it defers is not coalescing, it is waiting — and it was
  // larger than a FULL RENDER for every diagram up to about 64 nodes (22ms at 4, 43ms at
  // 16, 72ms at 32, 130ms at 64, 239ms at 128).
  //
  // WHAT REPLACES IT. Coalesce on COMPLETION rather than on a clock: at most one diagram
  // run in flight, and if the source moved while it ran, run again with the latest text.
  // That is strictly better than any constant, because it adapts to the diagram instead of
  // guessing at it — a small graph streams at render speed, and a 128-node one throttles
  // itself to one render per render rather than queueing eight behind a burst.
  //
  // The floor is one frame, not zero. A single host write produces several mutation
  // records, and the runtime's own transforms re-enter the observer; 16ms collapses those
  // into one pass and costs a frame nobody can see. Zero worked in the experiment but
  // leaves no margin for a burst of records arriving across two ticks.
  const COALESCE_MS = 16;
  /**
   * A monotonic-ish clock. `performance.now()` where it exists (monotonic, immune to a
   * wall-clock jump); `Date.now()` otherwise. Module-scope because two things need it now —
   * the settle backstop far below and the render-cost record just under this.
   */
  const nowMs = () => (typeof performance !== 'undefined' && typeof performance.now === 'function'
    ? performance.now()
    : Date.now());
  /** Ceiling on how long the parse gate may wait on Mermaid before standing aside. */
  const PARSE_CAP_MS = 250;
  // ── BEGIN BACK-OFF PORT — lifted verbatim by test/unit/runtime/diagram-backoff.test.js.
  /**
   * WAIT FOR THE SOURCE TO SETTLE, OR DRAW NOW? Two answers, not a curve.
   *
   * A CURVE WAS TRIED AND IT WAS NEVER OPERATIVE. The wait used to be twice the median
   * render, capped at 150 and floored at 50 — which returns anything other than 0 or 150
   * only while a render costs between 50 and 75ms. Against the measured size table (22ms at
   * 4 nodes, 43 at 16, 72 at 32, 130 at 64, 239 at 128) that band is diagrams of roughly 20
   * to 33 nodes. Everywhere else it was already a step function, and the arithmetic dressed
   * it up as adaptive. Worse, the published headline — a keystroke 194ms -> 78ms — was
   * measured on the bench's FIVE-NODE deck, where the wait is 0 and none of the machinery
   * runs at all: that number is the deleted debounce and nothing else.
   *
   * So this is the honest shape of what was actually bought:
   *
   *   cheap render  -> draw on every keystroke. This is the win, it is large, and it comes
   *                    from deleting a constant that was bigger than the work it deferred.
   *   costly render -> wait, using the same 150ms this work removed.
   *
   * The second arm is deliberately byte-for-byte the old behavior. A large diagram CANNOT
   * regress, because there is nothing left to regress against — and four rewrites of this
   * file were spent discovering that every cleverer answer regressed it in some cadence,
   * on some host, or with some second fence on the slide.
   */
  const renderCosts = [];
  const RENDER_COST_SAMPLES = 3;
  /**
   * The median of the last few renders, taking the LOWER of two.
   *
   * One sample is not a cost. The first `mermaid.render` of a session also pays Mermaid's
   * initialization — a 4-node fence that costs ~36ms every other time measured 210ms once —
   * and reading only the latest number let that one figure decide the policy for the whole
   * burst that followed.
   */
  function medianRenderCostMs() {
    if (!renderCosts.length) return 0;
    const sorted = [...renderCosts].sort((a, b) => a - b);
    return sorted[Math.floor((sorted.length - 1) / 2)];
  }
  /**
   * THE COLD FIRST RENDER IS KEPT, and that is a decision with a measurement behind it.
   *
   * It pays Mermaid's one-time initialization as well as the diagram — a 4-node fence that
   * costs ~36ms every other time measured 210ms once — so it overstates what the diagram
   * costs, and discarding it was tried. It makes `nav` cold ~126ms instead of ~225ms, and it
   * costs more than it buys: with no sample on record the first EDIT render is also
   * unthrottled, so an extra ~370ms render lands in the middle of the first burst on an
   * expensive diagram and the typing wall goes from ~1145ms to ~1515ms, against the old
   * build's ~1157ms. A once-per-visit gain on arrival, paid for by a once-per-visit
   * regression while typing — and only one of those two is worse than the build we replace.
   *
   * The median below takes the LOWER of two samples, which is what keeps one cold figure
   * from deciding the policy for long.
   */
  function recordRenderCost(ms) {
    renderCosts.push(ms);
    if (renderCosts.length > RENDER_COST_SAMPLES) renderCosts.shift();
  }

  /** Renders at or under this are free to repeat on every keystroke. */
  const CHEAP_RENDER_MS = 50;
  /**
   * IS THIS DIAGRAM CHEAP ENOUGH TO REDRAW ON EVERY KEYSTROKE?
   *
   * One question, asked in one place, because THREE things hang off it and an earlier
   * revision let them disagree. It decided the dispatch wait and nothing else, while the
   * parse gate and a redraw ceiling stayed on for every diagram whatever it cost. So the
   * "costly" arm was never the old behavior it claimed to be: it was the old behavior PLUS a
   * parse (45-61ms on a 64-node fence) PLUS two extra blocking renders the ceiling forced
   * mid-burst.
   *
   * Measured on the built Studio against `main`, one 24-character burst into a 64-node
   * fence, n=3 each, only `lattice-runtime.js` swapped between runs:
   *
   *   redraw after the last keystroke   590-605ms   vs main's 464-489ms
   *   main thread busy during the burst 30%         vs main's 10%
   *   renders during the burst          3           vs main's 1
   *   the harness's own fixed-delay typing took 4119-4188ms vs main's 3331-3341ms
   *
   * That last row is the one that settles it: the injected keystrokes were themselves
   * queued behind our renders. A change that makes TYPING slower is the worst thing this
   * file can do, and no redraw win pays for it.
   *
   * So the answer gates everything now, and the costly arm is the old behavior exactly — a
   * 150ms trailing debounce, nothing in front of it, no ceiling. It cannot regress because
   * there is nothing left on it to regress with.
   */
  function diagramIsCheap() {
    return medianRenderCostMs() <= CHEAP_RENDER_MS;
  }
  /**
   * THE ONE TIMER, sized to the diagram — read by `scheduleRun`, the only place a pass is
   * scheduled from.
   *
   * A cheap diagram gets the frame-level floor and redraws live; a costly one gets the 150ms
   * this work set out to delete, which is exactly the shape of the build it replaces.
   *
   * IT USED TO BE TWO TIMERS AND THEY COULD NOT BE MADE TO SUM. The dispatch carried its own
   * trailing back-off on top of this floor. Charging both put a 4-node fence at 152-171ms
   * against that build's 102-139ms; netting the floor out of the second timer fixed that and
   * dropped the effective wait to 150ms from the last KEYSTROKE, against a ~120ms typing
   * cadence — where a costly render supplies exactly the jitter that closes the margin, taking
   * a 64-node burst to 2 renders in 2 of 3 runs, 18-19% main thread against 10-11%, and
   * stretching the harness's own typing from 3331 to 3767ms. One timer has no margin to lose.
   * The coalescing the second timer was really for is `diagramRuns`, which is not a clock.
   */
  function contentFloorMs(liveWorthHaving = true) {
    return diagramIsCheap() && liveWorthHaving ? COALESCE_MS : DEBOUNCE_MS;
  }

  /**
   * Scopes in which a diagram has ever successfully DRAWN.
   *
   * WHAT THIS IS FOR. A live redraw keeps an existing picture up to date as the author types.
   * A fence that has never once rendered has no picture, so there is nothing to keep live —
   * and the frame-level floor plus the parse gate buy it exactly nothing while costing a
   * parse per keystroke. Measured on a 64-node fence with a half-written tail, 24 chars at
   * 120ms: 25-27% of the main thread against the old build's 1%, for 115-122 frames of raw
   * source against its 117-122. Identical output, 800ms more work per burst.
   *
   * WHY OUR OWN HISTORY AND NOT THE DOM. The obvious question is "does this fence's slot hold
   * an SVG", and it is the wrong one twice over. It is HOST-DEPENDENT: a drawing survives an
   * in-place edit only where the host stamps `data-lattice-swap` (the Studio and the
   * Playground), so on marp-vscode and every embedder each keystroke would look inkless and
   * the live redraw would silently never engage. And asked document-wide it is the defect
   * that was already reverted once here, where one fence in `error` — which clears its slot —
   * turned the policy off for every other fence on the slide. Asking what WE have drawn needs
   * no host cooperation and behaves identically wherever the runtime ships.
   *
   * KEYED ON SCOPE, not on the fence, because a fence has no identity across keystrokes: its
   * text is what changes. Scope is coarser than the question strictly wants — a second fence
   * in a scope that has already drawn counts as live-worthy — and that is the safe direction,
   * because it can only leave the current behavior in place, never take it away.
   */
  const drawnScopes = new Set();
  function noteScopeDrew(scopeKey) {
    if (scopeKey) drawnScopes.add(scopeKey);
  }
  function scopeHasDrawn(scopeKey) {
    return drawnScopes.has(scopeKey);
  }

  /**
   * Does the parse gate run for this fence on this pass?
   *
   * NAMED, rather than written inline at the one call site, because it is a POLICY decision and
   * the policy is what gets lifted and reasoned about. Left inline, the scheduling model in
   * `test/unit/runtime/fixtures/diagram-schedule-model.js` had to restate the condition — and a
   * restatement is not a test of anything: putting the parse back on the costly arm, which is
   * one of the three regressions this branch shipped and retracted, left all fourteen
   * metamorphic relations green. Both sides read this function now, so the same edit moves the
   * model and the relations catch it.
   *
   * The two halves are separate defects. `diagramIsCheap` keeps a 45-61ms parse off a large
   * diagram's critical path; `scopeHasDrawn` keeps it off a fence with no picture to protect,
   * where it measured ~800ms per burst for byte-identical output.
   */
  function parseGateApplies(scopeKey) {
    return diagramIsCheap() && scopeHasDrawn(scopeKey);
  }
  // ── END BACK-OFF PORT
  /**
   * How many diagram runs are queued and not yet settled.
   *
   * A COUNTER, not a boolean: `renderDiagramsIn` opens one run per consecutive scope-key
   * change, so a deck alternating palettes opens four in a single pass. As a boolean the
   * FIRST tail cleared it while three runs were still queued, silently weakening the
   * coalescing it exists to provide.
   */
  let diagramRuns = 0;
  /** The source moved while a run was in flight — start another as soon as it lands. */
  let rerunRequested = false;
/**
   * `initAndRun` handed this pass to machinery that will re-run it — today that is a diagram
   * run already in flight, and nothing else: the back-off timer this also used to name was
   * deleted when the wait moved to `scheduleRun`'s single floor. NOT an init failure, and
   * telling the two apart
   * is the point: both used to return the same `false` that means "theme vars have not
   * resolved", so `tick` burned its 30-frame theme budget on them and then logged
   * "theme vars never resolved; force-init" and called back with `force: true` — which hit
   * the same early return and was swallowed, after which `tick` never retried. A spurious
   * warning, and a `force` escape hatch consumed by a condition it was not written for.
   */
  const RUN_DEFERRED = 'deferred';
  ...
  function scheduleRun() {
    // NOTHING ABOUT A DIAGRAM GATES THIS FUNCTION, and an earlier version of the coalescing
    // got that wrong in a way worth naming. It early-returned while a diagram run was in
    // flight — but this callback is not diagram-only: it runs `runAllContentTransforms`
    // and then every `contentSettledListeners` entry, so the Form composition, the masthead,
    // the charts, the fit berth and section numbering all stopped updating too. A
    // `mermaid.render` that STALLS rather than rejects (the case RENDER_SETTLE_CAP_MS
    // exists for) froze the whole preview for the 20s cap. Coalescing belongs where the
    // renders are dispatched, not in front of the content pass — see `initAndRun`.
    ...
    scheduledRunHandle = setTimeout(() => {
      ...
      // ONE FLOOR, SIZED TO THE DIAGRAM, AND IT IS THE WHOLE POLICY. A cheap diagram gets the
      // frame-level floor so it redraws live; a costly one gets the 150ms this work set out to
      // delete, which puts it on exactly the old build's single-timer shape.
      //
      // THIS REPLACED A SECOND TIMER, and the reason is a measurement. The dispatch used to
      // carry its own trailing back-off on top of this floor, and two timers in series cannot
      // be made to add up: charging both put a small diagram at 152-171ms against the old
      // build's 102-139ms, and netting the floor out of the second one fixed that but dropped
      // the effective wait to 150ms from the last KEYSTROKE against a ~120ms typing cadence —
      // where a costly render supplies exactly the jitter that closes the margin. A 64-node
      // fence then took 2 renders in 2 of 3 runs, 18-19% main thread against 10-11%, and the
      // harness's own typing stretched 3331 -> 3767ms. One timer has no margin to lose.
      // NO FENCE AT ALL IS "WORTH HAVING", not "not worth having". A deck with no diagram in
      // it must keep the floor it has: this timer runs the whole content pass, and answering
      // false here would push every prose and chart deck in the product back to the 150ms
      // debounce for a reason that has nothing to do with them.
    }, contentFloorMs(anyPendingFenceIsLiveWorthy()));
  }

  /**
   * Is there anything on this document a live redraw could keep up to date?
   *
   * True when there is no pending fence at all (nothing to slow down for), or when at least
   * one pending fence sits in a scope that has already drawn. False only in the case this
   * exists for: fences are waiting and not one of them has ever produced a picture, so the
   * frame-level floor would buy a parse per keystroke and nothing else.
   */
  function anyPendingFenceIsLiveWorthy() {
    if (typeof document === 'undefined') return true;
    const waiting = document.querySelectorAll(`${PENDING_FENCE_SELECTOR},${UNTAGGED_FENCE_SELECTOR}`);
    if (!waiting.length) return true;
    for (const el of waiting) {
      const sectionEl = el.closest?.('section');
      if (sectionEl && scopeHasDrawn(diagramScopeKey(sectionEl))) return true;
    }
    return false;
  }
  ...
  function wrapFences() {
    ...
    //   deferred          → the PREVIOUS svg stays up; the source does not parse yet
    //                       and the author is still typing (see the parse gate)
  ...
  const FENCE_CODE_SELECTOR =
    'pre > code[class*="language-mermaid"],marp-pre > code[class*="language-mermaid"]';
  ...
  const UNTAGGED_FENCE_SELECTOR =
    ':is(pre, marp-pre):not([data-mermaid-state]) > code[class*="language-mermaid"]';
  ...
  const PENDING_FENCE_SELECTOR =
    'pre[data-mermaid-state="pending"],marp-pre[data-mermaid-state="pending"]';
  ...
  const RELEASED_FENCE_SELECTOR =
    'pre[data-mermaid-state="unavailable"],marp-pre[data-mermaid-state="unavailable"]';
   ...
   * three ways:
   *   · `[data-lattice-diagrams] …:not([data-mermaid-state])` withholds an UNTAGGED
  ...
  function releaseUnrenderableFences() {
    ...
    for (const preEl of document.querySelectorAll(PENDING_FENCE_SELECTOR)) {
      preEl.dataset.mermaidState = 'unavailable';
      released++;
    }
    return released;
  }
  ...
  function resetFenceAfterFailure(preEl) {
    if (preEl.dataset.mermaidState !== 'rendering') return;
    ...
    preEl.dataset.mermaidState = reclaimed?.has(preEl) ? 'unavailable' : 'pending';
  }
  ...
  function settleFenceFromCache(job) {
    ...
    // CONSUME ANY QUIET-TIMER RELEASE. `armErrorSurface` marks a released fence for a
    // forced, un-gated render; this path settles it from cache instead and never reaches
    // `renderDiagramJob`, so without this the entry outlives its errand on that node. If
    // the node is later given an unparseable source it would bypass the gate once and buy
    // a doomed render plus an error-box flash mid-word — the exact strobe the gate exists
    // to stop. A cache hit also PROVES the force is moot: this source has rendered before.
    forceRender.delete(fenceSourceOf(preEl));
    return true;
  }
  ...
  function adoptWithinNode(outgoing, incoming) {
    ...
    for (let i = 0; i < arrivals.length; i++) {
      ...
      if (preEl.dataset.mermaidState !== 'pending') continue;
      const target = preEl.nextElementSibling;
  ...
  function replayCachedFences() {
    ...
    // the source permanently. A host can load this runtime without Mermaid — a caller
    // that passes no `mermaidUrl`, or VS Code's plain markdown preview, whose
    // bierner.markdown-mermaid was believed to expose a render-blocks-only shim with no
    // `.render` — see the note at `isRealMermaid` in bootstrap; at 1.32.1 it installs no
  ...
  function initAndRun({ force = false } = {}) {
    // The transforms run FIRST and unconditionally — they are what keeps the rest of the
    // slide alive while a diagram is drawing, and they are idempotent. There is one entry
    // point again, so there is nothing left to make this conditional for.
    runAllContentTransforms();
    // THE COALESCING POINT. A diagram run is already in flight, so dispatching another
    // would queue a render per keystroke on a strictly serial queue and make the author
    // wait for every one their typing superseded. Record that the source moved and let the
    // run's completion hook start the next pass with whatever the text says by then.
    if (diagramRuns > 0) {
      rerunRequested = true;
      return RUN_DEFERRED;
    }
    ...
    // something else on that global (see the note at `isRealMermaid` in bootstrap for what
    ...
    // for handled fences because we filter on data-mermaid-state — and the
  ...
  function beginDiagramRun(mermaid, themeVars, look) {
    ...
    diagramQueue = diagramQueue
      ...
      // NEVER let the tail poison the counter. `diagramRuns` gates every later DISPATCH, so
      // a throw between here and the hook below would stop the document drawing diagrams for
      // the rest of the session — the failure mode is total and silent.
      .catch((err) => {
        if (typeof console !== 'undefined') console.warn('[lattice-runtime] diagram run tail failed', err);
      })
      // THE COALESCING POINT. Everything this run was going to draw has drawn, so the next
      // pass can start — and it starts from the DOM as it is now, not from the source that
      // was current when this run began. A burst therefore costs one render per render
      // rather than one per keystroke, and the last keystroke is never waiting behind a
      // render it already superseded.
      .then(() => {
        diagramRuns = Math.max(0, diagramRuns - 1);
        if (diagramRuns > 0 || !rerunRequested) return;
  ...
  // ── BEGIN PARSE-GATE PORT — lifted verbatim by test/unit/runtime/diagram-parse-gate.test.js.
  //
  // THE BRACKET OPENS HERE, ABOVE `erroredSources`, AND THAT IS A FIX RATHER THAN A TIDY.
  // It used to open below `attachErrorSafely`, so the test injected a STUB for it — and the
  // stub re-implemented the `remember` guard instead of exercising it. An independent pass
  // then showed that removing the real guard entirely (recording every failure again, the
  // permanent-error defect) left all 274 cells green: the suite pinned which value was PASSED
  // and never what the function DID with it. A guard that only a stub enforces is not gated.
  /** `attachError` must not be able to reject a job — see the allSettled note above. */
  /**
   * The failure for a source, remembered the way `mermaidSvgCache` remembers a success.
   *
   * A fence whose text has already failed does not need to be drawn again to fail again. Both
   * preview hosts rebuild the DOM on every keystroke, so without this the error box is
   * destroyed and re-derived constantly — and the two ways to re-derive it are both bad: let
   * the parse gate defer it, and the box blinks off for a whole quiet window on every
   * keystroke in a SIBLING fence (measured on the built Studio, visible in 6 of 24 samples);
   * or force a real render every pass, and a doomed diagram is drawn over and over (4 renders
   * against the old build's 2 on an 8-character burst). Replaying the remembered failure
   * costs nothing and keeps the box steady.
   *
   * Keyed on the fence TEXT, so editing the fence retires the entry and the gate applies
   * again; swept with `deferredSince` when no fence carries that text any more.
   */
  const erroredSources = new Map();

  /**
   * @param remember  Only true where the SOURCE has already been proved at fault.
   *
   * WHAT WENT WRONG WHEN EVERY CALLER REMEMBERED. This used to record unconditionally, so a
   * failure that had nothing to do with the fence text was replayed forever: the 20-second
   * settle cap firing on a stalled dynamic import or icon-pack fetch, or any `mermaid.render`
   * rejection. The old build retags the fence `pending` on the next host rebuild and renders
   * it again, and it succeeds; this one showed the error box for the life of the document.
   * Editing the fence is the only escape, and Present, a read-only embed and an export
   * capture frame do not offer one.
   *
   * The one caller that may remember is the forced render below — that fence reached it by
   * failing `mermaid.parse` and then sitting out a full quiet window, so its text is known
   * bad and drawing it again can only fail again in the same way.
   */
  function attachErrorSafely(preEl, target, err, remember = false) {
    try {
      // NEVER key on an empty source. `fenceSourceOf` falls back to '' when it cannot read the
      // fence text, and one shared '' key makes every unreadable fence inherit an unrelated
      // fence's failure — a diagram that renders perfectly well would show someone else's
      // error box and never be drawn.
      const errKey = remember ? fenceSourceOf(preEl) : '';
      if (errKey) erroredSources.set(errKey, err);
  ...
  /**
   * A source that does not parse yet, while the author is still typing, is not an ERROR —
   * it is a half-written diagram.
   *
   * `attachError` clears the slot and un-hides the `<pre>`, which is right once the author
   * has stopped and wrong while they are mid-word: most of the time spent building a
   * diagram from scratch is spent in a state that does not parse, and surfacing it on every
   * keystroke is what made the zero-timer experiment paint 155 frames of raw source where
   * the old fixed debounce painted 97.
   *
   * So a parse failure holds the picture the fence already has and arms this timer instead.
   * If the author keeps typing, the next pass re-arms it and nothing flashes. If they stop,
   * it fires and the render runs for real, so a genuinely broken diagram still gets its
   * error box — just after a pause rather than during a word.
   *
   * The quiet window is deliberately longer than the coalescing floor: it is measuring "has
   * this person stopped", not "have the mutation records settled".
   */
  const ERROR_QUIET_MS = 450;
  /** The `deferred` twin of the pending-fence selector the walk uses. */
  /** Every tagged fence, whatever its state — what the document currently holds. */
  const FENCE_TEXT_SELECTOR = 'pre[data-mermaid-state],marp-pre[data-mermaid-state]';
  const DEFERRED_FENCE_SELECTOR = 'pre[data-mermaid-state="deferred"],marp-pre[data-mermaid-state="deferred"]';
  let errorSurfaceTimer = null;
  /**
   * Fences the quiet timer has released: render these for real even though they will fail,
   * so the author gets their error box.
   *
   * A WeakSet, and the deferred set is gone entirely — both for the reason `reclaimed`
   * already documents a few hundred lines up: "the entries are DOM nodes a host may replace
   * wholesale on every keystroke". Strong Sets of `<pre>` elements retained one detached
   * subtree per keystroke of every broken-diagram episode, permanently (measured: 20
   * keystrokes, 20 detached nodes still held after the fence was fixed and re-rendered).
   * WHICH fences are waiting is already written on the fences themselves, so the timer asks
   * the document instead of keeping a list.
   */
  /**
   * Fences the quiet timer has released: render these for real even though they will fail,
   * so the author gets their error box.
   *
   * KEYED ON THE FENCE TEXT, not the element, and that is the whole fix. It was a WeakSet of
   * `<pre>` nodes — and both preview hosts replace that node on every keystroke, so a release
   * survived exactly one pass. The next host write handed the same unchanged text back to
   * `deferUntilQuiet` with no deadline on record, which opened a FRESH 450ms window and hid
   * the error again. Driven on the built Studio with a broken fence beside the one being
   * edited, the error box blinked on and off for the whole burst: 8 of 24 samples showed it,
   * against 24 of 24 on the build before this work. The parse-gate suite asserts the opposite
   * property and passed only because it reuses its `<pre>` objects across simulated
   * keystrokes, which is the one thing neither real host does.
   *
   * A Set of strings rather than a WeakSet of nodes cannot be collected for us, so entries
   * are consumed on use and swept with `deferredSince` — the same pruning, for the same
   * reason, since a burst mints one key per keystroke.
   */
  const forceRender = new Set();
  /**
   * Sources that have failed a FORCED render once. A second failure is what earns remembering.
   *
   * WHY A FAILURE IS NOT SELF-EVIDENTLY THE TEXT'S FAULT. The forced path's premise is that
   * `mermaid.parse` already rejected this source, so drawing it can only fail the same way.
   * `parsesCleanly` cannot support that premise: it ends in `.catch(() => false)`, so a parse
   * that rejects because Mermaid could not LOAD — it awaits dynamic imports, and for
   * architecture/C4 an external icon-pack fetch — is indistinguishable from a syntax error.
   * That fence is then deferred, forced, fails again for the same infrastructure reason, and
   * is remembered for the life of the document: replayed before the cache and before any
   * render, with no way back on Present, a read-only embed or an export capture frame. Which
   * is exactly the defect `remember` was introduced to eliminate, surviving on the one path
   * allowed to remember.
   *
   * TWO STRIKES RATHER THAN ERROR-SHAPE SNIFFING. Telling a jison syntax error from a failed
   * module import by its message is guesswork that breaks whenever Mermaid rewords one. Asking
   * the same question twice is not: a transient failure gets a second chance one quiet window
   * later, and a genuinely broken source pays one extra deferral and one extra render before
   * its box goes stable. Pruned with `forceRender`, which is what it shadows.
   */
  const forcedOnce = new Set();

  /**
   * When each deferred SOURCE first went quiet, so one fence cannot starve another.
   *
   * Keyed by the fence TEXT, not by the element, and that is the whole fix. The timer used
   * to be one document-global handle that every deferral cleared and re-armed, so a fence
   * broken at first paint waited on the author stopping typing ANYWHERE in the deck: with a
   * second fence edited every 150ms, a broken one sat on an empty slot — no drawing, no
   * error box, no source — for as long as the typing went on. Measured at 6.25s, and it is
   * unbounded, where before this diff that fence errored immediately.
   *
   * A per-ELEMENT deadline does not fix it either, and it is worth saying why the obvious
   * key is the wrong one: both preview hosts replace the `<pre>` wholesale on every
   * keystroke, so every fence in the deck gets a fresh node — and would get a fresh
   * deadline — on every keystroke aimed at any of them.
   *
   * The source is what actually says whether THIS fence is still being written. Unchanged
   * text keeps its original deadline across any number of node replacements; edited text is
   * a new key, and correctly starts its own window again.
   */
  const deferredSince = new Map();
  /** The fence text `deferredSince` is keyed by. */
  function fenceSourceOf(preEl) {
    const code = preEl.querySelector ? preEl.querySelector('code') : null;
    return (code ? code.textContent : preEl.textContent) || '';
  }

  /**
   * Release every deferred fence whose OWN quiet window has elapsed, and re-arm for the
   * earliest one that has not.
   *
   * Self-re-arming rather than re-armed by each deferral: a fence still inside its window
   * would otherwise wait for the next deferral to set a timer, which on a deck that has just
   * gone quiet never comes — the deck going quiet is the whole trigger.
   */
  function sweepDeferredFences() {
    errorSurfaceTimer = null;
    if (typeof document === 'undefined') return;
    const waiting = document.querySelectorAll(DEFERRED_FENCE_SELECTOR);
    const at = nowMs();
    let released = 0;
    let soonest = Infinity;
    for (const preEl of waiting) {
      const since = deferredSince.get(fenceSourceOf(preEl));
      // No record means this fence reached `deferred` without passing through
      // `deferUntilQuiet`. Treat it as due: a fence with no deadline must not be stranded.
      const waited = since === undefined ? ERROR_QUIET_MS : at - since;
      if (waited >= ERROR_QUIET_MS) {
        // Guarded for the reason `attachErrorSafely` writes out: '' is the fallback for a
        // fence we cannot read, and one shared key would force a render of every such fence.
        const releaseKey = fenceSourceOf(preEl);
        if (releaseKey) forceRender.add(releaseKey);
        // Back into the walk's selector, now that the author has stopped.
        preEl.dataset.mermaidState = 'pending';
        released++;
      } else {
        soonest = Math.min(soonest, ERROR_QUIET_MS - waited);
      }
    }
    // PRUNE AFTER RELEASING, not before, so a fence released on this very sweep drops its
    // entry now rather than lingering until the next one. Keyed by text, a burst mints a key
    // per character, so without this the map grows for the life of the page.
    const live = new Set();
    for (const preEl of document.querySelectorAll(DEFERRED_FENCE_SELECTOR)) live.add(fenceSourceOf(preEl));
    for (const key of [...deferredSince.keys()]) {
      if (!live.has(key)) deferredSince.delete(key);
    }
    // `forceRender` grows the same way and is pruned the same way — but NOT against the same
    // set. `live` holds the fences still DEFERRED, and a fence this sweep just released is
    // `pending`, so pruning against `live` deleted every entry the release had just created
    // and the forced render never happened. It is pruned against what the document actually
    // holds: an entry for text no fence carries any more is spent.
    if (forceRender.size || erroredSources.size) {
      const present = new Set();
      for (const preEl of document.querySelectorAll(FENCE_TEXT_SELECTOR)) present.add(fenceSourceOf(preEl));
      for (const key of [...forcedOnce]) {
        if (!present.has(key)) forcedOnce.delete(key);
      }
      for (const key of [...forceRender]) {
        if (!present.has(key)) forceRender.delete(key);
      }
      for (const key of [...erroredSources.keys()]) {
        if (!present.has(key)) erroredSources.delete(key);
      }
    }
    if (soonest !== Infinity) errorSurfaceTimer = setTimeout(sweepDeferredFences, soonest);
    if (released) scheduleRun();
  }

  /**
   * Make sure a sweep is pending. NOT re-armed per deferral — each fence's deadline lives in
   * `deferredSince`, so an existing timer is already the right one or will re-arm itself.
   */
  function armErrorSurface() {
    if (errorSurfaceTimer) return;
    errorSurfaceTimer = setTimeout(sweepDeferredFences, ERROR_QUIET_MS);
  }

  /**
   * Does this source parse? `false` means "not yet", not "broken".
   *
   * ASYNC ON PURPOSE. Mermaid v11's `parse` returns a PROMISE, and the first version of
   * this gate answered synchronously — so it saw a thenable, could not decide, and passed
   * everything through. It was inert: the broken-diagram arm still bought eight renders
   * and painted 147 frames of raw source. A gate that cannot fail is not a gate.
   *
   * Parse is far cheaper than render — ~9ms to reject against 22-239ms to draw — so the
   * await pays for itself the first time an author types half a node label.
   * `suppressErrors` returns false instead of throwing; older builds throw, so both count.
   */
  function parsesCleanly(mermaid, source) {
    if (typeof mermaid.parse !== 'function') return Promise.resolve(true); // no gate — behave as before
    const verdict = Promise.resolve()
      .then(() => mermaid.parse(source, { suppressErrors: true }))
      .then((v) => v !== false)
      .catch(() => false);
    // CAPPED, because this awaits a THIRD-PARTY promise. `renderDiagramNow` has carried a
    // settle cap since #1332 for exactly this reason, and a gate in front of it that can
    // hang forever just moves the hazard one call earlier. A parse we cannot get an answer
    // from within the cap resolves TRUE: the render owns failure, and it is itself capped,
    // so the fence settles either way.
    //
    // THE CAP IS CLEARED WHEN THE VERDICT WINS. Left armed, every parse held its closure for
    // a further 250ms — one live timer per keystroke on a burst — and on a deck of many
    // fences that is a steady drip of retained work for a race that is already decided.
    let capTimer = null;
    ...
    return Promise.race([verdict, capped]).then((v) => {
      if (capTimer !== null) {
  ...
  /**
   * Hold the picture and wait for a pause.
   *
   * Handing the fence back to `pending` is not optional bookkeeping: the walk stamps
   * `rendering` BEFORE the queue drains, and `pending` is what the walk looks for. A
   * deferral that left it `rendering` would strand the diagram for the life of the page —
   * including for the quiet timer, which finds fences the same way. The target is left
   * exactly as it is, which is the whole point: the last good drawing stays on screen.
   */
  function deferUntilQuiet(preEl) {
    // `deferred`, NOT `pending`, and this is the whole reason the first version of this
    // never surfaced an error. `pending` is what the walk SELECTS, so handing the fence
    // back to it made the next pass take the fence, fail the gate, defer again — and
    // re-arm the quiet timer — about every 16ms. The timer measures "has the author
    // stopped typing", and the deferral itself was what kept it from ever elapsing: an
    // instrumented trace showed `initAndRun -> walk.take -> job.defer` looping forever
    // with the timer never once firing, the fence stuck showing stale ink.
    //
    // A state the walk does not select ends the loop, and it costs nothing visually:
    // CSS hides the `<pre>` for every state except `error`/`unavailable`, and collapses
    // the `.mermaid` slot only for those two — so a `deferred` fence keeps showing the
    // SVG it already has, which is exactly the hold this gate exists to provide.
    preEl.dataset.mermaidState = 'deferred';
    // FIRST deferral of this exact text starts its window; later passes over unchanged text
    // must not push the deadline out, or a fence re-deferred by every unrelated keystroke
    // never becomes due. Editing the fence changes the key, which starts a new window.
    const src = fenceSourceOf(preEl);
    // Same guard, same reason: one shared '' deadline would let an unreadable fence carry
    // another fence's quiet window.
    if (src && !deferredSince.has(src)) deferredSince.set(src, nowMs());
    armErrorSurface();
  }

  function renderDiagramJob(mermaid, scopeKey, job) {
    // THE GATE. Hold what is on screen rather than clearing the slot for a source the
    // author is still in the middle of writing — unless the quiet timer has released this
    // fence, in which case they have stopped and are owed the error.
    // NOT `delete` — the release lasts as long as the TEXT does. Consuming it on use meant a
    // fence whose source had already been shown to be broken was re-deferred on the very next
    // keystroke in a SIBLING fence, hiding its error box for another quiet window: on the
    // built Studio the box blinked through 18 of 24 samples, cycling error -> deferred ->
    // pending -> error. Nothing about that fence was mid-word; its text had not changed. The
    // gate exists to protect an author still writing a diagram, and once a given source has
    // failed for real there is nothing left to protect it from. Editing the fence changes the
    // key and the gate applies again immediately; `sweepDeferredFences` prunes the entry when
    // no fence carries that text any more.
    // A SOURCE WE HAVE ALREADY WATCHED FAIL. Replay the failure rather than re-deriving it:
    // no render, no deferral, and the error box is simply there again on the next pass.
    const knownKey = fenceSourceOf(job.preEl);
    const known = knownKey ? erroredSources.get(knownKey) : undefined;
    if (known !== undefined) {
      attachErrorSafely(job.preEl, job.target, known);
      return Promise.resolve();
    }
    // The one path allowed to REMEMBER its failure — but only on the SECOND strike. One
    // failure here means the source did not parse and did not draw, which a broken diagram and
    // an unreachable renderer produce identically; see `forcedOnce` for why that distinction
    // cannot be read off the error.
    const forcedKey = fenceSourceOf(job.preEl);
    if (forcedKey && forceRender.has(forcedKey)) {
      const twice = forcedOnce.has(forcedKey);
      if (!twice) forcedOnce.add(forcedKey);
      return renderDiagramNow(mermaid, scopeKey, job, twice);
    }
    // A DELIBERATE TRADE, MEASURED BOTH WAYS. Gating only fences that currently hold ink
    // reads well — nothing to hold, nothing to gate — and it does fix the one case it was
    // written for: a diagram broken at FIRST PAINT shows its error at once instead of
    // blanking for the quiet window. But it is wrong for the case authors are actually in.
    // Once an error surfaces the slot is empty, so every further keystroke skips the gate
    // and buys a doomed render: `edit-broken` measured 146 frames of raw source and 8
    // renders, against 97 and 1 before any of this work. That is a strobe on the state an
    // author spends most of their time in while building a diagram from scratch.
    //
    // So the gate is unconditional, and the cost is paid where it is cheapest: a fence
    // broken at first paint shows an empty slot for ERROR_QUIET_MS before its error, rather
    // than immediately. Nothing strobes, no doomed render is bought (`edit-broken`: 0
    // source frames, 0 renders), and the delay lands on a diagram the reader was never
    // going to see draw anyway.
    // THE PARSE IS PART OF THE CHEAP ARM, NOT A TOLL ON EVERY DIAGRAM. It costs ~1.8ms on a
    // small fence, which is what makes holding the previous drawing while an author types
    // nearly free there. On a 64-node fence it measured 45-61ms, against an old build that
    // parses not at all, and it lands squarely on the critical path of the redraw the author
    // is waiting for. Charging a costly diagram for it is half of how "cannot be slower than
    // before" became false. So a costly diagram takes the old path exactly: render it, and
    // let it fail if it fails — which is also what it did before any of this work.
    // AND NOTHING TO PROTECT MEANS NOTHING TO GATE, asked per fence rather than per document.
    // The gate holds a fence's EXISTING drawing while its source is mid-word; a scope that has
    // never drawn has no such drawing, so the parse is pure cost. Measured above at 800ms per
    // burst on a 64-node fence for byte-identical output.
    if (!parseGateApplies(scopeKey)) return renderDiagramNow(mermaid, scopeKey, job, false);
    return parsesCleanly(mermaid, job.source).then((ok) => {
      if (!ok) return deferUntilQuiet(job.preEl);
      return renderDiagramNow(mermaid, scopeKey, job, false);
    });
  }

  // ── END PARSE-GATE PORT

  function renderDiagramNow(mermaid, scopeKey, { preEl, target, source }, remember = false) {
    ...
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        if (settled) return;
        // NEVER remembered: a stall is a statement about this moment, not about the text.
      ...
      Promise.resolve()
        ...
        .then((result) => {
          // Record the cost even when the write below is refused: what the NEXT dispatch
          // needs to know is what a render of this diagram costs, and a render that raced
          // the cap cost exactly as much main thread as one that won.
          ...
          if (!svg) {
            attachErrorSafely(preEl, target, new Error("Mermaid produced no SVG"), remember);
            return;
          }
          ...
          preEl.dataset.mermaidState = "rendered";
          // THIS is what makes the scope live-worthy: a picture now exists that a redraw can
          // keep up to date. Before it, the frame-level floor and the parse gate have nothing
          // to work on and are charged to the author for nothing.
          noteScopeDrew(scopeKey);
        ...
        .catch((err) => { if (!settled) attachErrorSafely(preEl, target, err, remember); })
        .then(() => {
          // THE COST IS RECORDED HERE, WHERE BOTH OUTCOMES REACH IT, and that placement is
          // the whole of a measured defect. It used to sit inside the success handler, so a
          // render that REJECTED recorded nothing — and a fence that has never once rendered
          // successfully therefore kept an EMPTY cost record for ever. An empty record reads
          // cheap (`medianRenderCostMs` returns 0), so the policy handed the frame-level
          // floor AND the parse gate to the one case it must never hand them to.
          //
          // That case is not a corner: it is an author building a large diagram from scratch,
          // where the source does not parse yet and nothing has drawn. Measured on a 64-node
          // fence with an unparseable tail, 24 chars at 120ms, against the build being
          // replaced — 2-3 renders against 1, main thread 14-15% against 1-2%, and the
          // harness's own fixed-delay typing 3763-3915ms against 3404-3428ms. Read out of the
          // running policy: `costs:[] cheap:true floor:16` at burst start, then a 38ms parse
          // and an unthrottled 457ms blocking render landing between keystrokes.
          //
          // A failed render costs the same main thread as a successful one, which is the only
          // thing the number is for. Recorded on the cap path too, deliberately: a render that
          // raced the cap spent exactly as much of the thread as one that won.
          recordRenderCost(nowMs() - startedAt);
          if (settled) return;
          clearTimeout(timer);
          finish();
        });
    ...
    for (const section of document.querySelectorAll('section.verdict-grid, section.pricing')) {
      ...
      for (const outerLi of section.querySelectorAll(':scope > ul > li, :scope > ol > li')) {
        ...
        for (const li of outerLi.querySelectorAll('li')) {
          ...
          li.replaceChildren(badgeSpan(`badge ${sem} ${shape}`, m[2]), ...sublists);
        }
      }
    }
  }
...
function transformInlinePills() {
  ...
  function transformObligationMatrixBadges() {
    ...
    for (const section of document.querySelectorAll('section.obligation-matrix, section.state-cells')) {
      for (const td of section.querySelectorAll('td')) {
        ...
        td.replaceChildren(badgeSpan(`state ${sem} ${shape}`, m[2]));
      }
    }
  }
  ...
  function deckClassConfigFrom(fm) {
      ...
      // Deck-wide GUARDS (`guards: strict` → guards-strict) — whether TRIM may cut text.
      // `loose`/unknown → no token, the pre-register baseline. See resolve-guards.js.
      const guardsName = frontMatterName(fm, 'guards') || '';
  ...
  function applyCachedDeckClass() {
    ...
    // the export path there is nothing to wait for — the same reasoning `deckFormMode`
    // already uses for `form: off`. Only the `.md` FETCH fallback genuinely cannot
  ...
  // (`url(#latt-a11y-tex-N)`, themes/a11y-base.css). Mirrors
  ...
  function bootstrap() {
     ...
     *
     * Every builder that injects Mermaid writes a PLAIN `<script src>` — no `async`, no
     * `defer` — BEFORE the runtime's own tag (deck-preview.js buildSrcdoc,
     * single-slide-render.ts, stage-window.js and marp-bundle.js all assemble the pair in
     * that order). A classic parser-inserted script that sits before ours has therefore
    ...
    const tick = () => {
      ...
      const outcome = initAndRun();
      // A deferred pass is not a stalled one: the diagram run's completion tail will call
      // back. Stop polling rather than spending theme-wait frames on a condition that has
      // nothing to do with the theme.
      //
      // THAT TAIL IS NOW THE ONLY GUARANTEE, where this comment used to name two. The
      // back-off timer was deleted with the second timer, and this return stops `tick`'s rAF
      // poll permanently — so if `endDiagramRuns` ever stops re-entering, nothing else here
      // will. It is reached on every path today (two `.catch`es plus the decrementing tail,
      // with `RENDER_SETTLE_CAP_MS` and `PARSE_CAP_MS` bounding the two ways a job can hang),
      // which is what makes the single guarantee sound rather than merely single.
      if (outcome === RUN_DEFERRED) return;
      if (outcome) {
        if (typeof console !== 'undefined') {
  ...
  function startRoughInk() {
    if (typeof document === 'undefined') return;
    // Nothing on the page uses the finish → do not install an observer. This
    // is the common case (most decks are not sketch decks) and it keeps the
    // whole feature off the hot path for them.
    if (!document.querySelector('section.sketch')) return;
  ...
  // per frame; scheduleRun's own debounce (above) stays separate on purpose —
  // it drives the Mermaid/content-transform pass, which wants its own settle
  // window, not the geometry/overflow watchers' every-frame feel. That window
  // is now COALESCE_MS (16ms), not the 150ms this comment used to name: the
  // fixed diagram debounce was removed, and the content pass inherited the
  // shorter floor. See the note at COALESCE_MS for what that costs.
  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
        ...
        if (guardsEnabled(s.className)) {
          ...
          if (trimModel.boxes.length) {
            ...
            if (plan.actions.length) {
              applyTrim(s, plan);
              // VERIFY, then revert if the cut did not buy the fit. planTrim's
              // fit-or-nothing guarantee holds over its MODEL — a prediction about
              // the DOM, not a measurement of it. Trusting it shipped a slide that
              // was trimmed AND still overflowed, which is the exact outcome rule 5
              // exists to prevent.
              //
              // PER BOX, through the kernel. This used to be `clearTrim(s)` — a
              // whole-section revert, which the export's own comment names as the
              // mechanism by which `guards: strict` quietly becomes inert on exactly
              // the split layouts it was meant to help. The two paths shared the
              // kernel and disagreed on the POLICY, so the same deck trimmed in the
              // PDF and reverted in the live preview (HARD RULE #1).
              // At the FIT target, not `TOL`. `TOL` is the overflow probe's slack and
              // answers "is this worth acting on"; the verdict on a cut that already
              // happened is a different question, and answering it at 12px certified
              // a sheared card as fitting on the export path.
              const stillOver = measureTrim(s, CLIP_CELL_SELECTOR, TRIM_FIT_EPSILON, trimNs)
                .boxes.map((b) => b.id);
              if (stillOver.length) clearTrimBoxes(s, plan, stillOver);
            }
          }
        }
    ...
    // 150ms USED TO match scheduleRun's own settle window, so an edit burst
    // produced one content pass and one sweep rather than interleaving them.
    // It no longer does: that window is COALESCE_MS (16ms) since the fixed
    // diagram debounce was removed, so a burst now runs roughly eight content
    // passes per sweep. The 150ms here is kept because it is the right unit for
    // THIS watcher — long enough that a burst does not re-measure geometry per
    // keystroke — but it is now its own number rather than a mirror of another,
    // and calling it a match would be false.
