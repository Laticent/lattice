 ...
 * and are left alone.
 *
 * The browser CAN see the coda cell, so unlike the exporter this path resolves the coda arm
 * directly instead of riding it along as `data-cards-coda`. Both attributes are still
 * written, so a section stamped here and one stamped server-side are identical (#1).
 */
...
(() => {
  ...
  function stampOrientation(s) {
    ...
    const before = s.getAttribute('data-family');
    if (fam === 'wide') s.removeAttribute('data-family');
    else if (before !== fam) s.setAttribute('data-family', fam);
    // A component may declare a DIFFERENT composition per family (cards-grid paces its
    // cards down a tall frame instead of centering them), so the resolved `data-cards`
    // is a function of the family and has to be recomputed whenever the family moves.
    // Nothing else re-runs the deck-token pass on a resize, so without this a box
    // dragged from wide to tall would keep the wide answer. See resolve-cards.js.
    if ((s.getAttribute('data-family') || null) !== (before || null)) stampCardsAlign(s);
  }
