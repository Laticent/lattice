...
const { diagramScopeKey, diagramCacheKey, groupDiagramsBySlide, isDiagramRevision } = require('../../lib/core/diagram-scope');
...
(() => {
  ...
  // …and 0 for a diagram that has JUST APPEARED where there was none.
  //
  // What the debounce buys is COALESCING: consecutive keystrokes collapse into one
  // `mermaid.render` instead of one per character, on a queue that is strictly serial
  // (see beginDiagramRun). That is worth 150ms whenever the author is editing a fence.
  // It is worth nothing when a fence has only just arrived — nobody is typing into a
  // diagram they have not seen yet, and the 150ms is added to the wait in front of the
  // render. Measured on the built Studio, navigating onto a diagram slide for the first
  // time: 214ms of empty slot, of which 150 was this timer.
  //
  // "JUST APPEARED" IS NOT "SHOWING NOTHING", and the difference is the whole guard.
  // An empty slot is also what an author sees while their in-progress source does not
  // parse — `attachError` clears the target — so keying on emptiness alone dropped the
  // debounce for exactly the case it exists for: 8 keystrokes into a broken diagram
  // queued 8 renders instead of 1, and reached the finished diagram LATER than the plain
  // debounce did (measured by the maker-checker on the real bundle). `burstFirstSight`
  // asks the question the author's intent actually turns on — did a fence arrive where
  // the outgoing DOM had none? — so a re-render of a fence that was already there keeps
  // the full debounce however empty its slot is.
  const COLD_MS = 0;
  let scheduledRunHandle = null;
  let scheduledRunDelay = DEBOUNCE_MS;
  ...
  /**
   * @param {{ delay?: number }} [opts] `delay` 0 for a burst that brought in a diagram
   *   nothing was showing before (see COLD_MS). Within one pending window the shorter
   *   delay wins — a fence with nothing to show is the case that decides how long the
   *   author waits, and a later ordinary burst must not push it back out.
   *
   * Called once per mutation BURST (the observer callback receives the whole batch), not
   * once per record.
   */
  function scheduleRun({ delay = DEBOUNCE_MS } = {}) {
    if (scheduledRunHandle) {
      clearTimeout(scheduledRunHandle);
      delay = Math.min(delay, scheduledRunDelay);
    }
    scheduledRunDelay = delay;
    scheduledRunHandle = setTimeout(() => {
      scheduledRunHandle = null;
      scheduledRunDelay = DEBOUNCE_MS;
      initAndRun();
   ...
   * · Adopt ink that is not a REVISION of what is arriving. `isDiagramRevision` asks
   *   whether the new source is the old one edited, which is the only thing that makes
   *   holding correct. The structural guards cannot answer it: the Studio's editor
   *   preview replaces the whole `.lattice` body in one mutation, so navigating between
   *   two diagram slides is shaped exactly like an edit.
   * · Donate from a fence that is not `rendered`. A `pending` fence whose slot holds an
   *   SVG is one this walk filled; donating it forward carries a placeholder from slide
   *   to slide.
  ...
  function adoptOutgoingDiagrams(records) {
    if (typeof document === 'undefined') return;
    // The SAME guard `replayCachedFences` opens with, and for the same reason.
  ...
  function adoptWithinNode(outgoing, incoming) {
    ...
    const donors = fencePres(outgoing).map((preEl) => {
      // ONLY A `rendered` FENCE MAY DONATE, and this is what stops a wrong hold from
      // travelling. A fence that is `pending` while its slot holds an SVG is one THIS walk
      // filled a moment ago — a placeholder, not that source's ink. Donating it forward
      // hands the next arrival ink that was already the wrong answer, and an author clicking
      // through a rail faster than the debounce carries one slide's diagram across every
      // slide they touch. Driven by the maker-checker, chaining across three slides.
      if (preEl.dataset.mermaidState !== 'rendered') return null;
      const sib = preEl.nextElementSibling;
      const svg = sib?.classList.contains('mermaid') ? sib.querySelector('svg') : null;
      if (!svg) return null;
      const codeEl = preEl.querySelector(':scope > code');
      return { svg, sectionEl: preEl.closest?.('section') || null, source: (codeEl?.textContent || '').trim() };
    });
    ...
    for (let i = 0; i < arrivals.length; i++) {
      ...
      // IS THIS THE SAME DIAGRAM, EDITED? The one question that makes holding the outgoing
      // ink correct, and the one the guards above cannot answer. `patchSlideBody` replaces
      // the whole `.lattice` body in a single mutation, so on the Studio's editor preview a
      // NAVIGATION between two diagram slides arrives in exactly the shape an edit does —
      // same node count, same fence count, same scope key — and without this the author
      // clicking onto a diagram slide saw the PREVIOUS slide's diagram until the render
      // landed. See isDiagramRevision for why slide identity is not the test.
      const codeEl = preEl.querySelector(':scope > code');
      if (!isDiagramRevision(donor.source, (codeEl?.textContent || '').trim())) continue;
  ...
  /**
   * Did this burst bring in a diagram that has NOTHING to show and NOTHING it replaced?
   *
   * The question `scheduleRun`'s delay turns on (see COLD_MS). True only for a fence that
   * ARRIVED — the node it came in with carried no fence of its own — and that is still
   * `pending` with an empty target after the replay and the adoption have had their turn.
   *
   * The negative case is the one that matters and it is easy to get wrong: an author
   * mid-edit of a diagram whose source does not parse ALSO sees an empty slot, because
   * `attachError` clears the target. Keying on emptiness alone therefore removed the
   * debounce from the single case it exists for. Here that burst's outgoing node carries
   * the fence they are editing, so it is not first sight and it keeps the full 150ms.
   *
   * Scoped to the burst's own arrivals, not the document: one broken diagram on slide 12
   * of a Playground filmstrip must not decide the delay for a keystroke on slide 1.
   */
  function burstFirstSight(records) {
    for (const record of records) {
      if (!record.addedNodes?.length) continue;
      for (let n = 0; n < record.addedNodes.length; n++) {
        const incoming = record.addedNodes[n];
        const arrivals = fencePres(incoming);
        if (!arrivals.length) continue;
        // Same index as the added node, so "what this node replaced" means what it says.
        // A record with no removed node at all (a pure insertion) is first sight outright.
        const outgoing = record.removedNodes?.length === record.addedNodes.length ? record.removedNodes[n] : null;
        if (outgoing && fencePres(outgoing).length) continue;
        for (const preEl of arrivals) {
          if (preEl.dataset.mermaidState !== 'pending') continue;
          const target = preEl.nextElementSibling;
          if (target?.classList.contains('mermaid') && !target.firstChild) return true;
        }
      }
    }
    return false;
...
function transformInlinePills() {
  ...
  function deckClassConfigFrom(fm) {
      ...
      const deckTokens = [...classTokens, ...colorModeTokens, ...finishTokens, ...modeTokens, ...claimTokens, ...stampTokens, ...toneStyleTokens, ...spectrumTokens, ...spectrumEdgeTokens, ...spectrumCardTokens, ...spectrumCardEdgeTokens, ...spectrumTrimTokens, ...cornersTokens, ...ruleTokens, ...eyebrowTokens, ...inlineCodeTokens, ...headlineTokens, ...liftTokens, ...cardsTokens];
  ...
  function bootstrap() {
    ...
    if (typeof MutationObserver !== "undefined") {
      new MutationObserver((records) => {
        ...
        // A diagram that ARRIVED with nothing to show is one nobody is editing, so the
        // debounce in front of its render buys nothing. Asked after both of the above, so
        // "nothing to show" means neither the cache nor the outgoing DOM could fill it.
