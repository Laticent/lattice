...
// The ONE place the engine palette meets an author's own `%%{init}%%` (#1311).
// Shared with the PDF path so a directive that sets layout/curve/renderer costs
// nothing but layout — see lib/integrations/mermaid/init-directive.js.
const {
  withEngineInit, engineInitConfig, DIAGRAM_FONT_STACK,
...
(() => {
  ...
  function buildMermaidThemeVars() {
    ...
    // SENTINEL. This read used to happen exactly once, inside ensureConfigured,
    // behind its `--cat-1-fill` check; it now runs per render pass, and
    // ensureConfigured returns early on the second pass without re-checking. If
    // the palette isn't resolvable at THIS instant, return nothing rather than a
    // set of colors read off an unstyled element: `vc()` answers an undefined var
    // with a real `rgb(0, 0, 0)`, which prune() cannot tell from a deliberate
    // black, so the diagram would bake solid black AND be written to the SVG
    // cache — a wrong render that never self-heals. An empty set means Mermaid
    // falls back to its own defaults, which is the documented behavior of
    // ensureConfigured's own force path.
    if (!getComputedStyle(scopeEl).getPropertyValue('--cat-1-fill').trim()) return {};
    ...
    // The containment rung — a surface that sits BEHIND the categorical node
    // fills. Distinct from bgAlt (the deck's card fill) on purpose; see the
    // clusterBkg note in the emulator's MERMAID_VAR_MAP.
    const contain = vc('c-container');
    ...
    const result = {
      // The SHARED diagram font (kernel const), not `--font-body`. Two reasons,
      // and they point the same way. (1) Mermaid's `sanitizeDirective` allow-list
      // for themeVariables values has no hyphen, so a body stack carrying
      // `system-ui`/`sans-serif` is silently blanked to `""` the moment it rides
      // in a directive — and a blank font makes Mermaid MEASURE in the host's
      // default font while the page RENDERS in the inherited one, clipping labels
      // mid-word. (2) The PDF path has always used this monospace stack; matching
      // it means preview and export finally agree on diagram type (HARD RULE #1).
      ...
      nodeBorder:               border,
      nodeTextColor:            text,
      // Containment surface, not the card fill — see MERMAID_VAR_MAP in
      // lattice-emulator.js. Only reaches PLAIN clusters; a `.section-N` cluster
      ...
      // audit already holds to AA in both modes) — NOT --cat-on-mark on
      // --diagram-critical. That older coupling made the error ink track the
      // categorical-mark tier, which fails when a theme's --diagram-critical doesn't
      // flip pale-in-dark (e.g. the achromatic ramps, where --diagram-critical is the
      // darkest gray for severity ordering and CAN'T go pale). Decoupled in #1181.
      errorBkgColor:  vc('fail'),
      errorTextColor: vc('bg'),
  ...
  function ensureConfigured(mermaid, { force = false } = {}) {
    ...
    mermaid.initialize({
      ...
      // NO `themeVariables` here. The palette rides in each diagram's SOURCE as a
      // `%%{init}%%` directive instead (see the render loop), which is the only
      // mechanism the PDF path has — one mechanism, one set of merge semantics
      // (#1311). Passing it here as well would ALSO break the author-pins-a-theme
      // case: mermaid folds `configFromInitialize.themeVariables` back in as user
      // overrides whenever a directive names a theme, so `%%{init: {'theme':
      // 'forest'}}%%` would render forest painted in our palette here while the
      // PDF path rendered forest clean.
      quadrantChart: {
        titleFontSize: 24,
        pointTextPadding: 20,
  ...
  // Cheap, stable string hash (djb2) — used only to keep the Mermaid SVG cache
  // key constant-size while still tracking the active palette. Not a checksum:
  // a collision would serve one deck's diagram bake for another palette, which
  // is the same class of staleness the cache already accepts on a theme switch.
  function djb2(str) {
    let h = 5381;
  ...
  function initAndRun({ force = false } = {}) {
    ...
    // Read the palette ONCE per pass, not once per fence: buildMermaidThemeVars
    // walks ~120 custom properties through a probe element, and every pending
    // fence in a pass sees the same cascade. Built lazily so a pass with no
    // pending fences costs nothing.
    let engineInit = null;
    const engineInitFor = () => (engineInit ??= engineInitConfig(buildMermaidThemeVars()));
    // A short signature of the resolved palette, for the SVG cache key. The key
    // has to be palette-aware now that the palette bakes into the source (a live
    // theme swap must not serve the old bake), but keying on the injected source
    // itself would put ~3.7 KB of serialized themeVariables in every entry of an
    // unbounded Map — and the cache's own "no size bound needed" note was written
    // when the key was just the fence text. Hash it instead: same invalidation,
    // constant-size key.
    let paletteSig = null;
    const paletteSigFor = () => (paletteSig ??= djb2(JSON.stringify(engineInitFor())));
    for (const preEl of document.querySelectorAll(FENCE_SELECTOR)) {
      ...
      const authored = reorientMermaidForPortrait((codeEl.textContent || "").trim(), orientation);
      // Palette goes in the SOURCE, not in `initialize` — same mechanism the PDF
      // path uses, so both agree about what an author `%%{init}%%` overrides
      // (#1311).
      const source = withEngineInit(authored, engineInitFor());
      // Cache on the AUTHORED text plus a palette signature, not on the injected
      // source: same invalidation on a live theme swap, without carrying the
      // whole serialized palette in every key.
      const cacheKey = `${paletteSigFor()}\u0000${authored}`;
