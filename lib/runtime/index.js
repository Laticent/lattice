...
(() => {
  ...
  const PREVIEW_ONLY_CONFIG = {
    ...
    //
    // WHAT THIS KEY DOES AND DOES NOT COVER, measured against Mermaid 11.14 rather than
    // assumed (#1246). It governs URL handling: `formatUrl` runs `sanitizeUrl` unless the
    // level is `loose`, and `setClickFun` refuses to bind a click callback unless it is.
    // It does NOT govern NODE LABELS — `sanitizeText` runs DOMPurify unconditionally and
    // consults this key only for an extra `removeScript` pass. So a label payload is held
    // by Mermaid's own sanitizer, not by this line. Both contracts are third-party and both
    // are pinned behaviorally in docs/e2e/mermaid-post-sanitize.spec.ts, against the real
    // Playground with the real Mermaid — a source assertion here could not see either.
