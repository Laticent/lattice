...
(() => {
  ...
  /** True from the moment a diagram run is queued until its whole chain settles. */
  ...
  function scheduleRun() {
    // A run is already in flight. Do NOT queue a second one behind it: the diagram queue is
    // strictly serial, so a keystroke-per-link chain makes the author wait for every render
    // their typing superseded. Record that the source moved; the completion hook starts the
    // next pass with whatever the text says by then.
    if (diagramRunActive) {
      rerunRequested = true;
      return;
    }
  ...
  function beginDiagramRun(mermaid, themeVars, look) {
    ...
    diagramQueue = diagramQueue
      ...
      // NEVER let the tail poison the flag. `diagramRunActive` gates every later pass, so a
      // throw between here and the hook below would stop the document rendering diagrams
      // for the rest of the session — the failure mode is total and silent.
      .catch((err) => {
  ...
  /** Fences whose latest source did not parse — waiting to see whether typing continues. */
  const parseDeferred = new Set();
  /**
   * Fences the quiet timer has released: render these for real even though they will fail,
   * so the author gets their error box.
   *
   * PER FENCE, NOT A GLOBAL FLAG, and that is not a stylistic choice. The first version
   * used a one-shot boolean cleared by the run's completion hook, which produced a loop
   * that never surfaced anything: the timer could fire DURING an in-flight run, set the
   * flag, and have the completion hook clear it before the pass it was set for ever
   * started — so the next pass hit the gate, deferred again, re-armed the timer, and the
   * author sat looking at a stale diagram indefinitely. Driven on the real Studio: the
   * fence stayed `pending` with the previous SVG four seconds after typing stopped. A
   * token attached to the thing it is about cannot be consumed by the wrong pass.
   */
  ...
  function armErrorSurface() {
    ...
    errorSurfaceTimer = setTimeout(() => {
      ...
      if (!parseDeferred.size) return;
      for (const preEl of parseDeferred) {
        forceRender.add(preEl);
        // Back into the walk's selector, now that the author has stopped.
        if (preEl.dataset.mermaidState === 'deferred') preEl.dataset.mermaidState = 'pending';
      }
      parseDeferred.clear();
      scheduleRun();
    }, ERROR_QUIET_MS);
  ...
  function renderDiagramJob(mermaid, scopeKey, job) {
    ...
    return parsesCleanly(mermaid, job.source).then((ok) => {
      ...
      parseDeferred.delete(job.preEl);
