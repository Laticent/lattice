  ...
  function startOverflowWatcher({ level: rawLevel = AUTHORING_DEFAULT_MARKER } = {}) {
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-lattice-slide]')) {
        ...
        // `.content-cut` in base.modifiers.css.
        ...
        // ONE class, orthogonal to `.overflow`: `.content-cut` IS `tell`. It replaced
        // `.overflow-silent` (`over && !tell`) and `.content-clipped` (`tell && !over`),
        // two conjunctions that between them left a slide which BOTH overflows and cuts
        // carrying neither — and let the CSS un-hide a population by one class while
        // styling it by the other. `policy.mark` is false only at `off`, which promises
        // to leave nothing. Idempotent, like the toggle above: the observer watches
        // attributes, so an unconditional write would let it react to its own mutation.
        const contentCut = policy.mark && tell;
        if (s.classList.contains('content-cut') !== contentCut) {
          s.classList.toggle('content-cut', contentCut);
        }
