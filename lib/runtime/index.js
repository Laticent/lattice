...
const listTabularMarksKernel = require('../../lib/core/list-tabular-marks');
...
(() => {
  ...
  function runAllContentTransforms() {
    ...
    // list-tabular's trailing marks bullet — the checkbox + pills cell. Without the
    // mirror a Marp render leaves a typed `[x]` in the description column.
    // Kernel: lib/core/list-tabular-marks.js (shared with the markdown-it plugin).
    transformListTabularMarks();
    transformSlotLabels();
  ...
  /**
   * Promotes a list-tabular row's trailing sublist bullet into its MARKS cell in
   * a live document (mirrors the `listTabularMarks` markdown-it plugin). For each
   * NESTED <li> in section.list-tabular that holds only a state marker and/or
   * inline `code` pills, strips the marker and adds
   *   class="marks [state {pass|warn|skip|todo} {state-full|…}]"
   * CSS lands it right-aligned in the trailing column and draws the marker as a
   * disc. Which bullets qualify comes from the shared kernel
   * (lib/core/list-tabular-marks.js). Idempotent — skips items already tagged.
   */
  function transformListTabularMarks() {
    if (typeof document === 'undefined') return;
    for (const section of document.querySelectorAll('section.list-tabular')) {
      for (const li of section.querySelectorAll(':scope ol > li > ul > li')) {
        if (li.classList.contains('marks')) continue;
        // A LOOSE list wraps an item's inline content in a `<p>`, so the marker
        // and the pills are one element deep. Walking `li.childNodes` directly
        // saw that `<p>` as "an element before any text", bailed, and left a
        // typed `[x]` on the reader's slide while the engine stripped it — the
        // drift this mirror exists to prevent (#1858's failure mode).
        const host = listTabularMarksKernel.contentHost(li);
        // A bullet carrying anything block-level is prose, not a status. The token
        // path rejects the same shapes; without this the two disagreed on a nested
        // sub-list, a fence, an HTML block and a rule.
        if (!listTabularMarksKernel.isSingleBlock(li, host)) continue;
        const firstText = host.firstChild?.nodeType === 3 ? host.firstChild : null;
        const marker = firstText ? listTabularMarksKernel.readMarker(firstText.nodeValue) : null;
        if (!marker) {
          const parts = [...host.childNodes].map((n) => ({
            code: n.nodeType === 1 && n.tagName === 'CODE',
            text: n.nodeType === 3 ? n.nodeValue : (n.nodeType === 1 ? 'x' : ''),
          }));
          if (!listTabularMarksKernel.isPillsOnly(parts)) continue;
        }
        li.classList.add(listTabularMarksKernel.MARKS_CLASS);
        if (!marker) continue;
        firstText.nodeValue = firstText.nodeValue.slice(marker.consumed);
        host.insertBefore(listTabularMarksKernel.stateNode(document, marker), host.firstChild);
      }
    }
  }
