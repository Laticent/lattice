...
(() => {
  ...
  function applyDefaultComponent() {
    ...
    for (const section of document.querySelectorAll('section')) {
      ...
      if (next !== cur) section.className = next.join(' ');
    }
  }
  ...
  function bootstrap() {
    ...
    // marp-fidelity.js's `coverage: 'mirrored'` should rest on. So re-run the
    // transforms inside the same continuation: they are idempotent, this still
    // lands before paint, and the two paths converge on the same DOM regardless of
    // what a future transform keys on (HARD RULE #1).
    afterDeckFrontMatter(() => { applyDefaultComponent(); runAllContentTransforms(); });
