...
(() => {
  ...
  function transformVerdictGridBadges() {
    ...
    for (const section of document.querySelectorAll('section.verdict-grid, section.pricing')) {
      ...
      for (const outerLi of section.querySelectorAll(':scope > ul > li, :scope > ol > li')) {
        ...
        // and printed the child raw. `querySelectorAll('li')` on the outer item is
        // exactly "depth >= 2", and for the depth-2-only shape every committed deck
        // uses it selects the same set the old `innerUl.children` did.
        for (const li of outerLi.querySelectorAll('li')) {
          ...
          // text into the badge label. Excluding nested lists is what makes the two
          // sides read the same string.
          ...
          // silently drops every sub-criterion from the slide — and, because the
          // walk above is a static NodeList, leaves those items detached and
          // unbadged rather than erroring. The engine has no such problem: it
          // rewrites the inline token and never touches the list tokens after it.
          // Badge first, then the sublists, which is the order the engine emits.
  ...
  function deckClassConfigFrom(fm) {
      ...
      const deckTokens = [...classTokens, ...colorModeTokens, ...finishTokens, ...modeTokens, ...claimTokens, ...stampTokens, ...toneStyleTokens, ...spectrumTokens, ...spectrumEdgeTokens, ...spectrumCardTokens, ...spectrumCardEdgeTokens, ...spectrumTrimTokens, ...cornersTokens, ...ruleTokens, ...eyebrowTokens, ...headlineTokens, ...liftTokens];
