...
const THEME_CANON =
  ...
  'accent HUE: each of 12 slots is one hue drawn as a fill + a mark (its border). ' +
  'Light canvas = pale fill + deep mark; dark canvas = deep "jewel" fill + pale mark; ' +
  'the label ink flips with the fill so it stays legible, and the mark always clears ' +
  '3:1 against the canvas. The engine derives and AA-repairs all of it — so pick an ' +
  'accent that is saturated and distinctly hued.\n' +
