...
/**
 * THE HEATMAP RAMP — step positions and the ink that sits ON each step.
 *
 * A SECOND PROBLEM SHAPE, not a second recipe. `solveInkArm` above puts a
 * categorical ink on the CANVAS; this puts an ink on the MARK — text on a
 * colored fill, the one placement `design/skills/chart-component.md` says the
 * chart family will not guarantee contrast for. The heatmap is the member that
 * has to, because intensity IS its encoding and the number is what a reader
 * checks it against. The solve underneath is `solveInk`, unchanged.
 *
 * WHY THE RAMP IS QUANTIZED, which is the whole reason this function exists.
 * The first cut printed the value on a CONTINUOUS ramp and flipped its ink
 * black/white at a fixed crossover. That cannot be made to work, and the failure
 * is structural rather than a matter of picking a better number:
 *
 *   - THERE IS A DEAD ZONE. At the fills where black and white are equally bad,
 *     the best ink of ANY color reaches about 4.6:1 — clearing AA by a hair and
 *     missing the AA + MARGIN floor this repo holds every other solved ink to.
 *     A continuous ramp climbs through every lightness between its ends, so it
 *     always crosses that zone. Measured: banding the ink over a continuous fill
 *     leaves 58-73 of 198-792 bands under the floor at EVERY band count from 3
 *     to 12 — more bands does not help, because the zone is in the fill, not in
 *     the banding.
 *   - A PER-PALETTE CROSSOVER IS A NUMBER, and a number cannot cross the
 *     light/dark boundary. `section.dark` flips `color-scheme` per SLIDE, and
 *     CSS has `light-dark()` for colors only. Quantizing converts the per-palette
 *     knowledge from a number into a COLOR, which `light-dark()` carries — the
 *     same move `--cat-on-mark` already makes.
 *
 * So the steps are chosen, not spaced. Each starts at an even position and is
 * NUDGED — measured: at most 2% — until both canvases clear the floor there.
 *
 * ONE SET OF POSITIONS FOR BOTH CANVASES, deliberately, and this is the
 * constraint that keeps the result expressible. A per-canvas nudge would be a
 * per-canvas NUMBER, which is the trap in the second bullet above wearing a
 * different hat. So a position is accepted only when the light AND dark fills at
 * that position both admit an ink. Measured over the 33 shipped palettes: every
 * step solves, the ramp stays monotonic, and the worst pair is 4.65:1.
 *
 * @param {object} o
 * @param {object} o.light  `{ hue, anchor, seed }` for the light canvas — the
 *                          ramp's hue, the neutral it mixes into, and the ink's
 *                          source color (hue and chroma are kept; only lightness
 *                          moves, so the ink stays the theme's own).
 * @param {object} o.dark   the same three for the dark canvas
 * @param {(hue: string, anchor: string, mix: number, dark: boolean) => string} o.fillAt
 *                          resolves one step's fill. Injected because mixing in
 *                          oklab is the CSS engine's job, not this module's —
 *                          keeping it out is what leaves this file fs-free and
 *                          browser-safe.
 * @param {number} [o.lo]   the ramp's floor, as a mix percentage
 * @param {number} [o.hi]   the ramp's top
 * @param {number} [o.steps]
 * @param {number} [o.maxNudge] how far a step may move off even spacing
 * @param {boolean} [o.strict] throw on an unsolvable palette, per the file header
 * @param {string} [o.label]
 * @returns {{ mixes: number[], inks: {light: string, dark: string}[], degraded: string[] }}
 */
function solveHeatmapRamp({
  light, dark, fillAt, lo = 18, hi = 92, steps = 5, maxNudge = 8,
  strict = false, label = 'theme',
}) {
  const mixes = [];
  const inks = [];
  const degraded = [];
  ...
  for (let k = 0; k < steps; k += 1) {
    const even = lo + ((hi - lo) * k) / (steps - 1);
    let found = null;
    // Outward from the even position, nearest first, so a step only moves as far
    // as it must. `d === 0` is tried alone — offering it twice would let a later
    // tie beat an earlier one for no reason.
    for (let d = 0; d <= maxNudge && !found; d += 1) {
      for (const delta of d === 0 ? [0] : [-d, d]) {
        const mix = Math.min(hi, Math.max(floor, even + delta));
        // MONOTONIC OR IT IS NOT A RAMP. A nudge that puts this step at or below
        // the previous one inverts the encoding for those two bins, which is a
        // worse defect than the contrast it was buying.
        if (k > 0 && mix <= mixes[k - 1]) continue;
        const fl = fillAt(light.hue, light.anchor, mix, false);
        const fd = fillAt(dark.hue, dark.anchor, mix, true);
        if (!fl || !fd) continue;
        const il = solveInk(light.seed, fl, fl);
        const id = solveInk(dark.seed, fd, fd);
        if (il && id) { found = { mix, light: il, dark: id }; break; }
      }
    }
    if (!found) {
      const message =
        `${label}: heatmap ramp step ${k + 1} of ${steps} has no legible ink on both canvases ` +
        `within ${maxNudge}% of its even position (${even.toFixed(1)}%). No shade of the theme's own ` +
        `ink clears ${AA + MARGIN}:1 against the fill there. Widen the ramp, or move the hue away ` +
        'from the lightness where black and white are equally weak.';
      if (strict) throw new Error(message);
      degraded.push(message);
      // Keep the ramp monotonic and keep going, so the caller sees every failure
      // rather than only the first.
      const mix = Math.min(hi, Math.max(floor, even));
      mixes.push(mix);
      inks.push({ light: light.seed, dark: dark.seed });
      floor = mix;
      continue;
    }
    mixes.push(found.mix);
    inks.push({ light: found.light, dark: found.dark });
    floor = found.mix;
  }
  return { mixes, inks, degraded };
}
