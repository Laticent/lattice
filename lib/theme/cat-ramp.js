 ...
 * neighbours on the wheel collapse. Solving the slots TOGETHER is the fix.
 *
 ...
 * order, and honouring it spreads the ramp into a SCATTER: near-white at slot 7,
 ...
 * 0.071 with neighbours much closer than that — and forcing slot order on those
 ...
 * §6 asks for. Under ADJACENT only neighbours must clear, and a two-level ZIG-ZAG
 * does that inside a range of one gap instead of eleven: `carbone`'s twelve light
 * marks need 0.089 between neighbours, which a zig-zag fits in its 0.53 band and a
 ...
 */
function solveRamp(hexes, { floor, scope, bandLo, bandHi, accept }) {
...
function solveAt(hexes, { floor, scope, bandLo, bandHi, headroom, descend = false }) {
  ...
  for (let round = 0; round < 24; round += 1) {
    ...
    for (const [i, j] of pairs) {
      // How much of the floor hue and chroma already pay for; lightness owes the rest.
      const owed = Math.sqrt(Math.max(0, floor * floor - chromaDistance(current[i], current[j]) ** 2));
      if (owed <= 0) continue;
      constraints.push({ i, j, gap: owed + headroom, sign: signOf(i, j) });
    }
...
module.exports = { solveRamp, requiredPairs, tierOf, chromaDistance, HEADROOM_LADDER, ramplessOrder, FLOOR, SCOPE };
