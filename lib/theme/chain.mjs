 ...
 * @param {(dir: string, name: string) => string} resolve  name → path, relative to the importer
...
export function flattenCssImports(entryPath, { read, resolve, exists }, seen = new Set()) {
  ...
  let imported = '';
  for (const m of body.matchAll(/@import\s*(['"])([A-Za-z0-9_-]+)\1\s*;?/g)) {
    // `lattice` is the ENGINE base, loaded separately by every caller.
    if (m[2] === 'lattice') continue;
    const p = resolve(entryPath, m[2]);
