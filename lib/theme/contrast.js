 ...
 *                     FOUR STARTERS in light mode — `dusk`, the Studio's default, emits
 *                     the two BYTE-IDENTICAL at #646c7a. NOT A ROW, because it would be
 *                     red on the default state and authors would learn to ignore the
 *                     panel.
 *                     BUT NOT BECAUSE IT IS CORRECT. `design/theming.md:88` says muted is
 *                     "Still quieter than --text-secondary", so this is the derivation
 *                     failing a written contract, not a design: `secondaryLight` is a mix
 *                     of body toward muted and both are then repaired to the same AA
 *                     floor, so they converge. Calling that "by construction" — as an
 *                     earlier draft of this comment did — dresses a producer defect as
 *                     intent. It is logged in the decision note §5, unfixed. Note the
 *                     asymmetry with the row above and own it: secondary EARNS a row
 *                     because the derivation collapses it onto body, and this pair is
 *                     DENIED one because the derivation collapses it onto muted. The
 *                     difference is that body is the reference every quiet tier is
 *                     defined against, and a meter that is red before the author touches
