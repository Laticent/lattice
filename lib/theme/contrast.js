...
/**
 * The value as a color this module can MEASURE, or `null`.
 *
 * Three forms come back readable and everything else comes back `null`:
 *
 *   - `#rgb` / `#rrggbb` — what every producer in this repo emits;
 *   - `#rgbf` / `#rrggbbff` — an 8- or 4-digit hex whose alpha is FULLY OPAQUE.
 *     The color is exactly the 6-digit one, so measuring it is not an
 *     approximation. A *translucent* alpha is deliberately NOT readable: the
 *     composited color depends on what is behind it, which a token map does not
 *     know, so any ratio computed from it would be a confident wrong number.
 *
 * `oklch()`, `color-mix()`, `rgb()`, `hsl()` and named colors are NOT converted,
 * and that is a decision rather than a gap. `lib/theme/color.js` could convert an
 * `oklch()` — but out-of-gamut components clamp on the way to sRGB, so the value
 * measured would not be the value painted, and the audit's whole job is to be the
 * same predicate the shipped gate asserts. An unmeasurable operand is reported as
 * unmeasurable (see `auditVars`), which is the outcome the design note demands and
 * the one this module used to be missing.
 */
function readableHex(v) {
  if (typeof v !== 'string') return null;
  const h = v.trim().replace(/^#/, '');
  if (/^[0-9a-f]{3}$/i.test(h) || /^[0-9a-f]{6}$/i.test(h)) return `#${h}`;
  // 4- and 8-digit: keep only when the alpha channel is fully opaque.
  if (/^[0-9a-f]{4}$/i.test(h)) return /f/i.test(h[3]) ? `#${h.slice(0, 3)}` : null;
  if (/^[0-9a-f]{8}$/i.test(h)) return /^ff$/i.test(h.slice(6)) ? `#${h.slice(0, 6)}` : null;
  return null;
}

const isHex = v => readableHex(v) !== null;
 ...
 *   · muted^secondary committed worst 0.0021 (brina/light), and 29 of 64 committed
 *                     palette-modes under the floor.  STILL NOT A ROW — but the REASON
 *                     changed, and the old reason is now fixed.
 *                     It used to be 0.0000-0.0049 on all four STARTERS in light mode,
 *                     with `dusk` — the Studio's default — emitting the two
 *                     BYTE-IDENTICAL at #646c7a. That was `derive.js` solving each quiet
 *                     tier independently against the canvas so they converged on the AA
 *                     floor, and it had a worse twin nothing measured: on a DARK canvas
 *                     the muted tier came out LOUDER than body on all four starters
 *                     (dusk 10.30:1 against 7.46:1). The derivation now builds the two
 *                     as one monotone ladder — muted is the quietest ink the canvas
 *                     admits, secondary is placed between it and body — so a DERIVED
 *                     theme clears this pair with room (0.052-0.083 on the starters).
 *                     What is left is HAND-AUTHORED: 29 committed palette-modes still
 *                     collapse the pair, so the row would be red the moment a Studio
 *                     author picked one of those palettes as a base — the same
 *                     "red before the author touches anything" objection, arriving from
 *                     the catalog instead of from the producer. Adding the row is
 *                     therefore palette work, not meter work; logged in the decision
 *                     note, unfixed.
 *   · muted-mark^muted exactly 0.0000 on 18 committed palette-modes, BY DESIGN — the
 *                     AUTHOR wrote the two tokens identical (onyx and the four a11y
 *                     palettes that import it, cuoio/dark, magnolia/dark and their `-dark`
 *                     twins). Note the cause: committed palettes are hand-authored and
 *                     never pass through `deriveTheme`, so it is not derive.js doing this
 *                     to them — though derive.js reaches the same place for a DERIVED
 *                     theme, keeping the author's seed for the decoration tier wherever it
 *                     already clears 3:1.  NOT A ROW.
 *   · label^body      committed worst 0.0118 (cuoio/light). `--text-label` is not a
 *                     de-emphasis tier at all — it is accent-hued EMPHASIS, and on the
 *                     palettes with chroma to spare it is separated by hue rather than by
 *                     a lightness step (laguna C 0.084 vs body 0.030; indaco 0.122 vs
 *                     0.074).  NOT A ROW — but do not read the 0.0118 as hue separation
 *                     doing its job: on cuoio, label #6F604F and body #6b5d4f decompose
 *                     to dL 0.0109, da -0.0002, db 0.0045, which is no hue distinction at
 *                     all, and the label sits ΔE 0.0638 from the accent it is supposed to
 *                     carry. That tier has collapsed onto body on that palette and
 *                     nothing measures it; see the decision note §5.
 *
...
function evalSeparation(resolved, [quiet, loud, floor, role]) {
  const quietRaw = resolved[quiet];
  const loudRaw = resolved[loud];
  if (quietRaw == null || loudRaw == null) {
  ...
  if (!isHex(quietRaw) || !isHex(loudRaw)) {
    return {
      kind: 'separation', quiet, loud, role, floor, status: 'skipped', ratio: null, distance: null,
      quietHex: quietRaw, loudHex: loudRaw,
      // WHICH operand could not be read, so a caller can say so instead of
      // rendering an unexplained warning. See `auditVars`'s `skipped` view.
      unreadable: [isHex(quietRaw) ? null : quiet, isHex(loudRaw) ? null : loud].filter(Boolean),
    };
  }
  const quietHex = readableHex(quietRaw);
  const loudHex = readableHex(loudRaw);
...
function evalPair(resolved, [fill, ink, threshold, role]) {
  const fillRaw = resolved[fill];
  const inkRaw = resolved[ink];
  if (fillRaw == null || inkRaw == null) {
    return { kind: 'contrast', fill, ink, role, threshold, status: 'missing', ratio: null };
  }
  if (!isHex(fillRaw) || !isHex(inkRaw)) {
    return {
      kind: 'contrast', fill, ink, role, threshold, status: 'skipped', ratio: null,
      fillHex: fillRaw, inkHex: inkRaw,
      unreadable: [isHex(fillRaw) ? null : fill, isHex(inkRaw) ? null : ink].filter(Boolean),
    };
  }
  const fillHex = readableHex(fillRaw);
  const inkHex = readableHex(inkRaw);
...
/**
 * Audit a token map for one mode. `level` selects how much of the CONTRAST
 * contract is checked: `'gate'` (default) only what the shipped contrast gate
 * asserts, `'full'` adds the content-tier and accent-container pairs.
 *
 * It does NOT select separation rows — those are a different predicate, not a
 * stricter contrast bar, and they are present at both levels. So `'gate'` is
 * "the gate's contrast pairs, plus the separation tier", and two older tests
 * named for gate parity (`theme-derive`, `theme-serialize`) are held to that
 * slightly stricter bar by design: the derivation is the producer this tier
 * exists to be able to indict.
 *
 * Returns `{ mode, level, results, separation, failures, missing, skipped, passed, total, ok }`.
 * `results` holds BOTH row kinds (see the module header); `separation` is a view
 * over the `kind === 'separation'` subset of the same rows.
 *
 * ── `skipped` COUNTS AGAINST `ok`, and it did not use to ────────────────────
 *
 * A row whose operand this module cannot read (`readableHex` → null) is
 * `skipped`, and the verdict counted `skipped` as neither a failure nor a
 * missing token — so an UNREADABLE palette reported a clean bill of health.
 * Measured: a map of all 107 contract tokens set to `oklch(50% 0.1 250)`
 * returned `ok: true` with **every single row skipped**. Nothing was ever
 * checked and the answer was yes.
 *
 * That was never load-bearing while the pickers were the only producer — they
 * emit hex and nothing else — but hand-editing (the CSS view,
 * `engineering/decisions/2026-08-25-hand-editing-generated-assets.md`) is
 * precisely where `oklch()` / `color-mix()` / `#RRGGBBAA` arrive, and "silently
 * reporting a clean bill of health on an unreadable theme is the one outcome not
 * available."
 *
 * So `ok` now requires that every row was actually MEASURED. The distinction a
 * consumer must keep is that a skipped row is **not a failure** — it is an
 * absence of evidence, and a surface that paints it red says something false in
 * the other direction. `docs/src/components/studio/audit-meter.ts` ranks it
 * between the two and the panel gives it its own tier.
 */
function auditVars(vars, { mode = 'light', level = 'gate' } = {}) {
  ...
  return {
    ...
    failures,
    missing,
    // Rows this module could not MEASURE — a non-hex or translucent operand. Each
    // carries `unreadable`, the token name(s) that could not be read.
    skipped,
    passed: passed.length,
    total: results.length,
    // Every row measured, and every measured row passed. A skipped row is not a
    // failure; it is a hole in the evidence, and a verdict that ignores it is a
    // claim nobody checked.
    ok: failures.length === 0 && missing.length === 0 && skipped.length === 0,
  };
}
