 ...
 * matrices, `tools/cvd-audit.js` loops over it, and a unit test pins it as exactly
 * the three. Added for #1715, where `a11y-achromatopsia`'s syntax family has to be
 * measured under the condition the palette is named for and there was no primitive
 * for it — `tools/cvd-audit.js` has never covered that palette's own deficiency.
 *
