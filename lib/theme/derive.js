...
function deriveTheme(essentials, options = {}) {
  ...
  //
  // Repaired against BOTH canvases, like the heading and body tiers above. The card
  // (`--bg-alt`) is the conservative worst case — darker than `--bg` in light mode, lighter
  // in dark — so clearing it and the plain canvas clears every surface between them. That
  // second repair is the one that matters: the accent alone clears the canvas and MISSES the
  // card, which is exactly the 4.36:1 that shipped.
  t['code-inline-fg'] = ld(
