 ...
 *       the 3:1 GRAPHICAL floor, and 63 of 200 sampled essential sets then carry at
 *       least one sub-AA categorical label (worst 3.30:1). Contract, not fallback.
 */
...
function deriveTheme(essentials, options = {}) {
  ...
  // the mark instead: legible as a shape, and measurably not as text — across 200
  // sampled essential sets, 63 carry at least one sub-AA label that way (worst
  // 3.30:1). Solved by the shared recipe in lib/theme/cat-ink.js, NON-STRICT: this
  ...
  const containment = (canvas, brandEdge) => {
    ...
    // The ladder is asserted on RELATIVE LUMINANCE, not on OKLCH lightness, and
    // the two disagree by a hair when the canvas is chromatic and the rung is not.
    // Widen the step until the rung has actually moved away — a checked fact, not
    // an assumption that holds for the four starters.
  ...
  const ribbonC = Math.max(0.09, hexToOklch(e.accent).C);
  const spectrumStops = [
    oklchToHex({ L: 0.34, C: ribbonC * 0.85, h: accentHue }),
    oklchToHex({ L: 0.62, C: ribbonC, h: accentHue }),
    oklchToHex({ L: 0.62, C: ribbonC, h: norm360(rampHue(strategy, accentHue, 4, CATEGORICAL_COUNT)) }),
  ];
