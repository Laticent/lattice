...
const { solveInkArm, solveInk, bestEffortInk, solveHeatmapRamp } = require('./cat-ink.js');
...
const REQUIRED_TOKENS = Object.freeze({
  ...
  // `muted-mark` is GRAPHICAL, not ink — it is the de-emphasis tier for rules,
  // hairlines and empty/skipped marks, held to 3:1 rather than 4.5:1 (#1715).
  graphical: ['muted-mark'],
  ...
  // The heatmap's quantized ramp: five stop positions and the ink solved against
  // the fill each stop paints. Here rather than on a var() fallback for the reason
  // the note above `code-inline-fg` gives — ink that falls back onto a token held
  // to no text contract is the --cat-N-ink shape, and this ink sits on a SATURATED
  // FILL, where --text-body is not merely untested but wrong by construction.
  heatmap: [
    ...Array.from({ length: 5 }, (_, i) => `heatmap-step${i + 1}`),
    ...Array.from({ length: 5 }, (_, i) => `heatmap-step${i + 1}-ink`),
  ],
...
function validateEssentials(essentials) {
  ...
  for (const k of ESSENTIAL_KEYS) {
    ...
    try {
      ...
      throw new Error(`essential "${k}" is not a hex color: ${essentials[k]}`);
    }
  }
  return out;
}
 ...
 */
/**
 * One arm of a `light-dark(a, b)` pair this file has already built. Used where a
 * later tier has to SOLVE against an earlier tier's value rather than just emit
 * it — the heatmap ramp needs the concrete hue per canvas, not the pair.
 */
function ldArm(pair, i) {
  const m = /^light-dark\(\s*([^,]+?)\s*,\s*([^)]+?)\s*\)$/.exec(String(pair));
  return m ? m[i + 1] : String(pair);
}
...
function deriveTheme(essentials, options = {}) {
  ...
  const bodyLight = ensureContrast(ensureContrast(e.textBody, e.bg, AA, 'darken'), e.bgAlt, AA, 'darken');
  t['text-body'] = ld(bodyLight, darkBody);
  // Accent-hued label — AA on the canvas.
  t['text-label'] = ld(ensureContrast(e.accent, e.bg, AA, 'darken'), darkLabel);
  // #1715 SPLIT THE ROLE. This said "Muted is DECORATIVE / WCAG-exempt — pass
  // through, no repair", and every hand-authored palette carried the same comment on
  // the same token — while `--text-muted` was ALSO the color 53 engine-CSS sites painted real
  // text with, and it was below AA on 44 of 72 palette-mode-surface pairs (worst
  // 2.11:1). The name says text; the comment said exempt; both could not be true.
  //
  // So the TEXT role keeps the name and gains the floor the name always promised,
  // and the DECORATIVE role — rules, hairlines, empty/skipped marks, glyph washes —
  // moves to `--muted-mark`, which is named for what it is and takes the 3:1
  // graphical floor instead. `mutedLight`/`darkMuted` stay the author's own value
  // wherever it already clears; the repair is a minimum lightness move.
  // SOLVED, NOT DARKENED. `ensureContrast(…, 'darken')` walks one direction and gives up
  // rather than reversing, so on a DARK `bg` or `bgAlt` it moves AWAY from the target and
  // returns a value under its own floor — measured on this repo's own "cruel" essential set
  // (theme-derive.test.js: `bg #FFFFFF`, `bgAlt #111111`) it produced 4.11:1 against
  // `--bg-alt`, and on a carbone-shaped dark canvas 1.97:1. `solveInk` tries BOTH lightness
  // poles against BOTH surfaces and keeps the smaller move, which is the same recipe the
  // hand-authored palette values use (HARD RULE #1) and what the record claims this path
  // does. `bestEffortInk` is the non-strict fallback the Studio needs: a browser mid-edit
  // gets the most legible value available rather than a throw (lib/theme/cat-ink.js).
  //
  // THE LADDER IS MONOTONE BY CONSTRUCTION, and it did not used to be. `--text-body`,
  // `--text-secondary` and `--text-muted` are one ordered contract — design/theming.md
  // says muted is "Still quieter than --text-secondary", which is only meaningful if
  // secondary is in turn quieter than body. Solving each tier independently against the
  // canvas broke that in BOTH directions, measured on all four shipped starters:
  //
  //   · LIGHT: muted and secondary both landed on the AA floor from opposite
  //     mechanisms (`solveInk` walking the author's pale seed down, `ensureContrast`
  //     walking a body/muted mix down) and converged — dusk emitted them
  //     BYTE-IDENTICAL at #646c7a, ember/pine/slate within dE 0.0049.
  //   · DARK: worse, and invisible to a separation predicate because the two are far
  //     apart. `darkMuted` lifted the author's LIGHT-mode muted seed toward white, and
  //     on a near-black canvas that is the LOUDEST ink in the ramp — dusk's muted read
  //     10.30:1 against body's 7.46:1. The de-emphasis tier out-shouted the prose it
  //     exists to be quieter than, on every starter.
  //
  // So the two quiet tiers are now derived FROM the ladder rather than beside it:
  // muted is the quietest ink the canvas still allows, and secondary is placed between
  // body and muted. `mix` is monotone in OKLab L and contrast is monotone in luminance
  // on one side of a surface, so an ink between two AA-clearing inks clears AA too —
  // the ordering needs no repair pass, only the fallbacks below for a canvas pair that
  // straddles (bg light, bg-alt dark), where "one side" does not exist.
  const worstOn = (hex, bgA, bgB) => Math.min(contrastRatio(hex, bgA), contrastRatio(hex, bgB));
  /**
   * The muted TEXT tier: the author's own seed wherever it both clears AA and is
   * already quieter than body, else the quietest ink this canvas admits — walked from
   * `body` toward the canvas until one more step would break the floor.
   */
  const mutedTier = (seed, body, bgA, bgB) => {
    const solved = solveInk(seed, bgA, bgB) ?? bestEffortInk(seed, bgA, bgB);
    if (worstOn(solved, bgA, bgB) <= worstOn(body, bgA, bgB)) return solved;
    let lo = 0, hi = 1, best = null;
    for (let i = 0; i < 24; i++) {
      const mid = (lo + hi) / 2;
      const cand = mix(body, bgA, mid);
      if (worstOn(cand, bgA, bgB) >= AA + 0.05) { best = cand; lo = mid; } else hi = mid;
    }
    return best ?? solved;
  };
  const mutedTextLight = mutedTier(e.textMuted, bodyLight, e.bg, e.bgAlt);
  const mutedTextDark = mutedTier(darkMuted, darkBody, darkBgDeeper, darkBgAlt);
  t['text-muted'] = ld(mutedTextLight, mutedTextDark);
  /**
   * The secondary content tier sits BETWEEN body and the muted tier above — 45% of the
   * way toward quiet, the same fraction the old body/seed mix used. On a straddling
   * canvas the between-ness argument does not hold, so the AA repair stays as a net.
   */
  const secondaryTier = (body, muted, bgA, bgB, dir) => {
    const between = mix(body, muted, 0.45);
    if (worstOn(between, bgA, bgB) >= AA) return between;
    return ensureContrast(ensureContrast(between, bgA, AA, dir), bgB, AA, dir);
  };
  const secondaryLight = secondaryTier(bodyLight, mutedTextLight, e.bg, e.bgAlt, 'darken');
  const secondaryDark = secondaryTier(darkBody, mutedTextDark, darkBgDeeper, darkBgAlt, 'lighten');
  t['text-secondary'] = ld(secondaryLight, secondaryDark);
  // The decoration tier: the author's muted, repaired only to the GRAPHICAL floor, and only
  // when it misses it — so a palette whose muted already clears 3:1 keeps the author's value.
  const graphical = (seed, bg, bgAlt) => {
    const worst = (h) => Math.min(contrastRatio(h, bg), contrastRatio(h, bgAlt));
    if (worst(seed) >= AA_LARGE) return seed;
    const solved = solveInk(seed, bg, bgAlt);
    return solved ?? bestEffortInk(seed, bg, bgAlt);
  };
  const mutedMarkLight = graphical(e.textMuted, e.bg, e.bgAlt);
  const mutedMarkDark = graphical(darkMuted, darkBgDeeper, darkBgAlt);
  t['muted-mark'] = ld(mutedMarkLight, mutedMarkDark);
  ...
  t['scheme-dark-text-secondary'] = secondaryDark;
  t['scheme-dark-text-label'] = darkLabel;
  t['scheme-dark-text-muted'] = mutedTextDark;
  t['scheme-dark-muted-mark'] = mutedMarkDark;
  ...
  // ── Heatmap ramp ink ───────────────────────────────────────────────────────
  // The heatmap prints a value INSIDE a colored cell — the one placement the chart
  // family otherwise refuses, allowed here because the ramp is QUANTIZED so each
  // stop carries an ink solved against the fill actually under it. Why quantized,
  // and the dead zone that forces it, are in lib/theme/cat-ink.js §
  // solveHeatmapRamp. Emitted here for the same reason `cat-N-ink` is: a token read
  // through a var() fallback is a token that can go missing for a year and degrade
  // silently (the --cat-N-ink lesson, #1545). NON-STRICT, per this file's policy.
  //
  // THE HUE IS THE THEME'S OWN, and it has to be. An earlier cut solved against
  // chart-family.css's engine blue on the reasoning that a derived theme names no
  // `--chart-catN` cycle — which is false: `REQUIRED_TOKENS.chart` carries
  // `chart-cat1`..`chart-cat8` and this file assigns them above, so
  // `--chart-cat-1-hue: var(--chart-cat1, …)` resolves to the theme's, never the
  // fallback. Solving against the engine blue produced an ink for a color the
  // theme does not paint: measured over the four shipped starters, 16 of 40 pairs
  // below AA, worst 3.68:1 on `ember`. It reported a clean 4.65:1 the whole time,
  // because it was re-asking its own assumption.
  //
  // THIS BLOCK THEREFORE RUNS AFTER the chart spectrum above, not before it. That
  // ordering is load-bearing, not stylistic.
  const heatmapHue = { light: ldArm(t['chart-cat1'], 0), dark: ldArm(t['chart-cat1'], 1) };
  const heatRamp = solveHeatmapRamp({
    light: {
      hue: heatmapHue.light,
      // heatmap.styles.css --heatmap-base: 16% of the muted mark over the canvas.
      anchor: mix(e.bg, mutedMarkLight, 0.16),
      seed: bodyLight,
    },
    dark: {
      hue: heatmapHue.dark,
      anchor: mix(darkBgDeeper, mutedMarkDark, 0.16),
      seed: darkBody,
    },
    fillAt: (hue, anchor, pct) => mix(anchor, hue, pct / 100),
  });
  for (let i = 0; i < heatRamp.inks.length; i++) {
    t[`heatmap-step${i + 1}-ink`] = ld(heatRamp.inks[i].light, heatRamp.inks[i].dark);
    // The POSITIONS travel with the inks or the pairing is broken: an ink solved at
    // a nudged stop is wrong at the even one. They are plain numbers because they
    // serve both canvases — see solveHeatmapRamp on why that is load-bearing.
    t[`heatmap-step${i + 1}`] = `${Number(heatRamp.mixes[i].toFixed(2))}%`;
  }
