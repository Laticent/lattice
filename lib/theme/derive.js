...
function deriveTheme(essentials, options = {}) {
  ...
  //
  // THE HUE IS THE ENGINE'S, not the theme's, and that is correct rather than a
  // shortcut: a derived theme declares no `--chart-catN` cycle, so
  // `--chart-cat-1-hue` resolves to chart-family.css's own blue and that is the
  // color the ramp will actually paint. The ANCHOR and the INK SEED are the
  // theme's, because those are what change under it.
  const heatmapHue = { light: "#0A6CE0", dark: "#2E8BFF" }; // chart-family.css --chart-cat-1-hue
