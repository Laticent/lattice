 ...
 *
 * WHAT CHANGED, AND WHY THIS GATE DID NOT SHRINK. `hoistImports` now DROPS a quoted
 * import whose target a BROWSER would read as a bare theme name — decoding escapes,
 * removing a string's line continuation, and stripping surrounding whitespace, all
 * three, because getting only the first left `@import 'ghost ';` hoisted and fetched.
 * So the engine no longer promotes a dangling name into a fetch, for the `.zip` path
 * and every other producer, not just this one. That fixes the root cause one layer down; it
...
/**
 * Contract tokens whose absence RENDERS but LOSES A GUARANTEE.
 *
 * Distinct from ENGINE_DEFAULTED_TOKENS above, which is "no palette declares it,
 * and the engine declares a default at `:root`". These are the other shape: every
 * shipped palette DOES declare them, and the default is a `var(token, fallback)`
 * at the point of use rather than a declaration. A theme missing them paints —
 * so "the engine gives it no default, a render that reads it gets an invalid
 * value" is simply untrue of them, and saying it told an author their theme was
 * broken when it was not.
 *
 * What is lost is the CONTRAST GUARANTEE, not the render: the heatmap's fallback
 * ink is the body ink, which is solved against the canvas and not against a
 * saturated cell. So these stay findings — an author should fix them — but at
 * warning level, saying the true thing.
 */
const FALLBACK_ONLY_TOKENS = Object.freeze({
  'heatmap-step1': 'heatmap.styles.css reads it as `var(--heatmap-step1, <the even stop>)`, so the ramp renders evenly spaced.',
  'heatmap-step1-ink': 'heatmap.styles.css reads it as `var(--heatmap-step1-ink, …)`, so the value renders in the body ink.',
  'heatmap-step2': 'heatmap.styles.css reads it as `var(--heatmap-step2, <the even stop>)`, so the ramp renders evenly spaced.',
  'heatmap-step2-ink': 'heatmap.styles.css reads it as `var(--heatmap-step2-ink, …)`, so the value renders in the body ink.',
  'heatmap-step3': 'heatmap.styles.css reads it as `var(--heatmap-step3, <the even stop>)`, so the ramp renders evenly spaced.',
  'heatmap-step3-ink': 'heatmap.styles.css reads it as `var(--heatmap-step3-ink, …)`, so the value renders in the body ink.',
  'heatmap-step4': 'heatmap.styles.css reads it as `var(--heatmap-step4, <the even stop>)`, so the ramp renders evenly spaced.',
  'heatmap-step4-ink': 'heatmap.styles.css reads it as `var(--heatmap-step4-ink, …)`, so the value renders in the body ink.',
  'heatmap-step5': 'heatmap.styles.css reads it as `var(--heatmap-step5, <the even stop>)`, so the ramp renders evenly spaced.',
  // NOT the body ink, and heatmap.styles.css says so on the rule itself: a
  // saturated cell needs the opposite pole, so stop 5's fallback is an explicit
  // light/dark pair where stops 1-4 fall back to --text-body.
  'heatmap-step5-ink': 'heatmap.styles.css reads it as `var(--heatmap-step5-ink, light-dark(#FFFFFF, #0A1628))`, so the value renders in a plain light/dark pair rather than the body ink.',
});
...
function gateThemeCss(css, { knownThemes = [BASE_THEME] } = {}) {
  ...
  if (!composes) {
    ...
    for (const name of requiredTokenList()) {
      ...
      const fallback = FALLBACK_ONLY_TOKENS[name];
      if (why) {
        findings.push(finding('token-default', 'warning', `--${name} is not declared. The engine supplies a default, so this renders — but the derivation sets it explicitly. ${why}`));
      } else if (fallback) {
        findings.push(finding('token-fallback', 'warning', `--${name} is not declared. ${fallback} What is lost is the contrast guarantee, not the render — the fallback is not solved against the surface this token paints on.`));
      } else {
        findings.push(finding('token-missing', 'error', `--${name} is not declared, and the engine gives it no default — a render that reads it gets an invalid value (engineering/decisions/2026-08-10-no-safe-default-token-contract.md).`));
      }
    }
  }
