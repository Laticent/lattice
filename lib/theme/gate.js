 ...
 * `THEME_IMPORT_RE` (`lib/engine/css.js:51`) and `THEME_NAME_IMPORT_RE`
 ...
 * `hoistImports` (`lib/engine/css.js:216`) then lifts every surviving quoted
 * import to the TOP of the composed sheet, specifically so it survives the
 * "@import must come first" rule. A target the registry does not know is
 * therefore a live fetch, hoisted into first position, in a frame that holds the
 * user's BYOK key (HARD RULE #24).
 *
 * What makes that inert TODAY is that theme CSS is never first in the `<style>` —
 * `single-slide-render.ts` and `deck-preview.js` both prepend rules, so CSS
 * ignores the import. That is an accident of concatenation order, gated by
 * nothing, one refactor from being false. This is the gate.
 *
