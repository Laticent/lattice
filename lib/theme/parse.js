 ...
 *
 * Every token / root declaration carries `conditional`: whether it sits below an
 * at-rule block. See the note in the walk.
 *
...
function themeRecordView(record) {
  ...
  for (const node of list) {
    if (node.type === 'atrule') {
      ...
      //
      // Everything below an at-rule is marked CONDITIONAL. It is in the view, because
      // it is in the record — but a consumer reducing the record to one unconditional
      // value per token (`themeTokenMap`) has nowhere to put "…when the viewport is
      // dark", and taking the value anyway lets a `@media (prefers-color-scheme: dark)`
      // block — the most idiomatic thing a hand-editor adds, and later in the file —
      // win the light canvas.
      if (node.nodes) walk(node.nodes, true);
      continue;
    }
    ...
    for (const child of node.nodes) {
      ...
      const entry = { selector: node.selector, node: child, rule: node, conditional };
...
/**
 * The record's tokens as the FLAT `{ name: value }` map `lib/theme/contrast.js`
 * audits and `lib/theme/serialize.js` emits — i.e. what the browser would resolve
 * for a slide on the UNCONDITIONAL cascade, not what the file happens to list last.
 *
 * THREE ORDERINGS, AND ONLY ONE OF THEM IS SOURCE ORDER. Within one selector, a
 * token declared twice resolves to the later value, so source order decides — that
 * is `tokenMapBySelector`'s rule and it is right. Above it sit two axes CSS resolves
 * FIRST, and a flatten that skipped either would measure a palette nobody renders:
 *
 *   `!important`     beats everything below it, wherever it was written. Missing
 *                    this made `:where(:root){--accent:red !important}` lose to a
 *                    plain `:root{--accent:blue}` — checked against Chromium, which
 *                    resolves red.
 *   specificity      `:where(:root)` 0 — `serializeTheme` emits the `color-scheme`
 *                    default at this weight precisely so an author's `:root` wins;
 *                    `:root` 1, where a palette declares; `:root:root` 2, the
 *                    packed-reach idiom (#1797, retired for palette tokens by #1826,
 *                    still shipped for `color-scheme` in `themes/a11y-base.css`).
 *
 * A CONDITIONAL AT-RULE IS NOT IN THIS MAP AT ALL, and that is the third decision.
 * `themeRecordView` descends into `@media` / `@supports` on purpose — a token
 * declared there is part of the record and must not vanish from the view. But a flat
 * map has nowhere to put "…when the viewport is dark", and treating such a value as
 * THE value is worse than omitting it: a `@media (prefers-color-scheme: dark)` block
 * is the most idiomatic thing a hand-editor adds, it sits later in the file, and it
 * would have won every time (checked against Chromium in a light context, which
 * resolves the top-level value). The audit resolves both canvases from `light-dark()`
 * pairs, which is the mechanism this repo's themes actually use.
 *
 * Names come back WITHOUT the `--` prefix, which is the shape every consumer in
 * `lib/theme` already speaks.
 */
function themeTokenMap(record) {
  const out = {};
  const seen = new Map();
  for (const t of themeRecordView(record).tokens) {
    if (t.conditional) continue;
    // Importance outranks specificity outranks source order. Encoded as one number
    // so the comparison below stays a single `>=` — and `>=` so a later declaration
    // at the SAME weight wins (source order), while a lower weight never displaces a
    // higher one however late it appears.
    const weight = (t.node.important ? 1000 : 0) + rootSpecificity(t.selector);
    if (!seen.has(t.name) || weight >= seen.get(t.name)) {
      seen.set(t.name, weight);
      out[t.name] = t.node.value;
    }
  }
  return out;
}
...
/**
 * How strongly a root-ish selector binds: `:where(:root)` 0, `:root` 1,
 * `:root:root` 2, and so on.
 *
 * A SELECTOR LIST TAKES THE MAX OF ITS PARTS, because that is what the cascade does:
 * the specificity of a declaration is that of the matching selector, and when several
 * parts match the same element the strongest one is the one that counts. Reading the
 * whole list as one string got `:where(:root), :root` wrong in both directions —
 * 0 for that one (the `:where(` prefix swallowed the list) and 2 for
 * `:root, :where(:root)` (counting `:root` twice across the comma).
 */
function rootSpecificity(selector) {
  const clean = String(selector).replace(/\/\*[\s\S]*?\*\//g, ' ').trim();
  const parts = selectorParts(clean);
  let best = 0;
  for (const part of parts.length ? parts : [clean]) {
    if (/^:where\(/i.test(part)) continue; // zero, and zero cannot raise the max
    const compound = /^:is\(\s*(.+?)\s*\)$/i.exec(part)?.[1] ?? part;
    best = Math.max(best, (compound.match(/:root/gi) || []).length);
  }
  return best;
}

const { renameThemeDirective, themeDirectiveName } = require('./directive.js');

module.exports = {
  parseTheme,
  serializeThemeRecord,
  renameThemeDirective,
  themeDirectiveName,
  themeRecordView,
  tokenMapBySelector,
  themeTokenMap,
