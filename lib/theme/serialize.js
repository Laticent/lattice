...
function commentSafe(value) {
  ...
  for (const ch of String(value)) {
    ...
    const c = ch < ' ' || ch === ' ' || ch === ' ' ? ' ' : ch;
    // Tested against what has ALREADY been emitted, not against the source. Dropping a
    // slash shifts the bytes after it, so `**//` would leave a live `*​/` behind for any
    // check that reads the input — the same class of mistake as a naive comment strip.
