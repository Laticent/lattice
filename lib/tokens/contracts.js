...
const SANCTIONED_TOKEN_CONTRACTS = Object.freeze([
  ...
  { token: 'state-color', floor: GRAPHICAL_FLOOR, role: 'graphical',
    why: 'roadmap legend: a per-item status hue (--pass / --warn / --text-label), painted as the '
       ...
       + '(--muted-mark, --border) both clear this floor. The first was --text-muted until '
       + '#1715 split the role: a 4.5 ink standing in for a 3:1 shape, and one that missed even '
       + '3:1 on four palette-modes. --muted-mark IS the graphical tier, so the row is now true '
