 ...
 *           a radar polygon and its dots). Shapes only, because radar's container
 *           `<div>` also writes `data-series`, as a COUNT.
 *
...
const SVG_SHAPES = new Set(['path', 'circle', 'ellipse', 'rect', 'polygon', 'polyline', 'line', 'g', 'text']);
const ATTR_AXES = {
...
function resolveAttrAxisHtml(inner, axis, indices) {
  ...
  for (let i = 0; i < parts.length; i += 2) {
    parts[i] = parts[i].replace(tagRe, (tag, name, n) => {
      if (!allows(name.toLowerCase())) return tag;
      hit = true;
...
function resolveAxisDom(sec, group) {
  ...
  if (ATTR_AXES[axis]) {
    ...
    for (const el of sec.querySelectorAll(`[${attr}]`)) {
      if (!allows(el.tagName.toLowerCase()) || el.closest('template')) continue;
