...
const { labelSetFor, derivedFrom, liftLabelSet, resolveLabelSet } = require('../core/label-set');
 ...
 *  decides it. `neutralEmpty` true is the reading every transformer-keyed
 *  component uses today: `[ ]` is "not subject to this", a neutral state, not a
 *  failure. A component that reads `[ ]` as a hard fail (verdict-grid, pricing)
 *  draws no key yet; when one does, this is the line that has to learn it. */
const MARKER_STATE = Object.freeze(Object.fromEntries(
  MARKERS.map((m) => [`[${m}]`, stateClassesFor(m, true)]),
));
...
/** `section` open tags carrying one of the served component classes. */
...
function applyToHtml(html) {
  ...
  return html.replace(SECTION_RE, (section) => {
...
function applyToDom(root) {
  ...
  for (const component of SERVED) {
    for (const section of root.querySelectorAll(`section.${component}`)) {
      ...
      if (!table) continue;
      // The authored set rides in a one-code paragraph, exactly as on the HTML
      // path. Reading it from the live DOM rather than re-serializing the
      // section keeps this walk free of a string round trip.
      let authored = null;
      for (const p of section.querySelectorAll('p')) {
        ...
        const parsed = liftLabelSet(`<p><code>${code.textContent}</code></p>`);
        if (parsed.set) { authored = parsed.set; p.remove(); break; }
      }
