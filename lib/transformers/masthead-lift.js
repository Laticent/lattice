...
function transformMastheadDom(root) {
  ...
  for (const sec of root.querySelectorAll('section.form:not(section section)')) {
    ...
    // A section that already carries its FOOTER CELL is a composed frame, not a flow body
    // awaiting the lift. The split envelope builds its cover and closing pages that way
    // (lib/core/split-envelope.js): cells assembled, no stage, after the render-time lift has
    // run. The CLI never lifts them again; a live preview's runtime does, and it swept the
    // cover's own footer into a new stage while the pagination Tile added a second footer, so
    // every split cover in the Playground and the Studio showed a stray page number mid-slide.
    // Mirrored in the string kernel (masthead.transform.js), so the two paths agree.
    if (sec.querySelector(':scope > .cell-footer')) continue;
    ...
    if (wrap) {
      ...
      // The frame's coda Cell is held OUT of the stage, mirroring the string
      // kernel's `peelCoda` at the same site (HARD RULE #1) — and for the running
      // footer's reason: it belongs to the section column, not the component's.
      // Inside the stage it inherited the component's own vertical alignment and
      // the band moved with `align-top` / `align-middle` / `align-bottom`; as the
      // stage's sibling it is the column's last content row, so the stage's
      // `flex: 1 1 auto` absorbs the slack and the band sits at content height
      // above the footer on every layout.
      let codaCell = null;
      // The page number is held out too, and it is the one piece of section chrome that
      // can already be PRESENT when this runs. Since #2206 the pagination Tile mints a
      // section-level `<span class="lat-pagination">` on any paginated frame without one
      // — and on this path the document is often ALREADY engine-rendered (the Studio /
      // Playground preview, a `--player` export, the fluid viewer all load a document the
      // engine's Tile has been through), so the span is a sibling here rather than
      // something a later pass adds. Swept into the stage it becomes in-flow body copy,
      // AND the footer Cell below mints a second one: the slide then draws the number
      // twice, once in the corner and once in the middle. The Tile's own idempotence
      // guard cannot repair it — that guard is a descendant query, so a swept span still
      // counts as present and the Tile no-ops.
      //
      // The realistic trigger is the runtime's deck-wide `class:` / `form:` registers,
      // which arrive from a one-shot async source-`.md` fetch: an early pass can run
      // before the register resolves (not wrappable → the Tile mints a section-level
      // span) and a later pass after it (now wrappable → this sweep). A live `_class:`
      // edit in the Studio opens the same window. Found by the maker-checker pass on
      // #2206; the string kernel has no equivalent hole, because there the Tile runs
      // after the lift rather than possibly before it.
      let pageNo = null;
      // With no band, the sweep starts at the section's first child, so a leading
      // Marp running <header> is stepped over here: the string kernel's
      // `extractHeader` keeps it at section level before the stage, and a
      // component that owns its title inside a card (wifi's `.qr-head`, video's
      // `.video-head`) builds no band for the header to precede.
      const isBlank = (n) => n.nodeType === 3 && !n.nodeValue.trim();
      let node = band ? band.nextSibling : sec.firstChild;
      if (!band) {
        let lead = node;
        while (lead && isBlank(lead)) lead = lead.nextSibling;
        if (lead && lead.tagName === 'HEADER') node = lead.nextSibling;
      }
      // A footer is TRAILING when only whitespace follows it, the string kernel's
      // `\s*$`. A bare `!next` missed the newline the card rebuilders keep after
      // `</footer>`, and the footer was swept into the stage.
      const onlyBlankAfter = (n) => {
        for (let m = n.nextSibling; m; m = m.nextSibling) if (!isBlank(m)) return false;
        return true;
      };
      while (node) {
        const next = node.nextSibling;
        if (node.tagName === 'FOOTER' && onlyBlankAfter(node)) { trailingFooter = node; break; }
        if (node.nodeType === 1 && node.classList?.contains('cell-coda')) { codaCell = node; node = next; continue; }
        if (node.nodeType === 1 && node.classList?.contains('lat-pagination')) { pageNo = node; node = next; continue; }
        cell.appendChild(node); // moves node out of the section into the cell
        node = next;
      }
      sec.appendChild(cell);
      if (codaCell) sec.appendChild(codaCell); // after the stage, before the footer cell
      ...
      if (trailingFooter || pagination) {
        ...
        if (pagination) {
          // REUSE the span the Tile already minted rather than adding a second one. The
          // frame has a footer Cell now, so the number's home is this band — moving the
          // existing node there is what keeps the count at one, and it carries whatever
          // value the attribute already agreed with (a split run's `2.3`, not a fresh 2).
          const pag = pageNo || doc.createElement('span');
      ...
      // A page number with no footer Cell to move into stays a section child, exactly
      // where the Tile put it — `paginate: false` on a frame that still has footer text
      // is the shape that reaches here.
      if (pageNo) sec.appendChild(pageNo);
    }
  }
}
