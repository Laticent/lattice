...
function transformMastheadDom(root) {
  ...
  for (const sec of root.querySelectorAll('section.form:not(section section)')) {
    ...
    if (wrap) {
      // Collect the body nodes BEFORE creating the cell: the cell's TAG depends on
      // what goes in it (`<figure>` when a `<figcaption class="chart-caption">` is
      // among them — engine.stageTag), and an element's tag can't be changed after
      // the fact without re-creating it. Mirrors the string kernel, which sees the
      // whole body before it picks the tag.
      ...
      const isFigure = bodyNodes.some(
        (n) => n.tagName === 'FIGCAPTION' && n.classList?.contains('chart-caption'),
      );
      const cell = doc.createElement(isFigure ? 'figure' : 'div');
