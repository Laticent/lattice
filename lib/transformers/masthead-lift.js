...
function transformMastheadDom(root) {
  ...
  // `:not(.frame-sovereign)` — every slide carries `form` now, so the Frame is what
  // decides whether a band is built. A sovereign Frame owns its own grid.
  for (const sec of root.querySelectorAll('section.form:not(.frame-sovereign):not(section section)')) {
...
module.exports = {
  name: 'masthead-lift',
  selector: 'section.form:not(.frame-sovereign)',
  applyToHtml(html) {
