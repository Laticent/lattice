...
function transformMastheadDom(root) {
  ...
  for (const sec of root.querySelectorAll('section.form:not(section section)')) {
    ...
    if (wrap) {
      // Collect the body nodes BEFORE creating the cell: the cell's TAG depends on what
      // goes in it, and an element's tag cannot be changed after the fact. Mirrors the
      // string kernel's `buildStageCell`, which likewise sees the whole body first.
      ...
      // MUST match `buildStageCell` exactly (HARD RULE #1). The predicate is: the FIRST
      // or LAST element child is a `<p class="chart-caption">`. First-or-last is the
      // spec's own content model for `<figure>` — a figcaption in the middle is invalid,
      // which is the shape `state-chart` produces, so it correctly stays a `<div>`.
      // An earlier split version derived this from `classList` here and from a regex
      // there; they disagreed on four real inputs.
      const els = bodyNodes.filter((n) => n.nodeType === 1);
      const isCap = (n) => !!n && n.tagName === 'P' && n.classList?.contains('chart-caption');
      const cap = [els[els.length - 1], els[0]].find((n) => els.length >= 2 && isCap(n));
      const cell = doc.createElement(cap ? 'figure' : 'div');
      cell.className = 'cell-stage';
      if (cap) {
        // Retag the caption in place, then NAME the figure with its text. Chrome maps
        // `<figure>` to a PDF `/Figure` whose `/Alt` comes from the accessible name, and
        // a nameless one is a PDF/UA failure that can swallow the chart inside it.
        const fc = doc.createElement('figcaption');
        for (const a of [...cap.attributes]) fc.setAttribute(a.name, a.value);
        while (cap.firstChild) fc.appendChild(cap.firstChild);
        cap.parentNode.replaceChild(fc, cap);
        bodyNodes[bodyNodes.indexOf(cap)] = fc;
        const label = (fc.textContent || '').replace(/\s+/g, ' ').trim();
        if (label) cell.setAttribute('aria-label', label);
      }
      for (const n of bodyNodes) cell.appendChild(n); // moves each out of the section
