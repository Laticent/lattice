...
// SPATIAL charts — an absolutely-positioned / hybrid HTML+SVG layout (a spiral word
// cloud, a swim-lane journey, a node-and-edge state graph) that does NOT linearize to a
// static figure or clean prose: re-hosting the bare SVG either strands content (orphan
// edge arrows, a lone mood glyph) or overflows and overlaps (the packed word cloud). They
// get the honest "best seen in Present / Read·Slides" placeholder instead of a broken
// re-host — the same treatment gantt / kanban / progress / roadmap / timeline-list reach
// by having no re-hostable SVG at all. (A richer prose projection for these is later work.)
const SPATIAL_PLACEHOLDER_COMPONENTS = new Set(['journey', 'state-chart', 'word-cloud']);
...
function projectMedia(stage, heading, component) {
	...
	// custom-property colours resolve outside `section.chart-frame`. A `diagram` (mermaid)
	// figure is HELD at prose width (`lp-figure-hold`): its node fills are still `section `-scoped
	// (black outside the slide — a separate re-host fix, tracked), so breaking it wider would only
	// enlarge that defect. Charts / math / images break out (see the breakout grid in player-core).
	const cls = CHART_TOKEN_COMPONENTS.has(component)
		? 'lp-figure chart-frame'
		: component === 'diagram'
			? 'lp-figure lp-figure-hold'
			: 'lp-figure';
