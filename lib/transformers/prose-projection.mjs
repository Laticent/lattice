...
function withRostersSwapped(stage, make) {
	...
	for (const ul of clonedRosters) {
		...
		const replacement = make(people, ul);
...
function speakTeamProfile(stage, eyebrow, wordMap = null) {
	...
	const clone = withRostersSwapped(stage, (people) => {
		const said = people.map(personSentence).filter(Boolean).join(' ');
		if (!said) return null;
...
function projectTeamProfile(stage, eyebrow) {
	...
	const clone = withRostersSwapped(stage, (people) => {
		const rows = people.map((li) => {
			...
			// labels — inline pills, marks and emphasis survive. `<strong>` wraps the
			// name only when there IS a name: a person whose only line is an image must
			// not emit an image inside a `<strong>`.
			const head = [
				name?.textContent.trim() ? `<strong>${name.innerHTML.trim()}</strong>` : '',
