 ...
 * the stage, where the body walker already speaks it; `stage.contains` is what stops that slide
 * hearing its coda twice.
 */
function speakCoda(section, body) {
	...
	for (const el of coda.children) {
		...
		// ALREADY SPOKEN? Skip it. This asks the question directly rather than inferring it from
		// DOM position. The first version tested `stage.contains(coda)`, which is only equivalent
		// while the component's walker happens to be `speakGeneric` — it selects section-wide, so
		// a stage-less slide's coda is picked up there. `speakStats`, `speakBigNumber` and
		// `speakQuote` do NOT walk it, so on a stage-less slide of those components the old guard
		// returned '' and the coda went silent again, with the guard looking like it worked. No
		// committed deck hits that today; the predicate was coupled to a table it never referenced.
		if (!text || said.includes(text)) continue; // `said` is always a string, so no guard is needed
		blocks.push(terminate(text));
	}
	return blocks.join('\n\n');
}
