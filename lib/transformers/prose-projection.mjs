...
function reidClone(html) {
	...
	// The prefix is NOT always `lat-svg`: the minter lengthens it (`lat-x0-svg`, …) when
	// the document already contains the token, so a hardcoded prefix would silently miss
	// exactly the decks that needed collision avoidance in the first place.
	const MINTED = /lat-(?:x\d+-)?svg[td]-\d+/;
	return html
		.replace(/\bid="(lat-(?:x\d+-)?svg[td]-\d+)"/g, 'id="$1-a"')
...
function projectGeneric(stage, eyebrow) {
	// `figcaption` is in this list because a chart caption IS one whenever its stage is a
	// `<figure>` (masthead.transform.js `buildStageCell`). Without it, `matrix-grid` —
	// a chart layout that falls through to this generic path — silently DROPPED its
	// legend from the reading view: the line telling the reader what the marks mean
	// simply vanished. A tag-keyed block list is the same footgun as a tag-keyed parser.
