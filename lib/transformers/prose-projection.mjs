...
// Exported so `test/unit/core/slide-absorption.test.js` can pin the absorption kernel's copy
// against this one by IDENTITY rather than by scraping this file's source. The kernel cannot
// import it — `.dependency-cruiser.cjs`'s `kernel-no-transformer-imports` bars `lib/core/` from
// reaching `lib/transformers/` — so the test is the seam, exactly as `pace-names.test.js` is the
// seam between the two copies of the pace presets.
