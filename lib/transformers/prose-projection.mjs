...
export function projectDeckToProse(sections) {
	...
	sections.forEach((section, idx) => {
		...
		else if (component === 'team-profile') body = projectTeamProfile(stage);
...
/**
 * team-profile — a roster of PEOPLE, spoken one person per sentence.
 *
 * Two things make the generic walker wrong here, and the second is the one that
 * matters. `speakGeneric` falls back to `speechText`, which is bare `textContent`:
 *
 *   1. The card is a flat run of SPANS — `.person-name`, `.person-role`,
 *      `.person-note` — with no whitespace between them, because they are laid out
 *      by flex rather than separated in the markup. `textContent` concatenates
 *      them, so a perfectly good roster narrated
 *      "Ada OkaforExecutive SponsorClears blockers above the program."
 *
 *   2. The MONOGRAM leaks in. A person with no headshot gets their initials drawn
 *      in the portrait cell, marked `aria-hidden="true"` precisely because the name
 *      is read out beside it — but `textContent` does not honor `aria-hidden`, so a
 *      photo-less roster narrated "AOAda Okafor…", spelling out initials nobody
 *      asked to hear. Reading the named spans directly skips it structurally, which
 *      is why this speaker needs no `aria-hidden` filter (and why one is not added
 *      to the shared `speechText` — that would change every component).
 *
 * EVERY note is read, not the first. A person carries one `.person-note` span PER
 * note line, and an author who writes the role as plain text instead of backticks
 * gets two — a shape the component's own `commonMistakes` list names. Reading only
 * `querySelector('.person-note')` therefore dropped a whole authored line, which is
 * WORSE than the concatenation it replaced: that was ugly but complete, this was
 * silent loss. Caught by a checker, against a deck `lint:deck` calls clean.
 *
 * The roster is found at ANY DEPTH, and every other block still reaches the generic
 * sentence path. Walking `stage.children` for `ul.team-roster` alone looked tidy and
 * had two failure modes: a wrapper between the stage and the roster (a second
 * component class puts one there) made this return null and fall back to the very
 * bug it fixes, and a mid-slide `blockquote` or `table` — which `speakGeneric`
 * speaks — was silently skipped. So rosters are collected wherever they are, and
 * anything else the generic walker would have said is still said, in document order.
 */
function speakTeamProfile(stage, eyebrow, wordMap = null) {
	const rosters = [...stage.querySelectorAll('ul.team-roster')];
	if (!rosters.length) return null;
	const sentence = (li) => {
		const name = speechText(li.querySelector('.person-name'));
		const role = speechText(li.querySelector('.person-role'));
		// ALL note spans, in order — see the note above about the two-note shape.
	...
	// Blocks the generic walker speaks, so a mid-stage quote or table is not lost.
	const OTHER = 'p, ol, dl, blockquote, table, figcaption, h3, h4';
	const out = [];
	const seen = [];
	for (const el of [...stage.querySelectorAll(`ul.team-roster, ${OTHER}`)]) {
		if (el.closest?.(SKIP_SELECTOR)) continue;
		if (seen.some((e) => e.contains(el))) continue; // inside a roster or a table already said
		if (el.tagName === 'UL') {
			const said = [...el.querySelectorAll('li.person')].map(sentence).filter(Boolean);
			seen.push(el);
			if (said.length) out.push(said.join(' '));
			continue;
		}
		if (el.tagName === 'P' && eyebrow && speechText(el) === eyebrow) continue; // lifted as kicker
		seen.push(el);
		if (el.tagName === 'TABLE') {
		...
		const t = speechText(el);
		if (t) out.push(terminate(t));
	}
	return out.length ? out.join('\n\n') : null;
}

/**
 * team-profile for READ·ARTICLE — the prose surface, which is a DIFFERENT function
 * from the speech one above and was missed the first time this was fixed.
 *
 * `projectDeckToProse` powers Read·Article in the self-contained `.html` player;
 * `projectDeckToSpeech` powers captions and Studio Present. Fixing only the second
 * left the article rendering the raw span run — "AOAda OkaforExecutive Sponsor…",
 * monogram initials and all — while the commit that shipped it claimed the surface
 * was fixed. It was not; a checker drove the real player and read it back.
 *
 * A roster in an article is a list of people, so it stays a `<ul>`: the name in
 * `<strong>`, the role after an em dash, the notes as their own sentences. Inline
 * markup inside the notes (pills, `<em>`, state marks) is preserved with `innerHTML`
 * of already-sanitized nodes, the way `projectStats` keeps its labels.
 */
function projectTeamProfile(stage) {
	const rosters = [...stage.querySelectorAll('ul.team-roster')];
	if (!rosters.length) return null;
	// Same block set the generic projector re-hosts, so a mid-slide quote, table or
	// lead-in paragraph is carried into the article rather than dropped — the exact
	// omission this function was written to fix, one surface over.
	const parts = [];
	const seen = [];
	for (const el of [...stage.querySelectorAll('ul.team-roster, p, ol, dl, blockquote, table, figcaption, h3, h4')]) {
		if (el.closest?.(SKIP_SELECTOR)) continue;
		if (seen.some((e) => e.contains(el))) continue;
		seen.push(el);
		if (el.tagName !== 'UL' || !el.classList.contains('team-roster')) { parts.push(el.outerHTML); continue; }
		const rows = [...el.querySelectorAll('li.person')].map((li) => {
			...
			const head = [
				name ? `<strong>${name.innerHTML.trim()}</strong>` : '',
				role ? `<em>${role.innerHTML.trim()}</em>` : '',
			].filter(Boolean).join(' — ');
			return `<li>${[head, ...notes].filter(Boolean).join(' — ')}</li>`;
		}).filter(Boolean);
		if (rows.length) parts.push(`<ul class="lp-roster">${rows.join('')}</ul>`);
	}
