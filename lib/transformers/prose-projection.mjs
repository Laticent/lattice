 ...
 *
 * THE CODA IS NOT AN EMPHASIS SOURCE, though it was one until measurement said otherwise. A coda is
 * appended LAST, so a single-block coda's span always ends in the slide's final cue — and the hold is
 * spent in the gap AFTER a cue, while `CaptionTrack.durationMs` is the last cue's end. The gap after
 * the final cue is therefore discarded, and every ordinary coda bought exactly zero. Verified rather
 * than reasoned: deleting this source left `examples/emphasis-narration.vtt` byte-identical, on the
 * deck whose whole job is to demonstrate the feature. Only a two-block coda ever held, in the seam
 * between its own blocks.
 *
 * Making it count is a real change and a separate one: it means putting silence after a slide's last
 * cue, which the `.vtt` deriver has never carried (`read-along-vtt.js` advances by `durationMs`) while
 * the baked player already does hold `g` on the final cue. Those two disagree today, before this
 * feature; adding a per-deck quantity that lands exactly on that seam is not something to do at a
 * merge gate. The coda's win here is that it is SPOKEN AT ALL, which is the larger half.
 *
 ...
 */
const EMPHASIS_SOURCES = [{ selector: 'strong, b', weight: 2 }];
