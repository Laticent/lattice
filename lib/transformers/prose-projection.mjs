...
function withRostersSwapped(stage, make) {
	...
	for (const ul of clonedRosters) {
		...
		//
		// Stripping the decoration HERE rather than teaching `speechText` to honor
		// `aria-hidden` is a deliberately narrow choice. Honoring it in the kernel is the
		// correct general answer — a speech projection is exactly the consumer
		// `aria-hidden` is written for — but seventeen modules emit the attribute, most
		// of them charts, so that change re-opens every component's narration and is not
		// this component's to make. Logged as a follow-up; this line closes the leak on
		// the one path team-profile owns.
