 ...
 */
function speakCoda(section, stage) {
	const coda = section.querySelector('.cell-coda');
	if (!coda || stage.contains(coda) || coda.closest(SKIP_SELECTOR)) return '';
	const blocks = [...coda.children].map((el) => terminate(speechText(el))).filter(Boolean);
	return blocks.length ? blocks.join('\n\n') : terminate(speechText(coda));
}
...
export function projectDeckToScript(sections) {
	return sections.map((section) => {
		...
		const coda = speakCoda(section, stage);
