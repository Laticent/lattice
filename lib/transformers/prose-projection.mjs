...
function reidClone(html) {
	...
	return html
		.replace(/(\sid=")([^"]*)(")/g, (m, open, id, close) => (defined.has(id) ? open + suffixed(id) + close : m))
		.replace(/\b(aria-labelledby|aria-describedby)="([^"]*)"/g, (_m, attr, refs) => {
			const next = refs.split(/\s+/).map((r) => (defined.has(r) ? suffixed(r) : r)).join(' ');
			return `${attr}="${next}"`;
		})
		// Every SVG reference form lands here — `fill`, `stroke`, `mask`, `filter`, `clip-path`, and
		// the same written inside a `style="…"`. Matching the `url(#…)` token rather than the
		// attributes that can carry it means a new one cannot be missed.
