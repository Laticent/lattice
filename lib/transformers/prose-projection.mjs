...
/**
 * Compose a "label: value" narration phrase WITHOUT doubled punctuation. A label
 * that already ends in a sentence terminator or separator — an authored card title
 * ("Write.", "Choose a component."), a lead ending in a dash ("Design —") or a stray
 * colon ("Q3:") — drops that trailing glyph before the colon is added, so the caption
 * reads "Write: value", never "Write.: value". An empty label yields the value alone.
 * The single home for every label:value join below, so the rule can't drift between
 * the stats / table / list / definition-list walkers.
 */
