 ...
 *      asked to hear. Reading the three named spans directly skips it structurally,
 *      which is why this speaker does not need an `aria-hidden` filter (and why one
 *      is not added to the shared `speechText` — that would change every component).
 *
 * Order is the card's reading order, not the DOM's paint order: name, then role,
 * then the note as its own clause. `sides` interleaves `h3` labels between its two
 * rosters, so blocks are walked in document order rather than by querying the
 * rosters alone — otherwise "Your team" and "Our team" would be dropped, or worse,
 * both rosters would read as one undifferentiated list of six.
 */
function speakTeamProfile(stage) {
	const people = [...stage.querySelectorAll('li.person')];
	if (!people.length) return null;
	const sentence = (li) => {
		const name = speechText(li.querySelector('.person-name'));
		const role = speechText(li.querySelector('.person-role'));
		const note = speechText(li.querySelector('.person-note'));
		// "Ada Okafor, Executive Sponsor: Clears blockers." A person with no role or
		// no note simply drops that part — never an empty clause or a stray colon.
		const who = [name, role].filter(Boolean).join(', ');
		if (!who) return terminate(note);
		return terminate(note ? labelValue(who, terminate(note)) : who);
	};
	const out = [];
	for (const el of [...stage.children]) {
		if (el.closest?.(SKIP_SELECTOR)) continue;
		if (el.tagName === 'UL' && el.classList.contains('team-roster')) {
			const said = [...el.children].filter((li) => li.classList.contains('person')).map(sentence).filter(Boolean);
			if (said.length) out.push(said.join(' '));
			continue;
		}
		// A `sides` roster label, or a lead-in sentence above the roster.
		if (['H3', 'H4', 'P'].includes(el.tagName)) {
...
export function projectDeckToScript(sections) {
	return sections.map((section) => {
		...
		else if (component === 'team-profile') body = speakTeamProfile(stage) || speakGeneric(stage, eyebrow, wordMap);
