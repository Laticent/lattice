...
//
// `[aria-hidden="true"]` is the LAST entry and the one that is not chrome: a speech
// projection is precisely the consumer that attribute is written for, so anything the
// engine has marked "not for assistive technology" must not be narrated either. Whether
// that is safe was measured rather than assumed (#2115): rendering fifteen decks that
// exercise every one of the seventeen emitting modules, exactly three aria-hidden shapes
// carry text a speech walker can reach, and all three are decoration duplicated by a
// visible sibling — journey's actor-dot initial (the label follows it, which is how the
// shipped `.vtt` read "Pprospect"), team-profile's monogram (the name is in
// `.person-name`), and KaTeX's `.katex-html` (the same expression is also the MathML and
// the TeX annotation). Every other emitter puts the attribute on an empty node or on an
// SVG `<g>` no walker selects. No aria-hidden element was found to be the ONLY carrier of
// its content.
//
// It changes the ARTICLE path barely at all, by construction: `projectGeneric` tests
// `closest(SKIP_SELECTOR)` on the top-level blocks it selects and then emits their whole
// `outerHTML`, so a `<p>` keeps the KaTeX inside it; only a block that is itself hidden
// (the zero-size `.latt-a11y-defs` texture defs, a decorative `<img>`) stops being
// re-hosted, which is the right answer for both. `projectMedia` never consulted this
// selector, so a chart's own SVG is untouched.
const SKIP_SELECTOR = 'header, footer, .cell-footer, .masthead-bay, .lat-pagination, aside, script, style, .lattice-notes, .lattice-description, [aria-hidden="true"]';
 ...
 *
 * SKIP_SELECTOR alone cannot do this. The walkers test `closest(SKIP_SELECTOR)` on whole
 * BLOCKS, and the decoration that leaks is a `<span>` INSIDE a block that must still be
 * read: journey's actor dot sits in the same `<li>` as its label, so `textContent` ran the
 * two together and the shipped caption said "Pprospect" (#2115). The exclusion has to
 * happen where the text is collected.
 *
...
function speechText(el) {
	...
	const hidden = el.querySelector ? el.querySelector('[aria-hidden="true"]') : null;
	if (!hidden) return el.textContent.replace(/\s+/g, ' ').trim();
	const clone = el.cloneNode(true);
	for (const dec of clone.querySelectorAll('[aria-hidden="true"]')) dec.remove();
...
function speakCoda(section, body) {
	...
	if (!coda || coda.closest(SKIP_SELECTOR)) return '';
	const said = String(body || '');
	const blocks = [];
	for (const el of coda.children) {
		if (el.closest(SKIP_SELECTOR)) continue; // a chrome node docked inside the coda cell
		...
		const clone = el.cloneNode(true);
		for (const n of clone.querySelectorAll(SKIP_SELECTOR)) n.remove();
