...
const trackedChanges = require('./tracked-changes');
...
const TRANSFORMERS = [
  ...
  belowNote,
  // After everything, including belowNote: this one is purely ADDITIVE inline
  // markup (a visually-hidden edge span either side of each `<ins>`/`<del>`),
  // and every structural transform above should make its decisions about
  // trailing elements and label paragraphs on unadorned content. Running last
  // also means no later pass can split a section between an edge span and the
  // element it labels. See lib/transformers/tracked-changes.js.
  trackedChanges,
];
