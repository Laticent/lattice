...
const topicTrack     = require('./topic-track');
...
const teamProfile    = require('./team-profile');
const labelSetKey    = require('./label-set-key');
...
// transformer.)
const TRANSFORMERS = [
  // FIRST, and the position is the point: the universal coda kernel lifts a
  // slide's trailing editorial beats (key insight, below-note) into the frame's
  // own `.cell-coda` cell BEFORE any structural transform can swallow or delete
  // them. The rebuilders below keep clear of that cell two ways: five peel it off
  // and re-append it (`peelCoda`), and compare-code's DOM arm steps over it with
  // `isFrameCell`. See lib/core/coda.js and
  // engineering/decisions/2026-08-24-universal-coda-cell.md.
  coda,
  chartFamily,
  splitPanels,
  // Sovereign frame (lib/forms/frame/premise) — rebuilds the section into
  // .premise-claim + the untouched <ul> before mastheadLift ever sees it,
  // same reasoning as splitPanels above.
  premise,
  // Sovereign frame (lib/forms/frame/topic) — derives each `topic` slide's
  // sibling track from the deck's own topic headings, the way
  // lib/forms/tile/progress does for sections one level up. It only APPENDS a
  // `<ul class="tile-track">`, so it neither rebuilds the section nor competes
  // with the cell builders; its position here keeps it with the other sovereign
  // kernels. `topic` is chrome-exempt, so mastheadLift never sees it and the
  // relative order of the two is immaterial. See lib/transformers/topic-track.js.
  topicTrack,
  // `contact` and `wifi` REBUILD their whole section from the raw postfix-key
  // list (renderCard replaces the section inner with a fresh `.qr-card`), so they
  // must run BEFORE mastheadLift — like the other structural rebuilders
  // chartFamily / splitPanels above. Both opted into `conformance:"strict"`
  // (model-driven frame render §4), so mastheadLift wraps their finished card in
  // the frame's `.cell-stage` cell; if they ran AFTER mastheadLift they would
  // wipe that wrapper (and the `.cell-footer`) when they rebuild. The frame stays
  // the sole cell-builder (HARD RULE #1) — the component only builds the card
  // that fills the stage. wifi additionally needs the depth-aware masthead h2
  // extraction (findTopLevelH2): it emits its own title as an in-card `.qr-head >
  // h2`, which must NOT be yanked into a masthead band.
  contact,
  wifi,
  ...
  // iframe), rebuilding the section into one `.video-card`. It is
  // `conformance:"strict"` like contact/wifi, so it runs above mastheadLift for
  // the same reason: the lift wraps the finished card in `.cell-stage`, and a
  // rebuild AFTER the lift would discard that cell. Its title rides nested in the
  // card (`.video-head > h2` / `.video-lead > h2`), so the depth-aware lift leaves
  // it there instead of pulling it into the band away from its lead.
  video,
  ...
  imageAdaptive,
  imageScrim,
  // Rewrites `logo-wall` marks from `<img>` to token-colored `.logo-mark`
  ...
  logoMarks,
  // Rebuilds `team-profile`'s authored people list (`- Name` + nested portrait /
  // role / note) into flat `.person` cards, drawing a token-colored monogram for
  // anyone with no photo. Disjoint section set (section.team-profile) from every
  // other transformer, so position is immaterial to correctness; it sits beside
  // logoMarks because both answer "the author supplied an asset, we supply the
  // color". It must precede `focus` and `pillTag` (both below), which read the
  // FINAL `<li>` / `li > code` shape — and it is the pass that changes that shape.
  teamProfile,
  ...
  // (`wifi`/`contact` — the QR-card connect components — run earlier, above
  // mastheadLift, because they are `conformance:"strict"` and must be wrapped in
  // their stage cell by the frame.)
  qrGeneral,
  // `scene` component (imagery bucket): wraps an inline Anima poster `<svg>` in
  // `.scene-figure` + the heading/caption in `.scene-text` for the image-mirrored
  // compositions. Disjoint section set (section.scene); position immaterial. The
  // inline svg (not a background-image) is what makes the poster recolor + bake.
  scene,
  ...
  build,
  // Draws the key for a component that DECLARES a label set but has no section
  // transform of its own to build one (obligation-matrix today, by its
  // manifest's `labelSet.keyedBy`). Position: after every structural transform,
  // because it reads the cells' own `state {sem} {shape}` classes to learn which
  // members the grid carries — those are stamped by the markdown-it cell rule, so
  // they are already in place, but a rebuilder that moved the table would strip a
  // key inserted earlier. Disjoint section set from everything above it.
  labelSetKey,
  ...
  pillTag,
  // `belowNote` USED TO RUN HERE, truly last, wrapping a layout's trailing `<p>`
  // once every structural transform above had settled what the section's final
  // trailing element was. The universal coda kernel at the TOP of this list now
  // makes that decision — for both trailing beats at once, on the AUTHORED body,
  // before a rebuilder can move or delete the node. Running last was the best a
  // pass could do while it had to cope with whatever the transforms left behind;
  // it is strictly worse than not being swallowed in the first place, and it could
  // never help the three components that rebuild their section outright (contact,
  // wifi, video), because by then there was nothing to wrap.
  //
  // The KERNEL (lib/core/below-note.js) stays: it owns the `no-note` opt-out the
  // coda reads, and `extractStage`, which lib/core/split-envelope.js uses. Only the
  // registry pass is retired — two mechanisms for one concept is what this change
  // exists to end (HARD RULE #1).
];
