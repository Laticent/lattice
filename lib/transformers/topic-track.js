...
/**
 * Read one attribute off a tag, walking name=value pairs.
 *
 * NOT `section-walk`'s `readAttr`, whose `(?:^|\s)name="([^"]*)"` is QUOTE-BLIND:
 * it also matches the text INSIDE another attribute's value, so
 * `<ul data-note=' class="tile-track"'>` read as one of ours. That made the
 * string arm treat an ordinary author list as an emitted track — dropping the
 * slide's band AND its name from every sibling's scale — while the DOM arm's
 * `:scope > ul.tile-track` was unmoved. Measured: a three-topic section rendered
 * `[[Beta,Gamma],[Beta,Gamma]]` on one arm and all three names on the other.
 *
 * Walking pairs closes it because a quoted value is consumed whole. This is the
 * same reader the authored-marker machinery carried before it was deleted, kept
 * because the hazard it was written for did not leave with it (#1358's family).
 */
/**
 * ANCHORED, and that is the whole correctness of it. A first cut named the
 * attribute `[a-zA-Z_:][-a-zA-Z0-9_:.]*` and matched it anywhere in the tag, so a
 * name carrying a character outside that set — `@class`, `9class`, `[class]`, the
 * shapes a framework template pastes in — could not match as a NAME, the walk
 * resynced INSIDE it, and the `class` tail matched as an attribute of its own.
 * `<ul @class="tile-track">` then read as our emitted track on the string arm
 * while the DOM (whose attribute really is named `@class`) said no: the slide
 * lost its band and its name left every sibling's scale. That is the same split
 * the quote-blind reader before it caused, on three new keys.
 *
 * So a name may start only at the start of the attribute string or after a
 * separator, and the name charset is the spec's — anything but whitespace, `/`,
 * `>`, `=` and the quotes.
 */
const ATTR_PAIR = /(?:^|[\s/])([^\s/>="']+)(?:\s*=\s*("([^"]*)"|'([^']*)'|([^\s"'`=<>]+)))?/g;

function tagAttr(tag, want) {
  ATTR_PAIR.lastIndex = 0;
  let m;
  while ((m = ATTR_PAIR.exec(String(tag)))) {
    if (m[1].toLowerCase() === want) return m[3] ?? m[4] ?? m[5] ?? '';
  }
  return null;
}
