...
const { mapSections, readAttr } = require('../core/section-walk');
const { hasTopLevelTag, maskInert, readTopLevelH2Text } = require('../core/top-level-h2');
 ...
 *
 * A depth walk alone cannot replace it: a section whose inner HTML ends at
...
function applyToHtml(html) {
  ...
  const sections = collectSections((visit) => {
    mapSections(html, (openTag, cls, inner) => {
      visit(cls, hasTrack(inner) || Boolean(readAttr(openTag, TRACK_ATTR)), inner);
      return null;
    });
  });
  ...
  return mapSections(html, (openTag, cls, inner) => {
    ...
    const declared = readAttr(openTag, TRACK_ATTR);
    if (declared) {
      if (si >= 0) ti += 1;
