...
const {
  VOID_TAGS, scanTags, findTopLevelTag, hasTopLevelTag, readTopLevelH2Text,
 ...
 * "does it end with one?" is the whole question — and asking it that way is
 * immune to the two shapes a raw `inner.includes('class="tile-track"')` got
 * wrong: a commented-out track, and one quoted inside a card at depth > 0.
 * Both made the engine treat the slide as overridden while the DOM arm's
 * `:scope > ul.tile-track` did not, which withheld that slide's name from
 * every sibling's track on one render path only (HARD RULE #1).
 *
...
function hasTrack(inner) {
  return new RegExp(`<ul class="${TRACK_CLASS}"[^>]*>[\\s\\S]*<\\/ul>\\s*$`).test(inner);
}
...
function whollyBold(liInner) {
  const m = liInner.match(/^\s*(?:<p[^>]*>\s*)?<strong>([\s\S]*?)<\/strong>\s*(?:<\/p>\s*)?$/);
  return Boolean(m) && plain(m[1]) !== '';
}

/** The `<ul>` this section owns, as offsets into `inner`. */
function topLevelUlRange(inner) {
  const hit = findTopLevelTag(inner, 'ul');
  if (!hit) return null;
  let depth = 0;
  for (const t of scanTags(inner)) {
    ...
    if (t.isClose) {
      depth -= 1;
      if (depth === 0 && t.name === 'ul') return { from: hit.tagEnd, to: t.start };
      continue;
    }
...
function withOnClass(attrs) {
  return /\sclass\s*=\s*"/.test(attrs)
    ? attrs.replace(/(\s)class\s*=\s*"/, '$1class="on ')
    : `${attrs} class="on"`;
}
...
function markAuthored(inner) {
  const range = topLevelUlRange(inner);
  if (!range) return inner;
  const body = inner.slice(range.from, range.to);
  // `\sclass`, never `\bclass`: a word boundary also sits inside `data-class`,
  // which carries the RAW `_class:` payload rather than the resolved list (#1358).
  if (/<li\b[^>]*\sclass\s*=\s*"[^"]*\bon\b/.test(body)) return inner;

  const items = [...body.matchAll(LI_RE)];
  if (!items.length) return inner;
  // A nested list inside an item truncates the non-greedy `</li>` match, so that
  // item reads as partial content — it can never be wholly bold and never equal
  // the heading, so it is simply never chosen. A wrong PICK is impossible; the
  // worst case is the slide keeping no mark, which is what it had before.
  const heading = plain(readTopLevelH2Text(inner));
  let pick = items.find((m) => whollyBold(m[2]));
  if (!pick && heading) pick = items.find((m) => plain(m[2]) === heading);
  if (!pick) return inner;

  const at = range.from + pick.index;
  const open = `<li${withOnClass(pick[1])}>`;
  return inner.slice(0, at) + open + inner.slice(at + `<li${pick[1]}>`.length);
}
...
function applyToHtml(html) {
  ...
  return mapSections(html, (_openTag, cls, inner) => {
    ...
    if (hasTrack(inner)) return null;                 // already ours — idempotent
