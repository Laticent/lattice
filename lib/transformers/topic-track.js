...
/** Read one `class` attribute value, whatever the author quoted it with. */
const CLASS_ATTR = /\sclass\s*=\s*("([^"]*)"|'([^']*)'|([^\s"'>]+))/;
const classValue = (attrs) => {
 ...
 *
 * A TOKEN test, not `\bon\b` over the raw attribute string: a word boundary
 * also sits inside `on-hold` and `add-on`, so an unrelated class made the
 * string arm read the list as already marked and mark nothing, while the DOM
 * arm's `classList.contains('on')` marked the right item.
 */
function hasOnClass(attrs) {
  return Boolean(classValue(attrs)?.split(/\s+/).includes('on'));
}
...
function withOnClass(attrs) {
  const m = CLASS_ATTR.exec(attrs);
  if (!m) return `${attrs} class="on"`;
  const v = m[2] ?? m[3] ?? m[4] ?? '';
  const at = m.index + m[0].length - m[1].length;
  return `${attrs.slice(0, at)}"${v ? `${v} ` : ''}on"${attrs.slice(at + m[1].length)}`;
}
 ...
 * was strictly worse than having no marker rule at all, and the DOM arm (which
 * only ever reads `[...ul.children]`) lit the right one.
 *
