...
function hasList(inner) {
  // The SENTINEL first, and it is not redundant with the depth walk. A section
  // whose inner HTML ends at depth > 0 (author raw HTML with an unclosed tag)
  // hides the appended `<ul>` from the scan, so a second pass appended a second
  // track — measured, 3 tracks became 5. Idempotence is a contract this kernel
  // states; inferring it from depth balance is not the same as checking it.
  return inner.includes(`class="${TRACK_CLASS}"`) || hasTopLevelTag(inner, 'ul');
}
