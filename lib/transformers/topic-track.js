 ...
 * corner label. What saves the track is the DEGRADATION: a `topic` slide whose
 * track derives to nothing renders as the `fact` form — eyebrow, name, claim,
 * basis — which is a shipped, composed layout, not a hole. The failure mode is a
 * different good slide rather than a broken one.
 *
 * ── AUTHORED OVERRIDE ───────────────────────────────────────────────────────
 *
...
const { readTopLevelH2Text } = require('../core/top-level-h2');
...
function hasList(inner) {
  return inner.includes(`class="${TRACK_CLASS}"`) || /<ul[\s>]/.test(inner);
}
