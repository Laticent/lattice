...
describe('build-css', () => {
  ...
  // unowned: `npm run css:check` runs in CI's `unit` job right after the full
  // build (.github/workflows/ci.yml). That is the one place it is a real
  // question — after a build, "did main() write what bundle() computed, and did
  // a later step clobber it?" — and it is mutation-checked there: making main()
  // write `freshMin` to OUTPUT turns css:check red while this file stays green,
  // which is exactly the coverage the disk comparison was carrying and the reason
  // it is restored in CI rather than simply dropped. See engineering/gotchas.md.
  //
