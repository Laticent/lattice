...
describe('component-manifest', () => {
  ...
  describe('groupByBucket', () => {
    ...
    test('shipped manifests partition the 24 known bucket-divergent components correctly', () => {
      const ms = loadAll();
      const g = groupByBucket(ms);
      // chart = 14: funnel, gantt, journey, kanban, map, matrix-grid, piechart, progress, quadrant, radar, roadmap, state-chart, timeline-list, word-cloud
      assert.equal(g.chart.length, 14, 'chart bucket has 14 components');
      assert.deepEqual(
    ...
    test('the 24 bucket-divergent components keep their function field unchanged', () => {
      const ms = loadAll();
      ...
      assert.equal(byName['matrix-grid'].function, 'comparison',
        'matrix-grid keeps comparison function despite living in chart bucket');
  ...
  describe('vocabulary', () => {
    ...
    test('UNIVERSAL_GROUPS has the eleven documented categories', () => {
      assert.deepEqual(Object.keys(UNIVERSAL_GROUPS).sort(), [
        'chrome', 'claim', 'decoration', 'insight', 'mood', 'note', 'social', 'state', 'table',
        'tone', 'typography',
      ]);
    });
