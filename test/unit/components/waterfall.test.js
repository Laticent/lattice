...
describe('waterfall kernel', () => {
  ...
  describe('buildWaterfall — the emitted figure', () => {
    ...
    test('one scaled SVG with a viewBox and the family aspect ratio', () => {
      ...
      assert.doesNotMatch(svg, /<svg[^<>]*aria-hidden/, 'the chart root must stay in the accessibility tree');
      assert.match(svg, /<g aria-hidden="true">/, 'the marks must be hidden, or the desc is read twice');
