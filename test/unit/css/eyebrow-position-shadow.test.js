 ...
 *
 * A KNOWN LIMIT, STATED RATHER THAN DISCOVERED LATER. The walk below is line-based, so it
 * sees a code-only paragraph written the ordinary way and misses two shapes that would also
 * promote: one written with DOUBLE backticks, and one WRAPPED across two source lines.
 * Measured on the corpus at the time of writing, a parser-based walk finds 13 eyebrow spans
 * this one does not — none of which dispatches, so the "zero collisions" conclusion is
 * unaffected today. Replacing this walk with a markdown-it parse was built and measured, and
 * it is deliberately NOT in this change: it belongs with the corpus-wide census on its own
 * branch, where four review rounds of churn cannot hold up a register. Until then this is a
 * census with a stated blind spot, which is worth more than one that has an unstated one.
 */
