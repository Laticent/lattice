 ...
 * site playground wraps each engine selector under `div.lattice > section`. Its
 ...
 * `div.lattice > section :is(section.map, …)` — which can never match the map
...
describe('packTheme selector scoping', () => {
  test('a leading :is(section.X, figure.Y) distributes — the section arm targets the SLIDE', () => {
    ...
    assert.match(out, /div\.lattice > section\.map \.map-region--on/,
    ...
    assert.match(out, /div\.lattice > section figure\.chart-frame \.map-region--on/,
  ...
  test('every chart :is(section.<comp>, figure.chart-frame) pattern distributes', () => {
    for (const comp of ['map', 'gantt', 'radar', 'quadrant', 'funnel', 'piechart']) {
      ...
      assert.match(out, new RegExp(`div\\.lattice > section\\.${comp} \\.x`),
        `${comp}: the section arm must target the slide`);
    }
  });

  test('non-:is selectors are UNCHANGED (no regression)', () => {
    assert.equal(scope('section.title'), 'div.lattice > section.title');
    assert.equal(scope('section.map .map-region'), 'div.lattice > section.map .map-region');
    assert.equal(scope('.chart-frame .chart-body'), 'div.lattice > section .chart-frame .chart-body');
    assert.equal(scope('h2'), 'div.lattice > section h2');
    assert.equal(scope('section:has(.x)'), 'div.lattice > section:has(.x)');
    assert.equal(scope('section.map, section.radar'),
      'div.lattice > section.map, div.lattice > section.radar');
  });

  test(':root still becomes the (0,1,0)-preserving slide-root marker', () => {
    assert.match(scope(':root'), /div\.lattice > :where\(section\):not\(\[\\20 root\]\)/);
  });
});
