...
<section data-lattice-slide="2" id="2" class="content"><h2>Two</h2><ul><li>a</li></ul></section>
	...
	// RE-BLESSED — the Read·Article's diagram sizing moved out of `lattice.css` and into this
	// file. IN THIS GOLDEN exactly ONE thing moves: the string
	// `#lp-article .lp-figure svg[aria-roledescription]{width:auto;max-width:100%}` enters the
	// stylesheet. Nothing else — removing just that string from the assembled artifact
	// reproduces the previous sha byte for byte, and the fixture carries no diagram, no
	// mermaid CSS and `now: 0`, so neither the reverted kernel rule nor a timestamp is in it.
	// No markup, script, attribute or block-order change, so the CSP sha is untouched.
	// (Separately, on a five-family diagram deck the whole shipped artifact moves by two CSS
	// hunks — that rule in, the reverted `figure>svg[aria-roledescription]` arm out — plus the
	// embedded `generatedAt`. That deck is not this fixture.)
	...
	// The rule sits HERE rather than in `mermaid.css` because the Studio's article pane ships
...
/** One narrated slide as the Studio's bake hands it over: the narration text, the Cadenza track
 *  built from it, and one clip per cue (null for a cue with no audio). */
const { buildTrack } = require('@laticent/cadenza');
const { unpack, validateLtt } = require('@laticent/ltt');
const CLIP_HASH = `sha256:${'ab'.repeat(32)}`;
const VOICE = { model: 'm', voice: 'v', speed: 1 };
const said = (text, clips = []) => ({ text, track: buildTrack(text), clips });
const clip = (audio, extra = {}) => ({ audio, clip: CLIP_HASH, ...extra });
/** Two slides: the first with a clip on its first sentence only, the second (a divider) spoken. */
const NARRATION = {
	voice: VOICE,
	slides: [said('First line. Second line.', [clip('data:audio/mpeg;base64,AAAA'), null]), said('On the divider.', [clip('data:audio/mpeg;base64,BBBB')])],
};
/** The same delivery with its captions switch off. */
const NARRATION_NO_CAPTIONS = { ...NARRATION, captions: false };
/** Captions with no clips — the read-along a player would have to synthesize to hear. */
const NARRATION_NO_AUDIO = { voice: null, slides: NARRATION.slides.map((s) => ({ ...s, clips: [] })) };
...
/** The embedded LTT, unpacked, and each audio block's clip list, keyed by block index. */
function carriers(html) {
	const { JSDOM } = require('jsdom');
	const doc = new JSDOM(html).window.document;
	const lttBlock = doc.querySelector('script[type="application/lattice+ltt"]');
	const audio = Object.fromEntries([...doc.querySelectorAll('script[type="application/lattice+audio"]')].map((b) => [b.getAttribute('data-lp-audio'), JSON.parse(b.textContent)]));
	return { doc, lttBlock, packed: lttBlock && JSON.parse(lttBlock.textContent), ltt: lttBlock && unpack(JSON.parse(lttBlock.textContent)), audio };
}
...
test('narration: the deck ships ONE packed LTT that validates, and one clip block per slide with audio', async () => {
	...
	assert.deepEqual(ltt.segments.map((s) => [s.kind, s.at.slide, s.holdMs]), [
		['slide', 1, 0],
		['slide', 2, 2600],
	], 'one segment per slide: no hold on slide 1, and the divider waits the section hold on arrival');
	assert.equal(ltt.segments[0].track.cues.length, 2);
	assert.deepEqual(ltt.segments[0].audio.clips, [{ cue: 0, src: '#lp-audio/0/0', clip: CLIP_HASH }], 'one clip per CUE; the silent sentence has none');
	assert.deepEqual(audio, { 0: ['data:audio/mpeg;base64,AAAA'], 1: ['data:audio/mpeg;base64,BBBB'] }, 'each src names its place in the clip blocks');
	assert.ok(ltt.segments[0].tailMs > 0, 'the breath after the last sentence rides as tailMs');
	assert.match(ltt.segments[0].hash, /^sha256:[0-9a-f]{64}$/);
...
test('narration: the pre-LTT array shape fails loudly instead of shipping a mute deck', async () => {
	await assert.rejects(
		...
		/an array of cue lists is the old shape/,
	);
});

test('narration: a slide with no narration is a hold segment and gets no clip block', async () => {
	const { html } = await buildPlayerHtml({ docHtml: NARRATION_DOC, source: '# x', now: 0, narration: { voice: VOICE, slides: [null, said('Only this.', [clip('data:audio/mpeg;base64,CC')])] } });
	const { ltt, audio } = carriers(html);
	assert.deepEqual(ltt.segments.map((s) => s.kind), ['hold', 'slide']);
	assert.deepEqual(Object.keys(audio), ['1'], 'the surviving block keeps its ORIGINAL slide index');
	assert.equal(ltt.segments[1].audio.clips[0].src, '#lp-audio/1/0');
});
...
test('narration: hostile caption text cannot break out of its block', async () => {
	...
	// §Security). The LTT carries deck TEXT, so it buys the same property by emitting every `<`
	// as the JSON escape — the HTML parser never sees one, and JSON.parse gives the original
	// characters back.
	const nasty = '</script><script>alert(1)</script> and <!-- a comment --> & an ampersand.';
	const { html } = await buildPlayerHtml({ docHtml: NARRATION_DOC, source: '# x', now: 0, narration: { voice: VOICE, slides: [said(nasty, [clip('data:audio/mpeg;base64,AA')])] } });
	const { doc, lttBlock, ltt } = carriers(html);
	// Exactly four script elements: the one hashed player script, the LTT, the clip block, and
	// the manifest envelope. Critically, only ONE of them is executable — an injected
	// `<script>alert(1)</script>` would show up as a second type-less script.
	const scripts = [...doc.querySelectorAll('script')];
	assert.equal(scripts.length, 4, 'no injected extra script element');
	...
	assert.equal(ltt.segments[0].track.cues.map((c) => c.display).join(' '), buildTrack(nasty).cues.map((c) => c.display).join(' '), 'the caption survives verbatim — including its ampersand');
	assert.doesNotMatch(lttBlock.textContent, /</, 'not one raw `<` inside the block');
});

test('narration: each clip src names its own place in the block, whatever cue it speaks', async () => {
	// Cue 0 silent, cues 1 and 2 voiced: the block holds two clips, and each src names its entry.
	const { html } = await buildPlayerHtml({
		docHtml: NARRATION_DOC,
		source: '# x',
		now: 0,
		narration: { voice: VOICE, slides: [said('A. B. C.', [null, clip('data:audio/mpeg;base64,BB'), clip('data:audio/mpeg;base64,CC')])] },
	});
	const { ltt, audio } = carriers(html);
	assert.deepEqual(ltt.segments[0].audio.clips.map((c) => [c.cue, c.src]), [
		[1, '#lp-audio/0/0'],
		[2, '#lp-audio/0/1'],
	]);
	assert.deepEqual(audio[0], ['data:audio/mpeg;base64,BB', 'data:audio/mpeg;base64,CC']);
	// And the player resolves a clip BY that src, not by its position in the list.
	assert.match(html, /function clipUri\(src,i,uris\)/);
});

test('narration: audio the LTT cannot place does not ship', async () => {
	// A bake one slide longer than the rendered deck, and a clip past its track's last cue: both
	// used to ship their bytes (and a media-src grant) with nothing to play them.
	const { html } = await buildPlayerHtml({
		docHtml: NARRATION_DOC,
		source: '# x',
		now: 0,
		narration: {
			voice: VOICE,
			slides: [
				said('Only one sentence.', [null, clip('data:audio/mpeg;base64,ORPHANCUE')]),
				said('Two.', []),
				said('A third slide the deck does not render.', [clip('data:audio/mpeg;base64,ORPHANSLIDE')]),
			],
		},
	});
	assert.doesNotMatch(html, /ORPHAN/);
	assert.doesNotMatch(html, /media-src/);
	assert.equal(carriers(html).ltt.segments.length, 2);
});
...
test('narration: only a data: URI is allowed through — the file stays network-free', async () => {
	...
	const { html } = await buildPlayerHtml({
		...
		source: '# x',
		now: 0,
		narration: { voice: VOICE, slides: [said('A. B. C.', [clip('https://example.com/a.mp3'), clip('blob:http://x/y'), clip('data:audio/mpeg;base64,AA')])] },
	});
	const { ltt, audio } = carriers(html);
	assert.deepEqual(audio, { 0: ['data:audio/mpeg;base64,AA'] });
	assert.deepEqual(ltt.segments[0].audio.clips.map((c) => c.cue), [2], 'the two refused clips play as silent cues');
	assert.doesNotMatch(html, /example\.com|blob:/);
});
...
test('narration: audio with captions OFF ships the voice and no band at all', async () => {
	...
	// The player script still LOOKS for a band — that one runtime reference is exactly what makes
	// the crawl no-op here. What must be absent is the element and its stylesheet.
	...
	// The LTT kernels ship anyway. The transport times every cue, breath and hold through
	// positionAt, so an audio-only export needs them as much as one with a band: the alternative
	// was a second, hand-written copy of the timing rules for this one case, which is exactly the
	// copy that drifts (the step-2 inversion pass).
	assert.match(html, /var unpackTrack=/);
	assert.match(html, /var makeCursor=/);
	assert.match(html, /var positionAt=/);
});
...
test('narration: captions with audio OFF ship a read-along that costs kilobytes', async () => {
	...
	const { html } = await narratedPlayer({ narration: NARRATION_NO_AUDIO });
	assert.match(html, /id="lp-caption"/, 'the band ships');
	assert.match(html, /var makeCursor=/, 'and the cursor that drives it');
	assert.match(html, /var positionAt=/, 'and positionAt, which reads it');
	...
	assert.doesNotMatch(html, /application\/lattice\+audio/, 'nor an empty clip block');
});
...
test('narration: an encoded clip carries its lead, and the player seeks past it', async () => {
	...
	assert.match(html, /au\.currentTime\*1000-cueLead/, 'and its crawl clock subtracts the silent head it skipped');

	// A clip that arrived already compressed carries none, and must not pay for a key.
	assert.equal('leadMs' in carriers((await narratedPlayer()).html).ltt.segments[0].audio.clips[0], false, 'no lead key when there is no encoder silence');
});

test('narration: the deck’s own pace is baked into the holds, because the player cannot read front matter', async () => {
	...
	// has no workspace preset to fall back on. The number has to be resolved here, and it rides
	// in the LTT as the arriving slide's holdMs (slide 2 of the fixture is a divider).
	const holdOf = async (source) => carriers((await narratedPlayer({ source })).html).ltt;
	const deliberate = await holdOf('---\npace: deliberate\n---\n\n# One\n');
	assert.equal(deliberate.segments[1].holdMs, 4000, 'the author’s declared rhythm travels');
	assert.equal(deliberate.inputs.deckPace, 'deliberate');
	const plain = await holdOf('---\ntheme: indaco\n---\n\n# One\n');
	assert.equal(plain.segments[1].holdMs, 2600, 'and an undeclared deck gets `natural`');
	assert.equal('deckPace' in plain.inputs, false, 'with no deckPace claimed for it');
	const typo = await holdOf('---\npace: delibrate\n---\n\n# One\n');
	assert.equal(typo.segments[1].holdMs, 2600, 'a typo falls through to the default, as the register documents');
});
...
test('narration: the manifest carries the read-along track, and NOT the audio bytes', async () => {
	...
	assert.deepEqual(manifest.readAlong, {
		...
		// It points at the deck's timing track, which rides beside the envelope, not in it.
		timing: { format: 'ltt', version: '1.0', encoding: 'packed', mime: 'application/lattice+ltt' },
	});
