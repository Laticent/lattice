...
/**
 * THE STAGE CELL's tag — `<div>` normally, `<figure>` around a captioned graphic
 * (semantic-HTML ADR §18.3). One function owns BOTH halves of the pair: the string
 * kernel's `buildStageCell` and the DOM mirror in `masthead-lift.js`. They must agree on
 * every input, or the same deck gets a different element on the engine and the web
 * preview — a HARD RULE #1 split that renders identically and so hides from every pixel
 * gate. An earlier split version (caption retagged in `chart-family`, tag decided here)
 * disagreed on four real inputs; those four are pinned below.
 */
describe('masthead-lift — the stage cell (figure vs div), engine == web', () => {
  const CAPTIONED =
    '<h2>Rollout</h2>' +
    '<div class="chart-body"><svg role="img"></svg></div>' +
    '<p class="chart-caption">Source: Linear.</p>';
  const UNCAPTIONED = '<h2>Rollout</h2><div class="chart-body"><svg role="img"></svg></div>';

  /** Both paths, reduced to the two facts that must match: the tag and the figure's name. */
  function both(inner, cls = 'roadmap chart-frame form') {
    const engineOut = kernel.transformMastheadSection(inner, cls);
    const engine = {
      tag: /<figure class="cell-stage"/.test(engineOut) ? 'figure' : 'div',
      label: (engineOut.match(/<figure class="cell-stage" aria-label="([^"]*)"/) || [])[1] || '',
      out: engineOut,
    };
    const doc = dom(`<section class="${cls}">${inner}</section>`);
    adapter.applyToDom(doc);
    const cell = doc.querySelector('section > .cell-stage');
    const web = {
      tag: cell ? cell.tagName.toLowerCase() : '(none)',
  ...
  test('a captioned graphic makes the stage a NAMED <figure> on both paths', () => {
    const { engine, web } = both(CAPTIONED);
    assert.equal(engine.tag, 'figure');
    assert.equal(web.tag, 'figure');
    // NAMED, not bare. Chrome maps `<figure>` to a PDF `/Figure` whose `/Alt` comes from
    // the accessible name; a nameless one is a PDF/UA failure and readers treat `/Figure`
    // as atomic, so it can swallow the chart's own named figure inside it.
    assert.equal(engine.label, 'Source: Linear.');
    assert.equal(web.label, 'Source: Linear.');
    assert.match(engine.out, /<figcaption class="chart-caption">Source: Linear\.<\/figcaption>/);
    assert.ok(web.doc.querySelector('figure.cell-stage > figcaption.chart-caption'));
  });

  test('an UNcaptioned graphic keeps a <div> stage and a <p> nowhere', () => {
    // Restraint: a <figure> with nothing to associate is an announced boundary carrying
    // no information (ADR §4A).
    const { engine, web } = both(UNCAPTIONED);
    assert.equal(engine.tag, 'div');
    assert.equal(web.tag, 'div');
    assert.doesNotMatch(engine.out, /figcaption/);
    assert.equal(web.doc.querySelector('figcaption'), null);
  });

  test('a caption in the MIDDLE does not qualify — the content model forbids it', () => {
    // `<figure>` is *figcaption then flow*, or *flow then figcaption* — never in between.
    // This is the `state-chart` shape ({chart-body, caption, state-legend}); it keeps a
    // <div> stage and a <p> caption. The gallery shipped the invalid form before this.
    const midway =
      '<h2>States</h2><div class="chart-body">x</div>' +
      '<p class="chart-caption">Cap</p><ol class="state-legend"><li>a</li></ol>';
    const { engine, web } = both(midway, 'state-chart chart-frame form');
    assert.equal(engine.tag, 'div');
    assert.equal(web.tag, 'div');
    assert.match(engine.out, /<p class="chart-caption">Cap<\/p>/, 'stays a paragraph');
    assert.equal(web.doc.querySelector('figcaption'), null);
  });

  test('a LEADING caption qualifies — that ordering is legal', () => {
    const leading = '<h2>Rollout</h2><p class="chart-caption">Cap</p><div class="chart-body">x</div>';
    const { engine, web } = both(leading);
    assert.equal(engine.tag, 'figure');
    assert.equal(web.tag, 'figure');
  });

  test('the two paths agree on the inputs that used to split them', () => {
    // Every row here diverged when the string kernel matched `/\bclass="chart-caption"/`
    // and the DOM mirror used `classList.contains`.
    const bodies = {
      'extra class token': '<div class="chart-body">x</div><p class="chart-caption note">Cap</p>',
      'leading class token': '<div class="chart-body">x</div><p class="note chart-caption">Cap</p>',
      'single-quoted class': "<div class='chart-body'>x</div><p class='chart-caption'>Cap</p>",
      'preceding comment': '<!-- <div> --><div class="chart-body">x</div><p class="chart-caption">Cap</p>',
    };
    for (const [name, body] of Object.entries(bodies)) {
      const { engine, web } = both(`<h2>T</h2>${body}`);
      assert.equal(engine.tag, web.tag, `${name}: engine=${engine.tag} web=${web.tag}`);
      assert.equal(engine.label, web.label, `${name}: label mismatch`);
      assert.equal(engine.tag, 'figure', `${name}: should qualify`);
    }
  });
  ...
  test('a caption nested inside content never promotes the stage', () => {
    ...
    const { engine, web } = both(nested);
    assert.equal(engine.tag, 'div');
    assert.equal(web.tag, 'div');
  });

  test('the retag survives a trailing running <footer>', () => {
    // The footer is peeled into `.cell-footer` BEFORE the cell is built, so it must not
    // count as the body's last child and defeat the last-child caption test.
    const { engine } = both(`${CAPTIONED}<footer>src</footer>`);
    assert.equal(engine.tag, 'figure');
    assert.match(engine.out, /<\/figure><div class="cell-footer"><footer>src<\/footer>/);
  });
});
