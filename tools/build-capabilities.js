...
const SCRIPT_META = {
  ...
  'equiv':                    ['Test & verify', 'Slice/deck equivalence sweep — for every slide of every committed deck, does rendering it ALONE match rendering it inside the deck? The headless half of the preview-fidelity diagnostic; the author-facing half is the Studio\'s "Preview fidelity" overlay. NOT a regression gate: it never executes the shipped repair (that lives in docs/src/lib/single-slide-render.ts), and ~81 of its 91.9 points are differences it neutralizes. It prints its neutralizer set, prelude count and skipped decks every run so the number stays legible. On-demand.'],
  ...
  'equiv:check':              ['Test & verify', 'Re-run `equiv` and compare vs the committed baseline: decks/slides/preludes must match exactly, and the rate may drift 1.5 points. Cannot produce a true alarm at its own resolution (1.5 points is 18 of 1201 slides), which is why it is on-demand rather than a blocking CI gate.'],
