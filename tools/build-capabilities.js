...
const SCRIPT_META = {
  ...
  'check:families':           ['Lint & audit', 'Adaptive-FAMILY agreement gate: renders one deck per registered `@size` and checks the CSS\'s own verdict (`--lat-family`, stamped on `.cell-stage`) against `familyFor()` on the deck geometry. The two classify DIFFERENT boxes — JS the deck, a container query the container\'s CONTENT box, which the section\'s asymmetric padding makes proportionally wider — so they need different boundary numbers for the same bands (BOUNDARIES vs CSS_BOUNDARIES in lib/adaptive/families.js). Conflating them made every `aspect-ratio <= 1.05` rule INERT on square decks across 18 files, silently (#1218). Fails on a new `@size`, a section-padding change, or anyone tidying one list into the other. `--report` prints the per-size table; skips loudly with no Chromium.'],
