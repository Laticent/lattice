...
const SCRIPT_META = {
  ...
  'cadenza-lib:build':        ['Build & bundle', 'Build the Cadenza library dist/ (ESM + CJS + .d.ts, esbuild + tsc) so import/require(\'@slidewright/cadenza\') resolves — the workspace package that retires the caption hand-mirrors.'],
  ...
  'lente-lib:build':          ['Build & bundle', 'Build the Lente library dist/ (ESM + CJS + .d.ts, esbuild + tsc) so import/require(\'@slidewright/lente\') and npm publish resolve — the fourth spin-off sibling\'s consumable artifact.'],
  ...
  'intent:tune':              ['Test & verify', 'Coordinate-descend the recommender\'s facet weights against the DEV half of the fit benchmark, over cached feature vectors so the search is free. DEV ONLY — the chosen weights go into docs/src/lib/fit-search.ts and the TEST split is reported once, afterwards, by `intent:fit`. Its verdict on the facet layer was to zero `whenToUse`, `antiPatterns` and the function cues outright; that negative result is documented in fit-search.ts\'s header.'],
  'intent:judge':             ['Test & verify', 'Measure whether an LLM judges component fit better than BM25, on the SAME held-out fit benchmark (`intent:fit`) so the answer is comparable to everything already measured. Shape under test is retrieve-then-judge: BM25 narrows 61 components to K, the model re-ranks only those while reading their authored `whenToUse`/`antiPatterns`/`capacity` — so recall@K is the judge\'s ceiling and is reported first. SPENDS OUR OpenRouter key (HARD RULE #24): opt-in via OPENROUTER_ALLOW_SPEND=1, prints the planned spend and exits without it, `--limit 1` to validate, token usage printed every run. Exists because the DETERMINISTIC facet scorer was refuted on this benchmark (docs/src/lib/fit-search.ts) — the authored notes may be the right prompt context even though they are useless as term-frequency features.'],
