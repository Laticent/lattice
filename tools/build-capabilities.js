...
const SCRIPT_META = {
  ...
  'check:family-tiers':       ['Lint & audit', 'Adaptive-family BEHAVIOR gate: renders one deck per family (wide/square/tall/strip) through the emulator and asserts each component actually reflows the way it intends — reading the COMPUTED style of a property only that family\'s rule can produce, plus the `data-family` stamp. Asserting behavior rather than classifier agreement is what catches a whole tier being INERT (#1218), alongside a lost stamp or a bad selector. Skips loudly with no Chromium.'],
  'check:family-conformance': ['Lint & audit', 'Does every family-reflowing component\'s `[data-family]` tier actually FIRE, per (component x @size)? The tier probe above asserts that for three hand-picked components; this derives it for all 33 by removing the `data-family` stamp from that same render and re-reading the properties each rule declares — same element, same viewport, rule on vs rule off. Verdicts: fires / no-effect / no-baseline / inert (the #1218 defect) / unexercised. Frozen in test/oracle/family-conformance.json, compared exactly both ways. On-demand diagnostic, not a CI gate.'],
  ...
  'contrast:palette-native':  ['Lint & audit', 'The REFEREE for `contrast:palette` — the fast sweep re-themes ONE render in place (which is what makes a 32-palette matrix affordable per-PR, on a textual assumption about the export shell); this re-renders all 32 FOR REAL, same `runEmulator` path as the integration tier. It answers what the sweep cannot ask about itself: does the sweep still AGREE, and what about the runs it structurally DROPS (Mermaid bakes its stylesheet into the SVG, so no swap can move it). ~11-13s per palette vs ~70s for the whole sweep: nightly, auditing the per-PR gate.'],
  'contrast:player':          ['Lint & audit','Nightly WCAG sweep of the REAL exported `--player`, in BOTH scheme states, with every text run\'s backdrop sampled from PIXELS — a screenshot taken with the glyphs made transparent, so a gradient or a photo resolves. Reuses `tools/check-slide-contrast.js`\'s PROBE, so the two cannot disagree about a ratio, and reaches what the static tools cannot: anything the export PIPELINE does to correct CSS reads as a PASS to them (#1645, #1642). Fails only on a finding new or worse by >0.05 vs test/oracle/player-contrast.json; known sub-AA runs are #1745. Nightly, never per-PR.'],
  ...
  'intent:fit':               ['Test & verify', 'The FIT benchmark for the component recommender — the manifest grading itself. Every authored `whenToUse` (190) becomes a described task whose component should rank first, and every `antiPattern` that backticks a better component becomes a REDIRECT case where the warned-against one must stay behind (the project\'s first precision test). Leave-one-out per case, since those notes are both the evidence and the questions. Reports `selection` separately from `authoring`, because only the first is answerable. On-demand.'],
  ...
  'intent:pick-agents':       ['Test & verify', 'RE-RUN the pick-surface bake-off with real agents, so its numbers stop being a hand transcription (#1734). Spawns N Opus agents per condition, each sandboxed in a temp dir holding ONLY its surface (components.pick.md vs components.json), and scores strict and defensible against the committed ground truth. Distinct from `intent:pick-eval`, which asks a LOCAL lexical ranker how much signal a pick row carries and spawns nothing. SPENDS Claude tokens: opt-in via LATTICE_ALLOW_AGENT_SPEND=1, `--dry-run` prices it for free.'],
  ...
  'intent:judge':             ['Test & verify', 'Measure whether an LLM judges component fit better than BM25, on the SAME held-out benchmark as `intent:fit` so the answer is comparable to everything already measured. Retrieve-then-judge: BM25 narrows 61 components to K and the model re-ranks only those while reading their authored notes, so recall@K is the judge\'s ceiling and is reported first. SPENDS OUR OpenRouter key (HARD RULE #24): opt-in via OPENROUTER_ALLOW_SPEND=1, `--limit 1` to validate, usage printed every run. Exists because the deterministic facet scorer was refuted on this benchmark.'],
  'intent:bakeoff':           ['Test & verify', 'Bake-off for the component intent ranker (docs/src/lib/intent-search.ts, HARD RULE #1 shared search core): four query corpora — headings and prose harvested from every committed deck via its `_class` directive, plus authored intent and adversarial phrasings — scored top1/top3/top5/MRR against the shipped substring+Fuse baseline. It BUNDLES the shipped module with esbuild rather than copying it, so it cannot drift from what users get. `-- --dev` shows the tuning split and its misses. On-demand, not a CI gate.'],
  'equiv':                    ['Test & verify', 'Slice/deck equivalence sweep — for every slide of every committed deck, does rendering it ALONE match rendering it inside the deck? The headless half of the preview-fidelity diagnostic; the Studio\'s "Preview fidelity" overlay is the author-facing half. It RUNS THE SHIPPED REPAIR — each slice is handed the deck position `supplyablePosition` would give it — so breaking that path collapses the rate instead of moving it 0.0 points. Prints its supplied-position, prelude, neutralizer and skipped-deck counts every run. On-demand, not a CI gate.'],
  ...
  'equiv:check':              ['Test & verify', 'Re-run `equiv` — the slice-vs-deck render equivalence sweep — and compare vs the committed baseline: decks/slides/preludes/positions must match exactly, and the rate may drift 1.5 points. Mutation-verified able to fail, re-derived after each re-bless (2026-08-25, at the 96.6% baseline): stubbing `positionIsTrustworthy` to false takes it to 9.9%, `deckSectionFor` to undefined takes it to 74.0%. NOTE `positions` no longer equals `slides` — 1453 of 1461, all 8 from `slide-class-forms.md`, measured rather than skipped.'],
...
// already removed from the generated artifacts. Same reasoning, and the same
// shape, as `ROW_CAP` in tools/build-decisions-index.js.
//
// TWO THINGS ARE DELIBERATE AND WILL LOOK WRONG OTHERWISE.
//
// It is a TRIM, not a ratchet. build-decisions-index.js pinned its cap at the
// widest row the live corpus had, so nothing was forced to change on the day it
// landed. This one is set BELOW the tail on purpose: eleven rows had grown into
// essays, and `grep -i intent` cost 158 tokens a row against a 27-token median
// because it happened to land on four of them. Every one of those rows is backed
...
function rowCostProblems(rows) {
  ...
  for (const row of rows) {
    ...
    problems.push(
      `${name}: its catalog row is ${row.length} characters, over the ${ROW_CAP} cap. ` +
        'A row says WHAT it does, WHEN to reach for it, and the one gotcha that stops ' +
        'you misusing it. Move the mechanism, the history and the operating detail into ' +
        "the tool's own header — that is the file a reader opens next, and it has no cap.",
    );
  }
  return problems;
}
