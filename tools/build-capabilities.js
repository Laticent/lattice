...
const SCRIPT_META = {
  ...
  'intent:pick-agents':       ['Test & verify', 'RE-RUN the pick-surface bake-off with real agents, so its numbers stop being a hand transcription (#1734). Spawns N Opus agents per condition, each in a temp dir holding ONLY its surface (components.pick.md vs components.json). The pin is the sandbox plus the DELIBERATE ABSENCE of a `Read` grant: `--allowed-tools Read` is a whole-tool allow rule that overrides the working-directory refusal, and measured on the real CLI an agent given it read a repo file outside its sandbox with `permission_denials` EMPTY. Without the grant, containment still allows the in-sandbox read and refuses the rest, recording the attempted path. Writes every agent\'s RAW return plus the harness\'s own tool-call/usage accounting, then scores strict (`expect`) and defensible (`expect ∪ ok`) against the committed ground truth, reporting strict-only misses separately so a defensible-but-wrong pick cannot hide. Distinct from `intent:pick-eval`, which asks a LOCAL LEXICAL RANKER how much signal a pick row carries and never spawns anything. SPENDS Claude tokens (not our OpenRouter key, so HARD RULE #24\'s gate has nothing to catch — and it stays out of test/** regardless): opt-in via LATTICE_ALLOW_AGENT_SPEND=1, `--dry-run` prints the prompt, tool policy and planned spend for free. Quote `model_tokens` (from modelUsage), not `context_tokens` (from usage, which re-counts the cached prefix every turn); neither compares to the Agent-tool figures in pick-surface-agent-runs.json.'],
