...
const SCRIPT_META = {
  ...
  'check:nightly-liveness':   ['Lint & audit', 'Does the nightly family still RUN? Probes every daily scheduled workflow for a run whose event is `schedule` AND which actually produced jobs (a parked or startup-failed run is a record of nothing), against a 48h threshold sized from the measured 34.7h maximum gap; plus a BACKLOG.md freshness arm, because sync-backlog reported success on all 24 runs across the window the mirror went 39% stale. One job, one rolling issue — the workflows measured all slipped on the SAME night, so a per-workflow alarm turns one GitHub-side slip into eight issues. Evaluation is pure + exported (unit-tested with no network); it cannot watch itself, which is stated in UNWATCHABLE_WORKFLOWS. Run by .github/workflows/nightly-liveness.yml.'],
