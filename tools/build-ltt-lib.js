...
const LIB_DIR = path.join(ROOT, 'docs', 'src', 'lib', 'ltt');
...
const CJS_BANNER =
  '/* Auto-generated by tools/build-ltt-lib.js — DO NOT EDIT.\n' +
  '   Source: docs/src/lib/ltt/*.ts. Rebuild: npm run ltt-lib:build */';
...
// `import '@laticent/ltt'` resolve for a plain Node-ESM / non-TS-bundler consumer —
...
async function main() {
  if (check) {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ltt-lib-'));
    try {
      await buildInto(tmp);
      const drift = diffTrees(readTree(tmp), readTree(DIST_DIR));
      if (drift.length) {
        console.error(
          '[build-ltt-lib] STALE — run `npm run ltt-lib:build` and commit docs/src/lib/ltt/dist:\n  ' +
            drift.join('\n  '),
        );
        process.exit(1);
      }
      if (!silent) console.log('[build-ltt-lib] up to date.');
    } finally {
  ...
  // the library builders in the background, so a build overlapping a test run is
  // enough to hit it.
  //
  ...
  if (!silent) {
    ...
    console.log(`[build-ltt-lib] wrote ${path.relative(ROOT, DIST_DIR)}/ (${files.join(', ')})`);
  }
}

main().catch((e) => {
  console.error('[build-ltt-lib] failed:', e.message);
  process.exit(1);
});
