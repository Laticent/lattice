...
function overflowOracle() {
  ...
  if (BLESS) {
    ...
    const note = [
      ...
      'the record has a stable baseline. Most of that set paginates in a real export; at portrait, 21',
      'components clip un-split and 5 with splitting on. The hd/4K rows are DIFFERENT: the split move does',
      'not run at a landscape @size (16:9 is the box a deck is authored in), so a clip recorded there is',
      'the real terminal a reader sees, and it is an authoring defect to fix rather than a baseline.',
