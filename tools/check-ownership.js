...
// ── Lint exclusions: a reason per entry, and no stale entries (#1223) ─────────
//
// THE RULE: nothing but vendor code, generated code, or source a standard parser
// cannot read may be excluded from lint. `biome.jsonc` states which class each
// exclusion belongs to, in prose, on the line above it. This is the check that keeps
// those statements true — the same shape as SANCTIONED_MARGINS /
// SANCTIONED_PREVIEW_BUILDERS / SANCTIONED_LAYER_BLOCKS (HARD RULES #20, #22, #26).
//
// Two failures it catches, both of which had already happened:
//   1. AN UNREASONED ENTRY. All 29 exclusions once carried zero justification, so
//      nothing distinguished "generated, generator at tools/build-playground.js" from
//      "we gave up on this one" — and `docs/src/components/ui`, 31 files of house
//      source, sat among them unremarked while `panel.tsx` grew by 222 lines without
//      ever passing through `npm run lint`.
//   2. A STALE ENTRY. `examples/**/*.json` matched NOTHING and survived indefinitely,
//      because an exclusion that covers no files is silent by construction.
//
// `dependency`, `build-output` and `transient` are exempt from the staleness half:
// they are legitimately absent from a clean checkout. Every other class asserts that
// real files are being skipped, so it must still be skipping some.
// The word classes take a `\b`; ``generated by `…` `` and `unparseable:` cannot — a
// backtick and a colon are non-word characters, so a trailing `\b` there can never match.
const LINT_EXCLUSION_CLASSES = /^(?:(dependency|build-output|transient|vendor|not-code)\b|generated by `([^`]+)`|unparseable:)/;
const STALENESS_EXEMPT = /^(dependency|build-output|transient)\b/;

function checkLintExclusions(errors) {
  const rel = 'biome.jsonc';
  const file = path.join(ROOT, rel);
  if (!fs.existsSync(file)) {
    errors.push(`${rel} is missing — the lint config must be .jsonc so every exclusion can carry its reason (#1223).`);
    return;
  }
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!reason) {
      errors.push(
        ...
        `above it — one of: dependency, build-output, transient, vendor, not-code, ` +
        "generated by `<path>`, unparseable: <why> (#1223).",
      );
      continue;
    }
    const cls = reason.match(LINT_EXCLUSION_CLASSES);
    if (!cls) {
      errors.push(
        `biome.jsonc excludes "${pattern}" with an unrecognized class: "${reason.slice(0, 60)}…". ` +
        "Use one of: dependency, build-output, transient, vendor, not-code, generated by `<path>`, " +
        'unparseable: <why> (#1223).',
      );
      continue;
    }
    // A named generator must exist, or "generated" is a claim about nothing.
    const generator = cls[2];
    if (generator && !fs.existsSync(path.join(ROOT, generator))) {
      errors.push(
        `biome.jsonc says "${pattern}" is generated by \`${generator}\`, which does not exist. ` +
        `Name the real generator, or the exclusion is no longer justified (#1223).`,
      );
    }
    if (STALENESS_EXEMPT.test(reason)) continue;
    // Everything else claims to be skipping real files — so it had better be.
    const matches = tracked.some((f) => matchesLintPattern(f, pattern));
    if (!matches) {
      errors.push(
        `biome.jsonc excludes "${pattern}", which matches ZERO tracked files — a stale exclusion. ` +
        `Delete it (this is exactly how \`examples/**/*.json\` survived pointing at nothing, #1223).`,
      );
    }
  }
}

/** Minimal glob match for the shapes biome.jsonc actually uses: a literal path, a
 *  directory prefix, and `**` / `*` segments. Deliberately not a glob library — a new
 *  dependency for six patterns would fail HARD RULE #15's own test. */
function matchesLintPattern(file, pattern) {
